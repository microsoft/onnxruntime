...
def make_add_f32():
    ...
    graph = helper.make_graph([node], "add_graph", [input_a, input_b], [output_c])
...
def make_matmul_dynamic():
    ...
    input_a = helper.make_tensor_value_info("A", TensorProto.FLOAT, ["batch", 4])
    input_b = helper.make_tensor_value_info("B", TensorProto.FLOAT, [4, 2])
    output_c = helper.make_tensor_value_info("C", TensorProto.FLOAT, ["batch", 2])

    node = helper.make_node("MatMul", inputs=["A", "B"], outputs=["C"])
    graph = helper.make_graph([node], "matmul_graph", [input_a, input_b], [output_c])
    model = helper.make_model(
        graph,
        opset_imports=[helper.make_opsetid("", 13)],
    )
...
def make_kvconcat():
    ...
    """
    past = helper.make_tensor_value_info("past", TensorProto.FLOAT, [1, 2, "seq", 4])
    new_val = helper.make_tensor_value_info("new_val", TensorProto.FLOAT, [1, 2, 1, 4])
    out = helper.make_tensor_value_info("out", TensorProto.FLOAT, [1, 2, "seq_plus_1", 4])

    node = helper.make_node("Concat", inputs=["past", "new_val"], outputs=["out"], axis=2)
