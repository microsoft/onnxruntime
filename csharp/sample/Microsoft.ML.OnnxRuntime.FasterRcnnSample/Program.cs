...
class Program
{
    public static void Main(string[] args)
    {
        ...
        for (int i = 0; i < boxes.Length - 4; i += 4)
        {
            ...
            {
                predictions.Add(new Prediction { Box = new Box(boxes[i], boxes[i + 1], boxes[i + 2], boxes[i + 3]),
            ...
            image.Mutate(x =>
                         {
                             x.DrawLines(Color.Red, 2f,
                                         new PointF[] {
                                             ...
                                             new PointF(p.Box.Xmin, p.Box.Ymax), new PointF(p.Box.Xmin, p.Box.Ymin)
                                         });
                             x.DrawText($"{p.Label}, {p.Confidence:0.00}", font, Color.White,
