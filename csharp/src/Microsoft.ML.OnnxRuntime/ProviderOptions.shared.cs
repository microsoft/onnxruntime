...
#region Public Methods
    ...
    public string GetOptions()
    {
        ...
        NativeApiStatus.VerifySuccess(
            NativeMethods.OrtGetTensorRTProviderOptionsAsString(handle, allocator.Pointer, out providerOptions));
    ...
    /// <summary>
    /// Updates  the configuration knobs of OrtTensorRTProviderOptions that will eventually be used to configure a
    /// TensorRT EP Please refer to the following on different key/value pairs to configure a TensorRT EP and their
    /// meaning: https://onnxruntime.ai/docs/execution-providers/TensorRT-ExecutionProvider.html
    /// </summary>
    ...
    public void UpdateOptions(Dictionary<string, string> providerOptions)
    {
        using (var cleanupList = new DisposableList<IDisposable>())
        {
            ...
            NativeApiStatus.VerifySuccess(NativeMethods.OrtUpdateTensorRTProviderOptions(
                handle, keysArray, valuesArray, (UIntPtr)providerOptions.Count));
...
#region Public Methods
    ...
    public string GetOptions()
    {
        ...
        IntPtr providerOptions = IntPtr.Zero;
        NativeApiStatus.VerifySuccess(
            NativeMethods.OrtGetCUDAProviderOptionsAsString(handle, allocator.Pointer, out providerOptions));
        using (var ortAllocation = new OrtMemoryAllocation(allocator, providerOptions, 0))
        {
            return NativeOnnxValueHelper.StringFromNativeUtf8(providerOptions);
        }
    ...
    public void UpdateOptions(Dictionary<string, string> providerOptions)
    {
        using (var cleanupList = new DisposableList<IDisposable>())
        {
            ...
            NativeApiStatus.VerifySuccess(NativeMethods.OrtUpdateCUDAProviderOptions(handle, keysArray, valuesArray,
        ...
        foreach (var p in paris)
        {
            ...
            if (keyValue.Length != 2)
            {
                throw new ArgumentException(
                    "Make sure input string contains key-value paris, e.g. key1=value1;key2=value2...", "s");
            }
...
/// </summary>
/// <see
/// cref="https://github.com/microsoft/onnxruntime/blob/main/include/onnxruntime/core/providers/coreml/coreml_provider_factory.h"/>
[Flags]
...
/// </summary>
/// <see
/// cref="https://github.com/microsoft/onnxruntime/blob/main/include/onnxruntime/core/providers/nnapi/nnapi_provider_factory.h"/>
[Flags]
