...
public interface IDisposableReadOnlyCollection<T> : IReadOnlyCollection<T>,
                                                    IDisposable
{}
...
public class DisposableNamedOnnxValue : NamedOnnxValue, IDisposable
{
    ...
    /// other things that would need to be disposed by this instance depending on how IOrtValueOwner is
    /// implemented.</param>
    private DisposableNamedOnnxValue(string name, Object value, TensorElementType elementType,
                                     IOrtValueOwner ortValueHolder)
    ...
    private static DisposableNamedOnnxValue FromNativeSequence(string name, OrtValue ortValueSequence,
        ...
        try
        {
            for (int i = 0; i < count.ToInt32(); i++)
            {
                ...
                NativeApiStatus.VerifySuccess(
                    NativeMethods.OrtGetValue(ortValueSequence.Handle, i, allocator.Pointer, out nativeOnnxValueSeq));
            ...
            // is also disposed.
            var nativeCollectionManager =
                new NativeOrtValueCollectionOwner<DisposableNamedOnnxValue>(ortValueSequence, sequence);
            result =
                new DisposableNamedOnnxValue(name, sequence, OnnxValueType.ONNX_TYPE_SEQUENCE, nativeCollectionManager);
        }
        catch (Exception)
        {
            sequence.Dispose();
    ...
    /// This function does not take ownership of the map as it we copy all keys an values into a dictionary. We let the
    /// caller dispose of it</param> <param name="allocator"></param> <returns>DisposableNamedOnnxValue</returns>
    private static DisposableNamedOnnxValue FromNativeMap(string name, OrtValue ortValueMap, OrtAllocator allocator)
    {
        ...
        using (var cleanUpList = new DisposableList<IDisposable>())
        {
            ...
            NativeApiStatus.VerifySuccess(
                NativeMethods.OrtGetValue(ortValueMap.Handle, 0, allocator.Pointer, out nativeOnnxValueMapKeys));
            var ortValueKeys = new OrtValue(nativeOnnxValueMapKeys);
            cleanUpList.Add(ortValueKeys);

            var typeAndShape = IntPtr.Zero;
            NativeApiStatus.VerifySuccess(
                NativeMethods.OrtGetTensorTypeAndShape(nativeOnnxValueMapKeys, out typeAndShape));
            ...
            NativeApiStatus.VerifySuccess(
                NativeMethods.OrtGetValue(ortValueMap.Handle, 1, allocator.Pointer, out nativeOnnxValueMapValues));
            var ortValueValues = new OrtValue(nativeOnnxValueMapValues);
            cleanUpList.Add(ortValueValues);

            typeAndShape = IntPtr.Zero;
            NativeApiStatus.VerifySuccess(
                NativeMethods.OrtGetTensorTypeAndShape(nativeOnnxValueMapValues, out typeAndShape));
            ...
            switch (keyElemType)
            {
                case TensorElementType.Int64:
                    switch (valueElemType)
                    {
                        case TensorElementType.Float:
                            result =
                                FromNativeMapElements<Int64, float>(name, ortValueMap, ortValueKeys, ortValueValues);
                            break;
                        case TensorElementType.Double:
                            result =
                                FromNativeMapElements<Int64, double>(name, ortValueMap, ortValueKeys, ortValueValues);
                            break;
                        case TensorElementType.Int64:
                            result =
                                FromNativeMapElements<Int64, Int64>(name, ortValueMap, ortValueKeys, ortValueValues);
                            break;
                        case TensorElementType.String:
                            result =
                                FromNativeMapElements<Int64, string>(name, ortValueMap, ortValueKeys, ortValueValues);
                            break;
                        default:
                            break;
                    }
                ...
                case TensorElementType.String:
                    switch (valueElemType)
                    {
                        case TensorElementType.Float:
                            result =
                                FromNativeMapElements<string, float>(name, ortValueMap, ortValueKeys, ortValueValues);
                            break;
                        case TensorElementType.Double:
                            result =
                                FromNativeMapElements<string, double>(name, ortValueMap, ortValueKeys, ortValueValues);
                            break;
                        case TensorElementType.Int64:
                            result =
                                FromNativeMapElements<string, Int64>(name, ortValueMap, ortValueKeys, ortValueValues);
                            break;
                        case TensorElementType.String:
                            result =
                                FromNativeMapElements<string, string>(name, ortValueMap, ortValueKeys, ortValueValues);
                            break;
                        default:
                            break;
                    }
    ...
    /// and disposes of them. The original ortValueElement looses ownership of the Tensor. The caller is still
    /// responsible for disposing these arguments. Disposing ortValueElement that does not have ownership is a no-op,
    /// however, either of the arguments may still need to be disposed on exception.
    /// </summary>
    ...
    private static DisposableNamedOnnxValue FromNativeMapElements<K, V>(string name, OrtValue ortValueMap,
                                                                        OrtValue ortValueTensorKeys,
                                                                        OrtValue ortValueTensorValues)
    {
        ...
        try
        {
            ...
            {
                var denseTensorKeys =
                    new DenseTensor<string>(tensorKeys.GetBytesAsStringMemory(), tensorKeys.Dimensions);

                if (typeof(V) == typeof(string))
                {
                    var map = new Dictionary<string, string>();
                    var denseTensorValues =
                        new DenseTensor<string>(tensorValues.GetBytesAsStringMemory(), tensorValues.Dimensions);
