...
public class OrtArenaCfg : SafeHandle
{
    ...
    {
        NativeApiStatus.VerifySuccess(NativeMethods.OrtCreateArenaCfg(
            (UIntPtr)maxMemory, arenaExtendStrategy, initialChunkSizeBytes, maxDeadBytesPerChunk, out handle));
    }
...
public class OrtMemoryInfo : SafeHandle
{
    ...
    private static OrtMemoryInfo CreateCpuMemoryInfo()
    {
        ...
        NativeApiStatus.VerifySuccess(
            NativeMethods.OrtCreateCpuMemoryInfo(OrtAllocatorType.DeviceAllocator, OrtMemType.Cpu, out memoryInfo));
    ...
    internal OrtMemoryInfo(IntPtr allocInfo, bool owned) : base(allocInfo, true)
    {
        _owned = owned;
    }
        ...
        : base(IntPtr.Zero, true)
    {
        ...
        {
            NativeApiStatus.VerifySuccess(
                NativeMethods.OrtCreateMemoryInfo(pinnedName.Pointer, allocatorType, deviceId, memoryType, out handle));
        }
        _owned = true;
    }
...
public class OrtExternalAllocation
{
    ...
    public OrtExternalAllocation(OrtMemoryInfo memInfo, long[] shape, Tensors.TensorElementType elementType,
                                 IntPtr pointer, long sizeInBytes)
    {
        Type type;
        int width;
        ...
        {
            var message =
                String.Format("Shape of {0} elements requires a buffer of at least {1} bytes. Provided: {2} bytes",
...
public class OrtMemoryAllocation : SafeHandle
{
    ...
    internal OrtMemoryAllocation(OrtAllocator allocator, IntPtr pointer, uint size) : base(pointer, true)
    {
...
public class OrtAllocator : SafeHandle
{
    ...
    /// </summary>
    public static OrtAllocator
        DefaultInstance // May throw exception in every access, if the constructor have thrown an exception
    {
        get {
    ...
    internal OrtAllocator(IntPtr allocator, bool owned) : base(allocator, true)
    {
        _owned = owned;
    }
    ...
    public OrtAllocator(InferenceSession session, OrtMemoryInfo memInfo) : base(IntPtr.Zero, true)
    {
