...
#region Public API
    ...
    /// </summary>
    /// <param name="inputs">specify a collection of <see cref="NamedOnnxValue"/> that indicates the input
    ...
    /// </summary>
    /// <param name="inputs">Specify a collection of <see cref="NamedOnnxValue"/> that indicates the input
    /// values.</param> <param name="outputNames">Specify a collection of string that indicates the output names to
    /// fetch.</param> <returns>Output Tensors in a Collection of NamedOnnxValue. User must dispose the
    /// output.</returns>
    public IDisposableReadOnlyCollection<DisposableNamedOnnxValue> Run(IReadOnlyCollection<NamedOnnxValue> inputs,
    ...
    /// <param name="inputs">Specify a collection of <see cref="NamedOnnxValue"/> that indicates the input
    /// values.</param> <param name="outputNames">Specify a collection of string that indicates the output names to
    /// fetch.</param> <param name="options"></param> <returns>Output Tensors in a Collection of NamedOnnxValue. User
    /// must dispose the output.</returns>
    public IDisposableReadOnlyCollection<DisposableNamedOnnxValue> Run(IReadOnlyCollection<NamedOnnxValue> inputs,
                                                                       IReadOnlyCollection<string> outputNames,
                                                                       RunOptions options)
    {
        using (var cleanupList = new DisposableList<IDisposable>())
        {
            ...
            var inputValuesArray =
                GetOrtValuesHandles(inputs, LookupInputMetadata, ExtractOrtValueForInput, cleanupList);
    ...
    /// Runs the loaded model for the given inputs, and fetches all the outputs.
    /// </summary>
    /// <param name="inputNames">Specify a collection of string that indicates the input names. Should match <paramref
    /// name="inputValues"/>.</param> <param name="inputValues">Specify a collection of <see
    /// cref="FixedBufferOnnxValue"/> that indicates the input values.</param> <returns>Output Tensors in a Collection
    /// of NamedOnnxValue. User must dispose the output.</returns>
    public IDisposableReadOnlyCollection<DisposableNamedOnnxValue> Run(
        IReadOnlyCollection<string> inputNames, IReadOnlyCollection<FixedBufferOnnxValue> inputValues)
    {
    ...
    /// <param name="inputNames">Specify a collection of string that indicates the input names. Should match <paramref
    /// name="inputValues"/>.</param> <param name="inputValues">Specify a collection of <see
    /// cref="FixedBufferOnnxValue"/> that indicates the input values.</param> <param name="outputNames">Specify a
    /// collection of string that indicates the output names to fetch.</param> <returns>Output Tensors in a Collection
    /// of NamedOnnxValue. User must dispose the output.</returns>
    public IDisposableReadOnlyCollection<DisposableNamedOnnxValue> Run(
        IReadOnlyCollection<string> inputNames, IReadOnlyCollection<FixedBufferOnnxValue> inputValues,
        IReadOnlyCollection<string> outputNames)
    ...
    /// name="inputValues"/>.</param> <param name="inputValues">Specify a collection of <see
    /// cref="FixedBufferOnnxValue"/> that indicates the input values.</param> <param name="outputNames">Specify a
    /// collection of string that indicates the output names to fetch.</param> <param name="options"></param>
    /// <returns>Output Tensors in a Collection of NamedOnnxValue. User must dispose the output.</returns>
    public IDisposableReadOnlyCollection<DisposableNamedOnnxValue> Run(
        IReadOnlyCollection<string> inputNames, IReadOnlyCollection<FixedBufferOnnxValue> inputValues,
        IReadOnlyCollection<string> outputNames, RunOptions options)
    {
    ...
    /// name="inputValues"/>.</param> <param name="inputValues">Specify a collection of <see
    /// cref="FixedBufferOnnxValue"/> that indicates the input values.</param> <param name="outputNames">Specify a
    /// collection of string that indicates the output names. Should match <paramref name="outputValues"/>.</param>
    /// <param name="outputValues">Specify a collection of <see cref="FixedBufferOnnxValue"/> that indicates the output
    /// values.</param>
    public void Run(IReadOnlyCollection<string> inputNames, IReadOnlyCollection<FixedBufferOnnxValue> inputValues,
                    IReadOnlyCollection<string> outputNames, IReadOnlyCollection<FixedBufferOnnxValue> outputValues)
    {
    ...
    /// collection of string that indicates the output names. Should match <paramref name="outputValues"/>.</param>
    /// <param name="outputValues">Specify a collection of <see cref="FixedBufferOnnxValue"/> that indicates the output
    /// values.</param> <param name="options"></param>
    public void Run(IReadOnlyCollection<string> inputNames, IReadOnlyCollection<FixedBufferOnnxValue> inputValues,
                    IReadOnlyCollection<string> outputNames, IReadOnlyCollection<FixedBufferOnnxValue> outputValues,
                    RunOptions options)
    {
        ...
        using (var cleanupList = new DisposableList<IDisposable>())
        {
            ...
            NativeApiStatus.VerifySuccess(
                NativeMethods.OrtRun(_nativeHandle, options.Handle, inputNamesArray, inputValuesArray,
                                     (UIntPtr)inputNames.Count, outputNamesArray, (UIntPtr)outputNames.Count,
    ...
    /// </summary>
    /// <param name="inputs">Specify a collection of <see cref="NamedOnnxValue"/> that indicates the input
    /// values.</param> <param name="outputs">Specify a collection of <see cref="NamedOnnxValue"/> that indicates the
    /// output values.</param>
    ...
    /// <param name="inputs">Specify a collection of <see cref="NamedOnnxValue"/> that indicates the input
    /// values.</param> <param name="outputs">Specify a collection of <see cref="NamedOnnxValue"/> that indicates the
    /// output values.</param> <param name="options"></param>
    public void Run(IReadOnlyCollection<NamedOnnxValue> inputs, IReadOnlyCollection<NamedOnnxValue> outputs,
                    RunOptions options)
    {
        using (var cleanupList = new DisposableList<IDisposable>())
        {
            ...
            var outputValuesArray =
                GetOrtValuesHandles(outputs, LookupOutputMetadata, ExtractOrtValueForOutput, cleanupList);

            NativeApiStatus.VerifySuccess(
                NativeMethods.OrtRun(_nativeHandle, options.Handle, inputNamesArray, inputValuesArray,
                                     (UIntPtr)inputs.Count, outputNamesArray, (UIntPtr)outputs.Count,
                                     outputValuesArray /* pointers to Pre-allocated OrtValue instances */
                                     ));
        }
    }
    ...
    /// </summary>
    /// <param name="inputs">Specify a collection of <see cref="NamedOnnxValue"/> that indicates the input
    /// values.</param> <param name="outputNames">Specify a collection of string that indicates the output names. Should
    /// match <paramref name="outputValues"/>.</param> <param name="outputValues">Specify a collection of <see
    /// cref="FixedBufferOnnxValue"/> that indicates the output values.</param>
    public void Run(IReadOnlyCollection<NamedOnnxValue> inputs, IReadOnlyCollection<string> outputNames,
    ...
    /// values.</param> <param name="outputNames">Specify a collection of string that indicates the output names. Should
    /// match <paramref name="outputValues"/>.</param> <param name="outputValues">Specify a collection of <see
    /// cref="FixedBufferOnnxValue"/> that indicates the output values.</param> <param name="options"></param>
    public void Run(IReadOnlyCollection<NamedOnnxValue> inputs, IReadOnlyCollection<string> outputNames,
                    IReadOnlyCollection<FixedBufferOnnxValue> outputValues, RunOptions options)
    {
        if (outputNames.Count != outputValues.Count)
        {
            throw new ArgumentException(
        ...
        using (var cleanupList = new DisposableList<IDisposable>())
        {
            ...
            NativeApiStatus.VerifySuccess(
                NativeMethods.OrtRun(_nativeHandle, options.Handle, inputNamesArray, inputValuesArray,
                                     (UIntPtr)inputs.Count, outputNamesArray, (UIntPtr)outputNames.Count,
                                     outputValuesArray /* pointers to Pre-allocated OrtValue instances */
                                     ));
        }
    }
    ...
    /// <param name="inputNames">Specify a collection of string that indicates the input names. Should match <paramref
    /// name="inputValues"/>.</param> <param name="inputValues">Specify a collection of <see
    /// cref="FixedBufferOnnxValue"/> that indicates the input values.</param> <param name="outputs">Specify a
    /// collection of <see cref="NamedOnnxValue"/> that indicates the output values.</param>
    public void Run(IReadOnlyCollection<string> inputNames, IReadOnlyCollection<FixedBufferOnnxValue> inputValues,
    ...
    /// name="inputValues"/>.</param> <param name="inputValues">Specify a collection of <see
    /// cref="FixedBufferOnnxValue"/> that indicates the input values.</param> <param name="outputs">Specify a
    /// collection of <see cref="NamedOnnxValue"/> that indicates the output values.</param> <param
    /// name="options"></param>
    public void Run(IReadOnlyCollection<string> inputNames, IReadOnlyCollection<FixedBufferOnnxValue> inputValues,
                    IReadOnlyCollection<NamedOnnxValue> outputs, RunOptions options)
    {
        if (inputNames.Count != inputValues.Count)
        {
            throw new ArgumentException(
        ...
        using (var cleanupList = new DisposableList<IDisposable>())
        {
            ...
            NativeApiStatus.VerifySuccess(
                NativeMethods.OrtRun(_nativeHandle, options.Handle, inputNamesArray, inputValuesArray,
                                     (UIntPtr)inputNames.Count, outputNamesArray, (UIntPtr)outputs.Count,
                                     outputValuesArray /* pointers to Pre-allocated OrtValue instances */
                                     ));
        }
    }
    ...
    /// call to retrieve output names. They will be paired with the returned OrtValues and combined into
    /// DisposbleNamedOnnxValues. Otherwise, the method will retrieve output names from the OrtIoBinding instance. It is
    /// an error if you supply a different number of names than the returned outputs</param> <returns>A disposable
    /// collection of DisposableNamedOnnxValue that encapsulate output OrtValues</returns>
    public IDisposableReadOnlyCollection<DisposableNamedOnnxValue> RunWithBindingAndNames(RunOptions runOptions,
        ...
        using (var ortValues = ioBinding.GetOutputValues())
        {
            ...
            if (outputNames.Length != ortValues.Count)
            {
                throw new OnnxRuntimeException(ErrorCode.InvalidArgument,
                                               "Number of specified names: " + names.Length +
                                                   " does not match the output number: " + ortValues.Count);
            }
    ...
    public string EndProfiling()
    {
        ...
        NativeApiStatus.VerifySuccess(
            NativeMethods.OrtSessionEndProfiling(_nativeHandle, allocator.Pointer, out nameHandle));
    ...
    private NodeMetadata LookupInputMetadata(string nodeName)
    {
        ...
        {
            throw new OnnxRuntimeException(ErrorCode.InvalidArgument,
                                           $"Input name: '{nodeName}' is not in the metadata");
        }
        return meta;
    }
    ...
    private NodeMetadata LookupOutputMetadata(string nodeName)
    {
        ...
        {
            throw new OnnxRuntimeException(ErrorCode.InvalidArgument,
                                           $"Output name: '{nodeName}' is not in the metadata");
        }
        return meta;
    }
    ...
    /// <returns></returns>
    private static OrtValue ExtractOrtValueForInput(NamedOnnxValue input, NodeMetadata metadata,
                                                    out IDisposable memOwner)
    {
    ...
    private static OrtValue ExtractOrtValueForOutput(NamedOnnxValue output, NodeMetadata metadata,
                                                     out IDisposable memOwner)
    {
    ...
    private IntPtr[] ConvertNamesToUtf8<T>(IReadOnlyCollection<T> values, NameExtractor<T> nameExtractor,
                                           MetadataLookup metaLookup, DisposableList<IDisposable> cleanupList)
    {
    ...
    private IntPtr[] GetOrtValuesHandles(IReadOnlyCollection<NamedOnnxValue> values, MetadataLookup metaLookup,
                                         OrtValueExtractor ortValueExtractor, DisposableList<IDisposable> cleanupList)
    {
    ...
    private DisposableList<OrtValue> RunImpl(RunOptions options, IntPtr[] inputNames, IntPtr[] inputValues,
                                             IntPtr[] outputNames, DisposableList<IDisposable> cleanupList)
    {
        ...
        NativeApiStatus.VerifySuccess(
            NativeMethods.OrtRun(_nativeHandle, options.Handle, inputNames, inputValues, (UIntPtr)inputNames.Length,
                                 outputNames, (UIntPtr)outputNames.Length,
    ...
    IDisposableReadOnlyCollection<DisposableNamedOnnxValue> CreateDisposableResult(
        List<OrtValue> ortValues, IReadOnlyCollection<string> outputNames)
    {
...
#region private methods
                      ...
                      PrePackedWeightsContainer prepackedWeightsContainer = null)
    {
        ...
        {
            NativeApiStatus.VerifySuccess(NativeMethods.OrtCreateSession(
                envHandle, NativeOnnxValueHelper.GetPlatformSerializedString(modelPath), options.Handle, out session));
        }

        else
        {
            NativeApiStatus.VerifySuccess(NativeMethods.OrtCreateSessionWithPrepackedWeightsContainer(
                envHandle, NativeOnnxValueHelper.GetPlatformSerializedString(modelPath), options.Handle,
                prepackedWeightsContainer.Pointer, out session));
        }
                      ...
                      PrePackedWeightsContainer prepackedWeightsContainer = null)
    {
        ...
        if (prepackedWeightsContainer == null)
        {
            NativeApiStatus.VerifySuccess(NativeMethods.OrtCreateSessionFromArray(
                envHandle, modelData, (UIntPtr)modelData.Length, options.Handle, out session));
        }
    ...
    private void InitWithSessionHandle(IntPtr session, SessionOptions options)
    {
        ...
        try
        {
            ...
            // get output count
            NativeApiStatus.VerifySuccess(
                NativeMethods.OrtSessionGetOutputCount(_nativeHandle, out UIntPtr outputCount));
            ...
            NativeApiStatus.VerifySuccess(
                NativeMethods.OrtSessionGetOverridableInitializerCount(_nativeHandle, out UIntPtr initilaizerCount));
            ...
            NativeApiStatus.VerifySuccess(
                NativeMethods.OrtSessionGetProfilingStartTimeNs(_nativeHandle, out UIntPtr startTime));
    ...
    private string GetOutputName(ulong index, out byte[] utf8)
    {
        string str;
        var allocator = OrtAllocator.DefaultInstance;
        NativeApiStatus.VerifySuccess(NativeMethods.OrtSessionGetOutputName(_nativeHandle, (UIntPtr)index,
                                                                            allocator.Pointer, out IntPtr nameHandle));
    ...
    private string GetInputName(ulong index, out byte[] utf8)
    {
        string str;
        var allocator = OrtAllocator.DefaultInstance;
        NativeApiStatus.VerifySuccess(NativeMethods.OrtSessionGetInputName(_nativeHandle, (UIntPtr)index,
                                                                           allocator.Pointer, out IntPtr nameHandle));
    ...
    private string GetOverridableInitializerName(ulong index, out byte[] utf8)
    {
        ...
        NativeApiStatus.VerifySuccess(NativeMethods.OrtSessionGetOverridableInitializerName(
            _nativeHandle, (UIntPtr)index, allocator.Pointer, out IntPtr nameHandle));
        using (var ortAllocation = new OrtMemoryAllocation(allocator, nameHandle, 0))
        {
            NativeOnnxValueHelper.StringAndUtf8FromNative(nameHandle, out str, out utf8);
        }
    ...
    {
        NativeApiStatus.VerifySuccess(
            NativeMethods.OrtSessionGetInputTypeInfo(_nativeHandle, (UIntPtr)index, out IntPtr typeInfo));
        try
        {
    ...
    {
        NativeApiStatus.VerifySuccess(
            NativeMethods.OrtSessionGetOutputTypeInfo(_nativeHandle, (UIntPtr)index, out IntPtr typeInfo));
        try
        {
            return GetMetadataFromTypeInfo(typeInfo);
        }
    ...
    {
        NativeApiStatus.VerifySuccess(NativeMethods.OrtSessionGetOverridableInitializerTypeInfo(
            _nativeHandle, (UIntPtr)index, out IntPtr typeInfo));
        try
        {
            return GetMetadataFromTypeInfo(typeInfo);
        }
    ...
    internal static NodeMetadata GetMetadataFromTypeInfo(IntPtr typeInfo)
    {
        ...
        throw new OnnxRuntimeException(ErrorCode.NotImplemented,
                                       $"Value type: '{valueType}' not supported in this code");
    }

    internal static NodeMetadata GetSequenceMetadataFromTypeInfo(IntPtr typeInfo)
    {
        NativeApiStatus.VerifySuccess(
            NativeMethods.OrtCastTypeInfoToSequenceTypeInfo(typeInfo, out IntPtr sequenceTypeInfo));
        // Casts API are broken. Always return success, but may return null for the result.
        if (sequenceTypeInfo == IntPtr.Zero)
        {
            throw new OnnxRuntimeException(
                ErrorCode.Fail, "TypeInfo cast to SequenceTypeInfo failed. The object does not represent a sequence");
        }

        NativeApiStatus.VerifySuccess(
            NativeMethods.OrtGetSequenceElementType(sequenceTypeInfo, out IntPtr elementType));
        try
        {
    ...
    internal static NodeMetadata GetOptionalMetadataFromTypeInfo(IntPtr typeInfo)
    {
        ...
        NativeApiStatus.VerifySuccess(
            NativeMethods.OrtCastTypeInfoToOptionalTypeInfo(typeInfo, out IntPtr optTypeInfo));
        // Casts API are broken. Always return success, but may return null for the result.
        if (optTypeInfo == IntPtr.Zero)
        {
            throw new OnnxRuntimeException(
                ErrorCode.Fail,
                "TypeInfo cast to OptionalTypeInfo failed. The object does not represent a optional value");
        }

        NativeApiStatus.VerifySuccess(
            NativeMethods.OrtGetOptionalContainedTypeInfo(optTypeInfo, out IntPtr elementTypeInfo));
        try
        {
    ...
    internal static NodeMetadata GetTensorNodeMetadata(OnnxValueType valueType, IntPtr typeInfo)
    {
        ...
        NativeApiStatus.VerifySuccess(
            NativeMethods.OrtCastTypeInfoToTensorInfo(typeInfo, out IntPtr tensorInfo)); //(IntPtr)(int)(uint)
        // Casts API are broken. Always return success, but may return null for the result.
        if (tensorInfo == IntPtr.Zero)
        {
            throw new OnnxRuntimeException(
                ErrorCode.Fail, "TypeInfo cast to TensorTypeInfo failed. The object does not represent a tensor");
        }
...
public class TensorTypeAndShape
{
    internal TensorTypeAndShape(TensorElementType elementType, int[] dimensions, string[] symbolicDimensions)
    {
        ...
        {
            throw new OnnxRuntimeException(ErrorCode.InvalidArgument,
                                           "Unregistered TensorElementType value of: " + elementType.ToString());
        }
...
/// <summary>
/// Resembles type and shape information of session-graph nodes, used for communicating the shape/type of input/output
/// nodes
/// </summary>
public class NodeMetadata
{
    ...
    {
        get {
            return (OnnxValueType == OnnxValueType.ONNX_TYPE_TENSOR) ||
                   (OnnxValueType == OnnxValueType.ONNX_TYPE_SPARSETENSOR);
        }
    }
}
...
public class ModelMetadata
{
    ...
    internal ModelMetadata(InferenceSession session)
    {
        ...
        NativeApiStatus.VerifySuccess(
            NativeMethods.OrtSessionGetModelMetadata(session.Handle, out modelMetadataHandle));

        try
        {
            // Process producer name
            IntPtr producerNameHandle = IntPtr.Zero;
            NativeApiStatus.VerifySuccess(NativeMethods.OrtModelMetadataGetProducerName(
                modelMetadataHandle, allocator.Pointer, out producerNameHandle));
            ...
            NativeApiStatus.VerifySuccess(NativeMethods.OrtModelMetadataGetGraphName(
                modelMetadataHandle, allocator.Pointer, out graphNameHandle));
            ...
            NativeApiStatus.VerifySuccess(
                NativeMethods.OrtModelMetadataGetDomain(modelMetadataHandle, allocator.Pointer, out domainHandle));
            ...
            NativeApiStatus.VerifySuccess(NativeMethods.OrtModelMetadataGetDescription(
                modelMetadataHandle, allocator.Pointer, out descriptionHandle));
            ...
            NativeApiStatus.VerifySuccess(NativeMethods.OrtModelMetadataGetGraphDescription(
                modelMetadataHandle, allocator.Pointer, out graphDescriptionHandle));
            ...
            long numKeys;
            NativeApiStatus.VerifySuccess(NativeMethods.OrtModelMetadataGetCustomMetadataMapKeys(
                modelMetadataHandle, allocator.Pointer, out customMetadataMapKeysHandle, out numKeys));

            // We have received an array of null terminated C strings which are the keys that we can use to lookup the
            // custom metadata map The OrtAllocator will finally free the customMetadataMapKeysHandle
            using (var ortAllocationKeysArray = new OrtMemoryAllocation(
                       allocator, customMetadataMapKeysHandle,
                       0)) using (var ortAllocationKeys = new DisposableList<OrtMemoryAllocation>((int)numKeys))
            {
                ...
                {
                    ortAllocationKeys.Add(new OrtMemoryAllocation(
                        allocator, Marshal.ReadIntPtr(customMetadataMapKeysHandle, IntPtr.Size * i), 0));
                }
                ...
                foreach (var allocation in ortAllocationKeys)
                {
                    ...
                    NativeApiStatus.VerifySuccess(NativeMethods.OrtModelMetadataLookupCustomMetadataMap(
                        modelMetadataHandle, allocator.Pointer, keyHandle, out valueHandle));
