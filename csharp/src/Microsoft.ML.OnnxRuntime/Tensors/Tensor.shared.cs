...
/// it is blittable as defined in
/// https://docs.microsoft.com/en-us/dotnet/framework/interop/blittable-and-non-blittable-types and as such, represented
/// the same way in managed and native memories. This means that arrays of this type do not have to be copied to be
/// passed to native memory but simply pinnned and read by native code. Thus, one can create a Tensor on top of an array
/// of these structures and feed it directly to Onnxruntime library. Binary wise, it is the same as ushort[] (uint16_t
/// in C++). However, we would like a separate type for type dispatching.
/// </summary>
...
public class TensorTypeInfo
{
    ...
    {
        get {
            return ElementType == TensorElementType.String;
        }
    }
    /// <summary>
    /// Ctor
...
public class TensorBase
{
    ...
    static TensorBase()
    {
        ...
        {
            tensorElementTypeInfoMap.Add(info.Value.ElementType,
                                         new TensorElementTypeInfo(info.Key, info.Value.TypeSize));
        }
    }
...
{
    /// <summary>
    /// Creates an identity tensor of the specified size.  An identity tensor is a two dimensional tensor with 1s in the
    /// diagonal.
    /// </summary>
    /// <typeparam name="T">type contained within the Tensor.  Typically a value type such as int, double, float,
    /// etc.</typeparam> <param name="size">Width and height of the identity tensor to create.</param> <returns>a
    /// <paramref name="size"/> by <paramref name="size"/> with 1s along the diagonal and zeros elsewhere.</returns>
    ...
    /// <summary>
    /// Creates an identity tensor of the specified size and layout (row vs column major).  An identity tensor is a two
    /// dimensional tensor with 1s in the diagonal.
    /// </summary>
    /// <typeparam name="T">type contained within the Tensor.  Typically a value type such as int, double, float,
    /// etc.</typeparam> <param name="size">Width and height of the identity tensor to create.</param> <param
    /// name="columMajor">>False to indicate that the first dimension is most minor (closest) and the last dimension is
    /// most major (farthest): row-major.  True to indicate that the last dimension is most minor (closest together) and
    /// the first dimension is most major (farthest apart): column-major.</param> <returns>a <paramref name="size"/> by
    /// <paramref name="size"/> with 1s along the diagonal and zeros elsewhere.</returns>
    ...
    /// <summary>
    /// Creates an identity tensor of the specified size and layout (row vs column major) using the specified one value.
    /// An identity tensor is a two dimensional tensor with 1s in the diagonal.  This may be used in case T is a type
    /// that doesn't have a known 1 value.
    /// </summary>
    /// <typeparam name="T">type contained within the Tensor.  Typically a value type such as int, double, float,
    /// etc.</typeparam> <param name="size">Width and height of the identity tensor to create.</param> <param
    /// name="columMajor">>False to indicate that the first dimension is most minor (closest) and the last dimension is
    /// most major (farthest): row-major.  True to indicate that the last dimension is most minor (closest together) and
    /// the first dimension is most major (farthest apart): column-major.</param> <param name="oneValue">Value of
    /// <typeparamref name="T"/> that is used along the diagonal.</param> <returns>a <paramref name="size"/> by
    /// <paramref name="size"/> with 1s along the diagonal and zeros elsewhere.</returns>
    ...
    /// <summary>
    /// Creates a n+1-rank tensor using the specified n-rank diagonal.  Values not on the diagonal will be filled with
    /// zeros.
    /// </summary>
    /// <typeparam name="T">type contained within the Tensor.  Typically a value type such as int, double, float,
    /// etc.</typeparam> <param name="diagonal">Tensor representing the diagonal to build the new tensor from.</param>
    /// <returns>A new tensor of the same layout and order as <paramref name="diagonal"/> of one higher rank, with the
    /// values of <paramref name="diagonal"/> along the diagonal and zeros elsewhere.</returns>
    ...
    public static Tensor<T> CreateFromDiagonal<T>(Tensor<T> diagonal, int offset)
    {
        if (diagonal.Rank < 1)
        {
            throw new ArgumentException($"Tensor {nameof(diagonal)} must have at least one dimension.",
                                        nameof(diagonal));
        }
...
public abstract class Tensor<T>
    : TensorBase, IList, IList<T>, IReadOnlyList<T>, IStructuralComparable, IStructuralEquatable
{
    ...
    /// <summary>
    /// Initialize an n-dimensional tensor with the specified dimensions and layout.  ReverseStride=true gives a stride
    /// of 1-element width to the first dimension (0).  ReverseStride=false gives a stride of 1-element width to the
    /// last dimension (n-1).
    /// </summary>
    /// <param name="dimensions">An span of integers that represent the size of each dimension of the Tensor to
    /// create.</param> <param name="reverseStride">False (default) to indicate that the first dimension is most major
    /// (farthest apart) and the last dimension is most minor (closest together): akin to row-major in a rank-2 tensor.
    /// True to indicate that the last dimension is most major (farthest apart) and the first dimension is most minor
    /// (closest together): akin to column-major in a rank-2 tensor.</param>
    ...
    /// <summary>
    /// Creates a new Tensor with the same layout and dimensions as this tensor with elements initialized to their
    /// default value.
    /// </summary>
    /// <returns>A new Tensor with the same layout and dimensions as this tensor with elements initialized to their
    ...
    /// <summary>
    /// Creates a new Tensor with the specified dimensions and the same layout as this tensor with elements initialized
    /// to their default value.
    /// </summary>
    /// <param name="dimensions">An span of integers that represent the size of each dimension of the DenseTensor to
    /// create.</param> <returns>A new Tensor with the same layout as this tensor and specified <paramref
    /// name="dimensions"/> with elements initialized to their default value.</returns>
    ...
    /// <summary>
    /// Creates a new Tensor of a different type with the same layout and size as this tensor with elements initialized
    /// to their default value.
    /// </summary>
    /// <typeparam name="TResult">Type contained within the new Tensor.  Typically a value type such as int, double,
    /// float, etc.</typeparam> <returns>A new Tensor with the same layout and dimensions as this tensor with elements
    /// of <typeparamref name="TResult"/> type initialized to their default value.</returns>
    ...
    /// <summary>
    /// Creates a new Tensor of a different type with the specified dimensions and the same layout as this tensor with
    /// elements initialized to their default value.
    /// </summary>
    /// <typeparam name="TResult">Type contained within the new Tensor.  Typically a value type such as int, double,
    /// float, etc.</typeparam> <param name="dimensions">An span of integers that represent the size of each dimension
    /// of the DenseTensor to create.</param> <returns>A new Tensor with the same layout as this tensor of specified
    /// <paramref name="dimensions"/> with elements of <typeparamref name="TResult"/> type initialized to their default
    /// value.</returns>
    ...
    /// </summary>
    /// <param name="offset">Offset of diagonal to set in returned tensor.  0 for the main diagonal, less than zero for
    /// diagonals below, greater than zero from diagonals above.</param> <returns>An n-1 dimension tensor with the
    /// values from the specified diagonal of this tensor.</returns>
    public Tensor<T> GetDiagonal(int offset)
    {
        ...
        if (Rank < 2)
        {
            throw new InvalidOperationException(
                $"Cannot compute diagonal of {nameof(Tensor<T>)} with Rank less than 2.");
        }
        ...
        var newTensorDimensions =
            newTensorRank < ArrayUtilities.StackallocMax ? stackalloc int[newTensorRank] : new int[newTensorRank];
        ...
        var diagProjectionStride =
            diagonalTensor.IsReversedStride && diagonalTensor.Rank > 1 ? diagonalTensor.strides[1] : 1;
    ...
    /// <summary>
    /// Gets a tensor representing the elements below and including the specified diagonal, with the rest of the
    /// elements zero-ed.
    /// </summary>
    /// <param name="offset">Offset of diagonal to set in returned tensor.  0 for the main diagonal, less than zero for
    /// diagonals below, greater than zero from diagonals above.</param> <returns>A tensor with the values from this
    /// tensor at and below the specified diagonal and zeros elsewhere.</returns>
    ...
    /// <summary>
    /// Gets a tensor representing the elements above and including the specified diagonal, with the rest of the
    /// elements zero-ed.
    /// </summary>
    /// <param name="offset">Offset of diagonal to set in returned tensor.  0 for the main diagonal, less than zero for
    /// diagonals below, greater than zero from diagonals above.</param> <returns>A tensor with the values from this
    /// tensor at and above the specified diagonal and zeros elsewhere.</returns>
    ...
    public Tensor<T> GetTriangle(int offset, bool upper)
    {
        ...
        {
            throw new InvalidOperationException(
                $"Cannot compute triangle of {nameof(Tensor<T>)} with Rank less than 2.");
        }
    ...
    /// </summary>
    /// <param name="dimensions">An span of integers that represent the size of each dimension of the Tensor to
    /// create.</param> <returns>A new tensor that reinterprets this tensor with different dimensions.</returns>
    public abstract Tensor<T> Reshape(ReadOnlySpan<int> dimensions);

    /// <summary>
    /// Obtains the value at the specified indices
    /// </summary>
    /// <param name="indices">A one-dimensional array of integers that represent the indices specifying the position of
    /// the element to get.</param> <returns>The value at the specified position in this Tensor.</returns>
    ...
    /// Obtains the value at the specified indices
    /// </summary>
    /// <param name="indices">A span integers that represent the indices specifying the position of the element to
    /// get.</param> <returns>The value at the specified position in this Tensor.</returns>
...
#region statics
    /// <summary>
    /// Performs a value comparison of the content and shape of two tensors.  Two tensors are equal if they have the
    /// same shape and same value at every set of indices.  If not equal a tensor is greater or less than another tensor
    /// based on the first non-equal element when enumerating in linear order.
    /// </summary>
    ...
    /// <summary>
    /// Performs a value equality comparison of the content of two tensors. Two tensors are equal if they have the same
    /// shape and same value at every set of indices.
    /// </summary>
    /// <param name="left"></param>
    /// <param name="right"></param>
    /// <returns></returns>
...
#region IList members
    object IList.this[int index]
    {
        ...
        set
        {
            ...
            {
                throw new ArgumentException(
                    $"The value \"{value}\" is not of type \"{typeof(T)}\" and cannot be used in this generic collection.");
            }
        }
    }
...
#region ICollection < T> members
    ...
    /// </summary>
    /// <param name="array">
    /// The one-dimensional Array that is the destination of the elements copied from Tensor&lt;T&gt;. The Array must
...
#region IStructuralComparable members
    ...
    private int CompareTo(Tensor<T> other, IComparer comparer)
    {
        ...
        {
            throw new ArgumentException(
                $"Cannot compare {nameof(Tensor<T>)} with Rank {Rank} to {nameof(other)} with Rank {other.Rank}.",
                nameof(other));
        }
        ...
        for (int i = 0; i < dimensions.Length; i++)
        {
            ...
            {
                throw new ArgumentException(
                    $"Cannot compare {nameof(Tensor<T>)}s with differning dimension {i}, {dimensions[i]} != {other.dimensions[i]}.",
                    nameof(other));
            }
        }
    ...
    private int CompareTo(Array other, IComparer comparer)
    {
        ...
        {
            throw new ArgumentException(
                $"Cannot compare {nameof(Tensor<T>)} with Rank {Rank} to {nameof(Array)} with rank {other.Rank}.",
                nameof(other));
        }
        ...
        for (int i = 0; i < dimensions.Length; i++)
        {
            ...
            {
                throw new ArgumentException(
                    $"Cannot compare {nameof(Tensor<T>)} to {nameof(Array)} with differning dimension {i}, {dimensions[i]} != {otherDimension}.",
                    nameof(other));
            }
        }
...
#region Translations
    ...
    /// <summary>
    /// Creates a copy of this tensor as a DenseTensor&lt;T&gt;.  If this tensor is already a DenseTensor&lt;T&gt;
    /// calling this method is equivalent to calling Clone().
    /// </summary>
    /// <returns></returns>
