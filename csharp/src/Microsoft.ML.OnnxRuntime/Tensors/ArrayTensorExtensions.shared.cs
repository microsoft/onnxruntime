...
public static class ArrayTensorExtensions
{
    ...
    /// <returns>A 1-dimensional DenseTensor&lt;T&gt; with the same length and content as <paramref
    /// name="array"/>.</returns>
    ...
    /// <typeparam name="T">Type contained in the array to copy to the DenseTensor&lt;T&gt;.</typeparam>
    /// <param name="array">The array to create a DenseTensor&lt;T&gt; from.</param>
    /// <param name="reverseStride">False (default) to indicate that the first dimension is most major (farthest apart)
    /// and the last dimension is most minor (closest together): row-major.  True to indicate that the last dimension is
    /// most major (farthest apart) and the first dimension is most minor (closest together): column-major.</param>
    /// <returns>A 2-dimensional DenseTensor&lt;T&gt; with the same dimensions and content as <paramref
    /// name="array"/>.</returns>
    ...
    /// <param name="array">The array to create a DenseTensor&lt;T&gt; from.</param>
    /// <param name="reverseStride">False (default) to indicate that the first dimension is most major (farthest apart)
    /// and the last dimension is most minor (closest together): akin to row-major in a rank-2 tensor.  True to indicate
    /// that the last dimension is most major (farthest apart) and the first dimension is most minor (closest together):
    /// akin to column-major in a rank-2 tensor.</param> <returns>A 3-dimensional DenseTensor&lt;T&gt; with the same
    /// dimensions and content as <paramref name="array"/>.</returns>
    public static DenseTensor<T> ToTensor<T>(this T[,,] array, bool reverseStride = false)
    {
        if (reverseStride)
        {
    ...
    /// and the last dimension is most minor (closest together): akin to row-major in a rank-2 tensor.  True to indicate
    /// that the last dimension is most major (farthest apart) and the first dimension is most minor (closest together):
    /// akin to column-major in a rank-2 tensor.</param> <returns>A 4-dimensional DenseTensor&lt;T&gt; with the same
    /// dimensions and content as <paramref name="array"/>.</returns>
    public static DenseTensor<T> ToTensor<T>(this T[,,,] array, bool reverseStride = false)
    {
        if (reverseStride)
        ...
        else
        {
            ...
            T[] copy = new T[array.Length];
            var dimensions =
                new int[] { array.GetLength(0), array.GetLength(1), array.GetLength(2), array.GetLength(3) };
    ...
    /// and the last dimension is most minor (closest together): akin to row-major in a rank-2 tensor.  True to indicate
    /// that the last dimension is most major (farthest apart) and the first dimension is most minor (closest together):
    /// akin to column-major in a rank-2 tensor.</param> <returns>A n-dimensional DenseTensor&lt;T&gt; with the same
    /// dimensions and content as <paramref name="array"/>.</returns>
