...
public class FixedBufferOnnxValue : IDisposable
{
    ...
    private FixedBufferOnnxValue(MemoryHandle pinnedMemory, OrtValue ortValue, OnnxValueType onnxValueType,
    ...
    public static FixedBufferOnnxValue CreateFromTensor<T>(Tensor<T> value)
    {
        ...
        {
            return new FixedBufferOnnxValue((MemoryHandle)memHandle, ortValue, OnnxValueType.ONNX_TYPE_TENSOR,
                                            elementType);
        }
        else
        {
            return new FixedBufferOnnxValue(default(MemoryHandle), ortValue, OnnxValueType.ONNX_TYPE_TENSOR,
                                            elementType);
        }
    }
    ...
    /// that are blittable as defined in
    /// https://docs.microsoft.com/en-us/dotnet/framework/interop/blittable-and-non-blittable-types I.e. those that have
    /// the same binary representation as the original type. This includes all existing types but may also allow using
    /// custom types for Float16 and BFloat16 providing they have the same layout and size. The resulting instance must
    /// be disposed of to release pinned memory and deallocate native OrtValue See example below.
    /// </summary>
    ...
    ///
    /// If you are using a library that has a class Half and it is blittable, that is its managed in memory
    /// representation matches native one and its size is 16-bits, you can use the following conceptual example to
    /// feed/fetch data for inference using Half array. This allows you to avoid copying data from your Half[] to
    /// Float16[]
    ///
    /// \code{.cs}
                                                           ...
                                                           TensorElementType elementType, long[] shape, long bytesSize)
    {
        ...
        try
        {
            ...
            var ortValue = OrtValue.CreateTensorValueWithData(memoryInfo, elementType, shape, memPtr, bytesSize);
