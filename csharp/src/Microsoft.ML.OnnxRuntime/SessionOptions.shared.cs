...
#region Constructor and Factory methods
    ...
    /// </summary>
    public SessionOptions() : base(IntPtr.Zero, true)
    {
    ...
    /// Use only if CUDA/TensorRT are installed and you have the onnxruntime package specific to this Execution
    /// Provider.
    /// </summary>
    /// <param name="deviceId"></param>
    /// <returns>A SessionsOptions() object configured for execution on deviceId</returns>
...
#region ExecutionProviderAppends
    ...
    public void AppendExecutionProvider_CUDA(OrtCUDAProviderOptions cudaProviderOptions)
    {
        ...
        NativeApiStatus.VerifySuccess(
            NativeMethods.SessionOptionsAppendExecutionProvider_CUDA_V2(handle, cudaProviderOptions.Handle));
#endif
    }
...
#else
        var deviceIdPinned =
            GCHandle.Alloc(NativeOnnxValueHelper.StringToZeroTerminatedUtf8(deviceId), GCHandleType.Pinned);
        using (var pinnedDeviceIdName = new PinnedGCHandle(deviceIdPinned))
        {
            NativeApiStatus.VerifySuccess(
                NativeMethods.OrtSessionOptionsAppendExecutionProvider_OpenVINO(handle, pinnedDeviceIdName.Pointer));
        }
#endif
    }
...
#else
        NativeApiStatus.VerifySuccess(
            NativeMethods.OrtSessionOptionsAppendExecutionProvider_Tensorrt(handle, deviceId));
#endif
    }
...
#else
        NativeApiStatus.VerifySuccess(
            NativeMethods.SessionOptionsAppendExecutionProvider_TensorRT_V2(handle, trtProviderOptions.Handle));
#endif
    }
...
#else
        NativeApiStatus.VerifySuccess(
            NativeMethods.OrtSessionOptionsAppendExecutionProvider_MIGraphX(handle, deviceId));
#endif
    }
...
#else
        var settingsPinned =
            GCHandle.Alloc(NativeOnnxValueHelper.StringToZeroTerminatedUtf8(settings), GCHandleType.Pinned);
        using (var pinnedSettingsName = new PinnedGCHandle(settingsPinned))
        {
            NativeApiStatus.VerifySuccess(
                NativeMethods.OrtSessionOptionsAppendExecutionProvider_Tvm(handle, pinnedSettingsName.Pointer));
        }
#endif
    }
    ...
    public void AppendExecutionProvider(string providerName, Dictionary<string, string> providerOptions = null)
    {
        ...
        {
            throw new NotSupportedException("Only SNPE and XNNPACK execution providers can be enabled by this method.");
        }
        ...
        using (var cleanupList = new DisposableList<IDisposable>())
        {
            ...
            var keysArray =
                NativeOnnxValueHelper.ConvertNamesToUtf8(providerOptions.Keys.ToArray(), n => n, cleanupList);

            var valuesArray =
                NativeOnnxValueHelper.ConvertNamesToUtf8(providerOptions.Values.ToArray(), n => n, cleanupList);
...
#region Public Methods
    ...
    public void RegisterCustomOpLibrary(string libraryPath)
    {
        ...
        var libraryPathPinned =
            GCHandle.Alloc(NativeOnnxValueHelper.StringToZeroTerminatedUtf8(libraryPath), GCHandleType.Pinned);
        using (var pinnedlibraryPath = new PinnedGCHandle(libraryPathPinned))
        {
            NativeApiStatus.VerifySuccess(
                NativeMethods.OrtRegisterCustomOpsLibrary(handle, pinnedlibraryPath.Pointer, out libraryHandle));
        }
    }
    ...
    /// <param name="ortValue">OrtValue containing the initializer. Lifetime of 'val' and the underlying initializer
    /// buffer must be managed by the user (created using the CreateTensorWithDataAsOrtValue API) and it must outlive
    /// the session object</param>
    ...
    {
        using (var pinnedConfigKeyName = new PinnedGCHandle(GCHandle.Alloc(
                   NativeOnnxValueHelper.StringToZeroTerminatedUtf8(configKey),
                   GCHandleType.Pinned))) using (var pinnedConfigValueName =
                                                     new PinnedGCHandle(GCHandle.Alloc(
                                                         NativeOnnxValueHelper.StringToZeroTerminatedUtf8(configValue),
                                                         GCHandleType.Pinned)))
        {
            NativeApiStatus.VerifySuccess(NativeMethods.OrtAddSessionConfigEntry(handle, pinnedConfigKeyName.Pointer,
                                                                                 pinnedConfigValueName.Pointer));
        }
    }
    ...
    {
        var utf8DimDenotationPinned =
            GCHandle.Alloc(NativeOnnxValueHelper.StringToZeroTerminatedUtf8(dimDenotation), GCHandleType.Pinned);
        using (var pinnedDimDenotation = new PinnedGCHandle(utf8DimDenotationPinned))
        {
            NativeApiStatus.VerifySuccess(
                NativeMethods.OrtAddFreeDimensionOverride(handle, pinnedDimDenotation.Pointer, dimValue));
        }
    }

    /// <summary>
    /// Override symbolic dimensions (by specific name strings) with actual values if known at session initialization
    ...
    {
        var utf8DimNamePinned =
            GCHandle.Alloc(NativeOnnxValueHelper.StringToZeroTerminatedUtf8(dimName), GCHandleType.Pinned);
        using (var pinnedDimName = new PinnedGCHandle(utf8DimNamePinned))
        {
            NativeApiStatus.VerifySuccess(
                NativeMethods.OrtAddFreeDimensionOverrideByName(handle, pinnedDimName.Pointer, dimValue));
        }
    }
#endregion
...
#region Public Properties
    ...
    public bool EnableProfiling
    {
        ...
        set {
            ...
            {
                NativeApiStatus.VerifySuccess(NativeMethods.OrtEnableProfiling(
                    handle, NativeOnnxValueHelper.GetPlatformSerializedString(ProfileOutputPathPrefix)));
    ...
    /// <summary>
    ///  Set filepath to save optimized model after graph level transformations. Default is empty, which implies saving
    ///  is disabled.
    /// </summary>
    ...
    public string OptimizedModelFilePath
    {
        ...
        set {
            ...
            {
                NativeApiStatus.VerifySuccess(NativeMethods.OrtSetOptimizedModelFilePath(
                    handle, NativeOnnxValueHelper.GetPlatformSerializedString(value)));
    ...
    public string LogId
    {
        ...
        set {
            var logIdPinned =
                GCHandle.Alloc(NativeOnnxValueHelper.StringToZeroTerminatedUtf8(value), GCHandleType.Pinned);
