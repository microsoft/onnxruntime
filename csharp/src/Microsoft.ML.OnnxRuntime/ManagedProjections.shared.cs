...
internal class ManagedTypeProjection : IDisposable
{
    ...
    internal OrtValue Value
    {
        get {
            return new OrtValue(_ortValue.Handle, false);
        }
    }
    ...
    /// <returns></returns>
    private OrtValue CreateDispatchProjection(NamedOnnxValue namedOnnxValue, NodeMetadata metadata,
        ...
        switch (namedOnnxValue.ValueType)
        {
            ...
            default:
                throw new OnnxRuntimeException(
                    ...
                    "ManagedTypeProjection can only project tensors, sequences, maps and optional types");
        }
        return result;
    }
    ...
    /// <returns></returns>
    /// <exception cref="OnnxRuntimeException"></exception>
    private OrtValue CreateSequenceProjection(NamedOnnxValue namedOnnxValue, NodeMetadata metadata,
                                              DisposableList<IDisposable> disposables)
    {
        ...
        NativeApiStatus.VerifySuccess(NativeMethods.OrtCreateValue(ortValHandles, (UIntPtr)sequenceOrtValues.Count,
                                                                   (IntPtr)OnnxValueType.ONNX_TYPE_SEQUENCE,
    ...
    /// <exception cref="OnnxRuntimeException"></exception>
    private OrtValue CreateMapProjection(NamedOnnxValue node, NodeMetadata elementMeta,
                                         DisposableList<IDisposable> disposables)
    {
        OrtValue result = null;
        var mapMeta = elementMeta.AsMapMetadata();
        Debug.Assert(mapMeta != null);
        // Maps currently support only primitive types expressed as two parallel tensors and not nested Sequences or
        // Maps
        ...
        var keys = node.GetDictionaryKeys();
            ...
            NativeApiStatus.VerifySuccess(NativeMethods.OrtCreateValue(
                ortValHandles, (UIntPtr)2, (IntPtr)OnnxValueType.ONNX_TYPE_MAP, out IntPtr ortValueMap));
    ...
    /// <returns></returns>
    /// <exception cref="OnnxRuntimeException"></exception>
    private OrtValue CreateTensorProjection(NamedOnnxValue node, NodeMetadata elementMeta,
                                            DisposableList<IDisposable> disposables)
    {
