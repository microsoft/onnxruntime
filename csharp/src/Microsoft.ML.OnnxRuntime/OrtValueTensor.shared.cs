        ...
        try
        {
            ...
            if (!TensorElementTypeConverter.GetTypeAndWidth(elemType, out type, out width))
            {
                throw new OnnxRuntimeException(ErrorCode.InvalidArgument,
                                               "Unable to query type information for data type: " +
                                                   elemType.ToString());
            }
            ...
            if (typeof(T) != type)
            {
                var message = String.Format(
                    "The OrtValueTensor<T> type being instantiated for T = : {0} while supplied OrtValue contains T = {1}",
                    typeof(T), type);
            ...
            NativeApiStatus.VerifySuccess(NativeMethods.OrtGetDimensionsCount(typeAndShape, out dimension));
            {
                ...
                NativeApiStatus.VerifySuccess(
                    NativeMethods.OrtGetTensorShapeElementCount(typeAndShape, out el_count)); // count can be negative.
                count = (long)el_count;
            }
            if (count < 0)
            {
            ...
            NativeApiStatus.VerifySuccess(NativeMethods.OrtGetDimensions(
                typeAndShape, shape, dimension)); // Note: shape must be alive during the call
            ...
            {
                NativeApiStatus.VerifySuccess(
                    NativeMethods.OrtGetTensorMutableData(ortValue.Handle, out _dataBufferPointer));
            }
            else
            {
                UIntPtr strLen;
                ...
                using (var dataBufferHandle = new Memory<byte>(dataBuffer).Pin()) using (
                    var offsetMemoryHandle = new Memory<UIntPtr>(offsets).Pin())
                {
                    unsafe
                    {
                        _dataBufferPointer = (IntPtr)dataBufferHandle.Pointer;
                        NativeApiStatus.VerifySuccess(NativeMethods.OrtGetStringTensorContent(
                            ortValue.Handle, _dataBufferPointer, strLen, (IntPtr)offsetMemoryHandle.Pointer,
                            (UIntPtr)Count));
                    }
                    ...
                    {
                        var length = (i == offsets.Length - 1) ? strLen.ToUInt64() - offsets[i].ToUInt64()
    ...
    public override MemoryHandle Pin(int elementIndex = 0)
    {
        ...
        unsafe
        {
            ...
            return new MemoryHandle(
                new IntPtr(_dataBufferPointer.ToInt64() + (long)elementIndex * ElementWidth).ToPointer());
        }
    }
