...
public partial class InferenceTest
{
    ...
    public void TestSessionOptions()
    {
        ...
        using (SessionOptions opt = new SessionOptions())
        {
            ...
            var ex = Assert.Throws<OnnxRuntimeException>(() =>
...
#else
            ex = Assert.Throws<OnnxRuntimeException>(() =>
                                                     { opt.AppendExecutionProvider("XNNPACK"); });
    ...
    public void TestRunOptions()
    {
        using (var opt = new RunOptions())
        {
            ...
            var ex = Assert.Throws<OnnxRuntimeException>(() =>
                                                         { opt.AddRunConfigEntry("", "missing key"); });
            Assert.Contains("[ErrorCode:InvalidArgument] Config key is empty", ex.Message);
        }
    }
    ...
    private void CanRunInferenceOnAModel(GraphOptimizationLevel graphOptimizationLevel, bool enableParallelExecution)
    {
        ...
        using (var cleanUp = new DisposableListTest<IDisposable>())
        {
            ...
            float[] inputData = TestDataLoader.LoadTensorFromEmbeddedResource(
                "bench.in"); // this is the data for only one input tensor for this model
            ...
            using (var runOptions = new RunOptions())
            {
                ...
                using (var results = session.Run(container, outputNames,
                                                 runOptions)) // results is an IReadOnlyList<NamedOnnxValue> container
                {
                    ValidateRunResults(results);
                }
            }
            ...
            using (var pinnedInputs = new DisposableListTest<FixedBufferOnnxValue>())
            {
                ...
                pinnedInputs.AddRange(
                    container.Select(i => FixedBufferOnnxValue.CreateFromTensor(i.AsTensor<float>())));

                // output names not specified
                using (var results = session.Run(inputNames,
                                                 pinnedInputs)) // results is an IReadOnlyList<NamedOnnxValue> container
                {
                    ValidateRunResults(results);
                }
                ...
                using (var results =
                           session.Run(inputNames, pinnedInputs,
                                       expectedOutputNames)) // results is an IReadOnlyList<NamedOnnxValue> container
                {
                    ValidateRunResults(results);
                }
            }
            ...
            using (var pinnedInputs = new DisposableListTest<FixedBufferOnnxValue>()) using (
                var pinnedOutputs = new DisposableListTest<FixedBufferOnnxValue>())
            {
                ...
                pinnedInputs.Add(FixedBufferOnnxValue.CreateFromMemory<float>(
                    memInfo, inputData, TensorElementType.Float, longShape, byteSize));
                ...
                pinnedOutputs.Add(FixedBufferOnnxValue.CreateFromMemory<float>(
                    memInfo, outputBuffer, TensorElementType.Float, longShape, byteSize));
            ...
            // Run inference with named inputs and named outputs
            {
                ...
                var expectedOutputValues = new List<NamedOnnxValue>() { NamedOnnxValue.CreateFromTensor(
                    "softmaxout_1", new DenseTensor<float>(expectedOutputDimensions)) };
            ...
            using (DisposableListTest<FixedBufferOnnxValue> pinnedInputs =
                       new DisposableListTest<FixedBufferOnnxValue>(),
    ...
    private void ThrowWrongOutputName()
    {
        ...
        // var outputs = new List<NamedOnnxValue> { NamedOnnxValue.CreateFromTensor<float>("bad_output_name",
        // outputTensor) };
    ...
    private void ThrowWrongOutputType()
    {
        ...
        // InferenceSession::ValidateOutputs() does not check type so far. Currently this will finally trigger an error
        // in Softmax.
        session.Dispose();
    }
    ...
    private void ThrowWrongOutputDimension()
    {
        ...
        var ex = Assert.Throws<OnnxRuntimeException>(() => session.Run(inputs, outputs));
        // TODO: check exception message
        // InferenceSession::ValidateOutputs() does not check dims so far. Currently this will finally trigger an error
        // in Softmax.
        session.Dispose();
    }
    ...
    private void ThrowInconsistentPinnedOutputs()
    {
        ...
        {
            var ex =
                Assert.Throws<ArgumentException>(() => session.Run(inputs, new string[] { "softmaxout_1" }, outputs));
    ...
    private void TestModelInputFloat()
    {
        ...
        {
            var container = new List<NamedOnnxValue>();
            var tensorIn = new DenseTensor<float>(new float[] { 1.0f, 2.0f, -3.0f, float.MinValue, float.MaxValue },
                                                  new int[] { 1, 5 });
                   ...
                   d = FixedBufferOnnxValue.CreateFromTensor(tensorD))
            {
                ...
                Assert.Throws<NotSupportedException>(
                    () =>
                    {
                        ...
                        session.Run(new[] { NamedOnnxValue.CreateFromTensor("input", tensorB) }, new[] { "output" },
                                    new[] { b });
                    });
                Assert.Throws<NotSupportedException>(() =>
                                                     {
                                                         // both FixedBufferOnnxValue for inputs and outputs
                                                         session.Run(new[] { "input" }, new[] { c }, new[] { "output" },
                                                                     new[] { d });
                                                     });
            }
        }
    ...
    private void TestModelInputINT32()
    {
        ...
        using (var session = new InferenceSession(model))
        {
            var container = new List<NamedOnnxValue>();
            var tensorIn =
                new DenseTensor<int>(new int[] { 1, -2, -3, int.MinValue, int.MaxValue }, new int[] { 1, 5 });
            var nov = NamedOnnxValue.CreateFromTensor("input", tensorIn);
            container.Add(nov);
            using (var res = session.Run(container))
            {
    ...
    private void TestModelInputSTRING()
    {
        ...
        {
            var container = new List<NamedOnnxValue>();
            var tensorIn = new DenseTensor<string>(
                new string[] { "hello", "École élémentaire", "mit freundlichen grüßen", "Понедельник",
                               "最好的问候," + "नमस्ते," + "こんにちは," + "안녕하세요" },
                new int[] { 1, 5 });
            var nov = NamedOnnxValue.CreateFromTensor("input", tensorIn);
            container.Add(nov);
            using (var res = session.Run(container))
    ...
    private void TestModelInputINT8()
    {
        ...
        using (var session = new InferenceSession(model))
        {
            var container = new List<NamedOnnxValue>();
            var tensorIn =
                new DenseTensor<sbyte>(new sbyte[] { 1, 2, -3, sbyte.MinValue, sbyte.MaxValue }, new int[] { 1, 5 });
            var nov = NamedOnnxValue.CreateFromTensor("input", tensorIn);
            container.Add(nov);
            using (var res = session.Run(container))
            {
    ...
    private void TestModelInputUINT8()
    {
        ...
        using (var session = new InferenceSession(model))
        {
            var container = new List<NamedOnnxValue>();
            var tensorIn =
                new DenseTensor<byte>(new byte[] { 1, 2, 3, byte.MinValue, byte.MaxValue }, new int[] { 1, 5 });
            var nov = NamedOnnxValue.CreateFromTensor("input", tensorIn);
            container.Add(nov);
            using (var res = session.Run(container))
            {
    ...
    private void TestModelInputUINT16()
    {
        ...
        using (var session = new InferenceSession(model))
        {
            var container = new List<NamedOnnxValue>();
            var tensorIn =
                new DenseTensor<UInt16>(new UInt16[] { 1, 2, 3, UInt16.MinValue, UInt16.MaxValue }, new int[] { 1, 5 });
            var nov = NamedOnnxValue.CreateFromTensor("input", tensorIn);
            container.Add(nov);
            using (var res = session.Run(container))
            {
    ...
    private void TestModelInputINT16()
    {
        ...
        using (var session = new InferenceSession(model))
        {
            var container = new List<NamedOnnxValue>();
            var tensorIn =
                new DenseTensor<Int16>(new Int16[] { 1, 2, 3, Int16.MinValue, Int16.MaxValue }, new int[] { 1, 5 });
            var nov = NamedOnnxValue.CreateFromTensor("input", tensorIn);
            container.Add(nov);
            using (var res = session.Run(container))
            {
    ...
    private void TestModelInputINT64()
    {
        ...
        using (var session = new InferenceSession(model))
        {
            var container = new List<NamedOnnxValue>();
            var tensorIn =
                new DenseTensor<Int64>(new Int64[] { 1, 2, -3, Int64.MinValue, Int64.MaxValue }, new int[] { 1, 5 });
            var nov = NamedOnnxValue.CreateFromTensor("input", tensorIn);
            container.Add(nov);
            using (var res = session.Run(container))
            {
    ...
    private void TestModelInputUINT32()
    {
        ...
        using (var session = new InferenceSession(model))
        {
            var container = new List<NamedOnnxValue>();
            var tensorIn =
                new DenseTensor<UInt32>(new UInt32[] { 1, 2, 3, UInt32.MinValue, UInt32.MaxValue }, new int[] { 1, 5 });
            var nov = NamedOnnxValue.CreateFromTensor("input", tensorIn);
            container.Add(nov);
            using (var res = session.Run(container))
            {
    ...
    private void TestModelInputUINT64()
    {
        ...
        using (var session = new InferenceSession(model))
        {
            var container = new List<NamedOnnxValue>();
            var tensorIn =
                new DenseTensor<UInt64>(new UInt64[] { 1, 2, 3, UInt64.MinValue, UInt64.MaxValue }, new int[] { 1, 5 });
            var nov = NamedOnnxValue.CreateFromTensor("input", tensorIn);
            container.Add(nov);
            using (var res = session.Run(container))
            {
    ...
    private void TestModelInputFLOAT16()
    {
        ...
        using (var session = new InferenceSession(model))
        {
            var container = new List<NamedOnnxValue>();
            var tensorIn =
                new DenseTensor<Float16>(new Float16[] { 15360, 16384, 16896, 17408, 17664 }, new int[] { 1, 5 });
            var nov = NamedOnnxValue.CreateFromTensor("input", tensorIn);
            container.Add(nov);
            using (var res = session.Run(container))
            {
    ...
    private void TestModelInputBFLOAT16()
    {
        ...
        using (var session = new InferenceSession(model))
        {
            var container = new List<NamedOnnxValue>();
            var tensorIn =
                new DenseTensor<BFloat16>(new BFloat16[] { 16256, 16384, 16448, 16512, 16544 }, new int[] { 1, 5 });
            var nov = NamedOnnxValue.CreateFromTensor("input", tensorIn);
            container.Add(nov);
            using (var res = session.Run(container))
            {
    ...
    {
        int device_id = 0;
        using (var info_cpu = new OrtMemoryInfo(OrtMemoryInfo.allocatorCPU, OrtAllocatorType.ArenaAllocator, device_id,
                                                OrtMemType.Default))
        {
    ...
    {
        int device_id = 0;
        using (var info_cuda = new OrtMemoryInfo(OrtMemoryInfo.allocatorCUDA, OrtAllocatorType.ArenaAllocator,
                                                 device_id, OrtMemType.Default))
        {
    ...
    {
        int device_id = 0;
        using (var info_rocm = new OrtMemoryInfo(OrtMemoryInfo.allocatorROCM, OrtAllocatorType.ArenaAllocator,
                                                 device_id, OrtMemType.Default))
        {
    ...
    private void TestSharingOfInitializerAndItsPrepackedVersion()
    {
        ...
        try
        {
            ...
            using (var sharedInitializer =
                       OrtValue.CreateTensorValueWithData(ortCpuMemInfo, Tensors.TensorElementType.Float, dims,
                                                          dataHandle.AddrOfPinnedObject(), dataBufferNumBytes))
            {
                using (var prepackedWeightsContainer = new PrePackedWeightsContainer())
                {
                    using (var options = new SessionOptions())
                    {
                        ...
                        // We want the pre-packed weights of the shared initializer to be shared between sessions
                        // (memory savings) and hence we pass in the 'prepackedWeightsContainer' at session creation
                        // time
                        byte[] modelData = model;

                        // Test both InferenceSession ctors that take PrePackedWeightsContainer instances
                        using (var session = new InferenceSession(model, options, prepackedWeightsContainer)) using (
                            var session2 = new InferenceSession(modelData, options, prepackedWeightsContainer))
                        {
                            var inputMeta = session.InputMetadata;
                            var container = new List<NamedOnnxValue>();
                            ...
                            foreach (var name in inputMeta.Keys)
                            {
                                Assert.Equal(typeof(float), inputMeta[name].ElementType);
                                Assert.True(inputMeta[name].IsTensor);
                                var tensor = new DenseTensor<float>(new float[] { 1, 2, 3, 4, 5, 6 },
                            ...
                            // Run inference with named inputs and outputs created with in Run()
                            using (var results =
                                       session.Run(container)) // results is an IReadOnlyList<NamedOnnxValue> container
                            {
                                foreach (var r in results)
                                {
                                    ValidateRunResultData(r.AsTensor<float>(), expectedOutput, expectedDimensions);
                            ...
                            // Run inference with named inputs and outputs created with in Run()
                            using (var results2 =
                                       session2.Run(container)) // results is an IReadOnlyList<NamedOnnxValue> container
                            {
    ...
    private void TestSharedAllocatorUsingCreateAndRegisterAllocator()
    {
        ...
        using (var memInfo = new OrtMemoryInfo(OrtMemoryInfo.allocatorCPU, OrtAllocatorType.ArenaAllocator, 0,
                                               OrtMemType.Default)) using (var arenaCfg =
                                                                               new OrtArenaCfg(0, -1, -1, -1))
        {
            ...
            using (var sessionOptions = new SessionOptions())
            {
                ...
                using (var session1 = new InferenceSession(model, sessionOptions)) using (
                    var session2 = new InferenceSession(
                        model, sessionOptions)) using (var session3 =
                                                           new InferenceSession(
                                                               model)) // Use the default SessionOptions instance
                {
                    // Input data
                    ...
                    // Run inference with named inputs and outputs created with in Run()
                    using (var results =
                               session1.Run(container)) // results is an IReadOnlyList<NamedOnnxValue> container
                    {
                        foreach (var r in results)
                        {
                    ...
                    // Run inference with named inputs and outputs created with in Run()
                    using (var results =
                               session3.Run(container)) // results is an IReadOnlyList<NamedOnnxValue> container
                    {
                        foreach (var r in results)
                        {
                            ValidateRunResultData(r.AsTensor<float>(), output, outputDims);
    ...
    internal static Tuple<InferenceSession, float[], DenseTensor<float>, float[]> OpenSessionSqueezeNet(
        int? deviceId = null)
    {
        var model = TestDataLoader.LoadModelFromEmbeddedResource("squeezenet.onnx");
#if USE_DML
        ...
        using (var option = new SessionOptions())
        {
            ...
            SetDllDirectory(null);
#elif USE_CUDA
        using (var option = (deviceId.HasValue) ? SessionOptions.MakeSessionOptionWithCudaProvider(deviceId.Value)
                                                : new SessionOptions())
        {
            if (!deviceId.HasValue)
            {
...
#elif USE_ROCM
        using (var option = (deviceId.HasValue) ? SessionOptions.MakeSessionOptionWithRocmProvider(deviceId.Value)
                                                : new SessionOptions())
        {
            if (!deviceId.HasValue)
            {
        ...
        using (var option = new SessionOptions())
        {
            option.AppendExecutionProvider_CPU(1);
#endif
            var session = (deviceId.HasValue) ? new InferenceSession(model, option) : new InferenceSession(model);
            ...
            return new Tuple<InferenceSession, float[], DenseTensor<float>, float[]>(session, inputData, tensor,
                                                                                     expectedOutput);
        }
    }
