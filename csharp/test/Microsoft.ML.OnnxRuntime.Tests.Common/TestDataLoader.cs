...
internal static class TestDataLoader
{
    ...
    static NamedOnnxValue LoadTensorPb(Onnx.TensorProto tensor, string nodeName, NodeMetadata nodeMeta)
    {
        ...
        if (!((protoDt == metaElementType) ||
            ...
            throw new InvalidDataException(
                $"For node: '{nodeName}' metadata expects: '{metaElementType}' but loaded loaded tensor type: '{protoDt}'");
        ...
        for (int i = 0; i < nodeMeta.Dimensions.Length; i++)
        {
            if ((nodeMeta.Dimensions[i] != -1) && (nodeMeta.Dimensions[i] != tensor.Dims[i]))
                throw new InvalidDataException(
                    $"Node: '{nodeName}' dimension at idx {i} is {nodeMeta.Dimensions}[{i}] " +
    ...
    internal static NamedOnnxValue CreateNamedOnnxValueFromTensorRawData(string nodeName, byte[] rawData,
        ...
        switch (elementType)
        {
            ...
            default:
                throw new NotImplementedException(
                    $"Tensors of type: {elementType} not currently supported by this function");
        }
    }
    ...
    internal static NamedOnnxValue LoadOnnxValueFromFilePb(string fullFilename, string nodeName, NodeMetadata nodeMeta)
    {
        ...
        using (var file = new FileStream(fullFilename, FileMode.Open, FileAccess.Read, FileShare.Read, readBufferSize))
        {
            switch (nodeMeta.OnnxValueType)
            {
                ...
                default:
                    throw new NotImplementedException(
                        $"Unable to load value type: {nodeMeta.OnnxValueType} not implemented");
            }
        }
    }

    private static void SequenceCheckMatchOnnxType(string nodeName, SequenceMetadata meta, OnnxValueType onnxType)
    {
        if (meta.ElementMeta.OnnxValueType == onnxType)
            return;

        throw new InvalidDataException($"Sequence node: '{nodeName}' " + $"has element type: '{onnxType}'" +
    ...
    internal static NamedOnnxValue CreateNamedOnnxValueFromSequence(Onnx.SequenceProto sequence, string nodeName,
    ...
    internal static NamedOnnxValue CastAndCreateFromMapKeys(string name, TensorElementType elementType,
                                                            IList<long> keys)
    {
        switch (elementType)
        {
            ...
            default:
                throw new NotImplementedException(
                    $"Tensors of type: " + elementType.ToString() +
    ...
    internal static NamedOnnxValue CreateNamedOnnxValueFromMap(Onnx.MapProto map, string nodeName,
                                                               NodeMetadata nodeMetadata)
    {
        ...
        //                            $"'{mapMeta.KeyDataType}', loaded from test data:
        //                            '{(TensorElementType)map.KeyType}'");
        // }
        ...
        //{
        //     keysTensor = CreateNamedOnnxValueFromStringTensor(map.StringKeys, nodeName, new int[] {
        //     map.StringKeys.Count });
        // }
        // else
        //{
    ...
    internal static NamedOnnxValue CreateNamedOnnxValueFromOptional(Onnx.OptionalProto optional, string nodeName,
                                                                    NodeMetadata nodeMetadata)
    {
        ...
        switch ((Onnx.OptionalProto.Types.DataType)optional.ElemType)
        {
            ...
            default:
                ...
                throw new NotImplementedException(
                    $"Unable to load '{nodeName}' optional element type of: {(Onnx.OptionalProto.Types.DataType)optional.ElemType} type");
        }
    }

    internal static NamedOnnxValue CreateNamedOnnxValueFromRawData<T>(string name, byte[] rawData, int elemWidth,
                                                                      int[] dimensions)
    {
        ...
        {
            using (var memSrcHandle = new Memory<byte>(rawData).Pin()) using (var memDstHandle =
                                                                                  new Memory<T>(typedArr).Pin())
            {
                unsafe
                {
                    Buffer.MemoryCopy(memSrcHandle.Pointer, memDstHandle.Pointer, typedArr.Length * elemWidth,
                                      rawData.Length);
                }
            }
        }
