    ...
    public NSString GetTestResults()
    {
        ...
        try
        {
            ...
            if (serializedResults != null)
                results = new NSString(serializedResults);
#endif
        }
        catch (Exception ex)
        {
...
public class OnnxRuntimeResultChannel : ITestListener, IResultChannel
{
    ...
    public string GetResults() => _resultProcessor.GetSerializedResults();

    public Task CloseChannel() => Task.CompletedTask;
    ...
    public void RecordResult(TestResultViewModel result) => _resultProcessor?.RecordResult(
        result.TestResultMessage, result.TestCase.TestCase, GetTestOutcomeFromTestState(result.TestCase.Result));
