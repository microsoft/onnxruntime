...
public partial class InferenceTest
{
    ...
    private void TestCUDAProviderOptions()
    {
        ...
        using (var cleanUp = new DisposableListTest<IDisposable>())
        {
            ...
            float[] inputData = TestDataLoader.LoadTensorFromFile(
    ...
    private static Func<DirectoryInfo, IEnumerable<DirectoryInfo>> getOpsetDirectories =
        delegate(DirectoryInfo modelsDirInfo)
    {
    ...
    private static Dictionary<string, string> GetSkippedModels(DirectoryInfo modelsDirInfo)
    {
        var skipModels = new Dictionary<string, string>() {
            ...
            { "fp16_coreml_LinearRegression_NYCTaxi",
              "Error in Node:featureVectorizer : No Op registered for FeatureVectorizer with domain_version of 1" },
            { "test_mnist",
              "Does not run in opset9, runs in other opsets. The model runs but I don't have a data set to debug output locally. Tensors of type ElementType not currently supported in the LoadTensorFromFile" },
            ...
            { "test_castlike_BFLOAT16_to_FLOAT",
              "Length is expected to be equal to Count (metadata and expected data mismatch) " },
            { "test_castlike_BFLOAT16_to_FLOAT_expanded",
              "Length is expected to be equal to Count metadata and expected data mismatch" },
            { "test_castlike_FLOAT_to_BFLOAT16",
              "Length is expected to be equal to Count. Testdata dims length do not match that of model metadata" },
            ...
            { "test_clip_default_int8_max_expanded",
              "Could not find an implementation for Less(13) nodeMeta with name ''" },
            ...
            { "test_optional_has_element_empty_optional_input",
              "OptionalProto test metadata. Unable to load 'optional_input' optional element type of: Undefined type" },
            { "test_loop13_seq",
              "3rd input is an empty sequence. Ort API does not tolerate empty seq: Number of values should be at least 1" },
        ...
        if (isMlOpsDisabled)
        {
            foreach (var opsetDir in getOpsetDirectories(modelsDirInfo))
            {
                foreach (var modelDir in opsetDir.EnumerateDirectories())
                {
                    ...
                    if (modelDirName.StartsWith("scikit_") || modelDirName.StartsWith("libsvm_") ||
                        modelDirName.StartsWith("coreml_") || modelDirName.StartsWith("keras2coreml_") ||
        ...
        if (System.Environment.Is64BitProcess == false)
        {
            ...
            skipModels["tf_pnasnet_large"] =
                "Get preallocated buffer for initializer ConvBnFusion_BN_B_cell_5/comb_iter_1/left/bn_sep_7x7_1/beta:0_203 failed";
            skipModels["tf_nasnet_large"] =
                "Get preallocated buffer for initializer ConvBnFusion_BN_B_cell_11/beginning_bn/beta:0_331 failed";
        ...
        foreach (var opsetDir in getOpsetDirectories(modelsDirInfo))
        {
            foreach (var modelDir in opsetDir.EnumerateDirectories())
            {
                if (skipModels.ContainsKey(modelDir.Name) ||
                    ...
                    modelDir.Name.Contains("qdq", StringComparison.OrdinalIgnoreCase))
                {
                    // Console.WriteLine("Model {0} is skipped due to the error: {1}", modelDir.FullName,
                    // skipModels[modelDir.Name]);
                    yield return new object[] { modelDir.Parent.FullName, modelDir.Name };
                }
            }
        }
    ...
    private string MatchInputOutputWithFile(string fileName, InferenceSession session, bool input,
                                            out NodeMetadata result)
    {
        ...
        try
        {
            ...
            else
            {
                throw new InvalidDataException(
                    $"Filename '{fileName}' {inpOut} number '{num}' is out of range for '{names.Count}' {inpOut}(s)");
            }
        }
        catch (Exception)
        {
    ...
    [MemberData(nameof(GetSkippedModelForTest),
                Skip = "Skipped due to Error, please fix the error and enable the test")]
    private void TestPreTrainedModels(string opsetDir, string modelName)
    {
        ...
        try
        {
            ...
            else
            {
                ...
                throw new Exception(
                    $"Opset {opset} Model {modelName}. Can't determine model file name. Found these :{modelNamesList}");
            }
                    ...
                    using (var resultCollection = session.Run(inputContainer))
                    {
                        foreach (var result in resultCollection)
                        {
                            ...
                            switch (outputValue.ValueType)
                            {
                                ...
                                default:
                                    Assert.True(
                                        false, $"TestPreTrainedModels cannot handle Onnxtype: {outputValue.ValueType}");
                                    break;
                            }
                        }
                    }
        ...
        catch (Exception ex)
        {
                    ...
                    "ONNX Runtime only *guarantees* support for models stamped with official released onnx opset versions"))
            {
                ...
                // not supported in ORT yet. In order to force these tests to run set env var
                // ALLOW_RELEASED_ONNX_OPSET_ONLY=0
                output.WriteLine(
                    "Skipping the model test as the latest ONNX opset is not supported yet. Error Message: " + msg);
            }
            else
            {
    ...
    private void VerifySequenceResults(NamedOnnxValue result, NamedOnnxValue expectedValue, NodeMetadata metaData)
    {
        ...
        foreach (var (resultItem, expectedItem) in resultSequence.Zip(expectedSequence, (r, e) => (r, e)))
        {
            ...
            switch (resultItem.ValueType)
            {
                ...
                default:
                    Assert.True(false,
                                "VerifySequenceResults cannot handle Onnxtype: " + resultItem.ValueType.ToString());
                    break;
            }
    ...
    private void VerifyTensorResults(TensorElementType elementType, NamedOnnxValue result, NamedOnnxValue outputValue)
    {
        switch (elementType)
        {
            ...
            case TensorElementType.Float16:
                Assert.Equal(result.AsTensor<Float16>(), outputValue.AsTensor<Float16>(),
                             new Float16Comparer { tolerance = 2 });
                break;
            case TensorElementType.BFloat16:
                Assert.Equal(result.AsTensor<BFloat16>(), outputValue.AsTensor<BFloat16>(),
            ...
            default:
                Assert.True(false,
                            "TestPreTrainedModels does not yet support output of type: " + elementType.ToString());
                break;
        }
    }
    ...
    private void TestRegisterCustomOpLibrary()
    {
        using (var option = new SessionOptions())
        {
            ...
            using (var session = new InferenceSession(modelPath, option))
            {
                ...
                inputContainer.Add(NamedOnnxValue.CreateFromTensor<float>(
                    "input_1", new DenseTensor<float>(new float[] { 1.1f, 2.2f, 3.3f, 4.4f, 5.5f, 6.6f, 7.7f, 8.8f,
                                                                    9.9f, 10.0f, 11.1f, 12.2f, 13.3f, 14.4f, 15.5f },
                                                      new int[] { 3, 5 })));

                inputContainer.Add(NamedOnnxValue.CreateFromTensor<float>(
                    "input_2", new DenseTensor<float>(new float[] { 15.5f, 14.4f, 13.3f, 12.2f, 11.1f, 10.0f, 9.9f,
                                                                    8.8f, 7.7f, 6.6f, 5.5f, 4.4f, 3.3f, 2.2f, 1.1f },
                                                      new int[] { 3, 5 })));
                ...
                using (var result = session.Run(inputContainer))
                {
                    ...
                    var expectedOut = new DenseTensor<int>(
                        new int[] { 17, 17, 17, 17, 17, 17, 18, 18, 18, 17, 17, 17, 17, 17, 17 }, new int[] { 3, 5 });
    ...
    private void VerifyNativeMethodsExist()
    {
        ...
        if (!RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
            return;
        var entryPointNames = new[] { "OrtGetApiBase",
