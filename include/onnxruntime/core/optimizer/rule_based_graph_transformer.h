...
class RuleBasedGraphTransformer : public GraphTransformer {
 public:
  ...
  const std::vector<std::reference_wrapper<RewriteRule>>* GetRewriteRulesForOpType(const std::string& op_type) const {
  ...
  const std::vector<std::reference_wrapper<RewriteRule>>* GetAnyOpRewriteRules() const {
 ...
 protected:
  ...
  common::Status ApplyRulesOnNode(Graph& graph, Node& node,
                                  const std::vector<std::reference_wrapper<RewriteRule>>& rules,
 ...
 private:
  ...
  std::unordered_map<std::string, std::vector<std::reference_wrapper<RewriteRule>>> op_type_to_rules_;
  // Rules that will be evaluated regardless of the op type of the node.
  std::vector<std::reference_wrapper<RewriteRule>> any_op_type_rules_;
