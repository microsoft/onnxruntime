...
typedef enum OrtCompileApiFlags {
  ...
  // Force compilation to return an error (ORT_FAIL) if the output model has no compiled nodes.
  // Otherwise, a model with basic optimizations (ORT_ENABLE_BASIC) is still generated even if no nodes were compiled.
  OrtCompileApiFlags_ERROR_IF_NO_COMPILED_NODES = 1 << 0,
...
struct OrtCompileApi {
  ...
  /** \brief Sets enabled options via an unsigned integer that is set to the bitwise OR of flags in OrtCompileApiFlags.
   *
   * \param[in] model_compile_options The OrtModelCompilationOptions instance.
   * \param[in] flags bitwise OR of enabled options. Refer to OrtCompileApiFlags.
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   *
   * \since Version 1.23.
   */
  ORT_API2_STATUS(ModelCompilationOptions_SetBoolOptions, _In_ OrtModelCompilationOptions* model_compile_options,
                  size_t flags);
};
