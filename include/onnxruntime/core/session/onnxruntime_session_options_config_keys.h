...
/// Requires the built-in CUDA EP with enable_cuda_graph=1. Uses the existing whole-session replay path when
...
/// Memory patterns are disabled only in partitioned mode. Control flow and plugin EPs are not supported;
/// partitioned mode also excludes shared/external allocators and parallel execution.
