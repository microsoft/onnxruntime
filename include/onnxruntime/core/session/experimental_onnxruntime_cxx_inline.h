...
inline void Session::Run(const std::vector<std::string>& input_names, const std::vector<Ort::Value>& input_values,
  ...
  Ort::Session::Run(run_options, input_names_.data(), input_values.data(), input_names_count, output_names_.data(),
                    output_values.data(), output_names_count);
}
...
inline Ort::Value Value::CreateTensor(void* p_data, size_t p_data_byte_count, const std::vector<int64_t>& shape,
