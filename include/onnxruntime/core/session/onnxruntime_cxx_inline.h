...
inline ArenaCfg::ArenaCfg(size_t max_mem, int arena_extend_strategy, int initial_chunk_size_bytes,
  ...
  ThrowOnError(GetApi().CreateArenaCfg(max_mem, arena_extend_strategy, initial_chunk_size_bytes,
                                       max_dead_bytes_per_chunk, &p_));
}
...
inline Env& Env::CreateAndRegisterAllocatorV2(const std::string& provider_type, const OrtMemoryInfo* mem_info,
  ...
  ThrowOnError(GetApi().CreateAndRegisterAllocatorV2(p_, provider_type.c_str(), mem_info, arena_cfg,
...
inline std::string ConstSessionOptionsImpl<T>::GetConfigEntry(const char* config_key) const {
  ...
  Ort::ThrowOnError(GetApi().GetSessionConfigEntry(p_, config_key, nullptr, &size));

  std::string out;
  out.resize(size);
  Ort::ThrowOnError(GetApi().GetSessionConfigEntry(p_, config_key, &out[0], &size));
...
inline bool ConstSessionOptionsImpl<T>::HasConfigEntry(const char* config_key) const {
  int out = 0;
  Ort::ThrowOnError(GetApi().HasSessionConfigEntry(p_, config_key, &out));
...
inline std::string ConstSessionOptionsImpl<T>::GetConfigEntryOrDefault(const char* config_key,
  ...
  return GetConfigEntry(config_key);
}
...
template <typename T>
inline SessionOptionsImpl<T>&
SessionOptionsImpl<T>::SetGraphOptimizationLevel(GraphOptimizationLevel graph_optimization_level) {
...
template <typename T>
inline SessionOptionsImpl<T>&
SessionOptionsImpl<T>::SetOptimizedModelFilePath(const ORTCHAR_T* optimized_model_filepath) {
...
template <typename T>
inline SessionOptionsImpl<T>&
SessionOptionsImpl<T>::AppendExecutionProvider_CUDA(const OrtCUDAProviderOptions& provider_options) {
...
template <typename T>
inline SessionOptionsImpl<T>&
SessionOptionsImpl<T>::AppendExecutionProvider_CUDA_V2(const OrtCUDAProviderOptionsV2& provider_options) {
...
template <typename T>
inline SessionOptionsImpl<T>&
SessionOptionsImpl<T>::AppendExecutionProvider_ROCM(const OrtROCMProviderOptions& provider_options) {
...
template <typename T>
inline SessionOptionsImpl<T>&
SessionOptionsImpl<T>::AppendExecutionProvider_TensorRT(const OrtTensorRTProviderOptions& provider_options) {
...
template <typename T>
inline SessionOptionsImpl<T>&
SessionOptionsImpl<T>::AppendExecutionProvider_TensorRT_V2(const OrtTensorRTProviderOptionsV2& provider_options) {
...
template <typename T>
inline SessionOptionsImpl<T>&
SessionOptionsImpl<T>::AppendExecutionProvider_MIGraphX(const OrtMIGraphXProviderOptions& provider_options) {
...
template <typename T>
inline SessionOptionsImpl<T>&
SessionOptionsImpl<T>::AppendExecutionProvider_CANN(const OrtCANNProviderOptions& provider_options) {
...
template <typename T>
inline SessionOptionsImpl<T>&
SessionOptionsImpl<T>::AppendExecutionProvider_Dnnl(const OrtDnnlProviderOptions& provider_options) {
  ThrowOnError(GetApi().SessionOptionsAppendExecutionProvider_Dnnl(p_, &provider_options));
  return *this;
}
...
template <typename T>
inline SessionOptionsImpl<T>&
SessionOptionsImpl<T>::SetCustomCreateThreadFn(OrtCustomCreateThreadFn ort_custom_create_thread_fn) {
...
template <typename T>
inline SessionOptionsImpl<T>&
SessionOptionsImpl<T>::SetCustomThreadCreationOptions(void* ort_custom_thread_creation_options) {
...
template <typename T>
inline SessionOptionsImpl<T>&
SessionOptionsImpl<T>::SetCustomJoinThreadFn(OrtCustomJoinThreadFn ort_custom_join_thread_fn) {
...
template <typename T>
inline SessionOptionsImpl<T>&
SessionOptionsImpl<T>::AppendExecutionProvider_OpenVINO(const OrtOpenVINOProviderOptions& provider_options) {
...
template <typename T>
inline SessionOptionsImpl<T>&
SessionOptionsImpl<T>::RegisterCustomOpsLibrary(const ORTCHAR_T* library_name,
                                                const CustomOpConfigs& custom_op_configs) {
  // Add custom op config entries before registering the custom op library.
  ...
  ThrowOnError(GetApi().RegisterCustomOpsLibrary_V2(p_, library_name));
  return *this;
}

template <typename T>
inline SessionOptionsImpl<T>&
SessionOptionsImpl<T>::RegisterCustomOpsUsingFunction(const char* registration_function_name) {
  ThrowOnError(GetApi().RegisterCustomOpsUsingFunction(p_, registration_function_name));
  return *this;
}

template <typename T>
inline SessionOptionsImpl<T>& SessionOptionsImpl<T>::UseDeterministicCompute() {
  ThrowOnError(GetApi().SessionOptionsUseDeterministicCompute(p_));
  return *this;
}
...
template <typename T>
inline AllocatedStringPtr
ConstSessionImpl<T>::GetOverridableInitializerNameAllocated(size_t index, OrtAllocator* allocator) const {
  char* out;
...
SessionImpl<T>::Run(const RunOptions& run_options,
                    const char* const* input_names, const Value* input_values, size_t input_count,
...
inline void SessionImpl<T>::Run(const RunOptions& run_options,
  ...
  ThrowOnError(GetApi().Run(p_, run_options, input_names, ort_input_values, input_count,
...
template <typename T>
inline void SessionImpl<T>::RunAsync(const RunOptions& run_options,
                                     const char* const* input_names, const Value* input_values, size_t input_count,
                                     const char* const* output_names, Value* output_values, size_t output_count,
                                     RunAsyncCallbackFn callback, void* user_data) {
  auto ort_input_values = reinterpret_cast<const OrtValue* const*>(input_values);
  auto ort_output_values = reinterpret_cast<OrtValue**>(output_values);
  ThrowOnError(GetApi().RunAsync(p_, run_options, input_names,
...
inline CustomOpConfigs& CustomOpConfigs::AddConfig(const char* custom_op_name,
...
inline Session::Session(const Env& env, const ORTCHAR_T* model_path, const SessionOptions& options,
  ...
  ThrowOnError(GetApi().CreateSessionWithPrepackedWeightsContainer(env, model_path, options,
...
inline Session::Session(const Env& env, const void* model_data, size_t model_data_length,
  ...
  ThrowOnError(GetApi().CreateSessionFromArrayWithPrepackedWeightsContainer(
      env, model_data, model_data_length, options, prepacked_weights_container, &p_));
}
...
inline AllocatedStringPtr
ModelMetadata::LookupCustomMetadataMapAllocated(const char* key, OrtAllocator* allocator) const {
  char* out;
  ThrowOnError(GetApi().ModelMetadataLookupCustomMetadataMap(p_, allocator, key, &out));
  return AllocatedStringPtr(out, detail::AllocatedFree(allocator));
}

inline std::vector<AllocatedStringPtr>
ModelMetadata::GetCustomMetadataMapKeysAllocated(OrtAllocator* allocator) const {
...
inline TypeInfo OptionalTypeInfoImpl<T>::GetOptionalElementType() const {
  OrtTypeInfo* info;
  ThrowOnError(GetApi().GetOptionalContainedTypeInfo(p_, &info));
...
inline ConstOptionalTypeInfo TypeInfoImpl<T>::GetOptionalTypeInfo() const {
  ...
  ThrowOnError(GetApi().CastTypeInfoToOptionalTypeInfo(p_, &info));
...
inline std::string ConstValueImpl<T>::GetStringTensorElement(size_t element_index) const {
  ...
  ThrowOnError(GetApi().GetStringTensorElementLength(p_, element_index, &buffer_length));

  std::string s;
  s.resize(buffer_length);
  ThrowOnError(GetApi().GetStringTensorElement(p_, buffer_length, element_index, &s[0]));
  return s;
}

template <typename T>
inline void ConstValueImpl<T>::GetStringTensorContent(void* buffer, size_t buffer_length,
                                                      size_t* offsets, size_t offsets_count) const {
...
template <typename T>
inline TensorTypeAndShapeInfo
ConstValueImpl<T>::GetSparseTensorIndicesTypeShapeInfo(OrtSparseIndicesFormat indices_format) const {
  OrtTensorTypeAndShapeInfo* output;
...
template <typename T>
template <typename R>
inline const R* ConstValueImpl<T>::GetSparseTensorIndicesData(OrtSparseIndicesFormat indices_format,
...
inline char* ValueImpl<T>::GetResizedStringTensorElementBuffer(size_t index, size_t buffer_length) {
  char* result;
  ThrowOnError(GetApi().GetResizedStringTensorElementBuffer(p_, index, buffer_length, &result));
  return result;
}
...
void ValueImpl<T>::FillSparseTensorCsr(const OrtMemoryInfo* data_mem_info,
  ...
  ThrowOnError(GetApi().FillSparseTensorCsr(p_, data_mem_info,
                                            values.values_shape, values.values_shape_len, values.data.p_data,
...
void ValueImpl<T>::FillSparseTensorBlockSparse(const OrtMemoryInfo* data_mem_info,
  ...
  ThrowOnError(GetApi().FillSparseTensorBlockSparse(p_, data_mem_info,
                                                    values.values_shape, values.values_shape_len, values.data.p_data,
...
inline Logger::Logger(const OrtLogger* logger) : logger_(logger) {
  Ort::ThrowOnError(GetApi().Logger_GetLoggingSeverityLevel(logger_, &cached_severity_level_));
}
...
inline Logger KernelContext::GetLogger() const {
  ...
  ThrowOnError(GetApi().KernelContext_GetLogger(ctx_, &out));
  return Logger{out};
}
...
inline std::string KernelInfoImpl<T>::GetInputName(size_t index) const {
  ...
  // Feed nullptr for the data buffer to query the true size of the string value
  Ort::ThrowOnError(GetApi().KernelInfo_GetInputName(p_, index, nullptr, &size));

  std::string out;
  out.resize(size);
  Ort::ThrowOnError(GetApi().KernelInfo_GetInputName(p_, index, &out[0], &size));
  out.resize(size - 1);  // remove the terminating character '\0'
...
inline std::string KernelInfoImpl<T>::GetOutputName(size_t index) const {
  ...
  // Feed nullptr for the data buffer to query the true size of the string value
  Ort::ThrowOnError(GetApi().KernelInfo_GetOutputName(p_, index, nullptr, &size));

  std::string out;
  out.resize(size);
  Ort::ThrowOnError(GetApi().KernelInfo_GetOutputName(p_, index, &out[0], &size));
  out.resize(size - 1);  // remove the terminating character '\0'
...
inline TypeInfo KernelInfoImpl<T>::GetInputTypeInfo(size_t index) const {
  ...
  ThrowOnError(GetApi().KernelInfo_GetInputTypeInfo(p_, index, &out));
  return TypeInfo{out};
}
...
inline TypeInfo KernelInfoImpl<T>::GetOutputTypeInfo(size_t index) const {
  OrtTypeInfo* out = nullptr;
  ThrowOnError(GetApi().KernelInfo_GetOutputTypeInfo(p_, index, &out));
  return TypeInfo{out};
}
...
inline Value KernelInfoImpl<T>::GetTensorAttribute(const char* name, OrtAllocator* allocator) const {
  OrtValue* out = nullptr;
  ThrowOnError(GetApi().KernelInfoGetAttribute_tensor(p_, name, allocator, &out));
  return Value{out};
}
...
inline ConstValue KernelInfoImpl<T>::GetTensorConstantInput(size_t index, int* is_constant) const {
  const OrtValue* out = nullptr;
  ThrowOnError(GetApi().KernelInfoGetConstantInput_tensor(p_, index, is_constant, &out));
  return ConstValue{out};
}
...
inline std::string KernelInfoImpl<T>::GetNodeName() const {
  ...
  // Feed nullptr for the data buffer to query the true size of the string value
  Ort::ThrowOnError(GetApi().KernelInfo_GetNodeName(p_, nullptr, &size));

  std::string out;
  out.resize(size);
  Ort::ThrowOnError(GetApi().KernelInfo_GetNodeName(p_, &out[0], &size));
  out.resize(size - 1);  // remove the terminating character '\0'
...
inline Logger KernelInfoImpl<T>::GetLogger() const {
  const OrtLogger* out = nullptr;
  ThrowOnError(GetApi().KernelInfo_GetLogger(p_, &out));
  return Logger{out};
}
