...
struct RocmContext : public CustomOpContext {
  ...
  void Init(const OrtKernelContext& kernel_ctx) {
    ...
    status = ort_api.KernelContext_GetResource(&kernel_ctx, ORT_ROCM_RESOURCE_VERSION, RocmResource::hip_stream_t, &resource);
    if (status) {
    ...
    resource = {};
    status = ort_api.KernelContext_GetResource(&kernel_ctx, ORT_ROCM_RESOURCE_VERSION, RocmResource::miopen_handle_t, &resource);
    if (status) {
    ...
    resource = {};
    status = ort_api.KernelContext_GetResource(&kernel_ctx, ORT_ROCM_RESOURCE_VERSION, RocmResource::rocblas_handle_t, &resource);
    if (status) {
