...
class ModelMetadefIdGenerator {
 public:
   ...
   */
  static int GenerateMetaDefId(const onnxruntime::GraphViewer& graph_viewer, HashValue& model_hash);

 private:
  inline static std::unordered_map<HashValue, HashValue> main_graph_hash_;  // map graph instance hash to model contents hash
  inline static std::unordered_map<HashValue, int> model_metadef_id_;       // current unique id for model
};
