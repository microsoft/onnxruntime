...
struct WorkspaceRequirement {
  ...
  // with addressable allocations, the allocation base must also satisfy this alignment. 0 means
  // "the allocator's default alignment is sufficient" - true for every kernel using this API today,
  // since CUDA's allocator already guarantees >= 256-byte aligned allocations. Reserved for a future
  // kernel with a stricter/exotic alignment need, or a future shared-arena packer that co-locates
  // multiple kernels' slots (see the "shared per-node memory-budget tracker" direction in issue #29775).
