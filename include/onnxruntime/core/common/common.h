...
#define ORT_REMOVE_FILE_LOCATION

#if defined(ORT_REMOVE_FILE_LOCATION)
#define ORT_CURRENT_FILE ""
#define ORT_CURRENT_LINE 0
#else
#define ORT_CURRENT_FILE __FILE__
#define ORT_CURRENT_LINE __LINE__
#endif
...
#define ORT_WHERE \
  ::onnxruntime::CodeLocation(ORT_CURRENT_FILE, ORT_CURRENT_LINE, __FUNCTION__)

#define ORT_WHERE_WITH_STACK \
  ::onnxruntime::CodeLocation(ORT_CURRENT_FILE, ORT_CURRENT_LINE, __PRETTY_FUNCTION__, ::onnxruntime::GetStackTrace())
...
#define ORT_RETURN_IF_ERROR_SESSIONID(expr, session_id)                                                      \
  do {                                                                                                       \
    auto _status = (expr);                                                                                   \
    if ((!_status.IsOK())) {                                                                                 \
      ::onnxruntime::LogRuntimeError(session_id, _status, ORT_CURRENT_FILE, __FUNCTION__, ORT_CURRENT_LINE); \
      return _status;                                                                                        \
    }                                                                                                        \
  } while (0)
...
#define ORT_THROW_IF_ERROR(expr)                                                                    \
  do {                                                                                              \
    auto _status = (expr);                                                                          \
    if ((!_status.IsOK())) {                                                                        \
      ::onnxruntime::LogRuntimeError(0, _status, ORT_CURRENT_FILE, __FUNCTION__, ORT_CURRENT_LINE); \
      ORT_THROW(_status);                                                                           \
    }                                                                                               \
  } while (0)
