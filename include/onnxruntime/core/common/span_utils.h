...
// Use as_span for less typing on any container including initializer list to create a span
...
// 
//   Example: as_span({1, 2, 3}) results in gsl::span<const int>
// 
// The above would not convert std::initializer_list<int> to gsl::span<const int64_t>
// However, this is not as critical as we use TensorShapeVector and InlinedVector<int64_t>
// that have small buffer optimizations and spans are automatically created from those.
// 
// We can also do std::array<int64_t, 3>{1, 2, 3}
...
namespace details {
template <class P>
constexpr auto AsSpanImpl(P* p, std::size_t s) {
