 ...
 * @param input_count [out] a pointer to a uint32_t variable to accept input_count.
 * @param output_count [out] a pointer to a uint32_t variable to accept output_count.
 * @returns ORT error code. If not zero, call OrtGetLastError() to get detailed error message.
 */
 ...
 * calling OrtFree().
 */
char* EMSCRIPTEN_KEEPALIVE OrtGetInputName(ort_session_handle_t session, uint32_t index);
/**
 ...
 * calling OrtFree().
 */
char* EMSCRIPTEN_KEEPALIVE OrtGetOutputName(ort_session_handle_t session, uint32_t index);
 ...
 */
ort_tensor_handle_t EMSCRIPTEN_KEEPALIVE OrtCreateTensor(int data_type, void* data, uint32_t data_length, uint32_t* dims, uint32_t dims_length, int data_location);
 ...
 * @returns ORT error code. If not zero, call OrtGetLastError() to get detailed error message.
 */
int EMSCRIPTEN_KEEPALIVE OrtGetTensorData(ort_tensor_handle_t tensor, uint32_t* data_type, void** data, uint32_t** dims, uint32_t* dims_length);
 ...
 */
ort_run_options_handle_t EMSCRIPTEN_KEEPALIVE OrtCreateRunOptions(uint32_t log_severity_level,
                                                                  uint32_t log_verbosity_level,
                                                                  bool terminate,
                                                                  const char* tag);
 ...
 */
ort_training_checkpoint_handle_t EMSCRIPTEN_KEEPALIVE OrtTrainingLoadCheckpoint(void* checkpoint_data_buffer, uint32_t checkpoint_size);
...
int EMSCRIPTEN_KEEPALIVE OrtTrainingRunTrainStep(ort_training_session_handle_t training_handle,
                                                 ort_tensor_handle_t* inputs, uint32_t input_count,
                                                 ort_tensor_handle_t* outputs,
                                                 uint32_t output_count,
