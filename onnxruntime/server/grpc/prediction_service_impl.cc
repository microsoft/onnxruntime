...
::grpc::Status PredictionServiceImpl::Predict(::grpc::ServerContext* context, const ::onnxruntime::server::PredictRequest* request, ::onnxruntime::server::PredictResponse* response) {
  ...
  MetricRegistry::GetInstance()->totalGRPCRequests->Add({}).Increment();
  // Log Request size
  MetricRegistry::GetInstance()->grpcRequestSize->
  ...
  if (!status.ok()) {
    ...
    MetricRegistry::GetInstance()->totalGRPCErrors->Add({
  ...
  MetricRegistry::GetInstance()->inferenceTimer->Add({{"name", "default"}, {"version", "1"}},
