...
class ORTNonBlockingThreadPool : public Eigen::ThreadPoolInterface {
 public:
  typedef typename Environment::Task Task;
  typedef RunQueue<Task, 1024> Queue;

  ORTNonBlockingThreadPool(int num_threads, Environment env = Environment())
      : env_(env),
  ...
  void Schedule(std::packaged_task<void()>& fn) {
  ...
  int CurrentThreadId() const final {
    const PerThread* pt =
        const_cast<ORTNonBlockingThreadPool*>(this)->GetPerThread();
    if (pt->pool == this) {
      return pt->thread_id;
    } else {
      return -1;
 ...
 private:
  ...
  struct PerThread {
    ...
    ORTNonBlockingThreadPool* pool;  // Parent pool, or null for normal threads.
...
typedef ORTNonBlockingThreadPool<StlThreadEnvironment> NonBlockingThreadPool;
