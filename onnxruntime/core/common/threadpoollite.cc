...
ThreadPoolLite4::~ThreadPoolLite4() {
  exit_ = true;
  for (std::thread& t : sub_threads_) {
    t.join();
  }
...
void ThreadPoolLite4::ParallelFor(std::ptrdiff_t total, const TensorOpCost& cost, const Fn& fn) {
  if (0 == total) {
    return;
  }
  std::ptrdiff_t block_size = GetBlockSize(total, cost, num_sub_threads_ + 1);
...
void ThreadPoolLite4::Schedule(SchdFn) {
  ORT_ENFORCE(false);
}

void ThreadPoolLite4::StartProfiling() {
  profiler_.Start();
}

std::string ThreadPoolLite4::StopProfiling() {
  return profiler_.Stop();
}

void ThreadPoolLite4::MainLoop(int idx) {
