...
Status WebGpuContext::Run(ComputeContext& context, const ProgramBase& program) {
  ...
  auto append_shape_uniforms = [&shape_uniforms, program_artifact](size_t i, const TensorShape& shape) {
    int expected_rank = program_artifact->shape_uniform_ranks[i];
    if (expected_rank > 0) {
      ...
      std::vector<uint32_t> dims(static_cast<size_t>(expected_rank));
      std::vector<uint32_t> stride(static_cast<size_t>(expected_rank - 1));
      for (int j = 0; j < expected_rank; ++j) {
        dims[j] = gsl::narrow<uint32_t>(shape[j]);
        if (j < expected_rank - 1) {
          stride[j] = gsl::narrow<uint32_t>(shape.SizeFromDimension(static_cast<size_t>(j + 1)));
        }
      }
