...
class ProgramBase {
 ...
 private:
  ...
  friend class detail0::ProgramWrapper;
};
...
template <typename T>
class Program : public detail0::ProgramWrapper {
 public:
  template <typename... Args>
  Program(Args&&... args) : detail0::ProgramWrapper{std::forward<Args>(args)..., GetMetadata()} {}

  static ProgramMetadata GetMetadata() {
    ProgramMetadata metadata;
    if constexpr (detail0::DerivedProgramClassTypeCheck<T>::has_constants) {
      constexpr const ProgramConstant* ptr = T::constants.data();
      constexpr size_t len = T::constants.size();

      static_assert(detail0::DerivedProgramClassTypeCheck<T>::has_constants_with_correct_type,
    ...
    if constexpr (detail0::DerivedProgramClassTypeCheck<T>::has_overridable_constants) {
      ...
      static_assert(detail0::DerivedProgramClassTypeCheck<T>::has_overridable_constants_with_correct_type,
    ...
    if constexpr (detail0::DerivedProgramClassTypeCheck<T>::has_uniform_variables) {
      ...
      static_assert(detail0::DerivedProgramClassTypeCheck<T>::has_uniform_variables_with_correct_type,
...
#define WEBGPU_PROGRAM_DEFINE_(identifier, T, ...)             \
  ...
  static constexpr const auto identifier =                     \
      onnxruntime::webgpu::detail0::_to_std_array(identifier##_own)

#define WEBGPU_PROGRAM_EXTEND_(identifier, T, BASE, ...)       \
  static constexpr const T identifier##_own[] = {__VA_ARGS__}; \
  static constexpr const auto identifier =                     \
      onnxruntime::webgpu::detail0::_concat2(BASE::identifier, identifier##_own)
