...
struct WebGpuExecutionProviderConfig {
  ...
  bool use_multi_rotary_cache_concat{false}; // multi rotary embedding cache concatenation is disabled by default
  uint32_t multi_rotary_cache_concat_offset{4096}; // offset for concatenated multi rotary cache (default 4096 for Phi models)
...
class WebGpuExecutionProvider : public IExecutionProvider {
 public:
  ...
  bool UseMultiRotaryCacheConcat() const { return use_multi_rotary_cache_concat_; }
 ...
 private:
  ...
  bool use_multi_rotary_cache_concat_ = false;
  uint32_t multi_rotary_cache_concat_offset_ = 4096;
