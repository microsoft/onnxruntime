...
Status ComputeMatMul(ComputeContext* context,
  ...
  if (need_split_k) {
    ...
    ORT_ENFORCE(dim_b_outer % 4 == 0, "Split-K MatMul requires dim_b_outer to be a multiple of 4.");
    ...
    // Each split writes to its own slot of `partials` rather than accumulating into the shared
    // output, so there are no atomics and the summation order is fixed by index. Bias is applied by
    // the reduction, which replaces the pre-fill dispatch the atomic path needed.
    use_bias_in_matmul = false;

    const uint64_t out_elems = static_cast<uint64_t>(batch_size) * dim_a_outer * (dim_b_outer / components);
    const uint64_t scratch_elems = SplitKScratchElements(splits_per_batch, out_elems);
    const uint64_t scratch_scalars = scratch_elems * components;
    ORT_ENFORCE(scratch_scalars <= static_cast<uint64_t>(std::numeric_limits<int64_t>::max()),
...
MatMulSplitKReduceProgram CreateMatMulSplitKReduceProgram(
  ...
  // Must stay last among uniform appends: definitions pair with values by index, not by name. Adding
  // the bias *input* below is safe because inputs are a separate list.
  AppendActivationUniformsData(activation, program);
