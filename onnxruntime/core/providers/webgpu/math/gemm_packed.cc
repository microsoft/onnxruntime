...
Status GemmProgram::GenerateShaderCode(ShaderHelper& shader) const {
  ...
  if (need_split_k) {
    // With Split-K the bound output is the partial-sum scratch buffer, not Y. `beta * C` is applied by
    // `MatMulSplitKReduceProgram` after the partials are summed.
...
Status ApplyGemmPacked(const Tensor* a,
  ...
  if (need_split_k) {
    ORT_ENFORCE(N % 4 == 0, "Split-K GEMM requires N to be a multiple of 4.");
    ...
    // so there are no atomics and the summation order is fixed by index in the reduction below.
    // `beta * C` is applied there too, which is why the pre-fill the atomic path needed is gone.
    const uint64_t out_elems = static_cast<uint64_t>(M) * (N / output_components);
    const uint64_t scratch_elems = SplitKScratchElements(splits, out_elems);
