...
Status MatMulProgram::GenerateShaderCode(ShaderHelper& shader) const {
  ...
  if (need_split_k) {
    // With Split-K the bound output is the partial-sum scratch buffer, not the real output. Bias and
    // activation are applied by `MatMulSplitKReduceProgram` after the partials are summed.
...
Status MatMulSplitKReduceProgram::GenerateShaderCode(ShaderHelper& shader) const {
  ...
  // Bias (or `beta * C`) and the activation are applied by the same emitters the non-Split-K path
  // uses, so there is one implementation of that arithmetic rather than two.
  const std::string apply_activation = GetActivationSnippet(activation_, "output_value_t", "output_element_t");
  if (is_gemm_) {
    // GEMM has no fused activation; `MatMulWriteFnSourceForGemm` applies `beta * C` only.
  ...
  // Summation order is fixed by index rather than by the order pass-1 workgroups retire, which is
  // what makes the result reproducible. The accumulator is f32 even for f16 output: rounding once
  // at the end is more accurate than accumulating in f16, and costs nothing here.
  const std::string acc_type = MakeScalarOrVectorType(static_cast<int>(output_components_), "f32");
  // Read through GetByOffset, matching how pass 1 writes, so both stay correct if the scratch
  // buffer is ever split across several bindings.
