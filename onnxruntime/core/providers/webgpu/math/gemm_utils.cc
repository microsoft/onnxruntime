...
void HandleMatMulWriteToSplitKPartials(ShaderHelper& shader,
                                       bool is_gemm,
                                       const ShaderVariableHelper& output) {
  // Deterministic two-pass Split-K, pass 1.
  //
  // Each split owns a disjoint slot of the `partials` scratch buffer and writes its tile there with a
  // plain store. Nothing is read back and nothing is shared between splits, so there is no atomic, no
  // compare-and-exchange retry loop, and no dependence on the order in which workgroups happen to
  // retire. `MatMulSplitKReduceProgram` then sums the slots in a fixed index order.
  //
  // The scratch buffer is bound as a rank-1 tensor laid out split-major:
  //
  //   partials[split_index * out_elems + (batch * dim_a_outer + row) * dim_b_outer_vec + colIn]
  //   out_elems = batch_size * dim_a_outer * dim_b_outer_vec
  //
  // Split-major keeps these writes at the same addresses and with the same coalescing the atomic path
  // had, and keeps the reduction's reads coalesced within each split it visits.
  //
  // `batch_size` is derived from uniforms that both programs already declare, so enabling Split-K adds
  // no uniform to `MatMulProgram` or `GemmProgram` and therefore cannot perturb the non-Split-K shader.
...
Status MakeMatMulPackedVec4Source(ShaderHelper& shader,
  ...
  if (split_k) {
    ...
    //  2. `MatMulSplitKReduceProgram` sums the 3 slots in a fixed index order, adds bias (B) and any
    //     activation, and writes Y. Because the order is fixed by index rather than by the order in
    //     which workgroups retire, the result is bit-reproducible.
    shader.MainFunctionBody()
