...
Status TopKProgram::GenerateShaderCode(ShaderHelper& shader) const {
  ...
  const std::string compare_op = largest_ ? "<" : ">";
  const std::string pad_value = largest_ ? ("-" + max_float) : max_float;

  // Declare shared memory for bubble sort
  shader.AdditionalImplementation()
      << "var<workgroup> shared_vals: array<x_element_t, " << wg_ << ">;\n"
      << "var<workgroup> shared_idxs: array<i32, " << wg_ << ">;\n";
  ...
  shader.MainFunctionBody()
      ...
      << "if (i32(local_idx) < cols) {\n"
      << "  shared_vals[local_idx] = x_element_t(" << input.GetByOffset("u32(i32(row) * cols + i32(local_idx))") << ");\n"
      << "  shared_idxs[local_idx] = i32(local_idx);\n"
      << "} else {\n"
      << "  shared_vals[local_idx] = x_element_t(" << pad_value << ");\n"
      << "  shared_idxs[local_idx] = -1;\n"
      << "}\n"
      << "workgroupBarrier();\n"
      << "\n"
      // Bubble sort - only K passes needed to get top-K at the front
      << "let sort_count = min(u32(k), " << wg_ << "u);\n"
      << "for (var i = 0u; i < sort_count; i++) {\n"
      << "  for (var j = " << wg_ << "u - 1u; j > i; j--) {\n"
      << "    if (local_idx == j - 1u && i32(j) < cols) {\n"
      << "      if (shared_vals[j - 1u] " << compare_op << " shared_vals[j] ||\n"
      << "          (shared_vals[j - 1u] == shared_vals[j] && shared_idxs[j - 1u] > shared_idxs[j])) {\n"
      << "        let tmp_val = shared_vals[j - 1u];\n"
      << "        let tmp_idx = shared_idxs[j - 1u];\n"
      << "        shared_vals[j - 1u] = shared_vals[j];\n"
      << "        shared_idxs[j - 1u] = shared_idxs[j];\n"
      << "        shared_vals[j] = tmp_val;\n"
      << "        shared_idxs[j] = tmp_idx;\n"
      << "      }\n"
      << "    }\n"
      << "    workgroupBarrier();\n"
      << "  }\n"
      ...
      // Write top-K results
      << "if (i32(local_idx) < k) {\n"
      << "  let out_offset = u32(i32(row) * k + i32(local_idx));\n"
      << "  values[out_offset] = shared_vals[local_idx];\n"
      << "  " << indices_out.SetByOffset("out_offset", "shared_idxs[local_idx]") << "\n"
      << "}\n";
...
Status TopK::ComputeInternal(ComputeContext& context) const {
  ...
  // Workgroup size must be >= cols and a power of 2, max 256
  uint32_t wg_size = NextPowerOf2(static_cast<uint32_t>(cols));
  if (wg_size > 256) {
    wg_size = 256;
  }
  if (wg_size < static_cast<uint32_t>(cols)) {
    // cols > 256: not supported by simple bubble sort
    return ORT_MAKE_STATUS(ONNXRUNTIME, NOT_IMPLEMENTED,
                           "TopK on WebGPU does not support axis dimension > 256, got ", cols);
  }
  ...
  TopKProgram program{wg_size, largest, is_fp16};
  ...
  program
      .CacheHint(std::to_string(wg_size), largest ? "largest" : "smallest", is_fp16 ? "fp16" : "fp32")
