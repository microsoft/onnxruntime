...
Status Conv<is_channels_last, is_fused>::ComputeInternal(ComputeContext& context) const {
  ...
  if (same_size || (kernel_height == 1 && kernel_width == 1 && pads[0] == 0 && pads[1] == 0 && strides[0] == 1 && strides[1] == 1)) {
    ...
    if (N < 8 && K < 8) {
      ...
      std::optional<bool> applied_is_channels_last;
      if (has_bias) {
        applied_is_channels_last = is_channels_last;
      }
      return ComputeMatMul(&context, activation_, matmul_inputs, output, applied_is_channels_last, matmul_input_reshapes[0], matmul_input_reshapes[1]);
    }
  }
