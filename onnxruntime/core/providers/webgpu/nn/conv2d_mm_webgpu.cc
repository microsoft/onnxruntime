...
Status Conv2dMMProgram::GenerateShaderCode(ShaderHelper& shader) const {
  ...
  return is_vec4_ ? MakeMatMulPackedVec4Source(shader, elements_per_thread_, WorkgroupSizeX(), WorkgroupSizeY(), data_type, /* batch_dims = */ nullptr, /* transpose_a = */ !is_channels_last_, /* transpose_b = */ false, 1.0f, 4, true, tile_inner_) : MakeMatMulPackedSource(shader, elements_per_thread_, WorkgroupSizeX(), WorkgroupSizeY(), data_type, /* batch_dims = */ nullptr, !is_channels_last_, false, 1.0f, true, tile_inner_, /* split_t = */ false, 0, sequentially_access_by_threads_);
}
