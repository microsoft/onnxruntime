...
Status ConvTranspose2DProgram::GenerateShaderCode(ShaderHelper& shader) const {
  ...
  auto calculate_remainder = [&]() -> std::string {
    ...
    if (input_channels_remainder_ > 0) {
      if (a_components_ == 1) {
        for (int i = 0; i < input_channels_remainder_; ++i) {
  ...
  if (pack_input_as4_) {
    shader.MainFunctionBody() << "    var x_offset = " << dy.IndicesToOffset(dy_indices_t('batch', 'idyR', 'idyC', 'inputChannel')) << " / " << a_components_ << ";\n"
                              << "    var w_offset = " << w.IndicesToOffset(w_indices_t('wRPerm', 'wCPerm', 'inputChannel', 'wOutChannel')) << " / " << b_components_ << ";\n";
  }
  ...
  shader.MainFunctionBody() << "    for (var d2: u32 = 0; d2 < uniforms.input_channels_per_group_int; d2 = d2 + " << pack_input_as4_ ? 4 : a_components_ << ") {\n"
                            ...
                            << "}\n"
                            << "let value = dotProd" << (hasBias_ ? " + bias[d1 / " + std::to_string(components_) + "]" : "") << "\n"
                            << output.setByOffset(" global_idx ", " value ") << "\n";
}

ConvTranspose2DProgram CreateConvTranspose2DProgram(const std::vector<const Tensor*>& inputs, const std::vector<uint32_t>& pads, const std::vector<uint32_t>& strides, const std::vector<uint32_t>& dilations, Tensor* output, bool is_channels_last, const std::vector<TensorShape>& modified_input_output_shapes, uint32_t group) {
  ...
  auto input_channels_per_group = weight_shape[2] / group;
  ...
  std::vector<uint32_t> filter_dims = {weight_shape[is_channels_last ? 1 : 2], weight_shape[is_channels_last ? 2 : 3]};
  ...
  ConvTranspose2DProgram program(is_channels_last, has_bias, components, a_components, b_componens, input_channels_remainder, pack_input_as4);
  ...
  program.AddUniforms({{output_size}, {strides}, {filter_dims}, {dilations}, {effective_filter_dims}, {pads}, {input_channels_per_group}, {input_channels_per_group_int}, {output_channels_per_group}});
  return program;
}
