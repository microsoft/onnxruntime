// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

#include <vector>

#include "core/util/math.h"
#include "core/providers/webgpu/quantization/quantize_linear.h"
#include "core/providers/webgpu/shader_helper.h"
#include "core/providers/webgpu/webgpu_supported_types.h"
#include "core/providers/webgpu/webgpu_utils.h"

namespace onnxruntime {
namespace webgpu {

namespace {
const std::vector<MLDataType>& DequantizeLinearConstraints() {
  static std::vector<MLDataType> types{
      DataTypeImpl::GetTensorType<int8_t>(),
      DataTypeImpl::GetTensorType<uint8_t>(),
      DataTypeImpl::GetTensorType<int32_t>()};
  return types;
}
}  // namespace

ONNX_OPERATOR_VERSIONED_KERNEL_EX(
    DequantizeLinear,
    kOnnxDomain,
    10, 12,
    kWebGpuExecutionProvider,
    (*KernelDefBuilder::Create())
        .TypeConstraint("T1", DequantizeLinearConstraints())
        .TypeConstraint("T2", WebGpuSupportedFloatTypes()),
    DequantizeLinear);

ONNX_OPERATOR_VERSIONED_KERNEL_EX(
    DequantizeLinear,
    kOnnxDomain,
    13, 18,
    kWebGpuExecutionProvider,
    (*KernelDefBuilder::Create())
        .TypeConstraint("T1", DequantizeLinearConstraints())
        .TypeConstraint("T2", WebGpuSupportedFloatTypes()),
    DequantizeLinear);

ONNX_OPERATOR_VERSIONED_KERNEL_EX(
    DequantizeLinear,
    kOnnxDomain,
    19, 20,
    kWebGpuExecutionProvider,
    (*KernelDefBuilder::Create())
        .TypeConstraint("T1", DequantizeLinearConstraints())
        .TypeConstraint("T2", WebGpuSupportedFloatTypes()),
    DequantizeLinear);

ONNX_OPERATOR_VERSIONED_KERNEL_EX(
    DequantizeLinear,
    kOnnxDomain,
    21, 22,
    kWebGpuExecutionProvider,
    (*KernelDefBuilder::Create())
        .TypeConstraint("T1", DequantizeLinearConstraints())
        .TypeConstraint("T2", WebGpuSupportedFloatTypes()),
    DequantizeLinear);

ONNX_OPERATOR_KERNEL_EX(
    DequantizeLinear,
    kOnnxDomain,
    23,
    kWebGpuExecutionProvider,
    (*KernelDefBuilder::Create())
        .TypeConstraint("T1", DequantizeLinearConstraints())
        .TypeConstraint("T2", WebGpuSupportedFloatTypes()),
    DequantizeLinear);

Status DequantizeLinearProgram::GenerateShaderCode(ShaderHelper& shader) const {
  const auto& x = shader.AddInput("x", ShaderUsage::UseUniform | ShaderUsage::UseIndicesTypeAlias | ShaderUsage::UseValueTypeAlias);
  const auto& x_scale = shader.AddInput("x_scale", ShaderUsage::UseUniform | ShaderUsage::UseIndicesTypeAlias | ShaderUsage::UseValueTypeAlias);
  const auto& output = shader.AddOutput("output", ShaderUsage::UseUniform | ShaderUsage::UseShapeAndStride | ShaderUsage::UseValueTypeAlias);

  int components = x.NumComponents();

  shader.AdditionalImplementation() << "";
  shader.MainFunctionBody()
      << shader.GuardAgainstOutOfBoundsWorkgroupSizes("uniforms.output_size")
      << "let output_indices = " << output.OffsetToIndices("global_idx") << ";\n";

  if (packed_) {
    std::string unpack = (signed_) ? "unpack4xI8(x)" : "unpack4xU8(x)";
    std::string veccomp = components_ == 1 ? "x_vec[global_idx % 4]" : "x_vec";
    shader.MainFunctionBody()
        << "let x = " << x.GetByOffset("global_idx / 4") << ";\n"
        << "let x_vec = " << unpack << ";\n"
        << "let x_value = " << veccomp << ";\n";
  } else {
    shader.MainFunctionBody()
        << "let x_value = " << x.GetByOffset("global_idx") << ";\n";
  }

  if (per_layer_quantization_) {
    // scale input is a scalar ()
    shader.MainFunctionBody()
        << "let scale_value = " << x_scale.GetByOffset("0") << ";\n";
  } else if (per_axis_quantization_) {
    shader.MainFunctionBody()
        << "let scale_index = " << output.IndicesGet("output_indices", "uniforms.axis") << ";\n"
        << "let scale_value = " << x_scale.GetByOffset("scale_index") << ";\n";
  } else {
    // Block quantization. Scale input rank is same as input/output rank.
    shader.MainFunctionBody()
        << "var scale_indices: x_scale_indices_t = output_indices;\n"
        << "let index = " << x_scale.IndicesGet("scale_indices", "uniforms.axis") << "/ uniforms.block_size;\n"
        << x_scale.IndicesSet("scale_indices", "uniforms.axis", "index") << ";\n"
        << "let scale_value = " << x_scale.GetByIndices("scale_indices") << ";\n";
  }

  if (has_zeropoint_) {
    const auto& x_zp = shader.AddInput("x_zp", ShaderUsage::UseUniform | ShaderUsage::UseIndicesTypeAlias | ShaderUsage::UseValueTypeAlias);

    std::string unpack = (signed_) ? "unpack4xI8(x_zp)" : "unpack4xU8(x_zp)";
    if (per_layer_quantization_) {
      // zero-point input is a scalar
      if (packed_) {
        shader.MainFunctionBody()
            << "let x_zp = " << x_zp.GetByOffset("0") << ";\n"
            << "let x_zp_vec = " << unpack << ";\n"
            << "let zp_value = x_zp_vec[0];\n";
      } else {
        shader.MainFunctionBody()
            << "let zp_value = " << x_zp.GetByOffset("0") << ";\n";
      }
    } else if (per_axis_quantization_) {
      // zero-point input is a 1D tensor
      if (packed_) {
        shader.MainFunctionBody()
            << "let zp_index = " << output.IndicesGet("output_indices", "uniforms.axis") << ";\n"
            << "let x_zp = " << x_zp.GetByOffset("zp_index / 4") << ";\n"
            << "let zp_vec = " << unpack << ";\n"
            << "let zp_value = zp_vec[zp_index % 4];\n";
      } else {
        shader.MainFunctionBody()
            << "let zp_index = " << output.IndicesGet("output_indices", "uniforms.axis") << ";\n"
            << "let zero_point_value = " << x_zp.GetByOffset("zp_index") << ";\n";
      }
    } else {
      // BlockedQuantization. The zero-point input shape is same as the input shape except along axis.
      if (packed_) {
        shader.MainFunctionBody()
            << "let zp_offset = " << x_scale.GetByIndices("scale_indices") << ";\n"
            << "let zero_point_input = " << x_zp.GetByOffset("zp_offset / 4") << ";\n"
            << "let zp_vec = " << unpack << ";\n"
            << "let zp_value = zp_vec[zp_offset % 4];\n";
      } else {
        shader.MainFunctionBody()
            << "let zp_value = " << x_zp.GetByIndices("scale_indices") << ";\n";
      }
    }
  } else {
    shader.MainFunctionBody()
        << "let zp_value = x_zp_value_t(0);\n";
  }

  // compute and write output
  shader.MainFunctionBody()
      << output.SetByOffset("global_idx", "output_value_t(x_value - zp_value) * scale_value");

  return Status::OK();
}

Status DequantizeLinear::ComputeInternal(ComputeContext& context) const {
  const auto* x = context.Input(0);
  const auto* x_scale = context.Input(1);
  const auto* x_zeropoint = context.Input(2);
  auto* output_tensor = context.Output(0, x->Shape());
  const auto x_shape = x->Shape();
  bool packed = x->GetElementType() == ONNX_TENSOR_ELEMENT_DATA_TYPE_INT8 || x->GetElementType() == ONNX_TENSOR_ELEMENT_DATA_TYPE_UINT8;
  bool is_signed = x->GetElementType() == ONNX_TENSOR_ELEMENT_DATA_TYPE_INT8;
  int64_t axis = (axis_ >= 0) ? axis_ : axis_ + x_shape.NumDimensions();
  int64_t size = x_shape.Size();
  auto max_components = GetMaxComponents(size);
  uint32_t vec_size = onnxruntime::narrow<uint32_t>((size + 3) / 4);
  // const zeroPointShape = zeroPointInput
  //   ? isPacked
  //     ? [Math.ceil(ShapeUtil.size(zeroPointInput.dims) / 4)]
  //     : zeroPointInput.dims
  //   : undefined;

  // Scales input is a scaler for per-tensor/per-layer quantization, 1-D tensor for per-axis quantization
  // or tensor with same rank as input for blocked quantization.
  bool per_layer_quantization = x_scale->Shape().NumDimensions() == 0 || (x_scale->Shape().NumDimensions() == 1 && x_scale->Shape()[0] == 1);
  bool per_axis_quantization = per_layer_quantization == false && x_scale->Shape().NumDimensions() == 1;
  bool use_components = per_layer_quantization && (!packed || max_components == 4);
  int components = use_components ? max_components : 1;
  int input_component = use_components && !packed ? max_components : 1;

  DequantizeLinearProgram program{axis_, block_size_, packed, is_signed, per_layer_quantization,
                                  per_axis_quantization, components, input_component, x_zeropoint != nullptr};

  program
      .AddInputs({{x, ProgramTensorMetadataDependency::TypeAndRank, static_cast<int>(components)}})
      .AddInputs({{x_scale, ProgramTensorMetadataDependency::TypeAndRank}})
      .AddOutput({output_tensor, ProgramTensorMetadataDependency::None, static_cast<int>(components)})
      .SetDispatchGroupSize((vec_size + WORKGROUP_SIZE - 1) / WORKGROUP_SIZE)
      .AddUniformVariables({{static_cast<uint32_t>(axis)}})
      .AddUniformVariables({{static_cast<uint32_t>(block_size_)}})
      .AddUniformVariables({{static_cast<uint32_t>(size)}})
      .CacheHint(std::to_string(axis));

  if (x_zeropoint) {
    program.AddInputs({{x_zeropoint, ProgramTensorMetadataDependency::TypeAndRank}});
  }

  return context.RunProgram(program);
}

}  // namespace webgpu
}  // namespace onnxruntime
