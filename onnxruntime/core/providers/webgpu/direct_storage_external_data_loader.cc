...
TensorKey MakeKey(const std::filesystem::path& path, std::string_view name,
  ...
  return {path.lexically_normal(), std::string{name}, offset, length};
}
...
struct DirectStorageExternalDataLoader::Impl {
  ...
  Impl(WebGpuContext& context_in, std::shared_ptr<DirectStorageInitializerState> state_in)
      : context{context_in}, state{std::move(state_in)} {
  }

  WebGpuContext& context;
  std::shared_ptr<DirectStorageInitializerState> state;
  mutable std::unique_ptr<Batch> batch;
};
...
DirectStorageExternalDataLoader::DirectStorageExternalDataLoader(
    ...
    : impl_{std::make_unique<Impl>(context, std::move(state))} {
  ORT_ENFORCE(impl_->state != nullptr && impl_->state->impl_ != nullptr,
              "DirectStorage allocator state is required.");
  ORT_ENFORCE(
      impl_->context.DeviceHasFeature(wgpu::FeatureName::SharedBufferMemoryD3D12Resource) &&
          impl_->context.DeviceHasFeature(wgpu::FeatureName::SharedFenceDXGISharedHandle),
...
common::Status DirectStorageExternalDataLoader::BeginLoad() const {
  AbortLoad();
  impl_->batch = std::make_unique<Impl::Batch>();
...
common::Status DirectStorageExternalDataLoader::PrepareTensor(
  ...
  TensorKey key = MakeKey(data_file_path, tensor_name, offset, length);
  ORT_RETURN_IF(impl_->batch->tensors_by_key.find(key) !=
  ...
  ORT_RETURN_IF(impl_->batch->request_count >
                    std::numeric_limits<size_t>::max() - requests,
                "DirectStorage request count overflow.");
  ORT_RETURN_IF(impl_->batch->total_bytes >
                    std::numeric_limits<size_t>::max() - length,
                "DirectStorage initializer byte count overflow.");

  const size_t index = impl_->batch->tensors.size();
  impl_->batch->tensors.push_back({std::move(key)});
  impl_->batch->tensors_by_key.emplace(impl_->batch->tensors.back().key, index);
  impl_->batch->request_count += requests;
  impl_->batch->total_bytes += length;
  return common::Status::OK();
}
...
common::Status DirectStorageExternalDataLoader::FinalizeLoad(
  ...
  if (wait_result != WAIT_OBJECT_0) {
    // Do not let Abort release destinations while DirectStorage may still own them.
  ...
  LOGS_DEFAULT(INFO)
      ...
      << Milliseconds(initialization_end - initialization_start) << " ms, "
      << "D3D12 allocation="
      << Milliseconds(allocation_end - allocation_start) << " ms, "
      << "overlapped preparation="
      << Milliseconds(preparation_end - preparation_start) << " ms, "
      << "enqueue=" << Milliseconds(enqueue_end - enqueue_start) << " ms, "
      << "file-to-GPU=" << Milliseconds(io_end - io_start) << " ms, "
...
common::Status DirectStorageExternalDataLoader::LoadTensor(
  ...
  const TensorKey key = MakeKey(
      data_file_path, tensor_name, static_cast<uint64_t>(data_offset), length);
