...
class StaticCodeGenerator:
    ...
    def __init__(self, use_string_table: bool = True) -> None:
        ...
        self._string_table: Optional[Dict[str, int]] = (
            {} if use_string_table else None
        )
    ...
    def method(
        self, obj: str, method_name: str, args: List[CodeSegmentArg]
    ) -> str:
        rendered = ", ".join(self._render_arg(a) for a in args)
    ...
    @builtins.property
    def string_table(self) -> Optional[Dict[str, int]]:
    ...
    def build(
        ...
        *,
        ...
        from ..types import TemplatePass2, TemplateRepository as _TR
        ...
        for name, template in repo.templates.items():
            ...
            if not name.endswith(template_ext):
                raise ValueError(
                    f'Template name "{name}" does not end with the expected '
                    f'extension "{template_ext}"'
                )
            ...
            impl_hashes[name] = hashlib.sha256(
        ...
        if self._string_table is not None:
            ...
            impl_hashes["string_table.h"] = hashlib.sha256(
                content.encode("utf-8")
            ).hexdigest()
        ...
        return _TR(base_path=repo.base_path, templates=dict(result_files))

    # ---- internal builders -------------------------------------------

    def _build_generate_index(self, repo: "TemplateRepository") -> str:
        from ..types import TemplatePass2
        ...
        out.append(
            '#error "This file is expected to be included by wgsl-gen header. '
    ...
    def _build_generate_string_table(self) -> str:
        ...
        sorted_entries = sorted(
            self._string_table.items(), key=lambda kv: kv[1]
        )
    ...
    def _build_generate_index_impl(
        ...
        if self._string_table is not None:
            ...
            out.append(
                f'#include "{include_path_prefix}/string_table.h"  // {sthash}'
            )
        out.append("")
    ...
    def _build_template_implementation(
        self, file_path: str, template: "object"
    ) -> str:
        from ..types import TemplatePass2
        ...
        if gr.params:
            ...
            for param_name in gr.params:
                out.append(
                    f"  auto& {self.param(param_name)} = params.param_{param_name};"
                )
            out.append("")
        ...
        if gr.variables:
            ...
            for var_name in gr.variables:
                out.append(
                    f"  auto& {self.variable(var_name)} = *params.var_{var_name};"
                )
            out.append("")
...
if TYPE_CHECKING:  # pragma: no cover
    from ..types import TemplateRepository
