...
namespace intel_ep {
std::string IntelCustomOp::hello;
...
//InferenceEngine::CNNNetwork cnetwork;
IntelCustomOp::IntelCustomOp(const ComputeContext* context,
...
void IntelCustomOp::CreateNGraphFunc(const OrtCustomOpApi* api, OrtKernelContext* context) const {
...
*/
    ...
    IntelCustomOp::cnetwork = network;
    //auto execNetwork = ie.LoadNetwork(network, ”CPU”);     // Load nGraph to plugin

    //auto nGraphPtr = networt.getFunction();                             // nGraphPtr == nGraph
    //auto inputsInfo = networt.getInputsInfo();

/*
    IntelCustomOp::hello = "hello";
    LOGS_DEFAULT(INFO) << "ONNX Import Done 2";
    std::cout << "In Ngraph Func" << std::endl;
    InferenceEngine::ICNNNetwork::Ptr network = InferenceEngine::convertFunctionToICNNNetwork(ng_function);
    LOGS_DEFAULT(INFO) << "ONNX Import Done 3";
    InferenceEngine::CNNNetwork cnetwork_temp(network);
    LOGS_DEFAULT(INFO) << "ONNX Import Done";
    //cnetwork(network);
    IntelCustomOp::cnetwork = cnetwork_temp;
    LOGS_DEFAULT(INFO) << "Network Created";
...
//This method gets called in critical path of execution: Optimize
void IntelCustomOp::Initialize(const OrtCustomOpApi* api, OrtKernelContext* context) const {
  Ort::CustomOpApi ort{*api};
...
//This method gets called in critical path of execution: Optimize
Status IntelCustomOp::Compute(const OrtCustomOpApi* api, OrtKernelContext* context) const {
  Ort::CustomOpApi ort{*api};
