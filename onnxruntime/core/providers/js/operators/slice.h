...
class Slice : public JsKernel, public SliceBase {
 public:
  Slice(const OpKernelInfo& info, bool dynamic = true) : JsKernel(info), SliceBase(info, dynamic) {
    ...
    JSEP_INIT_KERNEL_ATTRIBUTE(Slice, ({"starts" : $1 ? Array.from(HEAP32.subarray($2 >>> 0, ($2 >>> 0) + $1)) : [],
                                        "ends" : $3 ? Array.from(HEAP32.subarray($4 >>> 0, ($4 >>> 0) + $3)) : [],
                                        "axes" : $5 ? Array.from(HEAP32.subarray($6 >>> 0, ($6 >>> 0) + $5)) : []}),
                               gsl::narrow_cast<int32_t>(starts.size()),
                               JSEP_HEAP_INDEX((starts.size() > 0) ? starts.data() : nullptr) >> 2,
                               gsl::narrow_cast<int32_t>(ends.size()),
                               JSEP_HEAP_INDEX((ends.size() > 0) ? ends.data() : nullptr) >> 2,
