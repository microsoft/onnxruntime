...
#include "include/ncnn/layer/gemm.h"                        // all other MatMul
#include "include/ncnn/layer/vulkan/gemm_vulkan.h"          // all other MatMul
#include "include/ncnn/layer/vulkan/innerproduct_vulkan.h"  // 2D MatMul
...
Status MatMulKernel::SetupParamDict(const GraphViewer& /*graph_viewer*/, ncnn::ParamDict& params) {
  ...
  assert(have_shape_A && have_shape_B);
...
Status MatMulKernel::SetupConstantInitializers(const GraphViewer& graph_viewer, ncnn::Layer& /*layer*/) {
  ...
  if (use_inner_product_) {
    ...
    ORT_RETURN_IF_ERROR(DoTransposeEltWise(shape_B.size(), target_shape, narrow<size_t>(shape_B[1]),
                                           stride, src_data.data(), reinterpret_cast<uint8_t*>(target_data.data()),
                                           sizeof(float)));
