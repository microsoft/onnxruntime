...
static bool IsQuantizedIOSupported(const InitializedTensorSet& initializers, const NodeUnit& node_unit,
  ...
  bool is_quant_matmul = (quant_op_type == QuantizedOpType::QLinearMatMul) || (quant_op_type == QuantizedOpType::QDQMatMul);
  bool is_quant_gemm = (quant_op_type == QuantizedOpType::QDQGemm);
  bool is_quant_matmul_or_gemm = is_quant_matmul || is_quant_gemm;
  ...
  for (const auto idx : indices) {
    ...
    bool is_conv_matmul_weight = is_input && (is_quant_conv || is_quant_matmul_or_gemm) && idx == 1;
