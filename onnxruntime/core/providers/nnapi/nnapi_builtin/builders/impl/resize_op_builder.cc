...
Status ResizeOpBuilder::AddToModelBuilderImpl(ModelBuilder& model_builder, const NodeUnit& node_unit) const {
  ...
  const bool use_nchw = output_shape[3] != input_shape[3];  // not Channel last
...
bool ResizeOpBuilder::IsOpSupportedImpl(const InitializedTensorSet& initializers, const NodeUnit& node_unit,
  ...
  {  // scales and sizes (if present) must be initializers
    ...
    if (inputs.size() == 3) {  // we are using scales
      ...
      use_nchw = scales_data[1] == 1.0F;
      float const scale_n = scales_data[0];
      float const scale_c = !use_nchw ? scales_data[3] : scales_data[1];
      ...
      use_nchw = sizes_data[1] == input_shape[1];
      int channel_idx = !use_nchw ? 3 : 1;
    ...
    if (use_nchw && params.android_feature_level <= ANEURALNETWORKS_FEATURE_LEVEL_2) {
