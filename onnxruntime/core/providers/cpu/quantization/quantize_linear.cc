        ...
        for (size_t k1 = 0; k1 < K; k1 += quant_block_size) {
          for (size_t k2 = 0, k2_end = std::min(quant_block_size, K - k1); k2 < k2_end; ++k2) {
            for (size_t n = 0; n < N; n++) {
              ...
              auto sc = scale[n];
              auto v = std::clamp(static_cast<int32_t>(std::nearbyintf((*input++) / sc)) + zp, low, high);
      ...
      for (size_t m = 0; m < M; m++) {
        for (size_t k1 = 0; k1 < K; k1 += quant_block_size) {
          for (size_t k2 = 0, k2_end = std::min(quant_block_size, K - k1); k2 < k2_end; ++k2) {
            for (size_t n = 0; n < N; n++) {
              auto sc = scale[n];
              auto v = std::clamp(static_cast<int32_t>(std::nearbyintf((*input++) / sc)), low, high);
              *output++ = static_cast<TOut>(v);
            }
          }
...
struct BlockedQuantizeLinear<float, TOut, 2> {
        ...
        for (size_t k1 = 0; k1 < K; k1 += quant_block_size) {
          for (size_t k2 = 0, k2_end = std::min(quant_block_size, K - k1); k2 < k2_end; ++k2) {
            ...
            for (size_t n = 0; n < N; n++, output_index++, zp_index++) {
              ...
              auto v = std::clamp(static_cast<int32_t>(std::nearbyintf(input[output_index] / sc)) + zp, low, high);
  ...
  static void opBaseline2(const float* input, const float* scale, const TOut* zero_point, TOut* output,
                          size_t M, size_t K, size_t N, size_t quant_block_size, bool saturate) {
    ORT_UNUSED_PARAMETER(saturate);
    size_t zp_b_index = 0;
    size_t output_index = 0;
        ...
        for (size_t k1 = 0; k1 < K; k1 += quant_block_size) {
          for (size_t k2 = 0, k2_end = std::min(quant_block_size, K - k1); k2 < k2_end; ++k2) {
            ...
            for (; n < N - 1; n += 2, output_index += 2) {
              ...
              auto v0 = std::clamp(static_cast<int32_t>(std::nearbyintf(input[output_index] / sc0)) + zp0, low, high);
              auto v1 = std::clamp(static_cast<int32_t>(
                                       std::nearbyintf(input[output_index + 1] / sc1)) +
                                       zp1,
                                   low, high);
      ...
      for (size_t m = 0; m < M; m++) {
        for (size_t k1 = 0; k1 < K; k1 += quant_block_size) {
          for (size_t k2 = 0, k2_end = std::min(quant_block_size, K - k1); k2 < k2_end; ++k2) {
            ...
            if (output_index & 1) {
              auto sc = scale[zp_index];
              auto v = std::clamp(static_cast<int32_t>(std::nearbyintf(input[output_index] / sc)), low, high);
              output[output_index >> 1].SetElem(1, static_cast<TOut::UnpackedType>(v));
              output_index++;
              zp_index++;
              n++;
            ...
            for (; n < N - 1; n += 2, output_index += 2) {
              ...
              auto v0 = std::clamp(static_cast<int32_t>(std::nearbyintf(input[output_index] / sc0)), low, high);
              auto v1 = std::clamp(static_cast<int32_t>(std::nearbyintf(input[output_index + 1] / sc1)), low, high);
              output[output_index >> 1] = TOut(v0, v1);
            }
...
struct BlockedQuantizeLinear<MLFloat16, TOut, 2> {
  ...
  static void opBaseline2(const MLFloat16* input, const MLFloat16* scale, const TOut* zero_point, TOut* output,
                          size_t M, size_t K, size_t N, size_t quant_block_size, bool saturate) {
    ORT_UNUSED_PARAMETER(saturate);
    size_t zp_b_index = 0;
    size_t output_index = 0;
      ...
      for (size_t m = 0; m < M; m++) {
        for (size_t k1 = 0; k1 < K; k1 += quant_block_size) {
          for (size_t k2 = 0, k2_end = std::min(quant_block_size, K - k1); k2 < k2_end; ++k2) {
            ...
            if (output_index & 1) {
              auto sc = scale[zp_index].ToFloat();
              auto v = std::clamp(static_cast<int32_t>(std::nearbyintf(input[output_index].ToFloat() / sc)), low, high);
              output[output_index >> 1].SetElem(1, static_cast<TOut::UnpackedType>(v));
              output_index++;
              zp_index++;
              n++;
