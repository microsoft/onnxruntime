...
Status ConstantOfShape::Compute(OpKernelContext* ctx) const {
  ...
  switch (element_size) {
    case sizeof(int8_t):
      FilloutOutput(*(reinterpret_cast<const int8_t *>(value_ptr)), output_data, static_cast<uint64_t>(size));
      break;
    case sizeof(int16_t):
      FilloutOutput(*(reinterpret_cast<const int16_t *>(value_ptr)), output_data, static_cast<uint64_t>(size));
      break;
    case sizeof(int32_t):
      FilloutOutput(*(reinterpret_cast<const int32_t *>(value_ptr)), output_data, static_cast<uint64_t>(size));
      break;
    case sizeof(int64_t):
      FilloutOutput(*(reinterpret_cast<const int64_t *>(value_ptr)), output_data, static_cast<uint64_t>(size));
      break;
    default:
