...
Status CategoryMapper::Compute(OpKernelContext* context) const {
  ...
  if (input_type == DataTypeImpl::GetType<std::string>()) {
    if (Y.DataType() != DataTypeImpl::GetType<int64_t>())
      return ORT_MAKE_OP_STATUS(ONNXRUNTIME, FAIL, context->Kernel().Node(), "Input of string must have output of int64");
    ...
    if (Y.DataType() != DataTypeImpl::GetType<std::string>())
      return ORT_MAKE_OP_STATUS(ONNXRUNTIME, FAIL, context->Kernel().Node(), "Input of int64 must have output of string ");
