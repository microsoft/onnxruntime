...
static void FlattenOutputDims(gsl::span<const int64_t> input_dimensions,
  ...
  for (int64_t i = static_cast<int64_t>(starts.size()) - 1; i >= 0; --i) {
    ...
    if (steps[gsl::narrow_cast<size_t>(i)] == 1 && input_dimensions[gsl::narrow_cast<size_t>(i)] == output_dims[gsl::narrow_cast<size_t>(i)])
      ++num_to_combine;
    else
      break;
  }
...
Status SliceBase::FillVectorsFromInput(const Tensor& start_tensor,
  ...
  input_starts.reserve(gsl::narrow_cast<size_t>(size));
  input_ends.reserve(gsl::narrow_cast<size_t>(size));
  if (nullptr != axes_tensor)
    input_axes.reserve(gsl::narrow_cast<size_t>(size));
  // Slice V10
  if (nullptr != steps_tensor)
    input_steps.reserve(gsl::narrow_cast<size_t>(size));
