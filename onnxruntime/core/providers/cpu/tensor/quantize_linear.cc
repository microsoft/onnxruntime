...
Status DequantizeLinear<T>::Compute(OpKernelContext* ctx) const {
  ...
  ORT_ENFORCE(x != nullptr && x_scale != nullptr);
  ...
  if (has_axis_) {  // custom DequantizeLinear only
    ...
    ORT_ENFORCE(x_zero_point != nullptr &&
                    x_zero_point->Shape().NumDimensions() == 1 &&
                    x_zero_point->Shape().Size() == broadcast_dim,
                "x_zero_point must be 1D tensor with size ", broadcast_dim);
  } else {
    N = 1;
    broadcast_dim = 1;
