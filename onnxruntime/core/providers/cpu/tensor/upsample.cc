...
static std::vector<int64_t> UpsampleNearestSetupRank1InputMapping(
  ...
  for (int64_t output_dim0_idx = 0; output_dim0_idx < length_resized; ++output_dim0_idx) {
    ...
    input_mapping[static_cast<size_t>(output_dim0_idx)]= input_dim0_idx;
  }
...
UpsampleNearestSetupInputMappings(int64_t n_dim,
                                  ...
                                  const GetOriginalCoordinateFunc& get_original_coordinate,
                                  const GetNearestPixelFunc& get_nearest_pixel) {
  std::vector<std::vector<int64_t>> input_mappings(static_cast<size_t>(n_dim));

  for (int64_t axis = 0; axis < n_dim; ++axis) {
    std::vector<int64_t>& input_mapping = input_mappings[static_cast<size_t>(axis)];
    input_mapping.resize(static_cast<size_t>(output_shape[static_cast<size_t>(axis)]));
    ...
    if (scales[static_cast<size_t>(axis)] == 1.0f) {
      for (int64_t dim = 0; dim < output_shape[static_cast<size_t>(axis)]; dim++) {
        input_mapping[static_cast<size_t>(dim)] = dim * input_dim_factor[static_cast<size_t>(axis)];
      }
      continue;
    }
    ...
    for (int64_t dim = 0; dim < output_shape[static_cast<size_t>(axis)]; dim++) {
      float original_dim = get_original_coordinate(static_cast<float>(dim),
                                                   scales[static_cast<size_t>(axis)],
                                                   static_cast<float>(output_shape[static_cast<size_t>(axis)]),
                                                   static_cast<float>(input_shape[static_cast<size_t>(axis)]),
                                                   roi[static_cast<size_t>(axis)], roi[static_cast<size_t>(n_dim + axis)]);

      bool need_extrapolation = (extrapolation_enabled && (original_dim < 0 || original_dim > input_shape[static_cast<size_t>(axis)] - 1));
      int64_t input_dim = get_nearest_pixel(original_dim, scales[static_cast<size_t>(axis)] < 1);
      if (input_dim >= input_shape[static_cast<size_t>(axis)]) input_dim = input_shape[static_cast<size_t>(axis)] - 1;
      if (input_dim < 0) input_dim = 0;

      input_mapping[static_cast<size_t>(dim)] = need_extrapolation ? (-input_size) : (input_dim * input_dim_factor[static_cast<size_t>(axis)]);
    }
  }
...
static Status UpsampleNearestImpl(const T* input,
  ...
  std::vector<int64_t> input_dim_counters(static_cast<size_t>(n_dim));
  std::vector<int64_t> input_dim_factor(static_cast<size_t>(n_dim));
  input_dim_factor[static_cast<size_t>(n_dim - 1)] = 1;  // initialize dimension factor
  for (int64_t dim_idx = n_dim - 2; dim_idx >= 0; dim_idx--) {
    input_dim_factor[static_cast<size_t>(dim_idx)] = input_dim_factor[static_cast<size_t>(dim_idx + 1)] * input_shape[static_cast<size_t>(dim_idx + 1)];
  }
  ...
  if (n_dim == 1) {
    std::vector<int64_t> input_mapping = UpsampleNearestSetupRank1InputMapping(input_shape[0],
                                                                               output_shape[0],
                                                                               scales[0],
                                                                               roi[0], roi[static_cast<size_t>(n_dim + 0)],
                                                                               extrapolation_enabled,
                                                                               get_original_coordinate,
                                                                               get_nearest_pixel);

    for (int64_t output_dim0_idx = 0; output_dim0_idx < output_shape[0]; output_dim0_idx++) {
      int64_t input_dim0_idx = input_mapping[static_cast<size_t>(output_dim0_idx)];
      output[static_cast<size_t>(output_dim0_idx)]= input_dim0_idx < 0 ? extrapolation_value : input[input_dim0_idx];
    }
  ...
  if (n_dim == 2) {
    ...
    for (int64_t output_dim0_inx = 0; output_dim0_inx < output_shape[0]; output_dim0_inx++) {
      int64_t input_idx_0 = input_mapping_0[static_cast<size_t>(output_dim0_inx)];
      for (int64_t output_dim1_inx = 0; output_dim1_inx < output_shape[1]; output_dim1_inx++) {
        int64_t input_idx_1 = input_idx_0 + input_mapping_1[static_cast<size_t>(output_dim1_inx)];
    ...
    for (int64_t output_dim0_inx = 0; output_dim0_inx < output_shape[0]; output_dim0_inx++) {
      ...
      for (int64_t output_dim1_inx = 0; output_dim1_inx < output_shape[1]; output_dim1_inx++) {
        ...
        for (int64_t output_dim2_inx = 0; output_dim2_inx < output_shape[2]; output_dim2_inx++) {
          int64_t input_idx_2 = input_idx_1 + input_mapping_2[static_cast<size_t>(output_dim2_inx)];
      ...
      for (int64_t output_dim1_inx = 0; output_dim1_inx < output_shape[1]; output_dim1_inx++) {
        ...
        for (int64_t output_dim2_inx = 0; output_dim2_inx < output_shape[2]; output_dim2_inx++) {
          ...
          for (int64_t output_dim3_inx = 0; output_dim3_inx < output_shape[3]; output_dim3_inx++) {
            int64_t input_idx_3 = input_idx_2 + input_mapping_3[static_cast<size_t>(output_dim3_inx)];
            output[output_idx++] = (input_idx_3 < 0) ? static_cast<T>(extrapolation_value) : input[static_cast<size_t>(input_idx_3)];
          }
        }
      }
    }
  ...
  for (int64_t dim_idx = 0; dim_idx < n_dim; dim_idx++) {
    input_idx += input_mappings[static_cast<size_t>(dim_idx)][0 /* output_dim_counter[static_cast<size_t>(dim_idx)] */];
  }

  for (int64_t output_size = output_shape.Size(); output_idx < output_size; output_idx++) {
    output[static_cast<size_t>(output_idx)] = (input_idx < 0) ? extrapolation_value : input[static_cast<size_t>(input_idx)];

    for (int64_t dim_idx = n_dim - 1; dim_idx >= 0; dim_idx--) {
      input_idx -= input_mappings[static_cast<size_t>(dim_idx)][static_cast<size_t>(output_dim_counter[static_cast<size_t>(dim_idx)])];
      if (++output_dim_counter[static_cast<size_t>(dim_idx)] < output_shape[static_cast<size_t>(dim_idx)]) {
        input_idx += input_mappings[static_cast<size_t>(dim_idx)][static_cast<size_t>(output_dim_counter[static_cast<size_t>(dim_idx)])];
        break;
      }
      output_dim_counter[static_cast<size_t>(dim_idx)] = 0;
      input_idx += input_mappings[static_cast<size_t>(dim_idx)][0 /* output_dim_counter[static_cast<size_t>(dim_idx)] */];
    }
  }
...
static Status UpsampleLinearImpl(const std::function<void(size_t, size_t, float)>& apply,
  ...
  for (size_t i = 0, size = output_shape.Size(); i < size; i++) {
    ...
    while (step >= 0) {
      ...
      // output[static_cast<size_t>(i)] += input[old_idx] * w;
      apply(old_idx, i, w);
...
static Status UpsampleLinear(const T* input,
  ...
  auto apply = [&input, &output](size_t input_idx, size_t output_idx, float w) {
    output[static_cast<size_t>(output_idx)] += input[static_cast<size_t>(input_idx)] * w;
  };
...
BilinearParams SetupUpsampleBilinear(const int32_t input_height,
  ...
  for (int32_t y = 0; y < output_height; ++y) {
    ...
    p.dy1[static_cast<size_t>(y)] = std::fabs(in_y - in_y1);
    p.dy2[static_cast<size_t>(y)] = std::fabs(in_y - in_y2);

    if (in_y1 == in_y2) {
      p.dy1[static_cast<size_t>(y)] = 0.5f;
      p.dy2[static_cast<size_t>(y)] = 0.5f;
    }

    p.input_width_mul_y1[static_cast<size_t>(y)] = input_width * in_y1;
    p.input_width_mul_y2[static_cast<size_t>(y)] = input_width * in_y2;
  }
  ...
  for (int32_t x = 0; x < output_width; ++x) {
    ...
    p.in_x1[static_cast<size_t>(x)] = std::min(static_cast<int32_t>(in_x), input_width - 1);
    p.in_x2[static_cast<size_t>(x)] = std::min(p.in_x1[static_cast<size_t>(x)] + 1, input_width - 1);

    p.dx1[static_cast<size_t>(x)] = std::fabs(in_x - p.in_x1[static_cast<size_t>(x)]);
    p.dx2[static_cast<size_t>(x)] = std::fabs(in_x - p.in_x2[static_cast<size_t>(x)]);
    if (p.in_x1[static_cast<size_t>(x)] == p.in_x2[static_cast<size_t>(x)]) {
      p.dx1[static_cast<size_t>(x)] = 0.5f;
      p.dx2[static_cast<size_t>(x)] = 0.5f;
    }
  }
...
BilinearParamsInteger SetupUpsampleBilinearInteger(const int32_t input_height,
  ...
  for (int32_t y = 0; y < output_height; ++y) {
    ...
    const int32_t in_y1 = std::min(static_cast<int32_t>(in_y), input_height - 1);
    const int32_t in_y2 = std::min(in_y1 + 1, input_height - 1);
    p.dy1_scale_10[static_cast<size_t>(y)] = std::abs(in_y_scale_10 - in_y1 * (1 << 10));
    p.dy2_scale_10[static_cast<size_t>(y)] = std::abs(in_y_scale_10 - in_y2 * (1 << 10));

    if (in_y1 == in_y2) {
      p.dy1_scale_10[static_cast<size_t>(y)] = static_cast<int32_t>(0.5f * (1 << 10));
      p.dy2_scale_10[static_cast<size_t>(y)] = static_cast<int32_t>(0.5f * (1 << 10));
    }
  ...
  for (int32_t x = 0; x < output_width; ++x) {
    ...
    p.dx1_scale_10[static_cast<size_t>(x)] = std::abs(in_x_scale_10 - p.in_x1[static_cast<size_t>(x)] * (1 << 10));
    p.dx2_scale_10[static_cast<size_t>(x)] = std::abs(in_x_scale_10 - p.in_x2[static_cast<size_t>(x)] * (1 << 10));
    if (p.in_x1[static_cast<size_t>(x)] == p.in_x2[static_cast<size_t>(x)]) {
      p.dx1_scale_10[static_cast<size_t>(x)] = static_cast<int32_t>(0.5f * (1 << 10));
      p.dx2_scale_10[static_cast<size_t>(x)] = static_cast<int32_t>(0.5f * (1 << 10));
    }
  }
...
static TrilinearParams SetupUpsampleTrilinear(int64_t input_depth,
  ...
  p.z_original.reserve(static_cast<size_t>(output_depth));
  p.y_original.reserve(static_cast<size_t>(output_height));
  p.x_original.reserve(static_cast<size_t>(output_width));
  ...
  for (int64_t z = 0; z < output_depth; ++z) {
    ...
    p.dz1[static_cast<size_t>(z)] = std::fabs(in_z - in_z1);
    p.dz2[static_cast<size_t>(z)] = std::fabs(in_z - in_z2);

    if (in_z1 == in_z2) {
      p.dz1[static_cast<size_t>(z)] = 0.5f;
      p.dz2[static_cast<size_t>(z)] = 0.5f;
    }

    p.input_height_width_mul_z1[static_cast<size_t>(z)] = input_height * input_width * in_z1;
    p.input_height_width_mul_z2[static_cast<size_t>(z)] = input_height * input_width * in_z2;
  }
  ...
  for (int64_t x = 0; x < output_width; ++x) {
    ...
    p.in_x1[static_cast<size_t>(x)] = std::min(static_cast<int64_t>(in_x), input_width - 1);
    p.in_x2[static_cast<size_t>(x)] = std::min(p.in_x1[static_cast<size_t>(x)] + 1, input_width - 1);
            ...
            for (int64_t y = 0; y < output_height; ++y) {
              for (int64_t x = 0; x < output_width; ++x) {
                ...
                if (use_extrapolation &&
                    ((p.z_original[static_cast<size_t>(z)] < 0 || p.z_original[static_cast<size_t>(z)] > static_cast<float>(input_depth - 1)) ||
                     (p.y_original[static_cast<size_t>(y)] < 0 || p.y_original[static_cast<size_t>(y)] > static_cast<float>(input_height - 1)) ||
                     (p.x_original[static_cast<size_t>(x)] < 0 || p.x_original[static_cast<size_t>(x)] > static_cast<float>(input_width - 1)))) {
                ...
                T X111 = Xdata[p.input_height_width_mul_z1[static_cast<size_t>(z)] + p.input_width_mul_y1[static_cast<size_t>(y)] + p.in_x1[static_cast<size_t>(x)]];
                T X211 = Xdata[p.input_height_width_mul_z1[static_cast<size_t>(z)] + p.input_width_mul_y1[static_cast<size_t>(y)] + p.in_x2[static_cast<size_t>(x)]];
                T X121 = Xdata[p.input_height_width_mul_z1[static_cast<size_t>(z)] + p.input_width_mul_y2[static_cast<size_t>(y)] + p.in_x1[static_cast<size_t>(x)]];
                T X221 = Xdata[p.input_height_width_mul_z1[static_cast<size_t>(z)] + p.input_width_mul_y2[static_cast<size_t>(y)] + p.in_x2[static_cast<size_t>(x)]];

                T X112 = Xdata[p.input_height_width_mul_z2[static_cast<size_t>(z)] + p.input_width_mul_y1[static_cast<size_t>(y)] + p.in_x1[static_cast<size_t>(x)]];
                T X212 = Xdata[p.input_height_width_mul_z2[static_cast<size_t>(z)] + p.input_width_mul_y1[static_cast<size_t>(y)] + p.in_x2[static_cast<size_t>(x)]];
                T X122 = Xdata[p.input_height_width_mul_z2[static_cast<size_t>(z)] + p.input_width_mul_y2[static_cast<size_t>(y)] + p.in_x1[static_cast<size_t>(x)]];
                T X222 = Xdata[p.input_height_width_mul_z2[static_cast<size_t>(z)] + p.input_width_mul_y2[static_cast<size_t>(y)] + p.in_x2[static_cast<size_t>(x)]];

                Ydata[output_width * output_height * z + output_width * y + x] =
                    static_cast<T>(p.dx2[static_cast<size_t>(x)] * p.dy2[static_cast<size_t>(y)] * p.dz2[static_cast<size_t>(z)] * X111 +
                                   p.dx1[static_cast<size_t>(x)] * p.dy2[static_cast<size_t>(y)] * p.dz2[static_cast<size_t>(z)] * X211 +
                                   p.dx2[static_cast<size_t>(x)] * p.dy1[static_cast<size_t>(y)] * p.dz2[static_cast<size_t>(z)] * X121 +
                                   p.dx1[static_cast<size_t>(x)] * p.dy1[static_cast<size_t>(y)] * p.dz2[static_cast<size_t>(z)] * X221 +

                                   p.dx2[static_cast<size_t>(x)] * p.dy2[static_cast<size_t>(y)] * p.dz1[static_cast<size_t>(z)] * X112 +
                                   p.dx1[static_cast<size_t>(x)] * p.dy2[static_cast<size_t>(y)] * p.dz1[static_cast<size_t>(z)] * X212 +
                                   p.dx2[static_cast<size_t>(x)] * p.dy1[static_cast<size_t>(y)] * p.dz1[static_cast<size_t>(z)] * X122 +
                                   p.dx1[static_cast<size_t>(x)] * p.dy1[static_cast<size_t>(y)] * p.dz1[static_cast<size_t>(z)] * X222);
              }
            }
          }
...
float CubicInterpolation1D(const T* Xdata,
  ...
  for (int i = 0, j = -1; i < static_cast<int>(CubicModeGridLength); i++, j++) {
    ...
    result += coeff_array[static_cast<size_t>(i)] / coeff_sum * orig_data;
  }
...
void ResizeBiCubic(int64_t batch_size,
                   ...
                   const GetOriginalCoordinateFunc& get_original_coordinate) {
  std::vector<float> y_original;
  y_original.reserve(static_cast<size_t>(output_height));

  std::vector<float> x_original;
  x_original.reserve(static_cast<size_t>(output_width));
  ...
  for (int64_t y = 0; y < output_height; ++y) {
    ...
    auto s = y_original[static_cast<size_t>(y)] - std::floor(y_original[static_cast<size_t>(y)]);
  ...
  for (int64_t x = 0; x < output_width; ++x) {
    ...
    auto s = x_original[static_cast<size_t>(x)] - std::floor(x_original[static_cast<size_t>(x)]);
    if (cubic_coeffs.find(s) == cubic_coeffs.end()) {
      cubic_coeffs[s] = GetCubicCoeffs(s, cubic_coeff_a);
      coeff_to_1Dinterpolation_map[s] = {};
    }
  ...
  for (int64_t n = 0; n < batch_size; n++) {
    for (int64_t c = 0; c < num_channels; c++) {
      for (int64_t y = 0; y < output_height; ++y) {
        auto in_y = y_original[static_cast<size_t>(y)];
        ...
        if (exclude_outside) {
          ...
          for (int64_t i = 0, y_val = y_int - 1; y_val <= y_int + 2; y_val++, i++) {
            y_coeff_holder[static_cast<size_t>(i)] = (y_val < 0 || y_val >= static_cast<float>(input_height)) ? 0.0f : orig_y_coeffs[static_cast<size_t>(i)];
            y_coeff_sum += y_coeff_holder[static_cast<size_t>(i)];
          }
        }

        for (int64_t x = 0; x < output_width; ++x) {
          auto in_x = x_original[static_cast<size_t>(x)];
          ...
          if (exclude_outside) {
            ...
            for (int64_t i = 0, x_val = x_int - 1; x_val <= x_int + 2; x_val++, i++) {
              x_coeff_holder[static_cast<size_t>(i)] = (x_val < 0 || x_val >= static_cast<float>(input_width)) ? 0.0f : orig_x_coeff[static_cast<size_t>(i)];
              x_coeff_sum += x_coeff_holder[static_cast<size_t>(i)];
            }
          }
          ...
          for (int64_t y_val = y_int - 1, i = 0; y_val <= y_int + 2; y_val++, i++) {
            ...
            result += x_interpolation_result * coeff_y[static_cast<size_t>(i)] / y_coeff_sum;
          }
...
Status Upsample<T>::BaseCompute(OpKernelContext* context,
  ...
  for (std::size_t i = 0, end = output_dims.size(); i < end; i++) {
    if (no_scale && output_dims[static_cast<size_t>(i)] != dims[static_cast<size_t>(i)]) no_scale = false;
  }
...
Status Upsample<T>::Compute(OpKernelContext* context) const {
  ...
  if (!roi_cached_) {
    ...
    if (use_default_roi) {
      ...
      for (size_t i = 0; i < input_rank; ++i) {
        roi_array[static_cast<size_t>(i)] = 0;
  ...
  if (scales != nullptr && scales->Shape().Size() != 0) {
    ...
    memcpy(output_dims.data(), sizes->template Data<int64_t>(), static_cast<size_t>(sizes->Shape().Size())* sizeof(int64_t));
