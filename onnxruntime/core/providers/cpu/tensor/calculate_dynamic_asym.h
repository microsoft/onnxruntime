...
template <typename T>
class CalculateDynamicAsym : public OpKernel {
 public:
  CalculateDynamicAsym(const OpKernelInfo& info) : OpKernel(info) {
  ...
  Status Compute(OpKernelContext* context) const override {
    ...
    // Reduce Max
    // float max_co = Eigen::Map<const Eigen::Matrix<T, Eigen::Dynamic, 1>>(X, size).maxCoeff(); // <- Original
    float max_co = Eigen::Map(X).maxCoeff(); // How to use this Eigen maxCoeff thing correct on a ?tensor?
    // Max with 0
    max_co = max(0, max_co);
    // Reduce Min
    // float min_co = Eigen::Map<const Eigen::Matrix<T, Eigen::Dynamic, 1>>(X, size).minCoeff(); // <- Original
    float min_co = Eigen::Map(X).minCoeff();
    // Min with 0
    min_co = min(0, min_co);
    // Scale = (Max - Min) / QuantRange
    float scale = (max_co - min_co) / quantRange; // TODO: pass the quantrange param
    // ZP = uint8(Floor(QMin - (Min / Scale)))
    uint8_t zero_point = quantMin - (min_co / scale);
