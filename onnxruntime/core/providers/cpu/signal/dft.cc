...
ONNX_CPU_OPERATOR_KERNEL(DFT, 17, KernelDefBuilder().TypeConstraint("T1", BuildKernelDefConstraints<float, double>()).TypeConstraint("T2", BuildKernelDefConstraints<int32_t, int64_t>()), DFT);

ONNX_CPU_OPERATOR_KERNEL(STFT, 17, KernelDefBuilder().MayInplace(0, 0).TypeConstraint("T1", BuildKernelDefConstraints<float, double>()).TypeConstraint("T2", BuildKernelDefConstraints<int32_t, int64_t>()), STFT);
...
static Status fft_radix2(OpKernelContext* /*ctx*/, const Tensor* X, Tensor* Y, size_t X_offset, size_t X_stride, size_t Y_offset, size_t Y_stride, int64_t axis, size_t dft_length, const Tensor* window, bool is_onesided, bool inverse, InlinedVector<std::complex<T>>& V, InlinedVector<std::complex<T>>& temp_output) {
...
static Status dft_bluestein_z_chirp(
    OpKernelContext* ctx, const Tensor* X, Tensor* Y, Tensor& b_fft, Tensor& chirp, size_t X_offset, size_t X_stride, size_t Y_offset, size_t Y_stride, int64_t axis, size_t dft_length, const Tensor* window, bool inverse, InlinedVector<std::complex<T>>& V, InlinedVector<std::complex<T>>& temp_output) {
  static constexpr T pi = static_cast<T>(3.14159265);
  ...
  if (should_recreate) {
    ...
    ORT_RETURN_IF_ERROR((fft_radix2<T, std::complex<T>>(ctx, &b, &b_fft, 0, 1, 0, 1, 1, M, nullptr, false, false, V, temp_output)));
  }
  ...
  ORT_RETURN_IF_ERROR((fft_radix2<T, std::complex<T>>(ctx, &a, &a_fft, 0, 1, 0, 1, 1, M, nullptr, false, false, V, temp_output)));
  ...
  ORT_RETURN_IF_ERROR((fft_radix2<T, std::complex<T>>(ctx, &a_fft, &a, 0, 1, 0, 1, 1, M, nullptr, false, true, V, temp_output)));
...
template <typename T, typename U>
static Status discrete_fourier_transform(OpKernelContext* ctx, const Tensor* X, Tensor* Y, Tensor& b_fft, Tensor& chirp, int64_t axis, int64_t dft_length, const Tensor* window, bool is_onesided, bool inverse, InlinedVector<std::complex<T>>& V, InlinedVector<std::complex<T>>& temp_output) {
  // Get shape
  const auto& X_shape = X->Shape();
  const auto& Y_shape = Y->Shape();
  ...
  for (size_t i = 0; i < total_dfts; i++) {
    ...
    if (is_power_of_2(onnxruntime::narrow<size_t>(dft_length))) {
      ORT_RETURN_IF_ERROR((fft_radix2<T, U>(ctx, X, Y, X_offset, X_stride, Y_offset, Y_stride, axis, onnxruntime::narrow<size_t>(dft_length), window, is_onesided, inverse, V, temp_output)));
    } else {
      ORT_RETURN_IF_ERROR(
...
static Status discrete_fourier_transform(OpKernelContext* ctx, int64_t axis, bool is_onesided, bool inverse) {
  ...
  if (element_size == sizeof(float)) {
    ...
    if (is_real_valued) {
      ORT_RETURN_IF_ERROR((discrete_fourier_transform<float, float>(ctx, X, Y, b_fft, chirp, axis, number_of_samples, nullptr, is_onesided, inverse, V, temp_output)));
    ...
    if (is_real_valued) {
      ORT_RETURN_IF_ERROR((discrete_fourier_transform<double, double>(ctx, X, Y, b_fft, chirp, axis, number_of_samples, nullptr, is_onesided, inverse, V, temp_output)));
    } else if (is_complex_valued) {
...
static Status short_time_fourier_transform(OpKernelContext* ctx, bool is_onesided, bool /*inverse*/) {
  ...
  for (int64_t batch_idx = 0; batch_idx < batch_size; batch_idx++) {
    for (int64_t i = 0; i < n_dfts; i++) {
      ...
      // Run individual dft
      ORT_RETURN_IF_ERROR((discrete_fourier_transform<T, U>(ctx, &input, &output, b_fft, chirp, 1, window_size, window, is_onesided, false, V, temp_output)));
    }
  }
