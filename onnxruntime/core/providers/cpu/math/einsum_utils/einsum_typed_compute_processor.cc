...
void EinsumTypedComputeProcessor<T>::FinalizeOutput(const Tensor& candidate_output,
  ...
  const TensorShape& candidate_output_shape = info.transposed() ? info.output_shape : candidate_output.Shape();

  ORT_ENFORCE(candidate_output_shape.Size() == output_shape.Size(),
              "Einsum op: The candidate output cannot be reshaped into the op's output");

  const auto& candidate_output_dims = candidate_output_shape.GetDims();
  ...
  if (EinsumOp::IsTransposeRequired(candidate_output_shape_without_reduced_dims.size(), output_permutation)) {
    std::unique_ptr<Tensor> candidate_output_transposed;
    if (info.transposed()) {
      // TODO: optimize two consecutive transpose.
      // Tested with ExplicitEinsumAsBatchedMatmulWithBroadcasting_1.
      candidate_output_transposed = EinsumOp::Transpose(candidate_output, info.input_shape, info.permutation,
                                                        allocator_, einsum_ep_assets_, device_transpose_func_);
      candidate_output_transposed = EinsumOp::Transpose(*candidate_output_transposed, candidate_output_shape_without_reduced_dims,
                                                        output_permutation,
                                                        allocator_, einsum_ep_assets_, device_transpose_func_);
    } else {
      candidate_output_transposed = EinsumOp::Transpose(candidate_output, candidate_output_shape_without_reduced_dims,
                                                        output_permutation,
                                                        allocator_, einsum_ep_assets_, device_transpose_func_);
    }
    ...
    ORT_ENFORCE(status.IsOK(), "Einsum op: Could not copy the intermediate output's buffer into the op's output buffer. Error: ",
                status.ErrorMessage());
  } else if (info.transposed()) {
    auto candidate_output_transposed = EinsumOp::Transpose(candidate_output, info.input_shape, info.permutation,
                                                           allocator_, einsum_ep_assets_, device_transpose_func_);
    auto status = device_data_copy_func_(*candidate_output_transposed, output, einsum_ep_assets_);
    std::runtime_error("Not implemented : FinalizeOutput : not tested yet.");
  } else {
...
std::unique_ptr<Tensor> EinsumTypedComputeProcessor<T>::PairwiseOperandProcess(const Tensor& left,
  ...
  bool transposed_left = info_left.transposed();
  bool transposed_right = info_right.transposed();
  ...
  for (int64_t i = 0; i < left_rank; ++i) {
    ...
    if (reduce_dims_iter < reduce_dims_size && reduce_dims[reduce_dims_iter] == i) {  // This dimension is to be reduced after this pair-wise operation
      ...
      if (has_left_dim && has_right_dim) {  // Both inputs have non-trivial dim values along this dimension
        ...
        if (transposed_left) {
          throw std::runtime_error("not implemented: PairwiseOperandProcess A");
        }
        ...
        if (current_right == nullptr && transposed_right) {
          // Covered by ExplicitEinsumAsBatchedMatmulWithBroadcasting_1.
          current_right = EinsumOp::Transpose(right, info_right.input_shape, info_right.permutation,
                                              allocator_, einsum_ep_assets_, device_transpose_func_);
          current_right->Reshape(info_right.output_shape);
          current_right = EinsumOp::ReduceSum<T>(
              *current_right, right_dims, {i}, allocator_, tp_, einsum_ep_assets_, device_reduce_sum_func_);
  ...
  if (EinsumOp::IsTransposeRequired(current_left ? current_left->Shape().GetDims().size() : left_dims.size(),
    ...
    if (current_left && IsTransposeReshapeForEinsum(left_permutation,
                                                    ...
                                                    reshaped_dims)) {
      if (transposed_left) {
        throw std::runtime_error("not implemented: PairwiseOperandProcess C1");
      }
      ...
      if (transposed_left) {
        ...
        current_left = EinsumOp::Transpose(current_left ? *current_left : left,
                                           info_left.input_shape, info_left.permutation,
                                           allocator_, einsum_ep_assets_, device_transpose_func_);
        current_left->Reshape(info_left.output_shape);
        transposed_left = false;
        current_left = EinsumOp::Transpose(*current_left, current_left->Shape().GetDims(),
  ...
  if (EinsumOp::IsTransposeRequired(current_right ? current_right->Shape().GetDims().size() : right_dims.size(),
    ...
    if (current_right && IsTransposeReshapeForEinsum(right_permutation,
                                                     ...
                                                     reshaped_dims)) {
      if (transposed_left) {
        // ExplicitEinsumAsBatchedMatmulWithBroadcasting_1
        throw std::runtime_error("not implemented: PairwiseOperandProcess D");
      }
      ...
      if (transposed_right) {
        // EinsumTransposeMatMul1
        current_right = EinsumOp::Transpose(current_right ? *current_right : right,
                                            info_right.input_shape, info_right.permutation,
                                            allocator_, einsum_ep_assets_, device_transpose_func_);
        current_right->Reshape(info_right.output_shape);
        current_right = EinsumOp::Transpose(*current_right, current_right->Shape().GetDims(),
  ...
  // TODO: optimize values trans_a, trans_b, cst_b if possible.
  if (transposed_left) {
    throw std::runtime_error("not implemented: PairwiseOperandProcess E");
  }
  if (transposed_right) {
    current_right = EinsumOp::Transpose(current_right ? *current_right : right,
                                        info_right.input_shape, info_right.permutation,
  ...
  if (!is_final_pair) {  // This is not the final pair - so bring the axes order to what the inputs conformed to
    ...
    DelayedTransposedInfo info;
    FinalizeOutput(*output, current_subscript_order, info);
  }
...
Status EinsumTypedComputeProcessor<T>::Run() {
  ...
  auto& delayed_transposed_info = einsum_compute_preprocessor_.GetDelayedTransposedInfo();
...

  {
    ...
    if (reduced_dims.size() != 0) {
      if (delayed_transposed_info[0].transposed()) {
        // TODO: optimize the combination Transpose + ReduceSum
        preprocessed_inputs[0] = EinsumOp::Transpose(preprocessed_inputs[0] ? *preprocessed_inputs[0] : *raw_inputs[0],
                                                     delayed_transposed_info[0].input_shape,
                                                     delayed_transposed_info[0].permutation,
                                                     allocator_, einsum_ep_assets_, device_transpose_func_);
        preprocessed_inputs[0]->Reshape(delayed_transposed_info[0].output_shape);
        delayed_transposed_info[0].clear();
        result = EinsumOp::ReduceSum<T>(*preprocessed_inputs[0],
    ...
    if (num_inputs == 1) {
      ...
      FinalizeOutput(result ? *result : *raw_inputs[0], preserved_dims, delayed_transposed_info[0]);
      return Status::OK();
    }
  }
  ...
  {
    DelayedTransposedInfo not_transposed;
    ...
    for (int input = 1; input < num_inputs; ++input) {
      ...
      result = PairwiseOperandProcess(result ? *result : *raw_inputs[0],
                                      ...
                                      input == 0 ? delayed_transposed_info[input - 1] : not_transposed,
                                      delayed_transposed_info[input]);
    }
  }
