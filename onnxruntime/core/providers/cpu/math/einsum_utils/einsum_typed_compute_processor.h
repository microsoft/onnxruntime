...
class EinsumTypedComputeProcessor {
 public:
  ...
  void SetDeviceHelpers(const EinsumOp::DeviceHelpers::Transpose& device_transpose_func,
                        ...
                        const EinsumOp::DeviceHelpers::CreateTensor& device_create_tensor_func);
 ...
 private:
  ...
  EinsumOp::DeviceHelpers::CreateTensor device_create_tensor_func_;
...
class EinsumTypedComputeProcessor {
 public:
  explicit EinsumTypedComputeProcessor(EinsumOpKernelContext* context, AllocatorPtr allocator,
                                       ...
                                       const void* mlas_backend_config,
                                       EinsumComputePreprocessor& einsum_compute_preprocessor,
                                       void* einsum_ep_assets)
      : context_(context),
        allocator_(allocator),
        tp_(tp),
        mlas_backend_config_(mlas_backend_config),
        einsum_compute_preprocessor_(einsum_compute_preprocessor),
        einsum_ep_assets_(einsum_ep_assets) {}
  ...
  void SetDeviceHelpers(const EinsumOp::DeviceHelpers::Transpose& device_transpose_func,
    ...
    device_create_tensor_func_ = device_create_tensor_func;
  }
 ...
 private:
  ...
  const void* mlas_backend_config_;
  EinsumComputePreprocessor& einsum_compute_preprocessor_;
...
#ifndef SHARED_PROVIDER
template <typename T>
inline void EinsumTypedComputeProcessor<T>::FinalizeOutput(const Tensor& candidate_output,
                                                           const gsl::span<const int64_t>& ordered_subscript_indices_in_candidate) {
  ...
  if (EinsumOp::IsTransposeRequired(candidate_output_shape_without_reduced_dims.size(), output_permutation)) {
    auto candidate_output_transposed = EinsumOp::Transpose(candidate_output, candidate_output_shape_without_reduced_dims,
                                                           output_permutation,
                                                           allocator_, einsum_ep_assets_, device_transpose_func_, device_create_tensor_func_);
...
static inline bool IsTransposeReshapeForEinsum(const gsl::span<const size_t>& perm,
...
#ifndef SHARED_PROVIDER
template <typename T>
inline std::unique_ptr<Tensor> EinsumTypedComputeProcessor<T>::PairwiseOperandProcess(const Tensor& left,
                                                                                      const TensorShape& left_shape_override,
                                                                                      const Tensor& right,
                                                                                      const TensorShape& right_shape_override,
                                                                                      const gsl::span<const int64_t>& reduce_dims,
    ...
    if (reduce_dims_iter < reduce_dims_size && reduce_dims[reduce_dims_iter] == i) {
      ...
      if (has_left_dim && has_right_dim) {  // Both inputs have non-trivial dim values along this dimension
        ...
        current_left = EinsumOp::ReduceSum<T>(
            tensor_to_be_reduced, tensor_to_be_reduced_dims, AsSpan({i}), allocator_, tp_, einsum_ep_assets_, device_reduce_sum_func_, device_create_tensor_func_);
  ...
  TensorShapeVector current_subscript_order;
  ...
  auto output = EinsumOp::MatMul<T>(current_left ? *current_left : left, TensorShapeVector{lro_size, lo_size, reduced_size},
                                    ...
                                    allocator_, tp_, mlas_backend_config_,
                                    einsum_ep_assets_, device_matmul_func_, device_create_tensor_func_);
    ...
    if (EinsumOp::IsTransposeRequired(output_dims.size(), output_permutation)) {
      if (IsTransposeReshapeForEinsum(output_permutation,
        ...
        output = EinsumOp::Transpose(*output, output_dims, output_permutation, allocator_,
                                     einsum_ep_assets_, device_transpose_func_, device_create_tensor_func_);
      }
    }
...
#ifndef SHARED_PROVIDER
template <typename T>
inline Status EinsumTypedComputeProcessor<T>::Run() {
...

  {
    ...
    if (reduced_dims.size() != 0) {
      result = EinsumOp::ReduceSum<T>(preprocessed_inputs[0] ? *preprocessed_inputs[0] : *raw_inputs[0],
                                      ...
                                      einsum_ep_assets_, device_reduce_sum_func_, device_create_tensor_func_);
    } else {
...
#ifndef SHARED_PROVIDER
template <typename T>
inline void EinsumTypedComputeProcessor<T>::SetDeviceHelpers(const EinsumOp::DeviceHelpers::Transpose& device_transpose_func,
                                                             const EinsumOp::DeviceHelpers::MatMul<T>& device_matmul_func,
                                                             const EinsumOp::DeviceHelpers::ReduceSum<T>& device_reduce_sum_func,
                                                             const EinsumOp::DeviceHelpers::DataCopy& device_data_copy_func,
                                                             const EinsumOp::DeviceHelpers::ZeroBuffer& device_zero_buffer_func,
