...
class EinsumComputePreprocessor final {
 public:
  explicit EinsumComputePreprocessor(EinsumEquationPreprocessor& einsum_equation_preprocessor,
  ...
  inline const TensorShapeVector& GetOutputDims() const {
    return output_dims_;
  }

  inline std::vector<std::unique_ptr<Tensor>>& GetPreprocessedInputTensors() {
  ...
  inline const std::vector<TensorShape>& GetHomogenizedInputDims() {
    return homogenized_input_dims_;
  }

  inline const std::vector<int64_t>& GetMappedSubscriptIndicesToLastInputIndex() const {
    return subscript_indices_to_last_input_;
  }

  inline const std::vector<int64_t>& GetMappedSubscriptIndicesToOutputindices() const {
 ...
 private:
  inline Status ProcessSubscripts() {
      ...
      for (auto subscript_label : subscript) {
        ...
        if (subscript_label == '.') {
          ...
          if (letter_to_count_[onnxruntime::narrow<size_t>(letter_index)] == 0) {
            ...
            if (subscript_indices_to_dim_value_[onnxruntime::narrow<size_t>(mapped_index)] != dim_value) {
              // Set the value to the new dim value if the value is 1 in the map
              if (subscript_indices_to_dim_value_[onnxruntime::narrow<size_t>(mapped_index)] == 1) {
                subscript_indices_to_dim_value_[onnxruntime::narrow<size_t>(mapped_index)] = dim_value;
              } else {
                if (dim_value != 1) {
                  return ORT_MAKE_STATUS(ONNXRUNTIME, INVALID_ARGUMENT,
  ...
  inline Status PostProcessBroadcastedDims() {
  ...
  inline Status ParseOrCreateOutputSubscript() {
  ...
  inline Status CalculateOutputShape() {
    ...
    for (auto subscript_label : einsum_equation_preprocessor_.right_equation_) {
      if (subscript_label == '.') {
        ...
        subscript_indices_to_last_input_[onnxruntime::narrow<size_t>(mapped_index)] = -1;

        subscript_indices_to_output_indices_[onnxruntime::narrow<size_t>(mapped_index)] = output_dim_counter++;
      }
    }
  ...
  inline Status PreprocessInputs() {
    ...
    for (const auto* input : inputs_) {
      ...
      for (const auto& subscript_index : current_subscript_indices) {
        if (subscript_indices_to_input_index[onnxruntime::narrow<size_t>(subscript_index)] == -1) {  // This is the first time we are seeing this subscript label in this input
  ...
  // Device specific create tensor function
