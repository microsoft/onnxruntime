...
Status ScanImpl::Execute(const FeedsFetchesManager& ffm) {
  ...
  scan_input_stream_iterators.reserve(static_cast<size_t>(info_.num_inputs) - static_cast<uint64_t>(info_.num_loop_state_variables));
  ...
  status = IterateSequence(context_, session_state_, loop_state_variables, scan_input_stream_iterators,
                           sequence_len_, info_.num_loop_state_variables, static_cast<uint64_t>(info_.num_inputs), info_.num_outputs,
