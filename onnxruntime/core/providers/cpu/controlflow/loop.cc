...
Status LoopImpl::Execute(const FeedsFetchesManager& ffm) {
  ...
  auto copy_tensor_from_mlvalue_to_output = [this](OrtValue& input, int output_idx,
                                                   int64_t iter_num_value) {
    if (input.IsTensor()) {
      ...
      if (iter_num_value != 0) {
        ...
        *output = std::move(*input.GetMutable<TensorSeq>());
        ...
        // We can't move the feeds to the Loop operator into its fetches.
  ...
  if (iter_num_value != 0) {
    for (int i = 0; i < info_.num_loop_carried_vars; ++i) {
      ...
      copy_tensor_from_mlvalue_to_output(fetches[i + 1], i, iter_num_value);  // skip cond
    }
    ...
    for (int i = 0; i < info_.num_loop_carried_vars; ++i) {
      copy_tensor_from_mlvalue_to_output(feeds[i + 2], i, iter_num_value);  // skip iter# and cond
    }
