...
Status LoopImpl::Execute(const FeedsFetchesManager& ffm) {
  ...
  if (iter_num_value != 0) {
    for (uint64_t i = 0; i < info_.num_loop_carried_vars; ++i) {
      ...
      ORT_RETURN_IF_ERROR(copy_mlvalue_to_output(fetches[(i) + 1], static_cast<int>(i), iter_num_value, *info_.loop_carried_vars_types[i]));  // skip cond
    }
    ...
    for (uint64_t i = 0; i < info_.num_loop_carried_vars; ++i) {
      ORT_RETURN_IF_ERROR(copy_mlvalue_to_output(feeds[i + 2], static_cast<int>(i), iter_num_value, *info_.loop_carried_vars_types[i]));  // skip iter# and cond
    }
