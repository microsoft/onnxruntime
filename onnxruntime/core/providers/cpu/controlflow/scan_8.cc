    ...
    if (batch_size_ < 0)
      ...
      if (batch_size_ != this_batch_size) {
        return ORT_MAKE_STATUS(ONNXRUNTIME, FAIL, "Scan inputs have inconsistent batch size. Previous value was ",
                               batch_size_, " but ", graph_inputs[static_cast<uint64_t>(i)]->Name(),
                               " has batch size of ",
                               this_batch_size);
      }
    }
...
Status Scan8Impl::Execute(const FeedsFetchesManager& ffm) {
  ...
  for (int64_t b = 0; b < batch_size_; ++b) {
    ...
    scan_input_stream_iterators.reserve(
            static_cast<size_t>(info_.num_variadic_inputs) - static_cast<uint64_t>(info_.num_loop_state_variables));
    ...
    for (int i = info_.num_loop_state_variables, end = info_.num_variadic_inputs; i < end; ++i) {
      ...
      // forward
      if (directions_[static_cast<uint64_t>(static_cast<ptrdiff_t>(i) - info_.num_loop_state_variables)] ==
    ...
    for (int64_t i = sequence_len; i < max_sequence_len_; ++i) {
      for (auto output = info_.num_loop_state_variables; output < info_.num_outputs; ++output) {
