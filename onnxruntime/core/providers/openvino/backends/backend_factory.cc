...
BackendFactory::MakeBackend(const ONNX_NAMESPACE::ModelProto& model_proto,
  ...
  } else if (type == "CPU" || type.find("GPU") != std::string::npos || type == "MYRIAD" || type.find("HETERO") != std::string::npos ||
            type.find("MULTI") != std::string::npos || type.find("AUTO") != std::string::npos) {
