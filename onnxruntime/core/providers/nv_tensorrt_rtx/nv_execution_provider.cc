...
// External memory import support for D3D12 ↔ CUDA interop
bool NvExecutionProvider::CanImportExternalMemory(OrtExternalMemoryHandleType handle_type) const {
#ifdef _WIN32
  // Only support D3D12 resources and heaps on Windows
...
#ifdef _WIN32
// Include CUDA driver API header for external memory import
#include <cuda.h>
...
Status NvExecutionProvider::ImportExternalMemory(const OrtExternalMemoryDescriptor& mem_desc,
  ...
  if (mem_desc.handle_type != ORT_EXTERNAL_MEMORY_HANDLE_TYPE_D3D12_RESOURCE &&
    ...
    return ORT_MAKE_STATUS(ONNXRUNTIME, INVALID_ARGUMENT,
                           "NvTensorRtRtx EP only supports D3D12_RESOURCE and D3D12_HEAP handle types");
  }
  ...
  // Import D3D12 resource using CUDA driver API
  CUDA_EXTERNAL_MEMORY_HANDLE_DESC ext_mem_desc = {};
  ext_mem_desc.type = CU_EXTERNAL_MEMORY_HANDLE_TYPE_D3D12_RESOURCE;
  ...
  if (mem_desc.handle_type == ORT_EXTERNAL_MEMORY_HANDLE_TYPE_D3D12_HEAP) {
    ext_mem_desc.type = CU_EXTERNAL_MEMORY_HANDLE_TYPE_D3D12_HEAP;
  }

  CUexternalMemory ext_mem = nullptr;
  CUresult cu_result = cuImportExternalMemory(&ext_mem, &ext_mem_desc);
  if (cu_result != CUDA_SUCCESS) {
    return ORT_MAKE_STATUS(ONNXRUNTIME, FAIL,
                           "cuImportExternalMemory failed with error ", cu_result);
  }
  ...
  cu_result = cuExternalMemoryGetMappedBuffer(&device_ptr, ext_mem, &buf_desc);
  if (cu_result != CUDA_SUCCESS) {
    cuDestroyExternalMemory(ext_mem);
    return ORT_MAKE_STATUS(ONNXRUNTIME, FAIL,
                           "cuExternalMemoryGetMappedBuffer failed with error ", cu_result);
  }

  // Import timeline semaphores for synchronization
  CUexternalSemaphore wait_semaphore = nullptr;
  CUexternalSemaphore signal_semaphore = nullptr;
  ...
  if (mem_desc.wait_semaphore_handle != nullptr) {
    ...
    cu_result = cuImportExternalSemaphore(&wait_semaphore, &sem_desc);
    if (cu_result != CUDA_SUCCESS) {
      cuDestroyExternalMemory(ext_mem);
      return ORT_MAKE_STATUS(ONNXRUNTIME, FAIL,
                             "cuImportExternalSemaphore (wait) failed with error ", cu_result);
    }
  }
  ...
  if (mem_desc.signal_semaphore_handle != nullptr &&
    ...
    cu_result = cuImportExternalSemaphore(&signal_semaphore, &sem_desc);
    if (cu_result != CUDA_SUCCESS) {
      if (wait_semaphore) cuDestroyExternalSemaphore(wait_semaphore);
      cuDestroyExternalMemory(ext_mem);
      return ORT_MAKE_STATUS(ONNXRUNTIME, FAIL,
                             "cuImportExternalSemaphore (signal) failed with error ", cu_result);
    }
  }
  ...
  if (wait_semaphore != nullptr &&
    ...
    cu_result = cuWaitExternalSemaphoresAsync(&wait_semaphore, &wait_params, 1, stream_);
    if (cu_result != CUDA_SUCCESS) {
      if (signal_semaphore) cuDestroyExternalSemaphore(signal_semaphore);
      if (wait_semaphore) cuDestroyExternalSemaphore(wait_semaphore);
      cuDestroyExternalMemory(ext_mem);
      return ORT_MAKE_STATUS(ONNXRUNTIME, FAIL,
                             "cuWaitExternalSemaphoresAsync failed with error ", cu_result);
    }
  }

  // Create tensor wrapping the external memory
  // Note: The device pointer lifecycle is managed by the external memory handle
  // We need to ensure proper cleanup when the tensor is destroyed
  void* data_ptr = reinterpret_cast<void*>(device_ptr);
  
  // Get allocator for the device
  AllocatorPtr allocator = GetAllocator(default_device_.Id(), OrtMemTypeDefault);
  if (!allocator) {
    if (signal_semaphore) cuDestroyExternalSemaphore(signal_semaphore);
    if (wait_semaphore) cuDestroyExternalSemaphore(wait_semaphore);
    cuDestroyExternalMemory(ext_mem);
  ...
  auto ml_tensor = Tensor::Create(element_type, shape, data_ptr, allocator->Info());
  ...
  if (signal_semaphore != nullptr) {
    ...
    cu_result = cuSignalExternalSemaphoresAsync(&signal_semaphore, &signal_params, 1, stream_);
    if (cu_result != CUDA_SUCCESS) {
      // Log error but don't fail since tensor creation succeeded
      LOGS_DEFAULT(WARNING) << "cuSignalExternalSemaphoresAsync failed with error " << cu_result;
    }
  }
