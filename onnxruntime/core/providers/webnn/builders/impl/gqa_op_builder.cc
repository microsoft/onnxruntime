...
Status GroupQueryAttentionOpBuilder::AddToModelBuilderImpl(ModelBuilder& model_builder, const Node& node,
  ...
  */
  ...
  emscripten::val left_constant = model_builder.GetBuilder().call<emscripten::val>("constant", common_desc, left_buffer);
  ...
  */
  const std::vector<int32_t> mask_shape_ones_shape(batch_size * num_heads * qkv_sequence_length * past_sequence_length,
                                                   1);
  ORT_RETURN_IF_NOT(SetWebnnDataType(common_desc, ONNX_NAMESPACE::TensorProto_DataType_INT32),
  ...
  emscripten::val mask_shape_ones_shape_constant = model_builder.GetBuilder().call<emscripten::val>(
      "constant", common_desc, mask_shape_ones_shape_buffer);
  ...
  emscripten::val pre_neq_right_data_range_constant = model_builder.GetBuilder().call<emscripten::val>(
      "constant", common_desc, pre_neq_right_data_range_buffer);
...
bool GroupQueryAttentionOpBuilder::IsOpSupportedImpl(const InitializedTensorSet& initializers, const Node& node,
  ...
  if (!Contains(initializers, total_sequence_length_name)) {
    LOGS(logger, VERBOSE) << "total_sequence_length must be a constant initializer";
    return false;
  }
