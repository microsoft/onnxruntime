...
common::Status DataTransfer::CopyTensor(const Tensor& src, Tensor& dst) const {
  ...
  if (bytes > 0) {
    ...
    if (dst_device.Type() == OrtDevice::GPU) {
      ...
      if (trace) {
        console.call<void>("timeEnd", emscripten::val("ORT::DataTransfer::jsepUploadTensor"));
      }
    } else {
      ...
      if (trace) {
        console.call<void>("timeEnd", emscripten::val("ORT::DataTransfer::jsepDownloadTensor"));
      }
    }
  }
