    ...
    *model_proto->mutable_graph() = *graph_body.ToGraphProto();
      ...
      if (plan_file && engine_cache_enable_) {
        ...
        if (trt_engine == nullptr) {
          return ORT_MAKE_STATUS(ONNXRUNTIME, EP_FAIL,
                                 "TensorRT EP could not deserialize engine from file", cached_path);
        }
      } else if (!plan_file && engine_decryption_enable_ && engine_cache_enable_) {
    ...
    compute_info.create_state_func = [=](ComputeContext* context, FunctionState* state) {
      ...
      *p = {context->allocate_func, context->release_func, context->allocator_handle, parsers_[context->node_name].get(),
            ...
            &max_workspace_size_, trt_node_name_with_precision, engine_cache_enable_, engine_cache_path_,
	    runtime_, engine_decryption_enable_, engine_decryption_lib_path_};
      *state = p.release();
      return 0;
    };
    ...
    compute_info.compute_func = [](FunctionState state, const OrtCustomOpApi* api, OrtKernelContext* context) {
      ...
      if (engine_update) {
        ...
        } else if (!plan_file && trt_state->engine_decryption_enable && trt_state->engine_cache_enable) {
