...
std::vector<AllocatorPtr> TensorrtExecutionProvider::CreatePreferredAllocators() {
  ...
  std::vector<AllocatorPtr> ret;  // TODO(leca): REVIEW: Does tensorRT need its own allocators or reuse CUDA's allocator?
  ret.reserve(2);
  ret.push_back(CreateAllocator(default_memory_info));
  ret.push_back(CreateAllocator(pinned_allocator_info));
  return ret;
}
...
common::Status TensorrtExecutionProvider::Compile(const std::vector<FusedNodeAndGraph>& fused_nodes_and_graphs,
  ...
  for (auto& fused_node_graph : fused_nodes_and_graphs) {
    ...
    compute_info.compute_func = [this](FunctionState state, const OrtApi* api, OrtKernelContext* context) {
      ...
      OrtAllocator* alloc;
      Ort::ThrowOnError(api->KernelContext_CGetAllocator(context, &mem_info, &alloc));
