...
struct Tensorrt_Provider : Provider {
  ...
  std::cout << "CreateExecutionProviderFactory provider_options: device_id: " << options.device_id //slx
  << ", has_user_compute_stream: " << options.has_user_compute_stream  
  << ", user_compute_stream: " << options.user_compute_stream  
  << ", max_partition_iterations: " << options.trt_max_partition_iterations
  << ", min_subgraph_size: " << options.trt_min_subgraph_size  
  << ", max_workspace_size: " << options.trt_max_workspace_size  
  << ", fp16_enable: " << options.trt_fp16_enable  
  << ", int8_enable: " << options.trt_int8_enable  
  << ", int8_calibration_table_name: " << (options.trt_int8_calibration_table_name == nullptr ? "" : options.trt_int8_calibration_table_name) 
  << ", int8_use_native_calibration_table: " << options.trt_int8_use_native_calibration_table
  << ", dla_enable: " << options.trt_dla_enable  
  << ", dla_core: " << options.trt_dla_core  
  << ", fp16_enable: " << options.trt_fp16_enable  
  << ", dump_subgraphs: " << options.trt_dump_subgraphs  
  << ", engine_cache_enable: " << options.trt_engine_cache_enable 
  << ", engine_cache_path: " << (options.trt_engine_cache_path == nullptr ? "" : options.trt_engine_cache_path) 
  << ", engine_decryption_enable: " << options.trt_engine_decryption_enable  
  << ", engine_decryption_lib_path: " << (options.trt_engine_decryption_lib_path == nullptr ? "" : options.trt_engine_decryption_lib_path)
  << ", force_sequential_engine_build: " << options.trt_force_sequential_engine_build << std::endl;
