...
//     //assert(d_ >= 1 && d_ <= INT32_MAX);

//     for (shift = 0; shift < 32; shift++) if ((1 << shift) >= d_) break;

//     int64_t one = 1;
//     int64_t magic = ((one << 32) * ((one << shift) - d_)) / d_ + 1;
//     m1 = magic;
...
//   int d_;  // d above.
//   int m1;  // Magic number: m' above.
//   int shift;  // Shift amounts.
// };
