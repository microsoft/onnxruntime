...
void OneHotImpl(
  ...
  hipLaunchKernelGGL(HIP_KERNEL_NAME(_OneHotImpl<in_type, out_type>), dim3(blocksPerGrid), dim3(GridDim::maxThreadsPerBlock), 0, 0, 
    indices_data,
    fdm_depth_suffix,
    fdm_suffix,
    depth_val,
...
void OneHotWithZeroOffValueImpl(
  ...
  int blocksPerGrid = (int)(ceil(static_cast<float>(count) / GridDim::maxThreadsPerBlock));
  HIP_LONG N = static_cast<HIP_LONG>(count);
  hipLaunchKernelGGL(HIP_KERNEL_NAME(_OneHotWithZeroOffValueImpl<in_type, out_type>), dim3(blocksPerGrid), dim3(GridDim::maxThreadsPerBlock), 0, 0, 
    indices_data,
    fdm_suffix,
    depth_val,
    on_value,
