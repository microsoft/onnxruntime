...
Status Where<T>::ComputeInternal(OpKernelContext* context) const {
  ...
  WhereImpl<HipT>(
      ...
      reinterpret_cast<const HipT*>(prepare.b_tensor->template Data<T>()),
      prepare.c_index_type,
      prepare.c_padded_strides,
      reinterpret_cast<const HipT*>(prepare.c_tensor->template Data<T>()),
      prepare.fdm_output_strides,
      reinterpret_cast<HipT*>(output_tensor->template MutableData<T>()),
