...
struct WeightLayoutInfo {
  std::shared_ptr<Tensor> marshalled_initializer = nullptr;  // TODO: change it to unique_ptr
  std::string layout = "";                                   // layout name
  tvm::Tensor marshalled_tensor;
  tvm::Tensor unmarshalled_tensor;
  bool is_marshalled;

  WeightLayoutInfo(const tvm::Tensor& tvm_tensor)
...
struct InitializerInfo {
  ...
  std::unique_ptr<WeightLayoutInfo> layout_info = nullptr;
