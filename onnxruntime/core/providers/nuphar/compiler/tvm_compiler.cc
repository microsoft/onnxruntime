...
#include "core/providers/nuphar/common/nuphar_tvm_utils.h"  // rename this file
...
Status NupharCompiler::Build(const Node& node) {
  if (node.OpType() == "Scan")
  ...
  bool is_fused = (node.NodeType() == Node::Type::Fused);
  if (is_fused) {
    const onnxruntime::Graph& onnx_func_body = node.GetFunctionBody()->Body();
    ORT_RETURN_IF_ERROR(CreateTVMIR(GraphViewer(onnx_func_body), context_, /*use_placeholder_for_input*/ true));
  } else {
    ORT_RETURN_IF_ERROR(CreateTVMIR(node, context_));
  }
  ...
  node.ForEachWithIndex(
      ...
      [this, &visited_alias_def](const NodeArg& def, size_t) {
        auto input_def = codegen::Promote<codegen::CodeGenUnitStats>(context_.GetGraphStats())->SourceDefOfOutputAlias(&def);
        if (input_def) {
...
tvm::runtime::PackedFunc NupharCompiler::GetLoweredPackedFunc(
  ...
  // TODO: refactor the following logic for boht JIT-caching and AOT support
  ...
  if (cached_func == nullptr) {
    ...
    if (settings.HasOption(codegen::CodeGenSettings::kCodeGenDumpLower)) {
      if (settings.OptionMatches(codegen::CodeGenSettings::kCodeGenDumpLower, "verbose") ||
        ...
        LOGS_DEFAULT(CODEGEN_SETTINGS_LOG_LEVEL) << "[NUPHAR_DUMP_LOWER] Subgraph Type: "
...
static tvm::BuildConfig CreateConfig(const Node& node,
  ...
  if (IsRecurrentNode(node) || allow_unaligned_buffers)
    config->data_alignment = 1;
  return config;
}

// Lower compiles the tvm::Tensor to a function
// TODO: remove this funciton after confirming single node really won't happen.
Status NupharCompiler::Lower(const Node& node,
                             tvm::Target tvm_target,
                             tvm::Target tvm_host_target,
                             NupharFuncInfo* func_info) {
  return ORT_MAKE_STATUS(ONNXRUNTIME, FAIL, "Reach unexpected single node lowering in NupharCompiler");
}
...
Status NupharCompiler::Lower(const nuphar::NupharSubgraphUnit& subgraph,
  ...
  std::string func_name = nuphar_codegen::GetPackedFuncName(subgraph, target_codegen);
