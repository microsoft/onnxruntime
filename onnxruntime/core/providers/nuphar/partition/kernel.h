...
using NupharFuncStateToComputeCtxMap =
    std::unordered_map<const NupharKernelState*, std::unique_ptr<tvm_codegen::KernelComputeCtx>>;
...
class NupharKernelState {
 public:
  ...
  void Compile(const Node& node, TryGetConstantFunc try_get_constant_func);
 ...
 private:
  ...
  std::map<std::string, const Tensor*> initializers_;
  ...
  std::vector<std::unique_ptr<tvm_codegen::NupharFuncInfo>> func_infos_;
