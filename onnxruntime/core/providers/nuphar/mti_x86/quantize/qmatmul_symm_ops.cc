...
#include "core/providers/nuphar/mti_x86/quantize/qmatmul_symm_ops.h"
...
TVM_REGISTER_GLOBAL("tvm.contrib.onnxruntime.qmatmulsymmetric.mkl")
    .set_body([](tvm::TVMArgs args, tvm::TVMRetValue* /*ret*/) {
      ...
      auto quantized_param = reinterpret_cast<int16_t*>(static_cast<char*>(transposed_quantized_param->data) + transposed_quantized_param->byte_offset);
      auto quantized_X = reinterpret_cast<int16_t*>(static_cast<char*>(Q_X->data) + Q_X->byte_offset);
      ...
      MKLIntGemvS16S16S32R(quantized_param, quantized_X, embed_dim, batch, input_dim, quantized_Y);
    });

TVM_REGISTER_GLOBAL("tvm.contrib.onnxruntime.qmatmulsymmetric.avx2")
    .set_body([](tvm::TVMArgs args, tvm::TVMRetValue* /*ret*/) {
      DLTensor* transposed_quantized_param = args[0];
      DLTensor* Q_X = args[1];
      DLTensor* batch_tensor = args[2];
      ...
      if (batch == 1) {
        if (input_dim % 16 == 0)
          AVX2IntGemvS16S16S32R(quantized_param, quantized_X, input_dim, input_dim, embed_dim, quantized_Y);
        else
          AVX2IntGemvS16S16S32REx(quantized_param, quantized_X, input_dim, embed_dim, quantized_Y);
      } else {
        MKLIntGemvS16S16S32R(quantized_param, quantized_X, embed_dim, batch, input_dim, quantized_Y);
      }
    });
...
QMatMulSymmetricMKL(const tvm::Tensor& transposed_quantized_param,
  ...
  func_str = "tvm.contrib.onnxruntime.qmatmulsymmetric.mkl";
#else
...
QMatMulSymmetricAVX2(const tvm::Tensor& transposed_quantized_param,
  ...
  func_str = "tvm.contrib.onnxruntime.qmatmulsymmetric.avx2";
#else
