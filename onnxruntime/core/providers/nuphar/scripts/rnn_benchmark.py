...
def generate_model(rnn_type, input_dim, hidden_dim, bidirectional, layers, model_name):
    ...
    model.graph.input.add().CopyFrom(helper.make_tensor_value_info('input', onnx.TensorProto.FLOAT, ['s', 'b', input_dim]))
...
def perf_run(sess, feeds, min_duration_seconds=10, min_counts=5):
    # warm up
    sess.run([], feeds)
...
def perf_test(rnn_type, num_threads, input_dim, hidden_dim, bidirectional, layers, seq_len, batch_size, top_n=5):
    set_num_threads(num_threads)

    model_name = rnn_type + '_i' + str(input_dim) + '_h' + str(hidden_dim) + ('_bi' if bidirectional else '') + '_l' + str(layers) + '.onnx'
    generate_model(rnn_type, input_dim, hidden_dim, bidirectional, layers, model_name)
    feeds = {'input':np.random.rand(seq_len, batch_size, input_dim).astype(np.float32)}

    # run original model
    sess = onnxruntime.InferenceSession(model_name)
    count, duration, per_iter_cost = perf_run(sess, feeds, min_counts=top_n)
    ...
    scan_model_name = model_name + '_scan.onnx'
    ...
    int8_model_name = model_name + '_int8.onnx'
