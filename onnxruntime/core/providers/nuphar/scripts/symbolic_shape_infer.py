...
assert version.parse(onnx.__version__) >= version.parse("1.4.1") # need at least opset 9 for Scan shape inference
...
def get_shape_from_sympy_shape(sympy_shape):
    return [int(i) if is_literal(i) else str(i) for i in sympy_shape]
...
class SymbolicShapeInference:
    ...
    def _broadcast_shapes(self, shape1, shape2):
        ...
        for i in range(new_rank):
            ...
            if dim1 == 1 or dim1 == dim2:
                ...
                if not new_dim:
                    raise 'unsupported broadcast between ' + str(dim1) + ' ' + str(dim2)
    ...
    def _pass_on_sympy_data(self, node):
        ...
        if node.input[0] in self.sympy_data_ or node.input[0] in self.initializers_:
            self.sympy_data_[node.output[0]] = self._get_value(node, 0)
    ...
    def _infer_Concat(self, node):
        if any([i in self.sympy_data_ for i in node.input]):
            assert all([i in self.sympy_data_ or i in self.initializers_ for i in node.input])
            assert 0 == get_attribute(node, 'axis')
            self.sympy_data_[node.output[0]] = []
            for i in range(len(node.input)):
                value = self._get_value(node, i)
    ...
    def _infer_Expand(self, node):
        if node.input[1] in self.sympy_data_:
            input_shape = self._get_shape(node, 0)
            target_shape = get_shape_from_sympy_shape(self.sympy_data_[node.input[1]])
    ...
    def _infer_Mul(self, node):
        if all([i in self.sympy_data_ for i in node.input]):
            is_input0_list = type(self.sympy_data_[node.input[0]]) == list
            is_input1_list = type(self.sympy_data_[node.input[1]]) == list
            assert is_input0_list == is_input1_list
            if is_input0_list:
                self.sympy_data_[node.output[0]] = [d * self.sympy_data_[node.input[1]][i] for i,d in enumerate(self.sympy_data_[node.input[0]])]
            else:
                self.sympy_data_[node.output[0]] = self.sympy_data_[node.input[0]] * self.sympy_data_[node.input[1]]
    ...
    def _infer_Reshape(self, node):
        ...
        assert new_shape.count(-1) < 2
        if -1 in new_shape:
            new_dim = total / non_deferred_size
            new_shape[deferred_dim_idx] = new_dim
    ...
    def _infer_Slice(self, node):
        ...
        for i,s,e,t in zip(axes, starts, ends, steps):
            ...
            if is_literal(e) and is_literal(new_shape[i]) and int(e) > int(new_shape[i]):
                    e = new_shape[i]
    ...
    def _infer_impl(self, in_mp):
        ...
        for node in self.out_mp_.graph.node:
            ...
            for i_o in range(len(node.output)):
                ...
                if self.verbose_ > 2:
                    print('  ' + node.output[i_o] + ': ' + str(out_shape))
                    if node.output[i_o] in self.sympy_data_:
                        print('  Sympy Data: ' + str(self.sympy_data_[node.output[i_o]]))
                if None in out_shape:
                    if self.auto_merge_ and node.op_type in ['Add', 'Sub', 'Mul', 'Div', 'MatMul']:
    ...
    def infer_shapes(input_model, output_model, auto_merge=False, verbose=0):
        ...
        while symbolic_shape_inference.run_:
            symbolic_shape_inference._infer_impl(in_mp)
