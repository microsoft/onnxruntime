    ...
    if in_node.doc_string:
        ...
        if 'custom_attributes' in qcfg_str:
            ...
            if qcfg_str[1] == 'c':
                ...
                qcfg_str = qcfg_str.replace(',', ',"')
                qcfg_str = qcfg_str.replace(':', '":')
            custom_qcfg = json.loads(qcfg_str)['custom_attributes']
...
def quantize_matmul_2d_with_weight(in_node, in_graph, nf, converted_weights, qcfg_dict, update_qcfg_dict, default_qcfg):
...
def convert_matmul_model(input_model, output_model, only_for_scan=False, preset_str='asymm8_param0_input1', qcfg_json=None, export_qcfg_json=None):
    ...
    ms_opset = out_mp.opset_import.add()
    ...
    for in_n in in_mp.graph.node:
        ...
        if in_n.op_type == 'MatMul' and not only_for_scan:
            if quantize_matmul_2d_with_weight(in_n, in_mp.graph, nf, converted_weights, qcfg_dict, export_qcfg_json, default_qcfg):
                continue
        ...
        if in_n.op_type == 'Scan':
            ...
            for in_sn in in_body.g.node:
                if in_sn.op_type == 'MatMul':
                    if quantize_matmul_2d_with_weight(in_sn, in_body.g, scan_nf, converted_weights, qcfg_dict, export_qcfg_json, default_qcfg):
                        continue
...
if __name__ == '__main__':
    ...
    convert_matmul_model(args.input, args.output, args.only_for_scan, args.default_qcfg, args.qcfg_json, args.export_qcfg_json)
