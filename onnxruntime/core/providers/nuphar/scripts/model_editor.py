...
def handle_batch_size(X, nf, need_batch_node, onnx_opset_ver):
    ...
    if type(dim) == str and need_batch_node:
        ...
        if onnx_opset_ver < 11:
            ...
            node = nf.make_node(
                "Slice", [X_shape, np.asarray([1]), np.asarray([2]), np.asarray([0])]  # starts  # ends  # axes
            )
    else:
        node = None
    return dim, node
...
def handle_subgraph_outputs(
    nf_body, seq_len_subgraph, batch_size, hidden_size, subgraph_output_or_default, onnx_opset_ver
):
    ...
    if seq_len_subgraph:
        ...
        condition = nf_body.make_node_with_axes(
            "Unsqueeze",
            nf_body.make_node("Greater", [seq_len_subgraph, np.zeros(shape=(), dtype=np.int32)]),
            [1],
            onnx_opset_ver,
        )
...
def handle_final_scan_outputs(node, nf, scan_outputs, state_outputs, num_directions, onnx_opset_ver):
    if num_directions == 2:
        ...
        for i_o in range(1, len(node.output)):
            nf.make_node_with_axes(
                "Unsqueeze", state_outputs[i_o - 1], [0], onnx_opset_ver, output_names=node.output[i_o]
            )
    ...
    with nf.scoped_prefix(node.output[0]) as scoped_prefix:
        ...
        for direction_index in range(num_directions):
            ...
            with nf_body.scoped_prefix(name_prefix) as body_scoped_prefix:
                ...
                nf_body.make_split_node(
                    sum_x_proj_h_proj_bias, [hidden_size] * 4, onnx_opset_ver, {"axis": 1}, output_names=split_outputs
                )
                # manually add shape inference to split outputs
                for split_o in split_outputs:
                    nf_body.make_value_info(split_o, data_type=onnx.TensorProto.FLOAT, shape=(batch_size, hidden_size))
                ...
                c_subgraph = nf_body.make_node(
                    "Add", [nf_body.make_node("Mul", [ft, prev_c_subgraph]), nf_body.make_node("Mul", [it, ct])]
                )
                ...
                subgraph_outputs = handle_subgraph_outputs(
                    ...
                    batch_size,
                    hidden_size,
                    [(h_subgraph, prev_h_subgraph), (c_subgraph, prev_c_subgraph)]
                    + (
                        [(h_subgraph, np.zeros(shape=(), dtype=np.float32))] if node.output[0] else []
                    ),  # skip scan output if node.output[0] is empty
                    onnx_opset_ver,
                )

                scan_attribs = {"body": scan_body, "scan_input_directions": [is_backward], "num_scan_inputs": 1}
                if node.output[0]:
        ...
        handle_final_scan_outputs(
            node, nf, scan_outputs, [scan_h_outputs, scan_c_outputs], num_directions, onnx_opset_ver
        )
...
def convert_gru_to_scan(node, out_main_graph, onnx_opset_ver):
    ...
    with nf.scoped_prefix(node.output[0]) as scoped_prefix:
        ...
        for direction_index in range(num_directions):
            ...
            Rbh = Bh[1]
            X_proj = nf.make_node(
                "Add", [nf.make_node("MatMul", [X, W_t]), np.concatenate((Bzr, Wbh))]
            ...
            with nf_body.scoped_prefix(name_prefix) as body_scoped_prefix:
                ...
                nf_body.make_value_info(
                    "split_Xzr", data_type=onnx.TensorProto.FLOAT, shape=(batch_size, 2 * hidden_size)
                )
                nf_body.make_value_info("split_Xh", data_type=onnx.TensorProto.FLOAT, shape=(batch_size, hidden_size))

                activation_f, activation_g = activations[direction_index * 2 : (direction_index + 1) * 2]

                if linear_before_reset:
                    prev_h_proj = nf_body.make_node(
                        "Add",
                        [
                            nf_body.make_node("MatMul", [prev_h_subgraph, R_t]),
                            np.concatenate((np.zeros(2 * hidden_size).astype(np.float32), Rbh)),
                        ],
                    )
                    ...
                    nf_body.make_value_info(
                        "split_Hzr", data_type=onnx.TensorProto.FLOAT, shape=(batch_size, 2 * hidden_size)
                    )
                    nf_body.make_value_info(
                        "split_Hh", data_type=onnx.TensorProto.FLOAT, shape=(batch_size, hidden_size)
                    )
                    ztrt = nf_body.make_node(activation_f, nf_body.make_node("Add", ["split_Hzr", "split_Xzr"]))
                    split_ztrt_outputs = ["split_zt", "split_rt"]
                    nf_body.make_split_node(
                        ztrt, [hidden_size, hidden_size], onnx_opset_ver, {"axis": 1}, output_names=split_ztrt_outputs
                    )
                    nf_body.make_value_info(
                        "split_zt", data_type=onnx.TensorProto.FLOAT, shape=(batch_size, hidden_size)
                    )
                    nf_body.make_value_info(
                        "split_rt", data_type=onnx.TensorProto.FLOAT, shape=(batch_size, hidden_size)
                    )
                    ht = nf_body.make_node(
                        activation_g,
                        nf_body.make_node("Add", [nf_body.make_node("Mul", ["split_rt", "split_Hh"]), "split_Xh"]),
                    )
                else:
                    ztrt = nf_body.make_node(
                        activation_f,
                        nf_body.make_node("Add", [nf_body.make_node("MatMul", [prev_h_subgraph, Rzr_t]), "split_Xzr"]),
                    )
                    split_ztrt_outputs = ["split_zt", "split_rt"]
                    nf_body.make_split_node(
                        ztrt, [hidden_size, hidden_size], onnx_opset_ver, {"axis": 1}, output_names=split_ztrt_outputs
                    ...
                    ht = nf_body.make_node(
                        ...
                        nf_body.make_node(
                            ...
                            [
                                nf_body.make_node(
                                    "MatMul", [nf_body.make_node("Mul", [prev_h_subgraph, "split_rt"]), Rh_t]
                                ),
                                "split_Xh",
                            ],
                        ),
                ...
                Ht = nf_body.make_node(
                    ...
                    [
                        nf_body.make_node(
                            "Mul", [nf_body.make_node("Sub", [np.asarray([1]).astype(np.float32), "split_zt"]), ht]
                        ),
                ...
                subgraph_outputs = handle_subgraph_outputs(
                    ...
                    batch_size,
                    hidden_size,
                    [(Ht, prev_h_subgraph)] + ([(Ht, np.zeros(shape=(), dtype=np.float32))] if node.output[0] else []),
                    onnx_opset_ver,
                )
                ...
                scan = nf.make_node(
                    "Scan",
                    ([seq_len] if seq_len else []) + [init_h, X_proj],
                    scan_attribs,
                    output_names=[o.name for o in subgraph_outputs[(0 if seq_len else 1) :]],
                )
    ...
    with nf.scoped_prefix(node.output[0]) as scoped_prefix:
        ...
        for direction_index in range(num_directions):
            ...
            with nf_body.scoped_prefix(name_prefix) as body_scoped_prefix:
                ...
                Ht = nf_body.make_node(
                    activation_f,
                    nf_body.make_node("Add", [nf_body.make_node("MatMul", [prev_h_subgraph, R_t]), X_proj_subgraph]),
                )
        ...
        if in_n.op_type == "Loop":
            ...
            for n in in_mp.graph.node:
                if n.op_type == "Cast" and n.output[0] == in_n.input[1]:
                    cond_initializers = [
                        initializer for initializer in in_mp.graph.initializer if initializer.name == n.input[0]
                    ]
                    cond_const_nodes = [
                        n_c for n_c in in_mp.graph.node if n_c.op_type == "Constant" and n_c.output[0] == n.input[0]
                    ]
    ...
    for in_n in in_mp.graph.node:
        ...
        for in_sn in in_sg.node:
            if in_sn.op_type == "Concat" and len(in_sn.input) == 2 and all([i in in_sg_inputs for i in in_sn.input]):
                ...
                nf_subgraph.make_node(
                    "Add", [out_sg.input[-1].name, hidden_proj], output_names=replaced_matmul.output[0]
                )
...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        choices=["to_scan", "opt_inproj", "gemm_to_matmul", "remove_initializers_from_inputs", "loop_to_scan"],
    )
    ...
    parser.add_argument(
        ...
        help="Whether to keep unconvertible (to Scan) Loops. \
