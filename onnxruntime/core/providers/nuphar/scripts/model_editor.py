...
import numpy as np
import onnx
from node_factory import NodeFactory, remove_initializers_from_inputs
...
def handle_batch_size(X, nf, need_batch_node):
    ...
    assert X_vi
    dim = X_vi.type.tensor_type.shape.dim[1]
    assert type(dim.WhichOneof('value')) == str
    dim = getattr(dim, dim.WhichOneof('value'))
...
def default_init_state(X, batch_size, batch_node, hidden_size, nf):
    if batch_node:
        ...
        return nf.make_initializer(np.zeros((batch_size, hidden_size), dtype=np.float32), X + '_zero_init_states')
...
def convert_lstm_to_scan(node, out_main_graph):
    ...
    with nf.scoped_prefix(node.output[0]) as scoped_prefix:
        ...
        if InitHa is None or InitCa is None:
...
def convert_to_scan_model(input_model, output_model):
    ...
    for opset in out_mp.opset_import:
        if opset.domain == '' or opset.domain == 'onnx':
            opset.version = 9 # bump up to opset 9, which is required for Scan
    out_mp.graph.ClearField('node')
    remove_initializers_from_inputs(out_mp)
...
def parse_arguments():
  ...
  parser.add_argument('--mode', help='The modification mode', choices=['to_scan', 'opt_inproj'])
