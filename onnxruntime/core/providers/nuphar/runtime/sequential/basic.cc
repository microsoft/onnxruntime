...
void BasicExecBlock::InitContext(KernelComputeCtx* kernel_compute_ctx) const {
  ...
  for (int ort_input_idx = 0; ort_input_idx < gsl::narrow<int>(func_info_->ort_input_count); ++ort_input_idx) {
  ...
  // Assign outputs
  for (int ort_output_idx = 0; ort_output_idx < gsl::narrow<int>(func_info_->ort_output_count); ++ort_output_idx) {
