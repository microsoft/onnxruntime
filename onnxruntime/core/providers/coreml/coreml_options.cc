...
void CoreMLOptions::ValidateAndParseProviderOption(const ProviderOptions& options) {
  const std::unordered_map<std::string, COREMLFlags> available_computeunits_options = {
      {kCoremlComputeUnits_CPUAndNeuralEngine, COREML_FLAG_ONLY_ENABLE_DEVICE_WITH_ANE},
      {kCoremlComputeUnits_CPUAndGPU, COREML_FLAG_USE_CPU_AND_GPU},
      {kCoremlComputeUnits_CPUOnly, COREML_FLAG_USE_CPU_ONLY},
      {kCoremlComputeUnits_ALL, COREML_FLAG_USE_NONE},
  };
