...
Status ClipOpBuilder::AddToModelBuilderImpl(ModelBuilder& model_builder,
  ...
  if (!has_min && !has_max) {
    ...
    *layer->mutable_output()->Add() = output_name;
    ...
    if (has_min) {
      std::unique_ptr<COREML_SPEC::NeuralNetworkLayer> min_layer = CreateNNLayer(model_builder, node);
    ...
    if (has_max) {
      ...
      {  // Add threshold layer, which is actually max( -1 * min_output, -max)
        std::unique_ptr<COREML_SPEC::NeuralNetworkLayer> threshold_layer = CreateNNLayer(model_builder, node);
      ...
      {  // Add linear activation layer -1 * threshold_output
        std::unique_ptr<COREML_SPEC::NeuralNetworkLayer> linear_layer = CreateNNLayer(model_builder, node);
