...
struct DMLProviderFactory : IExecutionProviderFactory {
  DMLProviderFactory(IDMLDevice* dml_device,
                     ...
                     const OrtDmlProviderOptions& provider_options) : dml_device_(dml_device),
                                                                      cmd_queue_(cmd_queue),
                                                                      metacommands_enabled_(!provider_options.disable_metacommands),
                                                                      dynamic_graph_fusion_enabled_(provider_options.enable_dynamic_graph_fusion) {}
...
std::shared_ptr<IExecutionProviderFactory> CreateExecutionProviderFactory_DML(IDMLDevice* dml_device,
                                                                              ID3D12CommandQueue* cmd_queue,
                                                                              const OrtDmlProviderOptions& provider_options) {
#ifndef _GAMING_XBOX
  ...
  return std::make_shared<onnxruntime::DMLProviderFactory>(dml_device, cmd_queue, provider_options);
}
...
std::shared_ptr<IExecutionProviderFactory> DMLProviderFactoryCreator::Create(const OrtDmlProviderOptions& provider_options) {
  ComPtr<ID3D12Device> d3d12_device = CreateD3D12Device(provider_options.device_id, provider_options.skip_software_device_check);
  ...
  return CreateExecutionProviderFactory_DML(dml_device.Get(), cmd_queue.Get(), provider_options);
}
...
API_IMPL_BEGIN
  ...
  options->provider_factories.push_back(onnxruntime::DMLProviderFactoryCreator::Create(provider_options));
API_IMPL_END
  return nullptr;
}
