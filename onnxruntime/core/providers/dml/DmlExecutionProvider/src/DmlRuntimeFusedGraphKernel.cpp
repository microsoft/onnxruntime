...
namespace Dml
{
    ...
    public:
        ...
        {
            ORT_THROW_HR_IF(E_UNEXPECTED, static_cast<int>(m_subgraphInputs.size()) != kernelContext->InputCount());
            ...
            if (recompileNeeded)
            {
                ...
                for (auto& nodeArg : m_intermediateNodeArgs)
                {
                    ...
                    if (iter != m_inferredInputShapes.end())
                    {
                        ...
                        ORT_THROW_HR_IF(E_UNEXPECTED, tensorShape.dim_size() != static_cast<int>(iter->second.NumDimensions()));
