...
__global__ void _GatherKernel(
  ...
  int64_t idx = GetIndexValue(indices_data, 8, indices_index);
...
__global__ void _GatherAxis0Kernel(
    const int64_t indices_max,
    const fast_divmod block_size,
  ...
  CUDA_LONG input_index = 0;
  int indices_index, offset;
  block_size.divmod(id, indices_index, offset);
  ...
  input_index = idx * block_size.d_ + offset;
  output_data[id] = input_data[input_index];
}
  ...
  switch (element_size) {
    case sizeof(int8_t): {
      ...
      _GatherKernel<<<blocksPerGrid, GridDim::maxThreadsPerBlock, 0, stream>>>(
          input_block_size, indices_max, output_block_size, block_size, indices_data,
...
void GatherAxis0Impl(
  ...
  switch (element_size) {
    case sizeof(int8_t): {
      using CudaType = typename ToCudaType<int8_t>::MappedType;
      _GatherAxis0Kernel<<<blocksPerGrid, GridDim::maxThreadsPerBlock, 0, stream>>>(
          indices_max, block_size, indices_data,
          reinterpret_cast<const CudaType*>(input_data), reinterpret_cast<CudaType*>(output_data), (CUDA_LONG)N);
