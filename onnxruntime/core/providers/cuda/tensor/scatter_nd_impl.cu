...
void SliceOut(cudaStream_t stream,
  ...
  SliceOutKernel<tpb><<<grid, block, 0, stream>>>(reinterpret_cast<half*>(output), reinterpret_cast<const half*>(input), padded_vocab_size, vocab_size, batch_size, sequence_length);
}
