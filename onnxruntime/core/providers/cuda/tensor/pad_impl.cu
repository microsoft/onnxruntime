...
void PadImpl(
  ...
  switch (pad_mode) {
    case 0:
      _PadKernel<T, 0><<<blocksPerGrid, GridDim::maxThreadsPerBlock, 0, execution_stream>>>(
          shape_rank, input_dims, input_strides, lower_pads, upper_pads,
          pad_value, input_data, fdm_output_strides, output_data, N);
      break;
    case 1:
      _PadKernel<T, 1><<<blocksPerGrid, GridDim::maxThreadsPerBlock, 0, execution_stream>>>(
          shape_rank, input_dims, input_strides, lower_pads, upper_pads,
          pad_value, input_data, fdm_output_strides, output_data, N);
      break;
    case 2:
      _PadKernel<T, 2><<<blocksPerGrid, GridDim::maxThreadsPerBlock, 0, execution_stream>>>(
          shape_rank, input_dims, input_strides, lower_pads, upper_pads,
          pad_value, input_data, fdm_output_strides, output_data, N);
      break;
  }
...
#define SPECIALIZED_IMPL(T)                                         \
  template void PadImpl<T>(cudaStream_t execution_stream,           \
                           const size_t shape_rank,                 \
                           const int64_t* input_dims,               \
                           const int64_t* input_strides,            \
                           const int64_t* lower_pads,               \
