...
Status CudnnRnnBase<T>::ReorganizeWeights(const Tensor* W, const Tensor* R, const Tensor* B,
  ...
  std::vector<float> h_weights(reorganized_w_data_size_in_bytes / sizeof(float));
  cudaMemcpy(h_weights.data(), W_data, h_weights.size() * sizeof(float), cudaMemcpyDeviceToHost);
...
Status CudnnRnnBase<T>::CacheCudnnRnnWeights(const OpKernelInfo& info) {
  ...
  if (get_W && get_R) {
    ...
    if (get_B) {
      ...
      ORT_RETURN_IF_ERROR(ReorganizeWeights(W, R, nullptr, w_data_cache_size_in_bytes_, w_data_cache_, w_desc_cache_, tmp_rnn_desc, nullptr));
    }
...
Status CudnnRnnBase<T>::ComputeInternal(OpKernelContext* ctx) const {
  ...
  const int32_t* sequence_lens_data = (sequence_lens == nullptr) ? sequence_lengths_temp.data() : sequence_lens->Data<int32_t>();
  ...
  ORT_RETURN_IF_ERROR(y_desc1.Set(CudnnTensor::GetDataType<CudaT>(), seq_length, batch_size, ((rnn_mode_ == CUDNN_LSTM) ? proj_size : hidden_size_) * num_directions_, seq_len_array.data()));

  // TODO create CUDADataTensor
  CudnnTensor cx_desc;
  ...
  CUDNN_RETURN_IF_ERROR(cudnnGetRNNTempSpaceSizes(GetCudnnHandle(ctx), rnn_desc, CUDNN_FWD_MODE_INFERENCE, x_desc1, &workspace_bytes, &reservespace_bytes));
