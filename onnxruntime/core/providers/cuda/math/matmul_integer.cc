...
Status MatMulInteger<int8_t, int8_t>::ComputeInternal(OpKernelContext* ctx) const {
  ...
  if (b_offset != 0) {
    a_row_buf = GetScratchBuffer<int32_t>(helper.OutputShape().Size() / helper.N(), OrtStream(ctx));
  ...
  if (a_offset != 0) {
    b_col_buf = GetScratchBuffer<int32_t>(helper.OutputShape().Size() / helper.M(), OrtStream(ctx));
  ...
  if (a_offset != 0 || b_offset != 0) {
    ORT_RETURN_IF_ERROR(OffsetOutput(Stream(ctx),
                                     a_row_buf.get(),
                                     b_col_buf.get(),
                                     output_ptr,
                                     a_offset,
