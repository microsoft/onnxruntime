...
Status dispatch_warpwise_softmax_varlength_forward(Stream* ort_stream,
  ...
  if (max_seq_legnth == 0) {
    ...
    // int log2_elements = log2_ceil(softmax_elements);
    int log2_elements = log2_ceil(max_seq_legnth);
    const int next_power_of_two = 1 << log2_elements;
...
#define LAUNCH_KERNEL(kernel_name, log2_elements_value)                      \
  kernel_name<input_t, output_t, acc_t, log2_elements_value, is_log_softmax> \
      <<<blocks, threads, shared_memory_size, stream>>>(dst, src, batch_count, cu_seqlens, seqlen_count, head_count);

#define CASE_LOG2_ELEMENTS(log2_elements_value) \
  case log2_elements_value:                     \
    LAUNCH_KERNEL(softmax_varlength_warp_forward, log2_elements_value)
...
#define SPECIALIZED_WRAPWISE_SOFTMAX_VARLENGTH_IMPL(input_t, output_t, acc_t)                                             \
  template Status dispatch_warpwise_softmax_varlength_forward<input_t, output_t, acc_t, false>(Stream * ort_stream,       \
                                                                                               output_t * dst,            \
                                                                                               const input_t* src,        \
                                                                                               int max_seq_legnth,        \
                                                                                               int batch_count,           \
                                                                                               ...
                                                                                               const int seqlen_count,    \
                                                                                               const int head_count);     \
  template Status dispatch_warpwise_softmax_varlength_forward<input_t, output_t, acc_t, true>(Stream * ort_stream,        \
                                                                                              output_t * dst,             \
                                                                                              const input_t* src,         \
                                                                                              int max_seq_legnth,         \
                                                                                              int batch_count,            \
...
SPECIALIZED_WRAPWISE_SOFTMAX_VARLENGTH_IMPL(float, float, float);
SPECIALIZED_WRAPWISE_SOFTMAX_VARLENGTH_IMPL(half, half, float);
SPECIALIZED_WRAPWISE_SOFTMAX_VARLENGTH_IMPL(half, float, float);
SPECIALIZED_WRAPWISE_SOFTMAX_VARLENGTH_IMPL(double, double, double);
SPECIALIZED_WRAPWISE_SOFTMAX_VARLENGTH_IMPL(BFloat16, BFloat16, float);

#undef SPECIALIZED_WRAPWISE_SOFTMAX_VARLENGTH_IMPL
