...
template <typename input_t, typename output_t, typename acc_t, int log2_elements, bool is_log_softmax>
__global__ void softmax_varlength_warp_forward(output_t* dst,
                                               const input_t* src,
                                               int batch_size,  // softmax_batch_size
                                               // int stride,
                                               //  int element_count,
  ...
  // int first_batch = (blockDim.y * blockIdx.x + threadIdx.y) * WARP_BATCH;
  int first_batch = (blockDim.y * blockIdx.x + threadIdx.y) * WARP_BATCH;
  // int attn_batch_index = -1;
  int cur_seq_len = -1;
  int accumulated_batch_index = 0;
  int accumulated_element_offset = 0;
  for (int i = 0; i < seqlen_count; ++i) {
    int seq_len = static_cast<int>(cu_seqlens[i + 1] - cu_seqlens[i]);
    if (first_batch < head_count * static_cast<int>(cu_seqlens[i + 1])) {
      // attn_batch_index = i;
      cur_seq_len = seq_len;
      break;
    }

    accumulated_batch_index += head_count * seq_len;
    accumulated_element_offset += head_count * seq_len * seq_len;
  }

  int batch_index_in_current_attn_batch = first_batch - accumulated_batch_index;
  int element_offset = accumulated_element_offset + batch_index_in_current_attn_batch * cur_seq_len;
  int element_count = cur_seq_len;
  /**
   * input:
   *  attn_batch_index:          0                   1                       B-1
   *                             |                   |                        |
   *                             V                   V                        V
   *                    [[head, seq1, seq1], [head, seq2, seq2], ..., [head, seqB, seqB]]
   */

  // assert(attn_batch_index >= 0);
  ...
  src += element_offset + local_idx;
  dst += element_offset + local_idx;
