...
void CUDAExecutionProvider::CaptureStart() {
...
Status CUDAExecutionProvider::GraphReplay() {
  // TODO: Do we need OnRunStart() and OnRunEnd() while replaying the graph ?
  ORT_RETURN_IF_ERROR(OnRunStart());
  graph_.Replay();
  return OnRunEnd(true);
}
