...
struct fast_divmod_uint32 {
  fast_divmod_uint32(uint32_t d = 1) {
    d_ = d == 0 ? 1 : d;
    ORT_ENFORCE(d_ >= 1 && d_ <= std::numeric_limits<uint32_t>::max());
  ...
  __host__ __device__ inline uint32_t div(uint32_t n) const {
    ...
    return static_cast<uint32_t>((t + n) >> l_);
#endif
  }
  ...
  __host__ __device__ inline void divmod(uint32_t n, uint32_t& q, uint32_t& r) const {
    q = div(n);
    r = n - q * d_;
  }
