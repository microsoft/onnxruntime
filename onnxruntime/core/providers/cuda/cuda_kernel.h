...
class CudaKernel : public OpKernel {
 public:
  ...
  inline cudnnHandle_t GetCudnnHandle(OpKernelContext* ctx) const {
    ...
    return cuda_stream && cuda_stream->cudnn_handle_ ? cuda_stream->cudnn_handle_ : DefaultCudnnHandle();
  }

  inline cublasHandle_t GetCublasHandle(OpKernelContext* ctx) const {
    auto* cuda_stream = dynamic_cast<CudaStream*>(ctx->GetComputeStream());
    return cuda_stream && cuda_stream->cublas_handle_ ? cuda_stream->cublas_handle_ : DefaultCublasHandle();
  }
