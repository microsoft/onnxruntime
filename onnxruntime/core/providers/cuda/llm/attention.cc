...
using namespace ::onnxruntime::common; 1Code has alerts. Press enter to view.
using namespace ONNX_NAMESPACE; 1Code has alerts. Press enter to view.
using namespace onnxruntime::attention_helper; 1Code has alerts. Press enter to view.
...
Attention<T>::Attention(const OpKernelInfo& info) : CudaKernel(info) {
  ...
  scale_ = info.GetAttrOrDefault<float>("scale", std::numeric_limits<T>::quiet_NaN()); 1Code has alerts. Press enter to view.
