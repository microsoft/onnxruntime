...
/*
template <typename T>
Status Conv<T>::UpdateState(OpKernelContext*, bool) const {
  return Status::OK();
}
...

  {
    ...
    if (input_dims_changed || w_dims_changed) {
      ...
      if (!post_slicing_required) {
        ...
        // Post slicing needed. Create and fill in the Conv results in an intermediate buffer.
        memory_for_cudnn_conv_results = GetScratchBuffer<void>(TensorShape(y_dims_with_adjusted_pads).Size() * s_.element_size);
    ...
    if (!s_.y_data) {
      ...
      if (!s_.post_slicing_required) {
        ...
        // Post slicing needed. Create and fill in the Conv results in an intermediate buffer.
        memory_for_cudnn_conv_results = GetScratchBuffer<void>(TensorShape(s_.y_dims_with_adjusted_pads).Size() * s_.element_size);
        s_.y_data = reinterpret_cast<CudaT*>(memory_for_cudnn_conv_results.get());
      }
    }
    ...
    if (has_bias) {
      //const Tensor* B = context->Input<Tensor>(2);
      //s_.b_data = reinterpret_cast<const CudaT*>(B->template Data<T>());
      CUDNN_RETURN_IF_ERROR(cudnnAddTensor(CudnnHandle(), &alpha, s_.b_tensor, s_.b_data, &alpha, s_.y_tensor,
                                           s_.y_data));
    }
