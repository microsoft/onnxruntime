...
template <typename T>
Status LayerNorm<T>::ComputeInternal(OpKernelContext* p_op_kernel_context) const {
...
template <typename T>
Status LayerNormGrad<T>::ComputeInternal(OpKernelContext* p_op_kernel_context) const {
  typedef typename ToCudaType<T>::MappedType CudaT;
  const auto one = Consts<CudaT>::One;
  const auto zero = Consts<CudaT>::Zero;
