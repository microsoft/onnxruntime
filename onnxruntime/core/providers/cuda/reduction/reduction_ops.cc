...
template <>
template <typename T>
Status ReduceKernel<true>::ComputeImpl(OpKernelContext* ctx, cudnnReduceTensorOp_t cudnnReduceOp) const {
...
template <>
template <typename T>
Status ReduceKernel<false>::ComputeImpl(OpKernelContext* ctx, cudnnReduceTensorOp_t cudnnReduceOp) const {
  typedef typename ToCudaType<T>::MappedType CudaT;
  ...
  ORT_RETURN_IF_ERROR(reduce_desc.Set(cudnnReduceOp, cudnn_type_X, CUDNN_REDUCE_TENSOR_NO_INDICES));
  ORT_RETURN_IF_ERROR(input_tensor.Set(input_dims_cudnn, cudnn_type_X));
  ORT_RETURN_IF_ERROR(output_tensor.Set(output_dims_cudnn, cudnn_type_X));
  CUDNN_RETURN_IF_ERROR(cudnnGetReductionIndicesSize(CudnnHandle(), reduce_desc, input_tensor, output_tensor, &indices_bytes));
  CUDNN_RETURN_IF_ERROR(cudnnGetReductionWorkspaceSize(CudnnHandle(), reduce_desc, input_tensor, output_tensor, &workspace_bytes));
  ...
  if (input_count == output_count) {
    if (Y->template MutableData<T>() != X->template Data<T>()) {
      CUDA_RETURN_IF_ERROR(cudaMemcpyAsync(Y->template MutableData<T>(),
                                           X->template Data<T>(),
...
Status ReduceSumSquare<T>::ComputeInternal(OpKernelContext* ctx) const {
  ...
  ORT_RETURN_IF_ERROR(reduce_desc.Set(CUDNN_REDUCE_TENSOR_ADD, cudnn_type_X, CUDNN_REDUCE_TENSOR_NO_INDICES));
  ORT_RETURN_IF_ERROR(input_tensor.Set(input_dims_cudnn, cudnn_type_X));
  ORT_RETURN_IF_ERROR(output_tensor.Set(output_dims_cudnn, cudnn_type_X));
  CUDNN_RETURN_IF_ERROR(cudnnGetReductionIndicesSize(CudnnHandle(), reduce_desc, input_tensor, output_tensor, &indices_bytes));
  CUDNN_RETURN_IF_ERROR(cudnnGetReductionWorkspaceSize(CudnnHandle(), reduce_desc, input_tensor, output_tensor, &workspace_bytes));
  ...
  auto input_data = GetScratchBuffer<T>(input_count);
  fast_divmod tmp_div;
  Impl_Mul<CudaT>(static_cast<size_t>(SimpleBroadcast::NoBroadcast),
                  nullptr, /*lhs_padded_strides*/
                  reinterpret_cast<const CudaT*>(X->template Data<T>()),
                  nullptr, /*rhs_padded_strides*/
                  reinterpret_cast<const CudaT*>(X->template Data<T>()),
                  nullptr, /*fdm_output_strides*/
                  tmp_div,
                  tmp_div,
                  reinterpret_cast<CudaT*>(input_data.get()),
                  input_count);
  CUDNN_RETURN_IF_ERROR(cudnnReduceTensor(CudnnHandle(),
                                          reduce_desc,
                                          indices_cuda.get(),
...
template <typename T>
Status ReduceLogSum<T>::ComputeInternal(OpKernelContext* ctx) const {
  typedef typename ToCudaType<T>::MappedType CudaT;
...
/*
To avoid overflow of e^x, uses fomula below to compute ReduceLogSumExp
log(e^x1 + e^x2+ ...+ e^xn) = max(x1, x2, ..., xn) + 
                              log(e^(x1-max(x1,x2,..,xn) + 
                                  e^(x2-max(x1,x2,..,xn) + 
                                  ... + 
                                  e^(x2-max(x1,x2,..,xn))
*/
template <typename T>
Status ReduceLogSumExp<T>::ComputeInternal(OpKernelContext* ctx) const {
  typedef typename ToCudaType<T>::MappedType CudaT;
  ...
  size_t indices_add_bytes = 0;
  size_t indices_max_bytes = 0;
  size_t workspace_add_bytes = 0;
  size_t workspace_max_bytes = 0;
  CudnnReduceDescriptor reduce_add_desc;
  CudnnReduceDescriptor reduce_max_desc;
  ORT_RETURN_IF_ERROR(reduce_add_desc.Set(CUDNN_REDUCE_TENSOR_ADD, cudnn_type_X, CUDNN_REDUCE_TENSOR_NO_INDICES));
  ORT_RETURN_IF_ERROR(reduce_max_desc.Set(CUDNN_REDUCE_TENSOR_MAX, cudnn_type_X, CUDNN_REDUCE_TENSOR_NO_INDICES));
  CUDNN_RETURN_IF_ERROR(cudnnGetReductionIndicesSize(CudnnHandle(), reduce_add_desc, input_tensor, output_tensor, &indices_add_bytes));
  CUDNN_RETURN_IF_ERROR(cudnnGetReductionIndicesSize(CudnnHandle(), reduce_max_desc, input_tensor, output_tensor, &indices_max_bytes));
  CUDNN_RETURN_IF_ERROR(cudnnGetReductionWorkspaceSize(CudnnHandle(), reduce_add_desc, input_tensor, output_tensor, &workspace_add_bytes));
  CUDNN_RETURN_IF_ERROR(cudnnGetReductionWorkspaceSize(CudnnHandle(), reduce_max_desc, input_tensor, output_tensor, &workspace_max_bytes));
  IAllocatorUniquePtr<void> indices_cuda = GetScratchBuffer<void>(std::max(indices_add_bytes, indices_max_bytes));
  IAllocatorUniquePtr<void> workspace_cuda = GetScratchBuffer<void>(std::max(workspace_add_bytes, workspace_max_bytes));
  uint32_t* indice_cuda_ptr = static_cast<uint32_t*>(indices_cuda.get());
  CudaT* workspace_cuda_ptr = static_cast<CudaT*>(workspace_cuda.get());
  ...
  prepare.BinaryElementwiseBroadcastPrepareHelper(X->Shape(), TensorShape(output_dims_keepdims), X->Shape());
  prepare.CopyToGpu();
  ...
  // Log + ReduceMax
  fast_divmod tmp_div;
  Impl_Add<CudaT>(static_cast<size_t>(SimpleBroadcast::NoBroadcast),
                  nullptr /*lhs_padded_strides*/,
                  reinterpret_cast<CudaT*>(log_sum_result),
                  nullptr /*rhs_padded_strides*/,
                  reinterpret_cast<CudaT*>(Y->template MutableData<T>()),
...
REGISTER_KERNEL_INT32(ArgMax)
REGISTER_KERNEL_INT32(ArgMin)
