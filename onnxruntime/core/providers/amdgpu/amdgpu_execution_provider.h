...
constexpr auto kINT8UseNativeAMDGPUCalibrationTable = "ORT_MIGRAPHX_INT8_USE_NATIVE_CALIBRATION_TABLE";
...
class AMDGPUExecutionProvider final : public IExecutionProvider {
 public:
  explicit AMDGPUExecutionProvider(const AMDGPUExecutionProviderInfo& info);
  ~AMDGPUExecutionProvider() override = default;
  ...
  static AllocatorPtr CreateAMDGPUAllocator(OrtDevice::DeviceId device_id, size_t migx_mem_limit, ArenaExtendStrategy arena_extend_strategy,
                                              AMDGPUExecutionProviderExternalAllocatorInfo external_alloc_info, const OrtArenaCfg* arena_cfg);
  ...
  ProviderOptions GetProviderOptions() const override {
     return {{amdgpu_provider_option::kDeviceId, MakeStringWithClassicLocale(device_id_)},
       {amdgpu_provider_option::kFp16Enable, MakeStringWithClassicLocale(fp16_enable_)},
       {amdgpu_provider_option::kInt8Enable, MakeStringWithClassicLocale(int8_enable_)},
       {amdgpu_provider_option::kModelCacheDir, MakeStringWithClassicLocale(model_cache_path_)}
   };
  }
 ...
 private:
  ...
  bool int8_use_native_amdgpu_calibration_table_ = false;
