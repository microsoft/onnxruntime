...
Status GemmInt8(int m, int n, int k,
                ...
                void* alloc_stream, hipStream_t hip_stream,
  ...
  ORT_ENFORCE(hip_stream != nullptr, "Rocm kernel must have the hip stream");

  auto* ort_stream = static_cast<onnxruntime::Stream*>(alloc_stream);
  hipStream_t stream = hip_stream;
