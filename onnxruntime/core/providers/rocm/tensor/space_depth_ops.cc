...
static Status SpaceDepthOpRocmImpl(const hipDeviceProp_t& prop,
  ...
  return Transpose::DoTranspose(prop, stream, rocblas_handle, permutation, input, output, &virtual_input_shape);
}
...
Status DepthToSpace::ComputeInternal(OpKernelContext* context) const {
  ...
  ORT_RETURN_IF_ERROR(SpaceDepthOpRocmImpl(GetDeviceProp(), Stream(), RocblasHandle(), input, output,
                                           permutation,
                                           batch,
