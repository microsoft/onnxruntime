...
Status ScatterElementsImpl2D(
  ...
  if (axis == 0) {
    hipLaunchKernelGGL(HIP_KERNEL_NAME(_ScatterElementsKernel2D<T, Tin, true, FuncT>), dim3(blocksPerGrid), dim3(GridDim::maxThreadsPerBlock), 0, 0, 
    ...
    hipLaunchKernelGGL(HIP_KERNEL_NAME(_ScatterElementsKernel2D<T, Tin, false, FuncT>), dim3(blocksPerGrid), dim3(GridDim::maxThreadsPerBlock), 0, 0, 
...
Status ScatterElementsImplInternal(
  ...
  if (input_data != output_data) {
    HIP_RETURN_IF_ERROR(hipMemcpyAsync(output_data, input_data, input_size * sizeof(T), hipMemcpyDeviceToDevice, 0));
  }
  ...
  if (indices_size > 0) {
    ...
    int blocksPerGrid = gsl::narrow_cast<int>(CeilDiv(indices_size, GridDim::maxThreadsPerBlock));
    hipLaunchKernelGGL(HIP_KERNEL_NAME(_ScatterElementsKernel<T, Tin>), dim3(blocksPerGrid), dim3(GridDim::maxThreadsPerBlock), 0, 0, 
