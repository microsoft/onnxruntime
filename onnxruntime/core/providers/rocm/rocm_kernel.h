...
// Thin adapter matching the CUDA OrtStreamAdapter for hipified code compatibility.
...
class RocmKernel : public OpKernel {
 public:
  ...
  inline IAllocatorUniquePtr<T> GetScratchBuffer(size_t count_or_bytes, onnxruntime::Stream* stream) const {
    ...
    return IAllocator::MakeUniquePtr<T>(Info().GetAllocator(OrtMemType::OrtMemTypeDefault), count_or_bytes, false, stream);
  }

  // Overload accepting void* stream (for hipified CUDA code that passes alloc_stream as void*)
  template <typename T>
  ...
  // Overload accepting nullptr (stream=nullptr means use default allocator without stream)
  template <typename T>
  ...
  // Compatibility helper used by hipified CUDA kernels
  ...
  // Accept base Stream* for compatibility with hipified CUDA code
  static inline hipblasHandle_t GetHipblasHandle(onnxruntime::Stream* stream) {
    return GetHipblasHandle(static_cast<onnxruntime::RocmStream*>(stream));
  }

  inline hipblasHandle_t GetHipblasHandleOrDefault(onnxruntime::Stream* stream) const {
    return stream ? GetHipblasHandle(stream) : DefaultHipblasHandle();
  }
