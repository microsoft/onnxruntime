...
// hipStream_t overloads for hipified code (CUDA 1.28.0 SoftmaxComputeStreamT = cudaStream_t)
template <typename InputT, typename OutputT, typename AccT, bool IsLogSoftmax>
Status dispatch_warpwise_softmax_forward(hipStream_t stream, OutputT* dst, const InputT* src, int softmax_elements,
                                         int softmax_elements_stride, int batch_count, RocmTuningContext* tuning_ctx) {
  OrtDevice device;
  Stream ort_stream(static_cast<StreamHandle>(stream), device);
  return dispatch_warpwise_softmax_forward<InputT, OutputT, AccT, IsLogSoftmax>(
      &ort_stream, dst, src, softmax_elements, softmax_elements_stride, batch_count, tuning_ctx);
}
...
// hipStream_t overload for hipified code (CUDA 1.28.0 SoftmaxComputeStreamT = cudaStream_t)
template <typename InputT, typename OutputT, typename AccT, bool IsLogSoftmax>
Status dispatch_blockwise_softmax_forward(hipStream_t stream, OutputT* output, const InputT* input, int softmax_elements,
  ...
  OrtDevice device;
  Stream ort_stream(static_cast<StreamHandle>(stream), device);
  return dispatch_blockwise_softmax_forward<InputT, OutputT, AccT, IsLogSoftmax>(
      &ort_stream, output, input, softmax_elements, input_stride, output_stride, batch_count, tuning_ctx);
}
...
// Explicit instantiations for the hipStream_t overloads (used by hipified callers)
#define SPECIALIZED_BLOCKWISE_SOFTMAX_IMPL_HIPSTREAM(InputT, OutputT, AccT)                    \
  template Status dispatch_blockwise_softmax_forward<InputT, OutputT, AccT, false>(            \
...
SPECIALIZED_BLOCKWISE_SOFTMAX_IMPL_HIPSTREAM(float, float, float)
SPECIALIZED_BLOCKWISE_SOFTMAX_IMPL_HIPSTREAM(half, half, float)
SPECIALIZED_BLOCKWISE_SOFTMAX_IMPL_HIPSTREAM(half, float, float)
SPECIALIZED_BLOCKWISE_SOFTMAX_IMPL_HIPSTREAM(double, double, double)
SPECIALIZED_BLOCKWISE_SOFTMAX_IMPL_HIPSTREAM(BFloat16, BFloat16, float)
}  // namespace rocm
}  // namespace onnxruntime
