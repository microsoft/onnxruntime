...
std::unique_ptr<Tensor> ReduceSum(const Tensor& input, gsl::span<const int64_t> reduce_axes,
  ...
  EinsumRocmAssets* assets = static_cast<EinsumRocmAssets*>(einsum_rocm_assets);
  Stream* ort_stream = assets->ort_stream_;
  RocmStream* rocm_stream = static_cast<RocmStream*>(ort_stream);
  return rocm::ReductionOps::ReduceCompute<T>(assets->gpu_allocator_, MIOPEN_REDUCE_TENSOR_ADD,
...
Status ZeroBuffer(Tensor& input, void* einsum_rocm_assets) {
  HIP_RETURN_IF_ERROR(hipMemsetAsync(input.MutableDataRaw(), 0, input.SizeInBytes(),
                                     static_cast<EinsumRocmAssets*>(einsum_rocm_assets)->GetRocmStream()));
  return Status::OK();
}
