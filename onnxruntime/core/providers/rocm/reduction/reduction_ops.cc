...
Status ReduceComputeCore(const AllocatorPtr& gpu_allocator, const RocmKernel* /*kernel*/, const Tensor& input, PrepareReduceMetadata& prepare_reduce_metadata,
                         ...
                         hipStream_t hip_stream, void* compute_stream, miopenHandle_t /*miopen_handle*/,
                         const TensorShape* input_shape_override) {
  // compute_stream is the ORT Stream* wrapper; extract hipStream if needed
  Stream* ort_stream = static_cast<Stream*>(compute_stream);
  hipStream_t stream = hip_stream ? hip_stream : (ort_stream ? static_cast<hipStream_t>(ort_stream->GetHandle()) : nullptr);
  typedef typename ToHipType<T>::MappedType HipT;
    ...
    if (applicable_matrix_reduction != ApplicableMatrixReduction::None) {
      ...
      switch (applicable_matrix_reduction) {
        ...
        case ApplicableMatrixReduction::Columns: {
          ...
          auto buffer = buffer_size_bytes == 0 ? nullptr : IAllocator::MakeUniquePtr<void>(gpu_allocator, buffer_size_bytes, false, ort_stream);
  ...
  auto workspace_rocm = workspace_bytes == 0 ? nullptr : IAllocator::MakeUniquePtr<HipT>(gpu_allocator, workspace_bytes, false, ort_stream);
  ...
  auto indices_rocm = indices_bytes == 0 ? nullptr : IAllocator::MakeUniquePtr<uint32_t>(gpu_allocator, indices_bytes, false, ort_stream);
  ...
  if (ReduceTensorIndices == MIOPEN_REDUCE_TENSOR_NO_INDICES) {
    ...
    if (calculate_sqt) {
      ...
      if (input_count == output_count) {
        ...
        auto indices_rocm_max = indices_bytes == 0 ? nullptr : IAllocator::MakeUniquePtr<uint32_t>(gpu_allocator, indices_bytes, false, ort_stream);
      ...
      auto exp_result = exp_result_buffer.get();
      auto log_sum_result_buffer = output_count == 0 ? nullptr : IAllocator::MakeUniquePtr<T>(gpu_allocator, output_count, false, ort_stream);
      auto log_sum_result = log_sum_result_buffer.get();
      BinaryElementwisePreparation prepare;
    ...
    if (calculate_sqt) {
      ...
      if (input_count == output_count) {
        ...
        if (temp_X) {
          auto temp_output = output_count == 0 ? nullptr : IAllocator::MakeUniquePtr<float>(gpu_allocator, output_count, false, ort_stream);
          MIOPEN_RETURN_IF_ERROR(miopenReduceTensor(
              RocmKernel::GetMiopenHandle(rocm_stream), reduce_desc, indices_rocm.get(), indices_bytes,
              workspace_rocm.get(), workspace_bytes,
              &one, input_tensor, temp_X.get(),
    ...
    if (input_count == output_count) {
      ...
      if (temp_X) {
        ...
        auto temp_output = output_count == 0 ? nullptr : IAllocator::MakeUniquePtr<HipT>(gpu_allocator, output_count, false, ort_stream);
        MIOPEN_RETURN_IF_ERROR(miopenReduceTensor(
            RocmKernel::GetMiopenHandle(rocm_stream), reduce_desc, indices_rocm.get(), indices_bytes,
            workspace_rocm.get(), workspace_bytes,
            &one, input_tensor, reinterpret_cast<const HipT*>(input.Data<T>()),
...
template Status ReduceComputeCore<float, MIOPEN_REDUCE_TENSOR_NO_INDICES>(
    const AllocatorPtr& gpu_allocator, const RocmKernel* kernel, const Tensor& input, PrepareReduceMetadata& prepare_reduce_metadata,
    /*out*/ Tensor& output, miopenReduceTensorOp_t miopen_reduce_op,
    gsl::span<const int64_t> axes,
    bool calculate_log, bool calculate_sqt, bool log_sum_exp, bool fast_reduction,
    hipStream_t hip_stream, void* compute_stream, miopenHandle_t miopen_handle,
...
Status ReduceKernel<allow_multi_axes>::ComputeImpl(OpKernelContext* ctx, miopenReduceTensorOp_t miopen_reduce_op) const {
  ...
  return ReduceComputeCore<T, ReduceTensorIndices>(
      Info().GetAllocator(OrtMemType::OrtMemTypeDefault), this, *X, prepare_reduce_metadata, *Y,
      miopen_reduce_op, axes, calculate_log_, calculate_sqt_, log_sum_exp_, fast_reduction,
      Stream(ctx), ctx->GetComputeStream(), GetMiopenHandle(ctx));
}
...
std::unique_ptr<Tensor> ReduceCompute(const AllocatorPtr& gpu_allocator, miopenReduceTensorOp_t miopen_reduce_op, AllocatorPtr allocator,
                                      ...
                                      bool fast_reduction, Stream* stream, miopenHandle_t /*miopen_handle*/,
                                      const TensorShape* input_shape_override) {
  PrepareReduceMetadata prepare_reduce_metadata;
  ...
  status = ReduceComputeCore<T, ReduceTensorIndices>(
      gpu_allocator, nullptr /*kernel*/, input, prepare_reduce_metadata, *output, miopen_reduce_op, axes,
      calculate_log, calculate_sqt, log_sum_exp, fast_reduction,
      rocm_stream ? static_cast<hipStream_t>(rocm_stream->GetHandle()) : nullptr,
      rocm_stream,
      rocm_stream ? rocm_stream->miopen_handle_ : nullptr,
      input_shape_override);
