...
// hip_api_calls_to_trace is defined as a constexpr array in the header.
...
bool RoctracerManager::OnStartLogging() {
  ...
  for (const char* logged_api : hip_api_calls_to_trace) {
    uint32_t cid = 0;
    ROCTRACER_STATUS_RETURN_FALSE_ON_FAIL(
        roctracer_op_code(ACTIVITY_DOMAIN_HIP_API, logged_api, &cid, nullptr));
    ROCTRACER_STATUS_RETURN_FALSE_ON_FAIL(
