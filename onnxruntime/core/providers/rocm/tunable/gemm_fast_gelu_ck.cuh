...
auto GetCKGemmAddFastGeluTypeStringAndOps() {
  ...
  for (auto&& impl : InstanceFactory::GetInstances()) {
    ...
    auto ck_gemmaddfastgelu_op = [impl = std::move(impl), invoker = std::move(invoker)](const GemmFastGeluParams<T>* params) -> Status {
      ...
      auto arg = impl->MakeArgumentPointer(params->a, params->b,
                                           std::array<const void*, 1>{params->bias},
                                           params->c,
    ...
    ret.emplace_back(std::make_pair(std::move(type_string), std::move(ck_gemmaddfastgelu_op)));
  }
  return ret;
}
