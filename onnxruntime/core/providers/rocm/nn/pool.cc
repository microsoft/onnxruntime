...
class MiopenPoolingDescriptor final {
 public:
  ...
  Status Set(miopenPoolingMode_t mode,
    ...
    MIOPEN_RETURN_IF_ERROR(miopenSetNdPoolingDescriptor(
        desc_,
        mode,
...
Status Pool<T, PoolType>::ComputeInternal(OpKernelContext* context) const {
  ...
  TensorShapeVector x_dims_miopen(x_dims.cbegin(), x_dims.cend());
  ...
  constexpr bool pooling_no_backward = false;
  if constexpr (std::is_same<T, int8_t>::value || std::is_same<T, uint8_t>::value) {
    ...
    MIOPEN_RETURN_IF_ERROR(miopenPoolingForward(MiopenHandle(), pooling_desc,
                                                &alpha, x_tensor, temp_X.get(),
    ...
    MIOPEN_RETURN_IF_ERROR(miopenPoolingForward(MiopenHandle(), pooling_desc,
                                                &alpha, x_tensor, x_data,
                                                &beta, y_tensor, y_data,
                                                pooling_no_backward,
                                                nullptr, 0));;
  }
