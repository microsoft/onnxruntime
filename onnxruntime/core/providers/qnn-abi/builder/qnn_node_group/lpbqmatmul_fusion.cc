...
std::unique_ptr<IQnnNodeGroup> LowPowerBlockQuantizedMatMulFusion::TryFusion(
  ...
  const OrtNodeUnit* p_w_ql_node_unit = GetParentOfInput(qnn_model_wrapper,
                                                         matmul_node_unit,
...
Ort::Status ProcessInput0(QnnModelWrapper& qnn_model_wrapper,
  ...
  RETURN_IF_ERROR(qnn_model_wrapper.GetTensorInfo(input_def, input_0_info));
...
Ort::Status TwoDimensionTranspose(std::vector<uint8_t>& data,
  ...
  RETURN_IF_NOT(elem_byte_size != 0, "Can't get element byte size from given QNN type");
...
Ort::Status ProcessLPBQWeight(QnnModelWrapper& qnn_model_wrapper,
  ...
  if (block_scales_axis == 1) {
    RETURN_IF_ERROR(TwoDimensionTranspose(per_block_int_scale, block_scales_shape, QNN_DATATYPE_UFIXED_POINT_8));
    block_scales_axis = 0;
  }
...
Ort::Status CreateOrValidateOnQnn(QnnModelWrapper& qnn_model_wrapper,
  ...
  const OrtNodeUnitIODef& input_def = matmul_node_unit.Inputs()[0];
  const std::string& input_tensor_name = input_def.name;
  RETURN_IF_ERROR(ProcessInput0(qnn_model_wrapper, input_def, input_tensor_name, input_names, logger, validate));

  // Prepare LowPowerBlockQuantized(LPBQ) Weight
  RETURN_IF_ERROR(ProcessLPBQWeight(qnn_model_wrapper, scale_dql_node_unit, w_ql_node_unit,
                                    matmul_node_unit, input_names, logger));

  // Prepare Output
  const OrtNodeUnitIODef& output_def = matmul_node_unit.Outputs()[0];
  const std::string& op_output_name = output_def.name;
