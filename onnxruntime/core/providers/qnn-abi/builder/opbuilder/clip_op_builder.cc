...
static Ort::Status ProcessClipMinMax(QnnModelWrapper& qnn_model_wrapper,
  ...
  switch (input_info.qnn_data_type) {
    ...
    default:
      return MAKE_EP_FAIL("min/max input data type not supported.");
  }

  return Ort::Status();
}

Ort::Status ClipOpBuilder::ExplictOpCheck(QnnModelWrapper& qnn_model_wrapper, const OrtNodeUnit& node_unit) const {
