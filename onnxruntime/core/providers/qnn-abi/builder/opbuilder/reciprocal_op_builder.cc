...
Ort::Status ReciprocalOpBuilder::IsOpSupported(QnnModelWrapper& qnn_model_wrapper,
  ...
  RETURN_IF_NOT(inputs.size() == 1, "Reciprocal operator must have exactly 1 input.");
...
Ort::Status ReciprocalOpBuilder::ProcessAttributesAndOutputs(QnnModelWrapper& qnn_model_wrapper,
                                                             const OrtNodeUnit& node_unit,
                                                             std::vector<std::string>&& input_names,
                                                             const Ort::Logger& logger,
  ...
  if (input_info.quant_param.IsQuantized()) {
    ...
    RETURN_IF_ERROR(utils::Quantize(divisor_value, divisor_quant_param.Get().scaleOffsetEncoding.scale,
  ...
  RETURN_IF_NOT(qnn_model_wrapper.AddTensorWrapper(std::move(divisor_tensorwrapper)), "Failed to add divisor tensor.");
