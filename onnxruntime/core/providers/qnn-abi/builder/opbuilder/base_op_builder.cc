...
// Add operator related
Ort::Status BaseOpBuilder::AddToModelBuilder(QnnModelWrapper& qnn_model_wrapper,
                                             const OrtNodeUnit& node_unit,
                                             const Ort::Logger& logger,
                                             bool do_op_validation) const {
  ORT_CXX_LOG(logger,
              ORT_LOGGING_LEVEL_VERBOSE,
              ("QNN node builder is trying to add node. Onnx node name: [" + node_unit.Name() +
  ...
  RETURN_IF_ERROR(ProcessInputs(qnn_model_wrapper, node_unit, logger,
                                input_names, do_op_validation));
  RETURN_IF_ERROR(ProcessInt64Tensors(qnn_model_wrapper, node_unit, input_names));
  RETURN_IF_ERROR(ProcessAttributesAndOutputs(qnn_model_wrapper, node_unit, std::move(input_names),
                                              logger, do_op_validation));
  return Ort::Status();
}

Ort::Status BaseOpBuilder::ProcessDataTypes(QnnModelWrapper& qnn_model_wrapper,
...
Ort::Status BaseOpBuilder::CheckGpuDataTypes(const std::vector<Qnn_DataType_t>,
                                             const std::vector<Qnn_DataType_t>) const {
  return Ort::Status();
}

Ort::Status BaseOpBuilder::ProcessInput(QnnModelWrapper& qnn_model_wrapper,
  ...
  RETURN_IF_ERROR(qnn_model_wrapper.MakeTensorWrapper(input, input_tensorwrapper));
...
Ort::Status BaseOpBuilder::ProcessInputs(QnnModelWrapper& qnn_model_wrapper,
                                         const OrtNodeUnit& node_unit,
                                         const Ort::Logger& logger,
                                         std::vector<std::string>& input_names,
                                         bool do_op_validation) const {
...
Ort::Status BaseOpBuilder::AddZeroBiasInput(QnnModelWrapper& qnn_model_wrapper,
  ...
  RETURN_IF_NOT(input0_qparams.IsPerTensor(/*include_bw*/ true) && input1_qparams.IsQuantized(),
  ...
  RETURN_IF_ERROR(input0_qparams.GetScales(input0_quant_scales));
  RETURN_IF_ERROR(input1_qparams.GetScales(input1_quant_scales));

  const size_t num_bias_scales_offsets = input1_quant_scales.size();
  assert(input0_quant_scales.size() == 1);  // Expected for per-tensor.
  RETURN_IF_NOT(num_bias_scales_offsets >= input0_quant_scales.size(),
...
Ort::Status BaseOpBuilder::ProcessInt64Tensors(QnnModelWrapper& qnn_model_wrapper,
                                               const OrtNodeUnit& node_unit,
                                               std::vector<std::string>& input_names) const {
...
Ort::Status BaseOpBuilder::ProcessOutputs(QnnModelWrapper& qnn_model_wrapper,
                                          const OrtNodeUnit& node_unit,
                                          std::vector<std::string>&& input_names,
...
Ort::Status BaseOpBuilder::SetOutputQParamEqualToInputIfNearlyEqual(QnnModelWrapper& qnn_model_wrapper,
                                                                    const OrtNodeUnit& node_unit,
                                                                    const Ort::Logger& logger,
  ...
  RETURN_IF_NOT(input_tensor_wrapper.GetTensorDataType() == qnn_data_type,
  ...
  RETURN_IF_ERROR(CompareQnnQuantParams(quant_param.Get(), input_quant_param.Get(), scale_diff, offset_diff));
  ...
  if (scale_diff != 0.0f && offset_diff == 0) {
    if (scale_diff <= NEARLY_EQUAL_THRESHOLD) {
      ...
      const std::string msg = "QNN EP will override the output quantization parameters for " + node_unit.OpType() +
                              " operators to be equal to the input quantization parameters. Operator name: " +
                              node_unit.Name() + ", input_index: " + std::to_string(input_index) + ", output index: " +
      ...
      // degradation.
      const std::string msg = "The quantization parameters for the " + node_unit.OpType() + " operator '" +
                              node_unit.Name() + "' are not equal, which may result in latency degradation. " +
                              "input_index: " + std::to_string(input_index) + ", output index: " +
                              std::to_string(output_index) + ".";
      ORT_CXX_LOG(logger, ORT_LOGGING_LEVEL_WARNING, msg.c_str());
    }
  }
