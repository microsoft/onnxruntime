...
Status GetMainContextNode(const OrtGraph** graphs,
  ...
  for (size_t graph_idx = 0; graph_idx < count; ++graph_idx) {
    OrtArrayOfConstObjects* nodes = nullptr;
    OrtStatus* status = ort_api.Graph_GetNodes(graphs[graph_idx], &nodes);
    if (status != nullptr) {
      ort_api.ReleaseStatus(status);
    ...
    if (status != nullptr) {
      ort_api.ReleaseStatus(status);
      ort_api.ReleaseArrayOfConstObjects(nodes);
      return ORT_MAKE_STATUS(ONNXRUNTIME, FAIL, "Failed to get array size");
    }
    ORT_RETURN_IF(num_nodes != 1, "OrtGraph should has only one EPContext node.");

    const void* node = nullptr;
    status = ort_api.ArrayOfConstObjects_GetElementAt(nodes, 0, &node);
    if (status != nullptr) {
      ort_api.ReleaseStatus(status);
      ort_api.ReleaseArrayOfConstObjects(nodes);
      return ORT_MAKE_STATUS(ONNXRUNTIME, FAIL, "Failed to get element from array");
    }
...
Status TryGetMaxSpillFillSize(const OrtGraph** graphs,
  ...
  for (uint32_t idx = 0; idx < total_context_size; ++idx) {
    OrtArrayOfConstObjects* nodes = nullptr;
    OrtStatus* status = ort_api.Graph_GetNodes(graphs[idx], &nodes);
    if (status != nullptr) {
      ort_api.ReleaseStatus(status);
      return ORT_MAKE_STATUS(ONNXRUNTIME, FAIL, "Failed to get nodes from graph");
    }
...
Status LoadQnnCtxFromOnnxGraph(const OrtGraph* graph,
  ...
  const void* node = nullptr;
  ort_status = ort_api.ArrayOfConstObjects_GetElementAt(nodes, 0, &node);
  if (ort_status != nullptr) {
    ort_api.ReleaseStatus(ort_status);
    ort_api.ReleaseArrayOfConstObjects(nodes);
    return ORT_MAKE_STATUS(ONNXRUNTIME, FAIL, "Failed to get element from array");
