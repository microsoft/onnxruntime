...
namespace {
std::unique_ptr<IndexedSubGraph> GetSubGraph(SubGraph_t graph_nodes_index, int& kernels_index, const onnxruntime::GraphViewer& graph) {
...
SubGraphCollection_t GetSupportedList(SubGraphCollection_t nodes_vector_input, int iterations, const int max_iterations,
                                      const onnxruntime::GraphViewer& graph, bool* early_termination) {
  ...
  for (const auto& group : nodes_vector_input) {
    ...
    if (!group.first.empty()) {
      ...
      if (group.second) {
        ...
        TRTLogger trt_logger(nvinfer1::ILogger::Severity::kWARNING);
...
std::vector<std::unique_ptr<ComputeCapability>> CheckTensorRtCapability(const onnxruntime::GraphViewer& graph) {
  ...
  const int max_iterations = batch_env ? atoi(batch_env) : 100;
...
common::Status TRTCompiler::Compile(const onnxruntime::Node* fused_node, cudaStream_t stream,
  ...
  onnxruntime::Graph& graph_build = model.MainGraph();
  for (const auto& node : graph_body.Nodes()) {
    std::vector<onnxruntime::NodeArg*> inputs, outputs;
    for (auto input : node.InputDefs()) {
      auto& n_input = graph_build.GetOrCreateNodeArg(input->Name(), input->TypeAsProto());
      inputs.push_back(&n_input);
  ...
  node_compute_func.release_state_func = [](FunctionState state) {
    if (state)
      delete static_cast<TRTFuncState*>(state);
  };

  // Create compute function
  node_compute_func.compute_func = [](FunctionState state, const OrtCustomOpApi* api, OrtKernelContext* context) {
    ...
    cudaStream_t stream = trt_state->stream;
