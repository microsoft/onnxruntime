...
template <>
Status Gemm<float>::Compute(Prov_OpKernelContext* ctx) const {
  const auto X = ctx->Input<Prov_Tensor>(0);
  const auto W = ctx->Input<Prov_Tensor>(1);
  const auto B = ctx->Input<Prov_Tensor>(2);
