...
static Status CreateOrValidateOnQnn(QnnModelWrapper& qnn_model_wrapper,
  ...
  for (const Node* q_node : q_nodes) {
    ...
    for (; q_cur_edge != q_end_edge; ++q_cur_edge) {
      auto output_edge = Factory<Node_EdgeEnd>::Create(q_cur_edge->GetNode(), 0, q_cur_edge->GetDstArgIndex());
      output_edges.push_back(output_edge.get());
      output_edges_holder.push_back(std::move(output_edge));
    }
  }

  std::unique_ptr<NodeUnit> custom_node_unit = Factory<NodeUnit>::Create(dq_nodes, target_node,
