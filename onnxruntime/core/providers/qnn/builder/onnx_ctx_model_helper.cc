...
Status GetMainContextNode(const std::vector<IExecutionProvider::FusedNodeAndGraph>& fused_nodes_and_graphs,
  ...
  for (size_t i = 0; i < fused_nodes_and_graphs.size(); ++i) {
    ...
    // There is only one EPContext node in one filtered graph if the model is generated by QNN EP
    // There could be be multiple EPContext nodes in one filtered graph if it is created from user script
    int64_t is_main_context = 0;
    for (auto& ep_context_node : graph_viewer.Nodes()) {
      ORT_RETURN_IF_NOT(EPCONTEXT_OP == ep_context_node.OpType(), "Should only filter in the EPContext node.");
      qnn_models.emplace(ep_context_node.Name(), std::make_unique<qnn::QnnModel>(logger, qnn_backend_manager));
      NodeAttrHelper node_helper(ep_context_node);
      int64_t main_context = node_helper.Get(MAIN_CONTEXT, static_cast<int64_t>(0));
      is_main_context = main_context ? 1 : 0;
    }
