...
bool GraphHasEpContextNode(const OrtGraph* graph, const OrtApi& ort_api) {
  ...
  size_t num_nodes = 0;
  RETURN_DEFAULT_IF_API_FAIL(ort_api.Graph_GetNumNodes(graph, &num_nodes), ort_api, false);

  std::vector<const OrtNode*> nodes(num_nodes);
  RETURN_DEFAULT_IF_API_FAIL(ort_api.Graph_GetNodes(graph, nodes.data(), nodes.size()), ort_api, false);

  for (const OrtNode* node : nodes) {
    const char* op_type = nullptr;
    RETURN_DEFAULT_IF_API_FAIL(ort_api.Node_GetOperatorType(node, &op_type), ort_api, false);
...
Ort::Status GetMainContextNode(const OrtGraph** graphs,
  ...
  for (size_t graph_idx = 0; graph_idx < count; ++graph_idx) {
    ...
    const char* op_type = nullptr;
    ORT_CXX_RETURN_ON_API_FAIL(ort_api.Node_GetOperatorType(ep_context_node, &op_type));
  ...
  RETURN_IF(main_context_pos.size() < 1, "Failed to find the EPContext node with main_context=1.");
  return Ort::Status();
}
...
Ort::Status GetEpContextFromMainNode(const OrtNode* main_context_node,
  ...
  ORT_CXX_RETURN_ON_API_FAIL(ort_api.Node_GetName(main_context_node, &main_context_node_name));
...
Ort::Status LoadQnnCtxFromOnnxGraph(const OrtGraph* graph,
  ...
  const OrtNode* ep_context_node = nodes[0];
  Ort::Status load_status = GetEpContextFromMainNode(ep_context_node,
                                                     ort_api,
                                                     ctx_onnx_model_path,
                                                     qnn_backend_manager,
                                                     qnn_models,
  ...
  if (!load_status.IsOK()) {
    const std::string message = "Failed to load from EpContext model. " + load_status.GetErrorMessage();
    ORT_CXX_LOG(logger, ORT_LOGGING_LEVEL_ERROR, message.c_str());
    return Ort::Status(message.c_str(), ORT_INVALID_GRAPH);
  }
...
Ort::Status CreateEPContextNodes(const OrtNode** fused_nodes,
  ...
  for (size_t idx = 0; idx < count; ++idx) {
    ...
    ORT_CXX_RETURN_ON_API_FAIL(ort_api.Node_GetName(fused_node, &fused_node_name));
    const std::string graph_name(fused_node_name);

    auto qnn_model_kv = qnn_models.find(graph_name);
    RETURN_IF(qnn_model_kv == qnn_models.end(), (graph_name + " not exist in QnnModel table.").c_str());
    ...
    if (0 == idx) {
      if (qnn_context_embed_mode) {
        ...
        ORT_CXX_RETURN_ON_API_FAIL(ort_api.CreateOpAttr(EP_CACHE_CONTEXT.c_str(),
                                                        cache_payload.c_str(),
        ...
        std::basic_string<ORTCHAR_T> context_bin_path;
        ...
        if (context_bin_path.empty()) {
          ...
          context_bin_path = FILEPATH_TO_STRING(std::filesystem::path(graph_name));
        }

        context_bin_path = context_bin_path + ORT_TSTR("_qnn.bin");
      ...
      ORT_CXX_RETURN_ON_API_FAIL(ort_api.CreateOpAttr(MAIN_CONTEXT.c_str(),
                                                      &is_main_context,
                                                      1,
                                                      ORT_OP_ATTR_INT,
                                                      &attr));
    ...
    ORT_CXX_RETURN_ON_API_FAIL(ort_api.CreateOpAttr(EMBED_MODE.c_str(), &embed_mode, 1, ORT_OP_ATTR_INT, &attr));
    attributes.push_back(attr);

    attr = nullptr;
    ORT_CXX_RETURN_ON_API_FAIL(ort_api.CreateOpAttr(EP_SDK_VER.c_str(),
    ...
    attr = nullptr;
    ORT_CXX_RETURN_ON_API_FAIL(ort_api.CreateOpAttr(PARTITION_NAME.c_str(),
                                                    graph_name.c_str(),
