...
Status PoolOpBuilder::IsOpSupported(QnnModelWrapper& qnn_model_wrapper,
  ...
  if (!is1d && input_shape.size() != 4) {
    return ORT_MAKE_STATUS(ONNXRUNTIME, FAIL, "QNN Pool only supports 2D (4-D) or 1D (3-D)!");
  }
  ...
  if (is1d) {
    ...
    if (kernel_shape.size() != 1) {
      return ORT_MAKE_STATUS(ONNXRUNTIME, FAIL, "QNN Pool1D: kernel_shape must have length 1!");
    }
    auto pads = node_helper.Get("pads", std::vector<int32_t>{});
    if (pads.size() != 2) {
      return ORT_MAKE_STATUS(ONNXRUNTIME, FAIL, "QNN Pool1D: pads must have length 2!");
    }
    auto strides = node_helper.Get("strides", std::vector<int32_t>{});
    if (!strides.empty() && strides.size() != 1) {
      return ORT_MAKE_STATUS(ONNXRUNTIME, FAIL, "QNN Pool1D: strides must have length 1 or omitted!");
    }
    auto dilations = node_helper.Get("dilations", std::vector<int32_t>{1});
    if (dilations.size() != 1) {
      return ORT_MAKE_STATUS(ONNXRUNTIME, FAIL, "QNN Pool1D: dilations must have length 1 or omitted!");
    }
  } else {
...
Status PoolOpBuilder::SetCommonPoolParams(const NodeAttrHelper& node_helper,
                                          ...
                                          std::vector<uint32_t>&& output_shape) const {
  {
    auto raw_kernel_shape = node_helper.Get("kernel_shape", std::vector<uint32_t>{1, 1});
    if (raw_kernel_shape.size() == 1) {
      filter_size = {1, raw_kernel_shape[0]};
    } else {
      filter_size = raw_kernel_shape;
    }
  }
