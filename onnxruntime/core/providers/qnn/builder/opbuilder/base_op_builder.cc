...
bool IsNullNodeUnitIODef(QnnModelWrapper& qnn_model_wrapper,
  ...
  // only happen on one of Concat inputs.
...
Status BaseOpBuilder::ProcessInputs(QnnModelWrapper& qnn_model_wrapper,
  ...
  for (size_t input_i = 0; input_i < input_count; ++input_i) {
    if (!IsNullNodeUnitIODef(qnn_model_wrapper, node_unit, inputs[input_i], logger)) {
