...
class ConvTransposeBuilder : public BaseOpBuilder {
 public:
  ConvTransposeBuilder() : BaseOpBuilder("ConvTransposeBuilder") {}
  ORT_DISALLOW_COPY_ASSIGNMENT_AND_MOVE(ConvTransposeBuilder);
...
Status ConvTransposeBuilder::IsOpSupported(QnnModelWrapper& qnn_model_wrapper,
                                           const NodeUnit& node_unit,
                                           const logging::Logger& logger,
...
Status ConvTransposeBuilder::ProcessInputs(QnnModelWrapper& qnn_model_wrapper,
                                           const NodeUnit& node_unit,
                                           const logging::Logger& logger,
                                           bool is_quantized_model,
                                           std::vector<std::string>& input_names,
...
Status ConvTransposeBuilder::ProcessConv2DInputs(QnnModelWrapper& qnn_model_wrapper,
                                                 const NodeUnit& node_unit,
                                                 const logging::Logger& logger,
                                                 bool is_quantized_model,
                                                 std::vector<std::string>& input_names,
...
Status ConvTransposeBuilder::ProcessConv1DInputs(QnnModelWrapper& qnn_model_wrapper,
                                                 const NodeUnit& node_unit,
                                                 const logging::Logger& logger,
                                                 bool is_quantized_model,
                                                 std::vector<std::string>& input_names,
...
static Status GetAutoPadding(std::vector<uint32_t>& pads, const std::string& auto_pad, const std::array<uint32_t, 2>& strides,
...
Status ConvTransposeBuilder::ProcessAttributesAndOutputs(QnnModelWrapper& qnn_model_wrapper,
                                                         const NodeUnit& node_unit,
                                                         std::vector<std::string>&& input_names,
                                                         const logging::Logger& logger,
                                                         bool is_quantized_model,
  ...
  std::vector<uint32_t> output_padding = node_helper.Get("output_padding", std::vector<uint32_t>{0, 0});
  ...
  // Pads parameter.
  {
    ...
    if (auto_pad != "NOTSET") {
      ...
      ORT_RETURN_IF_ERROR(GetAutoPadding(pads, auto_pad, {strides[0], strides[1]},
...
void CreateConvTransposeOpBuilder(const std::string& op_type, OpBuilderRegistrations& op_registrations) {
  op_registrations.AddOpBuilder(op_type, std::make_unique<ConvTransposeBuilder>());
}
