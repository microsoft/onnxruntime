...
// input[0] * input[1]
Ort::Status ExpandOpBuilder::ProcessInputs(QnnModelWrapper& qnn_model_wrapper,
                                           const OrtNodeUnit& node_unit,
                                           const Ort::Logger& logger,
                                           std::vector<std::string>& input_names,
  ...
  RETURN_IF(inputs.size() != 2, "Expand should has 2 inputs!");
  ...
  RETURN_IF_NOT(is_constant_input, "QNN doesn't support dynamic shape.");
  ...
  if (is_quantized_tensor) {
    RETURN_IF_ERROR(utils::GetQnnDataType(true, input_type, qnn_data_type));
    float scale = 0.0f;
    ...
    switch (qnn_data_type) {
      ...
      default:
        return MAKE_EP_FAIL("Type not supported.");
    }  // switch
  } else {
...
Ort::Status ExpandOpBuilder::ProcessAttributesAndOutputs(QnnModelWrapper& qnn_model_wrapper,
                                                         const OrtNodeUnit& node_unit,
                                                         std::vector<std::string>&& input_names,
                                                         const Ort::Logger& logger,
                                                         bool do_op_validation) const {
...
Ort::Status ExpandOpBuilder::OverrideOutputQuantParam(QnnModelWrapper& qnn_model_wrapper,
                                                      const OrtNodeUnit& node_unit,
                                                      const Ort::Logger& logger,
                                                      const std::vector<std::string>& input_names,
                                                      size_t output_index,
