...
const std::array<std::vector<const char*>, REDUCE_OP_TYPE_COUNT> ReduceOpBuilder::data_input_quant_types = {
    std::vector<const char*>{"uint8", "int8", "uint16"},  // ReduceMax
    std::vector<const char*>{"uint8", "int8", "uint16"},  // ReduceMax
    std::vector<const char*>{"uint8", "int8", "uint16"},  // ReduceMean
    std::vector<const char*>{"uint8", "int8", "uint16"},  // ReduceProd
    std::vector<const char*>{"uint8", "int8", "uint16", "int32"},  // ReduceSum
};
...
Status ReduceOpBuilder::IsOpSupported(QnnModelWrapper& qnn_model_wrapper,
  ...
  if (opset >= opset_axes_as_input) {  // This Reduce* operator has an axes input.
    if (inputs.size() > 1) {
      ...
      num_axes = axes_shape[0];
      if (axes_shape.size() != 1 || num_axes > input_data_rank) {
...
Status ReduceOpBuilder::SetAxesParam(QnnModelWrapper& qnn_model_wrapper,
  ...
  if (opset < opset_axes_as_input) {
    // Axes is in ONNX node attribute.
    NodeAttrHelper node_helper(node_unit);
    reduce_axes = node_helper.Get(qnn_def::axes, reduce_axes);
  } else if (inputs.size() > 1) {
    // Axes is in ONNX input[1] initializer.
