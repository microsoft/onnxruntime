...
Ort::Status InverseOpBuilder::ProcessInputs(QnnModelWrapper& qnn_model_wrapper,
                                            const OrtNodeUnit& node_unit,
                                            const Ort::Logger& logger,
                                            std::vector<std::string>& input_names,
  ...
  if (do_op_validation) {
    ...
    RETURN_IF(QNN_DATATYPE_FLOAT_32 != target_tensor_type, "QNN Inverse Op support only float input tensor.");

    std::vector<uint32_t> input_shape;
    RETURN_IF_NOT(qnn_model_wrapper.GetOnnxShape(inputs[0].shape, input_shape), "Cannot get shape of input 0.");

    RETURN_IF(input_shape.size() < 2, "QNN expects Inverse Op has rank >= 2.");

    RETURN_IF(2 != input_shape[input_shape.size() - 2] || 2 != input_shape[input_shape.size() - 1],
              "QNN expects 2x2 Inverse Op: [..., 2, 2].");
  }
...
Ort::Status InverseOpBuilder::ProcessAttributesAndOutputs(QnnModelWrapper& qnn_model_wrapper,
                                                          const OrtNodeUnit& node_unit,
                                                          std::vector<std::string>&& input_names,
                                                          const Ort::Logger& logger,
                                                          bool do_op_validation) const {
  ...
  */
  ...
  for (uint32_t i = 0; i < 4; ++i) {
    ...
    RETURN_IF_ERROR(AddQnnScalar<uint32_t>(
        qnn_model_wrapper,
        node_unit.Index(),
        slice_name,
        end_mask,
    ...
    RETURN_IF_NOT(qnn_model_wrapper.AddTensorWrapper(std::move(slice_output)), "Failed to add tensor.");
  ...
  QnnTensorWrapper adj_cat_output(adj_cat_output_name,
                                  ...
                                  QnnQuantParamsWrapper(),
                                  std::vector<uint32_t>(reshaped_shape));
  RETURN_IF_NOT(qnn_model_wrapper.AddTensorWrapper(std::move(adj_cat_output)),
                "Failed to add Inverse - Adj - cat(d, b, c, a) output tensor.");

  RETURN_IF_NOT(qnn_model_wrapper.CreateQnnNode(adj_cat_name,
                                                QNN_OP_PACKAGE_NAME_QTI_AISW,
                                                QNN_OP_CONCAT,
                                                {sliced_output_names[3],
                                                 sliced_output_names[1],
                                                 sliced_output_names[2],
                                                 sliced_output_names[0]},
                                                {adj_cat_output_name},
  ...
  QnnTensorWrapper inverse_div_tensor(inverse_div_output_name,
                                      ...
                                      QnnQuantParamsWrapper(),
                                      std::vector<uint32_t>(reshaped_shape));
  RETURN_IF_NOT(qnn_model_wrapper.AddTensorWrapper(std::move(inverse_div_tensor)),
  ...
  RETURN_IF_NOT(qnn_model_wrapper.AddTensorWrapper(std::move(inverse_output)), "Failed to add tensor.");

  RETURN_IF_ERROR(qnn_model_wrapper.AddReshapeNode(inverse_div_output_name,
                                                   org_output_name,
                                                   std::vector<uint32_t>(reshaped_shape),
                                                   std::vector<uint32_t>(output_shape),
                                                   output_info.qnn_data_type,
