...
Status QnnBackendManager::ExtractBackendProfilingInfo(qnn::profile::ProfilingInfo& profiling_info) {
  ...
  if (ProfilingLevel::INVALID != profiling_level_etw_ && !tracelogging_provider_ep_enabled) {
    LOGS(*logger_, VERBOSE) << "ETW enabled previously, but disabled now. Can't do the switch! Won't output any profiling.";
    return Status::OK();
  }
...
std::unique_ptr<void, std::function<void(void*)>> QnnBackendManager::GetSystemContextHandle() {
  ...
  if (QNN_SUCCESS != rt) {
    LOGS(*logger_, ERROR) << "Failed to create system handle.";
    return nullptr;
  }
...
Status QnnBackendManager::GetGraphInfoAndBinVersion(QnnSystemContext_Handle_t sys_ctx_handle,
                                                    void* buffer,
