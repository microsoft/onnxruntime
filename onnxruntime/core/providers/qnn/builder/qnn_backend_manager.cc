...
Status QnnBackendManager::InitializeProfiling() {
  ...
  if (enable_optrace) {
    if (QNN_PROFILE_LEVEL_DETAILED != qnn_profile_level) {
      LOGS_DEFAULT(WARNING) << "Profiling level must be detailed to enable op trace.";
    } else {
...
Status QnnBackendManager::ExtractBackendProfilingInfo(qnn::profile::ProfilingInfo& profiling_info) {
  ...
  if (num_events > 0) {
    ...
    if (!profiling_file_path_.empty()) {
      ORT_RETURN_IF_ERROR(profile_writer->Init());
    }
#endif
