...
constexpr const char* PROVIDER_KEY = "provider";
constexpr const char* QNN_PROVIDER = "ort_qnn_ep";
constexpr const char* MODEL_NAME_KEY = "model_name";
constexpr const char* GRAPH_NAME_KEY = "graph_name";
const long long RESERVED_LENGTH = 255;
...
Status QnnBackendManager::DumpQnnContext(const onnxruntime::PathString& context_cache_pathstring,
  ...
  ORT_ENFORCE(length_required <= RESERVED_LENGTH, "Model name too long, exceeds context binary metadata length limit.");
  char context_buffer_meta[RESERVED_LENGTH];
  sprintf_s(context_buffer_meta, "{%s:%s|%s:%s|%s:%s}", PROVIDER_KEY, QNN_PROVIDER, MODEL_NAME_KEY, model_name.c_str(), GRAPH_NAME_KEY, graph_name.c_str());
  of_stream.write(context_buffer_meta, RESERVED_LENGTH);
...
Status QnnBackendManager::LoadCachedQnnContext(const onnxruntime::PathString& context_cache_pathstring, QnnModel& qnn_model) {
  ...
  if (ort_generated_ctx_cache_) {
    cache_file.seekg(RESERVED_LENGTH);
    buffer_size -= RESERVED_LENGTH;
  }
...
Status QnnBackendManager::GetMetadataFromOrtContextFile(const onnxruntime::PathString& context_cache_pathstring) {
  ...
  buffer_size = cache_file.tellg();
  ORT_RETURN_IF(0 == buffer_size, "Empty cache file encountered.");
  // Ort reserved 255 bytes for metadata
  if (buffer_size <= RESERVED_LENGTH) {
    return Status::OK();
  }

  std::unique_ptr<unsigned char[]> buffer = std::make_unique<unsigned char[]>(RESERVED_LENGTH);
  ORT_RETURN_IF(nullptr == buffer, "Failed to allocate memory for cache file.");

  // Load file into buffer
  cache_file.seekg(0, cache_file.beg);
  const auto& read_result = cache_file.read(reinterpret_cast<char*>(buffer.get()), RESERVED_LENGTH);
  cache_file.close();
  ORT_RETURN_IF(!read_result, "Failed to read contents from cached context file.");

  std::string metadata(reinterpret_cast<char*>(buffer.get()));
  std::string provider_value = "";
  auto metadata_end = metadata.find('}');
  if (metadata[0] == '{' && metadata_end != std::string::npos) {
    metadata.erase(metadata_end, 1);
    metadata.erase(0, 1);
    bool not_done = true;
    while (not_done) {
      size_t end_pos = metadata.find("|");
      if (end_pos == std::string::npos) {
        end_pos = metadata.length();
        not_done = false;
      }
      std::string token(metadata.substr(0, end_pos));
      auto pos = token.find(":");
      ...
      if (key == MODEL_NAME_KEY) {
        model_name_from_ctx_cache_ = value;
      } else if (key == GRAPH_NAME_KEY) {
        graph_name_from_ctx_cache_ = value;
      } else if (key == PROVIDER_KEY) {
        provider_value = value;
      }
      metadata.erase(0, end_pos + 1);
    }
    if (provider_value == QNN_PROVIDER) {
...
/* Validate the model file name and graph name with Ort generated context cache metadata
