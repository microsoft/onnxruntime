...
const std::string GetEventTypeString(QnnProfile_EventType_t event_type) {
...
const std::string ExtractQnnScalarValue(const Qnn_Scalar_t& scalar) {
...
Status Serializer::ProcessExtendedEvent(const QnnProfile_EventId_t event_id, const std::string& event_level,
  ...
  std::string message = GetEventTypeString(event_data.v1.type);
  std::string unit = GetUnitString(event_data.v1.unit);
...
Serializer::Serializer(const ProfilingInfo& profiling_info)
  ...
  output_filename_ = output_fs_filepath.filename().string();
...
Status Serializer::SerializeEvents() {
  ...
  QnnSystemProfile_SerializationTargetFile_t serialization_file{output_filename_.c_str(), output_directory_.c_str()};
