...
class ONNX_OPERATOR_SET_SCHEMA_CLASS_NAME(Microsoft, 1, FasterTransformerBert);//slx
class ONNX_OPERATOR_SET_SCHEMA_CLASS_NAME(Microsoft, 1, FasterTransformerVit);//slx
...
class OpSet_Microsoft_ver1 {
 public:
  static void ForEachSchema(std::function<void(ONNX_NAMESPACE::OpSchema&&)> fn) {
    ...
    fn(GetOpSchema<ONNX_OPERATOR_SET_SCHEMA_CLASS_NAME(Microsoft, 1, FasterTransformerBert)>());//slx	
    fn(GetOpSchema<ONNX_OPERATOR_SET_SCHEMA_CLASS_NAME(Microsoft, 1, FasterTransformerVit)>());//slx
  }
};
}  // namespace contrib
}  // namespace onnxruntime
