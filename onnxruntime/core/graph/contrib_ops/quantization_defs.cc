...
ONNX_MS_OPERATOR_SET_SCHEMA(QuantizeLinear, 1, OpSchema().Attr("axis",
                                                   ...
                                                   .TypeConstraint("T2", {"tensor(int8)", "tensor(uint8)"}, "Constrain 'y_zero_point' and 'y' to 8-bit integer tensors.")
...
ONNX_MS_OPERATOR_SET_SCHEMA(DequantizeLinear, 1, OpSchema().Attr("axis",
                                                                 "The axis along which same quantization parameters are applied. It's optional."
                                                                 "If it's not specified, it means per-tensor quantization and input 'x_scale' and 'x_zero_point' must be scalars."
                                                                 "If it's specified, it means per 'axis' quantization and input 'x_scale' and 'x_zero_point' must be 1-D tensors.",
                                                                 AttributeProto::INT, false)
                                                     ...
                                                     .TypeConstraint("T1", {"tensor(int8)", "tensor(uint8)"}, "Constrain 'x' and 'x_zero_point' to 8-bit integer tensors.")
...
ONNX_MS_OPERATOR_SET_SCHEMA(ReduceSumInteger, 1, OpSchema().SetDoc(R"DOC(
...
with the exception that numpy default keepdims to False instead of True.)DOC")
                                                     ...
                                                     .Attr("axes", "A list of integers, along which to reduce. The default is to reduce over all the dimensions of the input tensor.", AttributeProto::INTS)
                                                     .Attr("keepdims", "Keep the reduced dimension or not, default 1 mean keep reduced dimension.", AttributeProto::INT));

ONNX_MS_OPERATOR_SET_SCHEMA(MulInteger, 1, OpSchema().SetDoc(R"DOC(Performs element-wise binary quantized multiplication (with Numpy-style broadcasting support).
...
```
                                               ...
                                               .Input(1, "A_zero_point", "Input A zero point. Default value is 0 if it's not specified. It's a scalar, which means a per-tensor/layer quantization.", "T", OpSchema::Optional)
                                               .Input(2, "B", "Second operand.", "T")
                                               .Input(3, "B_zero_point", "Input B zero point. Default value is 0 if it's not specified. It's a scalar, which means a per-tensor/layer quantization.", "T", OpSchema::Optional)
...
ONNX_MS_OPERATOR_SET_SCHEMA(DynamicQuantizeMatMul, 1, OpSchema().Input(0, "A", "N-dimensional matrix A", "T1").Input(1, "B", "N-dimensional matrix B", "T2").Input(2, "b_scale",
...
ONNX_MS_OPERATOR_SET_SCHEMA(MatMulIntegerToFloat, 1, OpSchema().Input(0, "A", "N-dimensional matrix A", "T1").Input(1, "B", "N-dimensional matrix B", "T2").Input(2, "a_scale",
                                                         ...
                                                         .TypeConstraint("T2", {"tensor(int8)", "tensor(uint8)"}, "Constrain input B data type to 8-bit integer tensor.")
                                                         .TypeConstraint("T3", {"tensor(float)"}, "Constrain input a_scale, b_scale and output Y data type as float tensor.")
                                                         .TypeAndShapeInferenceFunction([](ONNX_NAMESPACE::InferenceContext& ctx) {
                                                           propagateElemTypeFromInputToOutput(ctx, 2, 0);
                                                           ONNX_NAMESPACE::matmulShapeInference(ctx, 0, 1);
                                                         }));

ONNX_MS_OPERATOR_SET_SCHEMA(QLinearAdd, 1, OpSchema().FillUsing(QLinearMathDocGenerator("addition", "C = (A_scale * (A - A_zero_point) + B_scale * (B - B_zero_point))/C_scale + C_zero_point")));

ONNX_MS_OPERATOR_SET_SCHEMA(QLinearMul, 1, OpSchema().FillUsing(QLinearMathDocGenerator("multiplication", "C = ((A - A_zero_point) * (B - B_zero_point)) * (A_scale * B_scale)/C_scale + C_zero_point")));

ONNX_MS_OPERATOR_SET_SCHEMA(QLinearReduceMean, 1, OpSchema().SetDoc(R"DOC(
...
```
                                                      ...
                                                      .Input(1, "data_scale", "Input scale. It's a scalar, which means a per-tensor/layer quantization.", "tensor(float)")
                                                      .Input(2, "data_zero_point", "Input zero point. Default value is 0 if it's not specified. It's a scalar, which means a per-tensor/layer quantization.", "T", OpSchema::Optional)
                                                      .Input(3, "reduced_scale", "Output scale. It's a scalar, which means a per-tensor/layer quantization.", "tensor(float)")
                                                      .Input(4, "reduced_zero_point", "Output zero point. Default value is 0 if it's not specified. It's a scalar, which means a per-tensor/layer quantization.", "T", OpSchema::Optional)
...
ONNX_MS_OPERATOR_SET_SCHEMA(QLinearSigmoid, 1, OpSchema().SetDoc(QLinearSigmoidDoc_ver1).Input(0, "X", "Input tensor", "T").Input(1, "X_scale", "Input X's scale. It's a scalar, which means a per-tensor/layer quantization.", "tensor(float)").Input(2, "X_zero_point", "Input X's zero point. Default value is 0 if it's not specified. It's a scalar, which means a per-tensor/layer quantization.", "T", OpSchema::Optional).Input(3, "Y_scale", "Output Y's scale. It's a scalar, which means a per-tensor/layer quantization.", "tensor(float)").Input(4, "Y_zero_point", "Output Y's zero point. Default value is 0 if it's not specified. It's a scalar, which means a per-tensor/layer quantization.", "T", OpSchema::Optional).Output(0, "Y", "Output tensor", "T").TypeConstraint("T", {"tensor(uint8)", "tensor(int8)"}, "Constrain input and output types to 8 bit tensors.").TypeAndShapeInferenceFunction(ONNX_NAMESPACE::propagateShapeAndTypeFromFirstInput));

ONNX_MS_OPERATOR_SET_SCHEMA(DynamicQuantizeLSTM, 1, OpSchema().Attr("direction",
...
ONNX_MS_OPERATOR_SET_SCHEMA(QLinearConcat, 1, OpSchema().Attr("axis", "Which axis to concat on", AttributeProto::INT).SetDoc("Concatenate a list of tensors into a single tensor."
...
ONNX_MS_OPERATOR_SET_SCHEMA(QGemm, 1, OpSchema().SetDoc("Quantized Gemm").Input(0, "A",
                                                                                "Input tensor A. "
...
ONNX_MS_OPERATOR_SET_SCHEMA(DequantizeWithOrder, 1, OpSchema().SetDoc(R"DOC(Dequantize input matrix to specific layout used in cublaslt. attr to specify output type, float16 or float32)DOC").Attr("order_input", "cublasLt order of input matrix", AttributeProto::INT).Attr("order_output", "cublasLt order of output matrix", AttributeProto::INT).Input(0, "input", "TODO: input tensor of (ROWS, COLS). if less than 2d, will broadcast to (1, X). If 3d, it is treated as (B, ROWS, COS)", "Q").Input(1, "scale_input", "scale of the input", "F").Output(0, "output", "output tensor", "F").TypeConstraint("Q", {"tensor(int8)"}, "Constrain input and output types to int8 tensors.").TypeConstraint("F", {"tensor(float16)", "tensor(float)"}, "Constrain to float types").TypeAndShapeInferenceFunction([](ONNX_NAMESPACE::InferenceContext& ctx) {
...
ONNX_MS_OPERATOR_SET_SCHEMA(QOrderedMatMul, 1, OpSchema().SetDoc(QOrderedMatMul_ver1_doc).Attr("order_A", "cublasLt order of matrix A", AttributeProto::INT).Attr("order_B", "cublasLt order of matrix B", AttributeProto::INT).Attr("order_Y", "cublasLt order of matrix Y and optional matrix C", AttributeProto::INT).Input(0, "A", "3-dimensional matrix A", "Q").Input(1, "scale_A", "scale of the input A", "S").Input(2, "B", "2-dimensional matrix B", "Q").Input(3, "scale_B", "scale of the input B", "S").Input(4, "scale_Y", "scale of the output Y", "S").Input(5, "bias", "1d bias", "S", OpSchema::Optional).Input(6, "C", "3d or 2d matrix C. if 2d expand to 3d first. Shape[0] should be 1 or same as A.shape[0] ", "Q", OpSchema::Optional).Input(7, "scale_C", "scale of the input A", "S", OpSchema::Optional).Output(0, "Y", "Matrix multiply results from A * B", "Q").TypeConstraint("Q", {"tensor(int8)"}, "Constrain input and output types to int8 tensors.").TypeConstraint("S", {"tensor(float)"}, "Constrain bias and scales to float32").TypeAndShapeInferenceFunction([](ONNX_NAMESPACE::InferenceContext& ctx) {
  propagateElemTypeFromInputToOutput(ctx, 0, 0);
  ONNX_NAMESPACE::matmulShapeInference(ctx, 0, 2);
}));

ONNX_MS_OPERATOR_SET_SCHEMA(QOrderedAdd, 1, OpSchema().SetDoc(R"DOC(Ordered Quantize Add.)DOC").Attr("order_A", "cublasLt order of input A", AttributeProto::INT).Attr("order_B", "cublasLt order of input B", AttributeProto::INT).Attr("order_Y", "cublasLt order of matrix Y", AttributeProto::INT).Input(0, "A", "N-dimensional input A", "Q").Input(1, "scale_A", "scale of the input A", "F").Input(2, "B", "N-dimensional input B", "Q").Input(3, "scale_B", "scale of the input B", "F").Input(4, "scale_Y", "scale of the output Y", "F").Output(0, "Y", "Added results from A + B", "Q").TypeConstraint("Q", {"tensor(int8)"}, "Constrain input and output types to int8 tensors.").TypeConstraint("F", {"tensor(float)"}, "Constrain to float32").TypeAndShapeInferenceFunction([](InferenceContext& ctx) {
  propagateElemTypeFromInputToOutput(ctx, 0, 0);
  if (hasInputShape(ctx, 0) && hasInputShape(ctx, 2))
    bidirectionalBroadcastShapeInference(
        ctx.getInputType(0)->tensor_type().shape(),
...
ONNX_MS_OPERATOR_SET_SCHEMA(QOrderedBiasGelu, 1, OpSchema().SetDoc(R"DOC(Ordered Quantize Add Bias and then Gelu.)DOC").Attr("order_A", "cublasLt order of input A", AttributeProto::INT).Attr("order_B", "cublasLt order of input B", AttributeProto::INT).Attr("order_Y", "cublasLt order of matrix Y", AttributeProto::INT).Input(0, "A", "N-dimensional input A", "Q").Input(1, "scale_A", "scale of the input A", "F").Input(2, "B", "N-dimensional input B", "Q").Input(3, "scale_B", "scale of the input B", "F").Input(4, "scale_Y", "scale of the output Y", "F").Output(0, "Y", "Added results from A + B", "Q").TypeConstraint("Q", {"tensor(int8)"}, "Constrain input and output types to int8 tensors.").TypeConstraint("F", {"tensor(float)"}, "Constrain to float32").TypeAndShapeInferenceFunction(ONNX_NAMESPACE::propagateShapeAndTypeFromFirstInput));
...
ONNX_MS_OPERATOR_SET_SCHEMA(QOrderedAttention, 1, OpSchema().SetDoc(Attention_QOrdered_doc).Attr("num_heads", "Number of attention heads", AttributeProto::INT).Attr("unidirectional", "Whether every token can only attend to previous tokens. Default value is 0.", AttributeProto::INT, static_cast<int64_t>(0)).Attr("order_input", "cublasLt order of input matrix", AttributeProto::INT).Attr("order_weight", "cublasLt order of weight matrix", AttributeProto::INT).Attr("order_bias", "cublasLt order of bias, (TODO: remove not used)", AttributeProto::INT).Attr("order_output", "cublasLt order of global bias", AttributeProto::INT).Input(0, "input", "3D input tensor with shape (batch_size, sequence_length, input_hidden_size)", "Q").Input(1, "scale_input", "scale of the input", "S").Input(2, "weight", "2D input tensor with shape (input_hidden_size, 3 * hidden_size), where hidden_size = num_heads * head_size", "Q").Input(3, "scale_weight", "scale of the weight", "S").Input(4, "bias", "1D input tensor with shape (3 * hidden_size)", "S").Input(5, "scale_bias", "scale of the bias, (TODO: remove)", "S").Input(6, "scale_gemm", "scale of the gemm", "S").Input(7, "mask_index",
...
ONNX_MS_OPERATOR_SET_SCHEMA(QOrderedAddBiasResidualLayerNorm, 1,
                            OpSchema()
                                .SetDoc("QOrderedAddBiasResidualLayerNorm")
                                .Attr("axis", "The first normalization dimension: normalization will be performed along dimensions axis : rank(inputs).", AttributeProto::INT, static_cast<int64_t>(-1))
                                .Attr("epsilon", "The epsilon value to use to avoid division by zero.", AttributeProto::FLOAT, 1e-5f)
                                .Attr("order_X", "cublasLt order of input X", AttributeProto::INT)
                                .Attr("order_R", "cublasLt order of matrix Y, must be same as order_X. ORDER_ROW or ORDER_COL32 supported", AttributeProto::INT)
                                ...
                                .Input(2, "R", "Residual. Should have the same shape as input X", "Q", OpSchema::Optional)
                                .Input(3, "scale_R", "scale of quantized residual", "S", OpSchema::Optional)
                                .Input(4, "B", "bias. 1-D input and the dim value should be equal to the last dim of input X", "F", OpSchema::Optional)
                                .Input(5, "scale_Y", "scale of the quantized X", "S")
                                .Input(6, "gamma", "Scale tensor, i.e., gamma vector", "F")
                                .Input(7, "beta", "Bias tensor for LayerNorm", "F", OpSchema::Optional)
                                .Output(0, "Y", "Output data tensor.", "Q")
                                .TypeConstraint("F", {"tensor(float16)"}, "Constrain input scale and bias could be float16 tensors.")
...
ONNX_MS_OPERATOR_SET_SCHEMA(QOrderedLayerNormalization, 1, OpSchema().SetDoc("QOrderedLayerNormalization").Attr("axis", "The first normalization dimension: normalization will be performed along dimensions axis : rank(inputs).", AttributeProto::INT, static_cast<int64_t>(-1)).Attr("epsilon", "The epsilon value to use to avoid division by zero.", AttributeProto::FLOAT, 1e-5f).Attr("order_X", "cublasLt order of input X", AttributeProto::INT).Attr("order_Y", "cublasLt order of matrix Y, must be same as order_X. ORDER_ROW or ORDER_COL32 supported", AttributeProto::INT).AllowUncheckedAttributes().Input(0, "X", "Input data tensor from the previous layer.", "Q").Input(1, "scale_X", "scale of the quantized X", "S").Input(2, "scale", "Scale tensor, i.e., gamma vector.", "F").Input(3, "B", "Bias tensor.", "F", OpSchema::Optional).Input(4, "scale_Y", "scale of the quantized X", "S").Output(0, "Y", "Output data tensor.", "Q").TypeConstraint("F", {"tensor(float16)"}, "Constrain input scale and bias could be float16 tensors.").TypeConstraint("S", {"tensor(float)"}, "quantization scale must be float tensors.").TypeConstraint("Q", {"tensor(int8)"}, "quantization tensor must be int8 tensors.").TypeAndShapeInferenceFunction([](ONNX_NAMESPACE::InferenceContext& ctx) {
  propagateShapeAndTypeFromFirstInput(ctx);
  propagateElemTypeFromInputToOutput(ctx, 0, 0);
}));

ONNX_MS_OPERATOR_SET_SCHEMA(QOrderedGelu, 1, OpSchema().SetDoc(R"DOC(Ordered Quantize Gelu.)DOC").Attr("order_X", "cublasLt order of input X", AttributeProto::INT).Attr("order_Y", "cublasLt order of matrix Y, must be same as order_X. ORDER_ROW or ORDER_COL32 supported", AttributeProto::INT).Input(0, "X", "N-dimensional input A", "Q").Input(1, "scale_X", "scale of the input A", "S").Input(2, "scale_Y", "scale of the output Y", "S").Output(0, "Y", "Output of the Gelu", "Q").TypeConstraint("Q", {"tensor(int8)"}, "Constrain input and output types to int8 tensors.").TypeConstraint("S", {"tensor(float)"}, "Constrain scales to float32").TypeAndShapeInferenceFunction(ONNX_NAMESPACE::propagateShapeAndTypeFromFirstInput));
