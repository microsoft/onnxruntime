...
This op functions in much the same was as Dropout-11 and Dropout-13 do, execpt that the mask is output as a bit-packed uint32 tensor, instead of a boolean tensor.
  ...
  ONNX_CONTRIB_OPERATOR_SCHEMA(AOTanyOp)
      .SetDomain(kMSDomain)
      .SinceVersion(1)
      .SetDoc(R"DOC()DOC")
      .Attr("func_name", "function name generated by AOT compiler", AttributeProto::STRING)
      .Attr("func_type", "dispath to correct function type according to input nums", AttributeProto::INT)
      .Attr("lib_path", "lib_path, the artifact of AOT compiler", AttributeProto::STRING)
      ...
      .Output(1, "Y", "tensor. The output tensor has the same rank as the input. ", "T", OpSchema::Optional)
      .Output(2, "Y", "tensor. The output tensor has the same rank as the input. ", "T", OpSchema::Optional)
      .Output(3, "Y", "tensor. The output tensor has the same rank as the input. ", "T", OpSchema::Optional)
      .TypeConstraint("T", {"tensor(float)", "tensor(int64)"}, "Constrain input and output types to float tensors.")
      .TypeAndShapeInferenceFunction([](ONNX_NAMESPACE::InferenceContext& ctx) {
        // Type inference
        propagateElemTypeFromInputToOutput(ctx, 0, 0);
