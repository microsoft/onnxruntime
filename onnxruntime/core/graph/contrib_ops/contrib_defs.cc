...
Example 4:
  ...
  ONNX_CONTRIB_OPERATOR_SCHEMA(MulUnsqueezeConcatFused)
      .SetDomain(kMSDomain)
      .SinceVersion(1)
      .Input(0, "data_0", "Input tensor - data 0.", "T")
      .Input(1, "data_1", "Input tensor -  data 1.", "T")
      .Input(2, "data_2", "Input tensor -  data 2.", "T")
      .Input(3, "data_3", "Input tensor -  data 3.", "T")
      .Output(0, "output", "Output tensor", "T")
      .TypeConstraint(
          "T",
          {"tensor(int64)"},
          "Constrain input and output types to int64 tensors.")
      .TypeAndShapeInferenceFunction([](ONNX_NAMESPACE::InferenceContext& ctx) {
        // Type inference
        propagateElemTypeFromInputToOutput(ctx, 0, 0);
        ...
        output_shape_0->add_dim()->set_dim_value(3);

        return;
      })
      .SetDoc(R"DOC(TODO)DOC");
