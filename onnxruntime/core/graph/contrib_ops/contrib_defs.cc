...
ONNX_MS_OPERATOR_SET_SCHEMA(
    QKNormRotaryEmbedding, 1,
    OpSchema()
        .SetDoc(
            "Per-token preparation of the query and a shared latent KV row, from the raw "
            "projections up to the attention kernel, as used by MLA-style attention.\n"
            "\n"
            ...
            " * sin[t]\n"
            "\n"
            "Finally, when `simulate_fp8` is set, the leading `head_dim - rope_head_dim` channels "
        ...
        .Attr("simulate_fp8", "Simulate the FP8 round trip on the KV row's non-rotary half.",
...
ONNX_MS_OPERATOR_SET_SCHEMA(
    InverseRotaryRegroup, 1,
    OpSchema()
        .SetDoc(
            "Undoes the rotation QKNormRotaryEmbedding applied to the query and regroups the "
