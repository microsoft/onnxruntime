...
ONNX_MS_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr("sliding_window_cache",
              ...
              "min(total_sequence_length, local_window_size) <= L <= min(total_sequence_length, capacity). "
              "Requires local_window_size > 0 and a cache capacity of at least local_window_size. "
              "Multi-token steps of any length are supported and produce the same layout as single-token "
              "steps, so the capacity need not cover the entire step. See the Windowed KV Cache section of "
              "the operator description for the exact resident-range, eviction and rollback contract. "
              "Default value is 0 (full-length cache).",
              AttributeProto::INT,
              static_cast<int64_t>(0))
        .Attr("do_rotary",
              "Whether to use rotary position embedding. Default value is 0.",
