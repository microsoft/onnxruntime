...
//                                        optional min_max_kv_len_for_split]
...
ONNX_MS_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Input(16,
               ...
               "optional min_max_kv_len_for_split] in CPU memory. "
               ...
               "over-estimating only costs empty work. min_max_kv_len_for_split is a replay-wide lower bound "
               "on the largest per-sequence KV length in the batch and 0 means 'unknown'. It is a "
               "performance-only hint used to enable split-KV; omitting it preserves the shape-(2) contract "
               "and disables that optimization unless the op reads exact lengths back from the device. "
