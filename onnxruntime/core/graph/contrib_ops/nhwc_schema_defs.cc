...
void convPoolShapeInferenceNhwc(InferenceContext& ctx, bool use_dilation, bool require_kernel_shape, int input1Idx, int input2Idx) {
...
ONNX_MS_OPERATOR_SET_SCHEMA(NhwcMaxPool, 1, OpSchema().Input(0, "x", "", "T").Output(0, "y", "", "T").TypeConstraint("T", {"tensor(int8)", "tensor(uint8)"}, "").Attr("auto_pad", "", AttributeProto::STRING, std::string("NOTSET")).Attr("kernel_shape", "", AttributeProto::INTS).Attr("dilations", "", AttributeProto::INTS, OPTIONAL_VALUE).Attr("strides", "", AttributeProto::INTS, OPTIONAL_VALUE).Attr("pads", "", AttributeProto::INTS, OPTIONAL_VALUE).Attr("ceil_mode", "", AttributeProto::INT, static_cast<int64_t>(0)).TypeAndShapeInferenceFunction([](InferenceContext& ctx) {
  propagateElemTypeFromInputToOutput(ctx, 0, 0);
  ::onnxruntime::contrib::convPoolShapeInferenceNhwc(ctx, true, true, 0, 1);
}));

ONNX_MS_OPERATOR_SET_SCHEMA(QLinearGlobalAveragePool, 1, OpSchema().SetDoc(R"DOC(
...
ONNX_MS_OPERATOR_SET_SCHEMA(
    QLinearAveragePool, 1, OpSchema().SetDoc(QLinearAveragePoolDoc_ver1).Attr("count_include_pad",
                               ...
                               .Attr("strides", "Stride along each spatial axis. If not present, the stride defaults to 1 along each spatial axis.", AttributeProto::INTS, OPTIONAL_VALUE)
                               ...
                               .Attr("ceil_mode", "Whether to use ceil or floor (default) to compute the output shape.", AttributeProto::INT, static_cast<int64_t>(0))
                               ...
                               .Input(0, "X",
                                      ...
                                      "T")
                               .Input(1, "x_scale", "Input scale. It's a scalar, which means a per-tensor/layer quantization.", "tensor(float)")
                               .Input(2, "x_zero_point",
                                      ...
                                      "T",
                                      OpSchema::Optional)
                               .Input(3, "y_scale", "Output scale. It's a scalar, which means a per-tensor/layer quantization.", "tensor(float)")
...
ONNX_MS_OPERATOR_SET_SCHEMA(NhwcFusedConv, 1, OpSchema().SetDoc(R"DOC(
