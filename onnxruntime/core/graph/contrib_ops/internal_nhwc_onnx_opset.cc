...
void OpSet_Internal_NHWC_ONNX::ForEachSchema(const std::function<void(ONNX_NAMESPACE::OpSchema&&)>& fn) {
  ...
  REGISTER_NHWC_SCHEMA_WITH_ACTIVATION(fn, DepthToSpace, 1);
  REGISTER_NHWC_SCHEMA_WITH_ACTIVATION(fn, DepthToSpace, 11);
  REGISTER_NHWC_SCHEMA_WITH_ACTIVATION(fn, DepthToSpace, 13);
  REGISTER_NHWC_SCHEMA(fn, QLinearConv, 10);
  REGISTER_NHWC_SCHEMA_FROM_MSDOMAIN(fn, QLinearAveragePool, 1);
  REGISTER_NHWC_SCHEMA_FROM_MSDOMAIN(fn, QLinearGlobalAveragePool, 1);
