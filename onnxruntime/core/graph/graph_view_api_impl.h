...
std::unique_ptr<TensorRef> CreateApiTensor(const onnx::TensorProto* tensor, const Path& path, AllocatorPtr cpu_allocator);

class ApiValueInfoView : virtual public ValueInfoViewRef {
 private:
...
class ApiNodeView : virtual public NodeViewRef {
 private:
  const Node& node_;
 public:
...
class ApiGraphView : virtual public GraphViewRef {
 ...
 public:
  ...
  std::unique_ptr<TensorRef> GetConstant(std::string_view name) const override;
  std::unique_ptr<ValueInfoViewRef> GetValueInfoView(std::string_view name) const override;
#ifdef INTREE_EP
