...
FunctionTemplate::FunctionTemplate(const std::string& function_domain,
  ...
  onnx_func_proto_ = iter->second;
  ORT_ENFORCE(onnx_func_proto_);
  ...
  op_schema_->SetName(function_name);
  op_schema_->SetDomain(function_domain);
  ...
  op_schema_->TypeAndShapeInferenceFunction(
      [this, &domain_version_map, &model_local_functions](ONNX_NAMESPACE::InferenceContext& ctx) {
          auto schema_registry = ONNX_NAMESPACE::OpSchemaRegistry::Instance();
          ONNX_NAMESPACE::ShapeInferenceOptions options{true, 1, false};
          InferShapeForFunctionNode(ctx, *onnx_func_proto_, domain_version_map, options, schema_registry, model_local_functions, function_utils::GetFunctionIdentifier);
  });
  
  op_schema_->Finalize();
}

std::unique_ptr<Function> FunctionTemplate::Instantiate(const onnxruntime::Graph& graph,
...
std::unique_ptr<Function> FunctionTemplate::Instantiate(const onnxruntime::Graph& graph,
  ...
  return FunctionTemplate::Instantiate(graph, node_index, *onnx_func_proto_, logger);
}

const std::string& FunctionTemplate::Name() const {
  return onnx_func_proto_->name();
}

const std::string& FunctionTemplate::Domain() const {
  return onnx_func_proto_->domain();
}
