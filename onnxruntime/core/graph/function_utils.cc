...
class Inliner {
 private:
  ...
  // Maps actual (call-site) argument names to their concrete TypeProtos. Populated when
  // Specialize is called with a Node& whose InputDefs() carry shape information. Used to
  // resolve symbolic dim_params in a Scan body's value_info entries at inline time, so that
  // the memory planner sees concrete sizes for carry-state buffers rather than dim_param="S".
  ...
  // renamed carry-state init input types (looked up from outer_var_types_) against the
  // Scan body's typed carry-state inputs. Only dimensions where the outer input provides a
  // concrete dim_value AND the body input uses a matching dim_param contribute to the map.
  void build_scan_dim_bindings(const NodeProto& scan_node, const GraphProto& body,
    ...
    // scan-sequence inputs; the rest (leading inputs) are carry-state init inputs.
    ...
    // num carry states = total Scan inputs - num_scan_inputs
  ...
  void transform(GraphProto& graph,
    ...
    for (auto& n : *graph.mutable_node())
      transform(n);
    // Rename value_info entries to match the renamed node outputs. Without this,
    // shape annotations become orphaned after inlining, causing the memory planner
    // (e.g. CUDA EP) to fall back to dim_value=0 for Scan body carry-state buffers.
    for (auto& vi : *graph.mutable_value_info())
      rename(*vi.mutable_name(), false);
    // Replace symbolic dim_params with concrete dim_values where the caller has
    // provided a concrete binding (e.g. S=4 from the Scan carry-init type).
...
void Specialize(ONNX_NAMESPACE::FunctionProto& called_function, const Node& calling_node, const std::string& unique_prefix) {
  ...
  // Build a map from actual argument name -> concrete TypeProto for all shaped input defs.
  // This is used by the Inliner to propagate concrete shapes from the call site into Scan
  // body value_info entries (replacing symbolic dim_params with concrete dim_values), so that
  // the memory planner can size carry-state buffers without a separate shape-inference pass.
  InlinedHashMap<std::string, const ONNX_NAMESPACE::TypeProto*> outer_var_types;
