...
inline bool UpdateDeviceCensusState(
    std::optional<DeviceCensusState> state,
    std::string_view census_identity,
    int64_t utc_day,
    std::string_view library_version,
    bool emit_current_day,
    const std::function<bool(const DeviceCensusState&)>& persist,
    const std::function<void(
        int64_t, const std::vector<std::string>&)>& emit_completed_day) {
  if (!IsValidDeviceCensusIdentity(census_identity) || utc_day < 0 ||
      !IsValidDeviceCensusVersion(library_version) || !persist ||
      !emit_completed_day) {
    return false;
  }

  if (!state || state->identity != census_identity) {
    DeviceCensusState new_state{
        kDeviceCensusSchemaVersion,
        std::string(census_identity),
        utc_day,
        emit_current_day,
        {}};
    if (!AddDeviceCensusVersion(new_state, std::string(library_version)) ||
        !persist(new_state)) {
      return false;
    }
    if (emit_current_day) {
  ...
  if (state->schema_version != kDeviceCensusSchemaVersion ||
      state->utc_day > utc_day) {
    return false;
  }

  if (state->utc_day == utc_day) {
    if (std::binary_search(state->versions.begin(), state->versions.end(),
                           std::string(library_version))) {
      return true;
    }
    return AddDeviceCensusVersion(*state, std::string(library_version)) &&
           persist(*state);
  }
