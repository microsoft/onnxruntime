...
// Lightweight JSON builder to avoid pulling in a full JSON library.
class JsonBuilder {
 ...
 public:
  ...
  JsonBuilder& Add(const char* key, bool val) {
    Sep();
    oss_ << '"' << key << "\":" << (val ? "true" : "false");
    return *this;
  }

  // Serialize a string vector as a comma-separated string value.
  ...
  // Serialize a map<string, int> as "k1=v1,k2=v2" string value.
  ...
  // Serialize a map<string, string> as "k1=v1,k2=v2" string value.
  ...
  // Serialize a map<int64_t, long long> as individual "prefix_key": value fields.
