...
Status AddReluFusion::ApplyImpl(Graph& graph, bool& modified, int graph_level, const logging::Logger& logger) const {
  ...
  for (auto node_index : node_topology_list) {
    ...
    if (input1_shape->dim_size() ==4) {
      relu_input.push_back(node.MutableInputDefs()[1]);
      relu_input.push_back(node.MutableInputDefs()[0]);
    } 
    ...
    if (!(graph_utils::IsSupportedOptypeVersionAndDomain(next_node, "Relu", {6}, kOnnxDomain)) ||
    ...
    Node& add_node = node;
    Node& relu_node = const_cast<Node&>(next_node);
    std::string op_type = "AddRelu";
    if (is_fast_gelu) op_type = "FastGelu";

    Node& relu_add_fusion_node = graph.AddNode(graph.GenerateNodeName(op_type),
                                               op_type,
                                               "fused Add and Relu",
                                               relu_input,
                                               {},
    ...
    relu_add_fusion_node.SetExecutionProviderType(relu_node.GetExecutionProviderType());

    // move output definitions and edges from gelu_node to gelu_add_fusion_node
    //delete add_node and gelu_node.
    graph_utils::FinalizeNodeFusion(graph, {add_node, relu_node}, relu_add_fusion_node);
