...
/**
@Class ConcatSliceFusion
Rewrite graph fusing reshape subgraph to a single Reshape node.
*/
class ConcatSliceFusion : public GraphTransformer {
 public:
  ConcatSliceFusion(const std::unordered_set<std::string>& compatible_execution_providers = {}) noexcept
      : GraphTransformer("ConcatSliceFusion", compatible_execution_providers) {}
