...
bool DoubleQDQPairsRemover::IsNodeRemovable(
  ...
  if (self_zp_type == ONNX_NAMESPACE::TensorProto_DataType::TensorProto_DataType_UINT16) {
    return ResetParentAndGrandchildZeroPointAndScale<uint16_t>(graph, *self, *child, *parent, *grandchild);
  }

  if (self_zp_type == ONNX_NAMESPACE::TensorProto_DataType::TensorProto_DataType_INT16) {
    return ResetParentAndGrandchildZeroPointAndScale<int16_t>(graph, *self, *child, *parent, *grandchild);
  }
