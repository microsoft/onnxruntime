...
Status DoubleQDQPairsRemover::ApplyImpl(
  ...
  for (const auto& node_index : node_topology_list) {
    ...
    if (IsNodeRemovable(graph, logger, node_index, parent_index, child_index, grandchild_index)) {
      ...
      graph_utils::ReplaceNodeInput(*graph.GetNode(grandchild_index), 0, *graph.GetNode(node_index)->MutableInputDefs()[0]);
...
bool DoubleQDQPairsRemover::IsNodeRemovable(
  ...
  if (node == nullptr || node->OpType() != "DequantizeLinear" || node->GetInputEdgesCount() != 1 || node->GetOutputEdgesCount() == 0) {
    return false;
  }
  ...
  if (parent == nullptr || parent->OpType() != "QuantizeLinear") {
    return false;
  }
  ...
  if (child == nullptr || child->OpType() != "QuantizeLinear" || child->GetOutputEdgesCount() == 0) {
    return false;
  }
  ...
  if (grandchild == nullptr || grandchild->OpType() != "DequantizeLinear") {
    return false;
  }
