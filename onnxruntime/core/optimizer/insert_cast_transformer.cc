...
static std::optional<float> GetAttributeFloat(const onnxruntime::Node& node,
                                              const std::string& attr_name,
                                              float default_value) {
  const auto& attrs = node.GetAttributes();
  const auto attr = attrs.find(attr_name);
  ...
  if (attr->second.type() != ONNX_NAMESPACE::AttributeProto_AttributeType_FLOAT) {
    return std::nullopt;
  }
...
static bool CanUseAcceleratedCpuFp16Gemm(
    const onnxruntime::Node& node,
    const CpuFp16GemmShape& shape,
    const MLAS_BACKEND_KERNEL_SELECTOR_CONFIG& mlas_backend_kernel_selector_config) {
  // Keep this aligned with the MLAS dispatch guard in Gemm_MLFloat16. Retaining any other
  // configuration in fp16 sends it to the Eigen fallback instead of accelerated MLAS.
  const auto trans_a = GetAttributeInt(node, "transA", 0);
  const auto trans_b = GetAttributeInt(node, "transB", 0);
  const auto alpha = GetAttributeFloat(node, "alpha", 1.0f);
  const auto beta = GetAttributeFloat(node, "beta", 1.0f);
  if (!trans_a || !trans_b || !alpha || !beta ||
      *trans_a != 0 || (*trans_b != 0 && *trans_b != 1) || *alpha != 1.0f) {
    return false;
  }

  const auto& inputs = node.InputDefs();
  const bool has_bias = inputs.size() > 2 && inputs[2] != nullptr && inputs[2]->Exists();
  if (has_bias && *beta != 0.0f) {
    ...
    const bool is_vector_bias = bias_shape->dim_size() == 1 && DimValue(bias_shape, 0) == shape.N;
    const bool is_matrix_bias =
        bias_shape->dim_size() == 2 &&
        ((DimValue(bias_shape, 0) == 1 && DimValue(bias_shape, 1) == shape.N) ||
         (DimValue(bias_shape, 0) == shape.N && DimValue(bias_shape, 1) == 1));
    if (!is_vector_bias && !is_matrix_bias) {
      return false;
    }
  }

  const auto rhs_transpose = *trans_b == 0 ? CblasNoTrans : CblasTrans;
  return (rhs_transpose == CblasNoTrans && MlasFp16AccelerationSupported()) ||
         MlasHalfGemmNativePackBSize(CblasNoTrans, rhs_transpose,
...
static bool ShouldKeepNativeCpuFp16ForMatMulOrGemm(
  ...
  return shape->M <= kMaxNativeFp16GemvM &&
         nk >= kMinNativeFp16NK &&
         CanUseAcceleratedCpuFp16Gemm(node, *shape, mlas_backend_kernel_selector_config);
}
...
Status InsertCastTransformer::ApplyImpl(onnxruntime::Graph& graph, bool& modified, int graph_level,
  ...
  for (onnxruntime::NodeIndex i : order) {
    ...
    // A CPU-assigned fallback node may have fp16 only on its output. In that case NeedInsertCast
    // never fires, but the node still needs its dtype/output converted and the CPU EP reassigned.
    const bool force_output_only_conversion =
        !casted && forced_fp32_nodes.find(node->Index()) != forced_fp32_nodes.end();
