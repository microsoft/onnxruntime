  ...
  switch (level) {
    ...
    case TransformerLevel::Level3: {
      ...
      if (MlasNchwcGetBlockSize() > 1) {
        const bool enable_nchwc_pointwise_activation_fusion =
            session_options.config_options.GetConfigOrDefault(
                kOrtSessionOptionsDisableNchwcPointwiseActivationFusion, "0") != "1";
        transformers.emplace_back(std::make_unique<NchwcTransformer>(enable_nchwc_pointwise_activation_fusion));
      }
