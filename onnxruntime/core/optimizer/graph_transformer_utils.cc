...
InlinedVector<std::unique_ptr<GraphTransformer>> GenerateTransformers(
  ...
  switch (level) {
    ...
    case TransformerLevel::Level2: {
      ...
      transformers.emplace_back(std::make_unique<GeluFusion>(cpu_acl_cuda_dml_rocm_eps));
      transformers.emplace_back(std::make_unique<LayerNormFusion>(cpu_acl_cuda_dml_rocm_eps));
