...
void NchwcTransformerImpl::TransformConv(Node& node) {
  ...
  if ((conv_B_tensor_proto != nullptr) && (output_channels != nchwc_output_channels)) {
    ...
    if (biases_it != aligned_biases_.end()) {
      ...
      std::vector<float> aligned_bias(static_cast<size_t>(nchwc_output_channels));
      ...
      nchwc_conv_B_tensor_proto.set_raw_data(aligned_bias.data(), static_cast<size_t>(nchwc_output_channels) * sizeof(float));
...
void NchwcTransformerImpl::TransformBatchNormalization(Node& node) {
  ...
  std::vector<float> padded_buffer(static_cast<size_t>(nchwc_channels));
  ...
  nchwc_conv_W_tensor_proto.set_raw_data(padded_buffer.data(), static_cast<size_t>(nchwc_channels) * sizeof(float));
  ...
  nchwc_conv_B_tensor_proto.set_raw_data(padded_buffer.data(), static_cast<size_t>(nchwc_channels) * sizeof(float));
