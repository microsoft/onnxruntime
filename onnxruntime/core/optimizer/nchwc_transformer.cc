...
void NchwcTransformerImpl::TransformConv(Node& node) {
  ...
  if (filters_it != filters_map->end()) {
    ...
    utils::SetRawDataInTensorProto(nchwc_conv_W_tensor_proto,reordered_filter.data(), reordered_filter.size() * sizeof(float));
  ...
  if ((conv_B_tensor_proto != nullptr) && (output_channels != nchwc_output_channels)) {
    ...
    if (biases_it != aligned_biases_.end()) {
      ...
      utils::SetRawDataInTensorProto(nchwc_conv_B_tensor_proto,aligned_bias.data(), gsl::narrow<size_t>(nchwc_output_channels) * sizeof(float));
...
void NchwcTransformerImpl::TransformBatchNormalization(Node& node) {
  ...
  utils::SetRawDataInTensorProto(nchwc_conv_W_tensor_proto,padded_buffer.data(), gsl::narrow<size_t>(nchwc_channels) * sizeof(float));
