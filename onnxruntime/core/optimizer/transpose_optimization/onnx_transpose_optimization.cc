...
static std::vector<int64_t> DataInt64(onnxruntime::interface::TensorRef& tensor) {
...
static std::vector<int32_t> DataInt32(onnxruntime::interface::TensorRef& tensor) {
  std::vector<uint8_t> raw_data = tensor.Data();
...
static void UnsqueezeInput(OptimizerCtx& ctx, api::NodeRef& node, size_t i, const std::vector<int64_t>& axes) {
  ...
  std::unique_ptr<onnxruntime::interface::TensorRef> constant = ctx.graph.GetLocalConstant(input);
...
static void Permute1DConstant(api::GraphRef& graph, api::NodeRef& node, onnxruntime::interface::TensorRef& constant,
...
static void TransposeInputImpl(api::GraphRef& graph,
  ...
  std::unique_ptr<onnxruntime::interface::TensorRef> constant = graph.GetLocalConstant(input);
...
static bool IsConstant(const api::GraphRef& graph, const api::NodeRef& node,
  ...
  if (!producer_node) {
    ...
    std::unique_ptr<onnxruntime::interface::TensorRef> constant = graph.GetConstant(value_name);
...
bool HandleReduceOps(HandlerArgs& args) {
  ...
  std::unique_ptr<onnxruntime::interface::TensorRef> axes_const = nullptr;
...
static std::vector<int64_t> TensorIntData(onnxruntime::interface::TensorRef& tensor, onnxruntime::DataType dtype) {
  if (dtype == onnxruntime::DataType::INT32) {
...
static bool HandleSlice(HandlerArgs& args) {
  ...
  if (inputs.size() < 4 || inputs[3] == "") {
    ...
    std::unique_ptr<onnxruntime::interface::TensorRef> axes_const = args.ctx.graph.GetConstant(axes_inp);
    if (axes_const == nullptr) {
      return false;
    }
...
static bool HandleTile(HandlerArgs& args) {
  ...
  std::unique_ptr<onnxruntime::interface::TensorRef> repeats_const = args.ctx.graph.GetConstant(repeats_inp);
