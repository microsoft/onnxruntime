...
/**
 * @brief Share initializer for those who hold same values and shapes.
 *
 * @tparam T The initializer value type, used to retrieved data values.
 * @param graph Target graph to iterate.
 * @param node Target node to check's initializer input.
 * @param input_index Input index of target node.
 * @param type_value_plus_rank_to_shared_arg_map Accumulated value/rank to initializer map.
...
bool ConstantSharing::ShareInitializer(Graph& graph, Node* node, int input_index,
  ...
  if (!optimizer_utils::AppendTensorFromInitializer(graph, *input_def, values, require_constant) || values.size() != 1) {
    return false;
  }

  int rank = input_def->Shape()->dim_size();
  std::tuple<int32_t, int64_t, int> p = std::make_tuple(data_type, values[0], rank);
...
Status ConstantSharing::ApplyImpl(Graph& graph, bool& modified, int graph_level, const logging::Logger& logger) const {
  ...
  std::map<std::tuple<int32_t, int64_t, int>, NodeArg*> value_plus_rank_to_shared_arg_map;
  ...
  for (NodeIndex i : order) {
    ...
    /**
     * Loop all inputs of Node node, find all constant initializers,
     * If it's data type is int32_t/int64_t or float/MLFloat16, and it contains single value,
     * Then we can share same initializer.
     */
