...
Status LayerNormFusion::ApplyImpl(Graph& graph, bool& modified, int graph_level, const logging::Logger& logger) const {
  ...
  for (auto node_index : node_topology_list) {
    ...
    const std::array<NodeArg*, 3> layer_norm_input_defs{reduce_mean_node.MutableInputDefs()[0], scale, bias};
  ...
  for (auto node_index : node_topology_list) {
    ...
    if (allow_precision_change_ && cast_1_present) {
      ...
      if (scale->Type() != nullptr &&
        ...
        const std::array<NodeArg*, 1> input_defs = {scale};
        const std::array<NodeArg*, 1> output_defs = {casted_scale};
