  ...
  if (utils::HasExternalData(tensor_proto)) {
    ...
    if (graph.GetOrtValueInitializer(name_, ort_value, check_outer_scope)) {
      ...
      if (check_outer_scope) {
        ...
        ORT_ENFORCE(src_tensor.Shape().GetDims() == tensor_shape_from_proto.GetDims(),
                    "Tensor shape mismatch. Expected: ", tensor_shape_from_proto.GetDims(),
                    " Found: ", src_tensor.Shape().GetDims());
      }
