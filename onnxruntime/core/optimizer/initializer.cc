...
static Status CreateTensorFromProto(const ONNX_NAMESPACE::TensorProto& tensor_proto,
                                    const std::filesystem::path& model_path, Tensor& tensor) {
...
Initializer::Initializer(const ONNX_NAMESPACE::TensorProto& tensor_proto, const std::filesystem::path& model_path) {
  ...
  Tensor tensor;
  ORT_THROW_IF_ERROR(CreateTensorFromProto(tensor_proto, model_path, tensor));
  Tensor::InitOrtValue(std::move(tensor), ort_value_);
  data_ = GetTensor(ort_value_);
}
...
Initializer::Initializer(const Graph& graph, const ONNX_NAMESPACE::TensorProto& tensor_proto,
  ...
  if (utils::HasExternalData(tensor_proto)) {
    if (graph.GetOrtValueInitializer(name_, ort_value_)) {
      data_ = GetTensor(ort_value_);
      return;
    }
#if !defined(__wasm__)
