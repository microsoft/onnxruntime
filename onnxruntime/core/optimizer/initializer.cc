...
template <typename T>
struct ScaleByAxis {
  void operator()(Tensor& data, const Tensor& scalers, const size_t block_size, const size_t num_blocks, const bool columnMajor) const {
    ToNumeric<T> to_numeric;
...
void Initializer::scale_by_axis(const Initializer& scalers, int axis, bool columnMajor) {
  ...
  ORT_ENFORCE(scalers.size() == 1 || 
             (columnMajor ? scalers.size() == block_size : scalers.size() == num_blocks), "Invalid other(scalers) size");
  utils::MLTypeCallDispatcher<MLFloat16, BFloat16, float, double, int32_t, int64_t> t_disp(data_.GetElementType());
  t_disp.Invoke<ScaleByAxis>(data_, scalers.data_, block_size, num_blocks, columnMajor);
}
