...
Status EmbedLayerNormFusion::ApplyImpl(Graph& graph, bool& modified, int graph_level, const logging::Logger& logger) const {
  ...
  for (auto node_index : node_topology_list) {
    ...
    //nodes_to_remove.push_back(skip_ln_node);
    ...
    //nodes_to_remove.push_back(segment_gather_node);
    ...
    nodes_to_remove.push_back(word_gather_node);*/
    ...
    if (position_gather_node.GetOutputEdgesCount() != 1 || expand_node.GetOutputEdgesCount() != 1 || shape_node.GetOutputEdgesCount() != 1) {
      continue;
    }
    std::cout << "Position Gather " << std::endl;

    /*nodes_to_remove.push_back(position_gather_node);
    nodes_to_remove.push_back(expand_node);
    nodes_to_remove.push_back(shape_node);*/
    ...
    // Check if the input of "Shape" node and the input of word embedding "Gather" node are the same.
    if (!graph_utils::NodeArgIsConstant(graph, *(shape_node.MutableInputDefs()[0])) &&
        !graph_utils::IsGraphInput(graph, shape_node.MutableInputDefs()[0])) {
      continue;
    }
    if (input_ids != shape_node.MutableInputDefs()[0]) {
      continue;
    }
    std::cout << "Word Gather input " << std::endl;
    ...
    //graph_utils::FinalizeNodeFusion(graph, nodes_to_remove, embed_layer_norm_node);
