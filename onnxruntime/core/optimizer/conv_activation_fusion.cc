...
Status ConvActivationFusion::ApplyImpl(Graph& graph, bool& modified, int graph_level, const logging::Logger& logger) const {
  ...
  for (auto index : order) {
    ...
    if (node->GetExecutionProviderType() == onnxruntime::kCudaExecutionProvider) {
      ...
      if (graph_utils::IsSupportedOptypeVersionAndDomain(next_node, "Sigmoid", {6, 13}) ||
          graph_utils::IsSupportedOptypeVersionAndDomain(next_node, "Relu", {6, 13}) ||
          graph_utils::IsSupportedOptypeVersionAndDomain(next_node, "Tanh", {6, 13})) {
        std::string node_name = conv_node.Name() + "_" + act_node.OpType();
        Node& act_conv = graph.AddNode(node_name,
                                         "Conv",
                                         node_name,
                                         ...
                                         "");
        act_conv.SetExecutionProviderType(conv_node.GetExecutionProviderType());
#ifdef _WIN32
        //std::cout << "setenv: " << node_name.c_str() << " = " << act_node.OpType() << std::endl;
        SetEnvironmentVariableA(node_name.c_str(), act_node.OpType().c_str());
#else
        setenv(node_name.c_str(), std::to_string(mode).c_str(), 1);
#endif
        graph_utils::FinalizeNodeFusion(graph, {conv_node, act_node}, act_conv);
        modified = true;
      }
    } else {
