...
bool IsQDQPairSupported(const Graph& graph, const Node& q_node, const Node& dq_node) {
  ...
  if (dq_input_defs.size() != QDQInputIndex::Count ||
      q_input_defs.size() != QDQInputIndex::Count ||
      !optimizer_utils::IsScalar(*q_input_defs[QDQInputIndex::Scale]) ||
      !optimizer_utils::IsScalar(*q_input_defs[QDQInputIndex::ZeroPoint]) ||
      !optimizer_utils::IsScalar(*dq_input_defs[QDQInputIndex::Scale]) ||
      !optimizer_utils::IsScalar(*dq_input_defs[QDQInputIndex::ZeroPoint])) {
    return false;
  }

  // if Q/DQ scale and zero point are not constant, return false
  const ONNX_NAMESPACE::TensorProto* dq_scale_tensor_proto =
      graph_utils::GetConstantInitializer(graph, dq_input_defs[QDQInputIndex::Scale]->Name());
  const ONNX_NAMESPACE::TensorProto* q_scale_tensor_proto =
      graph_utils::GetConstantInitializer(graph, q_input_defs[QDQInputIndex::Scale]->Name());
  const ONNX_NAMESPACE::TensorProto* dq_zp_tensor_proto =
      graph_utils::GetConstantInitializer(graph, dq_input_defs[QDQInputIndex::ZeroPoint]->Name());
  const ONNX_NAMESPACE::TensorProto* q_zp_tensor_proto =
      graph_utils::GetConstantInitializer(graph, q_input_defs[QDQInputIndex::ZeroPoint]->Name());
