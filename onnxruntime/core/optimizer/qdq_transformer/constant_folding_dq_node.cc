...
ConstantFoldingDQ::ConstantFoldingDQ(const IExecutionProvider& execution_provider,
    ...
    : ConstantFolding(execution_provider,skip_dequantize_linear, config_options, compatible_execution_providers, excluded_initializers),
      node_index_in_compute_capability_(node_index_in_compute_capability),
...
bool ConstantFoldingDQ::AllowConstantFolding(const Node& node) const {
  if (node_index_in_compute_capability_.find(node.Index()) != node_index_in_compute_capability_.end()) {
    return true;
  }
  return false;
}

Status ConstantFoldingDQ::ApplyImpl(Graph& graph, bool& modified, int graph_level, const logging::Logger& logger) const {
