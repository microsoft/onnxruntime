...
Status DuplicateDQForOutputEdge(const graph_utils::GraphEdge& original_dq_output_edge, const std::string& dq_domain,
                                Graph& graph) {
  // DQ
...
Status EnsureUniqueDQForEachExplicitOutputEdge(const Node& node, Graph& graph, bool& modified) {
  ...
  for (; edge_to_process != dq_output_edges.end(); ++edge_to_process) {
    ORT_RETURN_IF_ERROR(DuplicateDQForOutputEdge(*edge_to_process, dq_node.Domain(), graph));
    modified = true;
  }
