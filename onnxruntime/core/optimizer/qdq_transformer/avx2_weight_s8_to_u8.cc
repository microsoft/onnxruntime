...
static bool TryConvertDynamicQuantizeLSTM(Node& op_node, Graph& graph) {
  ...
  {
    int8_t* p = w_temp.MutableDataAsSpan<int8_t>().data();
  ...
  {
    int8_t* p = r_temp.MutableDataAsSpan<int8_t>().data();
  ...
  weights_proto_u8.set_raw_data(w_temp.MutableDataAsSpan<int8_t>().data(), w_temp.size());
  ...
  r_proto_u8.set_raw_data(r_temp.MutableDataAsSpan<int8_t>().data(), r_temp.size());
