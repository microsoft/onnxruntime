...
Status DQMatMulToMatMulNBitsAction::ProcessNewNode(Graph& graph,
  ...
  input_defs.push_back(&graph_utils::AddInitializer(graph, weight_T_tp, std::move(weight_dst)));
  replacement_node.MutableInputArgsCount().push_back(1);

  input_defs.push_back(&graph_utils::AddInitializer(graph, scale_T_tp, std::move(scale_dst)));
  replacement_node.MutableInputArgsCount().push_back(1);

  if (zp_T_tp) {
    input_defs.push_back(&graph_utils::AddInitializer(graph, zp_T_tp.value(), std::move(*zp_dst)));
    replacement_node.MutableInputArgsCount().push_back(1);
  }
