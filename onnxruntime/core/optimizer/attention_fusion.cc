...
static bool FuseSubGraphQKImpl(Node& layer_norm,
  ...
  const InlinedVector<NodeArg*> input_defs{layer_norm.MutableOutputDefs()[0], &qkv_weights, &qkv_bias, mask_int32};
  const InlinedVector<NodeArg*> output_defs{graph.GetNode(reshape.Index())->MutableOutputDefs()[0]};
