...
class GraphTransformerManagerWithCosts : public GraphTransformerManager {
 public:
  explicit GraphTransformerManagerWithCosts(unsigned steps) : GraphTransformerManager(steps) {
  }
  ...
  // Apply transformers registered for the given level using a cost model
  common::Status ApplyTransformers(Graph& graph, TransformerLevel level, const logging::Logger& logger) const;

 private:
  ORT_DISALLOW_COPY_ASSIGNMENT_AND_MOVE(GraphTransformerManagerWithCosts);
