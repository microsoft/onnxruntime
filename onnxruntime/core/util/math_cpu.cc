...
#define DELEGATE_SIMPLE_UNARY_FUNCTION(T, Funcname, expr)                     \
  template <>                                                                 \
  void Funcname<T, CPUMathUtil>(uint64_t N, const T* x, T* y, CPUMathUtil*) { \
...
#define EIGEN_SIMPLE_BINARY_FUNCTION(T, Funcname, expr)                                                       \
  template <>                                                                                                 \
  void Funcname<T, CPUMathUtil>(uint64_t N, const T* a, const T* b, T* y, CPUMathUtil*) {                     \
...
#define SPECIALIZED_SUM(T)                                                            \
  template <>                                                                         \
  void Sum<T, CPUMathUtil>(uint64_t N, const T* x, T* y, CPUMathUtil* /* unused */) { \
...
#define SPECIALIZED_SCALE(T)                                                                                \
  template <>                                                                                               \
  void Scale<T, CPUMathUtil>(uint64_t n, float alpha, const T* x, T* y, CPUMathUtil* /*provider*/) {        \
    EigenVectorMap<T>(y, n) = ConstEigenVectorMap<T>(x, n) * alpha;                                         \
  }                                                                                                         \
  template <>                                                                                               \
  void Scale<T, CPUMathUtil>(uint64_t n, const float* alpha, const T* x, T* y, CPUMathUtil* /*provider*/) { \
    EigenVectorMap<T>(y, n) = ConstEigenVectorMap<T>(x, n) * (*alpha);                                      \
  }
