...
ModelCompilationOptions::ModelCompilationOptions(const OrtEnv& env, const OrtSessionOptions& session_options)
  ...
  ORT_ENFORCE(session_options_.value.config_options.AddConfigEntry(kOrtSessionOptionsDisableEpCompile, "0").IsOK());
}
...
Status ModelCompilationOptions::Check() const {
  ...
  ORT_ENFORCE(session_options_.value.config_options.GetConfigOrDefault(kOrtSessionOptionsDisableEpCompile, "0") == "0");
