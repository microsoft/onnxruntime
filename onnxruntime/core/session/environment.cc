...
void Environment::InsertPluginEpFactory(const char* ep_name, OrtExecutionProviderFactory* ep_factory) {
  ...
  plugin_ep_factories_.insert({ep_name, std::move(p)});  // TODO(leca): review
}

OrtExecutionProviderFactory* Environment::GetPluginExecutionProviderFactory(const std::string& ep_name) {
  std::unordered_map<std::string, std::unique_ptr<OrtExecutionProviderFactory>>::const_iterator it = plugin_ep_factories_.find(ep_name);
  if (it == plugin_ep_factories_.end()) return nullptr;
