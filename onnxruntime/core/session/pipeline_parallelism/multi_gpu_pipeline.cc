...
RequestExecutionFrame::RequestExecutionFrame(PipelineSession& psess,  // passing by non-const exec_frame to create iobinding
  ...
  for (const auto& mcfg : psess.model_ensemble_cfg.model_config_vec) {
    ...
    auto past_present_state_shape = mcfg.past_present_state_shape;
    if (!mcfg.past_input_names.empty() && !mcfg.present_output_names.empty()) {
      ...
      auto num_elements = gsl::narrow_cast<int>(std::accumulate(
          std::begin(past_present_state_shape),
          std::end(past_present_state_shape), static_cast<int64_t>(1), std::multiplies<int64_t>()));
      size_t size_to_allocate = GetTypeSize(mcfg.past_present_state_type) * num_elements;
      ...
      for (size_t j = 0, end = mcfg.present_output_names.size(); j < end; ++j) {
        ...
        auto ort_val = Ort::Value::CreateTensor(
            ...
            past_present_state_shape.data(), past_present_state_shape.size(), mcfg.past_present_state_type);
...
static Token* ExecuteRequest(const ModelEnsembleConfig::ModelConfig& mcfg,
  ...
  if (!mcfg.present_output_names.empty()) {
    ...
    past_present_state_shape = mcfg.past_present_state_shape;
    ...
    past_present_type = mcfg.past_present_state_type;
  }
...
void PipelineSession::Init(const OrtEnv& env, ModelEnsembleConfig& model_ensemble_cfg) {
  ...
  for (auto& mcfg : model_ensemble_cfg.model_config_vec) {
    ...
    // session_options.DisablePerSessionThreads(); // TODO make this configurable
    ...
    // cache present/past shape and type
    auto rc = Contains(mcfg.input_names, mcfg.past_input_names[0]);
    ORT_ENFORCE(rc.second, mcfg.past_input_names[0], " must be a valid input");
    auto io_idx = rc.second;
    GetShapeAndType(session, io_idx, true, mcfg.past_present_state_type, mcfg.past_present_state_shape);
