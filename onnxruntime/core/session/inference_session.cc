...
static void ModelUseFP16Helper(const onnx::TypeProto type_proto, bool& use_fp16) {
  switch (type_proto.value_case()) {
    ...
    case ::onnx::TypeProto::ValueCase::kSequenceType: {
      if (type_proto.has_sequence_type()) {
        ...
        ModelUseFP16Helper(sequence_type.elem_type(), use_fp16);
      }
    } break;
    case ::onnx::TypeProto::ValueCase::kMapType: {
      if (type_proto.has_map_type()) {
        auto& map_type = type_proto.map_type();
        ModelUseFP16Helper(map_type.value_type(), use_fp16);
      }
    } break;
  }
}
...
static bool ModelUseFP16(const onnx::ModelProto& model_proto) {
  ...
  for (auto& input : inputs) {
    if (input.has_name() && input.has_type()) {
      auto& input_type_proto = input.type();
      ModelUseFP16Helper(input_type_proto, use_fp16);
      if (use_fp16) {
        break;
      }
    }
