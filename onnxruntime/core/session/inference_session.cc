...
common::Status InferenceSession::TransformGraph(onnxruntime::Graph& graph, bool saving_model_in_ort_format) {
  ...
  // get graph optimizations loop level from session config
  ...
  for (int steps = 0; steps < 10; steps++) {
    ...
    if (steps >= 3) {
      LOGS(*session_logger_, WARNING) << "Running graph optimizations in loop " << (steps + 1) << " times."
    ...
    // Re-Run the Level2+ optimizations. The idea behind re-runing Level2 and Level3 graph transforms is that,
