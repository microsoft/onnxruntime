...
struct ProviderHostImpl : ProviderHost {
  ...
  void* Allocator__AllocateBufferWithOptions(std::shared_ptr<IAllocator>& allocator, size_t size, bool use_reserve, Stream* stream) override { return AllocateBufferWithOptions(allocator, size, use_reserve, stream); }
