...
ORT_API_STATUS_IMPL(OrtApis::Node_GetNumAttributes, _In_ const OrtNode* node, _Out_ size_t* num_attrs) {
  ...
  *num_attrs = 0;
  ORT_API_RETURN_IF_STATUS_NOT_OK(node->GetNumImplicitInputs(*num_attrs));
  return nullptr;
  API_IMPL_END
}
...
ORT_API_STATUS_IMPL(OrtApis::Node_GetAttributes, _In_ const OrtNode* node,
  ...
  onnxruntime::InlinedVector<const OrtOpAttr*> attributes;
  ORT_API_RETURN_IF_STATUS_NOT_OK(node->GetAttributes(attributes));
  size_t num_attrs = std::min(max_num_attrs, attributes.size());
