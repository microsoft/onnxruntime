...
ORT_API_STATUS_IMPL(OrtApis::LogMessage, OrtLoggingLevel log_severity_level, _In_z_ const char* message,
...
ORT_API_STATUS_IMPL(OrtApis::GetLoggingSeverityLevel, _Out_ OrtLoggingLevel* out) {
  API_IMPL_BEGIN
  const auto& logger = onnxruntime::logging::LoggingManager::DefaultLogger();  // Throws if no default logger exists.
