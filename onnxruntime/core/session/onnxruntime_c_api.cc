...
ORT_API_STATUS_IMPL(OrtApis::GetCustomOpDomainsCount, _Inout_ OrtSessionOptions* options, _Out_ size_t* out) {
  API_IMPL_BEGIN
  *out = options->custom_op_domains_.size();
  return nullptr;
  API_IMPL_END
}

ORT_API_STATUS_IMPL(OrtApis::GetCustomOpDomain, _Inout_ OrtSessionOptions* options, size_t index, _Outptr_ OrtCustomOpDomain** custom_op_domain) {
  API_IMPL_BEGIN
  if (index < options->custom_op_domains_.size()) {
    *custom_op_domain = options->custom_op_domains_[index];
  } else {
...
static constexpr OrtApi ort_api_1_to_4 = {
    ...
    &OrtApis::TensorAt,
    &OrtApis::GetCustomOpDomainsCount,
    &OrtApis::GetCustomOpDomain};
