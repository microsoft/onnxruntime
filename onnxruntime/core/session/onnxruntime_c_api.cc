...
The following section contains stubs for each function to add an EP to the session options. 
There are two ways this addition currently happens:
...
2) Via an entry in the OrtApis struct with a name SessionOptionsAppendExecutionProvider_<EP>

When an EP moves to being an external library the provider bridge will have the implementation.
For other EPs we implement the stub here.

The function of style #1 is easier to use from places like the C# and Java bindings as it do not use
structs for passing the options.

When adding a new EP that is not behind the provider bridge you need to:
1) Add the ORT_API_STATUS entry for the function if EP not enabled in build
2) Add the ORT_API_STATUS_IMPL implementation if EP not enabled in build
3) add the exported symbol name for the stub to the symbols.txt file in the onnxruntime/core/session directory.

*/

/*
OrtAppendExecutionProvider_<EP> stubs for EPs not included in this build
*/
...
2) EP is build as separate library and uses the provider bridge. Need to include in a minimal build 
