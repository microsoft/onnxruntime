...
struct OrtEnv : public onnxruntime::ObjectBase<OrtEnv> {
  ...
  */
  ...
  ONNXRUNTIME_DISALLOW_COPY_AND_ASSIGNMENT(OrtEnv);
};
...
ORT_API_STATUS_IMPL(OrtGetStringTensorDataLength, _In_ const ONNXValue* value, _Out_ size_t* out) {
  ...
  if (len >= 0) {
    ...
    *out = ret;
    ...
    return CreateONNXStatus(ORT_INVALID_ARGUMENT, "shape is invalid");
  return nullptr;
  API_IMPL_END
}

ORT_API_STATUS_IMPL(OrtFillStringTensor, _In_ ONNXValue* value, _In_ const char* const* s, size_t s_len) {
  TENSOR_READWRITE_API_BEGIN
  auto* dst = tensor->MutableData<std::string>();
  auto len = static_cast<size_t>(tensor->Shape().Size());
  if (s_len < len) {
    return CreateONNXStatus(ORT_INVALID_ARGUMENT, "input array is too short");
  }
...
template <typename T>
ONNXStatus* CreateTensorImpl(const size_t* shape, size_t shape_len, OrtAllocatorInterface** allocator,
  ...
  if (!IAllocator::CalcMemSizeForArray(sizeof(T), elem_count, &size_to_allocate)) {
    return CreateONNXStatus(ORT_FAIL, "not enough memory");
  }
  void* p_data = (*allocator)->Alloc(allocator, size_to_allocate);
  if (p_data == nullptr)
    return CreateONNXStatus(ORT_FAIL, "size overflow");
 ...
 */
template <typename T>
ONNXStatus* CreateTensorImpl(const size_t* shape, size_t shape_len, const OrtAllocatorInfo* info,
  ...
  size_t size_to_allocate;
  if (!IAllocator::CalcMemSizeForArray(sizeof(T), elem_count, &size_to_allocate)) {
    return CreateONNXStatus(ORT_INVALID_ARGUMENT, "size overflow");
  }
  if (size_to_allocate > p_data_len) {
    std::ostringstream oss;
    oss << "not enough space: expected " << size_to_allocate << ", got " << p_data_len;
    return CreateONNXStatus(ORT_INVALID_ARGUMENT, oss.str().c_str());
  }
  *out = std::make_unique<Tensor>(DataTypeImpl::GetType<T>(),
                                  onnxruntime::TensorShape(shapes),
                                  p_data,
 ...
 * this function will create a copy of the allocator info
 */
ORT_API_STATUS_IMPL(OrtCreateTensorWithDataAsONNXValue, _In_ const OrtAllocatorInfo* info,
  ...
  switch (type) {
    ...
    default: {
      ...
      return CreateONNXStatus(ORT_NOT_IMPLEMENTED, errmsg.c_str());
    }
  }
...
ORT_API_STATUS_IMPL(OrtCreateTensorAsONNXValue, _Inout_ OrtAllocator* allocator,
...
template <typename T>
static ONNXStatus* CreateInferenceSessionImpl(_In_ OrtEnv* env, _In_ T model_path,
...
ORT_API_STATUS_IMPL(OrtRunInference, _In_ ONNXSession* sess,
  ...
  for (size_t i = 0; i != input_len; ++i) {
    ...
    if (!kvp.second) {
      return CreateONNXStatus(ORT_INVALID_ARGUMENT, "duplicated input name");
    }
  ...
  for (size_t i = 0; i != output_names_len; ++i) {
    if (output_names1[i] == nullptr || output_names1[i][0] == '\0') {
      return CreateONNXStatus(ORT_INVALID_ARGUMENT, "output name cannot be empty");
    }
...
ORT_API_STATUS_IMPL(OrtGetTensorMutableData, _In_ ONNXValue* value, _Out_ void** output) {
  TENSOR_READWRITE_API_BEGIN
...
ORT_API_STATUS_IMPL(OrtGetStringTensorContent, _In_ const ONNXValue* value,
                    _Out_ void* s, size_t s_len, _Out_ size_t* offsets, size_t offsets_len) {
  TENSOR_READ_API_BEGIN
  const auto* input = tensor.Data<std::string>();
  auto len = static_cast<size_t>(tensor.Shape().Size());
  if (offsets_len < len) {
    return CreateONNXStatus(ORT_FAIL, "space is not enough");
  }
  {
    size_t ret = 0;
    for (size_t i = 0; i != len; ++i) {
...
ORT_API_STATUS_IMPL(OrtTensorProtoToONNXValue, _Inout_ OrtAllocator* allocator,
  ...
  if (!proto.ParseFromArray(input, input_len)) {
    return CreateONNXStatus(ORT_FAIL, "parse input tensor proto failed");
  }
  std::unique_ptr<MLValue> value = std::make_unique<MLValue>();
...
#define DEFINE_RELEASE_ONNX_RUNTIME_OBJECT_FUNCTION(INPUT_TYPE, REAL_TYPE) \
  ORT_API(void, Release##INPUT_TYPE, INPUT_TYPE* value) {                  \
...
ORT_API_STATUS_IMPL(OrtInferenceSessionGetInputCount, _In_ const ONNXSession* sess, _Out_ size_t* out) {
  API_IMPL_BEGIN
...
ORT_API_STATUS_IMPL(OrtInferenceSessionGetOutputCount, _In_ const ONNXSession* sess, _Out_ size_t* out) {
  API_IMPL_BEGIN
  auto session = reinterpret_cast<const ::onnxruntime::InferenceSession*>(sess);
...
ORT_API_STATUS_IMPL(OrtInferenceSessionGetInputTypeInfo, _In_ const ONNXSession* sess, size_t index, _Out_ struct OrtTypeInfo** out) {
  API_IMPL_BEGIN
  auto session = reinterpret_cast<const ::onnxruntime::InferenceSession*>(sess);
  std::pair<Status, const InputDefList*> p = session->GetModelInputs();
  if (!p.first.IsOK())
  ...
  if (p.second->size() <= index)
    return CreateONNXStatus(ORT_FAIL, "out of index");
  const ONNX_NAMESPACE::TypeProto* type_proto = (*p.second)[index]->TypeAsProto();
  return OrtTypeInfo::FromDataTypeImpl(type_proto, out);
  API_IMPL_END
}
ORT_API_STATUS_IMPL(OrtInferenceSessionGetOutputTypeInfo, _In_ const ONNXSession* sess, size_t index, _Out_ struct OrtTypeInfo** out) {
  API_IMPL_BEGIN
  auto session = reinterpret_cast<const ::onnxruntime::InferenceSession*>(sess);
  std::pair<Status, const InputDefList*> p = session->GetModelOutputs();
  if (!p.first.IsOK())
...
static ONNXStatus* GetInputOutputNameImpl(_In_ const ONNXSession* sess, size_t index,
  ...
  if (!p.first.IsOK())
    return ToONNXStatus(p.first);
  if (p.second == nullptr)
    return CreateONNXStatus(ORT_FAIL, "internal error");
  const InputDefList& defs = *p.second;
  if (index >= defs.size())
    return CreateONNXStatus(ORT_FAIL, "index out of range");
...
ORT_API_STATUS_IMPL(OrtInferenceSessionGetInputName, _In_ const ONNXSession* sess, size_t index,
...
ORT_API_STATUS_IMPL(OrtInferenceSessionGetOutputName, _In_ const ONNXSession* sess, size_t index,
                    _Inout_ OrtAllocator* allocator, _Out_ char** output) {
  API_IMPL_BEGIN
