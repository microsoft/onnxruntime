...
AllocatorPtr IExecutionProvider::GetAllocator(int device_id, OrtMemType mem_type) const {
  ORT_ENFORCE(mem_type != OrtMemType::OrtMemTypeDefault || device_id == GetDeviceId(), "Rule out the case that search on OrtMemTypeDefault but device_id doesn't match GetDeviceId()");
