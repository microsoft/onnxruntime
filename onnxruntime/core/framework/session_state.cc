    ...
    if (node_provider.empty()) {
      ...
      if (node.OpType() == "EPContext") {
        ...
        return ORT_MAKE_STATUS(ONNXRUNTIME, NOT_IMPLEMENTED,
                               "EPContext node generated by '", source, "' was not ",
                               "assigned to any execution provider in the session. ",
                               "Please add a compatible execution provider to the session. ",
                               "EPContext node name: '", node.Name(), "'. Session execution providers: [",
