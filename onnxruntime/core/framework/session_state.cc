...
static Status KernelStorePrePackedWeight(OpKernel* kernel, int input_idx,
                                         const PrePackedWeights& prepacked_weight,
                                         const std::string& node_name) {
  bool stored_weight = false;
  ORT_RETURN_IF_ERROR(kernel->StorePrePackedWeight(prepacked_weight, input_idx, stored_weight));
            ...
            if (st->GetOrtValueNameIdxMap().GetIdx(input_name, ort_value_idx).IsOK()) {
              ...
              if (constant_initialized_tensors.count(ort_value_idx)) {
                ...
                if (is_shared_initializer && should_cache_prepacked_weights_for_shared_initializers &&
                  ...
                  // cached by another instance of the same op_type (for the ame constant initializer) is because
                  // to truly know if we can use a cached pre-packed weight, we would hav to compare the cached pre-packed
                  ...
                  if (is_packed) {
                    ...
                    ORT_ENFORCE(weights_to_be_filled_in.is_filled_, "The kernel corresponding to the node ", node.Name(),
                    ...
                    bool container_contains_packed_weight = prepacked_weights_container_->HasCachedWeight(prepacked_weights_container_key);
                    ...
                    if (container_contains_packed_weight) {
                      ...
                      ORT_RETURN_IF_ERROR(KernelStorePrePackedWeight(kernel, input_idx,
                                                                     prepacked_weights_container_->GetCachedWeight(prepacked_weights_container_key),
                                                                     node.Name()));

                      ++used_cached_pre_packed_weights_counter_;
                    } else {  // container doesn't contain the pre-packed weight - so write into it

                      // The cache has (atleast one) pre-packed weight corresponding to this op_type + shared initializer combo,
                      // but the pre-packed weight computed using this instance of the op_type and the same shared initializer has different contents
                      // than what exists in the cache. This is possible when the contents of the pre-packed buffer is a function of other
                      ...
                      prepacked_weights_container_->WriteCachedWeight(prepacked_weights_container_key, std::move(weights_to_be_filled_in));
                      prepacked_weights_container_->MarkHasPrepackedWeightForOpTypeAndConstantInitializer(op_type, const_initialized_tensor.DataRaw());
                      ORT_RETURN_IF_ERROR(KernelStorePrePackedWeight(kernel, input_idx,
                                                                     prepacked_weights_container_->GetCachedWeight(prepacked_weights_container_key),
                                                                     node.Name()));
                    }
