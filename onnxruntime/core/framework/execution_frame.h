...
#if !defined(ORT_MINIMAL_BUILD)
  // If the value indexed by `index` has a tensor partition spec, this function assigns it to `spec`
  // and returns true.
  // Otherwise, leaves `spec` untouched and returns false.
  // Default implementation always returns false since derived classes don't
  // always have tensor partition specs. Only distributed computation needs this.
  //
  // Arguments:
  //   index: name of a tensor.
  //   spec: holder of that tensor's partition spec.
  virtual bool TryGetPropagatedTensorPartitionSpec(const std::string& name, distributed::TensorPartitionSpec& spec) const;

  // When spec propagation doesn't exist in derived classes, this default function does nothing
  // and TryGetPropagatedTensorPartitionSpec always returns false.
  // If spec propagation exists, derived classes should override this function to
  // set the tensor partition spec for the value indexed by `index` so that
  // TryGetPropagatedTensorPartitionSpec returns true and the propagated specs.
  //
  // Inside a distributed operator, usually we
  //  1. call TryGetPropagatedTensorPartitionSpec for all input tensors,
  //  2. then compute the output tensor partition specs,
  //  3. and finally call SetPropagatedTensorPartitionSpec for all output tensors.
  //
  // Arguments:
  //   index: name of a tensor.
  //   spec: that tensor's partition spec to be assigned.
  virtual void SetPropagatedTensorPartitionSpec(const std::string& name, const distributed::TensorPartitionSpec& spec);

  // Get the planned tensor partition spec for the value indexed by `index`.
  // This spec is usually generated by external distributed compiler.
...
#if !defined(ORT_MINIMAL_BUILD)
  bool TryGetPropagatedTensorPartitionSpec(const std::string& name, distributed::TensorPartitionSpec& spec) const override;
  void SetPropagatedTensorPartitionSpec(const std::string& name, const distributed::TensorPartitionSpec& spec) override;
  bool TryGetPlannedTensorPartitionSpec(const std::string& name, distributed::TensorPartitionSpec& spec) const override;
#endif
...
#if !defined(ORT_MINIMAL_BUILD)
  // TODO: optimize the lookup and storage mechanism.
  //  1. use value index instead of string.
  //  2. use vector instead of unordered_map.
  // Check IExecutionFrame::all_values_ for more details.
  std::unordered_map<std::string, distributed::TensorPartitionSpec> propagated_tensor_partition_specs_;
#endif
