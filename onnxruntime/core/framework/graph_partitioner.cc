  ...
  if (mode == Mode::kNormal || mode == Mode::kAssignOnly) {
    ...
    if (ep_context_gen_options.enable) {
      if (ep_context_gen_options.output_model_type == epctx::ModelGenOptions::OutputModelType::kEpContext) {
        ORT_RETURN_IF_ERROR(epctx::CreateEpContextModel(providers_, graph, ep_context_gen_options, logger));
      }
      // For kOptimizedOnnx, saving is deferred to InferenceSession::Initialize after Level 2+ optimizations.
    }
#else
