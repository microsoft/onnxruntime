...
#endif
  Status status = ExecuteKernel(ctx, node_index_, stream_idx, cancellation_token, session_scope);
...
Status TriggerDownstreamStep::Execute(StreamExecutionContext& ctx,
                                      ...
                                      onnxruntime::CancellationToken cancellation_token,
                                      bool& continue_flag) {
  ScheduleDownstream(ctx, trigger_point_index_, ctx.SingleThreadMode(), cancellation_token, session_scope);
  continue_flag = true;
  return Status::OK();
}
