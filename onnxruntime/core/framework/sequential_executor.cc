...
Status SequentialExecutor::Execute(const SessionState& session_state, const std::vector<int>& feed_mlvalue_idxs,
                                   ...
                                   const logging::Logger& logger) {
  this->feed_mlvalue_idxs_.assign(feed_mlvalue_idxs.begin(), feed_mlvalue_idxs.end());
  this->feeds_ = &feeds;
  this->fetch_mlvalue_idxs_.assign(fetch_mlvalue_idxs.begin(), fetch_mlvalue_idxs.end());
  this->fetches_ = &fetches;
  ...
  if (session_state.execute_until_yield_op_) {
    frame = onnxruntime::make_unique<ExecutionFrame>(feed_mlvalue_idxs, feeds, fetch_mlvalue_idxs, fetches, fetch_allocators, session_state);
  }
...
// Enable TRACE_EXECUTION compile flag to dump execution plan
  ...
  for (size_t index = exec_index_; index < exec_plan_vec.size(); index += 1) {
    const auto& node_exec_plan = exec_plan_vec[index];
    if (terminate_flag_) {
    ...
    if (p_op_kernel->KernelDef().OpName() == "YieldOp") {
      exec_index_ = index;
      session_state.execute_until_yield_op_ = false;
      return Status::OK();
    }
  }

  session_state.execute_until_yield_op_ = true;
  exec_index_ = 0;
...
Status SequentialExecutor::Execute(const SessionState& session_state,
                                   const std::unordered_map<size_t, CustomAllocator>& fetch_allocators,
                                   const logging::Logger& logger) {
  const std::vector<int>& feed_mlvalue_idxs = feed_mlvalue_idxs_;
  const std::vector<OrtValue>& feeds = *feeds_;
  const std::vector<int>& fetch_mlvalue_idxs = fetch_mlvalue_idxs_;
  std::vector<OrtValue>& fetches = *fetches_;
