            ...
            for (auto* output : node->OutputDefs()) {
              if (output->Exists()) {
                if (std::find(it->InputDefs().begin(), it->InputDefs().end(), output) != it->InputDefs().end()) {
                  // ??? this seems to be incorrect as it assumes the output is only consumed in one place.
                  // would have expected that we need to iterate ImplicitInputDefs as well.
                  ...
                  // is #2 being specifically handled, or we're generating a notification for every transition to CPU?
                  // i.e. where is the check that the downstream node is on the same device?
                  // or should we in general generate a notification when going to CPU regardless?
                  if ((plan_.node_stream_map_[it->Index()] != i || output_arg_device == OrtDevice::CPU) &&
                      wait_handle != nullptr) {
      ...
      for (size_t j = 0; j < stream_nodes_[i].size(); ++j) {
        ...
        for (auto it = node->InputNodesBegin(); it != node->InputNodesEnd(); ++it) {
          ...
          if (std::find(stream_nodes_[i].begin(), stream_nodes_[i].end(), it->Index()) == stream_nodes_[i].end()
            ...
            // barrier from the downstream stream. but why bother? can't we initialize the barrier count to 1
        ...
        // but that seems confusing when we added to node_to_wait even if the EP wasn't necessarily changing
        // e.g. MemCpy to host from CUDA for input that is consumed on CPU by a CUDA op
