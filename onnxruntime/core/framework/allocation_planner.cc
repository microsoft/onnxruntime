  ...
  Status BuildExecutionPlan(const ExecutionProviders& execution_providers) {
    ...
    if (graph_viewer_.NumberOfNodes() > 0) {
      ...
      for (auto node_index : stream_nodes_[0]) {
        execution_plan[0]->steps_.emplace_back(std::make_unique<LaunchKernelStep>(node_indexx, graph_viewer_.GetNode(node_index)->Name()));
      }
    } else {
  ...
  Status BuildExecutionPlan(const ExecutionProviders& execution_providers,
    ...
    for (size_t i = 0; i < num_logic_streams_; ++i) {
      for (size_t j = 0; j < stream_nodes_[i].size(); ++j) {
        ...
        execution_plan[i]->steps_.emplace_back(std::make_unique<LaunchKernelStep>(node_indexx, graph_viewer_.GetNode(node_index)->Name()));
