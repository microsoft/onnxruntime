...
struct ReleasePlan {
  ...
  std::vector<std::vector<int>> node_ref_count_map;
};
...
struct ParallelExecutionPlanImpl {
  ...
  std::vector<std::vector<int>> node_value_map_;
};
...
ParallelExecutionPlanImpl::ParallelExecutionPlanImpl(const SessionState& session_state,
  ...
  node_value_map_.resize(graph_viewer.GetNodesInTopologicalOrder().size());
