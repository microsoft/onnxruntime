...
Status KernelRegistryManager::CreateKernel(const onnxruntime::Node& node,
  ...
  Status status;
  {
    for (auto& registry : custom_kernel_registries_) {
      status = registry->TryCreateKernel(node,
          ...
          session_state.GetAllocatorManager(),
		  op_kernel);
      if (status.IsOK()) {
        return status;
      }
  ...
  if (p != nullptr) {
    status = p->TryCreateKernel(node, execution_provider, session_state.GetConstantInitializedTensors(),
                                session_state.GetOrtValueNameIdxMap(), session_state.GetFuncMgr(), session_state.GetDataTransferMgr(), session_state.GetAllocatorManager(), op_kernel);
    if (status.IsOK()) {
      return status;
    }
  }
