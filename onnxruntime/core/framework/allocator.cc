...
ORT_API_STATUS_IMPL(OrtApis::CreateMemoryInfo, _In_ const char* name1, enum OrtAllocatorType type, int id1,
  ...
  if (strcmp(name1, onnxruntime::CPU) == 0) {
    ...
    *out = new OrtMemoryInfo(
    ...
    // TODO: Is it accurate enough to use the generic 'NONE' in this case?
    // As long as there's only one generic GPU EP it should be OK. Otherwise a fake vendor ID could be used.
    *out = new OrtMemoryInfo(
        name1, type,
