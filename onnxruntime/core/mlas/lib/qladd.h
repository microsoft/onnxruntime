...
MLAS_FORCEINLINE
static void
MlasCopyTailBytes(uint8_t* target, const uint8_t* src, size_t N)
{
...
bool
MlasCalcQLinearAddParameters(float ScaleRatio_AC,
                             float ScaleRatio_BC,
                             int32_t& Shift,
                             int32_t& MultiplierA,
                             int32_t& MultiplierB);
...
#define vst1_lane_u32_ex(pD, D, lane, align) \
    vst1_lane_u32((uint32_t*)__builtin_assume_aligned(pD, ((align) / 8)), D, lane)
#define vst1_lane_u16_ex(pD, D, lane, align) \
    vst1_lane_u16((uint16_t*)__builtin_assume_aligned(pD, ((align) / 8)), D, lane)
...
template <typename DataType>
MLAS_FORCEINLINE MLAS_INT8
MlasPackL8(const DataType* Input, __vector unsigned char vmask);

template <>
MLAS_FORCEINLINE MLAS_INT8
MlasPackL8<uint8_t>(const uint8_t* Input, __vector unsigned char vmask)
{
    __vector unsigned char va = vec_vsx_ld(0, Input);
    return reinterpret_cast<MLAS_INT8>(
        vec_sub(reinterpret_cast<__vector unsigned char>(va), vmask));
}

template <>
MLAS_FORCEINLINE MLAS_INT8
MlasPackL8<int8_t>(const int8_t* Input, __vector unsigned char vmask)
{
...
template <typename DataType>
MLAS_FORCEINLINE MLAS_SHORT
MlasPackS16(__vector short a, __vector short b);

template <>
MLAS_FORCEINLINE MLAS_SHORT
MlasPackS16<uint8_t>(__vector short a, __vector short b)
{
    return vec_add(a, b);
}
...
template <>
MLAS_FORCEINLINE MLAS_SHORT
MlasPackS16<int8_t>(__vector short a, __vector short b)
{
...
template <typename DataType>
MLAS_FORCEINLINE MLAS_INT32X4
MlasPackS16_128(__vector short a, __vector short b);

template <>
MLAS_FORCEINLINE MLAS_INT32X4
MlasPackS16_128<uint8_t>(__vector short a, __vector short b)
{
...
template <>
MLAS_FORCEINLINE MLAS_INT32X4
MlasPackS16_128<int8_t>(__vector short a, __vector short b)
{
