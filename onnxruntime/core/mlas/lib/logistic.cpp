...
Routine Description:
    ...
    This routine computes the Silu activation (y = x * sigmoid(x)) function.
...
--*/
{
    while (N >= 4) {
        ...
        MlasStoreFloat32x4(Output, MlasMultiplyFloat32x4(MlasClampFloat32x4(
            MlasAddFloat32x4(MlasDivideFloat32x4(p, q), MlasBroadcastFloat32x4(0.5f)),
            0.0f,
            1.0f), OrigValue));
    ...
    while (N > 0) {
        ...
        // they want a NaN on output whenever the input is a NaN.
        float v_tmp;
        v_tmp = (OrigValue < MlasLogisticConstants.LowerRange) ? MlasLogisticConstants.LowerRange : OrigValue;
        float Value = (v_tmp > MlasLogisticConstants.UpperRange) ? MlasLogisticConstants.UpperRange : v_tmp;
        ...
        *Output++ = (std::clamp((p / q) + 0.5f, 0.0f, 1.0f) * OrigValue);
