 ...
 public:
  ...
  void run(const Param& _param, ThreadProblem& thdp) {
    ...
    if (thdp.valid) {
      ...
      auto thdsptr = quan->template SPtr<float>() + thdp.loc[0] * quan->mCStep + thdp.loc[1] / quan->kblock;
      auto thdzptr = quan->template ZPtr<AType>() + thdp.loc[0] * quan->mCStep + thdp.loc[1] / quan->kblock;
      if constexpr (std::is_same_v<AType, uint8_t>) {
        kernel::wrapper::QuantizeU8ColBlock::template forward<ISA_T, SRC_T>(thdp.size[0], thdp.size[1], srcptr,
                                                                            _param.lda, thdqptr, quan->lda, thdsptr,
                                                                            quan->mCStep, thdzptr, quan->kblock);
      }
      if constexpr (std::is_same_v<AType, int8_t>) {
        kernel::wrapper::QuantizeS8ColBlock::template forward<ISA_T, SRC_T>(
            thdp.size[0], thdp.size[1], srcptr, _param.lda, thdqptr, quan->lda, thdsptr, quan->mCStep, quan->kblock);
      }
    }
  }
