    ...
    __m512i rf)
{
    ...
    _mm512_storeu_epi8(dest, r0);
    dest += 16;
    _mm512_storeu_epi8(dest, r1);
    dest += 16;
    _mm512_storeu_epi8(dest, r2);
    dest += 16;
    _mm512_storeu_epi8(dest, r3);
    dest += 16;
    _mm512_storeu_epi8(dest, r4);
    dest += 16;
    _mm512_storeu_epi8(dest, r5);
    dest += 16;
    _mm512_storeu_epi8(dest, r6);
    dest += 16;
    _mm512_storeu_epi8(dest, r7);
    dest += 16;
    _mm512_storeu_epi8(dest, r8);
    dest += 16;
    _mm512_storeu_epi8(dest, r9);
    dest += 16;
    _mm512_storeu_epi8(dest, ra);
    dest += 16;
    _mm512_storeu_epi8(dest, rb);
    dest += 16;
    _mm512_storeu_epi8(dest, rc);
    dest += 16;
    _mm512_storeu_epi8(dest, rd);
    dest += 16;
    _mm512_storeu_epi8(dest, re);
    dest += 16;
    _mm512_storeu_epi8(dest, rf);
    dest += 16;
}
...
)
{
    ...
    size_t bufsize = k_blks * (typename Q4TYPE::BlkLen) * 32 * sizeof(float);
    ...
    for (size_t n = 0; n < RangeCountN; n += CountN) {
        ...
        size_t RowsRemaining = RangeCountM;
        while (RowsRemaining > 0) {
            auto RowsHandled = GetMlasPlatform().GemmFloatKernel(a_row, dequant_b, c_blk, K, RowsRemaining,
                                                                 CountN, lda, ldc, 1.f, true);
            if (bias) {
                AddBiasAvx(bias, c_blk, RowsHandled, CountN, ldc);
            }
            if (DataParams->OutputProcessor != nullptr) {
                DataParams->OutputProcessor->Process(
                    DataParams->C, RangeStartM + RangeCountM - RowsRemaining, RangeStartN,
