...
.macro SdotByElement DestReg, Src1Reg, Src2Reg,
    Index

        .set Instruction,
    0x4F80E000 .set Instruction, Instruction + (\DestReg\() << 0).set Instruction,
    Instruction + (\Src1Reg\() << 5).set Instruction,
    Instruction + (\Src2Reg\() << 16).set Instruction,
    Instruction + ((\Index\() & 2) << 10).set Instruction,
    Instruction + ((\Index\() & 1) << 21)
    ...
    Index
        ...
        .set Instruction,
    0x6F80E000 .set Instruction, Instruction + (\DestReg\() << 0).set Instruction,
    Instruction + (\Src1Reg\() << 5).set Instruction,
    Instruction + (\Src2Reg\() << 16).set Instruction,
    Instruction + ((\Index\() & 2) << 10).set Instruction,
    Instruction + ((\Index\() & 1) << 21)
