...
MlasGemmQuantFixupZeroPointB<MLAS_GEMM_U8X8_KERNEL_LSX>(int32_t ZeroPointB, bool BIsSigned)
{
    if (!BIsSigned) {
...
template <>
void
MlasGemmQuantCopyPackA<MLAS_GEMM_U8X8_KERNEL_LSX>(MLAS_GEMM_U8X8_KERNEL_LSX::PackedAType* D,
                                                  const uint8_t* A,
                                                  size_t lda,
                                                  size_t CountM,
                                                  size_t CountK,
                                                  ...
                                                  bool AIsSigned)
{
    ...
    while (CountM > 0) {
        ...
        ReductionVector = __lsx_vadd_w(ReductionVector, __lsx_vshuf4i_w(ReductionVector, 0xee));
        ReductionVector = __lsx_vadd_w(ReductionVector, __lsx_vshuf4i_w(ReductionVector, 0x11));
...
MLAS_FORCEINLINE
void
MlasGemmU8X8CopyPackBProcessLSX(MLAS_GEMM_U8X8_KERNEL_LSX::PackedBType* D,
                                __m128i BytesRow0,
                                __m128i BytesRow1,
...
template <>
void
MlasGemmQuantCopyPackB<MLAS_GEMM_U8X8_KERNEL_LSX>(MLAS_GEMM_U8X8_KERNEL_LSX::PackedBType* D,
                                                  const uint8_t* B,
                                                  size_t ldb,
                                                  size_t CountN,
                                                  size_t CountK,
...
MLAS_FORCEINLINE
void
MlasGemmU8X8MultiplyAccumulateRowLSX(__m128i ABroadcast, const int16_t* B, __m128i Accumulators[2])
{
...
template <>
size_t
MlasGemmQuantKernel<MLAS_GEMM_U8X8_KERNEL_LSX>(const MLAS_GEMM_U8X8_KERNEL_LSX::PackedAType* A,
                                               ...
                                               bool ZeroMode)
{
    ...
    while (CountN > 0) {
        ...
        Accumulators[0] =
            __lsx_vadd_w(Accumulators[0], __lsx_vld((const __m128i*)&ColumnSumBuffer[0], 0));
        Accumulators[1] =
            __lsx_vadd_w(Accumulators[1], __lsx_vld((const __m128i*)&ColumnSumBuffer[4], 0));
        ColumnSumBuffer += 8;
        ...
        if (CountN >= 8) {
            ...
            if ((CountN & 2) != 0) {
                if (!ZeroMode) {
                    Accumulators[0] = __lsx_vadd_w(
                        Accumulators[0], __lsx_vinsgr2vr_d(__lsx_vld((__m128i*)&C[0], 0), 0, 1));
                }
