        ...
        MlasSveShiftLeftInt32(MLAS_SVBOOL Pred, MLAS_SVINT32 Vector)
{
    ...
    // Isolate even and odd indexed elements with padding for widening
    svint32_t evens = svzip1_s32(zeros, Vector);
    svint32_t odds = svzip2_s32(zeros, Vector);
    svint32_t even_shift, odd_shift;

    // Perform left shift
    even_shift = svlsl_n_s32_m(Pred, evens, ShiftCount);
    odd_shift = svlsl_n_s32_m(Pred, odds, ShiftCount);
    // Interleave the halves again to get final output
    svint32_t result = svzip1_s32(even_shift, odd_shift);
    return result;
}
...
__attribute__((target("arch=armv8.2-a+sve")))  // MLAS_SVE_TARGET
MLAS_SVBOOL
    MLAS_FORCEINLINE
    MlasSveptrue(void)
{
...
{
    MLAS_SVBOOL Pred = svwhilelt_b32(Lane, Lane + 1);
    MlasSveStoreFloat32(Pred, Buffer, Vector);
}
...
{
    MLAS_SVBOOL Pred = svwhilelt_b32(0, (int32_t)svcntw() / 2);
    MlasSveStoreFloat32(Pred, Buffer, Vector);
}
    ...
    MlasSveExtractLaneFloat32(MLAS_SVFLOAT32 Vector)
{
    float TmpBuffer[1];
    MLAS_SVBOOL Pred = svwhilelt_b32(Lane, Lane + 1);
    MlasSveStoreFloat32(Pred, TmpBuffer, Vector);
    return TmpBuffer[0];
}
...
{
    return svsel_f32(
        svcmpgt_f32(Pred, Vector1, Vector2),
        svdup_n_f32(1.0f),
        svdup_n_f32(0.0f)
    );
}
