...
static constexpr size_t MLAS_WINOGRAD_TILE_SIZE = 2;
static constexpr size_t MLAS_WINOGRAD_TRANSFORM_SIZE = 4;
static constexpr size_t MLAS_WINOGRAD_TRANSFORM_COUNT = MLAS_WINOGRAD_TRANSFORM_SIZE * MLAS_WINOGRAD_TRANSFORM_SIZE;
...
{
    const size_t OutputHeight = WorkBlock->OutputShape[0];
...
{
    return WorkBlock->UseWinograd && WorkBlock->GroupCount == 1 &&
           WorkBlock->KernelShape[0] == 3 && WorkBlock->KernelShape[1] == 3 &&
           WorkBlock->DilationShape[0] == 1 && WorkBlock->DilationShape[1] == 1 &&
           WorkBlock->StrideShape[0] == 1 && WorkBlock->StrideShape[1] == 1 &&
           WorkBlock->Padding[0] == 1 && WorkBlock->Padding[1] == 1 &&
           WorkBlock->Padding[2] == 1 && WorkBlock->Padding[3] == 1 &&
           WorkBlock->InputChannels >= MlasNchwcGetBlockSize() &&
           MlasNchwcShouldPreferWinograd3x3(WorkBlock);
}
...
MLAS_FORCEINLINE
void
MlasWinogradAccumulateOutputBlockAvx512(
    size_t InputChannels,
    const float* Filter,
    const float* TransformedInput,
    float* Accumulator
...
{
    __m512 Accumulators[MLAS_WINOGRAD_TRANSFORM_COUNT];

    for (size_t k = 0; k < MLAS_WINOGRAD_TRANSFORM_COUNT; k++) {
        Accumulators[k] = _mm512_setzero_ps();
    }
    ...
    for (size_t ic = 0; ic < InputChannels; ic += BlockSize) {
        ...
        for (size_t k = 0; k < MLAS_WINOGRAD_TRANSFORM_COUNT; k++) {
            ...
            for (size_t bi = 0; bi < BlockSize; bi++) {
                const __m512 InputValue = _mm512_set1_ps(InputTransform[bi]);
                const __m512 FilterValue = _mm512_loadu_ps(FilterTransform + bi * BlockSize);
                Accumulators[k] = _mm512_fmadd_ps(InputValue, FilterValue, Accumulators[k]);
            }
        }
    }

    for (size_t k = 0; k < MLAS_WINOGRAD_TRANSFORM_COUNT; k++) {
        _mm512_storeu_ps(Accumulator + k * BlockSize, Accumulators[k]);
    }
}
#endif
...
struct MLAS_NCHWC_CONV_WINOGRAD_ALGORITHM : MLAS_NCHWC_CONV_ALGORITHM
{
    ...
    void ExecuteTile(size_t Batch, size_t Group, size_t TileRow, size_t TileCol)
    {
        ...
        const size_t ValidRows = std::min<size_t>(MLAS_WINOGRAD_TILE_SIZE, OutputHeight - TileOutputY);
        const size_t ValidCols = std::min<size_t>(MLAS_WINOGRAD_TILE_SIZE, OutputWidth - TileOutputX);

        const float* InputGroup = WorkBlock->Input + (Batch * GroupCount + Group) * InputChannels * InputSize;
        ...
        std::fill_n(TransformedInput, InputBlockCount * MLAS_WINOGRAD_TRANSFORM_COUNT * BlockSize, 0.0f);
...
--*/
{
    ...
    WorkBlock.UseWinograd = UseWinograd;
    ...
    if (MlasNchwcIsWinograd3x3Supported(&WorkBlock)) {
