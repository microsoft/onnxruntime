...
template <typename DataType, bool IsScalarB>
static void
MlasQLinearAddKernelHelper(const DataType* InputA,
                           float ScaleA,
                           int32_t ZeroPointA,
                           const DataType* InputB,
                           float ScaleB,
                           ...
                           size_t N)
{
    ...
    const auto VectorScaleRatio_AC = MlasBroadcastFloat32x4(ScaleRatio_AC);
    const auto VectorScaleRatio_BC = MlasBroadcastFloat32x4(ScaleRatio_BC);
    auto VectorFixedPart = MlasBroadcastFloat32x4(
        (float)ZeroPointC - (ScaleRatio_AC * ZeroPointA + ScaleRatio_BC * ZeroPointB));
    ...
    while (N >= 8) {
        ...
        InputA += 8;
        va_lo =
            __lsx_vffint_s_w(MlasShiftRightInt32<DataType>(__lsx_vilvl_h(va_i16x8, va_i16x8), 24));
        va_hi =
            __lsx_vffint_s_w(MlasShiftRightInt32<DataType>(__lsx_vilvh_h(va_i16x8, va_i16x8), 24));

        if (!IsScalarB) {
            const auto vb_low_half =
                __lsx_vinsgr2vr_d(__lsx_vld((const MLAS_INT32X4*)InputB, 0), 0, 1);
            const auto vb_i16x8 = __lsx_vilvl_b(vb_low_half, vb_low_half);
            InputB += 8;
            vb_lo = __lsx_vffint_s_w(
                MlasShiftRightInt32<DataType>(__lsx_vilvl_h(vb_i16x8, vb_i16x8), 24));
            vb_hi = __lsx_vffint_s_w(
                MlasShiftRightInt32<DataType>(__lsx_vilvh_h(vb_i16x8, vb_i16x8), 24));
        }
        ...
        if (IsScalarB) {
            ...
            r_lo = __lsx_vftint_w_s(
                __lsx_vfadd_s(__lsx_vfmadd_s(va_lo, VectorScaleRatio_AC, VectorFixedPart),
                              __lsx_vfmul_s(vb_lo, VectorScaleRatio_BC)));
            r_hi = __lsx_vftint_w_s(
                __lsx_vfadd_s(__lsx_vfmadd_s(va_hi, VectorScaleRatio_AC, VectorFixedPart),
                              __lsx_vfmul_s(vb_hi, VectorScaleRatio_BC)));
        }
        ...
        N -= 8;
        __lsx_vst(
            __lsx_vinsgr2vr_d(__lsx_vld((MLAS_INT32X4*)OutputC, 0), __lsx_vpickve2gr_d(vc, 0), 0),
            (MLAS_INT32X4*)OutputC, 0);
        OutputC += 8;
    }
    ...
    if (N > 0) {
        ...
        MlasCopyTailBytes(TailData, (const uint8_t*)InputA, N);
        const auto va_low_half =
            __lsx_vinsgr2vr_d(__lsx_vld((const MLAS_INT32X4*)TailData, 0), 0, 1);
        const auto va_i16x8 = __lsx_vilvl_b(va_low_half, va_low_half);
        va_lo =
            __lsx_vffint_s_w(MlasShiftRightInt32<DataType>(__lsx_vilvl_h(va_i16x8, va_i16x8), 24));
        va_hi =
