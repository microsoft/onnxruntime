    ...
    )
{
    ...
    if (n > 0) {
        ...
        if (!IsScalarA) {
            typename SUI::T* tail = TailDataA;
            typename SUI::T* tail_end = tail + n;
            do {
                *tail = *(const typename SUI::T*)InputA;
                tail++;
                InputA++;
            } while (tail < tail_end);
        }
        if (!IsScalarB) {
            typename SUI::T* tail = TailDataB;
            typename SUI::T* tail_end = tail + n;
            do {
                *tail = *(const typename SUI::T*)InputB;
                tail++;
                InputB++;
            } while (tail < tail_end);
        }
