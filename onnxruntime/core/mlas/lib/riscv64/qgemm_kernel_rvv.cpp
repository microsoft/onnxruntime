    ...
    )
{
    ...
    /* The fallback below processes one A row per loop iteration. Force the
     * kernel to advertise RowsHandled=1 here so MlasGemmQuantOperation will
     * call us again to cover the remaining (CountM - 1) rows; otherwise we
     * would silently drop the second row when CountM == 2 or 3. */
    RowsHandled = 1;
