...
--*/
{
    ...
    while (size >= 4) {
        ...
        MLAS_FLOAT32X4 ResultVector = MlasCastToFloat32x4(MlasLoadInt32x4(Input));

        if (AccumulateMode) {
            MLAS_FLOAT32X4 OutputVector = MlasLoadFloat32x4(Output);
            ResultVector = MlasMultiplyAddFloat32x4(ResultVector, ScaleVector, OutputVector);
        } else {
            ResultVector = MlasMultiplyFloat32x4(ResultVector, ScaleVector);
        }

        MlasStoreFloat32x4(Output, ResultVector);
...
Arguments:
    ...
    Scale - Supplies the quantization scale per column.
