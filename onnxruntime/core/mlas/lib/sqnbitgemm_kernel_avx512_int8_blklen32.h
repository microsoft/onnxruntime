...
__m256 load_1blksum_256(const float* BlksumPtr) {
    ...
    __m256 result = _mm256_castsi256_ps(
        _mm256_mask_set1_epi32(_mm256_setzero_si256(), mask, *reinterpret_cast<int*>(value))
    );
...
accumulate_blklen32_r2c1blk4_avx512(
    ...
    const float* scale_b,
    const float* blksum_a0,
    const float* blksum_a1,
    const float* blksum_b,
    __m512& acc0,
    __m512& acc1
)
{
    ...
    const __m512 blksum_a0_ps = load_4blksum_512(blksum_a0);  // 0123000000000000
    const __m512 blksum_b_ps = load_4blksum_512(blksum_b);    // 0123000000000000
    acc0 = _mm512_fmadd_ps(blksum_a0_ps, blksum_b_ps, acc0);

    const __m512 blksum_a1_ps = load_4blksum_512(blksum_a1);  // 0123000000000000
    acc1 = _mm512_fmadd_ps(blksum_a1_ps, blksum_b_ps, acc1);
}
...
)
{
    if constexpr (vnni) {
        ...
        acc0 = _mm256_add_ps(load_1blksum_256(&combined_blksum00), acc0);
    } else {
...
)
{
    if constexpr (vnni) {
        ...
        acc0 = _mm256_add_ps(load_1blksum_256(&combined_blksum00), acc0);
        acc1 = _mm256_add_ps(load_1blksum_256(&combined_blksum10), acc1);
...
)
{
    ...
    for (size_t m = 0; m < CountM; m += NRows2) {
        ...
        const float* QuantBBlksumColPtr = QuantBBlkSum;
        ...
        for (size_t n = 0; n < CountN; n += NCols4) {
            ...
            const float* ABlockSumPtr = ABlockSum + m * BlockCountK;

            const std::byte* QuantBDataPtr = QuantBDataColPtr;
            const float* QuantBScalePtr = QuantBScaleColPtr;
            const float* QuantBBlkSumPtr = QuantBBlksumColPtr;
            ...
            for (; k_blks_remaining >= PerAccuBlk4; k_blks_remaining -= PerAccuBlk4) {
                ...
                if constexpr (vnni) {
                    accumulate_blklen32_r2c1blk4_avx512vnni(
                        ...
                        QuantAScalePtr, QuantAScalePtr + BlockCountK, QuantBScalePtr,
                        ABlockSumPtr, ABlockSumPtr + BlockCountK, QuantBBlkSumPtr,
                        acc[0], acc[NCols4]
                    );
                    accumulate_blklen32_r2c1blk4_avx512vnni(
                        ...
                        ABlockSumPtr, ABlockSumPtr + BlockCountK, QuantBBlkSumPtr + PerAccuBlk4,
                        acc[1], acc[NCols4 + 1]
                    );
                    accumulate_blklen32_r2c1blk4_avx512vnni(
                        av_00_epi8, av_01_epi8, av_10_epi8, av_11_epi8, QuantBDataPtr + 2 * StrideQuantBData,
                        QuantAScalePtr, QuantAScalePtr + BlockCountK, QuantBScalePtr + 2 * PerAccuBlk4,
                        ABlockSumPtr, ABlockSumPtr + BlockCountK, QuantBBlkSumPtr + 2 * PerAccuBlk4,
                        acc[2], acc[NCols4 + 2]
                    );
                    accumulate_blklen32_r2c1blk4_avx512vnni(
                        av_00_epi8, av_01_epi8, av_10_epi8, av_11_epi8, QuantBDataPtr + 3 * StrideQuantBData,
                        QuantAScalePtr, QuantAScalePtr + BlockCountK, QuantBScalePtr + 3 * PerAccuBlk4,
                        ABlockSumPtr, ABlockSumPtr + BlockCountK, QuantBBlkSumPtr + 3 * PerAccuBlk4,
                        acc[3], acc[NCols4 + 3]
                    );
                } else {
                    accumulate_blklen32_r2c1blk4_avx512(
            ...
            while (k_blks_remaining-- > 0) {
                ...
                const float& blksum_a00 = *ABlockSumPtr;
                const float& blksum_a10 = *(ABlockSumPtr + BlockCountK);

                {
                    // Col0
                    const float scale_00 = scale_a00 * (QuantBScalePtr)[0];
                    const float scale_10 = scale_a10 * (QuantBScalePtr)[0];
                    const float blksum_00 = blksum_a00 * QuantBBlkSumPtr[0];
                    const float blksum_10 = blksum_a10 * QuantBBlkSumPtr[0];

                    accumulate_blklen32_r2c1blk1_avx512<vnni>(av_00_epi8, av_10_epi8, QuantBDataPtr, scale_00, scale_10, blksum_00, blksum_10, acc2[0], acc2[NCols4]);
                }

                {
                    ...
                    const float blksum_00 = blksum_a00 * (QuantBBlkSumPtr + 1)[0];
                    const float blksum_10 = blksum_a10 * (QuantBBlkSumPtr + 1)[0];
                    accumulate_blklen32_r2c1blk1_avx512<vnni>(av_00_epi8, av_10_epi8, QuantBDataPtr + BlkDataSizeInBytes16, scale_00, scale_10, blksum_00, blksum_10, acc2[1], acc2[NCols4 + 1]);
                }

                {
                    ...
                    const float blksum_00 = blksum_a00 * (QuantBBlkSumPtr + 2)[0];
                    const float blksum_10 = blksum_a10 * (QuantBBlkSumPtr + 2)[0];
                    accumulate_blklen32_r2c1blk1_avx512<vnni>(av_00_epi8, av_10_epi8, QuantBDataPtr + 2 * BlkDataSizeInBytes16, scale_00, scale_10, blksum_00, blksum_10, acc2[2], acc2[NCols4 + 2]);
                }

                {
                    ...
                    const float blksum_00 = blksum_a00 * (QuantBBlkSumPtr + 3)[0];
                    const float blksum_10 = blksum_a10 * (QuantBBlkSumPtr + 3)[0];
                    accumulate_blklen32_r2c1blk1_avx512<vnni>(av_00_epi8, av_10_epi8, QuantBDataPtr + 3 * BlkDataSizeInBytes16, scale_00, scale_10, blksum_00, blksum_10, acc2[3], acc2[NCols4 + 3]);
                }
                QuantAPtr += BlkLen32;
                QuantAScalePtr++;
                ABlockSumPtr++;
            ...
            QuantBBlksumColPtr += NCols4 * BlockCountK;
    ...
    for (size_t m = 0; m < CountM; m += NRows2) {
        ...
        for (size_t n = 0; n < CountN; n++) {
            ...
            while (k_blks_remaining-- > 0) {
                ...
                const float& blksum_00 = blksum_a00 * (QuantBBlkSumPtr)[0];
                const float& blksum_10 = blksum_a10 * (QuantBBlkSumPtr)[0];
                accumulate_blklen32_r2c1blk1_avx512<vnni>(av_00_epi8, av_10_epi8, QuantBDataPtr, scale_00, scale_10, blksum_00, blksum_10, acc20, acc21);
            ...
            *(SumPtr + ldc) = hsum_float_8(acc21);
            ...
            QuantBBlkSumColPtr += StrideQuantBScale;
        ...
        for (size_t n = 0; n < CountN; n += NCols4) {
            ...
            for (; k_blks_remaining >= PerAccuBlk4; k_blks_remaining -= PerAccuBlk4) {
                ...
                if constexpr (vnni) {
                    accumulate_blklen32_r1c1blk4_avx512vnni(av_00_epi8, av_01_epi8, QuantBDataPtr,
                      ...
                      acc[0]);
                    accumulate_blklen32_r1c1blk4_avx512vnni(av_00_epi8, av_01_epi8, QuantBDataPtr + PerAccuBlk4 * BlkDataSizeInBytes16,
                      QuantAScalePtr, QuantBScalePtr + PerAccuBlk4,
                      ABlockSumPtr, QuantBBlkSumPtr + PerAccuBlk4,
                      acc[1]);
                    accumulate_blklen32_r1c1blk4_avx512vnni(av_00_epi8, av_01_epi8, QuantBDataPtr + 2 * PerAccuBlk4 * BlkDataSizeInBytes16,
                      QuantAScalePtr, QuantBScalePtr + 2 * PerAccuBlk4,
                      ABlockSumPtr, QuantBBlkSumPtr + 2 * PerAccuBlk4,
                      acc[2]);
                    accumulate_blklen32_r1c1blk4_avx512vnni(av_00_epi8, av_01_epi8, QuantBDataPtr + 3 * PerAccuBlk4 * BlkDataSizeInBytes16,
                      QuantAScalePtr, QuantBScalePtr + 3 * PerAccuBlk4,
                      ABlockSumPtr, QuantBBlkSumPtr + 3 * PerAccuBlk4,
                      acc[3]);
                } else {
                    accumulate_blklen32_r1c1blk4_avx512(av_00_epi8, av_01_epi8, QuantBDataPtr,
                      QuantAScalePtr, QuantBScalePtr,
                      ABlockSumPtr, QuantBBlkSumPtr,
                      acc[0]);
                    accumulate_blklen32_r1c1blk4_avx512(av_00_epi8, av_01_epi8, QuantBDataPtr + PerAccuBlk4 * BlkDataSizeInBytes16,
                      QuantAScalePtr, QuantBScalePtr + PerAccuBlk4,
                      ABlockSumPtr, QuantBBlkSumPtr + PerAccuBlk4,
                      acc[1]);
                    accumulate_blklen32_r1c1blk4_avx512(av_00_epi8, av_01_epi8, QuantBDataPtr + 2 * PerAccuBlk4 * BlkDataSizeInBytes16,
                      QuantAScalePtr, QuantBScalePtr + 2 * PerAccuBlk4,
                      ABlockSumPtr, QuantBBlkSumPtr + 2 * PerAccuBlk4,
                      acc[2]);
                    accumulate_blklen32_r1c1blk4_avx512(av_00_epi8, av_01_epi8, QuantBDataPtr + 3 * PerAccuBlk4 * BlkDataSizeInBytes16,
                      QuantAScalePtr, QuantBScalePtr + 3 * PerAccuBlk4,
                      ABlockSumPtr, QuantBBlkSumPtr + 3 * PerAccuBlk4,
                      acc[3]);
                }
            ...
            while (k_blks_remaining-- > 0) {
                ...
                const float& blksum_a00 = *ABlockSumPtr;
                {
                    const float& scale_00 = scale_a00 * (QuantBScalePtr)[0];
                    const float& blksum_00 = blksum_a00 * (QuantBBlkSumPtr)[0];
                    accumulate_blklen32_r1c1blk1_avx512<vnni>(av_00_epi8, QuantBDataPtr, scale_00, blksum_00, acc2[0]);
                }
                {
                    const float& scale_00 = scale_a00 * (QuantBScalePtr + 1)[0];
                    const float& blksum_00 = blksum_a00 * (QuantBBlkSumPtr + 1)[0];
                    accumulate_blklen32_r1c1blk1_avx512<vnni>(av_00_epi8, QuantBDataPtr + BlkDataSizeInBytes16, scale_00, blksum_00, acc2[1]);
                }
                {
                    const float& scale_00 = scale_a00 * (QuantBScalePtr + 2)[0];
                    const float& blksum_00 = blksum_a00 * (QuantBBlkSumPtr + 2)[0];
                    accumulate_blklen32_r1c1blk1_avx512<vnni>(av_00_epi8, QuantBDataPtr + 2 * BlkDataSizeInBytes16, scale_00, blksum_00, acc2[2]);
                }
                {
                    const float& scale_00 = scale_a00 * (QuantBScalePtr + 3)[0];
                    const float& blksum_00 = blksum_a00 * (QuantBBlkSumPtr + 3)[0];
                    accumulate_blklen32_r1c1blk1_avx512<vnni>(av_00_epi8, QuantBDataPtr + 3 * BlkDataSizeInBytes16, scale_00, blksum_00, acc2[3]);
                }
    ...
    for (size_t m = 0; m < CountM; m++) {
        ...
        for (size_t n = 0; n < CountN; n++) {
            ...
            while (k_blks_remaining-- > 0) {
                ...
                const float& blksum_a00 = *ABlockSumPtr;
                const float& blksum_00 = blksum_a00 * (QuantBBlkSumPtr)[0];
                accumulate_blklen32_r1c1blk1_avx512<vnni>(av_00_epi8, QuantBDataPtr, scale_00, blksum_00, acc2);
    ...
    )
{
    ...
    if (remainingCols > 0 && multipleRows > 0) {
        Q4Int8GemmR2C1BlkLen32Avx512<vnni>(
            ...
            ldc,
            ABlockSum,
            QuantBBlkSum + multipleCols * StrideQuantBScale);
    }
    ...
    if (remainingRows > 0 && multipleCols > 0) {
        Q4Int8GemmR1xC4BlkLen32Avx512<vnni>(
            ...
            Bias,
            ldc,
            ABlockSum + multipleRows * lda_scale,
            QuantBBlkSum);
    }
