...
Status GENERIC_OP_IR_CREATOR_CLASS(Split)::Evaluate(
  ...
  if (info.GetAttrs("split", split_sizes).IsOK()) {
    ...
    for (size_t i = 0; i < node.OutputDefs().size(); ++i) {
      ORT_RETURN_IF_NOT(split_sizes[i] == ShapeValue(node.OutputDefs()[i], static_cast<int>(axis)));
    }

  } else {
    for (size_t i = 0; i < node.OutputDefs().size(); ++i) {
      split_sizes.push_back(ShapeValue(node.OutputDefs()[i], static_cast<int>(axis)));
