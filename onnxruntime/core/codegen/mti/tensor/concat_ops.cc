...
tvm::te::Tensor Concat(const tvm::Array<tvm::te::Tensor>& inputs,
                   int64_t axis,
...
tvm::te::Tensor ConcatSafe(const tvm::Array<tvm::te::Tensor>& inputs,
                       int64_t axis,
                       const std::string& name) {
  ...
  tvm::Array<tvm::PrimExpr> axis_sizes;
  for (auto t : inputs) {
    axis_sizes.push_back(t->shape[axis]);
  }

   tvm::PrimExpr join_size = axis_sizes[0];
  ...
  return tvm::te::compute(
      out_shape, [&](const tvm::Array<tvm::tir::Var>& ovars) {
        tvm::Array<tvm::PrimExpr> indices;

        // preset
         tvm::PrimExpr min = 0;
         tvm::PrimExpr extent = axis_sizes[0];
         tvm::PrimExpr offset = 0;
         tvm::PrimExpr ret;

        //input i = 0
        for (size_t j = 0; j < ovars.size(); ++j) {
          if (j == gsl::narrow<size_t>(axis)) {
             tvm::PrimExpr ivar = ovars[j];
        ...
        for (size_t i = 1; i < inputs.size(); ++i) {
          ...
          auto j = gsl::narrow<size_t>(axis);
           tvm::PrimExpr ivar = ovars[j] - offset;
          indices.Set(j, tvm::max(tvm::min(ivar, min + extent - 1), min));

          ret = tvm::tir::Select(ivar >= 0,
                                 inputs[i](indices),
                                 ret);
        }
