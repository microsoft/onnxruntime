...
void RunOnQwenProviders(OpTester& test) {
...
TEST(EngramOpsTest, NGramHashMappingQwenCachedEosReset) {
  OpTester test("NGramHashMapping", 1, kMSDomain);
  test.AddAttribute<int64_t>("max_ngram_size", 3);
  test.AddAttribute<int64_t>("n_head_per_ngram", 1);
  test.AddAttribute<int64_t>("eos_token_id", 2);
  test.AddInput<int32_t>("input_ids", {1, 3}, {5, 2, 7});
  test.AddInput<int32_t>("multipliers", {3}, {11, 13, 17});
  test.AddInput<int32_t>("vocab_sizes", {2}, {101, 103});
  test.AddInput<int32_t>("past_tokens", {1, 2}, {3, 4});
  test.AddInput<int32_t>("head_offsets", {2}, {10, 200});
  test.AddOutput<int32_t>("hash_ids", {1, 3, 2}, {13, 248, 97, 219, 97, 214});
  test.AddOutput<int32_t>("present_tokens", {1, 2}, {2, 7});
  RunOnQwenProviders(test);
}

TEST(EngramOpsTest, EngramGateNormalizedOutput) {
  constexpr float epsilon = 1.0e-5f;
  const std::vector<float> embeddings{1.0f, 2.0f};
  const std::vector<float> hidden_states{3.0f, 4.0f};
  const std::vector<float> key_weight{0.5f, 1.0f, -0.25f, 0.75f};
  ...
  const float key1 = 2.5f;
  const float key_inv = 1.0f / std::sqrt((key0 * key0 + key1 * key1) / 2.0f + epsilon);
  const float query_inv = 1.0f / std::sqrt(12.5f + epsilon);
  const float dot = (key1 * key_inv * 4.0f * query_inv) / std::sqrt(2.0f);
  const float gate = Sigmoid(GateArg(dot));
  const std::vector<float> gated{2.0f * gate, -0.5f * gate};
  const float conv_inv = 1.0f / std::sqrt((gated[0] * gated[0] + gated[1] * gated[1]) / 2.0f + epsilon);
  const std::vector<float> normalized{gated[0] * conv_inv, gated[1] * conv_inv};
  ...
  test.AddInput<float>("conv_norm_scale", {1, 2}, unit_scale);
  test.AddOutput<float>("gated_value", {1, 1, 2}, gated, false, 1e-4f, 1e-4f);
  test.AddOutput<float>("gated_value_normed", {1, 1, 2}, normalized, false, 1e-4f, 1e-4f);
  RunOnQwenProviders(test);
}

TEST(EngramOpsTest, ShortConvQwenFlattenedInput) {
  OpTester test("ShortConv", 1, kMSDomain);
  test.AddAttribute<int64_t>("dilation", 2);
  test.AddAttribute<std::string>("activation", "none");
  test.AddInput<float>("input", {1, 3, 2}, {1.0f, 10.0f, 2.0f, 20.0f, 3.0f, 30.0f});
  test.AddInput<float>("weight", {2, 1, 2}, {0.5f, 1.0f, -1.0f, 0.25f});
  test.AddOptionalInputEdge<float>();
  test.AddOutput<float>("output", {1, 3, 2}, {1.0f, 2.5f, 2.0f, 5.0f, 3.5f, -2.5f});
  RunOnQwenProviders(test);
}
