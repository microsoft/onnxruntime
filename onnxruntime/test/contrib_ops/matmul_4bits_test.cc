...
namespace {
// Builds and serializes a single-node float MatMulNBits model. "A" is a runtime graph input; B, scales
// and zero_points are constant initializers and the single output is named "Y". The zero-point
// collision test below builds two such models that share byte-identical B and scales initializers but
// differ only in their zero_points.
void BuildMatMulNBitsModelBytes(int64_t M, int64_t N, int64_t K, int64_t block_size, int64_t accuracy_level,
                                const std::vector<uint8_t>& b_data, const std::vector<float>& scales_data,
                                const std::vector<uint8_t>& zp_data, std::string& model_bytes) {
  const int64_t k_blocks = (K + block_size - 1) / block_size;
  const int64_t blob_size = (block_size * QBits + 7) / 8;
  const int64_t zp_blob_size = (k_blocks * QBits + 7) / 8;

  const std::unordered_map<std::string, int> domain_to_version{{"", 21}, {kMSDomain, 1}};
  Model model("matmul_nbits_zp_collision", false, ModelMetaData(), PathString(),
  ...
  NodeArg* B = builder.MakeInitializer<uint8_t>({N, k_blocks, blob_size}, b_data);
  NodeArg* scales = builder.MakeInitializer<float>({N, k_blocks}, scales_data);
  NodeArg* zero_points = builder.MakeInitializer<uint8_t>({N, zp_blob_size}, zp_data);

  Node& node = builder.AddNode("MatMulNBits", {A, B, scales, zero_points}, {Y}, kMSDomain);
  ...
  node.AddAttribute("accuracy_level", accuracy_level);
...
void RunSharedPrepackSession(const std::string& model_bytes, const NameMLValMap& feeds,
  ...
  ASSERT_STATUS_OK(session.Run(run_options, feeds, std::vector<std::string>{"Y"}, &fetches));
  used_shared_count = session.GetSessionState().GetUsedSharedPrePackedWeightCounter();
}
}  // namespace

// Regression test for pre-packed weight sharing: two MatMulNBits initializers with byte-identical
// quantized B and identical scales but DIFFERENT zero points must not be treated as identical by the
// content-addressed sharing cache. For the CompInt8 path (accuracy_level 4) the zero points are folded
// into the packed buffer (as the per-block sum). Before the fix the sharing hash was taken before the
// zero points were folded in, so the second session silently adopted the first session's buffer and
// produced a wrong result.
TEST(MatMulNBits, SharedPrepackedWeights_DifferentZeroPointsDoNotCollide) {
  constexpr int64_t M = 2, N = 16, K = 128, block_size = 32;
  constexpr int64_t accuracy_level = 4;  // CompInt8 - the path that folds zero points into packed_b_.
  const int64_t k_blocks = (K + block_size - 1) / block_size;
  const int64_t blob_size = (block_size * QBits + 7) / 8;
  const int64_t zp_blob_size = (k_blocks * QBits + 7) / 8;

  // Byte-identical B and scales for both models; only the zero points differ.
  std::vector<uint8_t> b_data(static_cast<size_t>(N * k_blocks * blob_size));
  for (size_t i = 0; i < b_data.size(); ++i) {
    b_data[i] = static_cast<uint8_t>((i * 7 + 3) & 0xFF);
  }
  std::vector<float> scales(static_cast<size_t>(N * k_blocks));
  for (size_t i = 0; i < scales.size(); ++i) {
    scales[i] = 0.02f + 0.001f * static_cast<float>(i % 17);
  }
  std::vector<uint8_t> zp1(static_cast<size_t>(N * zp_blob_size));
  std::vector<uint8_t> zp2(zp1.size());
  for (size_t i = 0; i < zp1.size(); ++i) {
    zp1[i] = static_cast<uint8_t>((i * 5 + 1) & 0xFF);
    zp2[i] = static_cast<uint8_t>(~zp1[i]);  // every 4-bit zero point differs between the two models
  }

  std::string model_zp1, model_zp2;
  BuildMatMulNBitsModelBytes(M, N, K, block_size, accuracy_level, b_data, scales, zp1, model_zp1);
  BuildMatMulNBitsModelBytes(M, N, K, block_size, accuracy_level, b_data, scales, zp2, model_zp2);
  ...
  // Reference output for the zp2 model, computed in isolation (its own private container).
  std::vector<OrtValue> ref_fetches;
  size_t ref_used = 0;
  {
    PrepackedWeightsContainer ref_container;
    RunSharedPrepackSession(model_zp2, feeds, ref_container, ref_fetches, ref_used);
  }

  // Share one container across a zp1 session (which warms the container with its finalized buffer) and
  // a subsequent zp2 session.
  PrepackedWeightsContainer shared_container;
  std::vector<OrtValue> warm_fetches, shared_fetches;
  size_t warm_used = 0, shared_used = 0;
  RunSharedPrepackSession(model_zp1, feeds, shared_container, warm_fetches, warm_used);

  // If the platform did not pre-pack B for this configuration there is nothing to collide on.
  if (shared_container.GetNumberOfElements() == 0) {
    GTEST_SKIP() << "CompInt8 pre-packing not available on this platform";
  }

  RunSharedPrepackSession(model_zp2, feeds, shared_container, shared_fetches, shared_used);

  // The zp2 session must NOT have reused the zp1 session's finalized buffer: different zero points
  // produce different packed bytes and therefore a different sharing hash.
  EXPECT_EQ(shared_used, static_cast<size_t>(0));

  // And the shared zp2 run must match the isolated zp2 reference element for element.
  ASSERT_EQ(ref_fetches.size(), static_cast<size_t>(1));
  ASSERT_EQ(shared_fetches.size(), static_cast<size_t>(1));
  const Tensor& ref_tensor = ref_fetches[0].Get<Tensor>();
  const Tensor& shared_tensor = shared_fetches[0].Get<Tensor>();
  ASSERT_EQ(ref_tensor.Shape(), shared_tensor.Shape());
  const auto ref_span = ref_tensor.DataAsSpan<float>();
  const auto shared_span = shared_tensor.DataAsSpan<float>();
  for (size_t i = 0; i < ref_span.size(); ++i) {
    EXPECT_EQ(ref_span[i], shared_span[i]) << "output mismatch at index " << i;
  }
}
