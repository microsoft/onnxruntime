...
namespace test {
void TestActivationOp(const char* szOp, std::vector<float>& input_vals,
...
TEST(ActivationOpTest, ThresholdedRelu_version_1_to_9) {
  float alpha = 0.1f;
  TestActivationOp("ThresholdedRelu",
                   input_values,
...
TEST(ActivationOpTest, ParametricSoftplus) {
  ...
  TestActivationOp("ParametricSoftplus",
                   input_values,
                   [](float x) {
                     float bx = beta * x;
                     if (bx > 0)
