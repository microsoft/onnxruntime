...
static void RunQOrdered_LayerNorm_RowMajor(std::vector<int64_t> const& shape, int axis, float epsilon, const std::vector<int8_t>& vec_x, float scale_x, const std::vector<T>& gamma, const std::vector<T>* beta, float scale_y, const std::vector<int8_t>& vec_y) {
...
TEST(QOrderedTest, LayerNormalization_RowMajor) {
  ...
  std::vector<int8_t> vec_x = {
      -103, -24, -11, 125, 103, -117, 44, -109, 27, 122, 113, 0, -7, 26, 110, -34, 28, -61, -107, -35, -54, 69, -3, -58, -119, 61, -19, -115, 35, 76, 16, 54,
  ...
  RunQOrdered_LayerNorm_RowMajor({batch, sequence, hidden}, -1, 0.00001f, vec_x, scale_x, gamma_fp16, &beta_fp16, scale_y, vec_y);

  RunQOrdered_LayerNorm_RowMajor({batch, sequence, hidden}, -1, 0.00001f, vec_x, scale_x, gamma_fp32, &beta_fp32, scale_y, vec_y);
}
