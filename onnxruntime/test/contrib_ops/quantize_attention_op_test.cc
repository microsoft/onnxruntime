...
static void RunQAttentionDML(
  ...
  if (enable_dml) {
    quantization::Params<uint8_t> input_quant_params(/*scale=*/0.0f, /*zero_point=*/0);
    quantization::Params<uint8_t> weights_quant_params(/*scale=*/0.0f, /*zero_point=*/0);
    if (use_special_quantize_parameter) {
      input_quant_params.scale = 0.1f;
      weights_quant_params.scale = 0.1f;
      input_quant_params.zero_point = 128;
...
TEST(QAttentionTest, QAttentionPastState_DML_u8u8) {
  ...
  // 2 * 1 *2 * 4 * 2
  std::vector<float> present_data = {
      0.1f, -0.2f, 0.3f, 1.0f, 3.28f, 3.24f, 0.289f, -0.4f,
      1.1f, 0.3f, 0.5f, 0.2f, 2.5f, 5.16f, -0.52f, -1.0f,
      0.3f, -0.6f, 1.5f, 2.0f, 8.69f, -0.13f, -4.09f, 0.42f,
      0.5f, 0.1f, 0.4f, 1.6f, 4.25f, 5.65f, -0.11f, 0.569f};
        //                                   0.5f, 0.1f, 0.4f, 1.6f, 0.1f, -0.2f, 0.3f, 1.0f, 1.1f, 0.3f, 0.5f, 0.2f, 0.3f, -0.6f, 1.5f, 2.0f,
        //                                   0.5f, 0.1f, 0.4f, 1.6f};

  std::vector<float> output_data = {
      3.4731404781341553f, 0.5858999490737915f, 4.25f, 5.6499996185302734f,
      2.4670341014862061f, 0.51842659711837769f, 4.2499995231628418f, 5.6499991416931152f};
  RunQAttentionU8U8(input_data, weight_data, bias_data, mask_index_data, past_data, present_data, output_data,
                    batch_size, sequence_length, hidden_size, number_of_heads, past_sequence_length, true, false);
  RunQAttentionDML(input_data, weight_data, bias_data, mask_index_data, past_data, present_data, output_data,
                   batch_size, sequence_length, hidden_size, number_of_heads, past_sequence_length, true, false);
...
TEST(QAttentionTest, QAttentionPastState_Unidirectional_DML_u8u8) {
  int batch_size = 1;
  int sequence_length = 2;
  int past_sequence_length = 2;
  int hidden_size = 4;
  ...
  std::vector<float> output_data = {
      ...
      2.4670341014862061f, 0.51842659711837769f, 4.2499995231628418f, 5.6499991416931152f};
  RunQAttentionU8U8(input_data, weight_data, bias_data, mask_index_data, past_data, present_data, output_data,
                    batch_size, sequence_length, hidden_size, number_of_heads, past_sequence_length, true, true);
  RunQAttentionDML(input_data, weight_data, bias_data, mask_index_data, past_data, present_data, output_data,
                   batch_size, sequence_length, hidden_size, number_of_heads, past_sequence_length, true, true);
}
