...
static std::unique_ptr<IExecutionProvider> WebGpuEPWithTurboQuant4() {
  ...
  ORT_THROW_IF_ERROR(config_options.AddConfigEntry("ep.webgpuexecutionprovider.storageBufferCacheMode", "disabled"));
  ORT_THROW_IF_ERROR(config_options.AddConfigEntry("ep.webgpuexecutionprovider.turboQuant", "4"));
...
static std::vector<float> RunGQATurboQuant(
  ...
  tester.SetCustomOutputVerifier([output_size](const std::vector<OrtValue>& fetches,
    ...
    ASSERT_TRUE(fetches[0].IsTensor());
    const auto& out_tensor = fetches[0].Get<Tensor>();
    EXPECT_EQ(out_tensor.Shape().Size(), static_cast<int64_t>(output_size));
  });
...
TEST(GroupQueryAttentionTest, WebGPU_TurboQuant_RejectsNonPowerOf2HeadSize) {
  ...
  execution_providers.push_back(WebGpuEPWithTurboQuant4());
  tester.Run(OpTester::ExpectResult::kExpectFailure,
             "TurboQuant requires head_size >= 8 and a power of 2",
             {}, nullptr, &execution_providers);
}
...
// Multi-batch decode to test batch stride calculations with TQ.
TEST(GroupQueryAttentionTest, WebGPU_TurboQuant_Decode_MultiBatch_K24) {
  auto ep = WebGpuEPWithTurboQuant4();
  if (!ep) {
    GTEST_SKIP() << "WebGPU EP not available";
  }
  auto output = RunGQATurboQuant(/*batch_size=*/2, /*sequence_length=*/1, /*past_seq_len=*/24,
                                 /*num_heads=*/2, /*kv_num_heads=*/1, /*head_size=*/128,
                                 /*do_rotary=*/false, /*is_packed_qkv=*/false);
  bool all_zero = std::all_of(output.begin(), output.end(), [](float v) { return v == 0.0f; });
  EXPECT_FALSE(all_zero) << "TurboQuant multi-batch decode K=24 output should not be all zeros";
}
