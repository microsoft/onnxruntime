...
// Tests the basic kv_empty fast path in group_query_attention.cc.
TEST(GroupQueryAttentionTest, SharedKV_EmptyKV_WithPast_WebGPU) {
...
// WebGPU: kv_sequence_length=0 with past, GQA ratio 8:1.
// Tests head expansion (n_reps) with shared KV on WebGPU flash attention.
TEST(GroupQueryAttentionTest, SharedKV_EmptyKV_WithPast_GQARatio8_WebGPU) {
  auto webgpu_ep = DefaultWebGpuExecutionProvider();
  if (!webgpu_ep) {
    GTEST_SKIP() << "WebGPU EP not available";
  }
  ...
  ExpectOutputsMatch(webgpu_output, cpu_output, 0.15f, "SharedKV_GQA8_WebGPU_vs_CPU");
}

// WebGPU: kv_sequence_length=0 with past, prompt phase (q_seq_len > 1).
// Tests multi-token Q with shared KV on WebGPU.
TEST(GroupQueryAttentionTest, SharedKV_EmptyKV_WithPast_Prompt_WebGPU) {
  auto webgpu_ep = DefaultWebGpuExecutionProvider();
  if (!webgpu_ep) {
    GTEST_SKIP() << "WebGPU EP not available";
  }
...
// WebGPU: shared KV with do_rotary=1 (Q-only RoPE path), cross-checked against CPU.
// Tests the dummy-K rotary kernel path in group_query_attention.cc.
TEST(GroupQueryAttentionTest, SharedKV_EmptyKV_WithPast_Rotary_WebGPU) {
  auto webgpu_ep = DefaultWebGpuExecutionProvider();
  if (!webgpu_ep) {
    GTEST_SKIP() << "WebGPU EP not available";
  }
