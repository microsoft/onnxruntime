...
#ifdef USE_CUDA
TEST(SparseAttentionIndexerTest, QsaDecodeCudaGraphCaptureAndReplay) {
  ...
  contrib::cuda::SparseAttentionIndexerParams params;
  params.batch_size = 1;
  ...
  ASSERT_EQ(cudaSuccess, cudaMalloc(&query, kNumHeads * kHeadSize * sizeof(float)));
  ASSERT_EQ(cudaSuccess, cudaMalloc(&key, kHeadSize * sizeof(float)));
  ASSERT_EQ(cudaSuccess, cudaMalloc(&weight, kHeadSize * sizeof(float)));
  ASSERT_EQ(cudaSuccess, cudaMalloc(&cosine, kContextLength * kHeadSize * sizeof(float)));
  ASSERT_EQ(cudaSuccess, cudaMalloc(&sine, kContextLength * kHeadSize * sizeof(float)));
  ASSERT_EQ(cudaSuccess, cudaMalloc(&mask, kContextLength * sizeof(bool)));
  ASSERT_EQ(cudaSuccess, cudaMalloc(&key_cache, kContextLength * kHeadSize * sizeof(float)));
  ASSERT_EQ(cudaSuccess, cudaMalloc(&selected, params.capacity * sizeof(int32_t)));
  ASSERT_EQ(cudaSuccess, cudaMalloc(&float_workspace,
                                    contrib::cuda::GetQsaWorkspaceFloatCount(params) * sizeof(float)));
  ASSERT_EQ(cudaSuccess, cudaMalloc(&int_workspace,
                                    contrib::cuda::GetQsaWorkspaceIntCount(params) * sizeof(int32_t)));
  ...
  const auto launch = [&]() {
    return contrib::cuda::LaunchQsaSparseAttentionIndexer<float>(
