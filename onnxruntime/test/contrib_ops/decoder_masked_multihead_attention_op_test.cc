...
static void MergeVCacheWithV(T* v_cache,
                             ...
                             int head_size) {
  int num_elements = batch_size * num_heads * 1 * head_size;
  int iter = past_sequence_length * head_size;
  ...
  while (num_elements > 0) {
    ...
    num_elements -= head_size;
    v += head_size;
    iter += max_sequence_length * head_size;
  }
}
      ...
      for (int s = 0; s < total_seq_length; ++s) {
        for (int h = 0; h < head_size; ++h) {
          ...
          if (s < past_sequence_length) {
            ...
            int input_offset = b * num_heads * 1 * head_size + (n * 1 * head_size) + h;
...
TEST(DecoderMaskedSelfAttentionTest, SmallSequences_fp32) {
  /*
  int batch_size = 1;
  ...
  */
  ...
  for (int batch_size = 1; batch_size <= 4; ++batch_size) {
    for (int past_sequence_length = 1; past_sequence_length <= 30; past_sequence_length += 2) {
      int sequence_length = 1;
      int hidden_size = 768;
      int number_of_heads = 12;
      int head_size = (hidden_size / number_of_heads);
      int total_sequence_length = sequence_length + past_sequence_length;
      int max_sequence_length = past_sequence_length + 4;
      ...
      auto k_transpose = MergePastKWithPresentKAndTranspose<float>(kv_cache.data(), qkv.data() + hidden_size, batch_size,
                                                                   number_of_heads, past_sequence_length, max_sequence_length, head_size);

      auto qk_transpose = QK_Transpose<float>(qkv.data(), k_transpose.data(), batch_size, number_of_heads,
      ...
      // MergeVCacheWithV<float>(present.data() + k_cache_size, qkv.data() + 2 * hidden_size, batch_size,
      //                         number_of_heads, past_sequence_length, max_sequence_length, head_size);
      ...
      // Output(s)
      tester.AddOutput<float>("output", {batch_size, sequence_length, hidden_size}, output);
...
TEST(DecoderMaskedSelfAttentionTest, MediumSequences_fp32) {
  ...
  int max_sequence_length = 4;
      ...
      int head_size = (hidden_size / number_of_heads);
      int total_sequence_length = sequence_length + past_sequence_length;
      int max_sequence_length = past_sequence_length + 10; // Always keep >  past_sequence_length
  */

  for (int batch_size = 2; batch_size <= 2; ++batch_size) {
    for (int past_sequence_length = 352; past_sequence_length <= 352; past_sequence_length += 160) {
      int sequence_length = 1;
      int hidden_size = 768;
      int number_of_heads = 12;
      int head_size = (hidden_size / number_of_heads);
      ...
      auto bias = CreateValues<float>(3 * hidden_size, 202);
      tester.AddInput<float>("bias", bias_dims, bias);
...
TEST(DecoderMaskedSelfAttentionTest, LargeSequences_fp32) {
  /*
  int batch_size = 1;
  int sequence_length = 1;
  int hidden_size = 128;
  ...
  */
  ...
  for (int batch_size = 1; batch_size <= 1; ++batch_size) {
    for (int past_sequence_length = 2048; past_sequence_length <= 2048; past_sequence_length += 160) {
      int sequence_length = 1;
      int hidden_size = 768;
      int number_of_heads = 12;
      int head_size = (hidden_size / number_of_heads);
