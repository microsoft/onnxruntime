...
void RunBeamSearchTest(const ORTCHAR_T* test_onnx_path) {
  ...
  if (enable_cuda || enable_rocm) {
    ...
    Ort::Session session(*ort_env, test_onnx_path, session_options);
...
TEST(BeamSearchTest, GptBeamSearchFp32) {
  ...
  //        --output tiny_gpt2_beamsearch.onnx --use_gpu --max_length 20
  // (with separate_gpt2_decoder_for_init_run set to False as it is now set to True by default)
  RunBeamSearchTest(ORT_TSTR("testdata/transformers/tiny_gpt2_beamsearch.onnx"));
}
...
TEST(BeamSearchTest, GptBeamSearchWithInitDecoderFp16) {
  ...
  RunBeamSearchTest(ORT_TSTR("testdata/transformers/tiny_gpt2_beamsearch_with_init_decoder_fp16.onnx"));
}
...
TEST(BeamSearchTest, GptBeamSearchFp16_VocabPadded) {
  ...
  // such a scenario
  RunBeamSearchTest(ORT_TSTR("testdata/transformers/tiny_gpt2_beamsearch_fp16_padded_vocab.onnx"));
}
