...
def create_qordered_attention_graph():
    ...
    nodes = [
        helper.make_node('QuantizeWithOrder', inputs=['weight', 'scale_weight_fp16'], outputs=['weight_s8_COL32_2R_4R4'], name='764_QuantizeWithOrder', domain='com.microsoft', order_input=0, order_output=4),
        helper.make_node('QuantizeWithOrder', inputs=['bias', 'scale_bias_fp16'], outputs=['bias_s8_COL32'], name='769_QuantizeWithOrder', domain='com.microsoft', order_input=0, order_output=2),
        helper.make_node(
            'QOrderedAttention',
...
ort_inputs = {
    'input_s8_COL32' : numpy.random.randint(-127, 128, 8*768, dtype=numpy.INT8),
    'mask_index' : numpy.random.randint(1, 2, 8, dtype=numpy.INT32)
}
