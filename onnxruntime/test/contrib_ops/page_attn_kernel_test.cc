...
std::unique_ptr<T, std::function<void(T*)>> move_to_cuda(const std::vector<T>& cpu_vec) {
  ...
  hipMalloc(&dev_ptr, sizeof(T) * cpu_vec.size());
  hipMemcpy(dev_ptr, cpu_vec.data(), sizeof(T) * cpu_vec.size(), hipMemcpyHostToDevice);
...
void back_to_cpu(const T* dev_ptr, std::vector<T>& cpu_vec) {
  hipMemcpy(cpu_vec.data(), dev_ptr, sizeof(T) * cpu_vec.size(), hipMemcpyDeviceToDevice);
}
...
void CudaQuantizeReshapeAndCache(
  ...
  onnxruntime::contrib::rocm::reshape_and_cache(
      nullptr,
      key_dev.get(),
      value_dev.get(),
      k_cache_dev.get(),
  ...
  ASSERT_TRUE(hipPeekAtLastError() == hipSuccess);
