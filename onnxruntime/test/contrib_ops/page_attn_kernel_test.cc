...
#include "onnxruntime/contrib_ops/cuda/bert/paged_attention_impl.h"
    ...
    for (int h = 0; h < num_heads; h++) {
      for (int i = 0; i < head_size; i += kv_quant_chunk_size) {
        ...
        for (int j = 0; j < kv_quant_chunk_size; j++) {
          ...
          *dst_k = (int8_t)std::max(-128, std::min(127, std::nearbyintf((float)k[j] * inv_unit_scale_k)));
          *dst_v = (int8_t)std::max(-128, std::min(127, std::nearbyintf((float)v[j] * inv_unit_scale_v)));
        }
...
void CudaQuantizeReshapeAndCache(
    ...
    std::vector<int8_t>& k_cache,            // [num_blocks, num_heads, head_size/x, block_size, x]
    std::vector<int8_t>& v_cache,            // [num_blocks, num_heads, head_size, block_size]
    std::vector<ScaleT>& kv_params_cache)
{  // [num_blocks, 2, num_heads, head_size / kv_quant_chunk_size, block_size]
...
void check_eq(std::vector<T>& gold, std::vector<T>& r, T threshold) {
};

void check_eq(std::vector<int8_t>& gold, std::vector<int8_t>& r, int threshold = 1) {
    ASSERT_EQUAL(gold.size(), r.size());
...
void check_eq(std::vector<MLFloat16>& gold, std::vector<MLFloat16>& r, float abs_err, float rel_err) {
    ...
    for (size_t i = 0; i < gold.size(); i++) {
        ...
        EXPECT_NEAR((float)gold[i], (float)r[i], rel_err * std::fabsf((float)gold[i])) << " at:" << i;
    }
}
...
void RunQuantizeReshapeAndCacheKernelTest(
  ...
  std::vector<MLFloat16> key = random.Uniform<MLFloat16>(kv_shape, MLFloat16{-2.0f}, MLFloat16{2.0f});
  std::vector<MLFloat16> value = random.Uniform<MLFloat16>(kv_shape, MLFloat16{-2.0f}, MLFloat16{2.0f});
  ...
  int key_stride = num_heads * head_size * block_size;
  int value_stride = num_heads * head_size * block_size;
  ...
  CudaQuantizeReshapeAndCache(
    ...
    check_eq(kv_params_cache_erf, kv_params_cache, 0.001f, 0.05f);
};
