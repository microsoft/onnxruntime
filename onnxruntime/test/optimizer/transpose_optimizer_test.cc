...
// Unshared Transpose -> Relu -> Relu, with the second Relu fanning out to a merge Transpose and a dead-end. Immediate
// consumers of the first Relu look covered, but walking through the second Relu finds a stranded branch, so the push
// must not happen.
TEST(TransposeOptimizerTests, TestUnsharedTransposeMergeThroughUnaryThenFanout) {
  auto build_test_case = [&](ModelTestBuilder& builder) {
    auto* input0_arg = builder.MakeInput<float>({4, 6, 10}, 0.0, 1.0);
    auto* sigmoid_out = builder.MakeIntermediate();
    auto* transpose_1_out = builder.MakeIntermediate();
    auto* relu_1_out = builder.MakeIntermediate();
    auto* relu_2_out = builder.MakeIntermediate();

    builder.AddNode("Sigmoid", {input0_arg}, {sigmoid_out});
    auto& transpose_1 = builder.AddNode("Transpose", {sigmoid_out}, {transpose_1_out});
    transpose_1.AddAttribute("perm", std::vector<int64_t>{1, 2, 0});
    builder.AddNode("Relu", {transpose_1_out}, {relu_1_out});
    builder.AddNode("Relu", {relu_1_out}, {relu_2_out});
    auto& transpose_2 = builder.AddNode("Transpose", {relu_2_out}, {builder.MakeOutput()});
    transpose_2.AddAttribute("perm", std::vector<int64_t>{1, 2, 0});
    builder.AddNode("Relu", {relu_2_out}, {builder.MakeOutput()});
  };
...
TEST(TransposeOptimizerTests, TestSplitUnsharedTransposeTwoMerges) {
  auto build_test_case = [&](ModelTestBuilder& builder) {
    ...
    transpose_2.AddAttribute("perm", std::vector<int64_t>{1, 2, 0});
    auto& transpose_3 = builder.AddNode("Transpose", {split_out_1}, {builder.MakeOutput()});
    transpose_3.AddAttribute("perm", std::vector<int64_t>{1, 2, 0});
  };

  auto check_optimized_graph = [&](InferenceSessionWrapper& session) {
    const Graph& graph = session.GetGraph();
    std::map<std::string, int> op_to_count = CountOpsInGraph(graph);
    ASSERT_EQ(op_to_count["Transpose"], 2) << "incoming Transpose should merge into both Split outputs";
    ...
    for (const auto& node : graph.Nodes()) {
      ...
      const Node* producer = graph.GetProducerNode(node.InputDefs()[0]->Name());
      ASSERT_NE(producer, nullptr);
      EXPECT_EQ(producer->OpType(), "Split");
      ++transposes_after_split;
    }
    EXPECT_EQ(transposes_after_split, 2);
  };
