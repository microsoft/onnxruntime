...
TEST(TransposeOptimizerTests, LayoutTransformConvRecoverRankFromWeight) {
  ...
  Model model("LayoutTransformConvRecoverRankFromWeight", false, ModelMetaData(), PathString(),
              IOnnxRuntimeOpSchemaRegistryList(), domain_to_version, {},
              DefaultLoggingManager().DefaultLogger());
  Graph& graph = model.MainGraph();
  ModelTestBuilder builder(graph);
  ...
  // Weight has known shape [8, 3, 3, 3] - rank 4.
  auto* weight_arg = builder.MakeInitializer<float>({8, 3, 3, 3}, -1.0f, 1.0f);
  auto* output_arg = builder.MakeOutput();

  auto& conv = builder.AddNode("Conv", {input_arg, weight_arg}, {output_arg});
  conv.AddAttribute("pads", std::vector<int64_t>{1, 1, 1, 1});
  conv.AddAttribute("strides", std::vector<int64_t>{1, 1});
  conv.AddAttribute("kernel_shape", std::vector<int64_t>{3, 3});
...
TEST(TransposeOptimizerTests, LayoutTransformConvTransposeRecoverRankFromWeight) {
  std::unordered_map<std::string, int> domain_to_version{{kOnnxDomain, 13}};
  Model model("LayoutTransformConvTransposeRecoverRankFromWeight", false, ModelMetaData(), PathString(),
              IOnnxRuntimeOpSchemaRegistryList(), domain_to_version, {},
              DefaultLoggingManager().DefaultLogger());
  Graph& graph = model.MainGraph();
  ModelTestBuilder builder(graph);
  ...
  // Weight has known shape [3, 8, 3, 3] - rank 4.
  auto* weight_arg = builder.MakeInitializer<float>({3, 8, 3, 3}, -1.0f, 1.0f);
  auto* output_arg = builder.MakeOutput();

  auto& conv_transpose = builder.AddNode("ConvTranspose", {input_arg, weight_arg}, {output_arg});
  conv_transpose.AddAttribute("pads", std::vector<int64_t>{1, 1, 1, 1});
  conv_transpose.AddAttribute("strides", std::vector<int64_t>{1, 1});
  conv_transpose.AddAttribute("kernel_shape", std::vector<int64_t>{3, 3});
