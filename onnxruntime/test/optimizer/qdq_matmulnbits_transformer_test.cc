...
// Test: Activation Q→DQ + per-tensor weight DQ → MatMul, with qdq_float_activations enabled.
// This reproduces the scenario from fully-quantized models where both activations and weights
// are quantized. The qdq_float_activations option removes activation Q→DQ pairs, and then
// QDQFloatActivationsTransformer's sub-pass B fuses the remaining weight DQ→MatMul into MatMulNBits.
//
//  Input(float) → Q → DQ (activation) → MatMul → output
//                                          ↑
//            Weight (quantized) → DQ (per-tensor, weight) ─┘
template <typename WeightType, bool use_zp>
void RunDQMatMulWithFloatActivations(const std::vector<int64_t>& input1_shape,
                                     const std::vector<int64_t>& weight_shape,
  ...
  auto build_test_case = [&](ModelTestBuilder& builder) {
    ...
    // Activation: Q → DQ (UINT8 per-tensor)
    auto* activation_dq_output = AddQDQNodePair<uint8_t>(builder, input_arg, 0.01f, uint8_t(128));

    // Weight: DQ (per-tensor quantized)
    auto* weight_arg = builder.MakeInitializer(weight_shape, WeightType(WeightType::min_val, 0),
                                               WeightType(WeightType::max_val, 0));
    auto* weight_dq_output = builder.MakeIntermediate();
    auto* scale_arg = builder.MakeInitializer<float>({}, {0.05f});
    if constexpr (use_zp) {
      auto* zp_arg = builder.MakeInitializer(std::vector<int64_t>{}, WeightType(1, 0), WeightType(1, 0));
      builder.AddNode("DequantizeLinear", {weight_arg, scale_arg, zp_arg}, {weight_dq_output});
    } else {
      builder.AddNode("DequantizeLinear", {weight_arg, scale_arg}, {weight_dq_output});
    }

    builder.AddNode("MatMul", {activation_dq_output, weight_dq_output}, {output_arg});
  };

  auto check_graph = [&](InferenceSessionWrapper& session) {
    auto op_to_count = CountOpsInGraph(session.GetGraph());
    const QDQOpKeys qdq_keys = GetQDQOpKeys(false);
    EXPECT_EQ(op_to_count["MatMul"], 0) << "MatMul should be fused into MatMulNBits";
    EXPECT_EQ(op_to_count["com.microsoft.MatMulNBits"], 1);
    EXPECT_EQ(op_to_count[qdq_keys.quantize_linear], 0) << "Activation Q should be removed";
    EXPECT_EQ(op_to_count[qdq_keys.dequantize_linear], 0) << "All DQ should be removed";
  };

  auto add_session_options_fn = [&block_size_option](SessionOptions& sess_opts) {
    std::ignore = sess_opts.config_options.AddConfigEntry(kOrtSessionOptionsQDQFloatActivations, "1");
    if (!block_size_option.empty()) {
      std::ignore = sess_opts.config_options.AddConfigEntry(kOrtSessionOptionsQDQMatMulNBitsBlockSize,
                                                            block_size_option.c_str());
    }
  };

  // Tolerance is large because the baseline model includes activation Q->DQ noise
  // while the optimized model removes it (the whole point of qdq_float_activations).
  TransformerTester(build_test_case,
                    check_graph,
                    TransformerLevel::Level1,
                    TransformerLevel::Level2,
                    21 /*opset_version*/,
                    0.5 /*per_sample_tolerance*/,
                    0.1 /*relative_per_sample_tolerance*/,
                    nullptr,
                    add_session_options_fn);
}

TEST(QDQTransformerTests, DQMatMulWithFloatActivationsToMatMulNBits) {
  // 4-bit weights with activation Q→DQ
  RunDQMatMulWithFloatActivations<Int4x2, true>({12, 32}, {32, 16});
  RunDQMatMulWithFloatActivations<UInt4x2, false>({12, 32}, {32, 16});
...
template <bool use_zp>
void RunDQMatMulUint8WithFloatActivations(const std::vector<int64_t>& input1_shape,
                                          const std::vector<int64_t>& weight_shape,
                                          const std::string& block_size_option = "") {
  auto build_test_case = [&](ModelTestBuilder& builder) {
    auto* input_arg = builder.MakeInput(input1_shape, -1.0f, 1.0f);
    ...
    // Weight: DQ (UINT8 per-tensor quantized)
    auto* weight_arg = builder.MakeInitializer<uint8_t>(weight_shape, uint8_t(0), uint8_t(255));
    auto* weight_dq_output = builder.MakeIntermediate();
    auto* scale_arg = builder.MakeInitializer<float>({}, {0.05f});
    if constexpr (use_zp) {
  ...
  auto check_graph = [&](InferenceSessionWrapper& session) {
    // With UINT8 activation Q→DQ + UINT8 weight DQ, the first-pass QDQ transformer typically
    // fuses the full pattern into MatMulIntegerToFloat (both DQ inputs are UINT8).
    // The float_activations transformer then has nothing to do. This is correct behavior —
    // this test just verifies the graph loads and optimizes without crashing.
    (void)session;
  };
...
TEST(QDQTransformerTests, DQMatMulUint8WithFloatActivationsToMatMulNBits) {
  // UINT8 weights with activation Q→DQ — mimics real model pattern
  RunDQMatMulUint8WithFloatActivations<true>({12, 768}, {768, 768});
