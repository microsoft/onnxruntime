...
// Regression test: LayerNormFusion with zero-element epsilon initializer must not crash (SIGSEGV).
// A zero-element constant (shape [0], 0 bytes) is valid ONNX but caused out-of-bounds read
// when the optimizer dereferenced data<float>()[0] without checking size().
TEST_F(GraphTransformationTests, LayerNormFusion_ZeroElementEpsilon_NoCrash) {
  ...
  opset->set_version(17);  // Use opset 17+ for ReduceMean with 'axes' as attribute
  ...
  auto load_status = Model::Load(model_proto, model, nullptr, *logger_);
  if (!load_status.IsOK()) {
    // If model can't load, the bug path isn't reached - that's fine.
    return;
  }
  ...
  onnxruntime::GraphTransformerManager graph_transformation_mgr{5};
  ASSERT_STATUS_OK(graph_transformation_mgr.Register(std::make_unique<LayerNormFusion>(), TransformerLevel::Level1));
  // Must not crash (SIGSEGV).
  auto status = graph_transformation_mgr.ApplyTransformers(graph, TransformerLevel::Level1, *logger_);
  (void)status;
}

// Regression test: SimplifiedLayerNormFusion with zero-element epsilon initializer must not crash.
TEST_F(GraphTransformationTests, SimplifiedLayerNormFusion_ZeroElementEpsilon_NoCrash) {
  ONNX_NAMESPACE::ModelProto model_proto;
  model_proto.set_ir_version(8);
  auto* opset = model_proto.add_opset_import();
