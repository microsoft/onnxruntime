...
TEST_F(GraphTransformationTests, ConstantFoldingSubgraph) {
  ...
  auto create_subgraph = [&](GraphProto& graph_proto) {
    ...
    Model model("ConstantFoldingSubgraphTest_subgraph", false, ModelMetaData(), PathString(),
                IOnnxRuntimeOpSchemaRegistryList(), {{kOnnxDomain, 12}}, {}, *logger_);
  ...
  Model model("ConstantFoldingSubgraphTest_main_graph", false, ModelMetaData(), PathString(),
              IOnnxRuntimeOpSchemaRegistryList(), {{kOnnxDomain, 12}}, {}, *logger_);
  auto& graph = model.MainGraph();
...
TEST_F(GraphTransformationTests, ConstantFoldingShape_Shape15CannotFold) {
  /*
  ...
  */
  ...
  auto pre_graph_checker = [&](Graph& graph) {
    ...
    ASSERT_EQ(op_to_count["Identity"], 2);
  ...
  auto post_graph_checker = [&](Graph& graph) {
    ...
    ASSERT_FALSE(optimizer_utils::AppendTensorFromInitializer(graph, *input_arg, shape_values, true));
  };
...
TEST_F(GraphTransformationTests, ConstantFoldingShape_Shape15TakesGraphInput) {
  /*
   [attention_mask1_dim0,512,1536]
                  |
                Shape15
...
TEST_F(GraphTransformationTests, ConstantFoldingShape_Shape15GeneratesGraphOutput) {
  ...
  */
  ...
  auto post_graph_checker = [&](Graph& graph) {
    ...
    ASSERT_TRUE(!shape_output_name.empty());
...
TEST_F(GraphTransformationTests, ConstantFoldingShape_Gather) {
  /*
...
TEST_F(GraphTransformationTests, ConstantFoldingShape_ConcreteDimUsedBySlice) {
  ...
  */
  auto pre_graph_checker = [&](Graph& graph) {
    ...
    ASSERT_EQ(op_to_count["Slice"], 2);
    ASSERT_EQ(op_to_count["Squeeze"], 2);
  ...
  auto post_graph_checker = [&](Graph& graph) {
    ...
    ASSERT_EQ(op_to_count["com.microsoft.ConcatTraining"], 0);
    ASSERT_EQ(op_to_count["Shape"], 0);
    ASSERT_EQ(op_to_count["Reshape"], 1);
...
TEST_F(GraphTransformationTests, ConstantFoldingShape_ConcreteDimUsedByGatherSlice) {
  /*
  ...
  auto post_graph_checker = [&](Graph& graph) {
    ...
    for (auto& node : graph.Nodes()) {
      if (node.OpType() == "Reshape") {
        ...
        InlinedVector<int64_t> shape_values;
        ASSERT_TRUE(!optimizer_utils::AppendTensorFromInitializer(graph, *shape_input, shape_values, true));
        ASSERT_TRUE(graph.GetProducerNode(node.MutableInputDefs()[1]->Name())->OpType().compare("ConcatTraining") == 0);
