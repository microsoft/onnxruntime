...
TEST_F(GraphTransformationTests, RuleBasedGraphTransformerInsideFunction) {
  auto model_uri = MODEL_FOLDER "cast_elim_inside_func.onnx";
  std::shared_ptr<Model> model;
  ASSERT_TRUE(Model::Load(model_uri, model, nullptr, *logger_).IsOK());
  Graph& graph = model->MainGraph();

  const std::function<int(const Graph& graph)> count_cast_ops = [&](const Graph& graph) {
    int count = 0;
    for (auto& node : graph.Nodes()) {
      if (node.GetFunctionBody() != nullptr) {
        count += count_cast_ops(node.GetFunctionBody()->Body());
      }
      if (node.OpType() == "Cast") {
        count++;
      }
  ...
  ASSERT_TRUE(count_cast_ops(graph) == 3);
  ...
  ASSERT_TRUE(count_cast_ops(graph) == 2);
}
