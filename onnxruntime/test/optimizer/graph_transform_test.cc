...
TEST_F(GraphTransformationTests, ConstantFolding) {
  ...
  onnxruntime::GraphTransformerManager graph_transformation_mgr{5};
  std::map<OrtDevice, AllocatorPtr> allocators{{OrtDevice(), e->CreatePreferredAllocators()[0]}};
...
TEST_F(GraphTransformationTests, FuseConvBnAddMulFloat16) {
  ...
  CreateMLValue<MLFloat16>(std::make_shared<CPUAllocator>(),
...
TEST_F(GraphTransformationTests, BiasGeluSwitchedInputOrder) {
  ...
  CreateMLValue<float>(std::make_shared<CPUAllocator>(), dims_b_i,
  ...
  CreateMLValue<float>(std::make_shared<CPUAllocator>(), dims_a_i,
