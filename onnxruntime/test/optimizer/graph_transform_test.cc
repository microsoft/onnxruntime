...
TEST_F(GraphTransformationTests, NchwcTransformerConv) {
  auto build_test_case = [&](ModelTestBuilder& builder) {
    ...
    builder.AddNode("Reshape", {conv_output_arg, reshape_shape_after}, {output_arg});
  };
  ...
  auto pre_graph_checker = [](Graph& graph) -> Status {
    ...
    EXPECT_EQ(CountOpsInGraph(graph)["Conv"], 1);
    return Status::OK();
  };
...
TEST_F(GraphTransformationTests, NchwcTransformerConvUnsupportedActivation) {
  auto build_test_case = [&](ModelTestBuilder& builder) {
    auto* input_arg = builder.MakeInput<float>({{1, 16, 49}});
    auto* einsum_weight_arg = builder.MakeInitializer<float>({1}, {1.0f});
    auto* conv_weight_arg = builder.MakeInitializer<float>({32, 16, 1, 1}, std::vector<float>(32 * 16, 1.0f));
    auto* conv_output_arg = builder.MakeIntermediate();
    auto* elu_output_arg = builder.MakeIntermediate();
    auto* reshape_before_conv_out = builder.MakeIntermediate();
    auto* einsum_out = builder.MakeIntermediate();
    auto* output_arg = builder.MakeOutput();
    ...
    builder.AddNode("Reshape", {einsum_out, reshape_shape_before}, {reshape_before_conv_out});
    builder.AddNode("Conv", {reshape_before_conv_out, conv_weight_arg}, {conv_output_arg});
    builder.AddNode("Elu", {conv_output_arg}, {elu_output_arg});
    auto* reshape_shape_after = builder.MakeInitializer<int64_t>({3}, {1, 32, 49});
    builder.AddNode("Reshape", {elu_output_arg, reshape_shape_after}, {output_arg});
  };
...
TEST_F(GraphTransformationTests, NchwcTransformerMaxPool) {
  auto build_test_case = [&](ModelTestBuilder& builder) {
    auto* input_arg = builder.MakeInput<float>({{1, 16, 49}});
    auto* einsum_weight_arg = builder.MakeInitializer<float>({1}, {1.0f});
    auto* pool_output_arg = builder.MakeIntermediate();
    auto* reshape_before_pool_out = builder.MakeIntermediate();
    auto* einsum_out = builder.MakeIntermediate();
    auto* output_arg = builder.MakeOutput();
    ...
    einsum_node.AddAttribute("equation", "abc,d->abc");
    auto* reshape_shape_before = builder.MakeInitializer<int64_t>({4}, {1, 16, 7, 7});
    builder.AddNode("Reshape", {einsum_out, reshape_shape_before}, {reshape_before_pool_out});
    auto& pool_node = builder.AddNode("MaxPool", {reshape_before_pool_out}, {pool_output_arg});
    pool_node.AddAttribute("kernel_shape", std::vector<int64_t>{2, 2});
    pool_node.AddAttribute("strides", std::vector<int64_t>{2, 2});
    auto* reshape_shape_after = builder.MakeInitializer<int64_t>({3}, {1, 16, 9});
    builder.AddNode("Reshape", {pool_output_arg, reshape_shape_after}, {output_arg});
  };
  ...
  auto pre_graph_checker = [](Graph& graph) -> Status {
    ...
    EXPECT_EQ(CountOpsInGraph(graph)["MaxPool"], 1);
    return Status::OK();
  };

  auto post_graph_checker = [](Graph& graph) -> Status {
    auto op_counts = CountOpsInGraph(graph);
    EXPECT_EQ(op_counts["MaxPool"], 0);
    EXPECT_EQ(op_counts["com.microsoft.nchwc.ReorderInput"], 1);
    EXPECT_EQ(op_counts["com.microsoft.nchwc.MaxPool"], 1);
    EXPECT_EQ(op_counts["com.microsoft.nchwc.ReorderOutput"], 1);
    return Status::OK();
  };
