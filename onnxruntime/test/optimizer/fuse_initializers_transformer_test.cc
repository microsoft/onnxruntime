...
void test_graph_structure_after_session_init_without_graph_optimization_loop(const Graph& graph) {
  ...
  EXPECT_TRUE(2 == num_nodes_fp32);
  // Check if all conditions are met
  ASSERT_TRUE((2 == op_to_count["Cast"]) && (0 == num_initializers_fp16)  // Unable to calc no. of fp16/fp32 initializers, as when session
              && (0 == num_initializers_fp32)                             // state is finalized after init it removed initializers from graph.
              && (1 == num_nodes_fp16)                                    // Look for "session_state_->FinalizeSessionState" method in
              && (2 == num_nodes_fp32));                                  // inference_session.cc for more explanation.
}
...
void test_graph_structure_after_session_init_with_graph_optimization_loop(const Graph& graph) {
  ...
  // Count no. of FP32 nodes
  auto num_nodes_fp32 = CountNoOfNodesInGraph(graph, DataTypeImpl::GetTensorType<float>());
  EXPECT_TRUE(3 == num_nodes_fp32);
  // Check if all conditions are met
  ASSERT_TRUE((2 == op_to_count["Cast"]) && (0 == num_initializers_fp16)  // Unable to calc no. of fp16/fp32 initializers, as when session
              && (0 == num_initializers_fp32)                             // state is finalized after init it removed initializers from graph.
              && (1 == num_nodes_fp16)                                    // Look for "session_state_->FinalizeSessionState" method in
              && (3 == num_nodes_fp32));                                  // inference_session.cc for more explanation.
}
