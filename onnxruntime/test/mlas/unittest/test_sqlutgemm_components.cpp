...
class MlasSQLutGemmPackTest : public MlasTestBase {
 private:
 ...
 public:
  ...
  static const char* GetTestSuiteName() {
    static std::string suite_name = std::string("SQLutGemmPack") +
...
class MlasSQLutGemmLutGenTest : public MlasTestBase {
 private:
 ...
 public:
  void Test(size_t K) {
    ...
    // trust the integration test (SQLutGemm) to find bugs in the SIMD version.
  ...
  static const char* GetTestSuiteName() {
    static std::string suite_name = std::string("SQLutGemmLutGen") + "BlkLen" + std::to_string(BlkLen);
    return suite_name.c_str();
  }
};
...
template <size_t BlkBitWidth, size_t BlkLen>
class SQLutGemmPackShortExecuteTest : public MlasTestFixture<MlasSQLutGemmPackTest<BlkBitWidth, BlkLen>> {
 public:
  explicit SQLutGemmPackShortExecuteTest(size_t N, size_t K, bool Symmetric)
      : N_(N), K_(K), Symmetric_(Symmetric) {}

  void TestBody() override {
    MlasTestFixture<MlasSQLutGemmPackTest<BlkBitWidth, BlkLen>>::mlas_tester->Test(N_, K_, Symmetric_);
  }
  ...
  static size_t RegisterSingleTest(size_t N, size_t K, bool Symmetric) {
    ...
    testing::RegisterTest(
        MlasSQLutGemmPackTest<BlkBitWidth, BlkLen>::GetTestSuiteName(),
        test_name.c_str(),
        nullptr,
        test_name.c_str(),
        __FILE__,
        __LINE__,
        [=]() -> MlasTestFixture<MlasSQLutGemmPackTest<BlkBitWidth, BlkLen>>* {
          return new SQLutGemmPackShortExecuteTest<BlkBitWidth, BlkLen>(N, K, Symmetric);
        });
...
//
template <size_t BlkLen>
class SQLutGemmLutGenShortExecuteTest : public MlasTestFixture<MlasSQLutGemmLutGenTest<BlkLen>> {
 public:
  explicit SQLutGemmLutGenShortExecuteTest(size_t K) : K_(K) {}

  void TestBody() override {
    MlasTestFixture<MlasSQLutGemmLutGenTest<BlkLen>>::mlas_tester->Test(K_);
  }
  ...
  static size_t RegisterSingleTest(size_t K) {
    ...
    testing::RegisterTest(
        MlasSQLutGemmLutGenTest<BlkLen>::GetTestSuiteName(),
        test_name.c_str(),
        nullptr,
        test_name.c_str(),
        __FILE__,
        __LINE__,
        [=]() -> MlasTestFixture<MlasSQLutGemmLutGenTest<BlkLen>>* {
          return new SQLutGemmLutGenShortExecuteTest<BlkLen>(K);
        });
...
static size_t SQLutGemmComponentsRegisterAllShortExecuteTests() {
  size_t count = 0;

  // Pack tests for 2-bit quantization with various block lengths
  count += SQLutGemmPackShortExecuteTest<2, 32>::RegisterShortExecuteTests();
  count += SQLutGemmPackShortExecuteTest<2, 64>::RegisterShortExecuteTests();
  count += SQLutGemmPackShortExecuteTest<2, 128>::RegisterShortExecuteTests();

  // LUT generation tests
  count += SQLutGemmLutGenShortExecuteTest<32>::RegisterShortExecuteTests();
  count += SQLutGemmLutGenShortExecuteTest<64>::RegisterShortExecuteTests();
  count += SQLutGemmLutGenShortExecuteTest<128>::RegisterShortExecuteTests();
...
static UNUSED_VARIABLE bool added_to_main = AddTestRegister(
    [](bool is_short_execute) -> size_t {
      if (is_short_execute) {
        return SQLutGemmComponentsRegisterAllShortExecuteTests();
      }
      return 0;
    });
