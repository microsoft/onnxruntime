...
static void QGemmSize(benchmark::internal::Benchmark* b) {
  ...
  b->Args({32, 32, 32, 1, 1});
  b->Args({64, 64, 64, 1, 1});
  b->Args({128, 128, 128, 1, 1});
  b->Args({256, 256, 256, 1, 1});
  b->Args({512, 512, 512, 1, 1});
  b->Args({1024, 1024, 1024, 1, 1});
  b->Args({2048, 2048, 2048, 1, 1});
  b->Args({4096, 4096, 4096, 1, 1});
  b->Args({8192, 8192, 8192, 1, 1});
}
