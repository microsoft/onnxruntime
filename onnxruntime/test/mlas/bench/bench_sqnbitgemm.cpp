...
void Q4GEMM_Jblas(benchmark::State& state, int block_size, bool is_asym, MLAS_SQNBIT_GEMM_COMPUTE_TYPE cmp_type) {
  ...
  const size_t pack_b_size = MlasSQNBitGemmPackBSize(N, K, 4, block_size, is_asym, cmp_type);
  ...
  MlasSQNBitGemmPackB(B1_packed.data(), B1.data(), B_scale.data(), is_asym ? B_zp.data() : nullptr, N, K, K,
