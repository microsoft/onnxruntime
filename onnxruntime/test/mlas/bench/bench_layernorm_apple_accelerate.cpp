...
// scalar baseline it replaces on macOS arm64. Unlike bench_transcendental's
// Tanh benchmark -- which compares vForce against an ALREADY NEON-vectorized
// polynomial kernel (see PR #32036, which found vForce consistently slower
// there) -- LayerNorm/RMSNorm has NO SIMD-vectorized kernel on ARM64 today:
// GetMlasPlatform().LayerNormF32Kernel is only registered for AVX2 (x86) and
// RVV (RISC-V); see core/mlas/lib/platform.cpp. Every ARM64 build without
// this option falls back to the single-element-at-a-time Welford/sum-of-
// squares scalar loop in
...
// plus a few small sizes to observe any crossover where per-call vDSP/
// function-call overhead might outweigh its reduction speedup for very
// short rows -- the same question #32036 answered (negatively, for Tanh)
// for its own kernel; this benchmark exists to answer it for LayerNorm
// with real hardware data rather than assuming an answer.
//
...
void RunLayerNormBenchmark(
  ...
  auto run_once = [&]() {
    if (use_dispatch) {
      ...
      // uses. Falls back to the scalar loop itself if no kernel is
      // registered, so this remains a valid (if uninteresting) benchmark
      // even on configurations without this option.
...
static void LayerNormBenchmarkArgs(benchmark::internal::Benchmark* b) {
  ...
  for (int n : {1, 8, 64, 256, 768, 1024, 1600, 2048, 3072, 4096, 8192}) {
    b->Arg(n);
  }
}
