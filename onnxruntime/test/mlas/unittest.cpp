...
class MlasFgemmTest : public MlasTestBase {
 private:
  ...
  Test(
    ...
    auto end = Clock::now();
    auto mlas = std::chrono::duration_cast<std::chrono::microseconds>(end - begin).count();
    printf("mkl base: %f ms,", (double)(mlas) / (double)(1000000.0));
...
#if 0
    // mkl sgemm
    std::vector<float> res1(M * N, 0);
    // warm up
    cblas_sgemm(CBLAS_LAYOUT::CblasColMajor, CBLAS_TRANSPOSE::CblasNoTrans, CBLAS_TRANSPOSE::CblasNoTrans, N, M, K, 1, B, N, A, K, , 0, res1.data(), N);
    for (size_t f = 0; f < M * N; f++) {
      // Sensitive to comparing positive/negative zero.
      if (C[f] != res1[f]) {
        printf("mkl sgemm mismatch TransA=%d, TransB=%d, M=%zd, N=%zd, K=%zd, alpha=%f, beta=%f  %f %f!\n", TransA, TransB, M, N, K, alpha, beta, float(C[f]), float(CReference[f]));
        break;
      }
    }
    ...
    //auto size = cblas_sgemm_pack_get_size();
#endif
  }
 ...
 public:
  ...
  ExecuteShort(
    ...
    // m n k
    printf("128x768x768");
    Test(128, 768, 768, 1.0f, 0.0f);
    printf("3072x768x768");
    Test(3072, 768, 768, 1.0f, 0.0f);
#if 0
...
int
  ...
  for (int i = 0; i != 1; ++i) {
    ...
    printf("Done.\n");
#if !defined(MLAS_NO_ONNXRUNTIME_THREADPOOL)
    if (threadpool != nullptr) threadpool = new onnxruntime::concurrency::ThreadPool("test", 1);
#else
    break;
#endif
  }
