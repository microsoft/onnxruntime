...
protected:
        ...
        )
    {
        ...
        MLAS_GEMM_U8X8_DATA_PARAMS GemmParameters;
        ...
        MlasGemm(GemmShape, GemmParameters, threadpool);
    }
        ...
        )
    {
        ...
        MlasGemmBatch(GemmShape, &GemmParameters, 1, threadpool);
    }
...
private:
    ...
    {
         uint8_t* A = BufferA.GetBuffer(K * M);
         uint8_t* B = BufferB.GetBuffer(N * K);
        ...
        )
    {
        ...
        for (size_t f = 0; f < M * N; f++) {
            if (C[f] != CReference[f]) {
                printf("mismatch M=%zd, N=%zd, K=%zd, posision=%zd, offa=%d, offb=%d!\n", M, N, K, f, int(offa), int(offb));
                break;
            }
        }
    }
...
)
{
    ...
    //std::make_unique<MlasQgemmU8X8Test<int8_t, int32_t, false>>()->ExecuteShort();
    //printf("QGEMM U8S8=float tests.\n");
    //std::make_unique<MlasQgemmU8X8Test<int8_t, float, false>>()->ExecuteShort();
    printf("QGEMM U8U8=int32_t tests.\n");
    std::make_unique<MlasQgemmU8X8Test<uint8_t, int32_t, false>>()->ExecuteShort();
    printf("QGEMM U8U8=float tests.\n");
    std::make_unique<MlasQgemmU8X8Test<uint8_t, float, false>>()->ExecuteShort();

    if (MlasGemmPackBSize(128, 128, true) > 0) {
        printf("QGEMM U8S8=int32_t packed tests.\n");
        std::make_unique<MlasQgemmU8X8Test<int8_t, int32_t, true>>()->ExecuteShort();
        printf("QGEMM U8S8=float packed tests.\n");
        std::make_unique<MlasQgemmU8X8Test<int8_t, float, true>>()->ExecuteShort();
    }
    if (MlasGemmPackBSize(128, 128, false) > 0) {
        printf("QGEMM U8U8=int32_t packed tests.\n");
        std::make_unique<MlasQgemmU8X8Test<uint8_t, int32_t, true>>()->ExecuteShort();
        printf("QGEMM U8U8=float packed tests.\n");
        std::make_unique<MlasQgemmU8X8Test<uint8_t, float, true>>()->ExecuteShort();
    }
