...
model = helper.make_model(
    ...
    graph=make_graph(
        ...
        initializer=[
            numpy_helper.from_array(np.load(os.path.join(DATA_DIR, 'const0_535.npy')).astype('float32').reshape([16, 3, 3, 3]), name='535'),
            numpy_helper.from_array(np.load(os.path.join(DATA_DIR, 'const1_536.npy')).astype('float32').reshape([16]), name='536'),
        ],
        value_info=[
        ...
        nodes=[
            make_node('Conv', inputs=['input', '535', '536'], outputs=['534'], name='Conv_0', dilations=[1, 1], group=1, kernel_shape=[3, 3], pads=[1, 1, 1, 1], strides=[2, 2]),
            make_node('Constant', inputs=[], outputs=['247'], name='Constant_1', value=numpy_helper.from_array(np.array(3.0, dtype='float32'), name='')),
            make_node('Add', inputs=['534', '247'], outputs=['248'], name='Add_2'),
            make_node('Constant', inputs=[], outputs=['249'], name='Constant_3', value=numpy_helper.from_array(np.array(0.0, dtype='float32'), name='')),
            make_node('Constant', inputs=[], outputs=['250'], name='Constant_4', value=numpy_helper.from_array(np.array(6.0, dtype='float32'), name='')),
            make_node('Clip', inputs=['248', '249', '250'], outputs=['251'], name='Clip_5'),
            make_node('Constant', inputs=[], outputs=['252'], name='Constant_6', value=numpy_helper.from_array(np.array(6.0, dtype='float32'), name='')),
