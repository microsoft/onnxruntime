...
#define ORT_ENFORCE(cond, ...) \
  if (!cond) ORT_CXX_API_THROW(__VA_ARGS__);
#endif
...
ONNXTensorElementDataType CustomGemmOp::GetInputType(size_t index) const {
  ...
  default:
    ORT_CXX_API_THROW("Input index=", index, " is out of boundary.");
  }
}
...
CustomGemmOp::GetInputCharacteristic(size_t index) const {
  ...
  case 5:
    return OrtCustomOpInputOutputCharacteristic::INPUT_OUTPUT_OPTIONAL;
  default:
    ORT_CXX_API_THROW("Output index=", index, " is out of boundary.");
  }
}
...
CustomGemmKernel::CustomGemmKernel(const OrtApi &api,
  ...
  if (compute_type == "CUBLAS_COMPUTE_16F") {
    ...
    ORT_CXX_API_THROW("Unexpected value for compute_type '", compute_type, "'.");
  }
  ...
  if (activation == "DEFUALT") {
    ...
    ORT_CXX_API_THROW("Unexpected value for activation '", activation, "'.");
  }
...
void CustomGemmKernel::Compute(OrtKernelContext *context) {
  ...
  if (has_scales) {
    ...
    ORT_CXX_API_THROW("Number of inputs must be 2, 3 or 6 but is ", n_inputs, ".");
  }
  ...
  case 1:
    ...
    break;
  default:
    ORT_CXX_API_THROW("Unexpected value for rowMajor_=", rowMajor_, ".");
  }
}
...
void CustomGemmKernel::ComputeGemm(
  ...
  if (dtype_A == ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT &&
    ...
    ORT_CXX_API_THROW("Not implemented for dtype_A=", dtype_A, " dtype_B=", dtype_B,
