from onnx import helper, save_model, TensorProto
...
def dmmha_inside_mha_self_attn():
    ...
    past_k = helper.make_tensor_value_info("past_k", TensorProto.FLOAT, ["batch_size", num_heads, "max_sequence_length", head_size])
    past_v = helper.make_tensor_value_info("past_v", TensorProto.FLOAT, ["batch_size", num_heads, "max_sequence_length", head_size])
    past_seq_len = helper.make_tensor_value_info("past_seq_len", TensorProto.INT32, [1])
    cache_indir = helper.make_tensor_value_info("cache_indir", TensorProto.INT32, ["batch_size", "num_beams", "max_sequence_length"])
    ...
    present_k = helper.make_tensor_value_info("present_k", TensorProto.FLOAT, ["batch_size", num_heads, "max_sequence_length", head_size])
    present_v = helper.make_tensor_value_info("present_v", TensorProto.FLOAT, ["batch_size", num_heads, "max_sequence_length", head_size])
        ...
        helper.make_graph(
            [
                helper.make_node(
                    ...
                    inputs=["q", "k", "v", "", "", "past_k", "past_v", "past_seq_len", "beam_width", "cache_indir", "b"],
                    outputs=["o", "present_k", "present_v"],
...
def dmmha_inside_mha_cross_attn():
    ...
    # Inputs
    q = helper.make_tensor_value_info("q", TensorProto.FLOAT, ["batch_size", "sequence_length", hidden_size])
    past_k = helper.make_tensor_value_info("k", TensorProto.FLOAT, ["batch_size", num_heads, encoder_seq_len, head_size])
    past_v = helper.make_tensor_value_info("v", TensorProto.FLOAT, ["batch_size", num_heads, encoder_seq_len, head_size])
    b = helper.make_tensor_value_info("b", TensorProto.FLOAT, [hidden_size * 3])
    past_seq_len = helper.make_tensor_value_info("past_seq_len", TensorProto.INT32, [1])
    cache_indir = helper.make_tensor_value_info("cache_indir", TensorProto.INT32, ["batch_size", "num_beams", "max_sequence_length"])
    ...
    # Outputs
    o = helper.make_tensor_value_info("o", TensorProto.FLOAT, ["batch_size", "sequence_length", hidden_size])
    qk = helper.make_tensor_value_info("qk", TensorProto.FLOAT, ["batch_size", "num_heads", "sequence_length", "total_sequence_length"])
    outputs = [o, qk]
