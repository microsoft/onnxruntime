...
def create_model(
    ...
    decoder_graph = create_decoder(vocab_size, embed_dim, num_heads, head_size, sequence_as_input, decoder_needs_input_ids)
    ...
    length_penalty_t = onnx.numpy_helper.from_array(np.array(length_penalty, dtype=np.float32), name='length_penalty_as_tensor')
...
def create_decoder(vocab_size, embed_dim, num_heads, head_size, sequence_as_input, decoder_needs_input_ids) -> onnx.GraphProto:
    ...
    inputs = []
    inputs.append(onnx.helper.make_tensor_value_info('input_ids', onnx.TensorProto.INT32, ['batch_size', 'decode_sequence_length' if sequence_as_input else 1]))
    if decoder_needs_input_ids:
        inputs.append(onnx.helper.make_tensor_value_info('encoder_input_ids', onnx.TensorProto.INT32, ['batch_size', 'encode_sequence_length']))
    inputs.append(onnx.helper.make_tensor_value_info('encoder_attention_mask', onnx.TensorProto.INT32, ['batch_size', 'encode_sequence_length']))
    inputs.append(onnx.helper.make_tensor_value_info('past_self_key_0', onnx.TensorProto.FLOAT, ['batch_size', num_heads, 'decode_sequence_length', head_size]))
    inputs.append(onnx.helper.make_tensor_value_info('past_self_value_0', onnx.TensorProto.FLOAT, ['batch_size', num_heads, 'decode_sequence_length', head_size]))
    inputs.append(onnx.helper.make_tensor_value_info('past_cross_key_0', onnx.TensorProto.FLOAT, ['batch_size', num_heads, 'encode_sequence_length', head_size]))
    inputs.append(onnx.helper.make_tensor_value_info('past_cross_value_0', onnx.TensorProto.FLOAT, ['batch_size', num_heads, 'encode_sequence_length', head_size]))
    ...
    outputs = [
        ...
        onnx.helper.make_tensor_value_info('present_key_self_0', onnx.TensorProto.FLOAT, ['batch_size', num_heads, 'present_decode_sequence_length', head_size]),
        onnx.helper.make_tensor_value_info('present_value_self_0', onnx.TensorProto.FLOAT, ['batch_size', num_heads, 'present_decode_sequence_length', head_size]),
    ]

    # Tensors: decoder_embeddings, final_proj, self_state_before_tranpose_shape_no_batch, hidden_states_mean
    initializers = [
        onnx.numpy_helper.from_array(np.random.randn(vocab_size, embed_dim).astype(np.float32), name='decoder_embeddings'),
        onnx.numpy_helper.from_array(np.random.randn(embed_dim, vocab_size).astype(np.float32), name='final_proj'),
        onnx.numpy_helper.from_array(np.array([-1, num_heads, head_size], dtype=np.int64), name='self_state_before_tranpose_shape_no_batch'),
    ...
    if decoder_needs_input_ids:
        nodes.append(onnx.helper.make_node('Gather', ['decoder_embeddings', 'encoder_input_ids'], ['encoder_input_embeddings']))
        nodes.append(onnx.helper.make_node('ReduceMean', ['encoder_input_embeddings'], ['encoder_input_embeddings_mean'], axes=[1]))
        nodes.append(onnx.helper.make_node('Mul', ['decoder_hidden_states', 'encoder_input_embeddings_mean'], ['combined_hidden_states']))
    else:
    ...
    nodes.append(onnx.helper.make_node('Reshape', ['encoder_hidden_states_mean', 'hidden_states_mean_shape'], ['encoder_hidden_states_mean_reshaped']))
    if sequence_as_input:
        nodes.append(onnx.helper.make_node('ReduceMean', ['combined_hidden_states'], ['decoder_hidden_states_mean'], axes=[1]))
        nodes.append(onnx.helper.make_node('Add', ['decoder_hidden_states_mean', 'encoder_hidden_states_mean_reshaped'], ['encoder_decoder_sum']))
    else:
        nodes.append(onnx.helper.make_node('Add', ['combined_hidden_states', 'encoder_hidden_states_mean_reshaped'], ['encoder_decoder_sum']))
    nodes.append(onnx.helper.make_node('Shape', ['combined_hidden_states'], ['decoder_batch'], end=1))
    nodes.append(onnx.helper.make_node('Concat', ['decoder_batch', 'self_state_before_tranpose_shape_no_batch'], ['self_state_before_tranpose_shape_dec'], axis=0))
    nodes.append(onnx.helper.make_node('MatMul', ['encoder_decoder_sum', 'final_proj'], ['logits']))
    nodes.append(onnx.helper.make_node('Reshape', ['encoder_decoder_sum', 'self_state_before_tranpose_shape_dec'], ['self_state_before_tranpose']))
    nodes.append(onnx.helper.make_node('Transpose', ['self_state_before_tranpose'], ['single_self_key_0'], perm=[0, 2, 1, 3]))
    nodes.append(onnx.helper.make_node('Transpose', ['self_state_before_tranpose'], ['single_self_value_0'], perm=[0, 2, 1, 3]))
    nodes.append(onnx.helper.make_node('Concat', ['past_self_key_0', 'single_self_key_0'], ['present_key_self_0'], axis=2))
    nodes.append(onnx.helper.make_node('Concat', ['past_self_value_0', 'single_self_value_0'], ['present_value_self_0'], axis=2))
