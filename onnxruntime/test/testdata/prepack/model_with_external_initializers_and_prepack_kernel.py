...
def GenerateConvTransposeModel(model_name, external_data_name):  # noqa: N802
    input = helper.make_tensor_value_info("A", TensorProto.FLOAT, [1, 3, 64, 64])  # noqa: N806
    output = helper.make_tensor_value_info("Y", TensorProto.FLOAT, [1, 6, 127, 127])  # noqa: N806
...
def GenerateDeepCpuLSTMModel(model_name, external_data_name_w, external_data_name_r):  # noqa: N802
    input = helper.make_tensor_value_info("A", TensorProto.FLOAT, [16, 2, 32])  # noqa: N806
    output = helper.make_tensor_value_info("Y", TensorProto.FLOAT, [16, 1, 2, 4])  # noqa: N806
    ...
    graph_def = helper.make_graph(
        ...
        [output],
        [
            create_external_data_tensor(np.random.rand(1, 16, 32).astype(np.float32), external_data_name_w, TensorProto.FLOAT),
            create_external_data_tensor(np.random.rand(1, 16, 32).astype(np.float32), external_data_name_r, TensorProto.FLOAT),
        ],
    )
...
def GenerateAttentionModel(model_name, external_data_name):  # noqa: N802
    input = helper.make_tensor_value_info("A", TensorProto.FLOAT, [4, 16, 8])  # noqa: N806
    output = helper.make_tensor_value_info("Y", TensorProto.FLOAT, [4, 16, 2])  # noqa: N806
...
def GenerateQAttentionModel(model_name, external_data_name):  # noqa: N802
    input = helper.make_tensor_value_info("A", TensorProto.UINT8, [4, 16, 8])  # noqa: N806
    output = helper.make_tensor_value_info("Y", TensorProto.FLOAT, [4, 16, 2])  # noqa: N806
...
def GenerateDynamicQuantizeLSTMModel(model_name, external_data_name_w, external_data_name_r):  # noqa: N802
    input = helper.make_tensor_value_info("A", TensorProto.FLOAT, [4, 16, 8])  # noqa: N806
    output = helper.make_tensor_value_info("Y", TensorProto.FLOAT, [4, 1, 16, 2])  # noqa: N806

    # Create a node (NodeProto)
    node_def = helper.make_node(
        op_type="DynamicQuantizeLSTM",  # op type
        inputs=["A", external_data_name_w, external_data_name_r, 
                "B", "sequence_lens", 'initial_h', 'initial_c', 'P', 
                'W_scale', 'W_zero_point', 'R_scale', 'R_zero_point'],  # inputs
        outputs=["Y"],  # outputs
...
def GenerateConvFP16Model(model_name, external_data_name):  # noqa: N802
    input = helper.make_tensor_value_info("A", TensorProto.FLOAT, [1, 1, 1, 5])  # noqa: N806
    output = helper.make_tensor_value_info("Y", TensorProto.FLOAT, [1, 1, 1, 5])  # noqa: N806
    ...
    graph_def = helper.make_graph(
        ...
        [output],
        [
            create_external_data_tensor(np.random.rand(1, 1, 3, 3).astype(np.float16), external_data_name, TensorProto.FLOAT16),
        ],
    )
...
def GenerateGRUModel(model_name, external_data_name_w, external_data_name_r):  # noqa: N802
    ...
    graph_def = helper.make_graph(
        ...
        [Y],
        [
            create_external_data_tensor(np.random.rand(1, 12, 8).astype(np.float32), external_data_name_w, TensorProto.FLOAT),
            create_external_data_tensor(np.random.rand(1, 12, 4).astype(np.float32), external_data_name_r, TensorProto.FLOAT)
        ],
    )
...
def GenerateQLinearConvModel(model_name, external_data_name):  # noqa: N802
    input = helper.make_tensor_value_info("A", TensorProto.UINT8, [2, 4, 8, 16])  # noqa: N806
    output = helper.make_tensor_value_info("Y", TensorProto.UINT8, [2, 10, 8, 16])  # noqa: N806

    # Create a node (NodeProto)
    node_def = helper.make_node(
        op_type="QLinearConv",  # op type
        inputs=["A", 'x_scale', 'x_zero_point', external_data_name, "w_scale", "w_zero_point", "y_scale", 'y_zero_point'],  # inputs
        outputs=["Y"],  # outputs
...
if __name__ == "__main__":
    ...
    GenerateDynamicQuantizeLSTMModel('model_with_dynamic_quan_lstm.onnx', 'DynamicQuantizeLSTM.Weight', 'DynamicQuantizeLSTM.RecurrenceWeight')
