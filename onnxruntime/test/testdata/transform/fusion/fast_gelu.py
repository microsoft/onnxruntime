...
gelu_use_graph_input = (
    True  # change it to False to let Gelu don't have graph inputs as inputs.
)
...
if has_bias:
    add0 = helper.make_node(
        "Add", [gelu_input, bias_initializer.name], ["add0"], name="add0"
    )
    mul_input_name = "add0"
    nodes.append(add0)

mul1 = helper.make_node(
    "Mul", [mul_input_name, a_weight_initializer.name], ["mul1"], name="mul1"
)
nodes.append(mul1)
...
mul3 = helper.make_node(
    "Mul", [mul_input_name, b_weight_initializer.name], ["mul3"], name="mul3"
)
nodes.append(mul3)
...
mul5 = helper.make_node(
    "Mul", [mul_input_name, c_weight_initializer.name], ["mul5"], name="mul5"
)
nodes.append(mul5)

mul6 = helper.make_node("Mul", ["mul5", "add2"], ["mul6"], name="mul6")
ending_identity = helper.make_node(
    "Identity", ["mul6"], ["output"], name="identity_ending"
)
