...
def gen_fuse_back2back_casts(model_path):
    ...
    for (type1, type2) in list(
        itertools.product([TensorProto.FLOAT, TensorProto.FLOAT16], repeat=2)
    ):
...
def gen_fuse_sibling_casts(model_path):
    ...
    for (type1, type2) in list(
        ...
        nodes = [
            helper.make_node("MatMul", ["input_0", "input_1"], ["product"], "MatMul_0"),
            helper.make_node(
                "Cast", ["product"], ["cast_0_output"], "Cast_0", to=type1
            ),
            helper.make_node("Identity", ["cast_0_output"], ["output_0"], "Identity_0"),
            helper.make_node(
                "Cast", ["product"], ["cast_1_output"], "Cast_1", to=type2
            ),
...
def flip_type(type, flip=True):
    return (
        (TensorProto.FLOAT16 if type == TensorProto.FLOAT else TensorProto.FLOAT)
        if flip
        else type
    )
...
def do_cast_inputs(input_0, input_1, nodes, input_cast_type):
    nodes.extend(
        [
            helper.make_node(
                "Cast", [input_0], ["cast_" + input_0], "Cast_0", to=input_cast_type
            ),
            helper.make_node(
                "Cast", [input_1], ["cast_" + input_1], "Cast_1", to=input_cast_type
            ),
        ]
    )
...
def do_transpose_inputs(input_0, input_1, nodes):
    nodes.extend(
        [
            helper.make_node(
                "Transpose", [input_0], ["input_transpose_0"], "Transpose_0"
            ),
            helper.make_node(
                "Transpose", [input_1], ["input_transpose_1"], "Transpose_1"
            ),
        ]
    )
...
def do_cast_product(product, nodes, product_type):
    nodes.insert(
        1,
        helper.make_node(
            "Cast", [product], [product + "_cast"], "Cast_2", to=product_type
        ),
    )
...
def do_transpose_product(product, nodes):
    if transpose_product:
        nodes.append(
            helper.make_node(
                "Transpose", [product], [product + "_transpose"], "Transpose_2"
            )
        )
...
def do_cast_input2(input_2, nodes, type):
    nodes.append(
        helper.make_node("Cast", [input_2], ["cast_" + input_2], "Cast_4", to=type)
    )
...
def gen_propagate_cast_test_model(
    ...
    if insert_add:
        ...
        inputs.append(
            helper.make_tensor_value_info(input_2, add_input_type, ["N", "N"])
        )
        output = "sum"
...
def gen_matmul_two_products(
    model_path, transpose, transpose_before_cast, second_matmul, cast_inputs
):
    def do_transpose(output_0, output_1, transpose, nodes):
        nodes.append(
            helper.make_node(
                "Transpose", [output_0], ["transpose_0_" + output_0], "Transpose_0"
            )
        )
        ...
        if transpose > 1:
            nodes.append(
                helper.make_node(
                    "Transpose", [output_1], ["transpose_1_" + output_1], "Transpose_1"
                )
            )
    ...
    if second_matmul:
        nodes.append(
            helper.make_node(
                "MatMul", [input_0, input_1], ["second_" + output], "MatMul_1"
            )
        )
        outputs.append(
            helper.make_tensor_value_info("second_" + output, input_type, ["M", "N"])
        )
        if add_products:
            nodes.append(
                helper.make_node("Add", [output, "second_" + output], ["sum"], "Add_0")
            )
    ...
    model_path += (
        ("_transpose_before_cast" if transpose_before_cast else "_transpose_after_cast")
        if transpose > 0
        else ""
    )
...
def gen_bool_to_float16_cast(model_path):
    ...
    cast1 = helper.make_node(
        "Cast", ["less1"], ["cast1"], name="cast1", to=TensorProto.FLOAT16
    )
    cast2 = helper.make_node(
        "Cast", ["x3"], ["cast2"], name="cast2", to=TensorProto.FLOAT16
    )
...
def gen_bool_to_float_cast(model_path):
    ...
    less1 = helper.make_node("Less", ["x1", "x2"], ["less1"], name="less1")
    cast1 = helper.make_node(
        "Cast", ["less1"], ["cast1"], name="cast1", to=TensorProto.FLOAT
    )
    cast2 = helper.make_node(
        "Cast", ["x3"], ["cast2"], name="cast2", to=TensorProto.FLOAT
    )
    add1 = helper.make_node("Add", ["cast1", "cast2"], ["add1"])
    cast3 = helper.make_node(
        "Cast", ["add1"], ["output"], name="cast3", to=TensorProto.FLOAT16
    )
...
def gen_one_input_one_output_test(op, model_path, axes_attribute=False):
    ...
    graph_inputs = [X]
    cast1 = helper.make_node(
        "Cast", ["x"], ["cast1"], name="cast1", to=TensorProto.FLOAT
    )
    ...
    cast2 = helper.make_node(
        "Cast", ["op_output"], ["y"], name="cast2", to=TensorProto.FLOAT16
    )
...
for (
    ...
    itertools.product(
        [0, 1, 2], [False, True], [False, True], [False, True], [False, True]
    )
):
