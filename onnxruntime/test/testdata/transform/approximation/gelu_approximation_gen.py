...
graph = helper.make_graph(
    ...
    [  # inputs
        helper.make_tensor_value_info(
            "A", TensorProto.FLOAT, ["batch", "seq_len", 3072]
        ),
    ],
    [  # outputs
        helper.make_tensor_value_info(
            "C", TensorProto.FLOAT, ["batch", "seq_len", 3072]
        ),
    ],
    [],  # initializers
)
...
graph = helper.make_graph(
    [  # nodes
        # Add node before Gelu
        helper.make_node(
            "BiasGelu", ["A", "B"], ["C"], "AddGeluFusion_1", domain="com.microsoft"
        ),
    ],
    "Gelu_AddBias",  # name
    [  # inputs
...
graph = helper.make_graph(
    [  # nodes
        ...
        helper.make_node(
            "BiasGelu", ["C", "D"], ["E"], "AddGeluFusion_1", domain="com.microsoft"
        ),
    ],
    "MatMul_AddGeluFusion",  # name
    [  # inputs
        helper.make_tensor_value_info(
            "A", TensorProto.FLOAT, ["batch", "seq_len", "x"]
        ),
    ...
    [  # outputs
        helper.make_tensor_value_info(
            "E", TensorProto.FLOAT, ["batch", "seq_len", 3072]
        ),
    ],
    [],  # initializers
)
