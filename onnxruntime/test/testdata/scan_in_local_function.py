...
entries inside the Scan body (and propagate concrete call-site shapes) so
that the memory planner (e.g. CUDA EP) can use the shape annotations to
...
scan_body = helper.make_graph(
    ...
    value_info=[
        ...
        # The memory planner uses this entry to pre-allocate the buffer.
