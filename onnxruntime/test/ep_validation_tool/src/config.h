...
inline AccuracyMetric ParseAccuracyMetric(const std::wstring& w)
{
    ...
    if (s == "mae") return AccuracyMetric::MAE;
    if (s == "mse") return AccuracyMetric::MSE;
...
inline const char* ToString(AccuracyMetric m)
{
    switch (m)
    {
    case AccuracyMetric::MAE: return "mae";
    case AccuracyMetric::Cosine: return "cosine";
    case AccuracyMetric::MSE: return "mse";
...
public:
    Config()
    {
        ...
        m_arg_parser.AddArgument(
            L"--accMetrics",
            L"-am",
            L"Accuracy metrics to compute (space-separated). Supported values: L2Norm Cosine MAE MSE.",
            false,
            false);
