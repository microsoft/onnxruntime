...
{
    auto cos_opt = CosineSimilarity(a, b);
    if (!cos_opt.has_value())
        return std::nullopt;

    // Clamp cosine to valid domain to avoid NaNs from acos
    double c = static_cast<double>(cos_opt.value());
    if (c > 1.0)
        c = 1.0;
    else if (c < -1.0)
        c = -1.0;
    ...
    double angle_deg = angle_rad * (180.0 / kPI);
...
std::optional<float> MeanSquaredError(const Ort::Value& a, const Ort::Value& b)
{
    ...
    for (size_t i = 0; i < num_elems; ++i)
    {
        const double diff = static_cast<double>(data_a[i]) - static_cast<double>(data_b[i]);
        sum_sq += diff * diff;
    }
    ...
    double mse = sum_sq / static_cast<double>(num_elems);
    ...
    const std::unordered_map<std::string, std::string>& output_map)
{
    ...
    for (auto m : metrics)
    {
        ...
        if (m == AccuracyMetric::MAE) do_mae = true;
        if (m == AccuracyMetric::MSE) do_mse = true;
    }
    ...
    for (const auto& [output_name, _] : output_map)
    {
        ...
        if (do_mae)
        {
            metric_diffs[ToString(AccuracyMetric::MAE)].reserve(npu_samples.size());
        }
        if (do_mse)
        {
            metric_diffs[ToString(AccuracyMetric::MSE)].reserve(npu_samples.size());
        }
        ...
        for (size_t sample_idx = 0; sample_idx < npu_samples.size(); ++sample_idx)
        {
            ...
            float mse_threshold = 0.0f;
            {
                ...
                metric_it = acc_threshold.find(ToString(AccuracyMetric::MAE));
                if (metric_it != acc_threshold.end())
                {
                    auto t_it = metric_it->second.find(output_name);
                    if (t_it != metric_it->second.end())
                    {
                        mae_threshold = t_it->second;
                    }
                }
                metric_it = acc_threshold.find(ToString(AccuracyMetric::MSE));
                if (metric_it != acc_threshold.end())
                {
                    auto t_it = metric_it->second.find(output_name);
                    if (t_it != metric_it->second.end())
                    {
                        mse_threshold = t_it->second;
                    }
                }
            }
            ...
            if (do_mae)
            {
                auto mae_opt = MeanAngularError(f32_samples[sample_idx], npu_samples[sample_idx]);

                bool good = true;

                if (!mae_opt.has_value())
                {
                    good = false;
                }
                else
                {
                    float mae_value = mae_opt.value();

                    metric_diffs[ToString(AccuracyMetric::MAE)].push_back(mae_value);

                    // For MAE, if threshold is 0.0, treat all valid values as acceptable
                    good = (mae_threshold > 0.0f)
                        ? (mae_value <= mae_threshold)
                        : true;
                }
                ...
                if (!good)
                {
                    is_tolerable = false;
                    std::cout << "Sample " << sample_idx << " accuracy for '" << output_name
                              << "' (MAE) worse than the threshold set." << std::endl;
                }
            }
            ...
            if (do_mse)
            {
                auto mse_opt = MeanSquaredError(f32_samples[sample_idx], npu_samples[sample_idx]);

                bool good = true;

                if (!mse_opt.has_value())
                {
                    good = false;
                }
                else
                {
                    float mse_value = mse_opt.value();

                    metric_diffs[ToString(AccuracyMetric::MSE)].push_back(mse_value);

                    // For MSE, if threshold is 0.0, treat all valid values as acceptable
                    good = (mse_threshold > 0.0f)
                        ? (mse_value <= mse_threshold)
                        : true;
                }
                ...
                if (!good)
                {
                    is_tolerable = false;
                    std::cout << "Sample " << sample_idx << " accuracy for '" << output_name
                              << "' (MSE) worse than the threshold set." << std::endl;
                }
            }
        }
