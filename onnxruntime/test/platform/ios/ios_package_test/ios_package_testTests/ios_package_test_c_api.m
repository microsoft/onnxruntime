...
#define ASSERT_ON_ERROR(expr)                                                              \
  do {                                                                                     \
    ...
    XCTAssertEqual(NULL, status,                                                           \
                   @"Failed with error message: %@", @(ort_api->GetErrorMessage(status))); \
  } while (0);
...
- (void)testCAPI {
  ...
  ASSERT_ON_ERROR(ort_api->CreateTensorWithDataAsOrtValue(
  ...
  ASSERT_ON_ERROR(ort_api->Run(session, NULL, input_node_names,
