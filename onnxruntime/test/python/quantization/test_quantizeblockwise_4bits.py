...
def quantize_blockwise_4bits_ref(matrix_float:npt.ArrayLike, block_size:int, is_symmetric:bool):
    ...
    for n in range(cols):
        for k_id in range(0, rows, block_size):
            ...
            blk_int0 = np.clip(np.round(np.float32(matrix_float_padded[n, k_id:k_id+block_size:2] * reciprocal_scale + zp)), 0, 15).astype("uint8")
            blk_int1 = np.clip(np.round(np.float32(matrix_float_padded[n, k_id + 1:k_id+block_size:2] * reciprocal_scale + zp)), 0, 15).astype("uint8")
...
def quantize_blockwise_4bits_target(matrix_float:npt.ArrayLike, block_size:int, is_symmetric:bool):
    ...
    zero_point = np.full((cols, k_blocks), 8, dtype="uint8")
            ...
            for block_size in [16, 32, 64, 128]:
                for type in [np.float32, np.float16]:
                    for is_symmetric in [True, False]:
                        ...
                        quant_value_ref, scales_ref, zero_point_ref = quantize_blockwise_4bits_ref(matrix_float, block_size, is_symmetric)
                        quant_value, scales, zero_point = quantize_blockwise_4bits_target(matrix_float, block_size, is_symmetric)
                        ...
                        for c in range(quant_value_ref.shape[0]):
                            for k in range(quant_value_ref.shape[1]):
                                assert np.allclose(dequantize_blockwise_4bits(quant_value_ref[c][k], scales_ref[c][k], zero_point_ref[c][k], min(block_size, rows - k * block_size)),
                                                   dequantize_blockwise_4bits(quant_value[c][k], scales[c][k], zero_point[c][k], min(block_size, rows - k * block_size)),
