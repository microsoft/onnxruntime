...
class TestONNXModel(unittest.TestCase):
    def construct_model(self, model_path):
        ...
        conv_node_1 = onnx.helper.make_node(
            "Conv", ["GRU_O", "W1", "B1"], ["Conv1_O"], name="Conv1"
        )
        conv_node_2 = onnx.helper.make_node(
            "Conv", ["GRU_O", "W2", "B2"], ["Conv2_O"], name="Conv2"
        )
    ...
    def construct_model_Constant(self, model_path):
        ...
        constant_node = onnx.helper.make_node(
            "Constant", [], ["const_output"], value_float=42.0
        )
        add_node = onnx.helper.make_node(
            "Add", ["input", "const_output"], ["output"], name="Add"
        )
        graph = helper.make_graph(
    ...
    def test_topo_sort(self):
        ...
        check_op_type_order(
            self, onnx_model.model, ["Conv", "Relu", "Conv", "GRU", "Add"]
        )
        onnx_model.topological_sort()
        check_op_type_order(
            self, onnx_model.model, ["GRU", "Conv", "Conv", "Relu", "Add"]
        )
