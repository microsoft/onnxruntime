...
class TestDataReader(CalibrationDataReader):
    ...
    def __init__(self):
        ...
        for _ in range(self.count):
            self.input_data_list.append(
                np.random.normal(0, 0.33, [1, 3, 1, 3]).astype(np.float32)
            )
    ...
    def get_next(self):
        if self.preprocess_flag:
            ...
            self.enum_data_dicts = iter(
                [{input_name: input_data} for input_data in self.input_data_list]
            )
...
class TestCalibrate(unittest.TestCase):
    def test_augment_graph_config_1(self):
        ...
        graph = helper.make_graph(
            [conv_node, clip_node, matmul_node], "test_graph_1", [A, B, E], [F]
        )
        ...
        calibrater = MinMaxCalibrater(
            test_model_path, ["Conv", "MatMul"], augmented_model_path
        )
    ...
    def test_augment_graph_config_2(self):
        ...
        graph = helper.make_graph(
            [conv_node_1, conv_node_2], "test_graph_2", [G, H, J], [K]
        )
        model = helper.make_model(graph, opset_imports=[helper.make_opsetid("", 13)])
    ...
    def test_augment_graph_config_3(self):
        ...
        graph = helper.make_graph(
            [relu_node, conv_node, clip_node, matmul_node], "test_graph_3", [L, N], [Q]
        )
        model = helper.make_model(graph, opset_imports=[helper.make_opsetid("", 13)])
    ...
    def construct_test_compute_range_model(self, test_model_path):
        ...
        conv_node_1 = onnx.helper.make_node(
            "Conv", ["X1", "W1", "B1"], ["X2"], name="Conv1"
        )
        relu_node_2 = onnx.helper.make_node("Relu", ["X2"], ["X3"], name="Relu2")
        conv_node_2 = onnx.helper.make_node(
            "Conv", ["X3", "W3", "B3"], ["X4"], name="Conv2"
        )
        conv_node_3 = onnx.helper.make_node(
            "Conv", ["X1", "W5", "B5"], ["X5"], name="Conv3"
        )
    ...
    def test_compute_range(self):
        ...
        calibrater = MinMaxCalibrater(
            test_model_path, augmented_model_path=augmented_model_path
        )
        ...
        output_names = [
            infer_session.get_outputs()[i].name
            for i in range(len(infer_session.get_outputs()))
        ]
        output_min_max_dict = dict(zip(output_names, min_max_pairs))
        for output_name in output_min_max_dict.keys():
            self.assertEqual(
                output_min_max_dict[output_name], tensors_range[output_name]
            )
    ...
    def test_augment_graph_with_zero_value_dimension(self):
        ...
        resize_node_1 = onnx.helper.make_node(
            "Resize", ["K", "M", "N"], ["O"], name="Reize1"
        )
        graph = helper.make_graph(
