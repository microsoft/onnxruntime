...
def make_providers(nuphar_settings):
    return [
        ("NupharExecutionProvider", {"nuphar_settings": nuphar_settings}),
...
class TestNuphar(unittest.TestCase):
    def test_bidaf(self):
        ...
        subprocess.run(
            [
                sys.executable,
                "-m",
                "onnxruntime.nuphar.model_quantizer",
                "--input",
                bidaf_opt_scan_model,
                "--output",
        ...
        for model in [bidaf_opt_scan_model, bidaf_int8_scan_only_model]:
            ...
            subprocess.run(
                [
                    sys.executable,
                    "-m",
                    "onnxruntime.nuphar.create_shared",
                    "--input_dir",
                    cache_versioned_dir,
                    "--output_name",
                    so_name,
    ...
    def test_bert_squad(self):
        ...
        onnx_test_runner = os.path.join(cwd, "onnx_test_runner")
        subprocess.run(
            [onnx_test_runner, "-e", "nuphar", "-n", "BERT_Squad", cwd],
            check=True,
            cwd=cwd,
        )
    ...
    def test_gemm_to_matmul_with_scan(self):
        ...
        test_configs = [
            ...
            ({"alpha": 2.2}, {"alpha": 3.3}),
            (
                {"transA": 1, "transB": 1, "alpha": 2.2},
                {"transA": 1, "transB": 1, "alpha": 3.3},
            ),
            ({"withC": True, "initC": True}, {}),
            ...
            (
                {"C": "inputC", "withC": True, "initC": True},
                {"C": "inputC", "withC": True, "initC": True},
            ),
            (
                {
            ...
            (
                {"transB": 1, "alpha": 1.2, "B": "inputB"},
                {"transB": 1, "alpha": 2.2, "B": "inputB"},
            ),
            (
