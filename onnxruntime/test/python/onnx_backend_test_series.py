...
def GetVersionTag():
    ...
    # If file does not exist assume we are running in a dev environment
    # off the onnx commit within ORT
    if os.path.isfile(file_path):
       with open(file_path, 'r') as f:
        ...
        version2tag[onnx.version.git_version] = 'onnxtip'
...
def create_backend_test(testname=None):
    ...
    if testname:
        ...
        if version_tag == 'onnx141' or onnx.__version__ == '1.4.1':
            current_failing_tests = current_failing_tests + ('^test_shrink_cpu.*')
        if version_tag != 'onnxtip':
