...
def run_relevance_no_past(device, dtype=torch.float16):
    ...
    print("query", feed_dict["query"])
    print("key", feed_dict["key"])
    print("value", feed_dict["value"])
    ...
    expected_out = scaled_dot_product_gqa(
        query, key, value, scale=1.0 / scale, mask=dense_mask.repeat(config.batch_size, 1, 1, 1).bool()
    )
    ...
    print("ort_output shape", actual_out.shape)
