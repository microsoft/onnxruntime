...
torch.ops.load_library("weightOnlyQuantOp.cpython-38-x86_64-linux-gnu.so")
...
def create_moe_onnx_graph(
    ...
    initializers = [
        helper.make_tensor(
            ...
            TensorProto.UINT8,
            fc1_shape,
            fc1_experts_weights.flatten().numpy().astype(np_type).tolist(),
            raw=False,
        ),
        helper.make_tensor(
            "fc2_experts_weights",
            TensorProto.UINT8,
            fc2_shape,
            fc2_experts_weights.flatten().numpy().astype(np_type).tolist(),
            raw=False,
        ),
        helper.make_tensor(
            "fc3_experts_weights",
            TensorProto.UINT8,
            fc3_shape,
            fc3_experts_weights.flatten().numpy().astype(np_type).tolist(),
            raw=False,
        ),
    ]
...
class MixtralSparseMoeBlock(nn.Module):
    ...
    def __init__(self, config, batch_size, sequence_length):
        ...
        for i in range(self.num_experts):
            w1_scale, pre_qweight1, w1_qdq = quant_dequant(self.experts[i].w1.weight)
            w2_scale, pre_qweight2, w2_qdq = quant_dequant(self.experts[i].w2.weight)
            w3_scale, pre_qweight3, w3_qdq = quant_dequant(self.experts[i].w3.weight)
        ...
        moe_experts_weight_scale3 = torch.stack(w3_scale_list, dim=0)
    ...
    def parity_check(self):
        ...
        print(
            ...
            " parity:",
            torch.allclose(torch_output.half(), ort_output, rtol=1e-04, atol=1e-04),
        )
