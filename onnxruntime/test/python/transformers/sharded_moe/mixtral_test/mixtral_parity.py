...
ort_inputs = {
    ...
    "past_key_values.0.key": np.random.normal(0, 0.1, batch_size*8*past_sequence_length*128).reshape(batch_size, 8, past_sequence_length, 128).astype(NP_TYPE),
    "past_key_values.0.value": np.random.normal(0, 0.1, batch_size*8*past_sequence_length*128).reshape(batch_size, 8, past_sequence_length, 128).astype(NP_TYPE),
    "past_key_values.1.key": np.random.normal(0, 0.1, batch_size*8*past_sequence_length*128).reshape(batch_size, 8, past_sequence_length, 128).astype(NP_TYPE),
    "past_key_values.1.value": np.random.normal(0, 0.1, batch_size*8*past_sequence_length*128).reshape(batch_size, 8, past_sequence_length, 128).astype(NP_TYPE),
}

nh_start = local_rank * 8 // get_size()
nh_end = (local_rank + 1) * 8 // get_size()
local_ort_inputs = {
