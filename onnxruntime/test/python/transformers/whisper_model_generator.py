...
def create_whisper_decoder_with_past_multihead_self_attention(
    ...
    if fused:
        ...
        q_nodes = [
            ...
            helper.make_node("Mul", ["q_add_output", "q_scale"], ["q_mul_output"], "q_path_mul"),
            helper.make_node(
                "Reshape", ["q_mul_output", "q_bsnh_reshape"], ["q_4d_bsnh"], "q_reshape_to_4d"
            ),
            helper.make_node(
                "Transpose", ["q_4d_bsnh"], ["q_4d_bnsh"], "q_transpose_to_bnsh", perm=[0, 2, 1, 3]
            ),
            helper.make_node(
                "Reshape",
                ["q_4d_bnsh", "q_attn_heads_output"],
...
def create_whisper_decoder_with_past_multihead_cross_attention(
    ...
    if not fused:
        next_sln_inputs = [
            "layernorm_output_to_skiplayernorm", "add_after_attn_output", "layernorm_weight", "layernorm_bias"
        ]
        nodes.extend(
            [
                helper.make_node(
