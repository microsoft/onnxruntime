...
class TestRegularMoECPU(unittest.TestCase):
    """Test regular (non-quantized) MoE CPU implementation."""

    def setUp(self):
        # Set maxDiff to None to see full diff output when tests fail
        self.maxDiff = None

    def test_regular_moe_cpu_basic(self):
        """Test basic regular MoE CPU functionality."""
        print("Running regular MoE CPU test")
        ...
        # Create test data
        hidden_states = torch.randn(batch_size, sequence_length, hidden_size, dtype=torch.float32)
        router_probs = torch.softmax(
            torch.randn(batch_size * sequence_length, num_experts, dtype=torch.float32), dim=-1
        )

        # Create random weights for experts
        fc1_weights = torch.randn(num_experts, hidden_size, intermediate_size, dtype=torch.float32)
        fc2_weights = torch.randn(num_experts, intermediate_size, hidden_size, dtype=torch.float32)
        fc1_bias = torch.randn(num_experts, intermediate_size, dtype=torch.float32)
        fc2_bias = torch.randn(num_experts, hidden_size, dtype=torch.float32)

        # Create ONNX graph for regular (non-quantized) MoE
        try:
            onnx_model = create_cpu_moe_onnx_graph(
            ...
            self.skipTest(f"Failed to create ONNX model: {e}")

        if onnx_model is None:
            self.skipTest("Failed to create ONNX model")
        ...
        try:
            ...
            ort_session = onnxruntime.InferenceSession(onnx_model, sess_options, providers=providers)
        except Exception as e:
            self.skipTest(f"Failed to create ONNX Runtime session: {e}")

        # Prepare inputs
        ort_inputs = {
            "input": hidden_states.reshape(batch_size * sequence_length, hidden_size).numpy(),
            "router_probs": router_probs.numpy(),
        }
        ...
        try:
            ...
            self.assertFalse(numpy.allclose(output, 0.0), "Output should not be all zeros")

            # Check for NaN or Inf values
            self.assertFalse(numpy.isnan(output).any(), "Output contains NaN values")
            self.assertFalse(numpy.isinf(output).any(), "Output contains Inf values")

            print(f"✓ Regular MoE CPU test passed - output shape: {output.shape}, "
                  f"output range: [{output.min():.6f}, {output.max():.6f}]")

        except Exception as e:
            self.skipTest(f"Failed to run inference: {e}")

    def test_regular_moe_cpu_swiglu(self):
        """Test regular MoE CPU with SwiGLU activation."""
        print("Running regular MoE CPU test with SwiGLU")
        ...
        # For SwiGLU, FC1 output size is 2x intermediate_size (gate + value)
        fc1_output_size = 2 * intermediate_size

        # Create random weights for experts
        fc1_weights = torch.randn(num_experts, hidden_size, fc1_output_size, dtype=torch.float32)
        fc2_weights = torch.randn(num_experts, intermediate_size, hidden_size, dtype=torch.float32)
        fc1_bias = torch.randn(num_experts, fc1_output_size, dtype=torch.float32)
        fc2_bias = torch.randn(num_experts, hidden_size, dtype=torch.float32)

        # Create ONNX graph for regular MoE with SwiGLU
        try:
            onnx_model = create_cpu_moe_onnx_graph(
                hidden_size=hidden_size,
                sequence_length=batch_size * sequence_length,
        ...
        try:
            ...
            print(f"✓ Regular MoE CPU SwiGLU test passed - output shape: {output.shape}, "
                  f"output range: [{output.min():.6f}, {output.max():.6f}]")
