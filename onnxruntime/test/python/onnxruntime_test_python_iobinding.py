...
class TestIOBinding(unittest.TestCase):
    ...
    def test_bind_input_types(self, device, execution_provider, generate_device):
    ...
    def test_bind_input_and_preallocated_output(self, device, execution_provider):
        if execution_provider not in onnxrt.get_available_providers():
            self.skipTest(f"Not running on {device.upper()}.")
    ...
    def test_bind_input_and_non_preallocated_output(self, device, execution_provider):
        if execution_provider not in onnxrt.get_available_providers():
            self.skipTest(f"Not running on {device.upper()}.")
    ...
    def test_bind_input_and_bind_output_with_ortvalues(self, device, execution_provider):
        if execution_provider not in onnxrt.get_available_providers():
            self.skipTest(f"Not running on {device.upper()}.")
