...
void MyCustomKernel::Compute(OrtKernelContext* context) {
  ...
  cudaStream_t cuda_stream = static_cast<cudaStream_t>(ort_.KernelContext_GetGPUComputeStream(context));
...
StandaloneCustomKernel::StandaloneCustomKernel(Ort::CustomOpApi ort, const OrtKernelInfo* info) : ort_(ort) {
