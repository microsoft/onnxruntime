...
struct OCRCustomKernel {
  ...
  void GetOutputShape(OrtValue** inputs, size_t input_count, size_t output_index, OrtTensorTypeAndShapeInfo* info) {
    OrtTensorDimensions dimensions(ort_, inputs[0]);
    ort_.SetDims(info, dimensions.data(), dimensions.size());
  }

  void Compute(OrtValue** inputs, size_t input_count, OrtValue** outputs, size_t output_count) {
    const float* X;
    const float* Y;
    ort_.GetTensorMutableData(inputs[0], &reinterpret_cast<void*>(const_cast<float*>(X)));
    ort_.GetTensorMutableData(inputs[0], &reinterpret_cast<void*>(const_cast<float*>(Y)));

    float* out;
    ort_.GetTensorMutableData(outputs[0], &reinterpret_cast<void*>(out));
...
struct OCRCustomOp : OrtCustomOp {
  OCRCustomOp() {
    OrtCustomOp::version = ORT_API_VERSION;
    OrtCustomOp::CreateKernel = [](OrtCustomOp* this_, const OrtCustomOpApi* ort, const OrtKernelInfo* info, void** output) { *output = new OCRCustomKernel(*ort, *info); };
    OrtCustomOp::GetName = [](OrtCustomOp* this_) { return "OneOCRSeglinkProposals"; };
...
TEST_F(CApiTest, custom_op_handler_ocr) {
  ...
  OrtCustomOpDomain* custom_op_domain = OrtCreateCustomOpDomain("com.microsoft.oneocr");
