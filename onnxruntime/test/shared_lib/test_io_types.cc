...
static void TestModelInfo(const Ort::Session& inference_session, bool is_input, const std::vector<int64_t>& dims) {
  size_t input_count;
  if (is_input) {
    input_count = inference_session.GetInputCount();
  } else {
    input_count = inference_session.GetOutputCount();
  }
  ASSERT_EQ(1, input_count);
  Ort::TypeInfo input_type_info{nullptr};
  if (is_input) {
    inference_session.GetInputTypeInfo(0);
  } else {
    inference_session.GetOutputTypeInfo(0);
  }
