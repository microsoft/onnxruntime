...
def test_sysconf_fallback():
    """
    Test 2: Verify sysconf(_SC_NPROCESSORS_ONLN) works as a complete fallback.
    ...
    print()
    print("=" * 60)
    print("Test 2: sysconf(_SC_NPROCESSORS_ONLN) count and flag fallback validation")
    print("=" * 60)
...
def test_sysfs_hide_with_ld_preload():
    """
    ...
    print()
    print("=" * 60)
    print("Test 3: LD_PRELOAD sysfs-hiding shim")
    print("=" * 60)
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Compile shim
        result = subprocess.run(
            ["gcc", "-shared", "-fPIC", "-o", shim_so, shim_path, "-ldl"], check=False, capture_output=True, text=True
        )
        if result.returncode != 0:
            print(f"FAIL: Shim compilation failed: {result.stderr}")
            return False

        # Compile test
        result = subprocess.run(["gcc", "-o", test_exe, test_path], check=False, capture_output=True, text=True)
        if result.returncode != 0:
            print(f"FAIL: Test compilation failed: {result.stderr}")
            return False
...
def test_ort_import_with_hidden_sysfs():
    """
    Test 4: Integration test - import onnxruntime with hidden sysfs files.
    ...
    print()
    print("=" * 60)
    print("Test 4: Import onnxruntime with hidden sysfs (integration)")
    print("=" * 60)
    ...
    if result.returncode != 0:
        print("SKIP: onnxruntime not installed/importable")
        return None
...
def main():
    ...
    results["safe_logging"] = test_safe_logging_pattern()
    results["sysconf_fallback"] = test_sysconf_fallback()
    results["ld_preload_shim"] = test_sysfs_hide_with_ld_preload()
    results["ort_import"] = test_ort_import_with_hidden_sysfs()
