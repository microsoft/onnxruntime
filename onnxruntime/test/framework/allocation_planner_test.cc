...
class AllocationPlanTestUtility {
 public:
  ...
  static void CheckToBeFreed(const SequentialExecutionPlan::NodesExecutionPlan& plan, const std::vector<OrtValueIndex>& expected) {
  ...
  static void CheckFreedAtEachStep(const SequentialExecutionPlan::NodesExecutionPlan& plan, const std::vector<int>& expected_num_freed) {
    ASSERT_EQ(plan.nodes_execution_order.size(), expected_num_freed.size()) << "Execution plan is of wrong size";
    int start = 0;
    for (size_t i = 0; i < expected_num_freed.size(); ++i) {
      if (expected_num_freed[i] > 0) {
        EXPECT_EQ(plan.nodes_execution_order[i].free_from_index, start) << "Error in free_from_index at position " << i;
        EXPECT_EQ(plan.nodes_execution_order[i].free_to_index, start + expected_num_freed[i] - 1)
        ...
        EXPECT_GT(plan.nodes_execution_order[i].free_from_index, plan.nodes_execution_order[i].free_to_index);
      }
    }
  }

  static void BasicIntegrityCheck(const SequentialExecutionPlan::NodesExecutionPlan& plan, size_t num_ml_values) {
    ...
    for (const SequentialExecutionPlan::NodeExecutionPlan& step : plan.nodes_execution_order) {
...
class PlannerTest : public ::testing::Test {
 ...
 public:
  ...
  void CreatePlan(const std::vector<const NodeArg*>& outer_scope_node_args = {}) {
    ...
    EXPECT_TRUE(providers.Add(kCpuExecutionProvider, nullptr).IsOK());
    class MockStreamHandleRegsitry : public IStreamCommandHandleRegistry {
     public:
      ...
      virtual CreateStreamFn GetCreateStreamFn(const std::string& execution_provider_type) override {
        return nullptr;
      }
    ...
    status = SequentialPlanner::CreatePlan(nullptr, GraphViewer(graph_), outer_scope_node_args, execution_providers_,
                                           ...
                                           providers, MockStreamHandleRegsitry(), {}, {}, false,
                                           plan_);
    EXPECT_TRUE(status.IsOK()) << status.ErrorMessage();
    // default tests are single stream case.
    EXPECT_EQ(plan_->execution_plan.size(), 1);
    auto& seq_exec_plan = plan_->execution_plan[0];
    AllocationPlanTestUtility::BasicIntegrityCheck(seq_exec_plan, name_to_arg_.size());
  ...
  void CheckFreed(int step_number, std::initializer_list<std::string> freed_items) {
    ...
    EXPECT_EQ(plan_->execution_plan.size(), 1);
    auto& step_plan = plan_->execution_plan[0].nodes_execution_order[step_number];
    for (int i = step_plan.free_from_index; i <= step_plan.free_to_index; ++i)
      plan_result.insert(plan_->execution_plan[0].to_be_freed[i]);
