...
struct InsertIndices {
  void operator()(bool indices_1D, size_t values_size, size_t shape_size, TensorProto& indices_tp) const {
    ...
    ORT_IF_CONSTEXPR (sizeof(T) == sizeof(int8_t)) {
      ...
      std::vector<T> indices(indices_data.begin(), indices_data.end());
...
static NodeProto CreateConstantNode(bool indices_1D, int32_t indices_type,
  ...
  stp.mutable_dims()->Add(shape.begin(), shape.end());
...
static void RawDataChecker(gsl::span<const T> expected, const TensorProto& actual) {
  ...
  AssertSpanEquals(actual_span, expected);
}

template <>
void RawDataChecker<MLFloat16>(gsl::span<const MLFloat16> expected_bfloat, const TensorProto& actual) {
  int64_t actual_size = ActualSize(actual);
  auto expected = gsl::make_span<const uint16_t>(reinterpret_cast<const uint16_t*>(expected_bfloat.data()), expected_bfloat.size_bytes() / sizeof(uint16_t));
...
TEST(SparseTensorConversionTests, TestConstantNodeConversion) {
  TestConversion<float>(
      [](const std::vector<float>& values, TensorProto& tp) {
        ...
        tp.mutable_float_data()->Add(values.begin(), values.end());
      },
  ...
  TestConversion<double>(
      [](const std::vector<double>& values, TensorProto& tp) {
        ...
        tp.mutable_double_data()->Add(values.begin(), values.end());
      },
  ...
  TestConversion<int16_t>(
      [](const std::vector<int16_t>& values, TensorProto& tp) {
        ...
        tp.mutable_int32_data()->Add(values.begin(), values.end());
      },
  ...
  TestConversion<uint32_t>(
      [](const std::vector<uint32_t>& values, TensorProto& tp) {
        ...
        tp.mutable_uint64_data()->Add(values.begin(), values.end());
      },
  ...
  TestConversion<int64_t>(
      [](const std::vector<int64_t>& values, TensorProto& tp) {
        ...
        tp.mutable_int64_data()->Add(values.begin(), values.end());
      },
...
static void RawSparseDataChecker(gsl::span<const T> expected_values,
  ...
  AssertSpanEquals(actual_span, expected_values);
...
TEST(SparseTensorConversionTests, CsrConversion) {
  ...
  Tensor dense_cpu_src(DataTypeImpl::GetType<int32_t>(), dense_shape, dense_data.data(), cpu_allocator->Info());
  {
    ...
    ASSERT_TRUE(std::equal(expected_values.begin(), expected_values.end(), values.begin(), values.end()));

    auto csr_view = dst.AsCsr();
    auto inner = csr_view.Inner().DataAsSpan<int64_t>();
    ASSERT_EQ(expected_inner.size(), inner.size());
    ASSERT_TRUE(std::equal(expected_inner.begin(), expected_inner.end(), inner.begin(), inner.end()));

    auto outer = csr_view.Outer().DataAsSpan<int64_t>();
    ASSERT_EQ(expected_outer.size(), outer.size());
    ASSERT_TRUE(std::equal(expected_outer.begin(), expected_outer.end(), outer.begin(), outer.end()));
    ...
    ASSERT_TRUE(std::equal(dense_values_dst.begin(), dense_values_dst.end(), dense_data.begin(), dense_data.end()));
  }
  ...
  // Strings test
  {
    ...
    ASSERT_TRUE(std::equal(expected_values_str.begin(), expected_values_str.end(), values.begin(), values.end()));
    ...
    ASSERT_EQ(dense_values_dst.size(), dense_data.size());
    ASSERT_TRUE(std::equal(dense_values_dst.begin(), dense_values_dst.end(), dense_data_str.begin(), dense_data_str.end()));
  }
  ...
  }
  {
    ...
    ASSERT_TRUE(std::equal(dense_dst_data.begin(), dense_dst_data.end(), dense_data.begin(), dense_data.end()));
  }
  {
    ...
    ASSERT_TRUE(std::equal(dense_dst_data.begin(), dense_dst_data.end(), dense_data_all_zeros.begin(), dense_data_all_zeros.end()));
  }
#endif
}
...
TEST(SparseTensorConversionTests, CooConversion) {
  ...
  Tensor dense_cpu_src(DataTypeImpl::GetType<int32_t>(), dense_shape, dense_data.data(), cpu_allocator->Info());
  {
    ...
    ASSERT_TRUE(std::equal(indices.begin(), indices.end(), expected_linear_indices.begin(), expected_linear_indices.end()));
...

  {
    ...
    ASSERT_TRUE(std::equal(expected_linear_indices.begin(), expected_linear_indices.end(), indices.begin(), indices.end()));
  }

  {
    ...
    ASSERT_TRUE(std::equal(indices.begin(), indices.end(), expected_2d_indices.begin(), expected_2d_indices.end()));
...
TEST(SparseTensorConversionTests, BlockSparse) {
...

  {
    ...
    ASSERT_TRUE(std::equal(data_blocks.begin(), data_blocks.end(), data_span.begin(), data_span.end()));
    ...
    ASSERT_TRUE(std::equal(blocksparse_indices.begin(), blocksparse_indices.end(),
