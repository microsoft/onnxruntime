...
void RunModel(InferenceSession& session_object,
  ...
  OrtValue ml_value;
  CreateMLValue<float>(TestCPUExecutionProvider()->GetCachedAllocators()[0], dims_mul_x, values_mul_x,
                       &ml_value);
  NameMLValMap feeds;
...
void RunModelWithBindingMatMul(InferenceSession& session_object,
  ...
  CreateMLValue<float>(TestCPUExecutionProvider()->GetCachedAllocators()[0], dims_mul_x_B, values_mul_x,
                       &input_ml_value_B);
  ...
  if (is_preallocate_output_vec) {
    if (allocation_provider == kCpuExecutionProvider) {
      AllocateMLValue<float>(TestCPUExecutionProvider()->GetCachedAllocators()[0], expected_output_dims,
                             &output_ml_value);
    } else if (allocation_provider == kCudaExecutionProvider || allocation_provider == kRocmExecutionProvider) {
      AllocateMLValue<float>(gpu_provider->GetCachedAllocators()[0], expected_output_dims, &output_ml_value);
    } else {
  ...
  if ((is_preallocate_output_vec && (allocation_provider == kCudaExecutionProvider || allocation_provider == kRocmExecutionProvider)) ||
    ...
    auto cpu_allocator = TestCPUExecutionProvider()->GetCachedAllocators()[0];
...
TEST(InferenceSessionTests, TestIOBindingReuse) {
  ...
  OrtValue ml_value1;
  vector<float> v1{2.f};
  CreateMLValue<float>(TestCPUExecutionProvider()->GetCachedAllocators()[0], {1}, v1, &ml_value1);
  ...
  OrtValue ml_value2;
  vector<float> v2{3.f};
  CreateMLValue<float>(TestCPUExecutionProvider()->GetCachedAllocators()[0], {1}, v2, &ml_value2);
...
TEST(InferenceSessionTests, InvalidInputTypeOfTensorElement) {
  ...
  OrtValue ml_value;
  CreateMLValue<int64_t>(TestCPUExecutionProvider()->GetCachedAllocators()[0], dims_mul_x, values_mul_x,
                         &ml_value);
  NameMLValMap feeds;
  feeds.insert(std::make_pair("X", ml_value));
...
static common::Status RunOptionalInputTest(bool add_required_input,
  ...
  CreateMLValue<float>(TestCPUExecutionProvider()->GetCachedAllocators()[0],
...
TEST(ExecutionProviderTest, FunctionTest) {
  ...
  std::vector<float> values_mul_x = {1.0f, 2.0f, 3.0f, 4.0f, 5.0f, 6.0f};
  OrtValue ml_value_x;
  CreateMLValue<float>(TestCPUExecutionProvider()->GetCachedAllocators()[0], dims_mul_x, values_mul_x, &ml_value_x);
  OrtValue ml_value_y;
  CreateMLValue<float>(TestCPUExecutionProvider()->GetCachedAllocators()[0], dims_mul_x, values_mul_x, &ml_value_y);
  OrtValue ml_value_z;
  CreateMLValue<float>(TestCPUExecutionProvider()->GetCachedAllocators()[0], dims_mul_x, values_mul_x, &ml_value_z);
  NameMLValMap feeds;
...
TEST(InferenceSessionTests, Test3LayerNestedSubgraph) {
  ...
  OrtValue ml_value_x;
  CreateMLValue<bool>(TestCPUExecutionProvider()->GetCachedAllocators()[0], dim, va,
                      &ml_value_x);
  NameMLValMap feeds;
...
TEST(InferenceSessionTests, Test2LayerNestedSubgraph) {
  ...
  CreateMLValue<float>(TestCPUExecutionProvider()->GetCachedAllocators()[0], dim_input_0, data_input_0,
                       &ml_value_input_0);
  std::vector<int64_t> dim_input_1 = {1};
  std::vector<bool> data_input_1 = {false};
  OrtValue ml_value_input_1;
  CreateMLValue<bool>(TestCPUExecutionProvider()->GetCachedAllocators()[0], dim_input_1, data_input_1,
                      &ml_value_input_1);
  NameMLValMap feeds;
...
TEST(InferenceSessionTests, TestTruncatedSequence) {
  ...
  OrtValue ml_value;
  CreateMLValue<float>(TestCPUExecutionProvider()->GetCachedAllocators()[0], X_dims, X, &ml_value);
  ...
  for (auto truncated_len : truncated_lengths) {
    ...
    CreateMLValue<float>(TestCPUExecutionProvider()->GetCachedAllocators()[0], truncated_input_dims, truncated_input, &truncated_ml_value);
...
TEST(InferenceSessionTests, TestCopyToFromDevices) {
  ...
  std::vector<float> values_mul_x = {1.0f, 2.0f, 3.0f, 4.0f, 5.0f, 6.0f};
  OrtValue ml_value;
  CreateMLValue<float>(p_dummy_provider->GetCachedAllocators()[0], dims_mul_x, values_mul_x,
                       &ml_value);
...
TEST(InferenceSessionTests, ModelThatTriggersAllocationPlannerToReuseDoubleTensorForStringTensor) {
  ...
  OrtValue ml_value;
  CreateMLValue<float>(TestCPUExecutionProvider()->GetCachedAllocators()[0], dims_x, values_x,
                       &ml_value);
  NameMLValMap feeds;
...
TEST(InferenceSessionTests, InitializerSharing_EnsureSessionsUseUserAddedInitializer) {
  ...
  auto allocator = TestCPUExecutionProvider()->GetCachedAllocators()[0];
