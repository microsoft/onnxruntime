...
void RunModelWithBindingMatMul(InferenceSession& session_object,
  ...
  // Now run
  ASSERT_STATUS_OK(session_object.Run(run_options, *io_binding.get()));
  ...
  if ((is_preallocate_output_vec && (allocation_provider == kCudaExecutionProvider || allocation_provider == kRocmExecutionProvider)) ||
    ...
    ASSERT_STATUS_OK(GetProviderInfo_CUDA().CreateGPUDataTransfer(stream)->CopyTensor(rtensor, *cpu_tensor.get(), 0));
#endif
#ifdef USE_ROCM
    hipStream_t stream = static_cast<hipStream_t>(gpu_provider->GetComputeStream());
    ASSERT_STATUS_OK(GetProviderInfo_ROCM().CreateGPUDataTransfer(stream)->CopyTensor(rtensor, *cpu_tensor.get(), 0));
#endif
    OrtValue ml_value;
...
TEST(InferenceSessionTests, TestTruncatedSequence) {
  ...
  static constexpr const ORTCHAR_T* LSTM_MODEL_URI = ORT_TSTR("testdata/scan_1.onnx");
  // This model is a 4x forward LSTM. Parse it to find out mapping between init_state input/output
  ONNX_NAMESPACE::ModelProto model_proto;
  int model_fd;
  ASSERT_STATUS_OK(Env::Default().FileOpenRd(LSTM_MODEL_URI, model_fd));
  ...
  SessionOptions so;
  InferenceSession session_object(so, GetEnvironment());
  ASSERT_STATUS_OK(session_object.Load(LSTM_MODEL_URI));
  ASSERT_STATUS_OK(session_object.Initialize());
...
TEST(InferenceSessionTests, TestCopyToFromDevices) {
  ...
  auto run_test = [&](int run_num) {
    ...
    ASSERT_STATUS_OK(session_object.Run(run_options, feed_names, feeds, output_names, &fetches, nullptr));
...
TEST(InferenceSessionTests, TestL1AndL2Transformers) {
  ...
  std::vector<const ORTCHAR_T*> test_model_uris = {ORT_TSTR("testdata/transform/fusion/fuse-conv-bn-mul-add-unsqueeze.onnx"),
                                              ORT_TSTR ("testdata/transform/abs-id-max.onnx"),
                                              ORT_TSTR ("testdata/transform/slice-v11-elim.onnx"),
                                              ORT_TSTR ("testdata/transform/matmul_add_fusion/2Input/model.onnx"),
                                              ORT_TSTR ("testdata/transform/matmul_add_fusion/3Input/gemm_relu.onnx"),
                                              ORT_TSTR ("testdata/transform/fusion/fuse-conv-bn-add-mul-float16.onnx")};

  for (const ORTCHAR_T* model_uri : test_model_uris) {
    SessionOptions so;
    so.session_logid = "InferenceSessionTests.TestL1AndL2Transformers";
