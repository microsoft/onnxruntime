...
TEST(InferenceSessionTests, InvalidInputShape_InvalidRank) {
  SessionOptions so;

  so.session_logid = "InferenceSessionTests.InvalidInputShape_InvalidRank";
  ...
  // prepare inputs
  std::vector<int64_t> dims_mul_x = {3};
  std::vector<float> values_mul_x = {1, 2, 3};
  OrtValue ml_value;
  CreateMLValue<float>(TestCPUExecutionProvider()->GetAllocator(0, OrtMemTypeDefault), dims_mul_x, values_mul_x,
                       &ml_value);
  NameMLValMap feeds;
...
TEST(InferenceSessionTests, InvalidInputShape_InvalidDims) {
  SessionOptions so;

  so.session_logid = "InferenceSessionTests.InvalidInputShape_InvalidDims";
  ...
  // prepare inputs
  std::vector<int64_t> dims_mul_x = {3, 4};
  std::vector<float> values_mul_x(3 * 4);
  std::iota(values_mul_x.begin(), values_mul_x.end(), 1.f);
