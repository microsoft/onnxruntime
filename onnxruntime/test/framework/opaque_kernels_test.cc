...
TEST_F(OpaqueTypeTests, RunModel) {
  ...
  // prepare inputs
  OrtValue ml_values;
  CreateMLValue<int64_t>(TestAllocatorManager().GetAllocator(OrtDevice()), val_dims, values, &ml_values);

  std::vector<int64_t> ind_dims = {2};
  std::vector<int64_t> indicies = {1, 4};
  OrtValue ml_indicies;
  CreateMLValue<int64_t>(TestAllocatorManager().GetAllocator(OrtDevice()), ind_dims, indicies, &ml_indicies);

  std::vector<int64_t> shape_dims = {1};
  std::vector<int64_t> shape = {5};
  OrtValue ml_shape;
  CreateMLValue<int64_t>(TestAllocatorManager().GetAllocator(OrtDevice()), shape_dims, shape, &ml_shape);
