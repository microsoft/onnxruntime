...
static std::shared_ptr<Model> MakeCpuFp16GemmModelWithShapes(const std::string& model_name,
                                                             std::initializer_list<int64_t> a_shape,
                                                             std::initializer_list<int64_t> b_shape,
  ...
  TypeProto a_type = MakeFp16TensorType(a_shape);
  TypeProto b_type = MakeFp16TensorType(b_shape);
  TypeProto c_type = MakeFp16TensorType(c_shape);
  TypeProto y_type = MakeFp16TensorType(y_shape);

  auto& a = graph.GetOrCreateNodeArg("A", &a_type);
  auto& b = graph.GetOrCreateNodeArg("B", &b_type);
  auto& c = graph.GetOrCreateNodeArg("C", &c_type);
  auto& y = graph.GetOrCreateNodeArg("Y", &y_type);

  auto& node = graph.AddNode("Gemm", "Gemm", "fp16 gemm shape heuristic test",
                             ArgMap{&a, &b, &c}, ArgMap{&y});
  node.AddAttribute("transA", trans_a);
  node.AddAttribute("transB", trans_b);
...
static void ExpectCpuFp16GemmHeuristicUsesFp32(const std::shared_ptr<Model>& model) {
  auto& graph = model->MainGraph();
...
static void ExpectNativeCpuFp16OrFp32Fallback(const Graph& graph, const Node& node,
                                              size_t expected_fallback_cast_count) {
  const bool has_native_fp16_kernel = MlasHalfGemmAccelerationSupported(nullptr);
  const auto op_counts = CountOpsInGraph(graph);
  const auto cast_count = op_counts.count("Cast") == 0 ? 0U : static_cast<size_t>(op_counts.at("Cast"));
  EXPECT_EQ(cast_count, has_native_fp16_kernel ? 0U : expected_fallback_cast_count);

  const auto expected_type = has_native_fp16_kernel
                                 ? DataTypeImpl::GetTensorType<MLFloat16>()
                                 : DataTypeImpl::GetTensorType<float>();
  for (const auto* input_def : node.InputDefs()) {
    EXPECT_TRUE(IsNodeArgType(*input_def, expected_type));
  }
  for (const auto* output_def : node.OutputDefs()) {
    EXPECT_TRUE(IsNodeArgType(*output_def, expected_type));
  }
}

TEST(TransformerTest, CpuFp16MatMulUsesAvailableKernelOrFallbackWhenEnabled) {
  {
    ...
    ExpectNativeCpuFp16OrFp32Fallback(graph, *node, 3U);
  }
}

TEST(TransformerTest, CpuFp16CpuAssignedMatMulUsesAvailableKernelOrFallbackWhenEnabled) {
...
TEST(TransformerTest, CpuFp16CpuAssignedGemmUsesAvailableKernelOrFallbackWhenEnabled) {
  ...
  ExpectNativeCpuFp16OrFp32Fallback(graph, *node, 4U);
...
TEST(TransformerTest, CpuFp16UnassignedMatMulUsesAvailableKernelOrFallbackWhenEnabled) {
  auto model = MakeCpuFp16Model("cpu_fp16_matmul_unassigned", "MatMul", false);
  auto& graph = model->MainGraph();
  auto* node = graph.Nodes().begin().operator->();
  ASSERT_NE(node, nullptr);
...
TEST(TransformerTest, CpuFp16GemmHeuristicRoutesTransposedRhsByBackend) {
  constexpr int64_t kM = 1;
  constexpr int64_t kN = 4096;
  constexpr int64_t kK = 1024;
  auto model = MakeCpuFp16GemmModelWithShapes("cpu_fp16_gemm_heuristic_transposed_rhs",
                                              {kM, kK}, {kN, kK}, {kN}, {kM, kN},
                                              true, 0, 1);
  auto& graph = model->MainGraph();
  auto* node = graph.Nodes().begin().operator->();
  ASSERT_NE(node, nullptr);
  ...
  const bool has_transposed_rhs_packing =
      MlasHalfGemmNativePackBSize(CblasNoTrans, CblasTrans, kN, kK,
                                  &mlas_backend_kernel_selector_config) != 0;
  const auto op_counts = CountOpsInGraph(graph);
  EXPECT_EQ(op_counts.count("Cast"), has_transposed_rhs_packing ? 0U : 1U);
  if (!has_transposed_rhs_packing) {
    EXPECT_EQ(op_counts.at("Cast"), 4);
  }
  EXPECT_EQ(node->OpType(), "Gemm");
  EXPECT_EQ(node->GetExecutionProviderType(), kCpuExecutionProvider);
...
TEST(TransformerTest, CpuFp16GemmTransposedRhsSessionDefaultsToFp32) {
  constexpr int64_t kM = 14;
  constexpr int64_t kN = 4096;
  constexpr int64_t kK = 1024;
  const auto input_path = ToPathString("cpu_fp16_gemm_transposed_rhs.onnx");
  ORT_THROW_IF_ERROR(Model::Save(*MakeCpuFp16GemmModelWithShapes(
                                     "cpu_fp16_gemm_transposed_rhs",
                                     {kM, kK}, {kN, kK}, {kN}, {kM, kN}, false, 0, 1),
                                 input_path));

  SessionOptions so;
  so.session_logid = "cpu_fp16_gemm_transposed_rhs";
  so.graph_optimization_level = TransformerLevel::Level4;
  InferenceSessionWrapper session{so, GetEnvironment()};
  ...
  EXPECT_EQ(op_counts.at("Cast"), 4);
  const Node* gemm_node = FindNodeByOpType(session.GetGraph(), "Gemm");
  ASSERT_NE(gemm_node, nullptr);
  EXPECT_EQ(gemm_node->GetExecutionProviderType(), kCpuExecutionProvider);

  std::error_code ec;
  std::filesystem::remove(std::filesystem::path{input_path}, ec);
}

TEST(TransformerTest, CpuFp16GemmHeuristicRequiresAcceleratedBackend) {
  constexpr int64_t kM = 1;
  constexpr int64_t kN = 1024;
  constexpr int64_t kK = 1024;
  auto model = MakeCpuFp16GemmModelWithShapes("cpu_fp16_gemm_heuristic_backend",
                                              {kM, kK}, {kK, kN}, {kN}, {kM, kN}, true);
  auto& graph = model->MainGraph();
  auto* node = graph.Nodes().begin().operator->();
  ASSERT_NE(node, nullptr);
  ...
  const bool has_accelerated_half_gemm =
      ...
      MlasHalfGemmNativePackBSize(CblasNoTrans, CblasNoTrans, kN, kK,
                                  &mlas_backend_kernel_selector_config) != 0;
  const auto op_counts = CountOpsInGraph(graph);
  EXPECT_EQ(op_counts.count("Cast"), has_accelerated_half_gemm ? 0U : 1U);
...
TEST(TransformerTest, CpuFp16GemmHeuristicForcesUnsupportedParametersToFp32) {
  constexpr int64_t kM = 1;
  constexpr int64_t kN = 1024;
  constexpr int64_t kK = 1024;
  const std::vector<std::pair<float, float>> scales{{0.5f, 1.0f}, {1.0f, 0.5f}};
  for (const auto& [alpha, beta] : scales) {
    ExpectCpuFp16GemmHeuristicUsesFp32(
        MakeCpuFp16GemmModelWithShapes("cpu_fp16_gemm_heuristic_scale",
                                       {kM, kK}, {kK, kN}, {kN}, {kM, kN},
                                       true, 0, 0, alpha, beta));
  }

  ExpectCpuFp16GemmHeuristicUsesFp32(
      MakeCpuFp16GemmModelWithShapes("cpu_fp16_gemm_heuristic_bias_shape",
                                     {kM, kK}, {kK, kN}, {1, 1}, {kM, kN}, true));
}

TEST(TransformerTest, CpuFp16MatMulHeuristicKeepsConstantRhsBertLikeShapeNativeFp16) {
  if (MlasHalfGemmNativePackBSize(CblasNoTrans, CblasNoTrans, 1024, 512, nullptr) == 0) {
...
TEST(TransformerTest, CpuFp16SessionConfigRejectsInvalidBooleanValues) {
  const auto model = MakeCpuFp16Model("cpu_fp16_invalid_session_config", "MatMul", false);
  std::string model_data;
  ASSERT_TRUE(model->ToProto().SerializeToString(&model_data));

  for (const char* config_key : {kOrtSessionOptionsEnableCpuFp16,
                                 kOrtSessionOptionsCpuFp16UseFp32FallbackHeuristic}) {
    SCOPED_TRACE(config_key);

    SessionOptions so;
    ASSERT_STATUS_OK(so.config_options.AddConfigEntry(config_key, "true"));
    ...
    EXPECT_THAT(status.ErrorMessage(), ::testing::HasSubstr(config_key));
    EXPECT_THAT(status.ErrorMessage(), ::testing::HasSubstr("Valid values are \"0\" and \"1\"."));
  }
}
