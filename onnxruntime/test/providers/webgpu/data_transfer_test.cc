...
TEST(WebGpuDataTransferTest, AcceptsHostAndWebGpuEndpoints) {
  ...
  // WebGPU tensors always report device id 0, but a non-zero id must not change the decision.
...
TEST(WebGpuDataTransferTest, RejectsForeignGpuVendors) {
  // A vendor-tagged GPU handle is a foreign allocation (e.g. a CUDA pointer in a multi-EP build).
  // Copying it through the WebGPU transfer would reinterpret it as a WGPUBuffer.
