...
if (!fs.existsSync(ONNX_TEST_RUNNER_FILENAME)) {
    console.error('Error: onnx_test_runner not found.');
    console.error('Please perform a build and run this script in the build folder.');
    process.exit(1);
}
...
const tests = new Set(test_models);

const TEST_ROOT = path.join(__dirname, 'webgpu_test_root');

let test_data_ready = false;
const test_list_json_data = JSON.stringify(test_models, null, 2);
const test_list_json_filepath = path.join(TEST_ROOT, 'test_list.json');
if (fs.existsSync(TEST_ROOT)) {
    if (fs.existsSync(test_list_json_filepath)) {
        test_data_ready = fs.readFileSync(test_list_json_filepath).toString() == test_list_json_data;
    }
    if (!test_data_ready) {
        ...
        if (dirent.isDirectory()) {
            ...
            fs.readdirSync(path.join(test_data_path, opset), { withFileTypes: true }).forEach(dirent => {
                if (dirent.isDirectory()) {
                    ...
                    if (tests.has(name)) {
                        fs.symlinkSync(path.join(test_data_path, opset, name), path.join(TEST_ROOT, `${opset}_${name}`), 'junction');
                    }
                }
            });
        }
...
const args = ['-A', '-M', '-j', '1', '-e', 'webgpu', '-C', '"session.disable_cpu_ep_fallback|1 storageBufferCacheMode|disabled"', 'webgpu_test_root'];
if (VERBOSE) {
    args.unshift('-v');
}
process.exit(
