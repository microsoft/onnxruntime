...
TEST(InternalTestingEP, TestRegisterAllocatorHandlesUsageInMultipleSessions) {
  ...
  ASSERT_EQ(std::make_shared<CPUAllocator>(),
...
TEST(InternalTestingEP, TestReplaceAllocatorDoesntBreakDueToLocalAllocatorStorage) {
  ...
  ASSERT_EQ(replacement_alloc, std::make_shared<CPUAllocator>());
  ...
  ASSERT_NE(replacement_alloc, std::make_shared<CPUAllocator>());
  ASSERT_EQ(replacement_alloc, std::make_shared<CPUAllocator>());
}
...
TEST(InternalTestingEP, TestModelWithSubgraph) {
  ...
  CreateMLValue<float>(std::make_shared<CPUAllocator>(), {1}, {-2.f},
                       &ml_value);
  NameMLValMap feeds;
