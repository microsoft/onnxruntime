# ONNX Runtime QNN ABI Execution Provider Tests
## Overview
1. The `onnxruntime/test/providers/qnn-abi` directory contains integration tests for the Qualcomm Neural Network (QNN) execution provider.
...
2. Saving Test Artifacts
    ...
    - The artifacts will be saved to a directory named with `<TestSuite>_<TestName>`
        ```
        .
        ├── QnnABICPUBackendTests_BatchNorm2D_fp32      # RunQnnModelTest
        │   ├── dumped_f32_model.onnx                   # float32 ONNX model
        │   ├── QNNExecutionProvider_QNN_XXXX_X_X.dlc
        │   └── QNNExecutionProvider_QNN_XXXX_X_X.json
        ├── QnnABIHTPBackendTests_BatchNorm_FP16        # TestFp16ModelAccuracy
        │   ├── dumped_f16_model.onnx                   # float16 ONNX model
        │   ├── dumped_f32_model.onnx                   # float32 ONNX model
        │   ├── QNNExecutionProvider_QNN_XXXX_X_X.dlc
        │   └── QNNExecutionProvider_QNN_XXXX_X_X.json
        └── QnnABIHTPBackendTests_BatchNorm2D_U8U8S32   # TestQDQModelAccuracy
            ├── dumped_f32_model.onnx                   # float32 ONNX model
