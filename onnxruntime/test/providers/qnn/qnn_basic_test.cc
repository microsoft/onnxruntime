...
TEST_F(QnnHTPBackendTests, SliceQDQPropagation_MultConsumers) {
  Ort::SessionOptions so;
  ...
  const ORTCHAR_T* ort_model_path = ORT_MODEL_FOLDER "slice_qdq_propagation_mult_consumers.onnx";
  Ort::Session session(*ort_env, ort_model_path, so);

  // image: 1,3,640,640
  std::vector<uint8_t> input0_data(1 * 3 * 640 * 640);
  ...
  // Add input0
  std::array<int64_t, 4> inputs_shape{1, 3, 640, 640};
  ort_inputs.emplace_back(Ort::Value::CreateTensor<uint8_t>(
      memory_info, input0_data.data(), input0_data.size(), inputs_shape.data(), inputs_shape.size()));
  ort_input_names.push_back("image");
  ...
  EXPECT_THAT(output_shape, ::testing::ElementsAre(1, 8400, 4));
  const uint8_t* results = ort_output.GetTensorData<uint8_t>();

  for (size_t i = 0; i < typeshape.GetElementCount() && i < 10; i++) {
