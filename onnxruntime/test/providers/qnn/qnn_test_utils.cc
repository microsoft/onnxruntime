...
void RunQnnModelTest(const GetTestModelFn& build_test_case, const ProviderOptions& provider_options,
  ...
  std::function<void(const Graph&)> graph_verify = [num_nodes_in_graph, test_description](const Graph& graph) -> void {
    ASSERT_EQ(graph.NumberOfNodes(), num_nodes_in_graph) << test_description;
  };
