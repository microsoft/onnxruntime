...
TEST(MathOpTest, And_1) {
  OpTester test("And");
  std::vector<int64_t> dims_a{4};
  std::vector<int64_t> dims_b{1};
  test.AddInput<bool>("A", dims_a, {false, true, false, true});
  test.AddInput<bool>("B", dims_b, {true});
  test.AddOutput<bool>("C", dims_a, {false, true, false, true});
  test.Run();
}

TEST(MathOpTest, And_2) {
  OpTester test("And");
  std::vector<int64_t> dims_a{5};
  std::vector<int64_t> dims_b{1};
  test.AddInput<bool>("A", dims_a, {false, true, false, true, false});
  test.AddInput<bool>("B", dims_b, {true});
  test.AddOutput<bool>("C", dims_a, {false, true, false, true, false});
  test.Run();
}

TEST(MathOpTest, And_Bcast3v1d) {
  OpTester test("And");
    ...
    for (size_t j = 0; j < 4; j++) {
      for (size_t k = 0; k < 5; k++) {
        for (size_t h = 0; h < 6; h++) {
          a[i * stride1 + j * stride2 + k * stride3 + h] = (std::rand() % 2) == 0;
        }
      }
    }
  }
...
TEST(MathOpTest, And_Bcast4v4d) {
  OpTester test("And");
  std::vector<int64_t> dims_c{3, 4, 5, 6};
  std::vector<int64_t> dims_a{1, 4, 1, 6};
  std::vector<int64_t> dims_b{3, 1, 5, 6};
  bool a[1 * 4 * 1 * 6];
  bool b[3 * 5 * 6];
  bool c[3 * 4 * 5 * 6];
  std::srand(std::time(0));
  for (size_t i = 0; i < 1; i++) {
    for (size_t j = 0; j < 4; j++) {
      for (size_t k = 0; k < 1; k++) {
        for (size_t h = 0; h < 6; h++) {
          a[i * 4 * 6 + j * 6 + k * 6 + h] = (std::rand() % 2) == 0;
        }
      }
    }
  }
  for (size_t i = 0; i < 3; i++) {
    for (size_t j = 0; j < 1; j++) {
      for (size_t k = 0; k < 5; k++) {
        for (size_t h = 0; h < 6; h++) {
          b[i * 5 * 6 + j * 5 * 6 + k * 6 + h] = (std::rand() % 2) == 0;
        }
      }
    }
  }
  ...
  for (size_t i = 0; i < 3; i++) {
    for (size_t j = 0; j < 4; j++) {
      for (size_t k = 0; k < 5; k++) {
        for (size_t h = 0; h < 6; h++) {
          c[i * 4 * 5 * 6 + j * 5 * 6 + k * 6 + h] = (a[j * 6 + h]) && (b[i * 5 * 6 + k * 6 + h]);
        }
      }
    }
  }
  ...
  test.AddInput<bool>("A", dims_a, a, 4 * 6);
  test.AddInput<bool>("B", dims_b, b, 3 * 5 * 6);
  test.AddOutput<bool>("C", dims_c, c, 3 * 4 * 5 * 6);
  test.Run();
}

TEST(MathOpTest, And_4) {
  OpTester test("And");
  std::vector<int64_t> dims_a{3, 4, 5, 8};
  std::vector<int64_t> dims_b{5, 1};
  bool a[3 * 4 * 5 * 8];
  bool b[5 * 1];
  bool c[3 * 4 * 5 * 8];
  size_t stride1 = 4 * 5 * 8;
  size_t stride2 = 5 * 8;
  size_t stride3 = 8;
  std::srand(std::time(0));
  for (size_t i = 0; i < 3; i++) {
  ...
  for (size_t i = 0; i < 3; i++) {
    for (size_t j = 0; j < 4; j++) {
      for (size_t k = 0; k < 5; k++) {
        for (size_t h = 0; h < 8; h++) {
          c[i * stride1 + j * stride2 + k * stride3 + h] = (a[i * stride1 + j * stride2 + k * stride3 + h]) && (b[k]);
        }
      }
    }
  }
  ...
  test.AddInput<bool>("A", dims_a, a, 3 * 4 * 5 * 8);
  test.AddInput<bool>("B", dims_b, b, 5 * 1);
  test.AddOutput<bool>("C", dims_a, c, 3 * 4 * 5 * 8);
  test.Run();
}
