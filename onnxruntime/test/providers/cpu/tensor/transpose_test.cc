...
static void TestTransposeFloat16(
    const std::vector<int64_t>& perm,
    const std::vector<int64_t>& x_dims,
    const std::vector<int64_t>& y_dims,
    double error_tolerance = 1e-4) {
...
TEST(TransposeOpTest, TransposeBigFloat16) { // Exercises CanUse_cublasTransposeHelper_MLFloat16 (cuda 65535 grid dimension limit)
  ...
  TestTransposeFloat16(perm, X_dims, Y_dims);
}
