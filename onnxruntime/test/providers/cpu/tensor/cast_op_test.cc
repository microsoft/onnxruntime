...
TEST(CastOpTest, Float8E4M3FNToInt4x2SaturateTrue) {
  // GIVEN
  const std::vector<int64_t> shape{2, 2, 2};
  ...
  for (float val : input_values) {
    ...
    std::cout << "Float8(" << val << ") saturate=true: " << static_cast<float>(float8_input.back()) << std::endl;
  }
...
TEST(CastOpTest, Float8E4M3FNToInt4x2SaturateFalse) {
  // GIVEN
  const std::vector<int64_t> shape{2, 2, 2};
  std::vector<Float8E4M3FN> float8_input;
  const std::vector<float> input_values = {-8.0f, 7.0f, 0.0f, -1.0f, 3.0f, -5.0f, 6.0f, 2.0f};
  for (float val : input_values) {
    float8_input.emplace_back(Float8E4M3FN(val, false));
    std::cout << "Float8(" << val << ") saturate=false: " << static_cast<float>(float8_input.back()) << std::endl;
  }
...
TEST(CastOpTest, Float8E4M3FNToUInt4x2) {
  ...
  for (float val : uint_input_values) {
    uint_float8_input.emplace_back(Float8E4M3FN(val, false));
    std::cout << "Float8(" << val << ") saturate=false: " << static_cast<float>(uint_float8_input.back()) << std::endl;
  }
...
TEST(CastOpTest, DummyTestFloat8E4M3FNSaturation) {
  Float8E4M3FN x_saturate(-8.0f, true);
  std::cout << "Float8(-8.0f) saturate=true: " << static_cast<float>(x_saturate) << std::endl;
  EXPECT_EQ(static_cast<float>(x_saturate), -8.0f);

  Float8E4M3FN x_no_saturate(-8.0f, false);
  std::cout << "Float8(-8.0f) saturate=false: " << static_cast<float>(x_no_saturate) << std::endl;
  EXPECT_EQ(static_cast<float>(x_no_saturate), -8.0f);
}
