...
TEST(SqueezeOpTest, Squeeze_1) {
  ...
  test.Run(OpTester::ExpectResult::kExpectSuccess, "",  {kIntelExecutionProvider}); //Incorrect precision. Will be re-enabled after it's fixed
}
...
TEST(SqueezeOpTest, SqueezeNegAxis_2) {
  ...
  //Intel EP Incorrect precision. Will be re-enabled after its fixed
  test.Run(OpTester::ExpectResult::kExpectSuccess, "",  {kNGraphExecutionProvider, kIntelExecutionProvider});
}
