...
static void RunTest_v8(const std::string test_name, int64_t batch_size, int64_t max_sequence_len, int64_t input_size,
  ...
  Model model(test_name, false, ::onnxruntime::test::DefaultLoggingManager().DefaultLogger());
...
TEST(Scan9, TransposeOutputDim2) {
  ...
  Model model("ScanBody", false, ::onnxruntime::test::DefaultLoggingManager().DefaultLogger());
  auto& graph = model.MainGraph();
