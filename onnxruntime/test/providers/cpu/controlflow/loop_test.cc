...
static const ONNX_NAMESPACE::GraphProto CreateSubgraph(const RunOptions& options) {
  ...
  Model model("Loop subgraph", false, ::onnxruntime::test::DefaultLoggingManager().DefaultLogger());
...
TEST(Loop, InfiniteLoopTermination) {
  auto create_subgraph = [](const RunOptions&) {
    Model model("Infinite Loop subgraph", false, ::onnxruntime::test::DefaultLoggingManager().DefaultLogger());
    auto& graph = model.MainGraph();
...
TEST(Loop, Opset11WithNoVariadicInputsAndOutputs) {
  auto create_subgraph = []() {
    Model model("Loop opset 11 op body graph", false, ::onnxruntime::test::DefaultLoggingManager().DefaultLogger());
    auto& graph = model.MainGraph();
