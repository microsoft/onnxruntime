...
TEST(Loop, SubgraphInputShadowsOuterScopeValue) {
  ...
  CreateMLValue<float>(std::make_shared<CPUAllocator>(), scalar, a, &ml_value);
  feeds.insert(std::make_pair("a", ml_value));
  CreateMLValue<float>(std::make_shared<CPUAllocator>(), scalar, b, &ml_value);
  feeds.insert(std::make_pair("b", ml_value));
  CreateMLValue<int64_t>(std::make_shared<CPUAllocator>(), scalar, trip_count, &ml_value);
  feeds.insert(std::make_pair("max_trip_count", ml_value));
  CreateMLValue<bool>(std::make_shared<CPUAllocator>(), scalar, keep_going, &ml_value);
...
TEST(Loop, BugFixIssue4031_implicit_input_handling) {
  ...
  // prepare inputs
  OrtValue ml_value;
  CreateMLValue<float>(std::make_shared<CPUAllocator>(), {1}, {123.f},
                       &ml_value);
  NameMLValMap feeds;
