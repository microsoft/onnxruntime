...
template <typename T>
void MemoryLayoutTransposeRNNOutputCNTKToONNXRuntime(const T* X_data_cntk, T* X_data_onnx, int64_t seq_length, int64_t num_directions, int64_t batch_size, int64_t hidden_size) {
  for (int seq = 0; seq < seq_length; seq++) {
...
TEST(RNNTest, RNN_bidirectional_bias_initial_zigged_batch) {
  ...
  std::vector<float> X_data_in_batchs{1.64644051F, 2.14556813F, 1.80829012F, 1.63464952F, 1.27096438F, 1.93768239F, 1.31276166F, 2.67531896F, 2.89098835F, 1.15032458F,
  ...
  std::vector<float> W_data({0.4317745F, 0.37378395F, -1.0386457F, -0.22681296F, 0.4418987F, 0.49973935F, 0.47248289F, -0.63369429F, 0.89542073F, 0.69698066F, 0.65118814F, 1.0828459F});
  test.AddInput<float>("W", W_dims, W_data);

  std::vector<int64_t> R_dims = {num_directions, hidden_size, hidden_size};
  std::vector<float> R_data({-0.24072374F, -0.29326528F, -0.91741192F, 0.5447638F, 0.53938544F, 0.79502326F, -0.59813821F, 0.020413321F, -0.52225035F,
  ...
  std::vector<float> B_data({-0.44529742F, 0.80094892F, -1.0028138F, 0.0F, 0.0F, 0.0F, 0.57412368F, 0.13440208F, -0.85748988F, 0.0F, 0.0F, 0.0F});
  ...
  std::vector<float> Y_h_data({0.51060104F, -0.95055139F, -0.12672578F, -0.28076148F, -0.04275616F, -0.75480938F, -0.71697658F, 0.99646497F, 0.9980582F, 0.1513377F, 0.90150106F, 0.74947751F});
...
TEST(RNNTest, RNN_bidirectional_zigged_batch) {
  ...
  std::vector<int64_t> W_dims = {num_directions, hidden_size, input_size};
  std::vector<float> W_data({-0.68526405F, 0.3758406F, 0.13007233F, 0.6596455F, -0.68564546F, 0.22745803F, 0.37704858F, -0.075543992F, -0.92860377F, -0.014112951F, -1.0042796F, 0.83100969F});
  test.AddInput<float>("W", W_dims, W_data);

  std::vector<int64_t> R_dims = {num_directions, hidden_size, hidden_size};
  std::vector<float> R_data({0.22057047F, -0.25696567F, 0.93817306F, -0.1917963F, -0.41374302F, -0.76374459F, -0.96291065F, 0.098433927F, 0.049011F, 0.56542879F, 0.50024462F, 0.33647421F, -0.80293375F, 0.59855759F, -0.74431759F, -0.003538545F, -0.73175585F, 0.65632182F});
  test.AddInput<float>("R", R_dims, R_data);
  ...
  std::vector<int64_t> Y_h_dims{num_directions, batch_size, hidden_size};
  std::vector<float> Y_h_data({-0.85353446F, 0.34633741F, -0.93988168F, -0.79737622F, 0.62769204F, 0.30727068F, 0.39222997F, -0.99489242F, 0.86467457F, 0.0274523F, -0.9431532F, -0.60166585F});
  test.AddOutput<float>("Y_h", Y_h_dims, Y_h_data);
...
TEST(RNNTest, RNN_reverse_direction_zigged_batch) {
  ...
  std::vector<int64_t> X_dims = {seq_length, batch_size, input_size};
  std::vector<float> X_data_in_batchs{0.54881352F, 0.71518934F, 0.60276335F, 0.54488319F, 0.42365479F, 0.64589411F, 0.4375872F, 0.89177299F, 0.96366274F, 0.38344151F,
                                      // batch 2
                                      0.417021990F,
                                      0.720324516F,
                                      0.0001143748F,
  ...
  std::vector<int64_t> R_dims = {num_directions, hidden_size, hidden_size};
  std::vector<float> R_data({0.50877059F, 0.78382635F, 0.665046F, 0.89860243F, -0.71745688F, 0.80142093F, -0.76517141F, -0.88981366F, -0.48568386F});
  test.AddInput<float>("R", R_dims, R_data);
  ...
  std::vector<int64_t> Y_dims = {seq_length, num_directions, batch_size, hidden_size};
  std::vector<float> Y_data_in_batchs({0.87014002F, 0.09402763F, -0.54269236F, 0.37661949F, 0.28492415F, 0.15850827F, 0.8218801F, -0.33996487F, -0.7320742F, 0.90398145F, 0.61396617F, -0.70602065F, 0.68629962F, -0.00125255F, 0.4218055F,
                                       // batch 2
                                       0.64809889F,
                                       -0.19472955F,
                                       -0.24271242F,
...
TEST(RNNTest, RNN_forward_direction_zigged_batch) {
  ...
  std::vector<int64_t> X_dims = {seq_length, batch_size, input_size};
  std::vector<float> X_data_in_batchs{0.061169811F, 0.26296741F, 0.80939841F, 0.080034949F, 0.21000224F, 0.65772671F, 0.20081005F, 0.95461535F, 0.93818879F, 0.76034665F,
                                      // batch 2
                                      0.34715694F,
                                      0.0032335778F,
                                      0.72840774F,
  ...
  std::vector<int64_t> R_dims = {num_directions, hidden_size, hidden_size};
  std::vector<float> R_data({0.16146433F, -0.36291042F, 0.61149812F, -0.018460333F, -0.19345543F, 0.35175204F, 0.84270394F, 0.94917566F, -0.76469761F});
  test.AddInput<float>("R", R_dims, R_data);
  ...
  std::vector<int64_t> Y_dims = {seq_length, num_directions, batch_size, hidden_size};
  std::vector<float> Y_data_in_batchs({-0.0522899628F, -0.0629346371F, -0.211336553F, -0.482055902F, 0.238964200F, -0.313421130F, -0.474286675F, -0.274602413F, -0.461531579F, -0.412429035F, -0.325635254F, -0.792385221F, -0.746264696F, -0.0781838298F, -0.751394153F,
                                       // batch 2
                                       -0.171904743F,
                                       0.140247226F,
                                       -0.140494764F,
...
TEST(RNNTest, RNN_bidirectional_0) {
  ...
  std::vector<int64_t> X_dims = {seq_length, batch_size, input_size};
  std::vector<float> X_data({0.54881352F, 0.71518934F, 0.60276335F, 0.54488319F, 0.42365479F, 0.64589411F, 0.4375872F, 0.891773F, 0.96366274F, 0.38344151F});

  test.AddInput<float>("X", X_dims, X_data);

  std::vector<int64_t> W_dims = {num_directions, hidden_size, input_size};
  std::vector<float> W_data({-0.74535543F, 0.21360011F, 1.0782362F, 0.092641734F, -1.0087538F, -0.97021431F, 0.88425213F, 0.93182313F, 0.767329F, -0.541361F, 0.6218195F, -0.7977342F});
...
TEST(RNNTest, RNN_bidirectional_1) {
  ...
  std::vector<int64_t> B_dims = {num_directions, 2 * hidden_size};
  std::vector<float> B_data({0.0F, 0.0F, 0.0F, 0.0F, 0.0F, 0.0F, 0.0F, 0.0F});
  test.AddInput<float>("B", B_dims, B_data);
...
TEST(RNNTest, DISABLED_RNN_default_attributes_and_forward_direction) {
  ...
  auto run_test = [&](OpTester& test, bool useDefault, RNNOutputOption outputOption) {
    std::vector<int64_t> X_dims = {seq_length, batch_size, input_size};
    std::vector<float> X_data({0.061169811F, 0.26296741F, 0.80939841F, 0.080034949F, 0.21000224F, 0.65772671F, 0.20081005F, 0.95461535F, 0.93818879F, 0.76034665F});
    ...
    if (outputOption == RNNOutputY || outputOption == RNNOutputBoth) {
      std::vector<int64_t> Y_dims = {seq_length, num_directions, batch_size, hidden_size};
      std::vector<float> Y_data({-0.052289959F, -0.062934637F, -0.21133657F, -0.48205593F, 0.23896417F, -0.31342113F, -0.47428668F, -0.27460238F, -0.46153161F, -0.41242906F, -0.32563525F, -0.79238516F, -0.74626476F, -0.07818383F, -0.75139415F});
      test.AddOutput<float>("Y", Y_dims, Y_data);
    } else {
...
TEST(RNNTest, DISABLED_RNN_reverse_direction) {
  ...
  auto runTest = [&](OpTester& test, bool useDefault, RNNOutputOption outputOption) {
    ...
    std::vector<int64_t> R_dims = {num_directions, hidden_size, hidden_size};
    std::vector<float> R_data({-0.7322467F, -0.95795155F, -0.058495734F, -0.7271859F, -0.29820377F, -0.85114992F, -0.097570196F, 0.82271612F, 0.1396943F});
    test.AddInput<float>("R", R_dims, R_data);
    ...
    std::vector<int64_t> Y_dims = {seq_length, num_directions, batch_size, hidden_size};
    std::vector<float> Y_data({-0.55397642F, 0.83026606F, -0.51471221F, -0.55358219F, 0.8341592F, -0.44313878F, -0.60828412F, 0.78948581F, -0.34582433F, -0.40591392F, 0.89962566F, -0.61860478F, -0.56242156F, 0.79118007F, -0.872658F});
    if (outputOption == RNNOutputY || outputOption == RNNOutputBoth) {
      test.AddOutput<float>("Y", Y_dims, Y_data);
    } else {
      test.AddOptionalOutputEdge<float>();
...
TEST(RNNTest, RNN_invalid_sequence_lens) {
  ...
  auto run_test = [&](const std::vector<int>& sequence_lens,
    ...
    std::vector<int64_t> X_dims = {seq_length, batch_size, input_size};
    std::vector<float> X_data({0.1f, 0.2f, 0.3f, 0.4f, 0.5f, 0.6f, 0.7f, 0.8f, 0.9f, 0.11f, 0.12f, 0.13f});
    ...
    std::vector<int64_t> R_dims = {num_directions, hidden_size, hidden_size};
    std::vector<float> R_data({0.f, 0.f, 0.f, 0.f, 0.f, 0.f, 0.f, 0.f, 0.f});
    test.AddInput<float>("R", R_dims, R_data);
...
TEST(RNNTest, RNN_bidirectional_with_sequence_lens) {
  ...
  std::vector<int64_t> W_dims = {num_directions, hidden_size, input_size};
  std::vector<float> W_data{0.1f, 0.2f, 0.3f, 0.4f, 0.5f, 0.6f, 0.5f, 0.6f, 0.7f, 0.3f, 0.8f, 0.4f, 0.9f, 0.5f, 0.1f, 0.6f};

  test.AddInput<float>("W", W_dims, W_data);

  std::vector<int64_t> R_dims = {num_directions, hidden_size, hidden_size};
  std::vector<float> R_data{0.1f, 0.1f, 0.1f, 0.1f, 0.1f, 0.1f, 0.1f, 0.1f, 0.1f, 0.1f, 0.1f, 0.1f, 0.1f, 0.1f, 0.1f, 0.1f, 0.2f, 0.2f, 0.2f, 0.2f, 0.2f, 0.2f, 0.2f, 0.2f, 0.2f, 0.2f, 0.2f, 0.2f, 0.2f, 0.2f, 0.2f, 0.2f};
  test.AddInput<float>("R", R_dims, R_data);

  std::vector<int64_t> B_dims = {num_directions, 2 * hidden_size};
  std::vector<float> B_data{0.1f, 0.1f, 0.1f, 0.1f, 0.1f, 0.1f, 0.1f, 0.1f, 0.1f, 0.1f, 0.1f, 0.1f, 0.1f, 0.1f, 0.1f, 0.1f};
  test.AddInput<float>("B", B_dims, B_data);
  ...
  std::vector<float> Y_data{
      0.69999999f, 1.3000001f, 1.9000001f, 1.9000001f, 1.3000001f, 2.7f, 4.0999999f, 4.0999999f, 1.9000001f, 4.0999999f, 6.3000002f, 6.3000002f,
  ...
  std::vector<int64_t> Y_h_dims{num_directions, batch_size, hidden_size};
  std::vector<float> Y_h_data{0.69999999f, 1.3000001f, 1.9000001f, 1.9000001f, 2.6200001f, 4.2200003f, 5.8199997f, 5.8199997f, 3.2600002f, 5.0600004f, 6.8600001f, 6.8600001f,
