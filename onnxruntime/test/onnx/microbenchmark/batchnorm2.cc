...
static void BM_BatchNormOldEigen(benchmark::State& state) {
  ...
  for (auto _ : state) {
    ...
    EigenArrayMap<T> Y_arr(Y->MutableData<T>(), is_spatial_ ? sample_size : sample_size_incl_all_channels, is_spatial_ ? N * C : N);
    ConstEigenArrayMap<T> X_arr(X->Data<T>(), is_spatial_ ? sample_size : sample_size_incl_all_channels, is_spatial_ ? N * C : N);
