...
void usage() {
  ...
  printf(
      ...
      "\t    [QNN only] [use_cached_context]: true to enable load from cached QNN context. Default to false.\n"
    ...
    if (enable_qnn) {
      ...
      while (ss >> token) {
        ...
        if (key == "backend_path") {
          ...
          qnn_options[key] = value;
        }  else if (key == "use_cached_context") {
          qnn_options[key] = value;
...
['backend_path', 'dump_context', 'use_cached_context', 'profiling_level', 'rpc_control_latency'])");
        }
      }
