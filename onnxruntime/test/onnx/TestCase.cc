...
OrtValue* PbMapToOrtValue(const google::protobuf::Map<key_type, value_type>& map) {
  ...
  ORT_THROW_ON_ERROR(g_ort->CreateCpuAllocatorInfo(OrtDeviceAllocator, OrtMemTypeDefault, &info));
  std::unique_ptr<OrtAllocatorInfo, decltype(g_ort->ReleaseAllocatorInfo)> rel_info(info, g_ort->ReleaseAllocatorInfo);
