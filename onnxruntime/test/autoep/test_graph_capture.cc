...
TEST(PluginEpGraphCapture, WebGpuGraphCaptureAndReplay) {
  // Register plugin EP library for webgpu
  const char* ep_lib_registration_name = "webgpu_ep_library";
  ort_env->RegisterExecutionProviderLibrary(ep_lib_registration_name, ORT_TSTR("onnxruntime_providers_webgpu.dll"));
  auto unregister_ep_lib = gsl::finally([&ep_lib_registration_name] {
    ort_env->UnregisterExecutionProviderLibrary(ep_lib_registration_name);
  });

  // Get the OrtEpDevice for webgpu ep
...

  {
    ...
    // Input X = [2, 3, 4, 5, 6, 7]. Expected output: [1, 6, 12, 20, 30, 42].
    {
