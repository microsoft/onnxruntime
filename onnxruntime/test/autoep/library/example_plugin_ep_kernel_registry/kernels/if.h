...
class If : public OrtKernelImpl {
 private:
  struct PrivateTag {};

 public:
  static OrtStatus* Create(const OrtKernelInfo* info, void* state, /*out*/ std::unique_ptr<If>& kernel) noexcept;
  If(const OrtKernelInfo* info, void* state, OrtKernelImpl* control_flow_kernel, PrivateTag);
  ~If();
