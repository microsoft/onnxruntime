...
void RunKernelPluginEpProfilingTest(ProfilingMode mode) {
  ...
  const ORTCHAR_T* profile_prefix = ORT_TSTR("plugin_ep_profiling_test");
  ...
  if (mode == ProfilingMode::Session) {
    ...
    for (const auto& entry : std::filesystem::directory_iterator(ORT_TSTR("."))) {
      ...
      if (filename.find(profile_prefix) == 0 &&
  ...
  // Ensure profile file is cleaned up even if test assertions fail.
  auto cleanup_profile = gsl::finally([&profile_file_path] { std::filesystem::remove(profile_file_path); });
