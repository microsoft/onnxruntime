...
TEST(OrtEpLibrary, KernelPluginEp_Profiling) {
  ...
  EXPECT_NE(content.find("ExampleKernelEp_profiling_event"), std::string::npos)
      << "Expected to find EP summary profiling event in profile output. Profile content:\n"
      << content;

  // Verify per-op timing events generated by Start/Stop are present.
  EXPECT_NE(content.find("ep_op_"), std::string::npos)
      << "Expected to find per-op timing events (ep_op_*) in profile output. Profile content:\n"
      << content;
