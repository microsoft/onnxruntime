...
// Runs a model with session profiling enabled and verifies the example kernel EP's profiling events
...
TEST(OrtEpLibrary, KernelPluginEp_SessionProfiling) {
  ...
  // Run session and get outputs
  std::array output_names{"C"};
  session.Run(Ort::RunOptions{nullptr}, ort_input_names.data(), ort_inputs.data(),
              ort_inputs.size(), output_names.data(), output_names.size());
  ...
  // Verify Mul kernel timing events generated by EP are present.
  EXPECT_TRUE(content.find("Kernel") != std::string::npos &&
              content.find("ExampleKernelEp_Mul") != std::string::npos)
      << "Expected to find Mul kernel op timing events in profile output. Profile content:\n"
      << content;
...
// Runs a model with run profiling enabled and verifies the example kernel EP's profiling events
// appear in the output.
TEST(OrtEpLibrary, KernelPluginEp_RunProfiling) {
  ...
  // Create session and setup inputs
  std::unordered_map<std::string, std::string> ep_options;
  Ort::SessionOptions session_options;
  session_options.AppendExecutionProvider_V2(*ort_env, {plugin_ep_device}, ep_options);
  Ort::Session session(*ort_env, ORT_TSTR("testdata/if_mul.onnx"), session_options);
  ...
  // Run the model with run profiling enabled.
  const ORTCHAR_T* run_profile_prefix = ORT_TSTR("plugin_ep_run_profiling_test");

  Ort::RunOptions run_options;
  run_options.EnableProfiling(run_profile_prefix);
  session.Run(run_options, ort_input_names.data(), ort_inputs.data(),
              ort_inputs.size(), output_names.data(), output_names.size());

  // End profiling and read the profile file.

  // Find the profile file with the specified prefix. The APIs for run-profiling do not support
  // retrieving the name of the actual profile file generated by ORT.
  ...
  for (const auto& entry : std::filesystem::directory_iterator(ORT_TSTR("."))) {
    ...
    if (filename.find(run_profile_prefix) == 0 &&
        filename.find(ORT_TSTR(".json")) != std::basic_string<ORTCHAR_T>::npos) {
  ...
  // Clean up profile file.
  std::filesystem::remove(profile_file_path);
}
