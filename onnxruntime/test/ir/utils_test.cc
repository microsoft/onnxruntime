...
static void UpdateSubgraphWhenRemovingNode(bool include_nested = false) {
  ...
  bool removed = graph_utils::RemoveNodeAndMergeEdges(graph, node_to_remove);
  ASSERT_TRUE(removed);
...
static void DontRemoveNodeIfItWillBreakSubgraph(bool test_nested = false) {
  ...
  ASSERT_FALSE(graph_utils::CanRemoveNodeAndMergeEdges(graph, node_to_remove));
}
...
TEST(GraphUtils, TestMultiEdgeRemovalNodes) {
  ...
  ASSERT_TRUE(graph_utils::RemoveNodeAndMergeEdges(graph, *nodes[2]));
  ...
  // Remove id_0
  ASSERT_TRUE(graph_utils::RemoveNodeAndMergeEdges(graph, *nodes[0]));
...
TEST(GraphUtils, TestMultiOutputRemoveNode) {
  ...
  ASSERT_FALSE(graph_utils::CanRemoveNodeAndMergeEdges(graph, *nodes[0]));
  ...
  ASSERT_TRUE(graph_utils::CanRemoveNodeAndMergeEdges(graph, *nodes[1]));
  ASSERT_TRUE(graph_utils::RemoveNodeAndMergeEdges(graph, *nodes[1]));
  ASSERT_FALSE(graph_utils::IsOutputUsed(*nodes[0], 0));
  ASSERT_TRUE(graph_utils::CanRemoveNodeAndMergeEdges(graph, *nodes[0]));
  ASSERT_TRUE(graph_utils::RemoveNodeAndMergeEdges(graph, *nodes[0]));
}
