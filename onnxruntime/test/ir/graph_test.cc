...
TEST(ResolvingGraphTest, GraphConstruction_CheckIsAcyclic) {
  ...
  EXPECT_TRUE(Model::Load(ORT_TSTR("graph_1.onnx"), model2, nullptr, ::onnxruntime::test::DefaultLoggingManager().DefaultLogger()).IsOK());
  ...
  model2.reset(new Model(model_proto2, nullptr, ::onnxruntime::test::DefaultLoggingManager().DefaultLogger()));
...
TEST(ResolvingGraphTest, GraphConstruction_CheckGraphInputOutputOrderMaintained) {
  ...
  auto x = onnxruntime::Model::Load(model_proto, p_tmp_model, nullptr, ::onnxruntime::test::DefaultLoggingManager().DefaultLogger());
...
TEST(ResolvingGraphTest, UnusedInitializerIsIgnored) {
  ...
  Model model("UnusedInitializerIsIgnored", false, ::onnxruntime::test::DefaultLoggingManager().DefaultLogger());
  auto& graph = model.MainGraph();
...
TEST(ResolvingGraphTest, GraphConstruction_OnlyInitializer) {
  onnxruntime::Model model("graph", false, ::onnxruntime::test::DefaultLoggingManager().DefaultLogger());
  auto& graph = model.MainGraph();
...
TEST(ResolvingGraphTest, GraphConstruction_TypeInference) {
  ...
  */
  ...
  EXPECT_TRUE(Model::Load(ORT_TSTR("model_x.onnx"), loaded_model, nullptr, ::onnxruntime::test::DefaultLoggingManager().DefaultLogger()).IsOK());
...
TEST(ResolvingGraphTest, ShapeInferenceErrorHandling) {
  ...
  Model model("graph", false, ::onnxruntime::test::DefaultLoggingManager().DefaultLogger());
  auto& graph = model.MainGraph();
...
TEST(TypeInferenceTest, NonConstInitializer) {
  ...
  auto status = onnxruntime::Model::Load(model_proto, p_model, nullptr, ::onnxruntime::test::DefaultLoggingManager().DefaultLogger());
...
TEST(GraphUpdateTest, ReplaceInitializedTensor) {
  Model model{"GraphUpdateTest", false, ::onnxruntime::test::DefaultLoggingManager().DefaultLogger()};
  auto& graph = model.MainGraph();
...
TEST(GraphUpdateTest, AddRemoveInitializerHandling) {
  Model m{"test_model", false, ::onnxruntime::test::DefaultLoggingManager().DefaultLogger()};
