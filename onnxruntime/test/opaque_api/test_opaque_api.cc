...
static void RegisterCustomKernel() {
  ...
  ONNX_TEST_OPERATOR_SCHEMA(OpaqueCApiTestKernel)
      ...
      .SetDomain(onnxruntime::kMsTestDomain)
      .SinceVersion(1)
      .Input(
          0,
  ...
  BuildKernelCreateInfoFn fn = BuildKernelCreateInfo<ONNX_OPERATOR_KERNEL_CLASS_NAME(kCpuExecutionProvider, kMsTestDomain, 1, OpaqueCApiTestKernel)>;
...
std::string CreateModel() {
...

  {
    ...
    auto& node = graph.AddNode("OpaqueCApiTestKernel", "OpaqueCApiTestKernel", "Replace all h to underscore",
                               inputs, outputs, nullptr, onnxruntime::kMsTestDomain);
