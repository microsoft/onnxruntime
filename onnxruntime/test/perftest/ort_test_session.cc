...
OnnxRuntimeTestSession::OnnxRuntimeTestSession(Ort::Env& env, std::random_device& rd,
  ...
  if (provider_name == onnxruntime::kDnnlExecutionProvider) {
    ...
    bool enable_gpu_throttling = false;    // [enable_gpu_throttling]: Enables GPU Throttling (Reduces CPU Utilization when using GPU)
#ifdef _MSC_VER
    std::string ov_string = ToUTF8String(performance_test_config.run_config.ep_runtime_config_string);
#else
    std::string ov_string = performance_test_config.run_config.ep_runtime_config_string;
    ...
    while (ss >> token) {
      ...
      } else if (key == "enable_gpu_throttling") {
        if (value == "true" || value == "True") {
          enable_gpu_throttling = true;
        } else if (value == "false" || value == "False") {
          enable_gpu_throttling = false;
        } else {
          ORT_THROW("[ERROR] [OpenVINO] The value for the key 'enable_gpu_throttling' should be a boolean i.e. true or false. Default value is false.\n");
        }
    ...
    options.enable_gpu_throttling = enable_gpu_throttling;      // Enables GPU Throttling (Reduces CPU Utilization)
