...
inline void ThrowOnFailed(HRESULT hr) {
  if (FAILED(hr)) {
    ORT_THROW("Catastrophic failure!");
  }
}
...
static D3D12_COMMAND_LIST_TYPE CalculateCommandListType(ID3D12Device* d3d12_device) {
  ...
  ThrowOnFailed(d3d12_device->CheckFeatureSupport(
...
static void InitializeDmlValueFromCpuValue(
  ...
  auto winml_api = OrtGetWinMLAdapter(ORT_API_VERSION);
  ...
  ThrowOnFailed(upload_resource->Map(0, nullptr, &mapped_dml_data));
  ...
  ThrowOnFailed(device->CreateCommandAllocator(command_list_type, IID_PPV_ARGS(&command_allocator)));
  ...
  command_list->ResourceBarrier(1, &barrier);
  ThrowOnFailed(command_list->Close());

  ID3D12CommandList* ppCommandLists[] = {command_list.Get()};
  queue->ExecuteCommandLists(_countof(ppCommandLists), ppCommandLists);
  ThrowOnFailed(queue->Signal(fence.Get(), 1));

  // Wait until the fence is completed.
  auto fence_event = CreateEventEx(NULL, false, false, EVENT_ALL_ACCESS);
  ThrowOnFailed(fence->SetEventOnCompletion(1, fence_event));
  WaitForSingleObject(fence_event, INFINITE);
  CloseHandle(fence_event);
#else
  ORT_THROW("onnxruntime_perf_test.exe needs to be build with the --use_winml option in order to perform directx interop binding.");
#endif
}
...
std::pair<Ort::Value, UniqueNativePtr> CreateDmlValueFromCpuValue(
  ...
  InitializeDmlValueFromCpuValue(session, std::move(cpu_value), input_name, dml_value_pair.first);
