...
ABSL_FLAG(std::string, data_shape, "",
          ...
          "Requires -I (auto-generated input). All inputs must have the same number of shape groups.\n"
...
bool CommandLineParser::ParseArguments(PerformanceTestConfig& test_config, int argc, ORTCHAR_T* argv[]) {
  ...
  // --data_shape
  {
    ...
    if (!data_shape_str.empty()) {
      if (!test_config.run_config.generate_model_input_binding) {
        fprintf(stderr, "--data_shape requires -I (auto-generated input).\n");
        return false;
      }
