...
/*static*/ void CommandLineParser::ShowUsage() {
  printf(
      ...
      "\t    [offload_graph_input_quantization]: Offload graph input quantization to another EP (typically CPU EP). \n"
      "\t    Defaults to '0' (QNN EP does the graph input quantization). \n"
      "\t    [offload_graph_output_dequantization]: Offload graph output dequantization to another EP (typically CPU EP). \n"
          ...
          if (key == "backend_path" || key == "vtcm_mb" || key == "soc_model" || key == "htp_arch") {
            ...
            if (supported_htp_graph_final_opt_modes.find(value) == supported_htp_graph_final_opt_modes.end()) {
              ...
              ORT_THROW("Wrong value for htp_graph_finalization_optimization_mode. select from: " + str);
                     ...
                     key == "offload_graph_input_quantization" || key == "offload_graph_output_dequantization") {
