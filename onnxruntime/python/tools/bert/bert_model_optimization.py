...
class OnnxModel:
    ...
    def get_constant_value(self, output_name):
        ...
        tensor = self.get_initializer(output_name)
        if tensor:
...
class BertOnnxModel(OnnxModel):
    ...
    def fuse_gelu(self, gelu_op_name):
        ...
        self.fuse_gelu_with_tanh_pattern2(gelu_op_name)
    ...
    """
     ...
     Note that constant input for Add and Mul could be first or second input: like either A=0.5 or B=0.5 is fine.
    """
    def fuse_gelu_with_tanh_pattern2(self, gelu_op_name):
        input_name_to_nodes = self.input_name_to_nodes()
        output_name_to_node = self.output_name_to_node()
...
def main():
    ...
    bert_model.remove_identity()
    bert_model.fuse_word_embedding()
    bert_model.fuse_segment_embedding()
    bert_model.fuse_mask()

    bert_model.fuse_layer_norm()
    bert_model.fuse_layer_norm_tf()
