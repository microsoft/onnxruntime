...
class BertOnnxModelShapeOptimizer(OnnxModel):
    ...
    def shape_optimization(self, temp_model_path,
        ...
        session = onnxruntime.InferenceSession(temp_model_path, sess_options, providers=['CUDAExecutionProvider', 'CPUExecutionProvider'])
    ...
    def optimize(self, output_path:str,
        ...
        if enable_shape_opt or enable_reshape_opt:
            ...
            logger.warning(f'Shape optimization is done. The optimized model might only work for input with batch_size={batch_size} sequence_length={sequence_length}')
