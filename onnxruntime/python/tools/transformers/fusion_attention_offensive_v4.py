...
class FusionAttentionOffensiveV4(Fusion):
    """
    Fuse Attention subgraph into one Attention node.
    """
    ...
    def create_attention_node(self, mask_index: str, matmul: NodeProto, add: NodeProto, num_heads: int, hidden_size: int,
    ...
    def fuse(self, normalize_node, input_name_to_nodes, output_name_to_node):
        ...
        qkv_nodes = self.model.match_parent_path(start_node, ['Where', 'Add', 'MatMul', 'Reshape', 'Transpose', 'MatMul'],
                                                 [1, 1, 1, 0, 0, 0])
        ...
        v_nodes = self.model.match_parent_path(matmul_qkv, ['Transpose', 'Reshape', 'Slice', 'Add', 'MatMul'], [1, 0, 0, 0, 1])
        if v_nodes is None:
            return
        ...
        q_nodes = self.model.match_parent_path(matmul_qk, ['Mul', 'Transpose', 'Reshape', 'Slice', 'Add', 'MatMul'], [0, 0, 0, 0, 0, 1])
        if q_nodes is None:
            return
        add = q_nodes[-2]
        matmul = q_nodes[-1]

        k_nodes = self.model.match_parent_path(matmul_qk, ['Transpose', 'Reshape', 'Slice', 'Add', 'MatMul'], [1, 0, 0, 0, 1])
        if k_nodes is None:
            return
        add = k_nodes[-2]
        matmul = k_nodes[-1]
        ...
        if matmul.input[0] == root_input:
            ...
            q_num_heads, q_hidden_size = 16, 1024
            # number of heads are same for all the paths, hence to create attention node, we pass the q_num_heads
            # the input_hidden_size represents the input hidden size, this is used as needed but hidden sizes for Q, K are extracted appropriately
            new_node = self.create_attention_node(mask_index, matmul, add, q_num_heads, q_hidden_size, root_input, attention_last_node.output[0], extra_add_qk_nodes[0].input[0])
            if new_node is None:
                return
