...
def convert_tensor_float_to_bfloat16(tensor, min_positive_val=9.2e-41, max_finite_val=3.38953139e38):
    """Convert tensor float to bfloat16.

    Args:
        tensor (TensorProto): the tensor to convert.
        min_positive_val (float, optional): minimal positive value. Defaults to 9.2e-41.
        max_finite_val (float, optional): maximal finite value. Defaults to 3.38953139e38.
    ...
    if tensor.data_type == TensorProto.FLOAT:
        ...
        values[(values > max_finite_val) & (values < float("inf"))] = max_finite_val
        values[(values < min_positive_val) & (values > 0)] = min_positive_val
        values[(values > -min_positive_val) & (values < 0)] = -min_positive_val
        values[(values < -max_finite_val) & (values > float("-inf"))] = -max_finite_val

        tensor.CopyFrom(helper.make_tensor(
                name=tensor.name + "_casted_to_bf16",
                data_type=TensorProto.BFLOAT16,
                dims=values.shape if len(values.shape) != 0 else [],
                vals=values, #values.tostring(),
                raw=True,
        ))
...
DEFAULT_BF16_OP_BLACK_LIST = ["SkipSimplifiedLayerNormalization", "Attention", "MultiHeadAttention"] + list(ALWAYS_FLOAT_INPUTS.keys())
...
def convert_float_to_float16(
    ...
    *,
    use_bf16_as_blocked_dtype=False,
    bf16_op_black_list=None,
):
    """Convert tensor float type in the input ONNX model to tensor float16.

    The operators or nodes that are in block lists will not be converted to float16. They will keep the original data
    type or be converted to bfloat16 if use_bf16_as_blocked_dtype is True.
    ...
    Args:
        ...
        keep_io_types (Union[bool, List[str]], optional): It could be boolean or a list of float32 input/output names.
                                                          If True, model inputs/outputs keep the original data types.
                                                          Defaults to False.
        ...
        op_block_list (List[str], optional): List of op types to run in float32 or bfloat16.
                                             Defaults to None, which will use `float16.DEFAULT_OP_BLOCK_LIST`.
        node_block_list (List[str], optional): List of node names to to run in float32 or bfloat16. Defaults to None.
        ...
        use_bf16_as_blocked_dtype(bool): use bfloat16 as the data type for blocked nodes or operators.
                                               Default to False.
        bf16_op_black_list(List[str], optional): List of operators that do not support bfloat16.
                                                 The list is enabled only when use_bf16_as_blocked_dtype is True.
                                                 Defaults to None, which will use `float16.DEFAULT_BF16_OP_BLACK_LIST`.
    ...
    bf16_black_list: list[str] = (bf16_op_black_list or DEFAULT_BF16_OP_BLACK_LIST) if use_bf16_as_blocked_dtype else []
    def use_bf16_for_blocked_node(n: NodeProto) -> bool:
        return use_bf16_as_blocked_dtype and n.op_type not in bf16_black_list
        ...
        for q in queue:
            ...
            if isinstance(q, GraphProto):
                ...
                for n in q.node:
                    ...
                    # Update tracker for fp32 initializers used by this node.
                    ...
                    for i, input_name in enumerate(n.input):
                        if input_name in fp32_initializers:
                            ...
                            if i in ALWAYS_FLOAT_INPUTS.get(n.op_type, []) and i not in force_fp16_inputs_dict.get(
                                ...
                                dtype = TensorProto.BFLOAT16 if use_bf16_for_blocked_node(n) else TensorProto.FLOAT
                            else:
                    ...
                    if is_node_blocked:
                        ...
                        if n.op_type in ALWAYS_FLOAT_INPUTS and n.op_type not in force_fp16_inputs_dict:
                            mixed_float_type_node_list.append(n)

        queue = next_level

    bf16_initializers: Dict[str, TensorProto] = {} # map from float intiailizer name to bfloat16 initializer
    for value in fp32_initializers.values():
        ...
        if force_fp16_initializers or value.fp16_nodes:
            ...
            if value.fp32_nodes:
                logger.info(
                    f"initializer is used by both fp32 and bf16 nodes. Consider add these nodes to block list:{value.bf16_nodes}"
                )
            # If a float initializer is used by bfloat16 nodes, we can convert it to bfloat16.
            bf16_initializers[value.initializer.name] = convert_tensor_float_to_bfloat16(value.initializer)
            continue
    ...
    for node in node_list:
        ...
        use_bf16 = use_bf16_for_blocked_node(node)
        accuracy_type = TensorProto.BFLOAT16 if use_bf16 else TensorProto.FLOAT
        for i in range(len(node.input)):
            input_name = node.input[i]

            # It is an intializer that has been converted to bfloat16.
            if input_name in bf16_initializers:
                if use_bf16:
                    node.input[i] = bf16_initializers[input_name].name
                else: # Add a cast node to convert it from bfloat16 to float.
                    output_name = node.name + "_input_cast_" + str(i)
                    value_info = helper.make_tensor_value_info(
                        name=output_name, elem_type=accuracy_type, shape=bf16_initializers[input_name].dims
                    )
                    new_value_info = model.graph.value_info.add()
                    new_value_info.CopyFrom(value_info)
                    node_name = node.name + "_input_cast" + str(i)
                    new_node = [helper.make_node("Cast", [bf16_initializers[input_name].name], [output_name], to=accuracy_type, name=node_name)]
                    model.graph.node.extend(new_node)
                    node.input[i] = output_name
                continue

            # When the name is in value_info_list, its data type has been converted to float16.
            # We need to Add a cast node to convert it to float or bfloat16.
            for value_info in value_info_list:
                if input_name == value_info.name:
                    # create new value_info for current node's new input name
                    new_value_info = model.graph.value_info.add()
