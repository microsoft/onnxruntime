...
def convert_to_float16(args: argparse.Namespace, config: LlamaConfig, old_paths: List[str], rank: int=0):
...
def setup_torch_model(args, location, use_auth_token, use_cuda=True):
    world_size = get_size()
    logger.info(f'world_size: {world_size}')
    rank = get_rank()
    barrier()
    _prepare_dir(args.cache_dir)
    for i in range(world_size):
        if i == rank:
            l_config = LlamaConfig.from_pretrained(location, use_auth_token=use_auth_token)
            l_config.use_cache = True
            #llama = LlamaForCausalLM.from_pretrained(location, use_auth_token=use_auth_token, torch_dtype=l_config.torch_dtype, config=l_config, cache_dir=args.cache_dir)
            llama = LlamaForCausalLM.from_pretrained(location, use_auth_token=use_auth_token, config=l_config, cache_dir=args.cache_dir)
            if world_size > 1:
    ...
    for i in range(world_size):
        if i == rank:
            ...
            if args.precision == Precision.FLOAT16:
                new_paths = convert_to_float16(args, l_config, old_paths, rank)
                ...
                if args.execution_provider != "cpu":
                    old_paths = convert_to_float16(args, l_config, old_paths, rank)
            ...
            del llama  # Delete LLaMA model from memory since it will be loaded again during parity 
        barrier()
