...
class PastKeyValuesHelper:
    ...
    def get_past_names(num_layers, present: bool = False):
        ...
        for i in range(num_layers):
            past_self_names.extend([f'present_key_self_{i}', f'present_value_self_{i}']
                                   if present else [f'past_key_self_{i}', f'past_value_self_{i}'])
            past_cross_names.extend([f'present_key_cross_{i}', f'present_value_cross_{i}']
                                    if present else [f'past_key_cross_{i}', f'past_value_cross_{i}'])
    ...
    def group_by_layer(past, num_layers):
        ...
        return tuple([past[2 * i], past[2 * i + 1], past[2 * num_layers + 2 * i], past[2 * num_layers + 2 * i + 1]]
                     for i in range(num_layers))
