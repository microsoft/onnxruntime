...
class T5DecoderInit(torch.nn.Module):
    ...
    def __init__(self,
        ...
        self.config = config
        self.decoder_start_token_id = decoder_start_token_id if decoder_start_token_id is not None else self.config.decoder_start_token_id

    def forward(self, decoder_input_ids: torch.Tensor, encoder_attention_mask: torch.Tensor,
                encoder_hidden_states: torch.FloatTensor):
        if decoder_input_ids is None:
            batch_size = encoder_attention_mask.shape[0]
            decoder_input_ids = torch.ones(
                (batch_size, 1), dtype=torch.long, device=encoder_attention_mask.device) * self.decoder_start_token_id
...
class T5DecoderInputs:
    ...
    def create_dummy(config: T5Config, batch_size: int, encode_sequence_length: int, past_decode_sequence_length: int,
        ...
        decoder_input_ids = torch.randint(low=0,
                                          high=vocab_size - 1,
        ...
        if past_decode_sequence_length > 0:
            self_attention_past_shape = [
                batch_size, num_attention_heads, past_decode_sequence_length,
                int(hidden_size / num_attention_heads)
            ]
            cross_attention_past_shape = [
                batch_size, num_attention_heads, encode_sequence_length,
                int(hidden_size / num_attention_heads)
            ]
    ...
    def to_list(self) -> List:
        input_list = [self.decoder_input_ids, self.encoder_attention_mask, self.encoder_hidden_states]
...
class T5DecoderHelper:
    ...
    def export_onnx(decoder: Union[T5Decoder, T5DecoderInit],
        ...
        inputs = T5DecoderInputs.create_dummy(decoder.config,
                                              batch_size=2,
        ...
        for name in output_present_names:
            if "cross" in name:
                ...
                if isinstance(decoder, T5Decoder):
                    dynamic_axes[name] = {0: 'batch_size', 2: 'past_decode_sequence_length + 1'}
                else:
                    dynamic_axes[name] = {
                        0: 'batch_size',
                        #2: 'sequence_length'
    ...
    @staticmethod
    def verify_onnx(model: Union[T5Decoder, T5DecoderInit],
        ...
        for (batch_size, encode_sequence_length, past_decode_sequence_length) in test_cases[:max_cases]:
            ...
            inputs = T5DecoderInputs.create_dummy(model.config,
                                                  batch_size,
                                                  encode_sequence_length,
