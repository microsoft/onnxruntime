...
def initialize_config(args):
    ...
    #if args.spm_path:
    #    tokenizer = WhisperTokenizer(args.spm_path, args.input_text, args.vocab_path, config=config)
...
def initialize_model(config: WhisperConfig, tokenizer: WhisperTokenizer, args):
    ...
    model = WhisperForConditionalGeneration.from_pretrained(model_dir, config=config).eval()
    ...
    #input_features.append(tokenizer.eos_token_id)
    #input_data = torch.LongTensor(features).unsqueeze(0).to(device)
    input_data = torch.randn(1, 80, 3000).to(device)
