...
# derived copy from onnx_model_bart.FusionBartEncoderAttention
class FusionWav2vec2EncoderAttention(FusionAttention):
    ...
    def check_runtime_shape_path(
        self, reshape_qkv_2, reshape_qkv_1, reshape_q_2, reshape_k_2, reshape_v_2, root_input
    ):
    ...
    def fuse(self, normalize_node, input_name_to_nodes, output_name_to_node):
        ...
        qkv_nodes = self.model.match_parent_path(
            normalize_node, ["Add", "MatMul", "Reshape", "Transpose", "Reshape", "MatMul"], [None, 1, 0, 0, 0, 0]
        )
        ...
        q_nodes = self.model.match_parent_path(
            matmul_qk, ["Reshape", "Transpose", "Reshape", "Mul", "Add", "MatMul"], [0, 0, 0, 0, 0, 1]
        )
        ...
        k_nodes = self.model.match_parent_path(
            matmul_qk, ["Transpose", "Reshape", "Transpose", "Reshape", "Add", "MatMul"], [1, 0, 0, 0, 0, 1]
        )
        ...
        if not self.check_runtime_shape_path(
            reshape_qkv_2, reshape_qkv_1, reshape_q_2, reshape_k_2, reshape_v_2, root_input
        ):
            return
...
class FusionWav2vec2Reshape(FusionReshape):
    ...
    def fuse(self, reshape_node, input_name_to_nodes, output_name_to_node):
        ...
        path0 = self.model.match_parent_path(
            concat_node, ["Unsqueeze", "Gather", "Shape"], [0, 0, 0], output_name_to_node
        )
        if path0 is None:
            return
        ...
        path1 = self.model.match_parent_path(
            concat_node, ["Unsqueeze", "Gather", "Shape"], [1, 0, 0], output_name_to_node
        )
        if path1 is None:
