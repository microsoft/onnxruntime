...
class Wav2vec2OnnxModel(BertOnnxModel):
    ...
    def __init__(self, model, num_heads, hidden_size):
        super().__init__(model, num_heads, hidden_size)
        self.attention_mask = AttentionMask(self)
        self.attention_fusion = FusionWav2vec2EncoderAttention(self, self.hidden_size, self.num_heads,
                                                               self.attention_mask)
        self.wav2vec2_reshape_fusion_preprocess = FusionBartReshape(self)
