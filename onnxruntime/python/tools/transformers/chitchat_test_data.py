...
def fake_test_data(
    ...
    min_length,
    repetition_penalty,
    vocab_mask: TensorProto,
    presence_mask: TensorProto,
):
    ...
    for test_case in range(test_cases):
        ...
        inputs['max_length'] = np.array([50], dtype=np.int32)
        inputs['min_length'] = np.array([1], dtype=np.int32)
        inputs['repetition_penalty'] = np.array([1.], dtype=np.float32)
        inputs['vocab_mask'] = np.array(np.ones([50304], dtype=np.int32))
        inputs['presence_mask'] = np.array(np.ones([batch_size, 50304], dtype=np.int32))
...
def find_chitchat_inputs(
    onnx_model: OnnxModel,
    input_ids_name: Optional[str] = None,
    max_length_name: Optional[str] = None,
    min_length_name: Optional[str] = None,
    repetition_penalty_name: Optional[str] = None,
    vocab_mask_name: Optional[str] = None,
    presence_mask_name: Optional[str] = None,
    ...
    if input_ids_name is not None:
        ...
        max_length = onnx_model.find_graph_input(max_length_name)
        min_length = onnx_model.find_graph_input(min_length_name)
        repetition_penalty = onnx_model.find_graph_input(repetition_penalty_name)
        vocab_mask = onnx_model.find_graph_input(vocab_mask_name)
        presence_mask = onnx_model.find_graph_input(presence_mask_name)

        return input_ids, max_length, min_length, repetition_penalty, vocab_mask, presence_mask

    if len(graph_inputs) != 6:
        raise ValueError("Expect the graph to have 6 inputs. Got {}".format(len(graph_inputs)))
...
def get_chitchat_inputs(
    ...
    return find_chitchat_inputs(onnx_model, input_ids_name, max_length_name, min_length_name, repetition_penalty_name, 
        vocab_mask_name, presence_mask_name)
...
def create_and_save_test_data(
    ...
    max_length_name: Optional[str],
    min_length_name: Optional[str],
    repetition_penalty_name: Optional[str],
    vocab_mask_name: Optional[str],
    presence_mask_name: Optional[str],
    ...
    """
    input_ids, max_length, min_length, repetition_penalty, vocab_mask, presence_mask = get_chitchat_inputs(model, input_ids_name, max_length_name, min_length_name, repetition_penalty_name, vocab_mask_name, presence_mask_name)
