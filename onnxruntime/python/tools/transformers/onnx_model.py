...
class OnnxModel:
    ...
    def change_input_output_float32_to_float16(self):
        ...
        self.model = helper.make_model(graph_def, producer_name='onnxruntime.transformers')
    ...
    def convert_model_float32_to_float16(self, cast_input_output=True, **kwargs):
        logger.warn('The function convert_model_float32_to_float16 is deprecated. Use convert_float_to_float16 instead!')
        return self.convert_float_to_float16(keep_io_types=cast_input_output, **kwargs)
    ...
    def convert_float_to_float16(self, use_symbolic_shape_infer=True, **kwargs):
        ...
        try:
            ...
            logger.warn('Please install onnxconverter_common latest version to use advanced model conversion')
            self._naive_float_to_float16(kwargs["keep_io_types"])

        if Version(oc.__version__) < Version("1.8.1"):
            logger.warn('Please install onnxconverter_common latest version to use advanced model conversion.')
            self._naive_float_to_float16(kwargs["keep_io_types"])
            return
        ...
        if Version(oc.__version__) >= Version("1.9.0"):
            parameters.update({key: kwargs[key] for key in ['keep_io_types', 'min_positive_val', 'max_finite_val', 'op_block_list', 'node_block_list'] if key in kwargs})

        fp16_model = oc.float16.convert_float_to_float16(model, **parameters)
