...
def parse_arguments(argv=None):
    ...
    beam_search_group.add_argument(
        "--early_stopping", required=False, action="store_true"
    )
...
def gpt2_to_onnx(args):
    ...
    print(
        f"use convert_to_onnx.py to convert model {model_name} to onnx {args.decoder_onnx} ..."
    )
    arguments = [
        "--model_name_or_path",
        model_name,
...
def shape_inference(decoder_onnx_path):
    if version.parse(onnx.__version__) >= version.parse("1.11.0"):
        logger.warn(
            "SymbolicShapeInference might fail using onnx version 1.11. Please install 1.10.0 for now."
        )
    ...
    out = SymbolicShapeInference.infer_shapes(
        onnx.load(decoder_onnx_path), auto_merge=True, guess_output_rank=False
    )
    if out:
...
def convert_model(args):
    if os.path.exists(args.decoder_onnx):
        ...
        ), "please have onnx model ready for model type that is not gpt2"
        gpt2_to_onnx(args)
    ...
    if args.model_type == "t5":
        if enable_shape_inference:
            print(
                f"Run symbolic shape inference on {args.encoder_decoder_init_onnx}. The file will be overwritten."
            )
    ...
    length_penalty = helper.make_tensor_value_info(
        "length_penalty", TensorProto.FLOAT, [1]
    )
    repetition_penalty = helper.make_tensor_value_info(
...
def test_model(args, use_vocab_mask: bool = False, sentences: List[str] = None):
    ...
    if args.temperature != 1.0:
        ...
        print(
            "Skipping parity test as temperature is not implemented in BeamSearch operator"
        )
        return True
...
def main(argv=None, sentences=None):
    ...
    if args.model_type == "t5":
        ...
        ), "please export t5 to onnx models before using this tool"
