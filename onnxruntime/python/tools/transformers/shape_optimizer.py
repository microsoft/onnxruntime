...
class BertOnnxModelShapeOptimizer(OnnxModel):
    ...
    def add_shape_initializer(self, shape):
        ...
        constant_shape_name = self.create_node_name(
            "Constant", CONSTANT_SHAPE_NAME_PREFIX
        )
    ...
    def add_shape_for_reshape_input(self):
        ...
        for node in self.model.graph.node:
            if node.op_type == "Reshape":
                input = node.input[0]
                output_name = self.create_node_name(
                    "Reshape_Input", RESHAPE_INPUT_SHAPE_PREFIX
                )
                shape_node = onnx.helper.make_node(
                    "Shape", inputs=[input], outputs=[output_name]
                )
        ...
        for input in self.model.graph.input:
            if input.name in inputs:
                ...
                if dim_proto.HasField("dim_param"):
                    ...
                    dim_proto.HasField("dim_value")
                    and dim_proto.dim_value != max_seq_len
                ):
                    raise ValueError(
    ...
    def shape_optimization(
        ...
        inputs = self.create_dummy_inputs(
            input_ids, segment_ids, input_mask, batch_size, sequence_length, elem_type
        )
    ...
    def update_target_shape(self, shapes, shape_input, input_shape, verbose):
        ...
        logger.debug(
            f"source_shape={source_shape}, target_shape={target_shape}, new_target_shape={new_target_shape}"
        )
    ...
    def validate_input(self, input: str):
        if not self.find_graph_input(input):
            ...
            raise Exception(
                "Input {} does not exist in the graph inputs: {}".format(
                    input, valid_names
                )
            )
    ...
    def validate_outputs(self, output_names: List[str]):
        ...
        for name in output_names:
            if name not in valid_names:
                raise Exception(
                    "Output {} does not exist in the graph outputs: {}".format(
                        name, valid_names
                    )
                )
    ...
    def optimize(
        ...
        if enable_shape_opt or enable_reshape_opt:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                temp_file_name = "temp_{}.onnx".format(
