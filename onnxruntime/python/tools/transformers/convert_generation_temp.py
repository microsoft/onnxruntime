...
Example 5: convert gpt2 model with sampling:
    python convert_generation_temp.py -m gpt2 --output gpt2_sampling.onnx --num_beams 1 --num_return_sequences 1
"""
...
def convert_generation_model(args: argparse.Namespace, generation_type: GenerationType = GenerationType.BEAMSEARCH):
    ...
    attr_to_extend = (
        ...
        if not is_greedysearch
            ...
            onnx.helper.make_attribute("model_type", 0 if args.model_type == "gpt2" else 1),
            onnx.helper.make_attribute("no_repeat_ngram_size", args.no_repeat_ngram_size),
            onnx.helper.make_attribute("temperature", 1.1),
            onnx.helper.make_attribute("top_p", 0.6),
            onnx.helper.make_attribute("filter_value", 0.0),
