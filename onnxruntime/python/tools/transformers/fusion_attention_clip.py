...
class FusionAttentionClip(FusionAttention):
    ...
    def fuse(self, normalize_node, input_name_to_nodes, output_name_to_node):
        ...
        if node_before_layernorm is not None:
            ...
            for i in [0, 1]:
                node_before_layernorm = self.model.match_parent(normalize_node, "Add", i)
        ...
        qk_nodes = self.model.match_parent_path(
            ...
            [0, 0, 0, None, 0],
            return_indice=add_mask_indice,
        )
        if qk_nodes is None:
            logger.debug("fuse_attention: failed to match qk path")
            return
        assert len(add_mask_indice) == 1
        causal_mask_input_index = 1 - add_mask_indice[0]
