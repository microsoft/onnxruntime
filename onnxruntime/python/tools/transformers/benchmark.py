...
def run_onnxruntime(use_gpu, model_names, fp16, batch_sizes, sequence_lengths, repeat_times, input_counts,
                    optimize_onnx, validate_onnx, cache_dir, onnx_dir, verbose, overwrite, quantize, single_thread, disable_ort_io_binding):
    import onnxruntime
...
def output_details(results, csv_filename):
    with open(csv_filename, mode="a", newline='') as csv_file:
        column_names = [
            ...
            "sequence_length", "datetime", "test_times", "QPS", "average_latency_ms", "latency_variance", "latency_50_percentile",
            ...
            for input_count in [1, 2, 3]:
                for engine_name in args.engines:
                    for io_binding in [True, False, ""]:
                        row = {}
                        for result in results:
                            if "io_binding" not in result:
                                result["io_binding"] = True
    ...
    if enable_onnxruntime:
        try:
            results += run_onnxruntime(args.use_gpu, args.models, args.fp16, args.batch_sizes, args.sequence_lengths,
                                       ...
                                       args.cache_dir, args.onnx_dir, args.verbose, args.overwrite, args.quantize, args.single_thread, args.disable_ort_io_binding)
        except:
