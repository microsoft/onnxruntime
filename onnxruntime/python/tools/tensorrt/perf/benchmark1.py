...
cuda_execution_provider_fp16 = "CUDAExecutionProvider_fp16"
tensorrt_execution_provider = "TensorrtExecutionProvider"
tensorrt_execution_provider_fp16 = "TensorrtExecutionProvider_fp16"

ep_to_provider_list = {
    cpu_execution_provider: [cpu_execution_provider],
    cuda_execution_provider: [cuda_execution_provider],
    cuda_execution_provider_fp16: [cuda_execution_provider],
    tensorrt_execution_provider: [tensorrt_execution_provider, cuda_execution_provider],
...
def update_metrics_map_ori(model_to_metrics, name, ep_to_operator):
    ...
    for ep, op_map in ep_to_operator.items():
        if ep == cuda_execution_provider:
            cuda_op_map = op_map
        elif ep == cuda_execution_provider_fp16:
            cuda_fp16_op_map = op_map
        elif ep == tensorrt_execution_provider:
            trt_op_map = op_map
        elif ep == tensorrt_execution_provider_fp16:
...
def update_fail_model_map(model_to_fail_ep, model_name, ep, e_type, e):
    ...
    if ep == tensorrt_execution_provider:
        ep_ = tensorrt_execution_provider_fp16
...
def run_onnxruntime(args, models):
    ...
    for name, model_info in models.items():
        ...
        for ep in ep_list:
            ...
            if ep == cuda_execution_provider_fp16:
