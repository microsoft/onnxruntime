...
class HistogramCollector(CalibrationDataCollector):
    ...
    def get_optimal_threshold(self, histogram, num_quantized_bins):
        ...
        for i in range(num_half_quantized_bin, zero_bin_index + 1, 1):
            ...
            # print("zero_bin_index %s, start_index %s, end_index %s" % (zero_bin_index, start_index, end_index))
            # print(hist)
            # print(hist_edges)
            thresholds[i - num_half_quantized_bin] = (float(hist_edges[start_index]), float(hist_edges[end_index]))
            # print(thresholds[i-num_half_quantized_bin])
            ...
            # print(p)
            # print(len(p))
            # print("sliced_distribution")
            # print(sliced_distribution)
            # print(len(sliced_distribution))
            ...
            # print("num_merged_bins* num_merged_bins %s, sum %s" % (num_merged_bins * num_merged_bins, sum(sliced_distribution[num_quantized_bins*num_merged_bins:])))
...
class ONNXCalibrater:
    ...
    def augment_graph(self, calib_mode='naive'):
        ...
        for tensor in tensors_to_calibrate:
            ...
            if calib_mode == 'naive':
                ...
                # Adding Flatten nodes
                flatten_name = tensor + '_Flatten'
                flatten_node = onnx.helper.make_node('Flatten', [tensor], [tensor + '_Flatten'],
                                                      flatten_name) 

                added_nodes.append(flatten_node)
                added_outputs.append(helper.make_tensor_value_info(flatten_node.output[0], TensorProto.FLOAT, ()))
    ...
    def get_intermediate_outputs(self, calib_mode='naive', providers=None, ort_graph_optimization_enable=True):
        ...
        if calib_mode == 'naive':
            ...
            node_names = [added_node_output_names[i].rpartition('_')[0]
                          for i in range(len(added_node_output_names))]  #output names
            final_merged_dict = {}
            for i in range(len(added_node_output_names)):
                final_merged_dict[node_names[i]] = clean_merged_dict[added_node_output_names[i]]

            if not self.collector:
                self.collector = HistogramCollector()
            self.collector.collect(final_merged_dict)
...
def calculate_calibration_data(model,
    ...
    if not os.path.exists(augmented_model_path):
        augmented_model = calibrator.augment_graph(calib_mode='entropy')
        onnx.save(augmented_model, augmented_model_path)

    calibrator.get_intermediate_outputs(calib_mode='entropy', providers=["CUDAExecutionProvider"])
