...
class QDQQuantizer(BaseQuantizer):
    def __init__(
        ...
        if self.opset_version < 21:
            ...
            overrides_new_types = any(t.tensor_type in new_types for t in self.tensor_quant_override_qtypes)
            if not self.qdq_op_domain and (
                self.activation_qType in new_types or self.weight_qType in new_types or overrides_new_types
            ):
                logging.warning(
                    "ONNX QuantizeLinear and DequantizeLinear operators do not support "
                    "16-bit or 4-bit integer quantization types prior to opset 21. "
