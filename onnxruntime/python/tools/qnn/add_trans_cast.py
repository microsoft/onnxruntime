    ...
    with open(qnn_cpp_file) as cpp_file:
        for line in cpp_file:
            ...
            if (".dimensions" in line) and (qnn_tensor_item_count < 4):
                ...
                qnn_tensor_item_count += 1
    assert (
        len(qnn_tensor_names) == len(qnn_tensor_types)
        and len(qnn_tensor_types) == len(qnn_tensor_data_types)
        and len(qnn_tensor_data_types) == len(qnn_tensor_dim_alias)
...
def main():
    ...
    parser.add_argument(
        "-q", "--qnn_cpp", help="Required. Path to Qnn converted model.cpp file.", required=True, type=str
    )
