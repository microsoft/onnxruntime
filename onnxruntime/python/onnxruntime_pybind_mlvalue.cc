...
AllocatorPtr GetDmlAllocator(OrtDevice::DeviceId id) {
  ...
  if (hit == id_to_allocator_map->end()) {
    ...
    ComPtr<Dml::DmlReferenceWrapper<std::shared_ptr<Dml::BucketizedBufferAllocator>>> allocator_ref;
    uint32_t allocator_ref_size = gsl::narrow_cast<uint32_t>(sizeof(allocator_ref.GetAddressOf()));
    HRESULT hr = d3d12_device->GetPrivateData(Dml::AllocatorGUID, &allocator_ref_size, allocator_ref.GetAddressOf());

    // If the GUIDs were not found, it means that we have a brand new device that hasn't been used to allocate memory yet
    if (hr == DXGI_ERROR_NOT_FOUND) {
      ...
      auto upload_heap = std::make_unique<Dml::PooledUploadHeap>(d3d12_device.Get(), context);
      auto readback_heap = std::make_unique<Dml::ReadbackHeap>(d3d12_device.Get(), context);
      ...
      allocator_ref = wil::MakeOrThrow<Dml::DmlReferenceWrapper<std::shared_ptr<Dml::BucketizedBufferAllocator>>>(std::move(dml_allocator));
      auto context_ref = wil::MakeOrThrow<Dml::DmlReferenceWrapper<std::shared_ptr<Dml::ExecutionContext>>>(std::move(context));
      auto upload_heap_ref = wil::MakeOrThrow<Dml::DmlReferenceWrapper<std::shared_ptr<Dml::PooledUploadHeap>>>(std::move(upload_heap));
      auto readback_heap_ref = wil::MakeOrThrow<Dml::DmlReferenceWrapper<std::shared_ptr<Dml::ReadbackHeap>>>(std::move(readback_heap));
      ORT_THROW_IF_FAILED(d3d12_device->SetPrivateDataInterface(Dml::AllocatorGUID, allocator_ref.Get()));
      ORT_THROW_IF_FAILED(d3d12_device->SetPrivateDataInterface(Dml::ExecutionContextGUID, context_ref.Get()));
      ORT_THROW_IF_FAILED(d3d12_device->SetPrivateDataInterface(Dml::UploadHeapGUID, upload_heap_ref.Get()));
      ORT_THROW_IF_FAILED(d3d12_device->SetPrivateDataInterface(Dml::ReadbackHeapGUID, readback_heap_ref.Get()));
    } else {
      ORT_THROW_IF_FAILED(hr);
    }

    hit = id_to_allocator_map->emplace(id, allocator_ref->GetInner()).first;
  }
...
void CpuToDmlMemCpy(void* dst, const void* src, size_t num_bytes) {
  ...
  ComPtr<Dml::DmlReferenceWrapper<std::shared_ptr<Dml::ExecutionContext>>> context_ref;
  uint32_t context_ref_size = gsl::narrow_cast<uint32_t>(sizeof(context_ref.GetAddressOf()));
  ORT_THROW_IF_FAILED(d3d12_device->GetPrivateData(Dml::ExecutionContextGUID, &context_ref_size, context_ref.GetAddressOf()));

  ComPtr<Dml::DmlReferenceWrapper<std::shared_ptr<Dml::PooledUploadHeap>>> upload_heap_ref;
  uint32_t upload_heap_ref_size = gsl::narrow_cast<uint32_t>(sizeof(upload_heap_ref.GetAddressOf()));
  ORT_THROW_IF_FAILED(d3d12_device->GetPrivateData(Dml::UploadHeapGUID, &upload_heap_ref_size, upload_heap_ref.GetAddressOf()));

  upload_heap_ref->GetInner()->BeginUploadToGpu(
      dst_data, 0, D3D12_RESOURCE_STATE_UNORDERED_ACCESS, gsl::make_span(static_cast<const std::byte*>(src), num_bytes));
  context_ref->GetInner()->Flush();
  ...
  context_ref->GetInner()->GetCurrentCompletionEvent().WaitForSignal();
}
...
void DmlToCpuMemCpy(void* dst, const void* src, size_t num_bytes) {
  ...
  ComPtr<Dml::DmlReferenceWrapper<std::shared_ptr<Dml::ReadbackHeap>>> readback_heap_ref;
  uint32_t readback_heap_ref_size = gsl::narrow_cast<uint32_t>(sizeof(readback_heap_ref.GetAddressOf()));
  ORT_THROW_IF_FAILED(d3d12_device->GetPrivateData(Dml::ReadbackHeapGUID, &readback_heap_ref_size, readback_heap_ref.GetAddressOf()));

  // ReadbackFromGpu already syncs with the CPU and waits for the copy to be completed, so we don't need to sync after
  // this call
  readback_heap_ref->GetInner()->ReadbackFromGpu(
