 ...
 public:
  ...
  operator Arg2() {
    ...
    static LoggingManager default_logging_manager{std::unique_ptr<ISink>{new CErrSink{}},
                                                  ...
                                                  // default_severity_, false, LoggingManager::InstanceType::Default,
                                                  &default_logger_id};
  ...
  // static void SetLoggingSeverity(int severity) {
  //   if (severity <= static_cast<int>(Severity::kFATAL))
  //     default_severity_ = static_cast<Severity>(severity);
  // }

  //private:
  // static Severity default_severity_;
};

//Severity SessionObjectInitializer::default_severity_{Severity::kWARNING};
...
void addGlobalMethods(py::module& m) {
  ...
  //m.def(
  //    "set_logging_severity", [](int severity) { SessionObjectInitializer::SetLoggingSeverity(severity); },
  //    "Set the log severity level for the default logger. "
  //    "This must be called prior to get_session_initializer() (used by InferenceSession.__init__) to have any effect.");
