...
// memories involved.
static_assert(USE_MIGRAPHX && (USE_CUDA || USE_DML), "Too many GPU EPs included in the build");
...
void RegisterExecutionProviders(InferenceSession* sess, const std::vector<std::string>& provider_types,
  ...
  for (const std::string& type : provider_types) {
    if (type == kCpuExecutionProvider) {
      ...
      // If both CUDA and DML are requested, just register DML EP
      // as ORT doesn't have semantics of dealing with nodes assigned to multiple
      // device memory based GPU EPs. See comment above.
      // TODO: Register DML EP
