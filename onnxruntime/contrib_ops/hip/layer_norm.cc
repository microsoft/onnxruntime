...
Status LayerNorm<T, U>::ComputeInternal(OpKernelContext* ctx) const {
  ...
  HipU* mean_data = nullptr;
  if (mean != nullptr) {
    mean_data = reinterpret_cast<HipU*>(mean->template MutableData<U>());
  }

  HipU* inv_var_data = nullptr;
  if (var != nullptr) {
    inv_var_data = reinterpret_cast<HipU*>(var->template MutableData<U>());
  }
