...
#include "contrib_ops/hip/math/bias_softmax.h"

#include "core/providers/common.h"

using namespace onnxruntime;
using namespace onnxruntime::hip;
using namespace onnxruntime::contrib::hip;
...
void DispatchBiasSoftMaxForwardViaDnnLibraryImpl(
    const onnxruntime::hip::HipKernel* kernel,
    int element_count,
    int batch_count,
    int broadcast_axis,
    int softmax_axis,
...
Status BiasSoftmax::ComputeInternal(OpKernelContext* ctx) const {
  ...
  if (D <= 1024 && D * elem_size <= 4096) {
    ...
    // need to fallback to add kernel + HIP DNN library softmax call :/
    utils::MLTypeCallDispatcher<DispatchBiasSoftMaxForwardViaDnnLibrary, double, float, MLFloat16>
        t_disp(X->GetElementType());
    t_disp.Invoke(this, D, N, broadcast_axis, softmax_axis, X_shape, X, B_shape, B, Y);
  }
