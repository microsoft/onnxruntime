...
Status CheckInputs(const Tensor* query,
  ...
  int max_present_sequence_length = std::max(total_sequence_length, past_sequence_length);
  ...
  if (cos_cache != nullptr && sin_cache != nullptr) {
    ...
    if (cos_dims[0] < max_present_sequence_length) {
      return ORT_MAKE_STATUS(ONNXRUNTIME, INVALID_ARGUMENT,
                             "cos_cache dimension 0 should be of max_sequence_length.");
    }
    if (sin_dims[0] < max_present_sequence_length) {
      return ORT_MAKE_STATUS(ONNXRUNTIME, INVALID_ARGUMENT,
  ...
  if (parameters != nullptr) {
    RocmAttentionParameters* output_parameters = reinterpret_cast<RocmAttentionParameters*>(parameters);
    output_parameters->batch_size = batch_size;
    output_parameters->sequence_length = sequence_length;                  // sequence length of Q
    output_parameters->past_sequence_length = past_sequence_length;        // sequence length of past kv tensors
    output_parameters->total_sequence_length = sequence_length + past_sequence_length;  // past + new
    output_parameters->max_sequence_length = max_present_sequence_length;    // max sequence length of past kv tensors
    output_parameters->hidden_size = q_hidden_size;
    output_parameters->num_heads = num_heads;
    output_parameters->head_size = head_size;
    output_parameters->v_hidden_size = kv_hidden_size;
    output_parameters->v_head_size = head_size;
    ...
    output_parameters->pass_past_in_kv = past_key != nullptr && past_value != nullptr;
