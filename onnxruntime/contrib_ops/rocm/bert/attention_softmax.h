...
// Compatibility overload matching the 1.28.0 CUDA API:
//   const int* attention_mask (not int3 strides),
//   broadcast_attn_bias_dim_0/1, mask_dimension, max_sequence_length.
// Maps to the existing ROCm implementation by constructing int3 strides and
// ignoring params that don't affect correctness on ROCm.
template <typename T>
Status ComputeSoftmax(hipStream_t stream, const int all_sequence_length, const int sequence_length,
  ...
  return ComputeSoftmax<T>(stream, all_sequence_length, sequence_length,
                           batch_size, num_heads, attn_bias, input, output, causal);
}
...
Status ComputeSoftmaxWithRawMask(Stream* ort_stream,
                                 ...
                                 T* persistent_softmax_workspace,
                                 const float mask_filter_value) {
  // Delegate to the original ROCm implementation with zero strides (mask unused)
  const int3 attention_mask_strides = {0, 0, 0};
  return ComputeSoftmaxWithRawMask<T>(ort_stream, all_sequence_length, sequence_length,
                                      batch_size, num_heads, attention_mask_strides,
                                      nullptr, key_padding_mask, attn_bias, input, output,
                                      causal, rsqrt_head_size, use_persistent_softmax,
...
// Namespace alias for hipified code that uses attention_softmax_rocm
// (hipify converts attention_softmax_cuda → attention_softmax_rocm)
namespace onnxruntime { namespace contrib { namespace attention_softmax_rocm = rocm; } }
