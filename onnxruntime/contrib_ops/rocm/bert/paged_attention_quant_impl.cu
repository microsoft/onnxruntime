...
__device__ __half2 __hmax2(__half2 a, __half2 b) {
    __half a0 = a.x;
    __half a1 = a.y;
    __half b0 = b.x;
    __half b1 = b.y;
    __half max0 = __hmax(a0, b0);
    __half max1 = __hmax(a1, b1);
    return __half2(max0, max1);
}

// __device__ __half2 __hmax2(__half2 a, __half2 b) {
//     __half tmp1 = __hgt(__low2half(a), __low2half(b)) ? __low2half(a) : __low2half(b);
//     __half tmp2 = __hgt(__high2half(a), __high2half(b)) ? __high2half(a) : __high2half(b);
...
__global__ void quantize_reshape_and_cache_kernel(
  ...
  for (int iter = 0; iter < num_iters; iter++) {
    ...
    if (quant_chunk_idx < num_chunks_per_head && h_in_chunk < kv_quant_chunk_size) {
      ...
      k_vecs[iter] = Vec{{HIPRT_ZERO_FP16, HIPRT_ZERO_FP16}, {HIPRT_ZERO_FP16, HIPRT_ZERO_FP16}};
    ...
    for (int mask = num_threads_per_chunk / 2; mask >= 1; mask /= 2) {
      max_kv_fp16 = __hmax2(max_kv_fp16, __shfl_xor(max_kv_fp16, mask));
    }
  }
