...
LayerNorm<T, U, simplified>::LayerNorm(const OpKernelInfo& op_kernel_info) : RocmKernel(op_kernel_info) {
...
Status LayerNorm<T, U, simplified>::ComputeInternal(OpKernelContext* ctx) const {
  ...
  auto bias_data = simplified ? nullptr: reinterpret_cast<const HipT*>(bias->template Data<T>());
  ...
  HostApplyLayerNorm<HipT, CudaU, simplified>(GetDeviceProp(), Y_data, mean_data, inv_var_data, X_data, n1, n2, epsilon_, scale_data, bias_data);
  return Status::OK();
}
