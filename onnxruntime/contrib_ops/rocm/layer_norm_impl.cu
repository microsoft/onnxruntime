...
void HostApplyLayerNorm(
  ...
  hipLaunchKernelGGL(HIP_KERNEL_NAME(cuApplyLayerNorm<T, U, simplified>), dim3(blocks), dim3(threads), nshared, 0, 
      output,
      mean,
      invvar,
      input,
...
#define LAYERNORM_LINEAR_IMPL(T, U, simplified)                                                                       \
  template void HostApplyLayerNorm<T, U, simplified>(const hipDeviceProp_t& prop, T* output, U* mean, U* invvar, const T* input, int64_t n1, int64_t n2, \
