...
static void CalculateSqeuclidean(const Tensor& a, const Tensor& b, Tensor& c, concurrency::ThreadPool* threadpool) {
  ...
  std::vector<T> a_ss;
  a_ss.resize(static_cast<size_t>(m));
  const auto* cur_a = a_data;
  for (int64_t i = 0; i < m; ++i) {
    a_ss[static_cast<size_t>(i)] = ConstEigenVectorMap<T>(cur_a, static_cast<size_t>(k)).squaredNorm();
    cur_a += k;
  }

  // ReduceSumSquare for B
  std::vector<T> b_ss;
  b_ss.resize(static_cast<size_t>(n));
  const auto* cur_b = b_data;
  for (int64_t i = 0; i < n; ++i) {
    b_ss[static_cast<size_t>(i)] = ConstEigenVectorMap<T>(cur_b, static_cast<size_t>(k)).squaredNorm();
    cur_b += k;
  }
...
// use MLAS on 64-bit (no 32-bit dgemm)
  ...
  auto out_map = EigenMatrixMapRowMajor<T>(c_data, static_cast<size_t>(m), static_cast<size_t>(n));
  out_map.noalias() = static_cast<T>(-2.) *
                      (ConstEigenMatrixMapRowMajor<T>(a_data, static_cast<size_t>(m), static_cast<size_t>(k)) *
                       ConstEigenMatrixMapRowMajor<T>(b_data, static_cast<size_t>(n), static_cast<size_t>(k)).transpose());
#endif
  ...
  auto* cur_out = c_data;
  for (int64_t i = 0; i < m; ++i) {
    T a_val = a_ss[static_cast<size_t>(i)];
    for (int64_t j = 0; j < n; ++j) {
      *cur_out = (*cur_out + a_val) + b_ss[static_cast<size_t>(j)];
      ++cur_out;
    }
  }
}
...
common::Status CDist<T>::Compute(OpKernelContext* context) const {
  ...
  auto map_out = EigenVectorArrayMap<T>(output, static_cast<size_t>(output_shape.Size()));
