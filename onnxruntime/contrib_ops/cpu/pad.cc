...
template <>
Status DynamicPad<float>::Compute(OpKernelContext* ctx) const {
  ...
  std::vector<int64_t> pads_(2 * dimension_count, 0);
  ...
  for (size_t i = 0; i < pads_size; ++i) {
    pads_[i] = pads_tensor_raw_data[i];
  }
  ...
  float value_ = 0.0f;
  const auto* value_tensor = ctx->Input<Tensor>(2);
  if (nullptr != value_tensor) {
    ...
    value_ = value_tensor->template Data<float>()[0];
  }
