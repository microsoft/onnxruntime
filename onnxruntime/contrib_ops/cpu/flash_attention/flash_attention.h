...
void cpu_flash_attention(
  ...
  const size_t probs_matrix_bytes = SafeInt<size_t>(qSize) * kvSize * sizeof(accum_t);
  unit_cost.compute_cycles = static_cast<double>(2 * qSize * headSize * kvSize / qSlice);
  unit_cost.bytes_loaded = static_cast<double>((qSize + kvSize) * headSize * sizeof(scalar_t) / qSlice);
  unit_cost.bytes_stored = static_cast<double>(probs_matrix_bytes / qSlice);

  int64_t v_head_size = headSize;
  unit_cost.compute_cycles += static_cast<double>(2 * qSize * v_head_size * kvSize / qSlice);
  unit_cost.bytes_loaded += static_cast<double>((qSize + v_head_size) * kvSize * sizeof(scalar_t) / qSlice);
  unit_cost.bytes_stored += static_cast<double>(qSize * v_head_size * sizeof(scalar_t) / qSlice);
