...
Status MatMulNBits<T1>::PrePack(const Tensor& tensor, int input_idx, /*out*/ AllocatorPtr alloc,
  ...
  if (input_idx == InputIndex::B) {
    ...
    if (prefer_lut_gemm_) {
      ...
      // every prepack at load. The pack routines need not write every byte (alignment padding between
      // the CompInt8 sub-regions; any layout could gain padding) and the reserve allocation is not
      // zero-filled, so the hash would otherwise depend on uninitialized bytes. Zeroing the whole
      // buffer is a one-time O(packed_b_size_) load cost (the pack overwrites the data regions,
      // leaving only padding zeroed); inference is unaffected.
...
Status MatMulNBits<MLFloat16>::PrePack(const Tensor& tensor, int input_idx, /*out*/ AllocatorPtr alloc,
  ...
  if (input_idx == InputIndex::B) {
    ...
    // zero-fill runs on essentially every prepack at load. It keeps the dedup content hash
    // reproducible regardless of bytes the pack leaves uninitialized (alignment padding), for any
    // compute type. One-time O(packed_b_size_) load cost; inference is unaffected.
    if (prepacked_weights != nullptr) {
      std::memset(packed_b_.get(), 0, packed_b_size_);
    }
