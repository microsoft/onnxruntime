...
class ConvIntegerToFloat : public OpKernel {
 public:
  explicit ConvIntegerToFloat(const OpKernelInfo& info) : OpKernel(info),
                                                          conv_attrs_(info),
                                                          is_W_signed_(false),
                                                          is_W_packed_(false) {
...
Status ConvIntegerToFloat::PrePack(const Tensor& tensor, int input_idx, bool& is_packed) {
  is_packed = false;
...
Status ConvIntegerToFloat::Compute(OpKernelContext* context) const {
  ...
  if (!channels_last_) {
    ...
    auto* transpose_output = alloc->Alloc(SafeInt<size_t>(sizeof(float)) * Y_offset);
  ...
  for (int64_t image_id = 0; image_id < N; ++image_id) {
    ...
    if (!channels_last_) {
      ...
      output_data = static_cast<float*>(transpose_output_buffer.get());
    }
    ...
    //      reinterpret_cast<uint32_t*>(Ydata),
