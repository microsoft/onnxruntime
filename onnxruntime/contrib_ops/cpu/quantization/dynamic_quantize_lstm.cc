...
Status DynamicQuantizeLSTM::Compute(OpKernelContext* context) const {
  ...
  size_t W_scale_size = W_scale_shape.NumDimensions() == 2 ? gsl::narrow_cast<size_t>(W_scale_shape[1]) : 1;
  size_t R_scale_size = R_scale_shape.NumDimensions() == 2 ? gsl::narrow_cast<size_t>(R_scale_shape[1]) : 1;
  ...
  const size_t W_size_per_direction = gsl::narrow_cast<size_t>(W_shape[1] * W_shape[2]);
  const size_t R_size_per_direction = gsl::narrow_cast<size_t>(R_shape[1] * R_shape[2]);
