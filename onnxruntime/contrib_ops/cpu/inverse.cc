...
struct Inverse::ComputeImpl {
  void operator()(const Tensor* input, Tensor* output,
    ...
    Eigen::Map<const MatrixT<T>> input_matrix(input_data, gsl::narrow_cast<size_t>(rows), gsl::narrow_cast<size_t>(cols));
    Eigen::Map<MatrixT<T>> output_matrix(output_data, gsl::narrow_cast<size_t>(rows), gsl::narrow_cast<size_t>(cols));
...
struct Inverse::ComputeImpl<MLFloat16> {
  void operator()(const Tensor* input, Tensor* output,
    ...
    Eigen::Map<const MatrixT<Eigen::half>> input_matrix(input_data, gsl::narrow_cast<size_t>(rows), gsl::narrow_cast<size_t>(cols));
    Eigen::Map<MatrixT<Eigen::half>> output_matrix(output_data, gsl::narrow_cast<size_t>(rows), gsl::narrow_cast<size_t>(cols));
    output_matrix = input_matrix.inverse();
  }
};
...
Status Inverse::Compute(OpKernelContext* ctx) const {
  ...
  concurrency::ThreadPool::TryBatchParallelFor(ctx->GetOperatorThreadPool(), gsl::narrow_cast<size_t>(num_batches), std::move(fn), 0);
