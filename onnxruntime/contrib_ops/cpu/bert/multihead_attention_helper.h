...
Status CheckInputs(const T* query,
  ...
  if (past_present_share_buffer &&
      kv_sequence_length > max_sequence_length - past_sequence_length) {
    return ORT_MAKE_STATUS(
        ONNXRUNTIME, INVALID_ARGUMENT,
        "past_sequence_length (", past_sequence_length,
        ") plus kv_sequence_length (", kv_sequence_length,
        ") must not exceed max_sequence_length (", max_sequence_length, ").");
  }
