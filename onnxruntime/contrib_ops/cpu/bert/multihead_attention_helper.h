...
Status CheckInputs(const T* query,
  ...
  //   query            (Q)       : (B, S, D)
  //   key              (K)       : (B, L, D) for key or (B, N, L, H) for past key
  //   value            (V)       : (B, L, D_v) for value or (B, N, L, H_v) for past value
  //   bias             (Q/K/V)   : (D + D + D_v)
  ...
  int64_t kv_sequence_length = (key_dims.size() == 3) ? key_dims[1] : key_dims[2];
  ...
  if ((key_dims.size() == 3 && value_dims.size() == 3 && key_dims[1] != value_dims[1]) ||
      (key_dims.size() == 4 && value_dims.size() == 4 && key_dims[2] != value_dims[2])) {
    return ORT_MAKE_STATUS(ONNXRUNTIME, INVALID_ARGUMENT,
                           "Input 'key' and 'value' shall have same same dim 1 (sequence_length)");
  }
  v_hidden_size = (value_dims.size() == 3) ? value_dims[2] : value_dims[1] * value_dims[3];
...
Status CheckInputs(const T* query,
  ...
  return CheckInputs(query, key, value, bias, key_padding_mask, parameters, num_heads, mask_filter_value);
}
