...
Status CheckInputs(const OpKernelContext* context) {
  ...
  const Tensor* word_embedding = context->Input<Tensor>(1);
  const Tensor* position_embedding = context->Input<Tensor>(2);
  const Tensor* gamma = context->Input<Tensor>(3);
  const Tensor* beta = context->Input<Tensor>(4);
  const Tensor* mask = context->Input<Tensor>(5);  // optional. nullptr if not provided
  const Tensor* segment_ids = context->Input<Tensor>(6);  // optional. nullptr if it's distill-bert
  const Tensor* segment_embedding = context->Input<Tensor>(7);  // optional. nullptr if it's distill-bert
