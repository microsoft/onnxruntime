...
Status GroupQueryAttention<T>::Compute(OpKernelContext* context) const {
  ...
  std::vector<int64_t> output_qk_shape{static_cast<int64_t>(batch_size), static_cast<int64_t>(kv_num_heads_), static_cast<int64_t>(parameters.sequence_length), static_cast<int64_t>(present_kv_seqlen)};
  Tensor* output_qk = context->Output(3, output_qk_shape);

  ORT_RETURN_IF_ERROR(group_query_attention_helper::CheckOutputs(output_qk, output_post_softmax_qk_, &parameters));
  ...
  return ApplyAttention(q_rotary, packed_qkv ? nullptr : k_rotary, packed_qkv ? nullptr : V.Get<Tensor>().Data<T>(),
                        attention_bias, past_key, past_value, output, present_k, present_v, output_qk,
