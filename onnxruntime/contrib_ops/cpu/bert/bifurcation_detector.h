  ...
  Status Compute(OpKernelContext* context) const override {
    ...
    for (int64_t i = min_gram; i < max_gram + 1; ++i) {
      ...
      if (it == (src_tokens_data + src_tokens_len)) {
        ...
        idx = it - src_tokens_data;
        if ((idx + i) >= src_tokens_len) {
          break;
        }
        auto it_2 = std::search(src_tokens_data + idx + 1, src_tokens_data + src_tokens_len, tokens_data + tokens_len - i, tokens_data + tokens_len);
