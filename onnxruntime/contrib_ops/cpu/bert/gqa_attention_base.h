...
class GQAAttentionBase {
 protected:
  GQAAttentionBase(const OpKernelInfo& info, bool has_local) {
    ...
    output_post_softmax_qk_ = info.GetAttrOrDefault<int>("output_post_softmax_qk", -1) == 1;
  }
 ...
 private:
  ...
  size_t GetAttentionProbsNumElements(size_t batch_size, size_t sequence_length, size_t seqlen_present_kv_cache) {
    return SafeInt<size_t>(batch_size) * num_heads_ * sequence_length * seqlen_present_kv_cache;
  }

  template<typename T, typename U>
  void WriteOutputQK(T* output_qk, const U* attention_probs, size_t batch_size, size_t sequence_length, size_t seqlen_present_kv_cache)
  {
    ...
    if constexpr (std::is_same_v<U, T>) {
      ...
      static_assert(std::is_same_v<U, float> && std::is_same_v<T, MLFloat16>);
      MlasConvertFloatToHalfBuffer(static_cast<float*>(attention_probs), output_qk, elements_count);
    }
  }
};
