    ...
    ThreadPool::TryParallelFor(tp, loop_len, unit_cost, [&](std::ptrdiff_t begin, std::ptrdiff_t end) {
      for (std::ptrdiff_t i = begin; i != end; ++i) {
        ...
        for (size_t seq = 0; seq < sequence_length; seq++) {
          ...
          // Add attention bias to QxK' if provided and compute softmax
          if (attention_bias_thread != nullptr) {
            if constexpr (std::is_same_v<U, T>) {
              if (use_smooth_softmax_) {
                ComputeSmoothSoftmaxInplace(output_softmax + start_offset, 1, static_cast<int>(window_size), attention_bias_thread + start_offset, nullptr);
              } else {
                ComputeAttentionSoftmaxInplace(output_softmax + start_offset, 1, static_cast<int>(window_size), attention_bias_thread + start_offset, nullptr);
              }
            } else {
              ...
              if (use_smooth_softmax_) {
                ComputeSmoothSoftmaxInplace(output_softmax + start_offset, 1, static_cast<int>(window_size), attention_bias_thread_fp32, nullptr);
              } else {
                ComputeAttentionSoftmaxInplace(output_softmax + start_offset, 1, static_cast<int>(window_size), attention_bias_thread_fp32, nullptr);
              }
            }
          } else {
            if (use_smooth_softmax_) {
