...
class GQAAttentionBase {
 protected:
  GQAAttentionBase(const OpKernelInfo& info, bool has_local) {
    ...
    output_post_softmax_qk_ = static_cast<int>(info.GetAttrOrDefault<int64_t>("output_post_softmax_qk", -1)) == 1;
  }
  ...
  Status ApplyAttention(const T* Q,                                 // Q data with shape BxNxSxH
    ...
    size_t bytes = SafeInt<size_t>(GetAttentionProbsNumElements(batch_size, sequence_length, seqlen_present_kv_cache)) * (gqa_mlas_supported ? sizeof(T) : sizeof(float));
 ...
 private:
  ...
  void ComputeAttentionProbs(U* attention_probs,                                   // output buffer with size BxNxSxT
    ...
    ThreadPool::TryParallelFor(tp, loop_len, unit_cost, [&](std::ptrdiff_t begin, std::ptrdiff_t end) {
      for (std::ptrdiff_t i = begin; i != end; ++i) {
        ...
        if (output_post_softmax_qk_ == 0) {
          WriteOutputQK(output_qk, attention_probs, batch_size, sequence_length, present_buffer_sequence_length);
        }
  ...
  size_t GetAttentionProbsNumElements(size_t batch_size, size_t sequence_length, size_t seqlen_present_kv_cache) const {
    return SafeInt<size_t>(batch_size) * num_heads_ * sequence_length * seqlen_present_kv_cache;
  }

  template<typename T, typename U>
  void WriteOutputQK(T* output_qk, const U* attention_probs, size_t batch_size, size_t sequence_length, size_t seqlen_present_kv_cache) const {
    if (output_qk == nullptr) {
      return;
    }

    const size_t elements_count = GetAttentionProbsNumElements(batch_size, sequence_length, seqlen_present_kv_cache);
    if constexpr (std::is_same_v<U, T>) {
      std::memcpy(output_qk, attention_probs, SafeInt<size_t>(elements_count) * sizeof(T));
    } else {
      static_assert(std::is_same_v<U, float> && std::is_same_v<T, MLFloat16>);
      MlasConvertFloatToHalfBuffer(static_cast<const float*>(attention_probs), output_qk, elements_count);
    }
  }
};
