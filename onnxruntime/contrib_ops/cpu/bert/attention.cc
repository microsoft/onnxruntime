...
Status Attention<T>::Compute(OpKernelContext* context) const {
  ...
  T* Q = reinterpret_cast<T*>(gemm_data);
  T* K = Q + batch_size * sequence_length * hidden_size;
  T* V = K + batch_size * sequence_length * hidden_size;

  GenerateQKV<T>(Q, K, V, input_data, weights_data, bias_data,
