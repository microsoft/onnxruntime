...
bool Attention<T>::IsPackWeightsSuccessful(int qkv_index,
  ...
  size_t loop_len = gsl::narrow<size_t>(num_heads_);
...
Status Attention<T>::PrePack(const Tensor& weights, int input_idx, AllocatorPtr alloc,
  ...
  const size_t input_hidden_size = gsl::narrow<size_t>(weights_dims[0]);
  size_t q_hidden_size, k_hidden_size, v_hidden_size;

  if (qkv_hidden_sizes_.size() != 0) {
    q_hidden_size = gsl::narrow<size_t>(qkv_hidden_sizes_[0]);
    k_hidden_size = gsl::narrow<size_t>(qkv_hidden_sizes_[1]);
    v_hidden_size = gsl::narrow<size_t>(qkv_hidden_sizes_[2]);
    ...
    const size_t hidden_size_x3 = gsl::narrow<size_t>(weights_dims[1]);
...
Status Attention<T>::Compute(OpKernelContext* context) const {
  ...
  auto K = Q + gsl::narrow<size_t>(batch_size) * sequence_length * parameters.hidden_size;
  auto V = K + gsl::narrow<size_t>(batch_size) * sequence_length * parameters.hidden_size;
