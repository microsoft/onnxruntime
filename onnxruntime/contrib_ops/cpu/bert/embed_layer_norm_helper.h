...
inline Status CheckInputs(const TOpKernelContext* context, bool quantizedVersion = false) {
  const Tensor* input_ids = context->template Input<Tensor>(0);
  const Tensor* segment_ids = context->template Input<Tensor>(1);  // optional. nullptr if it's distill-bert
  const Tensor* word_embedding = context->template Input<Tensor>(2);
  const Tensor* position_embedding = context->template Input<Tensor>(3);
  const Tensor* segment_embedding = context->template Input<Tensor>(4);  // optional. nullptr if it's distill-bert
  const Tensor* gamma = context->template Input<Tensor>(5);
...
// dispatched to host implementation through g_host_cpu.embed_layer_norm__CheckInputs.
