...
void MoE::ProcessExpert(const float* input_data,
  ...
  if (is_swiglu) {
    // For SwiGLU, we need to handle the gating mechanism
    // Use interleaved format to match CUDA implementation: [glu, linear, glu, linear, ...]
    ApplySwiGLUActivation(fc1_output.data(), inter_size, true);
  } else {
    ApplyActivationInPlace(fc1_output.data(), fc1_inter_size);
  }

  // FC2: intermediate -> output using MLAS GEMM
  // FC2 input is the activated fc1_output with size inter_size
  float* fc2_input = fc1_output.data();
