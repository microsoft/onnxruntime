...
static inline T8Bits quantize_value(float y, float y_scale, T8Bits y_zero_point){
  constexpr int32_t min_8bits = std::numeric_limits<T8Bits>::min();
...
template<typename T>
Status QLinearAveragePool<T>::Compute(OpKernelContext* context) const {
  ...
  T x_zero_point = (tensor_x_zero_point ? *(tensor_x_zero_point->Data<T>()) : (T)0);
  T y_zero_point = (tensor_y_zero_point ? *(tensor_y_zero_point->Data<T>()) : (T)0);
  ...
  switch (kernel_shape.size()) {
    case 1: {
      if (channels_last_) {
        QLinearPoolNhwc1DTask<T, onnxruntime::AveragePool> avg_pool_task_1d = {
        ...
        QLinearPool1DTask<T, onnxruntime::AveragePool> avg_pool_task_1d = {
    ...
    case 2: {
      if (channels_last_) {
        QLinearPoolNhwc2DTask<T, onnxruntime::AveragePool> avg_pool_task_2d = {
        ...
        QLinearPool2DTask<T, onnxruntime::AveragePool> avg_pool_task_2d = {
            x_data_fp32, Y_data, y_scale, y_zero_point, x_image_size, y_image_size,
            pooled_height, pooled_width, strides[0], strides[1], height, width, kernel_shape, pads, pool_context_, pool_attrs_};
    ...
    case 3: {
      if (channels_last_) {
        QLinearPoolNhwc3DTask<T, onnxruntime::AveragePool> avg_pool_task_3d = {
            x_data_fp32, Y_data, y_scale, y_zero_point, x_image_size, y_image_size, kernel_size, channels,
        ...
        QLinearPool3DTask<T, onnxruntime::AveragePool> avg_pool_task_3d = {
            x_data_fp32, Y_data, y_scale, y_zero_point, x_image_size, y_image_size,
            pooled_height, pooled_width, pooled_depth, strides[0], strides[1], strides[2], height, width, depth,
            kernel_shape, pads, pool_context_, pool_attrs_};
