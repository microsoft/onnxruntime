 ...
 public:
  ...
  void Process(const ISequences* sequences,
    ...
    for (int i = 0; i < batch_beam_size; i++) {
      ...
      beam_token_scores[not_token_id_] = std::numeric_limits<T>::lowest();
      beam_token_scores[solm_token_id_] = std::numeric_limits<T>::lowest();

      // Suppress sot, translate and transcribe tokens
      if (seq_length > sample_begin) {
        beam_token_scores[sot_token_id_] = std::numeric_limits<T>::lowest();
        beam_token_scores[translate_token_id_] = std::numeric_limits<T>::lowest();
        beam_token_scores[transcribe_token_id_] = std::numeric_limits<T>::lowest();
      }
      ...
      if (timestamp_last >= beg_token_id_) {
        int timestamp_next = 0;
        if (last_was_timestamp && !penultimate_was_timestamp) {
          // For single timestamp at the end, next timestamp must not be smaller
          timestamp_next = timestamp_last;
        } else {
          // For paired timestamp at the end, next timestamp must be greater
          timestamp_next = timestamp_last + 1;
        }

        for (int w_id = beg_token_id_; w_id < timestamp_next; w_id++) {
          beam_token_scores[w_id] = std::numeric_limits<T>::lowest();
        }
      }

      if (seq_length == sample_begin) {
        const int last_allowed = beg_token_id_ + max_initial_timestamp_index_;
        for (int w_id = last_allowed + 1; w_id < vocab_size; w_id++) {
          beam_token_scores[w_id] = std::numeric_limits<T>::lowest();
        }
      }
