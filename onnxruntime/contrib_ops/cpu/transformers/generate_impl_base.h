...
class GenerateBase {
 public:
  GenerateBase(OpKernelContextInternal& context,
    ...
    cpu_allocator_ = decoder_session_state.GetAllocator(decoder_session_state.GetExecutionProviders().Get(onnxruntime::kCpuExecutionProvider)->GetMemoryInfo(OrtMemTypeDefault));
  }
