    ...
    v = tl.where(
        ...
        tl.where(
            ...
            tl.where(
                m == 2,
                1.0,
                tl.where(m == 3, 1.5, tl.where(m == 4, 2.0, tl.where(m == 5, 3.0, tl.where(m == 6, 4.0, 6.0)))),
            ),
        ),
    )
...
def mxfp4_grouped_gemm(
    ...
    for k0 in range(0, K, BLOCK_K):
        ...
        byte = tl.load(
            W + e0 * stride_we + offs_n[None, :] * stride_wn + (kk[:, None] // 2) * stride_wk,
            mask=(offs_n[None, :] < N) & (kk[:, None] < K),
            other=0,
        )
        nib = tl.where((kk[:, None] & 1) == 0, byte & 0xF, (byte >> 4) & 0xF)
        s = tl.load(
            S + e0 * stride_se + (kk[:, None] // 32) * stride_sk + offs_n[None, :] * stride_sn,
            mask=(offs_n[None, :] < N) & (kk[:, None] < K),
            other=0.0,
        )
