...
struct ELU {
  __device__ __inline__ T operator()(const T& a) const {
    // alpha = 1.0
    return a > (T)0 ? a : (T)(exp((float)a) - 1);
  }
};
...
void CutlassMoeFCRunner<T, WeightType, Enable>::run_moe_fc_arflow(
    ...
    configure_ws_ptrs(workspace_ptr, static_cast<size_t>(num_rows), static_cast<size_t>(in_features),
                      static_cast<size_t>(interm_features), static_cast<size_t>(num_experts), static_cast<size_t>(k));
    topk_gating_softmax_kernelLauncher<T>(gating_output, nullptr, expert_scales, softmax_out_, expert_for_source_row,
                                          source_rows_, num_rows, num_experts, k, normalize_routing_weights_, stream);
    ...
    initialize_moe_routing_kernelLauncher(input_activations, permuted_data_, permuted_rows_,
                                          expanded_source_row_to_expanded_dest_row, num_rows, active_rows, in_features,
                                          k, stream);
    ...
    ActivationType no_activation_type = ActivationType::Identity;
    const T* scales_placeholder = nullptr;

    // fc1
    moe_gemm_runner_.moe_gemm_bias_act(
        permuted_data_ + total_past_rows_ * in_features, fc1_expert_weights, scales_placeholder, fc1_expert_biases,
    ...
    // fc2
    moe_gemm_runner_.moe_gemm_bias_act(
        fc1_result_ + total_past_rows_ * interm_features, fc2_expert_weights, scales_placeholder, fc2_expert_biases,
    ...
    // fc3
    moe_gemm_runner_.moe_gemm_bias_act(
        fc3_result_ + total_past_rows_ * interm_features, fc3_expert_weights, scales_placeholder, fc3_expert_biases,
        fc1_result_ + total_past_rows_ * interm_features, total_rows_before_expert_ + local_experts_start_index,
        expanded_active_expert_rows, interm_features, interm_features, local_num_experts, no_activation_type, stream);
    ...
    moe_gemm_runner_.moe_gemm(
        fc1_result_ + total_past_rows_ * interm_features, fc4_expert_weights, scales_placeholder, nullptr,
        fc2_result + total_past_rows_ * out_features,
        total_rows_before_expert_ + local_experts_start_index, expanded_active_expert_rows,
        out_features, interm_features, local_num_experts, stream);
}
