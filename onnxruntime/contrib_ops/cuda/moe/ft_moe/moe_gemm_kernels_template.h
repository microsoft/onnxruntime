...
void generic_moe_gemm_kernelLauncher(const T *A, const WeightType *B, const T *weight_scales, const T *biases, T *C,
    ...
    // using ElementType_ =
    //     typename cutlass::platform::conditional<cutlass::platform::is_same<T, half>::value, cutlass::half_t,
    //     T>::type;
    ...
    //     cutlass::ComplexTransform::kNone, MixedGemmArchTraits::ElementsPerAccessB, ElementType,
    ...
    //     reinterpret_cast<const ElementType *>(biases), reinterpret_cast<ElementType *>(C), total_rows_before_expert,
    //     gemm_n, gemm_k);
...
struct dispatch_stages<T, WeightType, cutlass::arch::Sm80, EpilogueTag, ThreadblockShape, WarpShape, Stages,
    ...
    static void dispatch(const T *A, const WeightType *B, const T *weight_scales, const T *biases, T *C,
        ...
        generic_moe_gemm_kernelLauncher<T, WeightType, cutlass::arch::Sm80, EpilogueTag, ThreadblockShape, WarpShape,
                                        Stages>(A, B, weight_scales, biases, C, total_rows_before_expert, gemm_n,
                                                gemm_k, num_experts, gemm_config, multi_processor_count, stream,
                                                occupancy);
    }
};
...
void dispatch_moe_gemm_to_cutlass(const T *A, const WeightType *B, const T *weight_scales, const T *biases, T *C,
    ...
    case CutlassTileConfig::Undefined:
        ...
        break;
    case CutlassTileConfig::ChooseWithHeuristic:
        ORT_THROW("[FT Error][dispatch_moe_gemm_to_cutlass][SIMT] gemm config should have already been set by "
                  "heuristic.");
        break;
    default:
...
template <typename T, typename WeightType> MoeGemmRunner<T, WeightType>::MoeGemmRunner() {}

template <typename T, typename WeightType> void MoeGemmRunner<T, WeightType>::initialize(int sm_version) {
    int device{-1};
...
void MoeGemmRunner<T, WeightType>::dispatch_to_arch<EpilogueTag>(const T *A, const WeightType *B,
    ...
    if (sm_ >= 70 && sm_ < 75) {
        dispatch_moe_gemm_to_cutlass<T, WeightType, cutlass::arch::Sm70, EpilogueTag>(
            ...
            gemm_config, sm_, multi_processor_count_, stream, occupancy);
