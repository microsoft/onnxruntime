...
Status BiasSplitGelu<T>::ComputeInternal(OpKernelContext* context) const {
  ...
  LaunchBiasSplitGeluKernel<CudaT>(Stream(context), grid_size, half_hidden_size, reinterpret_cast<const CudaT*>(input->Data<T>()), reinterpret_cast<const CudaT*>(bias->Data<T>()), reinterpret_cast<CudaT*>(output->MutableData<T>()));
