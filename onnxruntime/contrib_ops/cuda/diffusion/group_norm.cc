...
ONNX_OPERATOR_KERNEL_EX(
    GroupNorm, kMSDomain, 1, kCudaExecutionProvider, (*KernelDefBuilder::Create()).TypeConstraint("T", BuildKernelDefConstraints<GROUP_NORM_TYPES>()), GroupNorm);
...
Status GroupNorm::ComputeInternal(OpKernelContext* context) const {
  ...
  return dispatcher.InvokeRet<Status, DispatchGroupNorm>(Stream(context), output, input, gamma, beta, workspace.get(), epsilon_, batch_size, num_channels, height, width, num_groups_, use_swish_activation_);
}
