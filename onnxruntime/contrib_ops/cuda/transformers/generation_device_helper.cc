...
Status AddToFeeds(const IExecutionProvider* execution_provider,
  ...
  auto pinned_buffer = IAllocator::MakeUniquePtr<void>(pinned_allocator, total_bytes, false, ort_stream);
...
Status ProcessLogits(const OrtValue& logits,                                 // logits output of subgraph
  ...
  if (parameters->repetition_penalty != 1.0f || run_ngram) {
    ...
    sequences_buffer = IAllocator::MakeUniquePtr<void>(allocator, bytes, false, ort_stream);
...
Status UpdateDecoderCrossQK(
  ...
  auto cross_qk_size = cross_qk_shape[0] * block_size_per_beam;
  ...
  for (size_t j = 0; j < beam_indices_gpu.size(); j++) {
    ...
    // TODO: Do pointer arithmatic right here to index old and new data by beam_indexes
