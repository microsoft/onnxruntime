...
template void mmha_launch_kernel<float, 32>(const DecoderMaskedSelfAttentionParams& params, cudaStream_t stream);

template void mmha_launch_kernel<uint16_t, 32>(const DecoderMaskedSelfAttentionParams& params, cudaStream_t stream);
