...
def softmax_kernel(output_ptr, input_ptr, input_row_stride, output_row_stride, n_cols, BLOCK_SIZE: tl.constexpr):
    ...
    block_start = input_row_stride * pid_0
    row_range = tl.arange(0, BLOCK_SIZE)

    row = tl.load(input_ptr + block_start + row_range, row_range < n_cols)
    tl.store(output_ptr + block_start + row_range, -row, row_range < n_cols)
