...
Status PagedAttention<T, TCACHE>::ComputeInternal(OpKernelContext* context) const {
  ...
  // Multi-token paged XQA is not quality-safe for speculative verification. Even though its
  // operator-level output meets the attention tolerance, it changes generated tokens and causes
  // statistically significant MMLU-Pro and GPQA regressions. Keep q_len > 1 on the validated
  // FlashAttention/decoder-attention path; single-token XQA remains enabled below.
  constexpr bool xqa_spec_dec_candidate = false;
