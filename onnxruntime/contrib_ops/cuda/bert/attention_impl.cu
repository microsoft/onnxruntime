...
Status UnfusedAttention(
  ...
  CUBLAS_RETURN_IF_ERROR(cublasGemmStridedBatchedHelper(
      ...
      &zero, positional_embed, total_sequence_length, batch_size * num_heads * total_sequence_length, sequence_length, device_prop));

  pos_emb_transposed = positional_embed;
  ORT_RETURN_IF_ERROR(LaunchTransQkv(stream, 1, sequence_length, batch_size, total_sequence_length, num_heads,
                                    device_prop.maxThreadsPerBlock, true, data.intermediate_embed, transposed_positional_embed));
  ...
  if (use_raw_attention_mask) {  // 2d, 3d or 4d attention mask
    ...
    ORT_RETURN_IF_ERROR(
        ComputeSoftmaxWithRawMask<T>(
            ...
            mask_index, nullptr, data.relative_position_bias, transposed_positional_embed, parameters.broadcast_res_pos_bias,
