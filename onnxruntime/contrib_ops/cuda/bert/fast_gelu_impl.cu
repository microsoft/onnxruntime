...
template <unsigned TPB>
__global__ void FastGeluPrecisionKernel(int input_length, int bias_length, const float* input, const float* bias, float* output) {
  const int idx = blockIdx.x * TPB + threadIdx.x;
  if (idx < input_length) {
    const float in = input[idx];
    const float x = (bias == nullptr) ? in : (in + bias[idx % bias_length]);
    // use same formula of gelu_new in huggingface transformer
    output[idx] = A * x * (1.0f + tanhf(B * (x + K * powf(x, 3.0f))));
  }
}

template <unsigned TPB>
__global__ void FastGeluPrecisionKernel(int input_length, int bias_length, const half* input, const half* bias, half* output) {
  const int idx = blockIdx.x * TPB + threadIdx.x;
  if (idx < input_length) {
    const half in = input[idx];
    const half add_bias = (bias == nullptr) ? in : (in + bias[idx % bias_length]);
    float x = __half2float(add_bias);
    output[idx] = __float2half(A * x * (1.0f + tanhf(B * (x + K * powf(x, 3.0f)))));   
  }
}

template <unsigned TPB>
__global__ void FastGeluExperimentalKernel(int input_length, int bias_length, const float* input, const float* bias, float* output) {
  const int idx = blockIdx.x * TPB + threadIdx.x;
  if (idx < input_length) {
    const float in = input[idx];
    const float x = (bias == nullptr) ? in : (in + bias[idx % bias_length]);
    // use same formula of gelu_fast in huggingface transformer
    output[idx] = A * x * (1.0f + tanhf(x * 0.7978845608f * (1.0f + K * x * x)));
  }
}

template <unsigned TPB>
__global__ void FastGeluExperimentalKernel(int input_length, int bias_length, const half* input, const half* bias, half* output) {
  const int idx = blockIdx.x * TPB + threadIdx.x;
  if (idx < input_length) {
    const half in = input[idx];
    const half add_bias = (bias == nullptr) ? in : (in + bias[idx % bias_length]);
    float x = __half2float(add_bias);
    output[idx] = __float2half(A * x * (1.0f + tanhf(x * 0.7978845608f * (1.0f + K * x * x))));
  }
}

template <unsigned TPB>
__global__ void FastGeluBetaKernel(int input_length, int bias_length, const half2* input, const half2* bias, half2* output) {
  // kernel from faster transformer
#if __CUDA_ARCH__ >= 530
  const int idx = blockIdx.x * TPB + threadIdx.x;
  if (idx < input_length) {
    const half2 x = input[idx];
    const half2 in = (bias == nullptr) ? x : __hadd2(x, bias[idx % bias_length]);
    half2 pow3 = __hmul2(in, __hmul2(in, in));
    float2 x3 = __half22float2(pow3);

    float2 cdf =  __half22float2(in);
    cdf.x = A + A * tanhf(B * cdf.x + C * x3.x);
    cdf.y = A + A * tanhf(B * cdf.y + C * x3.y);

    output[idx] = __hmul2(in, __float22half2_rn(cdf));
  }
#endif
}
...
bool LaunchFastGeluKernel(const cudaDeviceProp& prop, cudaStream_t stream, int input_length, int bias_length, const float* input, const float* bias, float* output) {
  ...
  if (options->IsPrecisionMode()) {
    FastGeluPrecisionKernel<blockSize><<<gridSize, blockSize, 0, stream>>>(input_length, bias_length, input, bias, output);
  } else if (options->IsExperimentalMode()){
    FastGeluExperimentalKernel<blockSize><<<gridSize, blockSize, 0, stream>>>(input_length, bias_length, input, bias, output);
  }
  else {
...
bool LaunchFastGeluKernel(const cudaDeviceProp& prop, cudaStream_t stream, int input_length, int bias_length, const half* input, const half* bias, half* output) {
  ...
  if (options->IsPrecisionMode()) {
  ...
  else if (0 == (bias_length & 1) && prop.major >= 7) {
    ...
    if (options->IsBetaMode()) {
      FastGeluBetaKernel<blockSize><<<gridSize, blockSize, 0, stream>>>(n, bias_length / 2, input2, bias2, output2);
    }
    else{
