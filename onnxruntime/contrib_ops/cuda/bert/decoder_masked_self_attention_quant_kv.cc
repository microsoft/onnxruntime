...
static constexpr int kPastScaleIndex = 9;
static constexpr int kPresentOutputIndex = 1;
static constexpr int kPresentScaleIndex = 2;

#define REGISTER_KERNEL_TYPED(T1, T2)                                         \
  ONNX_OPERATOR_TYPED_KERNEL_EX(                                              \
      DecoderMaskedSelfAttentionQuantKV,                                      \
      kMSDomain,                                                              \
      1,                                                                      \
      T1,                                                                     \
      ...
      (*KernelDefBuilder::Create())                                           \
          ...
          .MayInplace(kPastScaleIndex, kPresentScaleIndex)                    \
      ...
      DecoderMaskedSelfAttentionQuantKV<T1, T2>)

REGISTER_KERNEL_TYPED(MLFloat16, uint16_t);

template <typename T1, typename T2>
DecoderMaskedSelfAttentionQuantKV<T1, T2>::DecoderMaskedSelfAttentionQuantKV(const OpKernelInfo& info)
    : CudaKernel(info), AttentionBase(info, true) {
  int64_t quantize_block_size = 0;
  ORT_ENFORCE(info.GetAttr("quant_kv_block_size", &quantize_block_size).IsOK() && quantize_block_size > 0);
  ORT_ENFORCE((quantize_block_size & -quantize_block_size) == 0);
...
Status DecoderMaskedSelfAttentionQuantKV<T1, T2>::ComputeInternal(OpKernelContext* context) const {
  ...
  const Tensor* past_scales = context->Input<Tensor>(kPastScaleIndex);
  ...
  if (sequence_length != 1) {
    return ORT_MAKE_STATUS(ONNXRUNTIME, INVALID_ARGUMENT, "Input sequence length should be 1 to use DecoderMaskedSelfAttentionQuantKV");
  }
  ...
  if (parameters.head_size != parameters.v_head_size) {
    return ORT_MAKE_STATUS(ONNXRUNTIME, NOT_IMPLEMENTED, "QK head size should be same as V head size to use DecoderMaskedSelfAttentionQuantKV");
  }

  // TODO(hasesh): If there is a need, we will support this later
  if (relative_position_bias != nullptr) {
    return ORT_MAKE_STATUS(ONNXRUNTIME, NOT_IMPLEMENTED, "DecoderMaskedSelfAttentionQuantKV does not support relative position bias currently");
  }
  ...
  if (parameters.mask_type != AttentionMaskType::MASK_2D_KEY_PADDING &&
    ...
    return ORT_MAKE_STATUS(ONNXRUNTIME, NOT_IMPLEMENTED,
                           "DecoderMaskedSelfAttentionQuantKV only supports no mask or 2D key "
  ...
  Tensor* present_scales = context->Output(kPresentScaleIndex, past_scales->Shape());
  ...
  if (past_scales_data != present_scales_data) {
    CUDA_RETURN_IF_ERROR(cudaMemcpyAsync(present_scales_data, past_scales_data, past_scales->SizeInBytes(),
                                         cudaMemcpyDeviceToDevice, cuda_stream));
  }
  ...
  parameters.k_cache = present_data;
