  ...
  Status ComputeInternal(OpKernelContext* context) const override {
    ...
    ORT_RETURN_IF_ERROR(RunCompressor<T>(
        ...
        *context->Input<Tensor>(10), nullptr, nullptr, compress_rate_, rotary_dim_, epsilon_,
        4, 2, 3, -1, -1, state, entry_capacity_, context->Input<Tensor>(1)));
    ...
    ORT_RETURN_IF_NOT(state.rank4, "HCA past_entries must have rank 4.");
      Tensor* compressed_entries = context->Output(0, entries_shape);
      CUDA_RETURN_IF_ERROR(cudaMemcpyAsync(compressed_entries->MutableData<T>(), state.entries,
