...
// Quantized types accept FP32 scalar cache scales. A PER_CHANNEL V scale may be applied to the
// output by the caller, with v_cache_scale null. PagedAttention routes PER_CHANNEL K to portable
// decode, since FP16 query folding cannot preserve its dynamic range. The compiled kInt4 kernels
// support packed UINT8 heads, FP16 query/output, head_size 256, and group_size 6, with shared-memory
// and scratch layouts matching native FP16 XQA; PagedAttention currently does not dispatch them.
//
...
Status LaunchXQAPagedKernel(
    ...
    const float* k_cache_scale,    // per-tensor scale; nullptr means "1"
...
// INT8/FP8 paged KV, or packed INT4 with FP16 query/output and scalar cache scales.
