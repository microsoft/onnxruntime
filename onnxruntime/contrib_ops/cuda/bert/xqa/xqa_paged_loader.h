...
// by the caller. The K fold is divided by a power of two just above max|k_scale| and k_cache_scale
// carries that normalizer; v_cache_scale is null because the V scale is applied to the output. It
// supports FP16 query/output, head_size 256, and group_size 6 only. Other quantized types use FP32
// per-tensor scales or the same normalized PER_CHANNEL folding. The INT4 shared-memory and scratch
// layouts match native FP16 XQA.
//
