...
__device__ inline void loadLinearAsync(void* dst, const void* src, uint32_t nbBytes, CtaBarrier& bar) {
...
__device__ inline void prefetchLinear(const void* src, uint32_t nbBytes) {
...
__device__ inline void sm2smCopyAsync(void* dst, const void* src, uint32_t nbBytes, CgaBarrier& bar) {
...
__device__ inline void loadAsync(void* dst, const CUtensorMap& tensorMap, DimsLE<nbDims> offset, CtaBarrier& bar) {
...
__device__ inline void loadAsync(
    void* dst, const CUtensorMap& tensorMap, DimsLE<nbDims> offset, CtaBarrier& bar, uint64_t cacheHint) {
  if constexpr (nbDims == 1) {
    // nbDims==1 does not need tensormap and should just use cp.async.bulk
    asm volatile(
...
// shared::cta -> global
__device__ inline void store1DAsync(void* dst, const void* src, uint32_t nbBytes) {
...
template <uint32_t nbDims>
__device__ inline void storeAsync(const CUtensorMap& tensorMap, const DimsLE<nbDims>& offset, void* src) {
  if constexpr (nbDims == 1) {
    // nbDims==1 does not need tensormap and should just use cp.async.bulk
...
__device__ inline void prefetchTensorMap(const CUtensorMap& tensorMap, StateSpace loc = StateSpace::kGENERIC) {
