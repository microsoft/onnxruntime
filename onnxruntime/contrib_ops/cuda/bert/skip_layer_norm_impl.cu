...
__global__ void SkipLayerNormKernelSmall(
  ...
  if (threadIdx.x < ld) {
    val = input[idx] + skip[idx]+bias[threadIdx.x];
...
__global__ void SkipLayerNormKernel(
  ...
  for (int i = threadIdx.x; i < ld; i += TPB) {
    ...
    const T val = input[idx] + skip[idx]+bias[i];
    const T rldval = reverse_ld * val;
    thread_data = pair_sum(thread_data, cub::KeyValuePair<T, T>(rldval, rldval * val));
    output[idx] = val;
  }
...
bool LaunchSkipLayerNormKernel(
  ...
  if (element_size == 2) {
    return ComputeSkipLayerNorm(
        ...
        reinterpret_cast<const half*>(beta), reinterpret_cast<const half*>(gamma), reinterpret_cast<const half*>(bias),
        reinterpret_cast<half*>(output));
  } else {
    return ComputeSkipLayerNorm(
        stream, hidden_size, element_count,
        reinterpret_cast<const float*>(input), reinterpret_cast<const float*>(skip),
        reinterpret_cast<const float*>(beta), reinterpret_cast<const float*>(gamma), reinterpret_cast<const float*>(bias),
