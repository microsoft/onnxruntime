...
template void mmha_quant_kv_launch_kernel<uint16_t, 2, 64>(const DecoderMaskedMultiHeadAttentionQuantKVParams& params, cudaStream_t stream);

template void mmha_quant_kv_launch_kernel<uint16_t, 4, 64>(const DecoderMaskedMultiHeadAttentionQuantKVParams& params, cudaStream_t stream);
