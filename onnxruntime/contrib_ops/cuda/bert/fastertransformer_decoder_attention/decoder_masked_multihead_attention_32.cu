...
template void mmha_launch_kernel<float, float, 32>(const DecoderMaskedMultiHeadAttentionParams& params, cudaStream_t stream);

template void mmha_launch_kernel<uint16_t, half, 32>(const DecoderMaskedMultiHeadAttentionParams& params, cudaStream_t stream);
