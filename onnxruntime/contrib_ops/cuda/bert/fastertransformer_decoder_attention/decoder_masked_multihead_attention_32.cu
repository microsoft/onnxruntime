...
#define MMHA_QUANT_KV_LAUNCH_KERNEL(                                                               \
    T, ScaleT, head_size, THDS_PER_KEY, THDS_PER_VALUE, THDS_PER_BLOCK)                                    \
  size_t dynamic_block_memory = CalcDynamicBlockMemory<T>(params, THDS_PER_VALUE, THDS_PER_BLOCK); \
  dim3 grid(params.num_heads, params.batch_size);                                                  \
...
void mmha_quant_kv_launch_kernel(const DecoderMaskedMultiHeadAttentionQuantKVParams& params, cudaStream_t stream) {
  ...
  if constexpr (ScaleElemSize == 4) {
    using ScaleT = float;
    if (total_sequence_length < 32) {
      MMHA_QUANT_KV_LAUNCH_KERNEL(T, ScaleT, head_size, 4, THREADS_PER_VALUE, 64);
    } else if (total_sequence_length < 2048) {
      MMHA_QUANT_KV_LAUNCH_KERNEL(T, ScaleT, head_size, 2, THREADS_PER_VALUE, 128);
    } else {
      MMHA_QUANT_KV_LAUNCH_KERNEL(T, ScaleT, head_size, 1, THREADS_PER_VALUE, 256);
    }
  } else {
    static_assert(ScaleElemSize == 2);
    using ScaleT = __half;
    if (total_sequence_length < 32) {
      MMHA_QUANT_KV_LAUNCH_KERNEL(T, ScaleT, head_size, 4, THREADS_PER_VALUE, 64);
    } else if (total_sequence_length < 2048) {
...
// currently only for float16, may remove __half scale type
template void mmha_quant_kv_launch_kernel<uint16_t, 2, 32>(const DecoderMaskedMultiHeadAttentionQuantKVParams& params, cudaStream_t stream);

template void mmha_quant_kv_launch_kernel<uint16_t, 4, 32>(const DecoderMaskedMultiHeadAttentionQuantKVParams& params, cudaStream_t stream);
