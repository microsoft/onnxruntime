...
#define MMHA_LAUNCH_KERNEL(                                                                        \
    T, CudaT, head_size, THDS_PER_KEY, THDS_PER_VALUE, THDS_PER_BLOCK)                             \
...
void mmha_launch_kernel(const DecoderMaskedMultiHeadAttentionParams& params, cudaStream_t stream) {
  ...
  if (total_sequence_length < 32) {
    MMHA_LAUNCH_KERNEL(T, CudaT, head_size, 4, THREADS_PER_VALUE, 64);
  } else if (total_sequence_length < 2048) {
    MMHA_LAUNCH_KERNEL(T, CudaT, head_size, 2, THREADS_PER_VALUE, 128);
  } else {
    MMHA_LAUNCH_KERNEL(T, CudaT, head_size, 1, THREADS_PER_VALUE, 256);
  }
}
