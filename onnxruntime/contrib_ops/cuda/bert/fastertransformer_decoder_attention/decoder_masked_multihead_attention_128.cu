...
template void mmha_launch_kernel<float, half, 128>(const DecoderMaskedMultiHeadAttentionParams& params, cudaStream_t stream);
template void mmha_launch_kernel<uint16_t, float, 128>(const DecoderMaskedMultiHeadAttentionParams& params, cudaStream_t stream);
