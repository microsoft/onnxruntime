...
Status PrepareQKV(
  ...
  void* k_final_ptr = data.present_key;
  void* v_final_ptr = data.present_value;
  ...
  ORT_RETURN_IF_ERROR(LaunchUnpackRoPEQuantizeAppend<CudaT>(
      ...
      nullptr, nullptr,  // Float cache/scratch: no scales
...
// Fused kernel definition moved to group_query_attention_qdq.cuh
...
Status ExtremeDecoding(
  ...
  CudaT* q_rot_ptr = reinterpret_cast<CudaT*>(data.rotary_buffer);
  ...
  void* xqa_workspace = data.scratch;
  size_t xqa_workspace_size = data.scratch_bytes;
...
// Forward declaration to fix potential visibility issues
...
Status FlashAttentionAndQuantizeKV(
  ...
  using CudaT = typename ToCudaType<T>::MappedType;
  CudaT* q_final = reinterpret_cast<CudaT*>(data.rotary_buffer);
  if (q_final == nullptr) {
    q_final = reinterpret_cast<CudaT*>(data.unpacked_qkv_buffer);
  }

  // For FlashAttentionAndQuantizeKV, we need float K and V for attention.
  // We'll write them to unpacked_qkv_buffer.
  CudaT* k_final = reinterpret_cast<CudaT*>(data.unpacked_qkv_buffer);
  if (q_final == k_final) {
    k_final += q_elements;
  }
  ...
  ORT_RETURN_IF_ERROR(LaunchUnpackRoPEQuantizeAppend<CudaT>(
      ...
      nullptr, nullptr,  // Float scratch: no scales
      num_heads, kv_num_heads, head_size, sequence_length, batch_size,
...
template Status LaunchUnpackQKV<half, false>(const half* packed_qkv, half* unpacked_q, half* unpacked_k, half* unpacked_v, const int num_heads, const int kv_num_heads, const int head_size, const int sequence_length, const int batch_size, cudaStream_t stream, const int max_threads_per_block);
template Status LaunchUnpackQKV<half, true>(const half* packed_qkv, half* unpacked_q, half* unpacked_k, half* unpacked_v, const int num_heads, const int kv_num_heads, const int head_size, const int sequence_length, const int batch_size, cudaStream_t stream, const int max_threads_per_block);
template Status LaunchUnpackQKV<BFloat16, false>(const BFloat16* packed_qkv, BFloat16* unpacked_q, BFloat16* unpacked_k, BFloat16* unpacked_v, const int num_heads, const int kv_num_heads, const int head_size, const int sequence_length, const int batch_size, cudaStream_t stream, const int max_threads_per_block);
template Status LaunchUnpackQKV<BFloat16, true>(const BFloat16* packed_qkv, BFloat16* unpacked_q, BFloat16* unpacked_k, BFloat16* unpacked_v, const int num_heads, const int kv_num_heads, const int head_size, const int sequence_length, const int batch_size, cudaStream_t stream, const int max_threads_per_block);
