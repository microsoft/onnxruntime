...
// H is head_size
template <typename T>
__global__ void TransposeQKVPacked(
...
// H is head_size
template <typename T>
__global__ void TransposeQKVPackedCutlass(
    const T* input,
    int32_t N,
    int32_t H_QK,
    int32_t H_V,
  ...
  input += token_idx * N * (H_QK + H_QK + H_V);
  q += token_idx * N * H_QK;
  k += token_idx * N * H_QK;
  v += token_idx * N * H_QK;
  ...
  if (token_idx < token_count) {
    for (int i = threadIdx.x; i < N * H_QK; i += blockDim.x) {
      int n = i / H_QK;
      int h = i % H_QK;
      q[i] = input[n * (H_QK + H_QK + H_V) + h];
      k[i] = input[n * (H_QK + H_QK + H_V) + H_QK + h];
    }

    for (int i = threadIdx.x; i < N * H_V; i += blockDim.x) {
      int n = i / H_V;
      int h = i % H_V;
      v[i] = input[n * (H_QK + H_QK + H_V) + H_QK + H_QK + h];
    }
  }
}
...
void InvokeTranspose(
  ...
  if (key != nullptr || value != nullptr) {
    ...
    if (format == AttentionQkvFormat::Q_K_V_BNSH) {
      ...
      TransposeQKVPacked<T><<<grid, kMAX_THREADS_PER_BLOCK, 0, stream>>>(
          query,
          num_heads,
          qk_head_size,
          v_head_size,
      ...
      TransposeQKVPackedCutlass<T><<<grid, kMAX_THREADS_PER_BLOCK, 0, stream>>>(
          query,
          num_heads,
          qk_head_size,
          v_head_size,
...
Status FusedScaledDotProductAttentionCutlass(
  ...
  DUMP_TENSOR_D("PackedMHA cutlass data.gemm_buffer", data.gemm_buffer, parameters.token_count, 3, num_heads * qk_head_size);
  DUMP_TENSOR_D("PackedMHA cutlass data.bias", data.bias, 1, 3 * num_heads * qk_head_size);
...
Status UnfusedScaledDotProductAttention(
  ...
  DUMP_TENSOR_D("PackedMHA unfused gemm_buffer", data.gemm_buffer, parameters.token_count, (num_heads * (qk_head_size * 2 + v_head_size)));
