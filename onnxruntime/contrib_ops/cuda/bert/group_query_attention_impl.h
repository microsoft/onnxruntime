...
struct GQABufferRequirements {
  ...
  static GQABufferRequirements Compute(
    ...
    if (use_flash_attention) {
      ...
      //
      // Logic:
      // - If FlashAttentionAndQuantizeKV is used (first prompt + quant + share buffer),
      //   we need full Q+K+V in scratch because FA needs float K/V inputs, which we
      //   generate from packed/rotary kernel before they are quantized into cache.
      // - Otherwise, we generally only need Q buffer (for rotary Q) if we can write K/V directly to cache/output.
