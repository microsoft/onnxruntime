...
template <typename T>
__global__ void SequencePoolingCudaKernel(const T* input, const int64_t* sentence_lengthes, const int num_sequences, const int sequence_length_for_split, T* output, T* mask) {
  ...
  if (sentence_lengthes[seq_id_per_batch + offset] == 0) {
    ...
    mask[seq_id_per_batch + offset] = 0;
    ...
    mask[seq_id_per_batch + offset] = 1;
  }
}
...
bool SequencePooling(
  ...
  SequencePoolingCudaKernel<T><<<grid, block, 0, stream>>>(input, sentence_lengthes, num_sequences, sequence_length_for_split, output, mask);
