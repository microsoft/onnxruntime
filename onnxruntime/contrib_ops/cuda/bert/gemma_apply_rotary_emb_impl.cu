...
(q_rot) (sin_emb) (q) (cos_emb)  (sin_emb) (k_rot) (cos_emb) (k)
  \  /              \   /          \        /        \       /
   Mul               Mul               Mul               Mul
      \           /                        \     /
            Add                              Add  
...
using namespace onn11xruntime::cuda;
...
template <typename T>
__global__ void GemmaApplyRotaryEmb(
                                T* output1,
                                T* output2,
                                const T* sin_emb,
                                const T* cos_emb,
...
Status LaunchGemmaApplyRotaryEmbeddingKernel(
  ...
  GemmaApplyRotaryEmb<<<blocksPerGrid, kThreadsPerBlock, 0, stream>>>(
    output1, output2,
    sin_emb, cos_emb,
    q, q_rot, k, k_rot,
...
template Status LaunchGemmaApplyRotaryEmbeddingKernel<half>(
    cudaStream_t stream,
    half* output1,
    half* output2,
    // const float* emb,
