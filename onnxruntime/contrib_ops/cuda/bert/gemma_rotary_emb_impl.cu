...
__global__ void GemmaRotaryEmb(
    ...
    const int emb_idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (emb_idx < batch_size * seq_len * dim) {
...
Status LaunchGemmaRotaryEmbeddingKernel(
  ...
  int blocksPerGrid = static_cast<int>(ceil(float(batch_size * seq_len * dim) / kThreadsPerBlock));
