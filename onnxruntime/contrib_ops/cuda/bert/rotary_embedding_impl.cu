...
__global__ void RotaryEmbeddingBSNH(T* output,                    // BxSxNxH
  ...
  const int bs = blockIdx.z * blockDim.y + blockIdx.y;

  const int b = bs / sequence_length;
  const int s = bs % sequence_length;

  if (b >= batch_size || s >= sequence_length) {
    return;
  }
...
Status LaunchRotaryEmbeddingKernel(
  ...
  const dim3 grid(num_heads, 1024, batch_size * (sequence_length / 1024 + 1));
  ...
  RotaryEmbeddingBSNH<<<grid, block, 0, stream>>>(
      output, input, cos_cache, sin_cache, position_ids, batch_size, sequence_length, num_heads, head_size,
