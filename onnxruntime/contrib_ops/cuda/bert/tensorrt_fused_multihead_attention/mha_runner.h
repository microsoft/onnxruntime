...
class MHARunner {
 public:
  MHARunner(const int numHeads, const int headSize, const int wordSize, bool causal_mask, const float softmax_scale)
      : mS(0),
        mB(0),
        mOmatSize(0),
        mNumMats(0),
        ...
        mLdOut(0),
        mStrideOut(0),
        mSoftmaxScale(softmax_scale == 0.0f ? 1.f / sqrtf(static_cast<float>(headSize))
                      : softmax_scale),
