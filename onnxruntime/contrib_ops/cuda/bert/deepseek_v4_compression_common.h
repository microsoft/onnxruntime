...
Status RunCompressor(const CudaKernel& kernel, OpKernelContext* context,
  ...
  ORT_RETURN_IF_NOT(pending_count < compress_rate, "pending state must be shorter than compress_rate.");
  ...
  const int64_t old_entry_count = rank4 ? entries_shape[2] : entries_shape[1];
  ...
  const int64_t new_entry_count = total_count / compress_rate;
  const int64_t output_pending_count = total_count % compress_rate;
  const int64_t output_entry_count = old_entry_count + new_entry_count;
  ORT_RETURN_IF(new_entry_count > 0 && (output_entry_count - 1) * compress_rate >= cos_cache.Shape()[0],
  ...
  for (int64_t batch_index = 0; batch_index < batch; ++batch_index) {
    CUDA_RETURN_IF_ERROR(cudaMemcpyAsync(
        ...
        old_entries + batch_index * old_entry_count * head_size,
        static_cast<size_t>(old_entry_count * head_size) * sizeof(CudaT),
  ...
  CudaT* overlap_gate = overlap_gate_output
                            ...
                            : nullptr;
  if (is_overlap && new_entry_count == 0) {
  ...
  ORT_RETURN_IF_ERROR(LaunchDeepSeekV4CompressorKernel<CudaT>(
      ...
      current_kv.get(), current_gate.get(), reinterpret_cast<const CudaT*>(past_pending_kv.Data<T>()),
      reinterpret_cast<const CudaT*>(past_pending_gate.Data<T>()),
      is_overlap ? reinterpret_cast<const CudaT*>(past_overlap_kv->Data<T>()) : nullptr,
      is_overlap ? reinterpret_cast<const CudaT*>(past_overlap_gate->Data<T>()) : nullptr,
      ...
      reinterpret_cast<const CudaT*>(sin_cache.Data<T>()), narrow<int>(batch), narrow<int>(sequence),
      ...
      narrow<int>(cos_cache.Shape()[1]), epsilon, is_overlap, kernel.GetDeviceProp().maxThreadsPerBlock));
