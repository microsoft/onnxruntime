...
Status Attention<T>::ComputeInternal(OpKernelContext* context) const {
  ...
  if (use_flash_attention) {
    ...
    if (parameters.num_splits > 1) {
      ...
      out_accum_bytes = onnxruntime::flash::get_out_accum_size(parameters.num_splits, parameters.batch_size,
                                                               parameters.num_heads, parameters.sequence_length, head_size_rounded);
    }
  }
