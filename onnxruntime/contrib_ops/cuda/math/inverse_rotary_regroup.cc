...
#include "contrib_ops/cuda/math/inverse_rotary_regroup.h"
...
template <typename T>
InverseRotaryRegroup<T>::InverseRotaryRegroup(const OpKernelInfo& info) : CudaKernel(info) {
...
template <typename T>
Status InverseRotaryRegroup<T>::ComputeInternal(OpKernelContext* context) const {
  ...
  InverseRotaryRegroupParams params;
  ...
  return LaunchInverseRotaryRegroup<T>(Stream(context), params, input->Data<T>(),
