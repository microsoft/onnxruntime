...
//
// `capacity` is `row_id_offset / ratio`, fixed at export time, while the rows that hold live
...
Status ClampScoreCapacity(OpKernelContext* context, const Tensor& past_lens,
  ...
  int64_t needed =
      SparseRowSelectVisibleRows(longest, params.seq_len - 1, params.ratio) + 1;
...
template <typename T>
SparseRowSelect<T>::SparseRowSelect(const OpKernelInfo& info) : CudaKernel(info) {
  ...
  row_id_offset_ = info.GetAttrOrDefault<int64_t>("row_id_offset", static_cast<int64_t>(0));
  scale_ = info.GetAttrOrDefault<float>("scale", 1.0f);
  simulate_rotated_fp4_ = info.GetAttrOrDefault<int64_t>("simulate_rotated_fp4", static_cast<int64_t>(1)) != 0;
...
template <typename T>
Status SparseRowSelect<T>::ComputeInternal(OpKernelContext* context) const {
  ...
  params.row_id_offset = static_cast<int>(row_id_offset_);
  params.scale = scale_;
