...
#include "contrib_ops/cuda/math/dsv4_qkv_norm_rope_impl.h"
...
__global__ void DSV4QueryNormRopeKernel(const DSV4QKVNormRopeParams p,
  ...
  for (int c = tid; c < d; c += kThreads) {
    const float v = DSV4Conv<CudaT>::ToFloat(src[c]);
    s_row[c] = v;
    ss += v * v;
  }
  const float total = BlockSum(ss, s_red, tid);
  const float rs = DSV4Conv<CudaT>::Round(1.0f / sqrtf(total / d + p.epsilon));
  for (int c = tid; c < d; c += kThreads) s_row[c] = DSV4Conv<CudaT>::Round(s_row[c] * rs);
  __syncthreads();
  ...
  for (int c = tid; c < d; c += kThreads) dst[c] = DSV4Conv<CudaT>::FromFloat(s_row[c]);
}
...
template <typename CudaT>
__global__ void DSV4KvNormRopeKernel(const DSV4QKVNormRopeParams p,
  ...
  for (int c = tid; c < d; c += kThreads) {
    s_row[c] = DSV4Conv<CudaT>::Round(s_row[c] * rs * norm_weight[c]);
  }
  __syncthreads();
  ...
  if (p.act_quant) {
    ...
    for (int i = tid; i < blocks; i += kThreads) {
      ...
      s_scale[i] = DSV4BlockScale(amax, kDSV4Fp8Max, 1e-30f);
    }
    __syncthreads();
    for (int c = tid; c < p.nope_dim; c += kThreads) {
      ...
      s_row[c] = DSV4Conv<CudaT>::Round(DSV4RoundE4M3(q) * scale);
    }
    __syncthreads();
  }
...
int DSV4KvNormRopeSharedFloats(const DSV4QKVNormRopeParams& p) {
  return p.head_dim + kWarps + p.rope_head_dim + (p.act_quant ? p.nope_dim / 64 : 0);
}

template <typename T>
Status LaunchDSV4QKVNormRope(cudaStream_t stream, const DSV4QKVNormRopeParams& p,
  ...
  DSV4QueryNormRopeKernel<CudaT>
      <<<tokens * p.num_heads, kThreads, DSV4QueryNormRopeSharedFloats(p) * sizeof(float),
         stream>>>(p, reinterpret_cast<const CudaT*>(query), cos_table, sin_table,
                   reinterpret_cast<CudaT*>(query_out));
  CUDA_RETURN_IF_ERROR(cudaGetLastError());

  DSV4KvNormRopeKernel<CudaT>
      <<<tokens, kThreads, DSV4KvNormRopeSharedFloats(p) * sizeof(float), stream>>>(
...
#define INSTANTIATE(T)                                                                     \
  template Status LaunchDSV4QKVNormRope<T>(cudaStream_t, const DSV4QKVNormRopeParams&,     \
