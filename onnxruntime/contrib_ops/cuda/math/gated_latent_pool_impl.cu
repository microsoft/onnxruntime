...
constexpr int kThreads = kGatedLatentPoolThreads;
...
// Kill switch for the fast pooling kernel. Set ORT_DISABLE_GATED_LATENT_POOL_FAST=1 to fall back to the
// one-thread-per-channel reference kernel below, which is what the fast path is validated against.
bool GatedLatentPoolFastDisabledByEnv() {
  // Parsed once via ORT's environment helper (consistent parsing/thread-safety across platforms).
  static const bool disabled =
      onnxruntime::ParseEnvironmentVariableWithDefault<int>("ORT_DISABLE_GATED_LATENT_POOL_FAST", 0) == 1;
  return disabled;
}

int GatedLatentPoolLoaders(int span) {
  const int want = span < kPoolMinLoaders ? kPoolMinLoaders : span;
  return want > kPoolMaxLoaders ? kPoolMaxLoaders : want;
}

size_t GatedLatentPoolSharedBytes(int span) {
...
__global__ void GatedLatentPoolPoolKernel(const GatedLatentPoolParams p,
  ...
  for (int s = 0; s < p.span; ++s) {
    ...
    const float w = valid ? (use_past ? past_score[off] : QuantSimConv<CudaP>::ToFloat(score[off])) +
                                ape[k * p.feat + fo]
                          : kNegInf;
    m = fmaxf(m, w);
  }
  ...
  for (int s = 0; s < p.span; ++s) {
    ...
    const float w = valid ? (use_past ? past_score[off] : QuantSimConv<CudaP>::ToFloat(score[off])) +
                                ape[k * p.feat + fo]
                          : kNegInf;
    acc += (use_past ? past_kv[off] : QuantSimConv<CudaP>::ToFloat(kv[off])) * (expf(w - m) / denom);
  }

  rows[(static_cast<int64_t>(b) * p.num_rows + j) * p.head_dim + ch] =
      QuantSimConv<CudaT>::FromFloat(acc);
}
...
__global__ void GatedLatentPoolPoolFastKernel(const GatedLatentPoolParams p,
                                              const CudaP* __restrict__ kv,
                                              const CudaP* __restrict__ score,
                                              const float* __restrict__ past_kv,
                                              const float* __restrict__ past_score,
  ...
  auto stage = [&](int s) {
    ...
    s_w[slot] = (valid && live)
                    ? (use_past ? past_score[off] : QuantSimConv<CudaP>::ToFloat(score[off])) +
                          ape[static_cast<int64_t>(k) * feat + fo]
                    : kNegInf;
    s_v[slot] = live ? (use_past ? past_kv[off] : QuantSimConv<CudaP>::ToFloat(kv[off])) : 0.0f;
  };
...
__global__ void GatedLatentPoolFinishKernel(const GatedLatentPoolParams p,
  ...
  for (int c = tid; c < d; c += kThreads) {
    const float v = QuantSimConv<CudaT>::ToFloat(row[c]);
    s_row[c] = v;
    ss += v * v;
  }
#pragma unroll
  ...
  if (p.simulate_fp8) {
    ...
    if (tid < blocks) {
      ...
      s_scale[tid] = QuantSimBlockScale(amax, kQuantSimFp8Max, 1e-30f);
    }
    __syncthreads();
    for (int c = tid; c < p.nope_dim; c += kThreads) {
      ...
      s_row[c] = QuantSimRoundE4M3(q) * scale;
    }
    __syncthreads();
  }
  ...
  for (int c = tid; c < d; c += kThreads) row[c] = QuantSimConv<CudaT>::FromFloat(s_row[c]);
}
...
__global__ void GatedLatentPoolStateKernel(const GatedLatentPoolParams p,
                                           const CudaP* __restrict__ kv,
                                           const CudaP* __restrict__ score,
                                           const float* __restrict__ past_kv,
                                           const float* __restrict__ past_score,
  ...
  for (int64_t i = static_cast<int64_t>(blockIdx.x) * blockDim.x + threadIdx.x; i < n; i += stride) {
    ...
    if (src < p.span) {
      ...
      out_kv[dst] = QuantSimConv<CudaP>::ToFloat(kv[off]);
      out_score[dst] = QuantSimConv<CudaP>::ToFloat(score[off]);
    }
  }
...
Status LaunchGatedLatentPool(cudaStream_t stream, const GatedLatentPoolParams& params,
  ...
  const size_t pool_shared = GatedLatentPoolSharedBytes(params.span);
  if (!GatedLatentPoolFastDisabledByEnv() && pool_shared <= kPoolMaxSharedBytes) {
    const dim3 grid((params.head_dim + kPoolChannels - 1) / kPoolChannels, params.num_rows,
                    params.batch);
    const dim3 block(kPoolChannels, GatedLatentPoolLoaders(params.span));
    if (params.window_multiplier == 2) {
      GatedLatentPoolPoolFastKernel<CudaT, CudaP, 2><<<grid, block, pool_shared, stream>>>(
          params, proj_kv, proj_score, past_state_kv, past_state_score, ape, past_lens, out);
    } else {
      GatedLatentPoolPoolFastKernel<CudaT, CudaP, 1><<<grid, block, pool_shared, stream>>>(
          params, proj_kv, proj_score, past_state_kv, past_state_score, ape, past_lens, out);
    }
  } else {
    const int channel_blocks = (params.head_dim + kThreads - 1) / kThreads;
    GatedLatentPoolPoolKernel<CudaT, CudaP><<<dim3(channel_blocks, params.num_rows, params.batch),
                                              kThreads, 0, stream>>>(
        params, proj_kv, proj_score, past_state_kv, past_state_score, ape, past_lens, out);
  }

  const size_t shared = GatedLatentPoolFinishSharedFloats(params) * sizeof(float);
  if (shared > 48u * 1024u) {
    return ORT_MAKE_STATUS(ONNXRUNTIME, INVALID_ARGUMENT,
                           "GatedLatentPool needs ", shared,
                           " bytes of shared memory per row, "
                           "which is over the 48 KiB limit.");
  }
  GatedLatentPoolFinishKernel<CudaT><<<dim3(params.num_rows, params.batch), kThreads, shared,
                                       stream>>>(
  ...
  GatedLatentPoolStateKernel<CudaP><<<dim3(state_blocks, params.batch), kThreads, 0, stream>>>(
...
#define INSTANTIATE(T, P)                                                                   \
  template Status LaunchGatedLatentPool<T, P>(                                              \
      cudaStream_t, const GatedLatentPoolParams&, const P*, const P*, const float*,         \
