...
__global__ void DSV4InvRopeGroupKernel(const DSV4InvRopeGroupParams p,
  ...
  for (int64_t idx = blockIdx.x * static_cast<int64_t>(blockDim.x) + threadIdx.x; idx < total;
    ...
    output[out_idx] = DSV4Conv<CudaT>::FromFloat(v);
  }
}
...
template <typename T>
Status LaunchDSV4InvRopeGroup(cudaStream_t stream, const DSV4InvRopeGroupParams& p,
  ...
  DSV4InvRopeGroupKernel<CudaT><<<blocks, kThreads, 0, stream>>>(
...
#define INSTANTIATE(T)                                                                   \
  template Status LaunchDSV4InvRopeGroup<T>(cudaStream_t, const DSV4InvRopeGroupParams&, \
