...
template <typename T>
__device__ __inline__ void _Debug(T a0, T a1, T b0, T b1, T* output_data, bool is_conj) {
  output_data[0] = 0;
  output_data[1] = 0;
};
...
#pragma unroll
      ...
      /*
      a[i] = lhs_data[lhs_index];
      b[i] = lhs_data[lhs_index + 1];
      c[i] = rhs_data[rhs_index];
      d[i] = rhs_data[rhs_index + 1];
      */
...
#pragma unroll
  for (int i = 0; i < NumElementsPerThread; i++) {
    if (id < N / 2) {
      ...
      //_Debug(a[i], b[i], c[i], d[i], &output_data[2 * id], is_conj);
      id += NumThreadsPerBlock;
    }
  }
};
