...
template <typename CudaT>
__global__ void MoERouterKernel(const MoERouterParams p,
  ...
  if (log_routed_experts && tid == 0 && topk == 6) {
    printf("ORT_MOE_ROUTER_FORCE_EXPERTS tokens=%d token=%d ids=%d,%d,%d,%d,%d,%d\n",
  ...
  for (int c = tid; c < local_count; c += kThreads) {
    ...
    out_row[c] = QuantSimConv<CudaT>::FromFloat(log_weight);
    local_sum += weight;
  }
...
Status LaunchMoERouter(cudaStream_t stream, const MoERouterParams& p, const float* scores,
  ...
  const static bool log_routed_experts =
      onnxruntime::ParseEnvironmentVariableWithDefault<int>("ORT_MOE_ROUTER_LOG_EXPERTS", 0) == 1;
