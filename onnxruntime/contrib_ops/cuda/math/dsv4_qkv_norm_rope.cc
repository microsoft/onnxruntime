...
#include "contrib_ops/cuda/math/dsv4_qkv_norm_rope.h"
...
template <typename T>
DSV4QKVNormRope<T>::DSV4QKVNormRope(const OpKernelInfo& info) : CudaKernel(info) {
  ...
  act_quant_ = info.GetAttrOrDefault<int64_t>("act_quant", static_cast<int64_t>(1)) != 0;
...
template <typename T>
Status DSV4QKVNormRope<T>::ComputeInternal(OpKernelContext* context) const {
  ...
  return LaunchDSV4QKVNormRope<T>(Stream(context), params, query->Data<T>(), kv->Data<T>(),
