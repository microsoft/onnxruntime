...
#include "contrib_ops/cuda/math/dsv4_compressor.h"
...
template <typename T>
DSV4Compressor<T>::DSV4Compressor(const OpKernelInfo& info) : CudaKernel(info) {
  ratio_ = info.GetAttrOrDefault<int64_t>("ratio", static_cast<int64_t>(4));
  coff_ = info.GetAttrOrDefault<int64_t>("coff", static_cast<int64_t>(1));
  ...
  act_quant_ = info.GetAttrOrDefault<int64_t>("act_quant", static_cast<int64_t>(0)) != 0;
  rotate_fp4_ = info.GetAttrOrDefault<int64_t>("rotate_fp4", static_cast<int64_t>(0)) != 0;

  ORT_ENFORCE(ratio_ >= 1, "ratio must be positive, got ", ratio_);
  ORT_ENFORCE(coff_ == 1 || coff_ == 2, "coff must be 1 or 2, got ", coff_);
  ...
  ORT_ENFORCE(!act_quant_ || (head_dim_ - rope_head_dim_) % 64 == 0,
              "act_quant needs head_dim - rope_head_dim to be a multiple of 64, got ",
...
template <typename T>
Status DSV4Compressor<T>::ComputeInternal(OpKernelContext* context) const {
  ...
  if (kv_dims.size() != 3) {
    return ORT_MAKE_STATUS(ONNXRUNTIME, INVALID_ARGUMENT,
                           "kv is expected to be [batch, sequence, coff * head_dim], got rank ",
                           kv_dims.size());
  }

  const int64_t batch = kv_dims[0];
  const int64_t seq_len = kv_dims[1];
  const int64_t feat = coff_ * head_dim_;
  ...
  params.coff = static_cast<int>(coff_);
