...
constexpr int kSparseRowSelectThreads = 256;

struct SparseRowSelectParams {
  int batch;          // B
  ...
  int row_id_offset;          // L, the logical offset that marks a compressed row
  float scale;                // folded into the per-head weight
  bool simulate_rotated_fp4;  // Hadamard-rotate the query and simulate the FP4 round trip
};

// Scratch the launcher needs, in elements.
inline int64_t SparseRowSelectQueryElems(const SparseRowSelectParams& p) {
  return static_cast<int64_t>(p.batch) * p.seq_len * p.num_heads * p.head_dim;
}
inline int64_t SparseRowSelectCacheElems(const SparseRowSelectParams& p) {
...
inline int64_t SparseRowSelectScoreElems(const SparseRowSelectParams& p) {
  return static_cast<int64_t>(p.batch) * p.seq_len * p.num_heads * p.capacity;
}
inline int64_t SparseRowSelectKeyElems(const SparseRowSelectParams& p) {
...
inline int64_t SparseRowSelectQueryBf16Bytes(const SparseRowSelectParams& p) {
  return SparseRowSelectQueryElems(p) * 2;
}
...
#endif
    int64_t
    SparseRowSelectVisibleRows(int64_t past_len, int64_t s, int64_t ratio) {
  return (past_len + s + 1) / ratio;
}

// Shared floats the query kernel needs: the row, a copy of its rotary slice, and one scale
// per FP4 block.
inline int SparseRowSelectQuerySharedFloats(const SparseRowSelectParams& p) {
  return p.head_dim + p.rope_head_dim + (p.simulate_rotated_fp4 ? p.head_dim / 32 : 0);
}

template <typename T>
Status LaunchSparseRowSelect(cudaStream_t stream, cublasHandle_t cublas,
                             const cudaDeviceProp& prop, bool use_tf32,
                             const SparseRowSelectParams& params,
