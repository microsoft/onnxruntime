...
// DeepSeek-V4 was trained against simulated low-precision grids, so its graph carries the
// quantisation arithmetic explicitly and a fused kernel has to reproduce it step for step --
...
constexpr float kDSV4Log2 = 0.69314718f;  // the graph divides by float(math.log(2.0))
constexpr float kDSV4Fp8Max = 448.0f;
constexpr float kDSV4Fp4Max = 6.0f;

template <typename CudaT>
struct DSV4Conv;

template <>
struct DSV4Conv<float> {
...
__device__ __forceinline__ float DSV4BlockScale(float amax, float limit, float floor_value) {
  float r = amax / limit;
  if (r < floor_value) r = floor_value;
  return amax > 0.0f ? exp2f(ceilf(logf(r) / kDSV4Log2)) : 1.0f;
}
...
__device__ __forceinline__ float DSV4RoundE4M3(float v) {
...
template <typename CudaT>
__device__ __forceinline__ void DSV4RotateFp4(float* s_row, float* s_scale, int d,
  ...
  for (int c = tid; c < d; c += nthreads) {
    s_row[c] = DSV4Conv<CudaT>::Round(s_row[c] * hscale);
  }
  __syncthreads();
  ...
  for (int i = tid; i < blocks; i += nthreads) {
    ...
    s_scale[i] = DSV4BlockScale(amax, kDSV4Fp4Max, 1e-38f);
  }
  __syncthreads();
  for (int c = tid; c < d; c += nthreads) {
    const float scale = s_scale[c >> 5];
    const float v = fminf(fmaxf(s_row[c] / scale, -kDSV4Fp4Max), kDSV4Fp4Max);
