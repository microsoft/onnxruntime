...
#include "contrib_ops/cuda/llm/kernels/cutlass_kernels/cutlass_heuristic.h"
  ...
  if (config & CutlassGemmConfig::GROUPED_GEMM) {
    ...
    for (int cluster_m = 1; cluster_m <= 2; cluster_m++) {
      ...
      for (int cluster_n = 1; cluster_n <= 2; cluster_n++) {
        ...
        constexpr std::array cluster_shapes = {std::array{ClusterShape::ClusterShape_1x1x1, ClusterShape::ClusterShape_1x2x1},
