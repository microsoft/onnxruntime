...
#define TLLM_CHECK(val)                                                                                                \
    ...
    {                                                                                                                  \
        TLLM_LIKELY(static_cast<bool>(val)) ? ((void) 0)                                                               \
                                            : ort_llm::common::throwRuntimeError(__FILE__, __LINE__, #val);       \
    } while (0)
...
#define TLLM_CHECK_WITH_INFO(val, info, ...)                                                                           \
    ...
    {                                                                                                                  \
        ...
        ? ((void) 0)                                                                                                   \
        : ort_llm::common::throwRuntimeError(                                                                     \
            __FILE__, __LINE__, ort_llm::common::fmtstr(info, ##__VA_ARGS__));                                    \
    } while (0)
...
#define TLLM_WRAP(ex)                                                                                                  \
    NEW_TLLM_EXCEPTION("%s: %s", ort_llm::common::TllmException::demangle(typeid(ex).name()).c_str(), ex.what())
