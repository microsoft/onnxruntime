...
#pragma unroll
    for (int mask = 16; mask > 0; mask >>= 1){
        //val = add<T>(val, ); //__shfl_sync bf16 return float when sm < 80    
        auto tmp = __shfl_xor_sync(FINAL_MASK, val, mask, 32);
        //val = add<T>(val, tmp); //__shfl_sync bf16 return float when sm < 80  
        val = val + static_cast<T>(tmp);
    }
