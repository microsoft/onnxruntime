...
#define INSTANTIATE_TMA_WARP_SPECIALIZED_MOE_GEMM(ArchTag_, DataType_, WeightType_, OutputType_, EpilogueTag_,                                                                                                                                    \
    ...
    {                                                                                                                                                                                                                                             \
        ...
        {                                                                                                                                                                                                                                         \
            ...
            /* SM120 FP4xFP4 (same-type): NV-native nv_float4_t (ue4m3 SF, IsMXFPX=false) */                                                                                                                                                     \
            /* SM120 FP8xFP4 (mixed): MX-format tuple (ue8m0 SF, IsMXFPX=true)  */                                                                                                                                                               \
            /* SM100: MX-format tuple (ue8m0 SF) for both same-type and mixed    */                                                                                                                                                               \
            using ElementABlockScaled                                                                                                                                                                                                             \
                = std::conditional_t<IsSM120 && !IsWFP4AFP8, cutlass::nv_float4_t<ElementA>, cute::tuple<ElementA, ElementSF>>;                                                                                                                   \
            using ElementBBlockScaled                                                                                                                                                                                                             \
                = std::conditional_t<IsSM120 && !IsWFP4AFP8, cutlass::nv_float4_t<ElementB>, cute::tuple<ElementB, ElementSF>>;                                                                                                                   \
                                                                                                                                                                                                                                                  \
            ...
            using MainloopTileShape = std::conditional_t<IsBlackwell, MainloopTileShapeSm100, MainloopTileShapeSm90>;                                                                                                                             \
                                                                                                                                                                                                                                                  \
            /* For SM120 FP8xFP4, do NOT swap A/B — SM120 block-scaled builder expects natural order with TN layout. */       \
            /* The SM90 A/B swap (B^T * A^T trick) is not compatible with SM120's block-scaled TN layout requirement. */ \
            using BuilderElementA = std::conditional_t<IsSM120 && IsWFP4AFP8, MainloopElementA, MainloopElementB>;      \
            using BuilderLayoutA  = std::conditional_t<IsSM120 && IsWFP4AFP8, cutlass::layout::RowMajor, LayoutB>;      \
            constexpr static int BuilderAlignmentA = (IsSM120 && IsWFP4AFP8) ? AlignmentA : AlignmentB;                 \
            using BuilderElementB = std::conditional_t<IsSM120 && IsWFP4AFP8, MainloopElementB, MainloopElementA>;      \
            using BuilderLayoutB  = std::conditional_t<IsSM120 && IsWFP4AFP8, cutlass::layout::ColumnMajor, LayoutA>;   \
            constexpr static int BuilderAlignmentB = (IsSM120 && IsWFP4AFP8) ? AlignmentB : AlignmentA;                 \
                                                                                                                                                                                                                                                  \
            using CollectiveMainloop = typename cutlass::gemm::collective::CollectiveBuilder</**/                                                                                                                                                 \
                Arch, TensorOp,                                                              /**/                                                                                                                                                 \
                BuilderElementA, BuilderLayoutA*, BuilderAlignmentA,                         /* A & B swapped for SM90; natural order for SM120 FP8xFP4 */                                                                                        \
                BuilderElementB, BuilderLayoutB*, BuilderAlignmentB,                         /**/                                                                                                                                                 \
                ElementAccumulator,                                                          /**/                                                                                                                                                 \
            ...
            {                                                                                                                                                                                                                                     \
                if constexpr (IsBlockScaled && IsSM120 && IsWFP4AFP8)                                                                                                                                                                          \
                {                                                                                                                                                                                                                                 \
                    /* SM120 FP8xFP4: No A/B swap — CUTLASS A = ORT A (activation), CUTLASS B = ORT B (weight) */                                                                                                                                 \
                ...
                else if constexpr (IsBlockScaled)                                                                                                                                                                                                 \
                {                                                                                                                                                                                                                                 \
                    return construct_if_true<IsBlockScaled, MainloopArguments>(                                                                                                                                                                   \
                        reinterpret_cast<ElementB const**>(tma_ws_input.ptr_b), tma_ws_input.stride_b,                                                                                                                                            \
                        reinterpret_cast<ElementA const**>(tma_ws_input.ptr_a), tma_ws_input.stride_a,                                                                                                                                            \
