...
#include "contrib_ops/cuda/llm/cutlass_extensions/arch/copy_red_global.hpp"
#include "contrib_ops/cuda/llm/cutlass_extensions/util/gather_tensor.hpp"
...
class EpilogueMoeFusedFinalize {
 public:
  ...
  CUTLASS_HOST_DEVICE static bool can_implement(
    ...
    if (problem_shape.is_host_problem_shape_available()) {
      ...
      for (int i = 0; i < problem_shape.groups(); i++) {
        ...
        implementable = implementable && cutlass::detail::check_alignment<AlignmentD>(cute::make_shape(M, N, L), InternalStrideD{});
      }
    }
  ...
  bool is_source_needed() {
    ...
    return params.ptr_C != nullptr && (params.thread.beta_ptr_array || params.thread.beta_ptr || params.thread.beta != 0);
  }
  ...
  CUTLASS_HOST_DEVICE void operator()(ProblemShapeMNKL problem_shape_mnkl, BlockShapeMNK blk_shape_MNK,
    ...
    auto read_scatter_map = onnxruntime::llm::cutlass_extensions::IndexedGather(
    ...
    Tensor mD_mnl = onnxruntime::llm::cutlass_extensions::make_gather_tensor(
    ...
    Tensor gScale_mnl = local_tile(mScale_mnl, blk_shape_MNK, make_coord(_, _, _), Step<_1, _1, X>{});  // (BLK_M,BLK_N,m,n,l)
...
struct EpilogueMoeFusedFinalizeBuilder {
  ...
  using ThreadEpilogueOp = cutlass::epilogue::thread::LinearCombination<ElementCompute, 1, ElementAccumulator, ElementCompute,
                                                                        cutlass::epilogue::thread::ScaleType::Default, cutlass::FloatRoundStyle::round_to_nearest, ElementC>;

  using SmemLayoutAtomD = decltype(detail::sm90_get_epilogue_smem_swizzle_layout_atom<StrideD, ElementAccumulator, EpilogueTile>());
  using CopyAtomR2S = decltype(detail::sm90_get_smem_store_op_for_accumulator<StrideD, ElementAccumulator, EpilogueTile>());
