...
inline int compute_occupancy_for_kernel()
{
    ...
    if (smem_size > (48 << 10))
    {
        ...
        onnxruntime::llm::common::check_cuda_error(cudaGetDevice(&device));
        onnxruntime::llm::common::check_cuda_error(
            cudaDeviceGetAttribute(&max_smem_per_block, cudaDevAttrMaxSharedMemoryPerBlockOptin, device));
        if constexpr (enable_cutlass_3x)
        {
            onnxruntime::llm::common::check_cuda_error(cudaFuncGetAttributes(&attr, cutlass::device_kernel<GemmKernel>));
        }
        else
        {
            onnxruntime::llm::common::check_cuda_error(cudaFuncGetAttributes(&attr, cutlass::Kernel<GemmKernel>));
        }
        ...
        if constexpr (enable_cutlass_3x)
        {
            onnxruntime::llm::common::check_cuda_error(cudaFuncSetAttribute(
    ...
    {
        onnxruntime::llm::common::check_cuda_error(cudaOccupancyMaxActiveBlocksPerMultiprocessor(
