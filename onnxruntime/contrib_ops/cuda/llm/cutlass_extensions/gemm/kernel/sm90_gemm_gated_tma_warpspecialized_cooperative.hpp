...
class GemmUniversalGated<ProblemShape_, CollectiveMainloop_, CollectiveEpilogue_, TileScheduler_,
                         cute::enable_if_t<
                             cute::is_base_of_v<KernelTmaWarpSpecializedCooperative, typename CollectiveMainloop_::DispatchPolicy::Schedule> && CollectiveMainloop_::isGated>> {
 public:
  //
  // Type Aliases
  //
  ...
  static bool can_implement(Arguments const& args) {
    bool implementable = (args.mode == GemmUniversalMode::kGemm) or (args.mode == GemmUniversalMode::kBatched && cute::rank(ProblemShape{}) == 4);
    if (!implementable) {
    ...
    if (warp_group_role == WarpGroupRole::Producer) {
      ...
      while (work_tile_info.is_valid()) {
        ...
        if (TileScheduler::compute_epilogue(work_tile_info, params.scheduler)) {
          ...
          auto [epi_load_pipe_consumer_state_next, epi_store_pipe_producer_state_next] = collective_epilogue.store(epi_load_pipeline, epi_load_pipe_consumer_state, epi_store_pipeline,
