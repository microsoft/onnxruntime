...
template <>
constexpr Activation_Type EpilogueRouting<onnxruntime::llm::cutlass_extensions::EpilogueOpDefault>(bool /*is_gate*/) {
  return Activation_Type::Identity;
}

template <>
constexpr Activation_Type EpilogueRouting<onnxruntime::llm::cutlass_extensions::EpilogueOpDefaultReLU>(bool /*is_gate*/) {
  return Activation_Type::Relu;
}

template <>
constexpr Activation_Type EpilogueRouting<onnxruntime::llm::cutlass_extensions::EpilogueOpDefaultSilu>(bool is_gate) {
  return is_gate ? Activation_Type::Swiglu : Activation_Type::Silu;
}

template <>
constexpr Activation_Type EpilogueRouting<onnxruntime::llm::cutlass_extensions::EpilogueOpDefaultFtGelu>(bool is_gate) {
...
struct Fused_Moe_Kernel_traits_sm80 {
  ...
  using ThreadLayoutMNK = std::conditional_t<kTileM == 16, cute::Layout<cute::Shape<cute::_1, cute::Int<kWarpsCount / 1>, cute::_1>>,
  ...
  using DefaultOperandA = DefaultGemm_TensorOpSm80_OperandA<ElementInput, cutlass::layout::RowMajor, kAlignment, kBlcokKSmem>;
  using SmemLayoutAtomA = typename DefaultOperandA::SmemLayoutAtom;
  using SmemCopyAtomA = typename DefaultOperandA::SmemCopyAtom;
  using GmemTiledCopyA = typename DefaultOperandA::GmemTiledCopy;

  // B memory copy operand
  using DefaultOperandB = DefaultGemm_TensorOpSm80_OperandB<ElementWeight, cutlass::layout::ColumnMajor, kAlignment, kBlcokKSmem>;
  ...
  using GmemLayoutAtomO = cute::Layout<cute::Shape<cute::Int<kThreadCount / kGmemTrheadsPerRow>, cute::Int<kGmemTrheadsPerRow>>,
  ...
  using ActivationFn = std::conditional_t<activation_type == Activation_Type::Gelu || activation_type == Activation_Type::Geglu,
