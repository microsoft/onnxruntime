...
struct MoeFCGemm {
 public:
  ...
  struct Arguments {
    ...
    CUTLASS_HOST_DEVICE
    Arguments()
        : problem_count(0), threadblock_count(0), ptr_A(nullptr), ptr_B(nullptr), weight_scales(nullptr), ptr_C(nullptr), ptr_D(nullptr), total_tokens_including_expert(nullptr), gemm_n(0), gemm_k(0), host_problem_sizes(nullptr), C_is_broadcast{true} {
    }
  ...
  struct Params {
    ...
    CUTLASS_HOST_DEVICE
    Params()
        : ptr_A(nullptr), ptr_B(nullptr), weight_scales(nullptr), ptr_C(nullptr), ptr_D(nullptr), C_is_broadcast(true) {
    }
 ...
 public:
  ...
  void run_kernel_(Params const& params, SharedStorage& shared_storage) {
    ...
    while (problem_visitor.next_tile()) {
      ...
      ElementC* ptr_C = reinterpret_cast<ElementC*>(params.ptr_C) + (params.C_is_broadcast ? problem_idx : rows_to_jump) * gemm_n;
