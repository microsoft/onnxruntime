...
void ker_curand_setupLauncher(curandState_t* state,
        ...
        cudaStream_t stream)
    {
        ...
        ker_curand_setup<<<grid, block, 0, stream >>>(state, static_cast<int>(args.batch_size_));
    }
                                        ...
                                        const int batch_size)
{
    ...
    int topk_tmp_ids_buf_size = static_cast<int>(args.batch_size_) * args.candidate_num_; // type int
    int topk_tmp_val_buf_size = static_cast<int>(args.batch_size_) * args.candidate_num_; // type T
    topk_tmp_ids_buf_size = (topk_tmp_ids_buf_size + 3) / 4 * 4; // (int)(ceil(topk_tmp_ids_buf_size / 4.)) * 4;
    topk_tmp_val_buf_size = (topk_tmp_val_buf_size + 3) / 4 * 4; // (int)(ceil(topk_tmp_val_buf_size / 4.)) * 4;
...
#undef ON_RANGE_K
#define ON_RANGE_K(K_MIN, K_MAX, BLOCK_SIZE_1_, BLOCK_SIZE_2_, BLOCKS_PER_BEAM_) \
  if (candidate_num >= K_MIN && candidate_num <= K_MAX) \
  {\
    topk_stage_1_opt3<T, BLOCK_SIZE_1_, BLOCKS_PER_BEAM_><<<batch_size * BLOCKS_PER_BEAM_, BLOCK_SIZE_1_, 0, stream>>>( \
        log_probs, \
        temp_log_probs, \
                                            ...
                                            const int batch_size)
{
    ...
    else
    {
        ...
        return;
        */
        ON_RANGE_K(1,16,128,128,8);
        ON_RANGE_K(17,32,256,128,8);
        ON_RANGE_K(33,64,256,256,8);
        printf("[ERROR] Topk kernel does not support candidate_num = %d \n", candidate_num);
        exit(0);
    }
}
