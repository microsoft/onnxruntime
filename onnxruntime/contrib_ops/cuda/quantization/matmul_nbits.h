  ...
  MatMulNBits(const OpKernelInfo& info) : CudaKernel(info) {
    ...
    if constexpr (std::is_same<T, MLFloat16>::value || std::is_same<T, BFloat16>::value) {
      ...
      if (prepacked) {
        ...
        "This compact fpA_intB build supports prepacked weights for FP16 activations with "
        "INT2 (block_size=64), INT4 or INT8 (block_size=32) weights, or BF16 activations with "
        "INT4 or INT8 (block_size=32) weights; it requires scale-only quantization without zero "
        "points, bias, or g_idx, the SM80 weight layout (weight_prepacked=1), and compute capability "
