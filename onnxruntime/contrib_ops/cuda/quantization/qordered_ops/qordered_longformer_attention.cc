...
QOrderedLongformerAttention::ComputeInternal(OpKernelContext* context) const {
  ...
  // Build Global Index
  auto global_index_buffer = GetScratchBuffer<int>(static_cast<size_t>(batch_size) * static_cast<size_t>(sequence_length), OrtStream(context));
  ...
  auto gemm_buffer = GetScratchBuffer<int8_t>(qkv_3, OrtStream(context));
  ...
  auto workspace_buffer = GetScratchBuffer<void>(workSpaceSize + output_elements * element_size, OrtStream(context));
  ...
  this->AddDeferredReleaseCPUPtr(pinned_buffer.release(), OrtStream(context));
