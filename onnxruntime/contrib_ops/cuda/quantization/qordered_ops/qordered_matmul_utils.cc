...
static Status cublasLtMatMulInt8SetupAlgo(cublasLtHandle_t cublasLt_handle, cublasLtMatmulAlgo_t& algo, int algo_id, int swizzle, int custom_option, int tile, int splitk_val, int reduction_scheme, int stages) {
  CUBLAS_RETURN_IF_ERROR(cublasLtMatmulAlgoInit(cublasLt_handle, CUBLAS_COMPUTE_32I, CUDA_R_32F, CUDA_R_8I, CUDA_R_8I, CUDA_R_8I, CUDA_R_8I, algo_id, &algo));
  ...
  CUBLAS_RETURN_IF_ERROR(cublasLtMatmulAlgoConfigSetAttribute(&algo, CUBLASLT_ALGO_CONFIG_CTA_SWIZZLING, &(swizzle), sizeof(swizzle)));

  CUBLAS_RETURN_IF_ERROR(cublasLtMatmulAlgoConfigSetAttribute(&algo, CUBLASLT_ALGO_CONFIG_REDUCTION_SCHEME, &(reduction_scheme), sizeof(int)));

  CUBLAS_RETURN_IF_ERROR(cublasLtMatmulAlgoConfigSetAttribute(&algo, CUBLASLT_ALGO_CONFIG_STAGES_ID, &(stages), sizeof(stages)));
...
static inline std::string AlgoKey(const cudaDeviceProp& /*device_prop*/,
                                  ...
                                  cublasLtOrder_t input_output_order) {
  std::stringstream ss;
...
void CublasLtMMAlgoMap::GetAlgo(cublasLtHandle_t cublasLt_handle, cublasLtMatmulAlgo_t& algo, const cudaDeviceProp& device_prop, int batch_count, int m, int n, int k, cublasLtOrder_t weight_order, cublasLtOrder_t input_output_order) const {
  ...
  if (algo_it != best_algos_.end() && algo_it->second.workspace_size == 0) {
    ...
    ORT_THROW_IF_ERROR(cublasLtMatMulInt8SetupAlgo(cublasLt_handle, algo, algo_info.algo_id, algo_info.swizzle, algo_info.custom_option, algo_info.tile, algo_info.splitk_val, algo_info.reduction_scheme, algo_info.stages));
  } else {
    // Default algo
    int algo_id = 21;
    int stages = 0;
...
static Status CreateLtMatrixLayout(cublasLtMatrixLayout_t& layout_desc,
                                   ...
                                   int64_t const rows_after_op,
                                   int64_t const cols_after_op,
                                   cudaDataType_t const data_type,
                                   cublasLtOrder_t const mat_order,
                                   cublasOperation_t const mat_trans) {
  if (mat_trans == CUBLAS_OP_T) {
    CUBLAS_RETURN_IF_ERROR(cublasLtMatrixLayoutCreate(&layout_desc, data_type, cols_after_op, rows_after_op, CalcLeadingDimensionLt(cols_after_op, rows_after_op, mat_order)));
  } else {
    CUBLAS_RETURN_IF_ERROR(cublasLtMatrixLayoutCreate(&layout_desc, data_type, rows_after_op, cols_after_op, CalcLeadingDimensionLt(rows_after_op, cols_after_op, mat_order)));
  }
...
Status QOrdered_MatMul(cublasLtHandle_t cublasLt_handle, cudaStream_t stream, const cudaDeviceProp& device_prop, int32_t batch_count, int64_t m, int64_t n, int64_t k, const float* alpha, const int8_t* A, const int8_t* B, int32_t batch_B, const float* bias, const float* beta, const int8_t* C, int32_t batch_C, int8_t* D, cublasLtOrder_t weight_order, cublasLtPointerMode_t pointer_mode) {
  ...
  ORT_RETURN_IF_ERROR(CreateLtMatrixLayout(desc_A, batch_count, k, m, CUDA_R_8I, CUBLASLT_ORDER_COL, CUBLAS_OP_N));  // for A'

  ORT_RETURN_IF_ERROR(CreateLtMatrixLayout(desc_B, batch_B, k, n, CUDA_R_8I, CUBLASLT_ORDER_COL, CUBLAS_OP_N));  // For B

  CUBLAS_RETURN_IF_ERROR(cublasLtMatrixLayoutSetAttribute(desc_B, CUBLASLT_MATRIX_LAYOUT_BATCH_COUNT, &batch_count, sizeof(batch_count)));

  ORT_RETURN_IF_ERROR(CreateLtMatrixLayout(desc_D, batch_count, n, m, CUDA_R_8I, CUBLASLT_ORDER_COL,
                                           CUBLAS_OP_N));  // For D'

  if (C != nullptr) {
    ORT_RETURN_IF_ERROR(CreateLtMatrixLayout(desc_C, batch_C, n, m, CUDA_R_8I, CUBLASLT_ORDER_COL, CUBLAS_OP_N));  // For C'

    CUBLAS_RETURN_IF_ERROR(cublasLtMatrixLayoutSetAttribute(desc_C, CUBLASLT_MATRIX_LAYOUT_BATCH_COUNT, &batch_count, sizeof(batch_count)));
  }

  cublasLtMatmulAlgo_t algo;
  CublasLtMMAlgoMap::Instance().GetAlgo(cublasLt_handle, algo, device_prop, batch_count, static_cast<int>(m), static_cast<int>(n), static_cast<int>(k), weight_order, CUBLASLT_ORDER_ROW);

  CUBLAS_RETURN_IF_ERROR(cublasLtMatmul(cublasLt_handle, matmul_desc, alpha, B, desc_B, A, desc_A, beta, C == nullptr ? D : C, C == nullptr ? desc_D : desc_C, D, desc_D, &algo, nullptr, 0,  // algo, workspace, workspace_size
                                        stream));
  return Status::OK();
}
