...
class CublasLtMMAlgoMap {
 public:
  ...
  void GetAlgo(cublasLtHandle_t cublasLt_handle, cublasLtMatmulAlgo_t& algo, const cudaDeviceProp& device_prop, int batch_count, int m, int n, int k, cublasLtOrder_t weight_order, cublasLtOrder_t input_output_order = CUBLASLT_ORDER_ROW) const;
...
inline Status QOrdered_MatMul(cublasLtHandle_t cublasLt_handle, cudaStream_t stream, const cudaDeviceProp& device_prop, int32_t batch_count, int64_t m, int64_t n, int64_t k, const float* alpha, const int8_t* A, const int8_t* B, const float* bias, int8_t* D, cublasLtOrder_t order_weight, cublasLtPointerMode_t pointer_mode = CUBLASLT_POINTER_MODE_HOST) {
  return QOrdered_MatMul(cublasLt_handle, stream, device_prop, batch_count, m, n, k, alpha, A, B, 1, bias, (const float*)nullptr, nullptr, 1, D, order_weight, pointer_mode);
}
