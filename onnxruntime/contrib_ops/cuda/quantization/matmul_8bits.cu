...
bool TryMatMul8Bits(
...
#ifndef NDEBUG
      printf("Warning n (%d) must be a multiple of %d, and k (%d) must be a multiple of %d for Algo 0/1/2\n", n, kColsPerThreadBlock, k, kElementsPerThreadPerIteration);
#endif
        return false;
    }
...
#ifndef NDEBUG
            printf("Warning k_per_iter (%d) must be a multiple of block_size (%d) for Algo 0\n", k_per_iter, block_size);
#endif
    ...
    // Add check for algo 1 & 2 needing k % block_size == 0 if scales/zp are per block
    if constexpr (kKernelAlgo == 1 || kKernelAlgo == 2) {
        if (k % block_size != 0) {
            ...
            // causing issues with scale/zp indexing depending on exact implementation details.
            // Let's enforce K % block_size == 0 for simplicity/correctness guarantee here.
            // Depending on requirements, could add padding or more complex tail handling.
            // For now, return false.
  #ifndef NDEBUG
            printf("Warning K (%d) must be a multiple of block_size (%d) for Algo 1/2\n", k, block_size);
  #endif
            return false;
        }
      }
