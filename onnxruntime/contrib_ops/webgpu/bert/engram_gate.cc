...
Status EngramGateNormProgram::GenerateShaderCode(ShaderHelper& shader) const {
  ...
  shader.AdditionalImplementation() << "var<workgroup> row_partials: array<f32, " << kGateWorkgroupSize << ">;\n";
  shader.MainFunctionBody()
      << "  let row = workgroup_idx;\n"
      << "  if (row >= uniforms.rows) { return; }\n"
      << "  let g = row % uniforms.hc_mult;\n"
      ...
      << "  var sum_sq = 0.0;\n"
      << "  for (var i = local_idx; i < uniforms.hidden_size; i += " << kGateWorkgroupSize << "u) {\n"
      << "    let value = f32(" << input.GetByOffset("row_base + i") << ");\n"
      << "    sum_sq += value * value;\n"
      << "  }\n"
      ...
      << "  for (var i = local_idx; i < uniforms.hidden_size; i += " << kGateWorkgroupSize << "u) {\n"
      << "    let normalized = f32(" << input.GetByOffset("row_base + i") << ") * inv_rms * f32("
      << norm_scale.GetByOffset("g * uniforms.hidden_size + i") << ");\n"
      << "    " << output.SetByOffset("row_base + i", "output_element_t(normalized)") << "\n"
      << "  }\n";
  return Status::OK();
}
...
Status EngramGate::ComputeInternal(ComputeContext& context) const {
  ...
  const bool qwen_mode = conv_norm_scale != nullptr;
  const TensorShape output_shape = qwen_mode
                                       ? TensorShape({batch_size, sequence_length, hc_mult * hidden_size})
                                       : hidden_shape;
  auto* output = context.Output(0, output_shape);
  auto* normalized_output = qwen_mode ? context.Output(1, output_shape) : nullptr;
  const int64_t total = output_shape.Size();
  if (total == 0) {
    return Status::OK();
  }
  ...
  if (normalized_output != nullptr) {
    ...
    norm_program.AddInputs({{output, ProgramTensorMetadataDependency::Type},
        ...
        .AddOutput({normalized_output, ProgramTensorMetadataDependency::None})
        .SetWorkgroupSize(kGateWorkgroupSize)
        .SetDispatchGroupSize(onnxruntime::narrow<uint32_t>(rows))
        .AddUniformVariables({{onnxruntime::narrow<uint32_t>(rows)},
                              {onnxruntime::narrow<uint32_t>(hc_mult)},
