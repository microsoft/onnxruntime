...
Status TransferBSDToBNSHProgram::GenerateShaderCode(ShaderHelper& shader) const {
  shader.AddInput("qkv_input", ShaderVariable::UseUniform);
  const auto& qkv_output = shader.AddOutput("qkv_output", ShaderVariable::UseUniform |
      ShaderVariable::UseOffsetToIndices);

  if (has_bias_) {
    shader.AddInput("bias", ShaderVariable::UseUniform);
  }
  ...
  shader.SetMainFunctionBody(shader.GuardAgainstOutOfBoundsWorkgroupSizes("uniforms.data_size"),
      ...
      has_bias_ ? " + bias[bias_offset_idx];\n" : ";\n");
...
Status AttentionProbsProgram::GenerateShaderCode(ShaderHelper& shader) const {
  shader.AddInput("q", ShaderVariable::UseUniform | ShaderVariable::UseValueTypeAlias);
  shader.AddInput("key", ShaderVariable::UseUniform | ShaderVariable::UseValueTypeAlias);
  if (feed_past_key_) {
    shader.AddInput("past_key", ShaderVariable::UseUniform);
  }
  if (has_attention_bias_) {
    shader.AddInput("attention_bias", ShaderVariable::UseUniform);
  }

  shader.AddOutput("output", ShaderVariable::UseUniform | ShaderVariable::UseValueTypeAlias);
  if (has_present_key_) {
  ...
  ss << "// x holds the N and y holds the M\n"
  ...
  if (feed_past_key_ && has_present_key_) {
    ss << "let kOffset = uniforms.kv_sequence_length * uniforms.K * headIdx;\n"
       << "let pastKeyOffset = uniforms.past_sequence_length * uniforms.K * headIdx;\n";
  } else {
    ss << "let kOffset = uniforms.N * uniforms.K * headIdx + n * uniforms.K;\n";
  }

  if (has_present_key_) {
    ss << "let presentKeyOffset = headIdx * uniforms.N * uniforms.K;\n";
  }
  ...
  if (feed_past_key_ && has_present_key_) {
    ss << "if (n + local_id.y < uniforms.past_sequence_length) {\n"
    ...
    ss << "tileK[idx] = key[kOffset + local_id.y * uniforms.K + w + local_id.x];\n";
  }

  if (has_present_key_) {
    ss << "present_key[presentKeyOffset + (n + local_id.y) * uniforms.K + w + local_id.x] = tileK[idx];\n";
  }
  ...
  ss << "let headOffset = headIdx * uniforms.M * uniforms.N;\n"
     ...
     << "var sum: f32 = " << (components_ == 4 ? "value.x + value.y + value.z + value.w" :
        (components_ == 2 ? "value.x + value.y" : "value")) << ";\n";

  ss << "output[outputIdx] = output_value_t(sum * uniforms.alpha) + "
     << (has_attention_bias_ ? "attention_bias[outputIdx]" : "0.0") << ";\n"
     << "}\n";
...
Status InPlaceSoftmaxProgram::GenerateShaderCode(ShaderHelper& shader) const {
  shader.AddOutput("x", ShaderVariable::UseUniform | ShaderVariable::UseValueTypeAlias |
  ...
  std::string f32_str = components_ == 4 ? "vec4<f32>" : (components_ == 2 ? "vec2<f32>" : "f32");
  std::ostringstream ss;
  ss << "let local_offset = local_idx * uniforms.elements_per_thread;\n"
     ...
     << "}\n"
     << "thread_max[local_idx] = " << (components_ == 4 ?
        "max(max(thread_max_vector.x, thread_max_vector.y), max(thread_max_vector.z, thread_max_vector.w))" :
        (components_ == 2 ? "max(thread_max_vector.x, thread_max_vector.y)" : "thread_max_vector")) << ";\n"
     << "workgroupBarrier();\n"
     ...
     << "}\n"
     << "thread_sum[local_idx] = " << (components_ == 4 ? "sum_vector.x + sum_vector.y + sum_vector.z + sum_vector.w" :
        (components_ == 2 ? "sum_vector.x + sum_vector.y" : "sum_vector")) << ";\n"
     << "workgroupBarrier();\n"
     << "var sum: f32 = 0;\n"
     << "for (var i = 0u; i < " << work_group_size_ << "; i++) {\n"
     << "sum += thread_sum[i]\n;"
...
Status VxAttentionScoreProgram::GenerateShaderCode(ShaderHelper& shader) const {
  shader.AddInput("probs", ShaderVariable::UseUniform | ShaderVariable::UseValueTypeAlias |
                           ShaderVariable::UseElementTypeAlias);
  shader.AddInput("v", ShaderVariable::UseUniform | ShaderVariable::UseValueTypeAlias);
  if (feed_past_value_) {
    shader.AddInput("past_value", ShaderVariable::UseUniform);
  }
  ...
  std::ostringstream ss;
  ss << "let headIdx = workgroup_id.z;\n"
     << "let m = global_id.y;\n"
     << "let n = global_id.x;\n"
     << "let offsetA = headIdx * (uniforms.M * uniforms.K) + m * uniforms.K;\n";

  if (feed_past_value_ && has_present_value_) {
    ss << "let pastValueOffset = headIdx * uniforms.N * uniforms.past_sequence_length + n;\n"
       << "let vOffset = headIdx * uniforms.N * uniforms.kv_sequence_length + n;\n";
  } else {
    ss << "let offsetB = headIdx * uniforms.N * uniforms.K + n;\n";
  }

  if (has_present_value_) {
    ss << "let presentValueOffset = headIdx * uniforms.N * uniforms.K + n;\n";
  }
  ...
  if (feed_past_value_ && has_present_value_) {
    ss << "if (w + local_id.y < uniforms.past_sequence_length) {\n"
       << "tileK[idx] = past_value[pastValueOffset + (w + local_id.y) * uniforms.N];\n"
       << "} else {\n"
       << "tileK[idx] = v[vOffset + (w + local_id.y - uniforms.past_sequence_length) * uniforms.N];\n"
       << "}\n";
  } else {
    ss << "tileK[idx] = v[offsetB + (w + local_id.y) * uniforms.N];\n";
  }

  if (has_present_value_) {
    ss << "present_value[presentValueOffset + (w + local_id.y) * uniforms.N] = tileK[idx];\n";
  }
...
Status ApplyAttention(const Tensor* Q, const Tensor* K, const Tensor* V, const Tensor* attention_bias,
  ...
  const int output_count = std::min({context.OutputCount(), 1 + (past_key != nullptr ? 1 : 0)
                                     + (past_value !=nullptr ? 1 : 0)});
