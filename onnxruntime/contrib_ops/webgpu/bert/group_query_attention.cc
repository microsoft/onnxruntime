...
Status GroupQueryAttention::ComputeInternal(onnxruntime::webgpu::ComputeContext& context) const {
  ...
  if (CanApplyFlashAttention(/*bias*/ nullptr, present_key, present_value, parameters, context)) {
    return ApplyFlashAttention(query, key, value, /*attention_bias*/ nullptr, output, past_key, present_key, past_value,
