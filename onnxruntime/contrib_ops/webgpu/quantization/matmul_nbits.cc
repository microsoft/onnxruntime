...
#if !defined(ORT_MINIMAL_BUILD) && !defined(ORT_USE_EP_API_ADAPTERS)
Status MatMulNBits::DeclareWorkspaceRequirements(
  ...
  requirements.clear();
  const auto* a_shape = GetWorkspaceInputShape(input_shapes, 0).GetShape();
  if (a_shape == nullptr || a_shape->Size() < 0) {
    return Status::OK();
  }
