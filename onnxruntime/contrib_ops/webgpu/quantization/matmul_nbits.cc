...
Status MatMulNBits::ComputeInternal(onnxruntime::webgpu::ComputeContext& context) const {
  ...
  const bool use_subgroup = context.Device().HasFeature(wgpu::FeatureName::Subgroups) && context.AdapterInfo().vendor == std::string_view{"intel"} && context.AdapterInfo().architecture == std::string_view{"gen-12lp"} && components_a == 4 && block_size == 32;
