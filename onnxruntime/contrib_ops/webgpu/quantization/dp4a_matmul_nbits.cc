...
Status ApplyDP4AMatrixMatMulNBits(const Tensor* a, const Tensor* b, const Tensor* scales,
  ...
  // When per_row_weight_indirect is enabled (fused MoE), force SmallM path since
  // only SmallM supports per-row expert selection. M is small (k=4) in this case.
  if (per_row_weight_indirect || M < min_M_for_tile_optimization) {
    ...
    DP4AMatMulNBitsSmallMProgram mul_program{tile_size_k_vec, tile_size_n, nbits, has_zero_points, has_bias, has_weight_idx, has_weight_idx_indirect, single_scale_weights, per_row_weight_indirect};
    ...
    mul_program.AddInputs({{&a_quant, ProgramTensorMetadataDependency::TypeAndRank, static_cast<int>(kVec4Components)},
        ...
        .CacheHint(nbits, tile_size_k_vec, tile_size_n, has_zero_points, single_scale_weights, has_bias, has_weight_idx, has_weight_idx_indirect, per_row_weight_indirect);
    if (has_zero_points) {
