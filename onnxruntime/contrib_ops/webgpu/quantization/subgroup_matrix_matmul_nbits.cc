...
Status SubgroupMatrixMatMulNBitsProgram::GenerateShaderCode(ShaderHelper& shader) const {
    ...
    var<workgroup> scratch: array<array<array<f16, 64>, 4>, 4>;  // 64 * 4 * 4
    ...
    // Write out
    // Also, handle cases where M is not divisible by 8. That is for the last row
    // tile only some rows need to be written out. Only one, either the top block
    // belonging to matA0 or the bottom block belonging to matA1 needs the special
    // write out. That is if the top block is partial the bottom block does not
    // need to be written out at all.
    // Use the scratch space to write out the matrices, and then unfortunately a
    // barrier is required before write out to output. That is even though the matrix
    // is completely within a subgroup, a workgroup barrier is required to ensure
    // fullwrite out to workgroup memory.
    // We only only use workgroup or storage memory at this point with
    // subgroupMatrixStore. row_limit is the count of number of rows to be written
    // out for either of the blocks needing special treatment.
    ...
    {
        row_limit = uniforms.M - (a_global_base + base_A);
        // Write out top block
    ...
    {
        matrix_c_offset = matrix_c_offset + 8 * uniforms.N;
        row_limit = uniforms.M - (a_global_base + base_A + 8);
    ...
    if (row_limit != 0)
    {
        ...
        if (row < row_limit)
        {
            output[matrix_c_offset + row * uniforms.N + col] = scratch[subtile_id][0][row * 8 + col];
            output[matrix_c_offset + row * uniforms.N + col + 8] = scratch[subtile_id][1][row * 8 + col];
            output[matrix_c_offset + row * uniforms.N + col + 16] = scratch[subtile_id][2][row * 8 + col];
            output[matrix_c_offset + row * uniforms.N + col + 24] = scratch[subtile_id][3][row * 8 + col];
            col++;
            output[matrix_c_offset + row * uniforms.N + col] = scratch[subtile_id][0][row * 8 + col];
            output[matrix_c_offset + row * uniforms.N + col + 8] = scratch[subtile_id][1][row * 8 + col];
            output[matrix_c_offset + row * uniforms.N + col + 16] = scratch[subtile_id][2][row * 8 + col];
