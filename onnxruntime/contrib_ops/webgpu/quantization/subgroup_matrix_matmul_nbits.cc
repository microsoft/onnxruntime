...
bool CanApplySubgroupMatrixMatMulNBits(onnxruntime::webgpu::ComputeContext& context,
  ...
  bool supports_subgroup_matrix = context.HasFeature(wgpu::FeatureName::ChromiumExperimentalSubgroupMatrix);
  if (supports_subgroup_matrix) {
    // Check if the adapter reports a subgroup matrix config we support.
    supports_subgroup_matrix = IsSubgroupMatrixConfigSupported(context, is_fp16, config_index);
    if (supports_subgroup_matrix) {
      if (context.AdapterInfo().vendor == std::string_view{"apple"}) {
        ...
        // FP32 is around 7s.
        supports_subgroup_matrix = accuracy_level == 4;
      }
    }
  }
