...
if platform.system() == 'Linux':
  libs = ['onnxruntime_pybind11_state.so', 'libmkldnn.so.0', 'libmklml_intel.so', 'libiomp5.so', 'mimalloc.so', 'libonnxruntime_pywrapper.so']
  # nGraph Libs
  ...
  libs = ['onnxruntime_pybind11_state.so', 'libmkldnn.0.dylib', 'mimalloc.so', 'onnxruntime_pywrapper.so'] # TODO add libmklml and libiomp5 later.
else:
  libs = ['onnxruntime_pybind11_state.pyd', 'mkldnn.dll', 'mklml.dll', 'libiomp5md.dll', 'onnxruntime_pywrapper.dll']
...
if is_manylinux2010:
    ...
    ext_modules = [
        Extension(
            ...
            ['onnxruntime/capi/onnxruntime_pybind11_state_manylinux2010.so', 'onnxruntime/capi/libonnxruntime_pywrapper_manylinux2010.so'],
        ),
    ]
else:
