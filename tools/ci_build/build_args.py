...
def add_cmake_build_config_args(parser):
    ...
    parser.add_argument(
        "--cmake_generator",
        choices=[
            "MinGW Makefiles", "Ninja", "NMake Makefiles", "NMake Makefiles JOM",
            "Unix Makefiles", "Visual Studio 17 2022", "Xcode",
        ],
...
def add_testing_args(parser):
    ...
    parser.add_argument(
        "--skip_onnx_tests", action="store_true", help="Explicitly disable ONNX related tests."
    )
    parser.add_argument(
        "--skip_winml_tests", action="store_true", help="Explicitly disable WinML related tests."
    )
    parser.add_argument(
        "--skip_nodejs_tests", action="store_true", help="Explicitly disable Node.js binding tests."
    )
    parser.add_argument("--test_all_timeout", default="10800", help="Timeout for onnxruntime_test_all (seconds).")
    parser.add_argument(
        "--enable_transformers_tool_test", action="store_true", help="Enable transformers tool test."
    )
...
def add_android_args(parser):
    ...
    parser.add_argument("--android_run_emulator", action="store_true", help="Start an Android emulator if needed for tests.")
...
def add_apple_args(parser):
    ...
    parser.add_argument(
        "--apple_sysroot", default="", help="Specify the macOS platform SDK location name."
    )
    parser.add_argument(
        "--ios_toolchain_file",
        default="",
    ...
    parser.add_argument(
        "--xcode_code_signing_team_id", default="", help="Development team ID for Xcode code signing."
    )
    parser.add_argument(
...
def add_webassembly_args(parser):
    ...
    parser.add_argument(
        "--disable_wasm_exception_catching", action="store_true", help="Disable exception catching."
    )
    parser.add_argument(
        "--enable_wasm_api_exception_catching", action="store_true", help="Catch exceptions only at top-level API calls."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--enable_wasm_debug_info", action="store_true", help="Build WASM with DWARF debug info."
    )
...
def add_windows_specific_args(parser):
    ...
    # Build tools / config
    parser.add_argument(
        "--msvc_toolset", help="MSVC toolset version (e.g., 14.11). Avoid [14.36, 14.39]."
    )
    parser.add_argument("--windows_sdk_version", help="Windows SDK version (e.g., 10.0.19041.0).")
    parser.add_argument(
        "--enable_msvc_static_runtime", action="store_true", help="Statically link MSVC runtimes."
    )
...
def add_execution_provider_args(parser):
    ...
    cuda_group.add_argument("--enable_nvtx_profile", action="store_true", help="Enable NVTX profile markers for CUDA EP.")
    cuda_group.add_argument(
    ...
    dnnl_group.add_argument(
        ...
        choices=["ocl", ""], help="DNNL GPU backend (e.g., ocl for OpenCL)."
    )
    dnnl_group.add_argument(
        "--dnnl_opencl_root", action="store", default="", help="Path to OpenCL SDK (for DNNL GPU)."
    )
    dnnl_group.add_argument(
        "--dnnl_aarch64_runtime", action="store", default="", type=str.lower,
        choices=["acl", ""], help="DNNL AArch64 backend (e.g., acl for Arm Compute Library)."
    )
    dnnl_group.add_argument(
    ...
    trt_group.add_argument(
        "--use_tensorrt_builtin_parser", action="store_true", default=True, help="Use TensorRT internal ONNX parser (default)."
    )
    ...
    acl_group.add_argument("--no_kleidiai", action="store_true", help="Disable KleidiAI integration (used with ACL/ArmNN).")
    ...
    webgpu_group.add_argument("--use_external_dawn", action="store_true", help="Use external Dawn dependency for WebGPU.")
...
def add_other_feature_args(parser):
    ...
    parser.add_argument(
        "--enable_lazy_tensor", action="store_true", help="Enable ORT backend for PyTorch LazyTensor."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--use_telemetry", action="store_true", help="Enable telemetry (official builds only)."
    )
    parser.add_argument("--use_triton_kernel", action="store_true", help="Use Triton compiled kernels (requires Triton).")
...
def parse_arguments():
    ...
    is_cross_compiling = any([
        ...
        args.android, args.ios, args.visionos, args.tvos, args.build_wasm, args.use_gdk
    ])
    ...
    if not (args.update or args.build or args.test or args.clean):
        ...
        if not is_cross_compiling and not args.skip_tests:
            args.test = True
             ...
             print("Cross-compiling build detected: Defaulting to --update --build. Specify --test explicitly to run tests.")
