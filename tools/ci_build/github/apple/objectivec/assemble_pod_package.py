...
def main():
    ...
    def as_variable_value(patterns: list[str]):
        return ", ".join([f'"{pattern}"' for pattern in patterns])

    variable_substitutions = {
        "VERSION": args.pod_version,
        "SOURCE_HTTP": args.pod_source_http,
        "LICENSE_FILE": as_variable_value([license_file]),
        "INCLUDE_DIR_LIST": as_variable_value(include_dirs),
        "PUBLIC_HEADER_FILE_LIST": as_variable_value(public_header_files),
        "SOURCE_FILE_LIST": as_variable_value(source_files),
        "TEST_SOURCE_FILE_LIST": as_variable_value(test_source_files),
        "TEST_RESOURCE_FILE_LIST": as_variable_value(test_resource_files),
    }
