...
# Copy CUDA libraries.
Copy-Item -Path "$Env:BUILD_BINARIESDIRECTORY\RelWithDebInfo\nuget-artifacts\onnxruntime-win-x64-cuda-*\lib\*" -Destination "$Env:BUILD_BINARIESDIRECTORY\RelWithDebInfo"
...
$ort_dirs = Get-ChildItem -Path "$Env:BUILD_BINARIESDIRECTORY\RelWithDebInfo\nuget-artifacts\onnxruntime-*" -Directory
