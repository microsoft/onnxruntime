...
then
    ...
    cp $BINARY_DIR/$ARTIFACT_NAME/jar/$NATIVE_FOLDER/$LIB_NAME $BINARY_DIR/$ARTIFACT_NAME/libonnxruntime.dylib
    # JNI Lib
    ...
    cp $BINARY_DIR/$ARTIFACT_NAME/jar/$NATIVE_FOLDER/$NATIVE_LIB_NAME $BINARY_DIR/$ARTIFACT_NAME/libonnxruntime4j_jni.dylib
elif [[ $LIB_NAME == *.so.* ]]
then
