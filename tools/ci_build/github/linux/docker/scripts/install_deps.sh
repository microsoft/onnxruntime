...
if [[ $SYS_LONG_BIT = "64" && "$GLIBC_VERSION" -gt "9" ]]; then
  ...
  GetFile https://github.com/Kitware/CMake/releases/download/v3.16.5/cmake-3.16.5-Linux-x86_64.tar.gz /tmp/src/cmake-3.16.5-Linux-x86_64.tar.gz
  tar -zxf /tmp/src/cmake-3.16.5-Linux-x86_64.tar.gz --strip=1 -C /usr
else
  echo "Installing cmake"
  GetFile https://github.com/Kitware/CMake/releases/download/v3.16.5/cmake-3.16.5-Linux-x86_64.tar.gz /tmp/src/cmake-3.16.5.tar.gz
  tar -xf /tmp/src/cmake-3.16.5.tar.gz -C /tmp/src
  pushd .
  cd /tmp/src/cmake-3.16.5
...
if [ $DEVICE_TYPE = "Normal" ]; then
    ...
    if [[ $BUILD_EXTR_PAR = *--enable_training* ]]; then
      ...
      echo "import torch, torchvision passed;"
    fi
fi
