...
${PYTHON_EXE} -m pip install --upgrade --force-reinstall numpy==1.15.0
${PYTHON_EXE} -m pip install --upgrade --force-reinstall requests==2.21.0
if [ $DEVICE_TYPE = "Normal" ]; then
    ${PYTHON_EXE} -m pip install --upgrade --force-reinstall sympy==1.1.1
fi
${PYTHON_EXE} -m pip install --upgrade scipy pytest
