...
if [ $DEVICE_TYPE = "gpu" ]; then
    if [[ $BUILD_EXTR_PAR = *--enable_training* ]]; then
      ...
      if [[ $BUILD_EXTR_PAR = *--enable_training_python_frontend_e2e_tests* || $BUILD_EXTR_PAR = *enable_training_pipeline_e2e_tests* || $BUILD_EXTR_PAR = *--enable_training_multi_gpu_tests* ]]; then
