...
if [ $BUILD_OS = "android" ]; then
    ...
    if [ $BUILD_DEVICE = "gpu" ]; then
        ...
        $DOCKER_CMD --pull -t "onnxruntime-$IMAGE" --build-arg BUILD_USER=onnxruntimedev --build-arg BUILD_UID=$(id -u) --build-arg PYTHON_VERSION=${PYTHON_VER} --build-arg OPENVINO_VERSION=${OPENVINO_VERSION} -f $DOCKER_FILE .
    else
        IMAGE="$BUILD_OS"
