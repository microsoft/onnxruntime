...
docker run --rm \
    ...
    /bin/bash -c "
        ...
        --skip_tests \
        --disable_ml_ops \
        --disable_types double \
        --cmake_extra_defines onnxruntime_DISABLE_GENERATION_OPS=ON onnxruntime_USE_FP8_KV_CACHE=OFF onnxruntime_DISABLE_FLOAT4_TYPES=ON onnxruntime_USE_FPA_INTB_GEMM=OFF onnxruntime_DISABLE_STRING_TYPE=ON onnxruntime_USE_TRT_FUSED_ATTENTION=OFF \
