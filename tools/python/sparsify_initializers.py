...
import argparse
import numpy as np
from typing import Text, Sequence, Any, Callable, Tuple, List
import onnx
import os
import sys
import logging
from datetime import datetime
from onnx import ModelProto, GraphProto, SparseTensorProto, TensorProto, numpy_helper
...
def parse_arguments():
    ...
    parser.add_argument('--exclude', required=False, type=str, help='semicolon separated list of initializer names to exclude')
    parser.add_argument('--tolerance', required=False, type=float, default=1e-6, help='FP tolerance. If not given simple compare to 0')
    parser.add_argument('--sparsity_threshold', required=False, 
                        type=float, default=0.5, help='convert to sparse initializers if sparsity is at least this much')
...
def convert_tensor_to_sparse(tensor, tolerance):  # type: (TensorProto) -> Tuple[SparseTensorProto, float]
    ...
    logger.debug(f"initializer={tensor.name}, dtype={tensor_data.dtype}, len={data_len}, nnz={nnz_count}, sparsity={sparsity}")
...
def convert_initializers(model, exclude_names, sparsity_threshold, tolerance):  # type: (ModelProto, List[str], float) -> None
    graph = model.graph
    converted_sparse = []
