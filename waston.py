...
activities = [
    ...
    ('gemm', lambda x : any(substr in x for substr in ['volta', 'cutlass']) and 'gemm' in x),
    ('gemv', lambda x : any(substr in x for substr in ['cublasGemvTensorStridedBatched'])),
    ('memcpy DtoD', lambda x : any(substr in x for substr in ['Memcpy DtoD'])),
    ('memcpy HtoD', lambda x : any(substr in x for substr in ['Memcpy HtoD'])),
    ('memcpy DtoH', lambda x : any(substr in x for substr in ['Memcpy DtoH'])),
    ('memset', lambda x : any(substr in x for substr in ['memset [async]'])),
    ('adam', lambda x : x.lower().find('adam') >= 0),
    ('lamb', lambda x : x.lower().find('lamb') >= 0),
    ('dropout', lambda x : x.lower().find('dropout') >= 0),
    ('gelu', lambda x : 'Gelu' in x),
    ...
    ('compare', lambda x : any(substr in x for substr in ['at::native::CompareGEFunctor', 'CompareNEFunctor','CompareEqFunctor', 'CompareLTFunctor', 'OP_GreaterOrEqual', 'OP_Equal', 'OP_Less'])),
    ('isfinite', lambda x : 'foreach_non_finite_check_and_unscale' in x),
    ('layernorm', lambda x : any(substr in x for substr in ['LayerNorm', 'layer_norm', 'bn_fw_tr_1C11_singleread', 'cuCompute'])),
    ...
    ('embedding', lambda x : any(substr in x for substr in ['indexSelectLargeIndex', 'Embedding'])),
