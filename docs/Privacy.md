...
- **Disable non-essential events via the API.** The C API (and the C#, Python, and Java bindings) can suppress non-essential telemetry. On every telemetry-enabled platform, the initialization event may already have been emitted before the API can be called. On **Windows**, ETW events are recorded only when an external trace session is collecting.
