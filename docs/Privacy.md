...
The Web provider (`onnxruntime-web`) uses the [1DS SDK](https://www.npmjs.com/package/@microsoft/1ds-core-js) to send telemetry events from the browser to Microsoft's OneCollector endpoint. No personally identifiable information (PII) is collected. Telemetry can be disabled at runtime by setting `ort.env.telemetry.enabled = false`, or excluded at build time by removing the `onnxruntime_USE_TELEMETRY` CMake flag. An observer callback (`ort.env.telemetry.onEvent`) is available for inspecting telemetry events.
