...
Because the first-pass tags are tentative, ONNX Runtime performs complete second-pass support discovery without budget gating, rebuilds the surviving first-pass reservations so shared initializers are charged exactly once, and then performs final budget admission. Only final assignments remain committed. This keeps the accumulated memory estimate accurate when `prefer_nhwc` is combined with `session.resource_cuda_partitioning_settings`, prevents dropped nodes from consuming phantom budget, and prevents later survivors from being reclaimed at zero cost after an early budget stop.
