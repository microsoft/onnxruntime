...
Because the first-pass tags are tentative, ONNX Runtime provisionally reserves their costs while evaluating the second pass. It then rolls back dropped first-pass nodes and, if provisional budget stopped admission, retries final capability admission so pass-two-only candidates are evaluated against actual survivors. Only final assignments remain committed. This keeps the accumulated memory estimate accurate when `prefer_nhwc` is combined with `session.resource_cuda_partitioning_settings`, and prevents a dropped node from consuming phantom budget that blocks a later node.
