...
* Python
    ...
    for i in range(test_num):
        x = np.array(x_base[: i + 1][:], dtype=np.float32)
        y = np.array(y_base[: i + 1][:], dtype=np.float32)
    ...
    for i in range(test_num):
        ...
        expected_y = np.array(expected_y_base[: i + 1][:], dtype=np.float32)
    ...
    for i in range(test_num):
        ...
        x_ortvalues[i].update_inplace(np.array(x_base_mul_10[: i + 1][:], dtype=np.float32))
        if i > 0:
            ro.add_run_config_entry("gpu_graph_id", str(i))
        io_bindings[i].synchronize_inputs()
        session.run_with_iobinding(io_bindings[i], ro)
        io_bindings[i].synchronize_outputs()
        expected_y = np.array(expected_y_base_mul_10[: i + 1][:], dtype=np.float32)
        np.testing.assert_allclose(expected_y, y_ortvalues[i].numpy(), rtol=1e-05, atol=1e-05)
    ```
* C/C++
    ```c++
