...
* Description: control if sparsity can be used by TRT.
  * Check `--sparsity` in `trtexec` command-line flags for [details](https://github.com/NVIDIA/TensorRT/blob/v10.9.0/samples/common/sampleOptions.cpp#L2547-L2554).
...
* Description: set the builder optimization level.
  ...
  > WARNING: levels below 3 do not guarantee good engine performance, but greatly improve build time.  Default 3, valid range [0-5]. Check `--builderOptimizationLevel` in `trtexec` command-line flags for [details](https://github.com/NVIDIA/TensorRT/blob/v10.9.0/samples/common/sampleOptions.cpp#L2625).
...
* Description: set maximum number of auxiliary streams per inference stream.
  ...
  * Check `--maxAuxStreams` in `trtexec` command-line flags for [details](https://github.com/NVIDIA/TensorRT/blob/v10.9.0/samples/common/sampleOptions.cpp#L2651-L2652).
...
* `ORT_TENSORRT_SPARSITY_ENABLE`: Control if sparsity can be used by TRT. Default 0 = false, 1 = true. Check `--sparsity` in `trtexec` command-line flags for [details](https://github.com/NVIDIA/TensorRT/blob/v10.9.0/samples/common/sampleOptions.cpp#L2547-L2554).

* `ORT_TENSORRT_BUILDER_OPTIMIZATION_LEVEL`: Set the builder optimization level. WARNING: levels below 3 do not guarantee good engine performance, but greatly improve build time.  Default 3, valid range [0-5]. Check `--builderOptimizationLevel` in `trtexec` command-line flags for [details](https://github.com/NVIDIA/TensorRT/blob/v10.9.0/samples/common/sampleOptions.cpp#L2625).

* `ORT_TENSORRT_AUXILIARY_STREAMS`: Set maximum number of auxiliary streams per inference stream. Setting this value to 0 will lead to optimal memory usage. Default -1 = heuristics. Check `--maxAuxStreams` in `trtexec` command-line flags for [details](https://github.com/NVIDIA/TensorRT/blob/v10.9.0/samples/common/sampleOptions.cpp#L2651-L2652).
