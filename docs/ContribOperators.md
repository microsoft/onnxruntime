...
<dd>Optional global fp32 activation scale (scalar). When present, A is statically quantized to FP8 E4M3 with this scale (W8A8 numerics). The default path dequantizes A back before the matmul; the opt-in SM90 DeepGEMM path uses native FP8 multiplication. When absent, A stays in full FP16/BF16 precision.</dd>
<dt><tt>bias</tt> (optional) : T</dt>
<dd>Optional bias of shape [N].</dd>
</dl>
