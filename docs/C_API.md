...
chooses to override this by setting ```session_state.use_env_allocators``` to "0".
      ...
      * Configuring *OrtArenaCfg*:
         ...
         * ```arena_extend_strategy```: This can take only 2 values currently: kSameAsRequested or kNextPowerOfTwo. As the name suggests kNextPowerOfTwo (the default) extends the arena by a power of 2 which can be much more than the allocation request. This has the potential to increase the overall memory consumption if a large portion of it remains unused.
