...
def ort_training_session_run_helper(session, iobinding, inputs, input_descs, output_descs, device, run_options = None):
    ...
    for output_desc in output_descs_resolved:
        ...
        iobinding.bind_output(output_desc.name_, torch_tensor.device.type, device_index, dtype_torch_to_numpy(torch_tensor.dtype), 
...
def create_ort_training_session_bind_parameters(model, device):
    ...
    for param in torch_params.keys():
        ...
        train_io_binding.bind_input(param, torch_tensor.device.type, device_index, 
            dtype_torch_to_numpy(torch_params[param].dtype), list(torch_tensor.size()), torch_tensor.data_ptr())
    ...
    def train_step(self, input, label, fetches = None):            
        ...
        if self.session is None:
            ...
            session_run_results = ort_training_session_run_helper(self.session, self.train_io_binding, (input, label), \
                self.model_desc_.inputs_, self.model_desc_.outputs_, self.device_)
    ...
    def eval(self, input, fetches = None):
        if self.session is None:
            ...
            session_run_results = ort_training_session_run_helper(self.session, self.eval_io_binding, (input), \
                self.model_desc_.inputs_[:-1], self.model_desc_.outputs_[1:], self.device_, run_options)
...
class ORTModel():
    ...
    def __call__(self, *inputs):
        return ort_training_session_run_helper(self.session_, self.train_io_binding, inputs, self.model_desc_.inputs_, self.model_desc_.outputs_, self.device_).values()
