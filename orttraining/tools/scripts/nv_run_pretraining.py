            ...
            for f_id in range(f_start_id + 1 , len(files)):
                ...
                for step, batch in enumerate(train_iter):
                    ...
                    if not is_model_exported:
                      onnx_path = os.path.join(args.output_dir, 'bert_for_pretraining_without_loss_' + config.to_string() + '.onnx')
                      lm_score, sq_score = model(input_ids=input_ids, token_type_ids=segment_ids, attention_mask=input_mask)
                      torch.onnx.export(model, (input_ids, segment_ids, input_mask), onnx_path,
                                        verbose = True,
                                        #input_names = ['input_ids', 'token_type_ids', 'input_mask'],
                                        input_names = ['input1', 'input2', 'input3'],
                                        output_names = ['output1', 'output2'],
                                        dynamic_axes={'input1': {0: 'batch'}, 'input2': {0: 'batch'}, 'input3': {0: 'batch'}, 'output1': {0: 'batch'}, 'output2': {0: 'batch'}},
                                        training=True)
                      is_model_exported = False
