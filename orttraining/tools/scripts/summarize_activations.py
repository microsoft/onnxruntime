...
parser.add_argument('--ort_dir', type=str)
...
def generate_summary_per_step(o_path, d_path, merge_dir_name):
    ...
    merge_dir = os.path.join(d_path, merge_dir_name)
    ...
    for sub_dir_name in os.listdir(d_path):
        ...
        if os.path.isdir(sub_dir_full_path):
            merge_filename_for_sub_dir = os.path.join(merge_dir, f"{sub_dir_name}_.txt")
    ...
    print(f"Finish generating summary per step for {d_path} following typological order in {o_path}, "
