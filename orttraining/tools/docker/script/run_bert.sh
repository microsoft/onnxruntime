if [ "$#" -ne 9 ]; then
  echo "Usage: $0 ngpu batch_size seq_len num_train_steps optimizer model_size training_mode[fp32|fp16] display_loss_steps gradient_accumulation_steps"
  exit 1
fi
...
training_bert_dir=${ORT_DIR}/build/Release
