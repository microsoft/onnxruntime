...
void TorchProxy::RunInputTensorOnCpuFunction(
  ...
  PyObject* input_on_cpu_func = reinterpret_cast<PyObject*>(input_on_cpu_function);
  ORT_ENFORCE(PyCallable_Check(input_on_cpu_func), "input_on_cpu_func is not callable.");

  // All arguments created for Python call will be destroyed along with PythonObjectPtr.
  PythonObjectPtr args(Ort_PyTuple_New(1, "input_on_cpu_func_arguments_tuple"), PythonObjectDeleter);
  PyObject* node_proto_ptr_arg = PyBytes_FromStringAndSize(node_proto_str.c_str(), node_proto_str.size());
  Ort_PyTuple_SetItem_NoIncref(args.get(), 0, node_proto_ptr_arg, "node_proto_ptr_arg");

  PythonObjectPtr result_ptr(PyObject_CallObject(input_on_cpu_func, args.get()), PythonObjectDeleter);
  if (PyErr_Occurred()) {
    PyErr_Print();
    ORT_THROW("Python function execution fails with the above information.");
  }
  ...
  for (Py_ssize_t tuple_index = 0; tuple_index < ret_tuple_size; ++tuple_index) {
    ...
    std::vector<int64_t>& output_to_input_alias_map =
        tuple_index == 0 ? fw_input_tensor_on_cpu_map : bw_output_to_input_alias_map;
