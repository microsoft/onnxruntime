...
std::vector<NodeArg*> create_gather_internal_output_node_args(const std::vector<NodeArg*>& gather_inputs,
  ...
  gather_internal_outputs.insert(gather_internal_outputs.end(), {&graph.GetOrCreateNodeArg(graph.GenerateNodeArgName(NumSegmentsNodeArgName), &num_segments_type_proto),
