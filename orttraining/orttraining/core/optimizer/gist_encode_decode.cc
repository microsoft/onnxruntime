...
bool GistEncodeDecode::AddEncodeDecode(Graph& graph, Node& curr_node, std::string compression_type) const {
  if (curr_node.GetOutputEdgesCount() < 2 ){ // min 2 required for gist applicability - one to fw pass, one to bw pass
    return false;
  }
  ...
  for (auto& st_act : decode_map) {
    ...
    auto& encode = graph.AddNode(encode_node_name, compression_type + "Encoder", "Encode", {curr_node_output_arg}, {&encode_output_def_compressed_arg});
    ...
    int curr_dec_priority = graph.GenerateDecodePriority();
    assert(curr_dec_priority > 0);
    auto& decode = graph.AddNode(decode_node_name, compression_type + "Decoder", "Decode", {&encode_output_def_compressed_arg}, {&decode_output_def_uncompressed_arg});
    decode.SetPriority(curr_dec_priority);

    // Connect decode node to destionation nodes/edges
...
Status GistEncodeDecode::Apply(Graph& graph, Node& node, RewriteRuleEffect& rule_effect, const logging::Logger& ) const {
  if (node.Description() != "Backward pass") {
    if (GistEncodeDecode::AddEncodeDecode(graph, node, compression_type)) {
      std::cout << "Gist applied to node name - " << node.Name() << ", node type - " << node.OpType() << ", of compr type - " << compression_type << std::endl;
