...
Status PythonOpRewriter::Apply(Graph&, Node& node, RewriteRuleEffect& rule_effect, const logging::Logger&) const {
  ...
  if (graph_utils::IsSupportedOptypeVersionAndDomain(node, "PythonOp", {1}, kMSDomain)) {
    if (node.GetAttributes().find("tensor_reuse_map") == node.GetAttributes().end()) {
    ...
    if (node.GetAttributes().find("input_tensor_on_cpu") == node.GetAttributes().end()) {
      auto func_name = static_cast<std::string>(node.GetAttributes().at("func_name").s());

      std::optional<PyObject*> input_tensor_on_cpu_function =
          language_interop_ops::torch::OrtTorchFunctionPool::GetInstance().TryGettingInputTensorOnCpuFunction(func_name);
      if (input_tensor_on_cpu_function.has_value()) {
        // Serialize node proto to string
        ONNX_NAMESPACE::NodeProto node_proto;
        node.ToProto(node_proto);
        std::string node_proto_str;
        ...
        std::vector<int64_t> fw_all_output_to_tensor_input_reuse_map;
        std::vector<int64_t> bw_all_output_to_tensor_input_reuse_map;
        language_interop_ops::torch::TorchProxy::GetInstance().RunInputTensorOnCpuFunction(
            static_cast<void*>(input_tensor_on_cpu_function.value()),
            node_proto_str,
            fw_all_output_to_tensor_input_reuse_map,
            bw_all_output_to_tensor_input_reuse_map);
        ...
        {
          // Handle forward input alias map.
          node.AddAttribute("input_tensor_on_cpu", fw_all_output_to_tensor_input_reuse_map);
          std::cout << "set input_tensor_on_cpu for node : " << node.Name() << std::endl;
        }
