 ...
 * 1. Some ops generate in-deterministic results (for example using random number generator). We need evaluate whether
 *   this is a problem for recompute before adding the support, instead of fixing this after we find and try to
 *   fix convergence issues (which will be very hard if we have multiple in-deterministic operators by default supported.)
