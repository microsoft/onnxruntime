...
Status MemoryOptimizer::ApplyImpl(Graph& graph, bool& modified, int /*graph_level*/, const logging::Logger& logger)
  ...
  // Note1: Iterate through the nodes in reversed topological order and find the subgraph that can be alleviated.
  // Note2: The reason we do reversed topological order is that we want the later layers' recompute nodes can be appended
  // earlier than the earlier layers, in this way, the execution order of later layers will be in front of the earlier
  // layers.
  // Note3: Use default order which tries to BFS from the outputs, so the nearest node to graph output will be visited first.
...
Status MemoryOptimizer::CreateRecomputeGraph(Graph& graph,
  ...
  for (size_t i = 0; i < nodes_in_topological_order.size(); ++i) {
    ...
    std::cout << "~~Adding reompute node " << recompute_node.Name() << " with op type " << recompute_node.OpType() << " node index: " << recompute_node.Index() << std::endl;
