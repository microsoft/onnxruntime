...
void Conv1dToMatmul(Graph& graph, Node& conv, const std::string transformer_name) {
  ...
  if (onnx_opset_version > 12) {
    ...
    onnxruntime::utils::SetRawDataInTensorProto(initializer_proto, initializer_proto_value.data(), initializer_proto_value.size() * sizeof(int64_t));
    auto& axes_input = graph_utils::AddInitializerWithOrtValue(graph, initializer_proto);
