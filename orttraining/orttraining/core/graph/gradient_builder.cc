...
IMPLEMENT_GRADIENT_BUILDER(GetPythonOpGradient) {
  ...
  // input_types[i] store the type of autograd.Function.apply's ith output. 
  ...
  attrs.push_back(MakeAttribute("input_types", input_types));

  // output_types[i] stores the type of autograd.Function.apply's ith input.
  std::vector<int64_t> output_types;
  for (const auto output_type : src_attrs["input_types"].ints()) {
    output_types.push_back(output_type);
  }
  attrs.push_back(MakeAttribute("output_types", output_types));
