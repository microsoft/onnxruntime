...
IMPLEMENT_GRADIENT_BUILDER(GetPythonOpGradient) {
  ...
  for (const auto input_type : src_attrs["output_tensor_types"].ints()) {
    input_tensor_types.push_back(input_type);
  }
  attrs.push_back(MakeAttribute("output_tensor_types", input_tensor_types));

  // output_tensor_types[i] stores the type of autograd.Function.apply's ith input.
  // We assume tensor and its gradient have the same type.
  ...
  attrs.push_back(MakeAttribute("input_tensor_types", output_tensor_types));

  std::vector<ArgDef> input_args;
  // Python context generated by PythonOp.
