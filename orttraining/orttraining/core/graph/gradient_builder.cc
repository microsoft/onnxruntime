...
IMPLEMENT_GRADIENT_BUILDER(GetPythonOpGradient) {
  std::cout << "[gradient_builder.cc, GetPythonOpGradient] start" << std::endl;
  std::vector<NodeDef> result;
  auto src_attrs = SrcNodeAttributes();
  std::vector<AttributeProto> attrs;
  std::cout << "[gradient_builder.cc, GetPythonOpGradient] 1" << std::endl;
  ORT_ENFORCE(utils::HasString(src_attrs.at("name")));
  attrs.push_back(MakeAttribute("name", src_attrs.at("name").s()));
  // attrs.push_back(MakeAttribute("module", src_attrs.at("module").s()));
  // attrs.push_back(MakeAttribute("compute", "backward"));

  std::cout << "[gradient_builder.cc, GetPythonOpGradient] 2" << std::endl;
  std::vector<int64_t> input_types;
  // Context.
  input_types.push_back(0);
  for (const auto input_type : src_attrs["output_types"].ints()) {
    // TODO: consider if it's differentiable.
    input_types.push_back(input_type);
  }
  std::cout << "[gradient_builder.cc, GetPythonOpGradient] 3" << std::endl;
  std::vector<int64_t> output_types;
  for (const auto output_type : src_attrs["input_types"].ints()) {
    output_types.push_back(output_type);
  }

  std::cout << "[gradient_builder.cc, GetPythonOpGradient] 4" << std::endl;
  std::vector<ArgDef> input_args;
  // Context.
  ...
  std::cout << "[gradient_builder.cc, GetPythonOpGradient] 5" << std::endl;
  std::vector<ArgDef> output_args;
  for (int i = 0; i < GetSrcNodeInputSize(); ++i) {
    output_args.push_back(GI(i));
  }

  std::cout << "[gradient_builder.cc, GetPythonOpGradient] 6" << std::endl;
  result.push_back(NodeDef(OpDef{"PythonOpGrad", kMSDomain, 1},
            input_args,
            output_args, attrs));
      
  std::cout << "[gradient_builder.cc, GetPythonOpGradient] end" << std::endl;
  return result;
}
