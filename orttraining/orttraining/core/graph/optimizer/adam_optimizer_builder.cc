...
Status AdamOptimizerBuilder::Build(
  ...
  for (size_t i = 0; i < weight_argdefs.size(); ++i) {
    ...
    const std::string update_count_name = weight_name + "_Step";  // per weight optimizer requires a per weight update count
    TensorProto uc_tensor_proto = CreateTensorProto<int64_t>(update_count_name, 1);
    ...
    input_args.push_back(ArgDef(update_count_name));
    ...
    output_args.push_back(ArgDef(update_count_name + "_Out", step_type_proto));
    ...
    for (const auto& moments_suffix : moments_suffixes) {
      ...
      input_args.push_back(ArgDef(moment_name, moment_type_proto));
      output_args.push_back(ArgDef(moment_name + "_Out", moment_type_proto));
    }
