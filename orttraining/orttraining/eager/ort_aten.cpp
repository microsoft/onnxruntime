...
bool equal(
  ...
  auto* ort_tensor_self = ort_input_self.GetMutable<onnxruntime::Tensor>();
  auto& shape_self = ort_tensor_self->Shape();
  auto* ort_tensor_other = ort_input_other.GetMutable<onnxruntime::Tensor>();
  auto& shape_other = ort_tensor_other->Shape();

  if (shape_self != shape_other) return false;

  // do element wise equal
