...
template <typename T> inline std::ostream &operator<<(std::ostream &out, const ORTLogHelper<T> &helper) {
  out << helper.GetValue();
  return out;
}
...
template <typename T> inline std::ostream &operator<<(std::ostream &out, const ORTLogHelper<c10::optional<T>> helper) {
