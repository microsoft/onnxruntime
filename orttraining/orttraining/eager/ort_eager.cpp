...
static at::ScalarType aten_scalar_type_from_ort(onnxruntime::MLDataType dtype) {
...
OrtValue ORTTensor_toORTValue(const at::Tensor &data) { return torch_ort::eager::create_ort_value(data); }
...
at::Tensor OrtValue_To_ATen_Tensor(OrtValue &ortvalue) {
  ...
  return torch_ort::eager::aten_tensor_from_ort(
      std::move(ortvalue), at::TensorOptions()
...
void addObjectMethodsForEager(py::module &m) {
  ...
  m.def(
      "device",
      [](int device_index) {
        return py::cast<py::object>(THPDevice_New(at::Device(at::DeviceType::ORT, device_index)));
      },
      py::arg("device_index") = 0);

  m.def("aten_ort_tensor_to_ort_value", [](at::Tensor data) { return ORTTensor_toORTValue(data); });
  m.def("to_aten_ort_device_tensor", [](OrtValue &ortvalue) { return OrtValue_To_ATen_Tensor(ortvalue); });

  m.def("set_device", [](size_t device_index, const std::string &provider_type,
  ...
  m.def("get_ort_device",
        [](size_t torch_device_index) { return GetORTBackendsManager().GetOrtDeviceInfo(torch_device_index); });
  m.def("get_ort_device_provider_info",
