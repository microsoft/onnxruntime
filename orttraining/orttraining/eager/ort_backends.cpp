...
onnxruntime::ORTInvoker &GetORTInvoker(const at::Device device) { return GetORTBackendsManager().GetInvoker(device); }
...
onnxruntime::Status ORTBackendsManager::set_device(size_t device_index, const std::string &provider_type,
  ...
  auto ep = onnxruntime::python::GetOrCreateExecutionProvider(
      provider_type, ProviderOptionsMap{{provider_type, provider_options}}, SessionOptions{});

  auto invoker = std::make_unique<onnxruntime::ORTInvoker>(std::move(ep), logger_, custom_op_schema_);
