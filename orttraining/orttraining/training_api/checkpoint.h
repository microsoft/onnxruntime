...
namespace api_test {
// todo: replace those once implemented.
struct Parameter;
...
struct TypedCheckpointProperty {
 public:
  static_assert(std::is_base_of<int64_t, T>::value ||
                    std::is_base_of<float, T>::value ||
                    std::is_base_of<std::string, T>::value,
                "T must be one of int64_t, float or std::string");

  TypedCheckpointProperty(const std::string& prop_name, const T& prop_value)
      : CheckpointProperty(prop_name), prop_value_(prop_value) {
  }

  TypedCheckpointProperty(const ONNX_NAMESPACE::TensorProto& t);
...
struct CheckpointStates {
 ...
 public:
  ...
  std::unordered_map<std::string, std::shared_ptr<api_test::OptimizerState>> grouped_optimizer_states;
