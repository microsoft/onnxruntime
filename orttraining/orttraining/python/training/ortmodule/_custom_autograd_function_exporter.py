...
def custom_autograd_function_exporter(g, n, *args, **kwargs):
    print('[_custom_autograd_function_exporter.py]')
    name = kwargs['name']
    ...
    for i, arg, call_type in zip(range(len(args)), args, cconv):
        if call_type == 'd':
            ...
            scalar_type = symbolic_helper.cast_pytorch_to_onnx[arg.type(
            ).scalarType()]
