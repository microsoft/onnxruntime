...
def _process_inplace_outputs(
    ...
    2. Validate the detected inplace_map with the registered inplace_map in ORT. For the output tensor,
        ...
        2.1 If the reuse mapping value is NOT equal in both inplace_map and detected inplace_map:
            ...
            2.1.3 Other cases (for example user gives a wrong mapping index compared with detected ones), raise an error.
    ...
    if is_backward is False and (is_first_time_init or kernel_info.tensor_input_indices_to_save_in_ctx):
        for raw_tensor_input_index, raw_input_tensor in raw_input_tensors_used_inplace.items():
            ...
            # We did not do the check with tensor_input_indices_to_save_in_ctx because even for those tensor indices
            # not in tensor_input_indices_to_save_in_ctx, we still need to do the copy for the first-time run.
