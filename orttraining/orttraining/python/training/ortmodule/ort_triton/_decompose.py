...
class DecomposeDispatch(object):
    ...
    def LayerNormalization(self, node: NodeProto, graph: GraphProto, **kwargs):
        ...
        if dtype == TensorProto.FLOAT16 or dtype == TensorProto.BFLOAT16:
            ...
            cast_node3, _ = self._new_node(node_name, "Cast", [layer_norm_node_outputs[1]], outputs=[y], to=dtype)
            return [cast_node, cast_node1, cast_node2, layer_norm_node_outputs[0], cast_node3]
        rank = len(shape)
        if axis < 0:
            axis += rank
        axes = list(range(axis, rank))
        epsilon_tensor = helper.make_tensor(name="epsilon_const", data_type=dtype, dims=(1,), vals=np.array([epsilon]))
    ...
    def LayerNormalizationGrad(self, node: NodeProto, graph: GraphProto, **kwargs):
        ...
        if dtype == TensorProto.FLOAT16 or dtype == TensorProto.BFLOAT16:
            ...
            cast_node1, x = self._new_node(node_name, "Cast", [x], to=TensorProto.FLOAT)
            ...
            cast_node3, _ = self._new_node(node_name, "Cast", [layer_norm_grad_node_outputs[1]], outputs=[dx], to=dtype)
            decomposed_nodes = [cast_node, cast_node1, cast_node2, layer_norm_grad_node_outputs[0], cast_node3]
            if dw is not None:
                cast_node4, _ = self._new_node(
                    node_name, "Cast", [layer_norm_grad_node_outputs[2]], outputs=[dw], to=dtype
                )
                decomposed_nodes.append(cast_node4)
            if db is not None:
                cast_node5, _ = self._new_node(
                    node_name, "Cast", [layer_norm_grad_node_outputs[3]], outputs=[db], to=dtype
                )
