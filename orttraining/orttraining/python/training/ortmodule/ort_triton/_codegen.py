...
from ._common import CodegenContext, NodeVisitor
from ._ir import (
    ComputeNode,
    DropoutNode,
...
class TritonCodegen(NodeVisitor):
    ...
    def ElementwiseKernelNode(self, node: ElementwiseKernelNode, context: CodegenContext, indent: int) -> str:
        ...
        if node.has_dropout:
            ...
            src_code += f"{space_indent}t_seed_cuda = tl.broadcast_to(t_seed_cuda, [XBLOCK])\n"
    ...
    def ReduceKernelNode(self, node: ReduceKernelNode, context: CodegenContext, indent: int) -> str:
        ...
        if node.has_dropout:
            src_code += f"{space_indent}t_seed_cuda = tl.load(seed_cuda)\n"
            src_code += f"{space_indent}t_seed_cuda = tl.broadcast_to(t_seed_cuda, [XBLOCK, RBLOCK])\n"

        if not offset_calc.autotune_configs.requires_for_loop:
            src_code += f"{space_indent}rindex = rbase\n"
            if offset_calc.requires_r_mask:
