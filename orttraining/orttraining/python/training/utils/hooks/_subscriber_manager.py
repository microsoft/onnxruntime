...
def no_increase_global_step():
    ...
    The ORT session run will start with 0, aligned with PyTorch run.
    """
    try:
...
class _IncrementStep(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, run_ctx: _RuntimeStates, *input_tensor_list: Tuple[torch.Tensor, ...]) -> Tuple[torch.Tensor, ...]:
        """
...
class SubscriberManager:
    ...
    def _initialize(self, module: torch.nn.Module):
        ...
        # the same tensor). In this way, we keep ORT and PyTorch run to have the same boundary to check activation
        # equality.
        next_module_index = [0]
    ...
    def _reset_global_states(self, module: torch.nn.Module):
        def _reset_recursively(module: torch.nn.Module, depth: int, next_module_index: List[int]):
            ...
            module.id = module_index  # needed by DeepSpeed
    ...
    def _register_hooks_recursively(self, module: torch.nn.Module, depth: int, next_module_index: List[int]):
        ...
        # "with_kwargs" is not available for lower version of PyTorch.
