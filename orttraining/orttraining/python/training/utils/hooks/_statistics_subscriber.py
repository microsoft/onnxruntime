...
class _InspectActivation(torch.autograd.Function):
    ...
    def forward(
        ...
        """
        print("In _InspectActivation.forward() step: ", run_ctx.global_states)
        depth = -1
...
class StatisticsSubscriber(SubscriberBase):
    ...
    def post_forward_tensor_apply(
        ...
        print("post_forward_tensor_apply step: ", run_rtx.global_states)
        ...
        if name not in run_rtx.execution_step_states.observed_activation_names:
            run_rtx.execution_step_states.observed_activation_names[name] = True
    ...
    def module_post_forward_impl(self, activation: torch.Tensor, depth: int, name: str, step: int):
        print("module_post_forward_impl step: ", step)
