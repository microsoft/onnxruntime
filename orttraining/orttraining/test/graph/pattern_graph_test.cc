...
Status TryReplace(Graph& graph, PatternGraph& pattern, const PGraphNode& alternative,
  ...
  PatternMatchResult match_results;
  ORT_RETURN_IF_ERROR(pattern.TryMatch(graph, match_results));
  InlinedVector<std::reference_wrapper<Node>> matched_nodes;
  matched_nodes.push_back(*match_results.GetNodeByName(start_node_name));
  ORT_ENFORCE(match_results.GetNodesWithCondition(
                               matched_nodes, [&](std::string const& node_name, MatchedNodeGroup&) -> bool {
                                 return node_name != start_node_name && node_name != end_node_name;
                               })
                  .IsOK());

  matched_nodes.push_back(*match_results.GetNodeByName(end_node_name));

  auto add_node_args_with_name = [&match_results](std::string name, int idx, std::vector<NodeArg*>& args) {
    auto target_node = match_results.GetNodeByName(name);
  ...
  const std::unordered_map<std::string, std::vector<int>>& domain_version_map = alternative.GetDomainVersionMap();
  std::string node_domain = kOnnxDomain;
  if (!domain_version_map.empty()) {
    node_domain = domain_version_map.begin()->first;
  }

  Node& replace_node = graph.AddNode(alternative.GetNodeName(), alternative.GetOpType(), "",
                                     input_args, output_args, {}, node_domain);
...
TEST(GraphParser, SimpleMatch1) {
  ...
  class CustomNodeCompareFunc : public NodeCompareFunc {
   public:
    bool operator()(const Graph&, const Node* target_node,
                    const PatternGraph&, const PGraphNode* pattern_node) const override {
      return pattern_node->NameEquals(target_node->Name());
    }
  };
  ...
  PatternGraph pattern(
      ...
      {PGraphNode("Shape", {"X"}, {"Y1"}, "p_shape"),
       ...
       PGraphNode("Unsqueeze", {"Y2"}, {"Y3"}, "p_unsqueeze", {{kOnnxDomain, {1, 11}}},
                  {ONNX_NAMESPACE::MakeAttribute("axes", std::vector<int64_t>{static_cast<int64_t>(0)})}),
       PGraphNode("Concat", {"Y3"}, {"Y4"}, "p_concat",
                  {{kOnnxDomain, {1, 4, 11}}}, {ONNX_NAMESPACE::MakeAttribute("axis", int64_t(0))}),
...
TEST(GraphParser, LayerNormFusionTest) {
  ...
  PGraphInputTypes float_type(TypesCategory::AllFloatTensorTypes);
  PGraphInputTypes double_type({ONNX_NAMESPACE::TensorProto_DataType::TensorProto_DataType_DOUBLE});
  ...
  PatternGraph pattern3(
      {PGraphInput("C0", double_type),
       PGraphInput("C1", double_type),
       PGraphInput("C2", double_type),
       PGraphInput("C3", double_type),
       PGraphInput("X", float_type)},
      {PGraphNode("ReduceMean", {"X"}, {"Y"}, "p_rm1"),
       PGraphNode("Sub", {"X", "Y"}, {"Sub1"}, "p_sub1"),
       ...
       PGraphNode("Add", {"C1", "Z"}, {"Add1"}, "p_add1"),
       PGraphNode("Sqrt", {"Add1"}, {"Sqrt"}, "p_sqrt"),
       PGraphNode("Div", {"Cast", "Sqrt"}, {"Div"}, "p_div"),
       PGraphNode("Mul", {"Div", "C2"}, {"Mul"}, "p_mul"),
       PGraphNode("Add", {"Mul", "C3"}, {"Final"}, "p_final")},
      "layer_norm_pattern3");
  ...
  PatternGraph pattern4(
      ...
      {PGraphNode("ReduceMean", {"X"}, {"Y"}, "p_rm1"),
       PGraphNode("Sub", {"X", "Y"}, {"Sub1"}, "p_sub1"),
       PGraphNode("Cast", {"C0"}, {"Cast"}, "p_cast", {}, {ONNX_NAMESPACE::MakeAttribute("to", int64_t{float_type.GetDefaultType()})}),
...
TEST(GraphParser, NoopEliminationTest) {
  ...
  PGraphInputTypes all_int_and_float_type(TypesCategory::AllIntegerAndFloatTensorTypes);
  PatternGraph pattern1(
      {PGraphInput("C0", all_int_and_float_type),
       PGraphInput("X", all_int_and_float_type)},
      {PGraphNode("Add", {"X", "C0"}, {"Y"}, "p_add", {}, {}, 0, true /*ignore_input_arg_orders*/)});

  class CustomNodeCompareFunc : public NodeCompareFunc {
   public:
    bool operator()(const Graph& target, const Node* g,
                    const PatternGraph&, const PGraphNode*) const override {
  ...
  while (match) {
    ...
    InlinedVector<std::reference_wrapper<Node>> removal_nodes;
    ORT_ENFORCE(res.GetNodesWithCondition(
                       removal_nodes, [&](std::string const&, MatchedNodeGroup& node_group) -> bool {
                         return graph_utils::CanRemoveNode(graph, *node_group.matched_node, logging::LoggingManager::DefaultLogger());
                       })
                    .IsOK());

    for (auto& gnode : removal_nodes) {
...
//   PGraphInputShapes X_shape({2});
//   PGraphInputShapes indices_shape({0});
//   PatternGraph pattern(
//       {PGraphInput("X", float_type, X_shape),
//        PGraphInput("indice0", integer_type, indices_shape),
//        PGraphInput("indice1", integer_type, indices_shape),
//        PGraphInput("C3", integer_type),
//        PGraphInput("C4", integer_type)},
//       {PGraphNode("Shape", {"X"}, {"Shape1"}, "p_shape1"),
//        PGraphNode("Gather", {"Shape1", "indice0"}, {"Gather1"}, "p_gather1"),
...
//        PGraphNode("Gather", {"Shape2", "indice1"}, {"Gather2"}, "p_gather2"),
...
//    public:
//     bool operator()(const Graph& target, const Node* g,
//                     const PatternGraph&, const PGraphNode* p) const override {
...
//   auto x = res.GetNodeByName("p_reshape")->MutableInputDefs()[0];
//   std::vector<NodeArg*> input_args({x});
//   InlinedVector<int64_t> shape_value;
//   auto concat_node = res.GetNodeByName("p_concat");
...
//   nodes_to_remove.push_back(*res.GetNodeByName("p_shape1"));

//   ORT_ENFORCE(res.GetNodesWithCondition(
//                      nodes_to_remove, [&](std::string const& node_name, MatchedNodeGroup&) -> bool {
//                        return (node_name != "p_shape1" && node_name != "p_reshape");
//                      })
//                   .IsOK());

//   nodes_to_remove.push_back(*res.GetNodeByName("p_reshape"));
...
TEST(GraphParser, GemmActivationFusionTest1) {
  ...
  PGraphInputTypes float_type(TypesCategory::AllFloatTensorTypes);
  PGraphInputShapes shapes({2});
  PatternGraph pattern(
      {PGraphInput("X", float_type, shapes),
       PGraphInput("C0", float_type, shapes),
       PGraphInput("C1", float_type, shapes)},
  ...
  class CustomNodeCompareFunc : public NodeCompareFunc {
   public:
    bool operator()(const Graph&, const Node* target_node,
                    const PatternGraph&, const PGraphNode*) const override {
      return IsFusableActivation(*target_node);
    }
  };
  ...
  // replace
  ASSERT_EQ(TryReplace(graph, pattern, PGraphNode("FusedGemm", {}, {}, "replaced_node"),
                       {{"p_gemm", 0}, {"p_gemm", 1}, {"p_gemm", 2}}, {}, "p_gemm", "p_active")
                .ToString(),
            "OK");
...
TEST(GraphParser, GemmActivationFusionTest2) {
  ...
  // replace
  ASSERT_EQ(TryReplace(graph, pattern, PGraphNode("FusedGemm", {}, {}, "replaced_node", {{"com.microsoft", {1}}}),
                       {{"p_gemm", 0}, {"p_gemm", 1}, {"p_gemm", 2}}, {}, "p_gemm", "p_active")
                .ToString(),
            "OK");
...
//   PGraphInputTypes float_type(TypesCategory::AllFloatTensorTypes);
//   PGraphInputTypes integer_type(TypesCategory::AllIntegerTensorTypes);
//   PGraphInputShapes rank_2_shapes({2});
//   PatternGraph pattern(
//       {PGraphInput("X", float_type, rank_2_shapes),
//        PGraphInput("C0", integer_type, rank_2_shapes),
//        PGraphInput("C1", integer_type, rank_2_shapes),
//        PGraphInput("C2", integer_type, rank_2_shapes),
//        PGraphInput("C3", float_type),
...
//   auto layer_norm_node = res.GetNodeByName("p_layernorm");
//   auto layer_norm_add_node = res.GetNodeByName("p_add2");
//   auto segment_gather_node = res.GetNodeByName("p_gather3");
//   NodeArg* segment_embedding = segment_gather_node->MutableInputDefs()[0];
//   auto add_node = res.GetNodeByName("p_add1");
//   auto word_gather_node = res.GetNodeByName("p_gather1");
...
//   } else {
//     auto position_gather_node = res.GetNodeByName("p_gather2");
...
//     bool operator()(const Graph&, const Node* target_node,
//                     const PatternGraph&, const PGraphNode*) const override {
//       return target_node->OpType() == "ConstantOfShape";
//     }
//   };
...
//   auto layer_norm_node = res.GetNodeByName("p_layernorm");
//   auto layer_norm_add_node = res.GetNodeByName("p_add2");
//   auto segment_gather_node = res.GetNodeByName("p_gather4");
//   NodeArg* segment_embedding = segment_gather_node->MutableInputDefs()[0];
//   auto add_node = res.GetNodeByName("p_add1");
//   auto word_gather_node = res.GetNodeByName("p_gather3");
//   NodeArg* word_embedding = word_gather_node->MutableInputDefs()[0];
//   NodeArg* input_ids = word_gather_node->MutableInputDefs()[1];
//   NodeArg* segment_ids = segment_gather_node->MutableInputDefs()[1];
//   auto input_shape = input_ids->Shape();
...
//   PGraphInputShapes shapes({3});
//   PatternGraph pattern(
//       {PGraphInput("X", PGraphInputTypes(TypesCategory::AllFloatTensorTypes), shapes),
...
//        PGraphNode("Concat", {"Unsqueeze2", "Unsqueeze3"}, {"Concat"}, "p_concat", {},
//                   {ONNX_NAMESPACE::MakeAttribute("axis", int64_t(0))}, -1),
...
//        PGraphNode("Attention", {"ReduceSum", "LayerNorm", "C1"}, {"Attention"}, "p_attention", {{"com.microsoft", {1}}},
//                   {ONNX_NAMESPACE::MakeAttribute("num_heads", int64_t{1})}, -1)});

//   class CustomNodeCompareFunc : public NodeCompareFunc {
//    public:
//     bool operator()(const Graph&, const Node* target_node,
//                     const PatternGraph&, const PGraphNode*) const override {
//       return target_node->OpType() == "Unsqueeze";
//     }
//   };
