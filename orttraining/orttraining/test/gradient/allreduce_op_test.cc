...
TEST(AllreduceTest, GPUHierarchicalAdasumAllreduceOptimizerTest) {
  ...
  CreateMLValue<float>(std::make_shared<CPUAllocator>(), {1}, {eta}, &ml_value_eta);
  feeds.insert(std::make_pair(lr_string, ml_value_eta));

  // Grad scale inputs
  OrtValue ml_value_scale;
  CreateMLValue<float>(std::make_shared<CPUAllocator>(), {1}, {scale}, &ml_value_scale);
  feeds.insert(std::make_pair(grad_divisor_string, ml_value_scale));

  // Update count inputs
  OrtValue ml_value_uc;
  CreateMLValue<int64_t>(std::make_shared<CPUAllocator>(), {1}, {update_count}, &ml_value_uc);
  feeds.insert(std::make_pair(update_count_string, ml_value_uc));

  // Gradient inputs
  OrtValue ml_value_input_t;
  CreateMLValue<float>(std::make_shared<CPUAllocator>(), dims_allreduce_input, values_allreduce_input, &ml_value_input_t);
