  ...
  try {
    ...
    if (execution_providers) {
      ...
      fetches_ = ExecuteModel<onnxruntime::training::TrainingSession>(
          *p_model, session_object, expect_result, expected_failure_string, run_options, feeds, output_names, provider_types);
      ...
      for (const std::string& provider_type : all_provider_types) {
        ...
        fetches_ = ExecuteModel<onnxruntime::training::TrainingSession>(*p_model, session_object, expect_result, expected_failure_string, run_options, feeds, output_names, provider_type);
      }
    }
