...
def run_with_pytorch_on_device(device, model, input_list, label_input, is_eval_mode=False):
    ...
    forward_outputs = []
    grad_outputs = []
    mem_stat("before going into loop")
    for i in range(100):
        if i == 1:
            mem_stat("1 step, before clean up")
            torch.cuda.reset_peak_memory_stats()
            mem_stat("1 step, after clean up")
        mem_stat("====== {} bnefore forward pass ======".format(i))
        output = model(*inputs_on_device)
        mem_stat("====== {} after forward pass ======".format(i))
        #forward_outputs = [output]
        #grad_outputs = []
        if not is_eval_mode:
            criterion = torch.nn.MSELoss()
            loss = criterion(output, target)
            mem_stat("====== {} after loss  ======".format(i))
            loss.backward()
            mem_stat("====== {} after backward pass ======".format(i))
...
def run_with_ort_on_device(device, model, input_list, label_input, is_eval_mode=False):
    ...
    model = ORTModule(model, debug_options=DebugOptions(save_onnx=True, onnx_prefix='my_model',log_level=LogLevel.VERBOSE))
    if is_eval_mode:
        model.eval()
    else:
        model.train()
    ...
    for i in range(100):
        ...
        if not is_eval_mode:
            criterion = torch.nn.MSELoss()
            loss = criterion(output, target)
            # torch_interop_utils.clear_grad_fns_for_next_edges(loss)
            mem_stat("====== {} after loss  ======".format(i))
            loss.backward()
            mem_stat("====== {} after backward pass ======".format(i))
        #for name, param in model.named_parameters():
...
def compare_tensor_list(val_list_a, val_list_b):
    for val_a, val_b in zip(val_list_a, val_list_b):
       assert_values_are_close(val_a, val_b, atol=1e-6, rtol=1e-5)
