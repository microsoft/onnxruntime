...
def create_orttrainer_and_load_checkpoint(
    device, trainer_opts, checkpoint_dir, use_lamb=True, seed=1, learning_rate=0.1
):
    ...
    trainer = orttrainer.ORTTrainer(
        model, model_desc, optim_config, loss_fn=loss_fn, options=orttrainer.ORTTrainerOptions(trainer_opts)
    )
...
def verify_model_state(trainer, expected_state_dict, is_mixedprecision):
    ...
    assert len(expected_state_dict["fp32_param"]) == len(
        actual_model_state["full_precision"]
    ...
    for weight_name, tensor in expected_state_dict["fp32_param"].items():
        if not weight_name in actual_model_state["full_precision"]:
            assert "_view_" in weight_name, "only zero shared weight may not match name"
    ...
    if is_mixedprecision:
        ...
        assert len(expected_state_dict["fp16_param"]) == len(
            actual_model_state["mixed_precision"]
        ), "expected and actual should have same number of tensors"
        ...
        for info, value in weight_info.items():
            ...
            if info == "original_dim":
                if is_zero_run:
                    ...
                    if is_mixedprecision:
                        assert_array_equal(
                            part_info[weight_name]["original_dim"],
                            expected_state_dict["fp16_param"][weight_name + "_fp16"].shape,
                        )
                    else:
                        assert_array_equal(
                            part_info[weight_name]["original_dim"], expected_state_dict["fp32_param"][weight_name].shape
                        )
...
def create_orttrainer_and_save_checkpoint(
    device, trainer_opts, checkpoint_dir, state_dict_key_name="state_dict", use_lamb=True, seed=1, learning_rate=0.1
):
    torch.manual_seed(seed)
    set_seed(seed)
    ...
    if "distributed" in trainer_opts:
        train_data = next(
            islice(
                _chunkify(train_data, trainer_opts["distributed"]["world_size"]),
