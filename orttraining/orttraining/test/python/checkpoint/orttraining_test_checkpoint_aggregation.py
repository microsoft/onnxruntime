...
def test_zero_aggregation(checkpoint_dir, loaded_state_dict, is_mixedprecision):
    ...
    aggregate_state_dict_from_checkpoint = checkpoint.aggregate_checkpoints(
        glob.glob(os.path.join(checkpoint_dir, "checkpoint*.ortcp")), pytorch_format=False
    )
    ...
    assert_all_states_close_ort(
        aggregate_state_dict_from_test, aggregate_state_dict_from_checkpoint, reshape_states=True
    )
...
def test_aggregation_from_distributed_zero_full_precision_adam(
    device="cuda", checkpoint_dir="checkpoint_dir/distributed_zero/full_precision/adam/"
):
...
def test_aggregation_from_distributed_zero_mixed_precision_adam(
    device="cuda", checkpoint_dir="checkpoint_dir/distributed_zero/mixed_precision/adam/"
):
...
def test_aggregation_from_distributed_zero_full_precision_lamb(
    device="cuda", checkpoint_dir="checkpoint_dir/distributed_zero/full_precision/lamb/"
):
    opts = {"device": {"id": device}, "debug": {"deterministic_compute": True}}
...
def test_aggregation_from_distributed_zero_mixed_precision_lamb(
    device="cuda", checkpoint_dir="checkpoint_dir/distributed_zero/mixed_precision/lamb/"
):
    opts = {"device": {"id": device}, "mixed_precision": {"enabled": True}, "debug": {"deterministic_compute": True}}
...
def test_aggregation_from_distributed_megatron_full_precision_adam(
    device="cuda", checkpoint_dir="checkpoint_dir/distributed_megatron/full_precision/adam/"
):
    opts = {"device": {"id": device}, "debug": {"deterministic_compute": True}}
...
def test_aggregation_from_distributed_megatron_mixed_precision_adam(
    device="cuda", checkpoint_dir="checkpoint_dir/distributed_megatron/mixed_precision/adam/"
):
    opts = {"device": {"id": device}, "mixed_precision": {"enabled": True}, "debug": {"deterministic_compute": True}}
...
def test_aggregation_from_distributed_megatron_full_precision_lamb(
    device="cuda", checkpoint_dir="checkpoint_dir/distributed_megatron/full_precision/lamb/"
):
    opts = {"device": {"id": device}, "debug": {"deterministic_compute": True}}
...
def test_aggregation_from_distributed_megatron_mixed_precision_lamb(
    device="cuda", checkpoint_dir="checkpoint_dir/distributed_megatron/mixed_precision/lamb/"
):
    opts = {"device": {"id": device}, "mixed_precision": {"enabled": True}, "debug": {"deterministic_compute": True}}
...
def test_aggregation_from_distributed_zero_megatron_full_precision_adam(
    device="cuda", checkpoint_dir="checkpoint_dir/distributed_zero_megatron/full_precision/adam/"
):
    opts = {"device": {"id": device}, "debug": {"deterministic_compute": True}}
...
def test_aggregation_from_distributed_zero_megatron_mixed_precision_adam(
    device="cuda", checkpoint_dir="checkpoint_dir/distributed_zero_megatron/mixed_precision/adam/"
):
    opts = {"device": {"id": device}, "mixed_precision": {"enabled": True}, "debug": {"deterministic_compute": True}}
...
def test_aggregation_from_distributed_zero_megatron_full_precision_lamb(
    device="cuda", checkpoint_dir="checkpoint_dir/distributed_zero_megatron/full_precision/lamb/"
):
    opts = {"device": {"id": device}, "debug": {"deterministic_compute": True}}
...
def test_aggregation_from_distributed_zero_megatron_mixed_precision_lamb(
    device="cuda", checkpoint_dir="checkpoint_dir/distributed_zero_megatron/mixed_precision/lamb/"
):
    opts = {"device": {"id": device}, "mixed_precision": {"enabled": True}, "debug": {"deterministic_compute": True}}
