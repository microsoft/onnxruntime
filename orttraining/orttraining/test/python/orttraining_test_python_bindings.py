...
from orttraining.orttraining.python.training.engine import TrainingModule
...
def create_ort_vectors_data(self):
...
def test_train_step_training_module(self):
    ...
    with tempfile.TemporaryDirectory() as checkpoint_dir_name:
        ...
        model = TrainingModule(checkpoint_file_path, model_file_path)

        forward_inputs, forward_output = self.create_ort_vectors_data()

        model.train(forward_inputs, forward_output)
        print("Loss : ",forward_output[0].numpy())
