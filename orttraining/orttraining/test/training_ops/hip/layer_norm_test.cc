...
static void TestLayerNormGrad(
  ...
  test.CompareWithCPU(kHipExecutionProvider, error_tolerance);
}

TEST(HipKernelTest, LayerNormGrad_SmallSizeTensor) {
  const std::vector<int64_t> X_dims{4, 20, 128};
  TestLayerNormGrad(X_dims);
}

TEST(HipKernelTest, LayerNormGrad_SmallSizeTensor_IntermediateAxis) {
  const std::vector<int64_t> X_dims{4, 20, 16, 8};
  const int64_t axis = -2;
  TestLayerNormGrad(X_dims, axis);
}

TEST(HipKernelTest, LayerNormGrad_MidSizeTensor) {
  const std::vector<int64_t> X_dims{8, 80, 768};
  TestLayerNormGrad(X_dims);
}

TEST(HipKernelTest, LayerNormGrad_LargeSizeTensor) {
