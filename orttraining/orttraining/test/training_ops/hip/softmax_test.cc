...
static void TestSoftmax(const std::vector<int64_t>& X_dims,
  ...
  test.CompareWithCPU(kHipExecutionProvider, per_sample_tolerance, relative_per_sample_tolerance);
}
TEST(HipKernelTest, Softmax_SmallTensor) {
  std::vector<int64_t> X_dims{8, 2, 128, 128};
  std::vector<int64_t> Y_dims{8, 2, 128, 128};
  TestSoftmax(X_dims, Y_dims);
}

TEST(HipKernelTest, Softmax_LargeTensor) {
...
TEST(HipKernelTest, SoftmaxGrad_SmallTensor) {
...
TEST(HipKernelTest, SoftmaxGrad_LargeTensor) {
