...
TEST(ATenOpConfigTest, ValidATenOpConfig) {
  {
    ...
    std::string backward_str =
        "at::a_bw(Tensor<T> grad_output, Tensor<int64> indices, int p=-1, float eps=0.5) -> Tensor<T> grad_input";
    ...
    expected.backward_argument_configs.emplace_back(std::make_tuple(FLOAT, "eps", false));
    ...
    expected.default_float_values["eps"] = 0.5f;
...

  {
    ...
    std::string backward_str =
        "at::b_bw(Tensor<T> grad_r1, Tensor<float> grad_r2, Tensor<float> r2) -> (Tensor<T> grad_weight, Tensor<U> grad_bias)";
    ATenOperatorConfig config = Parse(forward_str, backward_str);
    ATenOperatorConfig expected;
