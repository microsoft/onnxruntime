...
void RunDropoutTest(const bool use_mask, const std::vector<int64_t>& input_shape, float ratio = -1,
...
TEST(DropoutTest, Basic) {
  RunDropoutTest(false, {10, 10, 10}, 0.75);
}

TEST(DropoutTest, Mask) {
  RunDropoutTest(true, {1000}, 0.25);
}

TEST(DropoutTest, RatioLimit) {
  RunDropoutTest(true, {1000}, 0.0f);
}
...
TEST(DropoutTest, Float16Ratio) {
  RunDropoutTest(true, {1000}, 0.0f, true);
}
