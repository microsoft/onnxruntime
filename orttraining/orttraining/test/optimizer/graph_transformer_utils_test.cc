...
TEST(GraphTransformerUtilsTestsForTraining, TestGenerateGraphTransformers) {
  ...
  std::shared_ptr<CPUExecutionProvider> cpu_ep =
      std::make_shared<CPUExecutionProvider>(CPUExecutionProviderInfo());
  ExecutionProviders execution_providers;
  auto status = execution_providers.Add(kCpuExecutionProvider, std::move(cpu_ep));
  ASSERT_TRUE(status.IsOK()) << status.ErrorMessage();

  auto all_transformers = optimizer_utils::GenerateTransformers(TransformerLevel::Level1, {}, execution_providers);
  auto filtered_transformers = optimizer_utils::GenerateTransformers(
      TransformerLevel::Level1, {}, execution_providers, disabled);
  ...
  filtered_transformers = optimizer_utils::GenerateTransformers(
      TransformerLevel::Level2, {}, execution_providers, disabled);
  ASSERT_TRUE(filtered_transformers.size() == all_transformers.size() - 1);
#endif
}
