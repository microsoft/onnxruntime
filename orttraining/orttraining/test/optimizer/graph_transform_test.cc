...
TEST_F(GraphTransformationTests, GatherNodeReplacement) {
  // Given
  // onnx graph generated by running the python test
  // orttraining_test_ortmodule_api.py::test_gradient_correctness_embedding
  // *_torch_exported_training.onnx
  auto model_uri = MODEL_FOLDER "gather_node_replacement.onnx";
  std::shared_ptr<Model> model;
  ASSERT_STATUS_OK(Model::Load(model_uri, model, nullptr, *logger_));
  Graph& graph = model->MainGraph();
  auto op_to_count = CountOpsInGraph(graph);
  ASSERT_TRUE(op_to_count["Gather"] == 2);
  ASSERT_FALSE(op_to_count.count("GatherInternal"));

  auto rule_transformer_L1 = std::make_unique<RuleBasedGraphTransformer>("RuleTransformer1");
  EXPECT_STATUS_OK(rule_transformer_L1->Register(std::make_unique<GatherReplacement>()));
  onnxruntime::GraphTransformerManager graph_transformation_mgr{5};
  ...
  // Then
  op_to_count = CountOpsInGraph(graph);
  EXPECT_FALSE(op_to_count.count("Gather"));
  EXPECT_EQ(op_to_count["com.microsoft.GatherInternal"], 2);
}
