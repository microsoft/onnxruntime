...
void setup_training_params(BertParameters& params) {
  ...
  params.providers.emplace(kCudaExecutionProvider, CreateExecutionProviderFactory_CUDA(device_id, cuda_mem_limit, ArenaExtendStrategy::kSameAsRequested));
...
static Status RunTraining(const BertParameters& params, const Environment& env) {
  ...
  while (GetParametersForPhase(runner->GetRound(), params, params_for_phase)) {
    ...
    auto rank_in_data_parallel_group = (params_for_phase.mpi_context.world_rank / params_for_phase.horizontal_parallel_size) % params_for_phase.data_parallel_size;
