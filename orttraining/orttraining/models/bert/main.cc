...
Status ParseArguments(int argc, char* argv[], BertParameters& params, OrtParameters& ort_params) {
  ...
  try {
    ...
    // num_pipeline_stages controls the number of pipeline's stages.
    // num_pipeline_stages=1 means no model partition, which means all processes run
    // the same model. We only partition model when num_pipeline_stages > 1.
    ...
    // pipeline stages. The i-th file should correspond to the i-th pipeline stage's forward pass.
    ...
    if (params.pipeline_stage_paths.size() != 0) {
      ORT_RETURN_IF_NOT(params.pipeline_stage_paths.size() == params.num_pipeline_stages,
        "The numer of pipeline stage files does not match the number of specified pipeline stages.");
    }
