...
struct T5Parameters : public TrainingRunner::Parameters {
  int max_sequence_length = 16;
  int sequence_length_target = 4;
};
...
Status ParseArguments(int argc, char* argv[], T5Parameters& params, OrtParameters& ort_params) {
  cxxopts::Options options("T5 Training", "Main Program to train T5");
  // clang-format off
  options
    .add_options()
...
const std::map<std::string, std::pair<std::string, size_t>> input_to_dimension_mapping = {
    {"inputs", {"SeqLen", 0}},  // int64[batch,seqlen]    "seqlen" -> "SeqLen", 0
};

// generic properties for storing perf metrics
MapStringToString mapped_dimensions;

void setup_training_params(T5Parameters& params) {
...
static Status RunTraining(const T5Parameters& params, const Environment& env) {
