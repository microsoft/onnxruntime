...
struct ZcodeParameters : public TrainingRunner::Parameters {
...
Status ParseArguments(int argc, char* argv[], ZcodeParameters& params, OrtParameters& ort_params) {
  cxxopts::Options options("Zcode Training", "Main Program to train Zcode");
  // clang-format off
  options
    .add_options()
      ...
      ("num_labels", "the number of classification labels, default 150 for Clinic data", cxxopts::value<int>()->default_value("150"))
  ...
  try {
    ...
    params.num_labels = flags["num_labels"].as<int>();
...
const std::map<std::string, std::pair<std::string, size_t>> input_to_dimension_mapping = {
  ...
  {"attention_mask", {"SeqLen", 0}},  // int64[batch,seqlen]
  {"sequence_label", {"SeqLen", 0}}  // int64[batch,seqlen]
};

// generic properties for storing perf metrics
MapStringToString mapped_dimensions;

void setup_training_params(ZcodeParameters& params) {
  ...
  params.loss_func_info = LossFunctionInfo(OpDef("SparseSoftmaxCrossEntropy", kOnnxDomain),
                                           "mlm_loss",
                                           {/*prediction_name*/ "output_0",
                                            /*label_name*/ "sequence_label"});
...
static Status RunPerformanceTest(const ZcodeParameters& params, const Environment& env) {
  // setup fake data
...
static Status RunTraining(const ZcodeParameters& params, const Environment& env) {
  ...
  auto training_data_loader = onnxruntime::make_unique<ZcodeDataLoader>(params.input_name_map,
                                                                        params.train_data_dir,
                                                                        max_num_files_preload,
                                                                        rank_in_data_parallel_group,
                                                                        params.data_parallel_size);

  std::unique_ptr<ZcodeDataLoader> test_data_loader;
  // Evaluation is only done in device #0
  if (MPIContext::GetInstance().GetWorldRank() == 0) {
    test_data_loader = onnxruntime::make_unique<ZcodeDataLoader>(params.input_name_map,
                                                                  params.test_data_dir,
                                                                  max_num_files_preload);
  }

  if (!params.perf_output_dir.empty()) {
    // collecting Zcode related params from training data
