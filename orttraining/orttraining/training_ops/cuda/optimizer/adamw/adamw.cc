...
Status AdamWOptimizer::ComputeInternal(OpKernelContext* ctx) const {
  ...
  if (update_signal == nullptr || *update_signal->template Data<bool>()) {
    ...
    launch_multi_tensor_functor<MTA_ADAMW_GROUP_SIZE, TFunctor>(
        Stream(ctx), MTA_ADAMW_CHUNK_SIZE, tensor_sizes, grouped_tensor_pointers, functor,
  ...
  ORT_RETURN_IF_ERROR(GenerateOutputs(ctx, Stream(ctx), weights, updated_weights, num_of_weights));
  ORT_RETURN_IF_ERROR(GenerateOutputs(ctx, Stream(ctx), momentums_1, updated_momentums_1, num_of_weights));
  ORT_RETURN_IF_ERROR(GenerateOutputs(ctx, Stream(ctx), momentums_2, updated_momentums_2, num_of_weights));
