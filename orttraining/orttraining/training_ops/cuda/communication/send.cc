...
Status Send<T>::ComputeInternal(OpKernelContext* ctx) const {
  ...
  ORT_ENFORCE(cudaMemcpy(
    buffer.get(), x_tensor->Data<T>(), tensor_size * sizeof(T), cudaMemcpyDeviceToHost) == cudaSuccess);
  ...
  cudaLaunchHostFunc(commStream, HostSend, &info_shape_size);
  cudaLaunchHostFunc(commStream, HostSend, &info_shape);
