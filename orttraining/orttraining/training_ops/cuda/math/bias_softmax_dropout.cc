...
struct DispatchBiasSoftmaxDropoutImpl {
  Status operator()(cudaStream_t stream, const cudaDeviceProp& prop, cudnnHandle_t cudnn_handle, Tensor* dropout_Y, Tensor* mask, Tensor* softmax_Y, const Tensor* X, const Tensor* B, int element_count, int batch_count, bool is_inner_broadcast, int bias_broadcast_size, const float ratio, PhiloxGenerator& generator) {
    ...
    return BiasSoftmaxDropoutImpl<CudaT>(stream, prop, cudnn_handle, dropout_output_data, mask_data, softmax_output_data, input_data, bias_data, element_count, batch_count, is_inner_broadcast, bias_broadcast_size, ratio, generator);
  }
};
...
ONNX_OPERATOR_KERNEL_EX(BiasSoftmaxDropout, kMSDomain, 1, kCudaExecutionProvider, (*KernelDefBuilder::Create()).TypeConstraint("T", BuildKernelDefConstraints<BIAS_SOFTMAX_DROPOUT_TYPES>()).TypeConstraint("T1", DataTypeImpl::AllIEEEFloatTensorTypes()).InputMemoryType(OrtMemTypeCPUInput, 2), BiasSoftmaxDropout);
...
Status BiasSoftmaxDropout::ComputeInternal(OpKernelContext* ctx) const {
  ...
  return t_disp.InvokeRet<Status, DispatchBiasSoftmaxDropoutImpl>(
      Stream(ctx), GetDeviceProp(), GetCudnnHandle(ctx), dropout_Y, mask, softmax_Y, X, B, element_count, batch_count, is_inner_broadcast_, bias_broadcast_size, ratio_data, generator);
}
