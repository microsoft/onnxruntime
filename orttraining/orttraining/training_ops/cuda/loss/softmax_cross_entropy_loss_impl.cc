...
Status SoftmaxCrossEntropyLoss<T, TLabel, TOut>::ComputeInternal(OpKernelContext* ctx) const {
  ...
  if (reduction_ == ReductionType::NONE) {
    ...
    tmp_loss_sample = GetScratchBuffer<TOut>(N_D);
  ...
  if (ctx->OutputCount() > 1) {
    ...
    log_prob_scratch_buffer = GetScratchBuffer<TOut>(logit_shape.Size());
  ...
  if (logit_shape.NumDimensions() > 2) {
    ...
    ORT_RETURN_IF_ERROR(cuda::Transpose::DoTranspose(cuda::Transpose(info), permutations, logit,
                                                     *transpose_output.GetMutable<Tensor>()));
    logit_data = (*transpose_output.GetMutable<Tensor>()).template Data<T>();
  }

  // Calculate logsoftmax
  auto status = SoftMaxComputeHelper<T, TOut, true>(Stream(), logit_data, logit_reshape, log_prob_data, 1);
  ...
  IAllocatorUniquePtr<CudaT_OUT> weight_data_nd = GetScratchBuffer<CudaT_OUT>(N_D);
  ...
  if (logit_shape.NumDimensions() > 2 && log_prob != nullptr) {
    ...
    ORT_RETURN_IF_ERROR(cuda::Transpose::DoTranspose(cuda::Transpose(info), permutations, *log_prob,
                                                     *transpose_output.GetMutable<Tensor>()));
...
Status SoftmaxCrossEntropyLossGrad<T, TLabel, TOut>::ComputeInternal(OpKernelContext* ctx) const {
  ...
  if (probability_shape.NumDimensions() > 2) {
    ...
    ORT_RETURN_IF_ERROR(cuda::Transpose::DoTranspose(cuda::Transpose(info), permutations, *d_logit,
                                                     *transpose_output.GetMutable<Tensor>()));
