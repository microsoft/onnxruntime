...
#define TYPED_FUNCTION_CALL_FWD(T)                                                                                                            \
  if (T_type == DataTypeImpl::GetType<T>()) {                                                                                                 \
    GatherNDImpl<ToHipType<T>::MappedType>(num_slices, kernel_input_data, kernel_output_data, slice_size, input_slice_offsets_buffer.get()); \
  }

#define TYPED_FUNCTION_CALL_BWD(T)                                                                                                                \
  if (T_type == DataTypeImpl::GetType<T>()) {                                                                                                     \
    GatherNDGradImpl<ToHipType<T>::MappedType>(num_slices, kernel_input_data, kernel_output_data, slice_size, input_slice_offsets_buffer.get()); \
  }
...
Status GatherNDBase::CommonComputeKernel(
  ...
  // what the GatherND[Grad] HIP kernel accepts as input.
