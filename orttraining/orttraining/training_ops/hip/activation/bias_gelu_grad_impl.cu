...
void LaunchBiasGeluGradDxKernel(
  ...
  hipLaunchKernelGGL(HIP_KERNEL_NAME(BiasGeluGradDxKernel<T, GeluComputationMode, num_elements_per_thread>), dim3(grid_dim), dim3(num_threads_per_block), 0, 0, bias_size, dY, X, B, dX);
}
