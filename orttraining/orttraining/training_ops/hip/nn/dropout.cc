...
struct DropoutGradComputeImpl {
  void operator()(const int64_t N,
    ...
    DropoutGradientKernelImpl<HipT>(N, dY_data, mask_data, ratio_data, dX_data);
  }
};
...
struct BiasDropoutComputeImpl {
  Status operator()(const hipDeviceProp_t& prop,
                    const int64_t N,
    ...
    const HipT* X_data = reinterpret_cast<const HipT*>(X.template Data<T>());
    const HipT* bias_data = reinterpret_cast<const HipT*>(bias.template Data<T>());
    ...
    if (residual) {
      ...
      residual_data = reinterpret_cast<const HipT*>(residual->template Data<T>());
    }

    HipT* Y_data = reinterpret_cast<HipT*>(Y.template MutableData<T>());

    BiasDropoutKernelImpl<HipT>(prop, N, fdm_dim, ratio_data, generator, X_data, bias_data, residual_data, Y_data, mask_data);
