...
Status NcclAllGather::ComputeInternal(OpKernelContext* context) const {
  ...
  for (int i = 0; i < context->InputCount(); i++) {
    ...
    if (rank_start <= offset && offset < rank_end) {
      ...
      const void* input_data = input_tensor->DataRaw();
      HIP_RETURN_IF_ERROR(hipMemcpyAsync(fusion_data_at_offset, input_data, tensor_bytes, hipMemcpyDeviceToDevice));
    }
  ...
  for (int i = 0; i < context->InputCount(); i++) {
    ...
    if (offset < rank_start || offset >= rank_end) {
      ...
      HIP_RETURN_IF_ERROR(hipMemcpyAsync(output_data, fusion_data_at_offset, tensor_bytes, hipMemcpyDeviceToDevice));
    } else {
      const void* input_data = input_tensor->DataRaw();
      void* output_data = output_tensor->MutableDataRaw();
      if (input_data != output_data) {
        HIP_RETURN_IF_ERROR(hipMemcpyAsync(output_data, input_data, tensor_bytes, hipMemcpyDeviceToDevice));
      }
    }
