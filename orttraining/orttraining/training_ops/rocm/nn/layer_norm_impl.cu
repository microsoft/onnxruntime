...
void HostLayerNormGradient(
  ...
  if (mean == nullptr) {
    hipLaunchKernelGGL(HIP_KERNEL_NAME(cuComputePartGradGammaBeta<T, U, false>), dim3(blocks2), dim3(threads2), nshared2, 0, 
      dout,
      input,
      output,
      gamma,
    ...
    hipLaunchKernelGGL(HIP_KERNEL_NAME(cuComputePartGradGammaBeta<T, U, true>), dim3(blocks2), dim3(threads2), nshared2, 0, 
      dout,
      input,
      output,
      gamma,
  ...
  if (mean == nullptr) {
    hipLaunchKernelGGL(HIP_KERNEL_NAME(cuComputeGradInput<T, U, false>), dim3(blocks1), dim3(threads1), nshared, 0, 
      dout,
      input,
      output,
      gamma,
    ...
    hipLaunchKernelGGL(HIP_KERNEL_NAME(cuComputeGradInput<T, U, true>), dim3(blocks1), dim3(threads1), nshared, 0, 
        dout,
        input,
        output,
        gamma,
...
#define LAYERNORMGRAD_IMPL(T, U)                                                                                                              \
  template void HostLayerNormGradient(const hipDeviceProp_t& prop, const T* dout, const T* input, const T* output,                             \
