...
LayerNormGrad<T, U, simplified>::LayerNormGrad(const OpKernelInfo& op_kernel_info) : RocmKernel(op_kernel_info) {
...
Status LayerNormGrad<T, U, simplified>::ComputeInternal(OpKernelContext* p_op_kernel_context) const {
  ...
  HipT* bias_grad_data = nullptr;
  if (!simplified) {
    Tensor* bias_grad = p_op_kernel_context->Output(2, scale->Shape());
    bias_grad_data = reinterpret_cast<HipT*>(bias_grad->template MutableData<T>());
  }
  ...
  HostLayerNormGradient<HipT, CudaU, simplified>(GetDeviceProp(), Y_grad_data, X_data, reinterpret_cast<const HipT*>(NULL),
...
Status InvertibleLayerNormGrad<T, U>::ComputeInternal(OpKernelContext* p_op_kernel_context) const {
  ...
  HostLayerNormGradient<HipT, CudaU, false>(GetDeviceProp(), Y_grad_data, reinterpret_cast<const HipT*>(NULL), Y_data,
