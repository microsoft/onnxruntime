...
void DropoutGradientKernelImpl(
  ...
  if (ratio == 0.0f) {
    ...
    hipLaunchKernelGGL(HIP_KERNEL_NAME(DropoutGradientKernel<T, GridDim::maxThreadsPerBlock, GridDim::maxElementsPerThread>), dim3(blocksPerGrid), dim3(GridDim::maxThreadsPerBlock), 0, 0, N, dY_data, mask_data, scale, dX_data);
  }
}
...
void BiasDropoutKernelImpl(
  ...
  if (residual_data == nullptr) {
    hipLaunchKernelGGL(HIP_KERNEL_NAME(BiasDropoutKernel<T, false>), dim3(grid_size), dim3(block_size), 0, 0, N, fdm_dim, ratio, seeds, X_data, bias_data, residual_data, Y_data, mask_data);
  } else {
    hipLaunchKernelGGL(HIP_KERNEL_NAME(BiasDropoutKernel<T, true>), dim3(grid_size), dim3(block_size), 0, 0, N, fdm_dim, ratio, seeds, X_data, bias_data, residual_data, Y_data, mask_data);
  }
}
