...
Status AdamOptimizer<T1, T2, T3, T4, T_GRAD, T_GRAD_NORM, T_MIXED_PRECISION_FP>::ComputeInternal(OpKernelContext* ctx) const {
  ...
  HipT_MIXED_PRECISION_FP* mixed_precision_weights_out = nullptr;
  if (NW_MIXED_FP != nullptr) {
    mixed_precision_weights_out = reinterpret_cast<HipT_MIXED_PRECISION_FP*>(NW_MIXED_FP->template MutableData<T_MIXED_PRECISION_FP>());
  }

  const HipT3* loss_scale = nullptr;
  if (loss_scale_tensor != nullptr) {
    loss_scale = reinterpret_cast<const HipT3*>(loss_scale_tensor->template Data<T3>());
  }
  ...
  AdamOptimizerImpl(
      ...
      *S_in,
      ...
      reinterpret_cast<const HipT_GRAD*>(G.template Data<T_GRAD>()),
      ...
      NG != nullptr ? reinterpret_cast<HipT_GRAD*>(NG->template MutableData<T_GRAD>()) : nullptr,
