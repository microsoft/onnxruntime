...
// This file is ported from onnxruntime/core/providers/cpu/rnn/uni_directional_lstm.cc
// with a few changes:
// 1. iofc is made to be an output of the kernel.
// 2. all cell states are stashed and is made an output of this kernel.

#include "orttraining/training_ops/cpu/rnn/uni_directional_lstm.h"
...
void UniDirectionalLstmTraining<T>::Compute(const gsl::span<const T>& inputs_arg,
  ...
  // output_iofc = alpha * (inputs * input_weights) + beta * output_iofc
  ...
  //                 N = 4*hidden_size
  // output_iofc = [max_sequence_length * batch_size_, 4*hidden_size]
...
void UniDirectionalLstmTraining<T>::GateComputations(
  ...
  for (int b = 0; b < local_fused_hidden_rows; b++) {
    ...
    // These lines are the only significant change for the LSTM internal.
    // i.e. LSTMInternal needs the batched cell state as output.
    float* pC =
        SafeRawPointer<T>(batched_cell_state + row * hidden_size_ + b * hidden_size_, batched_cell_state_end, hidden_size_);
    memcpy(pC, pC_cur, hidden_size_ * sizeof(float));
...
template void UniDirectionalLstmTraining<float>::Compute<float>(
    ...
    gsl::span<float>& final_hidden_state, gsl::span<float>& final_cell_state, gsl::span<float>& output_iofc);
