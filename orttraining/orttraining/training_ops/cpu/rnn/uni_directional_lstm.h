...
// This file is ported from onnxruntime/core/providers/cpu/rnn/uni_directional_lstm.h
// with a few changes:
// 1. iofc is made to be an output of the kernel.
// 2. all cell states are stashed and is made an output of this kernel.
...
template <typename T>
class UniDirectionalLstmTraining {
 public:
  UniDirectionalLstmTraining(AllocatorPtr allocator, const logging::Logger& logger, int seq_length, int batch_size,
  ...
  void Compute(const gsl::span<const T>& inputs, const gsl::span<const int>& sequence_lengths, int num_directions,
               ...
               gsl::span<T>& final_cell_state, gsl::span<T>& output_iofc);
 ...
 private:
  ...
  IAllocatorUniquePtr<T> batched_internal_memory_prev_ptr_;
  IAllocatorUniquePtr<T> batched_internal_memory_clipped_ptr_;
  gsl::span<T> batched_internal_memory_prev_;
