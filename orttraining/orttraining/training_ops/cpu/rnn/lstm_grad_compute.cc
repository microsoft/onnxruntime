...
void LSTMGradImpl<T>::ComputeGradient(const LSTMGradInputs<T>& inputs, LSTMGradOutputs<T>& outputs) {
  ...
  float* grad_Wi = grad_weights_required
                       ? SafeRawPointer<T>(grad_W.begin(), grad_W.end(), 4 * hidden_size_ * input_size_)
                       : nullptr;
  float* grad_Wo = grad_weights_required ? grad_Wi + hidden_size_ * input_size_ : nullptr;
  float* grad_Wf = grad_weights_required ? grad_Wo + hidden_size_ * input_size_ : nullptr;
  float* grad_Wc = grad_weights_required ? grad_Wf + hidden_size_ * input_size_ : nullptr;
  ...
  float* grad_Ri = grad_recurrence_weights_required
                       ? SafeRawPointer<T>(grad_R.begin(), grad_R.end(), 4 * hidden_size_ * hidden_size_)
                       : nullptr;
  float* grad_Ro = grad_recurrence_weights_required ? grad_Ri + hidden_size_ * hidden_size_ : nullptr;
  float* grad_Rf = grad_recurrence_weights_required ? grad_Ro + hidden_size_ * hidden_size_ : nullptr;
  float* grad_Rc = grad_recurrence_weights_required ? grad_Rf + hidden_size_ * hidden_size_ : nullptr;
  ...
  float* grad_Wbi = grad_bias_required ? SafeRawPointer<T>(grad_B.begin(), grad_B.end(), 8 * hidden_size_)
                                       : nullptr;
  ...
  float* grad_pi = grad_peephole_weights_required ? SafeRawPointer<T>(grad_P.begin(), grad_P.end(), 3 * hidden_size_)
                                                  : nullptr;
  ...
  for (int idx = 0; idx < batch_size_; ++idx) {
    ...
    const float* grad_Cfinal = inputs.grad_final_cell_state.empty()
                                   ? nullptr
                                   : SafeRawPointer<const T>(inputs.grad_final_cell_state.begin() + idx * hidden_size_,
    ...
    const float* grad_Hfinal = inputs.grad_final_hidden_state.empty()
                                   ? nullptr
                                   : SafeRawPointer<const T>(inputs.grad_final_hidden_state.begin() + idx * hidden_size_,
    ...
    for (int t = sequence_length_ - 1; t >= 0; --t) {
      ...
      const float* grad_Ht2 = inputs.grad_all_hidden_states.empty()
                                  ...
                                  : SafeRawPointer<const T>(
                                        inputs.grad_all_hidden_states.begin() + (t * batch_size_ + idx) * hidden_size_,
      ...
      if (grad_recurrence_weights_required) {
        const float* Htminus1 = t > 0 ? SafeRawPointer<const T>(
                                            inputs.all_hidden_states.begin() + ((t - 1) * batch_size_ + idx) * hidden_size_,
                                            inputs.all_hidden_states.end(), hidden_size_)
                                      : SafeRawPointer<const T>(
                                            inputs.initial_hidden_state.begin() + idx * hidden_size_,
