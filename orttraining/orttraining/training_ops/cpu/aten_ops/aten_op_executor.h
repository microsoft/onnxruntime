...
typedef bool (*IsTensorArgumentFunc)(const char* op_name, size_t index);
typedef std::vector<DLManagedTensor*> (*ExecuteATenOperatorFunc)(const char* op_name,
...
class ATenOperatorExecutor {
 public:
  ...
  bool IsTensorArgument(const std::string& op_name, size_t index) {
    ORT_ENFORCE(p_is_tensor_argument_func_, "ATenOperatorExecutor is not initialized.");
    return p_is_tensor_argument_func_(op_name.c_str(), index);
  }
  std::vector<DLManagedTensor*> operator()(const std::string& op_name, const std::vector<DLManagedTensor*>& dlpacks) {
    ORT_ENFORCE(p_execute_aten_op_func_, "ATenOperatorExecutor is not initialized.");
    return p_execute_aten_op_func_(op_name.c_str(), dlpacks);
  }
