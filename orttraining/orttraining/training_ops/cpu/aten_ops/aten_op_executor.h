...
typedef std::vector<DLManagedTensor*> (*ExecuteATenOperatorFunc)(const char* op_name, const char* overload_name,
                                                                 ...
                                                                 const std::vector<int64_t>& requires_grad,
                                                                 int64_t* p_context_id);
typedef std::vector<DLManagedTensor*> (*ExecuteATenOpBackwardFunc)(const DLManagedTensor* dlpack, int64_t context_id);
...
class ATenOperatorExecutor {
 public:
  ...
  static void Initialize(void* p_is_tensor_argument_func_raw, void* p_execute_aten_op_func_raw,
                         void* p_execute_aten_op_backward_func_raw) {
    InstanceImpl(p_is_tensor_argument_func_raw, p_execute_aten_op_func_raw, p_execute_aten_op_backward_func_raw);
  }
  ...
  std::vector<DLManagedTensor*> ExecuteATenOperator(const std::string& op_name, const std::string& overload_name,
                                                    const std::vector<DLManagedTensor*>& dlpacks,
    ...
    return p_execute_aten_op_func_(op_name.c_str(), overload_name.c_str(), dlpacks, requires_grad, p_context_id);
  }

  std::vector<DLManagedTensor*> ExecuteATenOpBackward(const DLManagedTensor* dlpack, int64_t context_id) {
    ORT_ENFORCE(p_execute_aten_op_backward_func_, "ATenOperatorExecutor is not initialized.");
    return p_execute_aten_op_backward_func_(dlpack, context_id);
  }
 ...
 private:
  static ATenOperatorExecutor& InstanceImpl(void* p_is_tensor_argument_func_raw = nullptr,
                                            ...
                                            void* p_execute_aten_op_backward_func_raw = nullptr) {
    static ATenOperatorExecutor instance(p_is_tensor_argument_func_raw, p_execute_aten_op_func_raw,
                                         p_execute_aten_op_backward_func_raw);
    return instance;
  }

  ATenOperatorExecutor(void* p_is_tensor_argument_func_raw, void* p_execute_aten_op_func_raw,
                       void* p_execute_aten_op_backward_func_raw) {
    ORT_ENFORCE(p_is_tensor_argument_func_raw && p_execute_aten_op_func_raw && p_execute_aten_op_backward_func_raw);
    ...
    p_execute_aten_op_backward_func_ = reinterpret_cast<ExecuteATenOpBackwardFunc>(p_execute_aten_op_backward_func_raw);
  }

  IsTensorArgumentFunc p_is_tensor_argument_func_;
  ExecuteATenOperatorFunc p_execute_aten_op_func_;
  ExecuteATenOpBackwardFunc p_execute_aten_op_backward_func_;
};
