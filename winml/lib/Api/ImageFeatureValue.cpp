...
struct ImageFeatureValue::ImageResourceMetadata
{
    ...
    ::Windows::AI::MachineLearning::Internal::IMG_TENSOR_DESC TensorDescriptor;
};
    ...
    static void DumpResourceToCPU(
        ...
        IMG_TENSOR_DESC tensorDescriptor,
    ...
    ImageFeatureValue::ImageFeatureValue(IVector<Windows::Media::VideoFrame> const& images) :
        m_videoFrames(images)
    {
        Initialize();
    }
    ...
    static IMG_TENSOR_DATA_TYPE GetTensorDataTypeFromTensorKind(TensorKind kind)
    {
        switch (kind)
        {
        case TensorKind::Float:
            return IMG_TENSOR_DATA_TYPE_FLOAT32;
        case TensorKind::Float16:
            return IMG_TENSOR_DATA_TYPE_FLOAT16;
        default:
    ...
    static unsigned GetSizeFromTensorDataType(IMG_TENSOR_DATA_TYPE type)
    {
        switch (type)
        {
        case IMG_TENSOR_DATA_TYPE_FLOAT32:
            return sizeof(float);
        case IMG_TENSOR_DATA_TYPE_FLOAT16:
    ...
    static IMG_TENSOR_DESC CreateImageTensorDescriptor(TensorKind tensorKind, BitmapPixelFormat pixelFormat, uint32_t batchSize, uint32_t width, uint32_t height)
    {
        IMG_TENSOR_DESC tensorDescription = {};
        ...
        {
            tensorDescription.channelType = IMG_TENSOR_CHANNEL_TYPE_RGB_8;
            tensorDescription.sizes[1] = 3;
        }
        else if (pixelFormat == Windows::Graphics::Imaging::BitmapPixelFormat::Bgra8)
        {
            tensorDescription.channelType = IMG_TENSOR_CHANNEL_TYPE_BGR_8;
            tensorDescription.sizes[1] = 3;
        }
        else if (pixelFormat == Windows::Graphics::Imaging::BitmapPixelFormat::Gray8)
        {
            tensorDescription.channelType = IMG_TENSOR_CHANNEL_TYPE_GRAY_8;
        ...
        void* pResource)
    {
        ...
        descriptor.PixelFormat = static_cast<DWORD>(BitmapPixelFormat::Bgra8);
        ...
        WinML::BindingContext& context)
    {
        ...
        CONVERTER_RESOURCE_DESC descriptor = {};
        descriptor.PixelFormat = static_cast<DWORD>(DirectXPixelFormat::B8G8R8X8UIntNormalized);
        descriptor.Width = tensorDescriptor.sizes[3];
        descriptor.Height = tensorDescriptor.sizes[2];
    ...
    static OrtValue CreateMLValue(
        com_ptr<LearningModelSession> spSession,
        const IMG_TENSOR_DESC& tensorDescriptor)
    {
        auto shape =
            std::vector<int64_t>(
        ...
        auto pTensor = new onnxruntime::Tensor(
            tensorDescriptor.dataType == IMG_TENSOR_DATA_TYPE_FLOAT32 ?
                onnxruntime::DataTypeImpl::GetType<float>() :
                onnxruntime::DataTypeImpl::GetType<onnxruntime::MLFloat16>(),
    ...
    std::optional<ImageFeatureValue::ImageResourceMetadata> ImageFeatureValue::GetInputMetadata(const WinML::BindingContext& context)
    {
        ...
        for (uint32_t i = 0; i < m_batchSize; ++i)
        {
            ...
            if (!tempBounds.has_value())
            {
                ...
                if (context.Type == BindingType::Input)
                {
    ...
    HRESULT ImageFeatureValue::GetOrtValue(WinML::BindingContext& context, OrtValue* p_ort_value)
    {
    ...
    HRESULT ImageFeatureValue::UpdateSourceResourceData(BindingContext& context, OrtValue& mlValue)
    {
        // Get the device
        auto spSession = context.Session.as<LearningModelSession>();
        auto spDevice = spSession->Device().as<LearningModelDevice>();
