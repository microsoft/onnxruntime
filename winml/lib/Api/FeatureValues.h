...
inline winrt::Windows::AI::MachineLearning::ILearningModelFeatureValue CreateFeatureValueFromInspectable(
  ...
  if (descriptor.Kind() == LearningModelFeatureKind::Sequence) {
    ...
    if (auto sequence = inspectable.try_as<IVector<float>>()) {
      return implementation::SequenceFloat::Create(sequence);
    }
    if (auto sequence = inspectable.try_as<IVector<double>>()) {
      return implementation::SequenceDouble::Create(sequence);
    }
    if (auto sequence = inspectable.try_as<IVector<int64_t>>()) {
      return implementation::SequenceInt64Bit::Create(sequence);
    }
    if (auto sequence = inspectable.try_as<IVector<winrt::hstring>>()) {
      return implementation::SequenceString::Create(sequence);
    }
    ...
    if (bindingType == Windows::AI::MachineLearning::BindingType::kInput) {
      // Feature inputs should be more permissive, and allow for views to be bound since they are read only
      if (auto sequence = inspectable.try_as<IVectorView<float>>()) {
        return implementation::SequenceFloat::Create(sequence);
      }
      if (auto sequence = inspectable.try_as<IVectorView<double>>()) {
        return implementation::SequenceDouble::Create(sequence);
      }
      if (auto sequence = inspectable.try_as<IVectorView<int64_t>>()) {
        return implementation::SequenceInt64Bit::Create(sequence);
      }
      if (auto sequence = inspectable.try_as<IVectorView<winrt::hstring>>()) {
        return implementation::SequenceString::Create(sequence);
      }
