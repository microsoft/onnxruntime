...
#define CREATE_TENSOR(type, element_type, element_view_type)                                                                  \
    ...
    {                                                                                                                         \
        ...
        {                                                                                                                     \
            ...
            type() = default;                                                                                                 \
                                                                                                                              \
            type(wfc::IIterable<int64_t> const& shape) :                                                                      \
                Base(shape){};                                                                                                \
                                                                                                                              \
            type(std::vector<int64_t> const& shape) :                                                                         \
                Base(shape){};                                                                                                \
                                                                                                                              \
            type(std::vector<int64_t> const& shape, ID3D12Resource* pResource) :                                              \
                Base(shape, pResource){};                                                                                     \
        };                                                                                                                    \
    }                                                                                                                         \
    ...
    {                                                                                                                         \
        ...
        {                                                                                                                     \
            STDMETHOD(CreateFromD3D12Resource)(ID3D12Resource *value, __int64 *shape, int shapeSize, IUnknown ** result)      \
            {                                                                                                                 \
...
#define CREATE_MAP(type, key_type, value_type)                                                                          \
    ...
    {                                                                                                                   \
        ...
        {                                                                                                               \
            type(wfc::IMap<key_type, value_type> const& data) :                                                         \
                MapBase<type, key_type, value_type>(data){};                                                            \
        };                                                                                                              \
    }                                                                                                                   \
...
#define CREATE_SEQUENCE(type, element_type)                                                                             \
    ...
    {                                                                                                                   \
        ...
        {                                                                                                               \
            type(wfc::IIterable<element_type> const& data) :                                                            \
                SequenceBase<type, element_type>(data){};                                                               \
        };                                                                                                              \
    }                                                                                                                   \
...
CREATE_SEQUENCE(SequenceInt64Bit, int64_t)
CREATE_SEQUENCE(SequenceFloat, float)
CREATE_SEQUENCE(SequenceDouble, double)
CREATE_SEQUENCE(SequenceString, hstring)
    ...
    const winrt::Windows::AI::MachineLearning::ITensorFeatureDescriptor& descriptor)
{
    ...
    if (descriptor.TensorKind() == WinML::TensorKindFrom<TDataType>::Type)
    {
        ...
        if (bindingType == Windows::AI::MachineLearning::BindingType::Input)
        {
	...
	const winrt::Windows::AI::MachineLearning::ITensorFeatureDescriptor& descriptor)
{
	...
	if (descriptor.TensorKind() == abi::TensorKind::Int8)
	{
		...
		if (bindingType == WinML::BindingType::Input)
		{
			// Feature inputs should be more permissive, and allow for views to be bound since they are read only
    ...
    const winrt::Windows::AI::MachineLearning::ILearningModelFeatureDescriptor& descriptor)
{
    ...
    if (auto map = inspectable.try_as<IMap<winrt::hstring, float>>())               { return implementation::MapStringToFloat::Create(map); }
    if (auto map = inspectable.try_as<IMap<winrt::hstring, double>>())              { return implementation::MapStringToDouble::Create(map); }
    if (auto map = inspectable.try_as<IMap<winrt::hstring, int64_t>>())             { return implementation::MapStringToInt64Bit::Create(map); }
    if (auto map = inspectable.try_as<IMap<winrt::hstring, winrt::hstring>>())      { return implementation::MapStringToString::Create(map); }
    if (auto map = inspectable.try_as<IMap<int64_t, float>>())                      { return implementation::MapInt64BitToFloat::Create(map); }
    if (auto map = inspectable.try_as<IMap<int64_t, double>>())                     { return implementation::MapInt64BitToDouble::Create(map); }
    if (auto map = inspectable.try_as<IMap<int64_t, int64_t>>())                    { return implementation::MapInt64BitToInt64Bit::Create(map); }
    if (auto map = inspectable.try_as<IMap<int64_t, winrt::hstring>>())             { return implementation::MapInt64BitToString::Create(map); }

    if (bindingType == Windows::AI::MachineLearning::BindingType::Input)
    {
        // Feature inputs should be more permissive, and allow for views to be bound since they are read only
        if (auto map = inspectable.try_as<IMapView<winrt::hstring, float>>())           { return implementation::MapStringToFloat::Create(map); }
        if (auto map = inspectable.try_as<IMapView<winrt::hstring, double>>())          { return implementation::MapStringToDouble::Create(map); }
        if (auto map = inspectable.try_as<IMapView<winrt::hstring, int64_t>>())         { return implementation::MapStringToInt64Bit::Create(map); }
        if (auto map = inspectable.try_as<IMapView<winrt::hstring, winrt::hstring>>())  { return implementation::MapStringToString::Create(map); }
        if (auto map = inspectable.try_as<IMapView<int64_t, float>>())                  { return implementation::MapInt64BitToFloat::Create(map); }
        if (auto map = inspectable.try_as<IMapView<int64_t, double>>())                 { return implementation::MapInt64BitToDouble::Create(map); }
        if (auto map = inspectable.try_as<IMapView<int64_t, int64_t>>())                { return implementation::MapInt64BitToInt64Bit::Create(map); }
        if (auto map = inspectable.try_as<IMapView<int64_t, winrt::hstring>>())         { return implementation::MapInt64BitToString::Create(map); }
    }
    ...
    if (descriptor.Kind() == LearningModelFeatureKind::Sequence)
    {
        ...
        if (auto sequence = inspectable.try_as<IVector<float>>())                           { return implementation::SequenceFloat::Create(sequence); }
        if (auto sequence = inspectable.try_as<IVector<double>>())                          { return implementation::SequenceDouble::Create(sequence); }
        if (auto sequence = inspectable.try_as<IVector<int64_t>>())                         { return implementation::SequenceInt64Bit::Create(sequence); }
        if (auto sequence = inspectable.try_as<IVector<winrt::hstring>>())                  { return implementation::SequenceString::Create(sequence); }
        if (auto sequence = inspectable.try_as<IVector<IMap<winrt::hstring, float>>>())     { return implementation::SequenceMapStringFloat::Create(sequence); }
        if (auto sequence = inspectable.try_as<IVector<IMap<int64_t, float>>>())            { return implementation::SequenceMapInt64BitFloat::Create(sequence); }

        if (bindingType == Windows::AI::MachineLearning::BindingType::Input)
        {
            // Feature inputs should be more permissive, and allow for views to be bound since they are read only
            if (auto sequence = inspectable.try_as<IVectorView<float>>())                       { return implementation::SequenceFloat::Create(sequence); }
            if (auto sequence = inspectable.try_as<IVectorView<double>>())                      { return implementation::SequenceDouble::Create(sequence); }
            if (auto sequence = inspectable.try_as<IVectorView<int64_t>>())                     { return implementation::SequenceInt64Bit::Create(sequence); }
            if (auto sequence = inspectable.try_as<IVectorView<winrt::hstring>>())              { return implementation::SequenceString::Create(sequence); }
            if (auto sequence = inspectable.try_as<IVectorView<IMap<winrt::hstring, float>>>()) { return implementation::SequenceMapStringFloat::Create(sequence); }
            if (auto sequence = inspectable.try_as<IVectorView<IMap<int64_t, float>>>())        { return implementation::SequenceMapInt64BitFloat::Create(sequence); }
        }
    }
