...
static ILearningModelFeatureValue CreateUnboundOuputFeatureValue(
  ...
  // Sequences
  else if (IsOfType<onnxruntime::VectorString>(mlValue)) {
    return implementation::SequenceString::Create();
  } else if (IsOfType<onnxruntime::VectorInt64>(mlValue)) {
    return implementation::SequenceInt64Bit::Create();
  } else if (IsOfType<onnxruntime::VectorFloat>(mlValue)) {
    return implementation::SequenceFloat::Create();
  } else if (IsOfType<onnxruntime::VectorDouble>(mlValue)) {
    return implementation::SequenceDouble::Create();
