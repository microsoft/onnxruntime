...
namespace winrt::Windows::AI::MachineLearning::implementation
{
    ...
    LearningModelDevice::LearningModelDevice(Windows::AI::MachineLearning::LearningModelDeviceKind const& deviceKind) try :
        m_deviceCache(std::make_unique<D3DDeviceCache>(deviceKind))
    {
        m_deviceKind = deviceKind;
        if (m_deviceKind == LearningModelDeviceKind::Cpu || m_deviceKind == LearningModelDeviceKind::Default)
        {
            m_isCpuDevice = true;
        ...
        {
            g_Profiler.Enable(ProfilerType::GPU);
            g_Profiler.Reset(ProfilerType::GPU);
            m_isCpuDevice = false;
        }
    }
    WINML_CATCH_ALL

    LearningModelDevice::LearningModelDevice(Windows::Graphics::DirectX::Direct3D11::IDirect3DDevice const& device) try :
        m_deviceCache(std::make_unique<D3DDeviceCache>(device))
    {
    ...
    bool
    LearningModelDevice::IsCpuDevice()
    {
    ...
    bool
    LearningModelDevice::MetacommandsEnabled()
    {
