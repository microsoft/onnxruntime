...
namespace Windows::AI::MachineLearning
{
    ...
    struct TensorBase : TBase
    {
        ...
        TensorBase(winrt::Windows::Foundation::Collections::IIterable<int64_t> const& shape) :
            m_shape(begin(shape), end(shape)),
            m_resources(std::make_shared<TensorResources<T>>())
        {
            GetCpuResource() = std::make_shared<WinML::Tensor<T>>(m_shape);
        }

        TensorBase(std::vector<int64_t> const& shape) :
            m_shape(shape),
            m_resources(std::make_shared<TensorResources<T>>())
        {
            GetCpuResource() = std::make_shared<WinML::Tensor<T>>(m_shape);
        ...
        TensorBase(std::vector<int64_t> const& shape, ID3D12Resource* pResource) :
            m_shape(shape),
            m_resources(std::make_shared<TensorResources<T>>())
        {
        ...
        STDMETHOD(GetBuffer)(BYTE **value, UINT32 *capacity)
        {
            // This Api is not supported for TensorString
            RETURN_HR_IF_MSG(
        ...
        STDMETHOD(GetD3D12Resource)(ID3D12Resource ** ppResource)
        {
            try
            {
                // This Api is not supported for TensorString
        ...
        winrt::Windows::Foundation::Collections::IVectorView<winrt::hstring> GetAsVectorView<std::string, winrt::hstring>() try
        {
            ...
            std::generate(
                ...
                [n = 0, &pData]() mutable
                {
                    return WinML::Strings::hstring_from_utf8(pData[n++]);
                });
    ...
    private:
        ...
        void SetBufferFromArray<std::string, winrt::hstring>(winrt::array_view<winrt::hstring const> data)
        {
            ...
            std::transform(
                ...
                {
                    return WinML::Strings::utf8_from_hstring(element);
                });
        }
