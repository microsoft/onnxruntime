        ...
        WinML::ILotusValueProviderPrivate>
{
    ...
    template <typename T> struct ValidLotusType { using Type = T; };
    template <> struct ValidLotusType<winrt::hstring> { using Type = std::string; };
    ...
    {
        return WinML::Strings::utf8_from_hstring(raw);
    }
    ...
    {
        return WinML::Strings::hstring_from_utf8(lotusValue);
    }
    ...
    STDMETHOD(get_KeyKind)(winrt::Windows::AI::MachineLearning::TensorKind* kind)
    {
    ...
    STDMETHOD(get_ValueDescriptor)(winrt::Windows::AI::MachineLearning::ILearningModelFeatureDescriptor* result)
    {
    ...
    STDMETHOD(GetOrtValue)(WinML::BindingContext& context, OrtValue* mlValue)
    {
        // TODO: Tensorized data should be cached so multiple bindings work more efficiently

        // Create a copy of the map
        auto map = context.Type == WinML::BindingType::Input ?
    ...
    STDMETHOD(UpdateSourceResourceData)(BindingContext& context, OrtValue& mlValue)
    {
        m_data.Clear();
