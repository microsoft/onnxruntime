...
wgi::BitmapAlphaMode ImageFeatureDescriptor::BitmapAlphaMode() try { return alpha_mode_; }
WINML_CATCH_ALL

uint32_t ImageFeatureDescriptor::Width() try { return width_; }
WINML_CATCH_ALL

uint32_t ImageFeatureDescriptor::Height() try { return height_; }
WINML_CATCH_ALL

hstring ImageFeatureDescriptor::Name() try { return name_; }
WINML_CATCH_ALL

hstring ImageFeatureDescriptor::Description() try { return description_; }
WINML_CATCH_ALL

winml::LearningModelFeatureKind ImageFeatureDescriptor::Kind() try { return LearningModelFeatureKind::Image; }
WINML_CATCH_ALL

bool ImageFeatureDescriptor::IsRequired() try { return is_required_; }
WINML_CATCH_ALL

winml::TensorKind ImageFeatureDescriptor::TensorKind() {
  return tensor_kind_;
}

wfc::IVectorView<int64_t> ImageFeatureDescriptor::Shape() {
  return winrt::single_threaded_vector<int64_t>(std::vector<int64_t>(std::begin(shape_), std::end(shape_))).GetView();
}

HRESULT
ImageFeatureDescriptor::GetName(const wchar_t** name, uint32_t* cchName) {
  *name = name_.data();
...
HRESULT
ImageFeatureDescriptor::GetDescription(const wchar_t** description, uint32_t* cchDescription) {
  *description = description_.data();
  *cchDescription = static_cast<uint32_t>(description_.size());
  return S_OK;
}

winml::LearningModelPixelRange ImageFeatureDescriptor::PixelRange() {
  return pixel_range_;
}
