...
HRESULT OnnxruntimeCUDASessionBuilder::RuntimeClassInitialize(OnnxruntimeEngineFactory* engine_factory, IExecutionProviderOptions* options) {
...
HRESULT
OnnxruntimeCUDASessionBuilder::CreateSessionOptions(
  ...
  // Request the dml ep
  RETURN_HR_IF_NOT_OK_MSG(ort_api->SessionOptionsAppendExecutionProvider_CUDA(session_options.get(), &cuda_options),
                          ort_api);
...
HRESULT OnnxruntimeCUDASessionBuilder::Initialize(
    OrtSession* session) {
