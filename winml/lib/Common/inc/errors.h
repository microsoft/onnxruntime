...
#define WINML_THROW_IF_NOT_OK(status)                                                                           \
    do {                                                                                                        \
        ...
        {                                                                                                       \
            ...
            g_Telemetry.LogRuntimeError(hresult, _status.ErrorMessage(), __FILE__, __FUNCTION__, __LINE__);     \
            winrt::hstring errorMessage(WinML::Strings::hstring_from_utf8(_status.ErrorMessage()));             \
...
#define WINML_THROW_HR_IF_FALSE_MSG(hr, value, message, ...)                                                    \
    do {                                                                                                        \
        ...
        {                                                                                                       \
            ...
            g_Telemetry.LogRuntimeError(_hr, msg, __FILE__, __FUNCTION__, __LINE__);                            \
            winrt::hstring errorMessage(WinML::Strings::hstring_from_utf8(msg));                                \
...
#define WINML_THROW_HR(hr)                                                                                      \
    {                                                                                                           \
        auto _result = hr;                                                                                      \
        g_Telemetry.LogRuntimeError(_result, "", __FILE__, __FUNCTION__, __LINE__);                             \
...
#define WINML_THROW_IF_FAILED(hr)                                                                               \
    do {                                                                                                        \
        ...
        if (FAILED(_hr))                                                                                        \
        {                                                                                                       \
            g_Telemetry.LogRuntimeError(_hr, "", __FILE__, __FUNCTION__, __LINE__);                             \
