...
HRESULT ImageConverter::SyncD3D11ToD3D12(_In_ D3DDeviceCache& deviceCache, _In_ ID3D11Texture2D* pD3D11Texture)
{
    ...
    if (spTextureDevice.Get() == deviceCache.GetD3D11Device())
    {
        // If the texture is on D3DDeviceCache's device, we sync using D3DDeviceCache's fences
        deviceCache.GPUSyncD3D11ToD3D12();
    }
    else
    {
        // Otherwise, sync using our own cached fences
        ComPtr<ID3D11Fence> spD3D11DeviceFence;
        THROW_IF_FAILED(FetchOrCreateFenceOnDevice(deviceCache, spTextureDevice.Get(), &spD3D11DeviceFence));

        deviceCache.SyncD3D11DeviceToConverter(spD3D11DeviceFence.Get());
    }

    return S_OK;
}

HRESULT ImageConverter::SyncD3D12ToD3D11(_In_ D3DDeviceCache& deviceCache, _In_ ID3D11Texture2D* spTexture)
{
    ...
    {
        // If the texture is on D3DDeviceCache's device, we sync using D3DDeviceCache's fences
        deviceCache.GPUSyncD3D12ToD3D11();
    }
    else
    {
        // Otherwise, sync using our own cached fences
        ...
        deviceCache.SyncConverterToD3D11Device(spD3D11DeviceFence.Get());
    }

    return S_OK;
}

HRESULT ImageConverter::FetchOrCreateFenceOnDevice(_In_ D3DDeviceCache& deviceCache, _In_ ID3D11Device* pD3D11Device, _Out_ ID3D11Fence** ppFence)
{
    ...
    if (FAILED(pD3D11Device->GetPrivateData(deviceCache.GetFenceGuid(), &comPtrSize, ppFence)) || *ppFence == nullptr)
    {
        ...
        THROW_IF_FAILED(spD3D11Device5->OpenSharedFence(deviceCache.GetConverterFenceHandle(), IID_PPV_ARGS(ppFence)));

        // Store the fence on the device
        THROW_IF_FAILED(spD3D11Device5->SetPrivateDataInterface(deviceCache.GetFenceGuid(), *ppFence));
    }

    return S_OK;
}

HRESULT ImageConverter::ResetCommandList(_In_ D3DDeviceCache& deviceCache)
{
    if (!_spCommandList)
    {
        assert(_spCommandAllocator == nullptr);

        THROW_IF_FAILED(deviceCache.GetD3D12Device()->CreateCommandAllocator(
            deviceCache.GetCommandQueue()->GetDesc().Type,
            IID_PPV_ARGS(_spCommandAllocator.ReleaseAndGetAddressOf())));

        THROW_IF_FAILED(deviceCache.GetD3D12Device()->CreateCommandList(
            0,
            deviceCache.GetCommandQueue()->GetDesc().Type,
            _spCommandAllocator.Get(),
            _spPipelineState.Get(),
            IID_PPV_ARGS(_spCommandList.ReleaseAndGetAddressOf())));
    }
    else
    {
        _spCommandList->Reset(_spCommandAllocator.Get(), _spPipelineState.Get());
    }
...
{
    return _spCommandAllocator->Reset();
}
