...
#pragma warning(disable: 26014) // warning about possible out of bounds accesing pData, but input is checked for BGRA8 format, so uiCapacity should be in multiples of 4
            ...
            uint32_t bytesPerPixel = formatTo == IMG_TENSOR_CHANNEL_TYPE_GRAY_8 ? 1 : 4;
            ...
            if (formatFrom == formatTo && (formatFrom == IMG_TENSOR_CHANNEL_TYPE_BGR_8 || formatFrom == IMG_TENSOR_CHANNEL_TYPE_RGB_8))
            {
            ...
            else if ((formatFrom == IMG_TENSOR_CHANNEL_TYPE_RGB_8 && formatTo == IMG_TENSOR_CHANNEL_TYPE_BGR_8) || (formatFrom == IMG_TENSOR_CHANNEL_TYPE_BGR_8 && formatTo == IMG_TENSOR_CHANNEL_TYPE_RGB_8))
            {
                for (uint32_t i = 0; i < tensorHeight; i++)
                {
                    BYTE* pPixel = pData;
            ...
            else if (formatFrom == IMG_TENSOR_CHANNEL_TYPE_BGR_8 && formatTo == IMG_TENSOR_CHANNEL_TYPE_GRAY_8)
            {
                for (uint32_t i = 0; i < end; i += bufferWidth)
                {
                    for (uint32_t j = i; j < i + bytesPerRow; j += 1)
            ...
            else if (formatFrom == IMG_TENSOR_CHANNEL_TYPE_RGB_8 && formatTo == IMG_TENSOR_CHANNEL_TYPE_GRAY_8)
            {
                for (uint32_t i = 0; i < end; i += bufferWidth)
                {
                    for (uint32_t j = i; j < i + bytesPerRow; j += 1)
