  ...
  if (inputVideoFrame.SoftwareBitmap()) {
    ...
    if (ImageConversionHelpers::TextureIsOnDevice(spVideoFrameTexture.Get(), pDeviceCache->GetD3D11Device())) {
      ...
      if (!D3D11_cached_texture_) {
        ...
        WINML_THROW_IF_FAILED(ShareD3D11Texture(D3D11_cached_texture_.Get(), pDeviceCache->GetD3D12Device(), &input_D3D12_resource_));
      } else {
      ...
      WINML_THROW_IF_FAILED(CopyTextureIntoTexture(spVideoFrameTexture.Get(), scaledBounds, D3D11_cached_texture_.Get()));
    } else {
      ...
      if ((FAILED(spVideoFrameTexture->GetPrivateData(d3d11_texture_GUID_, &comPtrSize, spSharedD3D11Texture.GetAddressOf())) || !spSharedD3D11Texture.Get()) || (FAILED(spVideoFrameTexture->GetPrivateData(handle_GUID_, &handleSize, &sharedHandle)) || sharedHandle != _sharedHandle)) {
        ...
        WINML_THROW_IF_FAILED(ShareD3D11Texture(spSharedD3D11Texture.Get(), pDeviceCache->GetD3D12Device(), &input_D3D12_resource_));
        ...
        WINML_THROW_IF_FAILED(spVideoFrameTexture->SetPrivateData(handle_GUID_, sizeof(_sharedHandle), &_sharedHandle));
      }
    ...
    WINML_THROW_IF_FAILED(ConvertDX12TextureToGPUTensor(batchIdx, input_D3D12_resource_.Get(), *pDeviceCache, tensorDesc, pOutputTensor));
  } else {
    // Invalid video frame
...
HRESULT VideoFrameToTensorConverter::ConvertSoftwareBitmapToGPUTensor(
  ...
  THROW_IF_FAILED(upload_heap_->Map(0, &CD3DX12_RANGE(0, 0), &pCPUTensorBuffer));
