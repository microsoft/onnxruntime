...
namespace winrt::Windows::AI::MachineLearning::implementation
{
    ...
    {
        WINML_THROW_IF_FAILED(CoCreateGuid(&m_fenceGuid));
        ...
        if (deviceKind == LearningModelDeviceKind::Cpu || deviceKind == LearningModelDeviceKind::Default)
        {
            ...
            m_deviceLuid.HighPart = m_deviceLuid.LowPart = 0;
            return;
        }
        ...
        if (support.hasDxgi)
        {
            ...
            WINML_THROW_IF_FAILED(D3D12CreateDevice(spAdapter.get(), D3D_FEATURE_LEVEL_11_0, IID_PPV_ARGS(m_device.put())));
        }
#ifdef ENABLE_DXCORE
...
#endif
        InitializeCommandQueue(m_device.get());

        m_deviceLuid = m_device->GetAdapterLuid();
    }
    ...
    D3DDeviceCache::D3DDeviceCache(Windows::Graphics::DirectX::Direct3D11::IDirect3DDevice const& device)
    {
        ...
        m_device11 = dxgiDevice.as<ID3D11Device>();

        com_ptr<ID3D11DeviceContext> spContext;
        m_device11->GetImmediateContext(spContext.put());
        ...
        WINML_THROW_IF_FAILED(D3D12CreateDevice(adapter.get(), D3D_FEATURE_LEVEL_11_0, IID_PPV_ARGS(m_device.put())));
    ...
    D3DDeviceCache::D3DDeviceCache(ID3D12CommandQueue* queue)
    {
        ...
        m_commandQueue.copy_from(queue);
        m_commandQueue->QueryInterface(IID_PPV_ARGS(m_sharingContract.put()));

        WINML_THROW_IF_FAILED(queue->GetDevice(IID_PPV_ARGS(m_device.put())));
    ...
    {
        // If this is a CPU instance m_device will not have been created.
        // Ensure the device is still valid before doing work.
        if (m_device != nullptr && (m_device->GetDeviceRemovedReason() == S_OK))
        {
            // dx11 stack is optional, and we lazy load it when available
            if (m_deviceContext11 != nullptr)
            {
    ...
    bool D3DDeviceCache::IsFloat16Supported()
    {
        if (m_device != nullptr)
        {
            return DeviceHelpers::IsFloat16Supported(m_device.get());
        }
    ...
    void D3DDeviceCache::InitializeCommandQueue(ID3D12Device1* device)
    {
        ...
        WINML_THROW_IF_FAILED(device->CreateCommandQueue(&queueDesc, winrt::guid_of<ID3D12CommandQueue>(), m_commandQueue.put_void()));
    ...
    void D3DDeviceCache::EnsureD3D11FromD3D12()
    {
        ...
        if (m_winrtDevice != nullptr)
            return;

        CWinML_AutoLock lock(&m_lock);
        ...
        WINML_THROW_IF_FAILED(DeviceHelpers::CreateD3D11On12Device(m_device.get(), m_device11.put()));
        com_ptr<ID3D11DeviceContext> spContext;
        m_device11->GetImmediateContext(spContext.put());
        spContext.as(m_deviceContext11);
             
        WINML_THROW_IF_FAILED(m_device11->QueryInterface(IID_PPV_ARGS(spDXGIDevice.put())));
        ...
        WINML_THROW_IF_FAILED(spInspectable->QueryInterface(winrt::guid_of<Windows::Graphics::DirectX::Direct3D11::IDirect3DDevice>(), reinterpret_cast<void**>(winrt::put_abi(m_winrtDevice))));
    }
    ...
    void D3DDeviceCache::EnsureD3D12Fence()
    {
        ...
        WINML_THROW_IF_FAILED(m_device->CreateFence(0, D3D12_FENCE_FLAG_SHARED, IID_PPV_ARGS(m_d3d12Fence.put())));
    }
...
//      m_d3d12Fence
    void D3DDeviceCache::EnsureSharedFences()
    {
        ...
        m_d3d12Fence.as(spD3D12DeviceChild);
        HANDLE hSharedFence;
        WINML_THROW_IF_FAILED(m_device->CreateSharedHandle(spD3D12DeviceChild.get(), NULL, GENERIC_ALL, nullptr, &hSharedFence));

        com_ptr<ID3D11Device5> spD3D11Device5;
        m_device11.as(spD3D11Device5);
        wil::unique_handle safe(hSharedFence);
        WINML_THROW_IF_FAILED(spD3D11Device5->OpenSharedFence(safe.get(), IID_PPV_ARGS(m_d3d11Fence.put())));
    }
    ...
    void D3DDeviceCache::GPUSyncD3D11ToD3D12()
    {
        ...
        WINML_THROW_IF_FAILED(m_deviceContext11->Signal(m_d3d11Fence.get(), currentFence));
        
        WINML_THROW_IF_FAILED(m_commandQueue->Wait(m_d3d12Fence.get(), currentFence));

        if (m_sharingContract != nullptr)
        {
            m_sharingContract->SharedFenceSignal(m_d3d12Fence.get(), currentFence);
        }
    }
    ...
    void D3DDeviceCache::GPUSyncD3D12ToD3D11()
    {
        ...
        UINT64 currentFence = m_fenceValue++;
        WINML_THROW_IF_FAILED(m_commandQueue->Signal(m_d3d12Fence.get(), currentFence));
        
        WINML_THROW_IF_FAILED(m_deviceContext11->Wait(m_d3d11Fence.get(), currentFence));
    }
    ...
    void D3DDeviceCache::WaitForFenceValue(UINT64 fenceValue)
    {
        ...
        WINML_THROW_IF_FAILED(m_d3d12Fence->SetEventOnCompletion(fenceValue, event.get()));
    ...
    {
        if (m_spTensorizeRootSignature == nullptr)
        {
            ...
            // Compute root signature.
            {
                ...
                WINML_THROW_IF_FAILED(m_device->CreateRootSignature(0, signature->GetBufferPointer(), signature->GetBufferSize(), IID_PPV_ARGS(newRootSignature.put())));
                newRootSignature->SetName(L"Tensorize Rootsignature");
            }
            
            if (InterlockedCompareExchangePointer(
                    m_spTensorizeRootSignature.put_void(),
        ...
        return m_spTensorizeRootSignature.get();
    }

    ID3D12RootSignature* D3DDeviceCache::GetDetensorizeRootSignature()
    {
        if (m_spDetensorizeRootSignature == nullptr)
        {
            com_ptr<ID3D12RootSignature> newRootSignature;
            D3D12_FEATURE_DATA_ROOT_SIGNATURE featureData = {};
            ...
            if (InterlockedCompareExchangePointer(
                    m_spDetensorizeRootSignature.put_void(),
                    newRootSignature.get(),
                    nullptr
                    ) == nullptr)
            {
        ...
        return m_spDetensorizeRootSignature.get();
    }
    ...
    {
        if (m_spCachedPipelineState[static_cast<int>(type)][static_cast<int>(formatFrom)][static_cast<int>(formatTo)][static_cast<int>(operation)] == nullptr)
        {
            com_ptr<ID3D12PipelineState> newPSO;
            if (operation == PipelineStateCacheOperation::Tensorize)
            {
            ...
            if (InterlockedCompareExchangePointer(
                    m_spCachedPipelineState[static_cast<int>(type)][static_cast<int>(formatFrom)][static_cast<int>(formatTo)][static_cast<int>(operation)].put_void(),
                    newPSO.get(),
                    nullptr
                    ) == nullptr)
            {
        ...
        return m_spCachedPipelineState[static_cast<int>(type)][static_cast<int>(formatFrom)][static_cast<int>(formatTo)][static_cast<int>(operation)].get();
    }
    ...
    {
        static_assert(static_cast<unsigned int>(PipelineStateCacheFormat::Count) == 3, "PipelineStateCacheFormat changed, update D3DDeviceCache::CreateTensorizePipelineState()");
        ...
        switch (formatFrom)
        {
        case PipelineStateCacheFormat::BGR8:
        case PipelineStateCacheFormat::RGB8:
            if (type == PipelineStateCacheType::Float32)
            {
                if (formatTo == PipelineStateCacheFormat::BGR8)
                {
                    shaderBytecode = float32::g_csSurfaceToTensorBGR8;
                    shaderBytecodeSize = sizeof(float32::g_csSurfaceToTensorBGR8);
                }
                else if (formatTo == PipelineStateCacheFormat::RGB8)
                {
                    shaderBytecode = float32::g_csSurfaceToTensorRGB8;
                    shaderBytecodeSize = sizeof(float32::g_csSurfaceToTensorRGB8);
                }
                else if (formatTo == PipelineStateCacheFormat::GRAY8)
                {
            ...
            else if (type == PipelineStateCacheType::Float16)
            {
                if (formatTo == PipelineStateCacheFormat::BGR8)
                {
            ...
            break;     
        case PipelineStateCacheFormat::GRAY8:
            if (type == PipelineStateCacheType::Float32)
            {
                if (formatTo == PipelineStateCacheFormat::BGR8 || formatTo == PipelineStateCacheFormat::RGB8)
                {
        ...
        WINML_THROW_IF_FAILED(m_device->CreateComputePipelineState(&computePsoDesc, IID_PPV_ARGS(pipelineState.put())));
    ...
    {
        static_assert(static_cast<unsigned int>(PipelineStateCacheFormat::Count) == 3, "PipelineStateCacheFormat changed, update D3DDeviceCache::CreateDetensorizePipelineState()");
        ...
        case PipelineStateCacheFormat::BGR8:
            ...
            else if (type == PipelineStateCacheType::Float32)
            {
                if (formatTo == PipelineStateCacheFormat::BGR8 || formatTo == PipelineStateCacheFormat::RGB8)
                {
    ...
    {
        if (m_detensorizeVertexBuffer == nullptr)
        {
            ...
            WINML_THROW_IF_FAILED(m_device->CreateCommittedResource(
                &heapProp,
            ...
            if (InterlockedCompareExchangePointer(
                    m_detensorizeVertexBuffer.put_void(),
                    newResource.get(),
                    nullptr
                    ) == nullptr)
            {
        ...
        *vertexBufferSize = sc_vertexBufferSize;
        return m_detensorizeVertexBuffer.get();
    }
    ...
    HANDLE D3DDeviceCache::GetConverterFenceHandle()
    {
        ...
        if (!m_converterFence)
        {
            WINML_THROW_IF_FAILED(m_device->CreateFence(0, D3D12_FENCE_FLAG_SHARED | D3D12_FENCE_FLAG_SHARED_CROSS_ADAPTER, IID_PPV_ARGS(m_converterFence.put())));

            HANDLE hSharedFence;
            WINML_THROW_IF_FAILED(m_device->CreateSharedHandle(m_converterFence.get(), nullptr, GENERIC_ALL, nullptr, &hSharedFence));

            m_converterFenceHandle = wil::unique_handle(hSharedFence);
        }

        return m_converterFenceHandle.get();
    }
    ...
    void D3DDeviceCache::SyncConverterToD3D11Device(_In_ ID3D11Fence* pD3D11Fence)
    {
        ...
        UINT64 newfenceValue = m_converterFenceValue++;
        WINML_THROW_IF_FAILED(m_commandQueue->Signal(m_converterFence.get(), newfenceValue));
    ...
    void D3DDeviceCache::SyncD3D11DeviceToConverter(_In_ ID3D11Fence* pD3D11Fence)
    {
        ...
        WINML_THROW_IF_FAILED(m_commandQueue->Wait(m_converterFence.get(), newfenceValue));
    }

    bool D3DDeviceCache::SharedHandleInitialized()
    {
        return m_d3d11Fence != nullptr;
    }
}