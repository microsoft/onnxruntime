...
ORT_API_STATUS_IMPL(winmla::SessionGetNamedDimensionsOverrides, _In_ OrtSession* session, _Out_ winrt::Windows::Foundation::Collections::IMapView<winrt::hstring, int64_t>* named_dimension_overrides) {
  API_IMPL_BEGIN
  auto inference_session = reinterpret_cast<::onnxruntime::InferenceSession*>(session);
  auto session_options = inference_session->GetSessionOptions();
  winrt::Windows::Foundation::Collections::IMap<winrt::hstring, int64_t> override_map = winrt::single_threaded_map<winrt::hstring, int64_t>();
  for (auto freeDimOverride : session_options.free_dimension_overrides)
  {
    ...
    {
      override_map.Insert(winrt::to_hstring(freeDimOverride.dim_identifier), freeDimOverride.dim_value);
    }
  }
