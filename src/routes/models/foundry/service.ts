...
export class FoundryModelService {
	...
	private async fetchModelsForMultipleDevices(devices: string[], filters: ApiFilters, sortOptions: ApiSortOptions): Promise<FoundryModel[]> {
		...
		for (const device of devices) {
			try {
				...
				console.warn(`Failed to fetch models for device ${device}:`, error);
	...
	private async fetchModelsForDevice(filters: ApiFilters, sortOptions: ApiSortOptions): Promise<FoundryModel[]> {
		...
		try {
			console.log('Making request to proxy API with body:', JSON.stringify(requestBody, null, 2));
			...
			if (apiData.indexEntitiesResponse) {
				console.log('indexEntitiesResponse keys:', Object.keys(apiData.indexEntitiesResponse));
				console.log('indexEntitiesResponse sample:', JSON.stringify(apiData.indexEntitiesResponse, null, 2).substring(0, 500) + '...');
			}
			
			const models = this.transformApiResponse(apiData);
			console.log(`Transformed ${models.length} models successfully`);
	...
	private buildRequestBody(filters: ApiFilters) {
		...
		if (filters.taskType) {
			baseFilters.push({
				field: "properties/taskType",
				operator: "eq",
				values: [filters.taskType]
			});
		}
	...
	private transformApiResponse(apiData: any): FoundryModel[] {
		...
		// Fallback: check other possible structures
			...
			console.log('Found entities in indexEntitiesResponse.entities:', entities.length);
		} 
			...
			console.log('Found entities in indexEntitiesResponse.items:', entities.length);
		}
			...
			console.log('Found entities in indexEntitiesResponse.results:', entities.length);
		}
		
		console.log(`Found ${entities.length} entities to transform`);
		console.log('First entity sample:', entities[0]);
		
		if (entities.length === 0) {
			console.warn('No entities found in API response');
			console.warn('Available keys in response:', Object.keys(apiData));
			if (apiData?.indexEntitiesResponse) {
				console.warn('Available keys in indexEntitiesResponse:', Object.keys(apiData.indexEntitiesResponse));
			}
			return [];
		}
	...
	private transformSingleModel(entity: any): FoundryModel {
		console.log('Transforming entity:', JSON.stringify(entity, null, 2).substring(0, 800) + '...');
	...
	async fetchGroupedModels(filters: ApiFilters = {}, sortOptions: ApiSortOptions = { sortBy: 'name', sortOrder: 'asc' }): Promise<GroupedFoundryModel[]> {
		...
		console.log(`Grouped ${allModels.length} individual models into ${groupedModels.length} groups`);
