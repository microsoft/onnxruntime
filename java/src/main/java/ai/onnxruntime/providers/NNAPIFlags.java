...
public enum NNAPIFlags implements OrtFlags {
  ...
  /** Uses channels first format. 
      Only recommended for developer usage to validate code changes to the execution provider implementation.
  */
  USE_NCHW(2), // NNAPI_FLAG_USE_NCHW(0x002)
  /** Disables NNAPI from using CPU. 
      If an operator could be assigned to NNAPI, but NNAPI only has a CPU implementation of that operator 
      on the current device, model load will fail. */
