...
export interface OrtWasmModule extends EmscriptenModule {
    ...
    // #region ORT WASM Traning
    _OrtLoadCheckpointForTraining(dataOffset: number, dataLength: number): number;
    _OrtCreateTrainingSession(sessionOptionsHandle: number, trainModelOffset: number,
        trainModelLength: number, evalModelOffset: number, evalModelLength: number, optimizerModelOffset: number,
        optimizerModelLength: number): number;

    _OrtRunTrainStep(trainingSessionHandle: number, inputsOffset: number, inputCount: number,
        outputsOffset: number, outputCount: number, runOptionsHandle: number): number;

    _OrtReleaseTrainingSession(trainingSessionHandle: number): void;

    _OrtReleaseTrainingCheckpoint(checkpointStateHandle: number): void;
    // #endregion
}
