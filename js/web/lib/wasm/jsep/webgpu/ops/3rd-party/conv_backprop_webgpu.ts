...
const createConvTranspose2DOpProgramShaderSource =
  ...
  fn setOutputAtIndex(flatIndex : i32, value : ${isVec4 ? 'vec4<f32>' : 'f32'}) {
    ...
    fn getBiasByOutputCoords(coords : vec4<i32>) -> ${isVec4 ? 'vec4<f32>' : 'f32'} {
      ...
      const codeSnippet4 = `{
        let batch: i32 = global_id.z / outShape[1];
        let r = i32(global_id.z) % outShape[1];
        let c = i32(global_id.y) * ${workPerThread};
        let d1 = i32(global_id.x) * 4;

        let dyCorner = vec2<i32>(r, c) - vec2<i32>(pads);
        ...
        for (var wR: i32 = 0; wR < filterDims[0]; wR = wR + 1) {
          var dyR = f32(dyCorner.x + wR) / f32(strides.x);
          let wRPerm: i32= filterDims[0] - 1 - wR;
          if (dyR < 0.0 || dyR >= f32(outBackprop[1]) ||
              fract(dyR) > 0.0) {
            continue;
          }
          let idyR: i32 = i32(dyR);

          for (var wC: i32 = 0; wC < filterDims[1]; wC = wC + 1) {
            let dyC = f32(dyCorner.y + wC) / f32(strides.y);
            let dyC2 = f32(dyCorner.y + 1 + wC) / f32(strides.y);
            let wCPerm: i32 = filterDims[1] - 1 - wC;
            var bDyCVal = true;
            var bDyCVal2 = true;
            ...
            let idyC: i32 = i32(dyC);
            let idyC2: i32 = i32(dyC2);
            if (bDyCVal && bDyCVal2) {
              let d2Length = outBackprop[3];
              for (var d2 :i32 = 0; d2 < d2Length; d2 = d2 + 4) {
        ...
        for (var i: i32 = 0; i < ${workPerThread}; i = i + 1) {
      ...
      const codeSnippet = `
          ...
          // ? = to be determined. : = across all values in that axis.
          var dotProd = 0.0;
          for (var wR: i32 = 0; wR < effectiveFilterDims.x; wR = wR + 1) {
            if (wR % dilations.x != 0) {
              continue;
            }
            ...
            for (var wC: i32 = 0; wC < effectiveFilterDims.y; wC = wC + 1) {
              if (wC % dilations.y != 0) {
                continue;
              }
              ...
              for (var d2: i32 = 0; d2 < outBackprop[3]; d2 = d2 + 1) {
                let xValue = ${
          isChannelsLast ? dy.get('batch', 'idyR', 'idyC', 'd2') : dy.get('batch', 'd2', 'idyR', 'idyC')};
  ...
  const dilations : vec2<i32> = vec2<i32>(${attributes.dilations[0]}, ${attributes.dilations[1]});
