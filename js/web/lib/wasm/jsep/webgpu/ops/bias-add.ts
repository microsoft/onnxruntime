...
const createBiasSplitGeluProgramInfo =
    (metadata: ProgramMetadata, inputs: readonly TensorView[]):
        ...
        const executionCount = gridSize * 320;

        const getShaderSource = (shaderHelper: ShaderHelper) => `
  const TPB = 320; // threads per block, copied from CUDA kernel
    ...
    for (var h: u32 = 0u; h < ${blockSize}; h++) {
      ...
      biasOffset += TPB;
        ...
        return {
            ...
            getShaderSource,
            dispatchGroup: () => ({x: Math.ceil(executionCount / 64 /* workgroup size */)})
        };
    };
