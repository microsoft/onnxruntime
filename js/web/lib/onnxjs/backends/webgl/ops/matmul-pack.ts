...
//     const getSamplerAInLoopSnippet = isBroadcast ? 'getAAtOutCoordsMatmul(i)' : `getA(${getA(allGlChannels,
//     aRank)})`; const getSamplerBInLoopSnippet = isBroadcast ? 'getBAtOutCoordsMatmul(i)' :
//     `getB(${getB(allGlChannels, bRank)})`; const getOutputCoordsSnippet = isBroadcast ? '' : `${coordsDataType} rc =
...
//     const inputTDs =
//         inputs.map((t) => handler.getOrCreateTextureData(t, handler.getOrCreateTextureLayout(t, 1, false, [],
//         true)));
//     return {
...
//     coordsDataType: string, allGlChannels: readonly string[], inputs: Tensor[], outShape: readonly number[]):
//     string {
