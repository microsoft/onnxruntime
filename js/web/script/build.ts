...
const WASM_BINDING_THREADED_SIMD_JSEP_JS_PATH = path.join(WASM_BINDING_FOLDER, 'ort-wasm-threaded-simd.jsep.js');
...
const WASM_BINDING_THREADED_SIMD_JSEP_MIN_JS_PATH =
    path.join(WASM_BINDING_FOLDER, 'ort-wasm-threaded-simd.jsep.min.js');
...
const WASM_THREADED_SIMD_JSEP_JS_PATH = path.join(WASM_DIST_FOLDER, 'ort-wasm-threaded-simd.jsep.js');
...
`;
  ...
  npmlog.info('Build', 'Minimizing file "ort-wasm-threaded-simd.jsep.js"...');
  try {
    const terser = spawnSync(
        'npx',
        [
          'terser', WASM_BINDING_THREADED_SIMD_JSEP_JS_PATH, '--compress', 'passes=2', '--format', 'comments=false',
    ...
    fs.writeFileSync(WASM_BINDING_THREADED_SIMD_JSEP_MIN_JS_PATH, terser.stdout);
    fs.writeFileSync(WASM_THREADED_SIMD_JSEP_JS_PATH, `${COPYRIGHT_BANNER}${terser.stdout}`);

    validateFile(WASM_BINDING_THREADED_SIMD_JSEP_MIN_JS_PATH);
    validateFile(WASM_THREADED_SIMD_JSEP_JS_PATH);
  } catch (e) {
    npmlog.error('Build', `Failed to run terser on ort-wasm-threaded.js. ERR: ${e}`);
    throw e;
  }
  npmlog.info('Build', 'Minimizing file "ort-wasm-threaded-simd.jsep.js"... DONE');
