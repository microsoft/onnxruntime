...
+ (Ort::Value)createInputTensor:(NSDictionary*)input ortAllocator:(OrtAllocator*)ortAllocator
  ...
  if (tensorType == ONNX_TENSOR_ELEMENT_DATA_TYPE_STRING) {
    ...
    auto inputTensor = Ort::Value::CreateTensor(ortAllocator, dims.data(), dims.size(), ONNX_TENSOR_ELEMENT_DATA_TYPE_STRING);
    size_t index = 0;
    ...
    Ort::Value inputTensor = [self createInputTensor:tensorType dims:dims buffer:buffer ortAllocator:ortAllocator allocations:allocatons];
    return inputTensor;
  }
}
...
template <typename T>
static Ort::Value createInputTensorT(OrtAllocator* ortAllocator,
...
template <typename T>
static NSString* createOutputTensorT(const Ort::Value& tensor) {
  const auto data = tensor.GetTensorData<T>();
  NSData* buffer = [NSData dataWithBytesNoCopy:(void*)data length:tensor.GetTensorTypeAndShapeInfo().GetElementCount() * sizeof(T) freeWhenDone:false];
