...
# Compilation and packaging are kept separate. onnxruntime_providers_obj always holds the objects;
# the onnxruntime_providers archive is only materialized where something needs an archive file.
#
# With onnxruntime_ENABLE_LTO on MSVC the objects carry whole-program IR, which pushes that archive
# past the 4 GiB limit lib.exe enforces on the COFF archive format (LNK1248). No lib.exe option
# raises the limit, and a shared-library build only needs the objects for the final link, so the
# archive is skipped there.
#
# Consumers link ${onnxruntime_providers_target} rather than either name directly, so the choice
# lives here alone. Two of them need the archive specifically rather than the objects: the Apple
# static-framework prelink in onnxruntime.cmake, and bundle_static_library() in
# onnxruntime_webassembly.cmake, which collects only dependencies whose type is STATIC_LIBRARY and
# would otherwise drop the providers silently. Neither Apple nor Emscripten builds set MSVC, so
# widening the condition below past MSVC would break them.
onnxruntime_add_object_library(onnxruntime_providers_obj ${onnxruntime_providers_src})
if (onnxruntime_REDUCED_OPS_BUILD)
  add_op_reduction_include_dirs(onnxruntime_providers_obj)
endif()

if (HAS_BITWISE_INSTEAD_OF_LOGICAL)
  target_compile_options(onnxruntime_providers_obj PRIVATE "-Wno-bitwise-instead-of-logical")
endif()

if (MSVC)
   target_compile_options(onnxruntime_providers_obj PRIVATE "/bigobj")
#   if(NOT CMAKE_SIZEOF_VOID_P EQUAL 8)
#      target_compile_options(onnxruntime_providers_obj PRIVATE "/wd4244")
#   endif()
endif()
onnxruntime_add_include_to_target(onnxruntime_providers_obj onnxruntime_common onnxruntime_framework onnx onnx_proto ${PROTOBUF_LIB} flatbuffers::flatbuffers Boost::mp11 safeint_interface)

if (onnxruntime_BUILD_MS_EXPERIMENTAL_OPS)
  target_compile_definitions(onnxruntime_providers_obj PRIVATE BUILD_MS_EXPERIMENTAL_OPS=1)
endif()
...
target_include_directories(onnxruntime_providers_obj PRIVATE ${ONNXRUNTIME_ROOT})
onnxruntime_add_include_to_target(onnxruntime_providers_obj re2::re2 Eigen3::Eigen)
add_dependencies(onnxruntime_providers_obj onnx ${onnxruntime_EXTERNAL_DEPENDENCIES})

if (onnxruntime_ENABLE_TRAINING_OPS)
  target_include_directories(onnxruntime_providers_obj PRIVATE ${ORTTRAINING_ROOT})
endif()

if (onnxruntime_ENABLE_ATEN)
  target_compile_definitions(onnxruntime_providers_obj PRIVATE ENABLE_ATEN)
endif()

if (onnxruntime_ENABLE_DLPACK)
  target_compile_definitions(onnxruntime_providers_obj PRIVATE ENABLE_DLPACK)
  # DLPack is a header-only dependency
  onnxruntime_add_include_to_target(onnxruntime_providers_obj dlpack::dlpack)
endif()

if (onnxruntime_ENABLE_TRAINING)
  add_dependencies(onnxruntime_providers_obj tensorboard)
  onnxruntime_add_include_to_target(onnxruntime_providers_obj tensorboard)
  if (onnxruntime_ENABLE_TRAINING_TORCH_INTEROP OR onnxruntime_ENABLE_TRITON)
    onnxruntime_add_include_to_target(onnxruntime_providers_obj Python::Module)
  endif()

  if (onnxruntime_USE_NCCL)
    target_include_directories(onnxruntime_providers_obj PUBLIC ${MPI_CXX_INCLUDE_DIRS})
  endif()
endif()
...
set_target_properties(onnxruntime_providers_obj PROPERTIES FOLDER "ONNXRuntime")
...
if (MSVC AND onnxruntime_BUILD_SHARED_LIB)
  ...
  if (onnxruntime_ENABLE_TRAINING AND onnxruntime_USE_NCCL)
