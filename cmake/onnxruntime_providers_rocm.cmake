...
# Licensed under the MIT License.
  ...
  if(NOT onnxruntime_DISABLE_CONTRIB_OPS)
    ...
    # Hand-written ROCm contrib files (not produced by hipify — excluded or ROCm-specific).
    # Exclude CK-dependent files (require composable_kernel) and fp8/float8 ops (not on gfx900).
    file(GLOB_RECURSE onnxruntime_rocm_contrib_ops_hand_cc CONFIGURE_DEPENDS
      "${ONNXRUNTIME_ROOT}/contrib_ops/rocm/*.cc"
      "${ONNXRUNTIME_ROOT}/contrib_ops/rocm/*.h"
    )
    file(GLOB_RECURSE onnxruntime_rocm_contrib_ops_hand_cu CONFIGURE_DEPENDS
      "${ONNXRUNTIME_ROOT}/contrib_ops/rocm/*.cu"
      "${ONNXRUNTIME_ROOT}/contrib_ops/rocm/*.cuh"
    )
    # Remove files that require composable_kernel (CK) or float8 — not available on gfx900
    # and not needed for base ROCm EP functionality.
    list(FILTER onnxruntime_rocm_contrib_ops_hand_cc EXCLUDE REGEX ".*(_ck_impl|gemm_float8|nhwc_conv|fused_conv|group_query_attention|multihead_attention).*")
    list(FILTER onnxruntime_rocm_contrib_ops_hand_cu EXCLUDE REGEX ".*(_ck_impl|gemm_float8|nhwc_conv|fused_conv|group_query_attention|multihead_attention).*")
    ...
    list(APPEND onnxruntime_providers_rocm_src ${onnxruntime_rocm_contrib_ops_hand_cc} ${onnxruntime_rocm_contrib_ops_hand_cu})
  ...
  if(NOT MSVC)
    ...
    # HIP clang warns when #pragma unroll cannot satisfy the requested unroll count
    target_compile_options(onnxruntime_providers_rocm PRIVATE -Wno-pass-failed)
  endif()
  ...
  target_include_directories(onnxruntime_providers_rocm SYSTEM
    PRIVATE
      ${ONNXRUNTIME_ROOT}
      ${ONNXRUNTIME_ROOT}/core/providers/rocm
  ...
  # Float4 types (Float4E2M1x2) use NVIDIA-specific intrinsics not available on ROCm.
  # DISABLE_FLOAT4_TYPES is defined on this target to suppress Float4 kernels/ops.
  # It MUST also be defined on onnxruntime_session (see onnxruntime_session.cmake) so that
  # provider_bridge_ort.cc and provider_bridge_provider.cc both compile provider_interfaces.h
  # with identical virtual function counts, keeping the ProviderHost vtable in sync.
  target_compile_definitions(onnxruntime_providers_rocm PRIVATE DISABLE_FLOAT4_TYPES)
