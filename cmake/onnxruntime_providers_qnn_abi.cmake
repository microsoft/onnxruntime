...
# QNN EP Shared Library Build (ABI-compatible)
# This file contains ONLY shared library build configuration
# NEVER include static library headers or types to avoid redefinition conflicts

add_compile_definitions(USE_QNN=1)

# STRICT: Force shared library build mode only
if(onnxruntime_BUILD_QNN_EP_STATIC_LIB)
  message(FATAL_ERROR "STRICT ENFORCEMENT: onnxruntime_providers_qnn_abi.cmake is EXCLUSIVELY for shared library builds. Use onnxruntime_providers_qnn_static.cmake for static builds.")
endif()

# Explicitly check that we're not accidentally in static mode
if(DEFINED BUILD_QNN_EP_STATIC_LIB AND BUILD_QNN_EP_STATIC_LIB)
  message(FATAL_ERROR "STRICT ENFORCEMENT: qnn-abi/ files detected BUILD_QNN_EP_STATIC_LIB=1. This violates shared library only design.")
endif()

# Ensure BUILD_QNN_EP_STATIC_LIB is explicitly NOT defined
# This enables the IsOpSupportedForABI method in BaseOpBuilder
...
# Collect QNN-ABI specific source files
file(GLOB_RECURSE
...
message(STATUS "QNN EP ABI: Found ${num_sources} source files")
if(num_sources GREATER 0)
  list(GET onnxruntime_providers_qnn_ep_srcs 0 first_source)
  message(STATUS "QNN EP ABI: First source file: ${first_source}")
endif()
...
# STRICT ENFORCEMENT: Explicitly ensure pure shared library mode
# This enables IsOpSupportedForABI method compilation in BaseOpBuilder
target_compile_definitions(onnxruntime_providers_qnn PRIVATE
    BUILD_QNN_EP_STATIC_LIB=0
    QNN_ABI_SHARED_LIBRARY_ONLY=1
    SHARED_PROVIDER=1
)

# No special compilation needed - pure shared library build
