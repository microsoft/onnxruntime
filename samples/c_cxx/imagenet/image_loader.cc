...
InceptionPreprocessing::InceptionPreprocessing(int out_height, int out_width, int channels)
  ...
  if (!SUCCEEDED(hr)) throw std::runtime_error("create COM intance of WICImagingFactory failed");
#endif
}
...
void InceptionPreprocessing::operator()(const void* input_data, void* output_data) const {
  ...
  HRESULT hr =
      piFactory->CreateDecoderFromFilename(file_name.c_str(), NULL, GENERIC_READ,
                                           WICDecodeMetadataCacheOnLoad,  // For JPEG lossless decoding/encoding.
                                           &piDecoder);

  if (!SUCCEEDED(hr)) throw std::runtime_error("decode image failed");
  UINT count = 0;
  ...
  hr = piFactory->CreateFormatConverter(&ppIFormatConverter);
  if (!SUCCEEDED(hr)) throw std::runtime_error("decode image failed");
  hr = ppIFormatConverter->Initialize(piFrameDecode,                // Source frame to convert
  ...
  hr = ppIFormatConverter->CopyPixels(&rect, stride, static_cast<UINT>(data.size()), data.data());
  if (!SUCCEEDED(hr)) throw std::runtime_error("decode image failed");
  std::vector<float> float_file_data(data.size());
  ...
  ResizeImageInMemory(float_file_data.data(), output_data_, bbox_h_size, bbox_w_size, out_height_, out_width_,
                      channels);
