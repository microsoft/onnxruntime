...
class OnnxRuntimeException : public std::exception {
  ...
  */
  ...
  OnnxRuntimeException(const std::string& msg) noexcept
      : OnnxRuntimeException(nullptr, msg) {
  }

  OnnxRuntimeException(const char* failed_condition, const std::string& msg) {
    std::ostringstream ss;
