...
void TestCompileBasedEp(const OrtApi* g_ort, OrtEnv* env, OrtSessionOptions* so) {
    THROW_ON_ERROR(g_ort->RegisterOrtExecutionProviderLibrary("../outTreeEp/build/liboutTreeEp.so", env, "outTreeEp"));
...
void TestKernelBasedEp(const OrtApi* g_ort, OrtEnv* env, OrtSessionOptions* so) {
    THROW_ON_ERROR(g_ort->RegisterOrtExecutionProviderLibrary("../outTreeEp_kernel/build/libkernelEp.so", env, "kernelEp"));
    std::vector<const char*> keys{"int_property", "str_property"}, values{"3", "strvalue"};
...
void TestTensorRTEp(const OrtApi* g_ort, OrtEnv* env, OrtSessionOptions* so) {
    THROW_ON_ERROR(g_ort->RegisterOrtExecutionProviderLibrary("../tensorRTEp/build/libTensorRTEp.so", env, "tensorrtEp"));
