...
# 2. build test module
cd outTreeEp && mkdir -p build && cd build
cmake -S ../ -B ./ -DCMAKE_BUILD_TYPE=Debug
cmake --build ./
cd ../../

cd outTreeEp_kernel && mkdir -p build && cd build
cmake -S ../ -B ./ -DCMAKE_BUILD_TYPE=Debug
cmake --build ./
cd ../../

cd tensorRTEp && mkdir -p build && cd build
# Download TensorRT with correct cuda version selected
cmake -S ../ -B ./ -DCMAKE_BUILD_TYPE=Debug -DCMAKE_CUDA_ARCHITECTURES=80 -DCMAKE_CUDA_COMPILER=/usr/local/cuda/bin/nvcc -DTENSORRT_HOME=/home/yifanl/TensorRT-10.4.0.26/
cmake --build ./
cd ../../

cp ../build/Linux/Debug/libonnxruntime.so c_test
cd c_test && mkdir -p build && cd build
cmake -S ../ -B ./ -DCMAKE_BUILD_TYPE=Debug
cmake --build ./
cd ../../
