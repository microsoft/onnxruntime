...
BasicBackend::BasicBackend(void* model_proto,
...
//        LOGS_DEFAULT(INFO) << log_tag << "IO Buffering Enabled";
        ...
        ie_cnn_network_ = CreateOVModel(model_proto, model_proto_len, global_context_, subgraph_context_, const_outputs_map_);
      ...
      if (is_ep_ctx_graph_) {
        ...
        ie_cnn_network_ = CreateOVModel(model_proto, model_proto_len, global_context_, const_outputs_map_);
        exe_network_ = global_context_.ie_core.CompileModel(
            ie_cnn_network_, hw_target, device_config, subgraph_context_.subgraph_name);
      }
#endif
...
void BasicBackend::EnableStreams() {
  ...
  if ((global_context_.device_type.find("MULTI") != std::string::npos) ||
    ...
    if (global_context_.num_streams != 1) {
      throw std::runtime_error(log_tag + "Cannot set NUM_STREAMS to " +
  ...
  try {
    ...
    for (auto input_info_iter = graph_input_info.begin();
      ...
      if (input_names.find(onnx_input_name) != input_names.end()) {
        ...
        throw std::runtime_error(log_tag +
  ...
  try {
    ...
    if (!const_outputs_map_.empty()) {
      for (const auto& item : const_outputs_map_) {
        ...
        auto mem_info = output_tensor.GetTensorMemoryInfo();
        if (mem_info.GetAllocatorName() == OpenVINO_GPU) {
          throw std::runtime_error(log_tag + "IO Buffering is not supported for constant subgraphs");
        } else {
...
//    LOGS_DEFAULT(INFO) << log_tag << "Inference successful";
    ...
    try {
      ...
      throw std::runtime_error(log_tag + " Exception at StartAsyncInference: " + e.what());
    }
#endif
    try {
      CompleteAsyncInference(context, infer_request);
    } catch (const std::runtime_error& e) {
      throw std::runtime_error(log_tag + " Exception at CompleteAsyncInference: " + e.what());
    }
