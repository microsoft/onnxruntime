...
GetOutputTensor(Ort::KernelContext& context, size_t batch_size,
  ...
  if (it == output_names.end()) {
    throw std::runtime_error(log_tag + "Output names mismatch between OpenVINO and ONNX");
  }
  int index = it->second;
...
void FillOutputsWithConstantData(std::shared_ptr<ov::Node> node, Ort::UnownedValue& out_tensor) {
  switch (node->get_element_type()) {
    ...
    default:
      throw std::runtime_error(log_tag + "Unsupported output data type");
  }
}
...
void FillInputBlob(OVTensorPtr inputBlob, size_t batch_slice_idx,
  ...
  if (mem_info.GetAllocatorName() == OpenVINO_GPU) {
    throw std::runtime_error(log_tag + "IO Buffering is not enabled, Please enable Input on CPU");
  }
