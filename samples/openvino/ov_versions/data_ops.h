...
using VarianceFunc = std::function<bool(const OrtNode*)>;
...
// protected:
  ...
  bool dimension_unsupported(const OrtNode* node);
  bool unsupported_op_mode(const OrtNode* node);
  bool type_is_supported(ONNXTensorElementDataType dtype, bool is_initializer);
  bool node_is_supported(const NodeIndex node_idx);

 public:
  DataOps(const OrtGraphViewer* graph_viewer_param, VersionNum ver,
  ...
  virtual bool SpecialConditionForClusterSizeOne(
      std::unordered_set<std::string>& ng_required_initializers, const OrtNode* node);
