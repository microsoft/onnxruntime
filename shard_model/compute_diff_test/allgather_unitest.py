...
def generate_allgather_model(dtype_str, world_size, data_shape):
    rank_shape = list(data_shape)
    assert rank_shape[0] % world_size == 0
    rank_shape[0] = rank_shape[0] // world_size
    # generate input data
    for i in range(world_size):
        data = np.random.rand(*rank_shape).astype(dtype_str)
        with open(INPUT_DATA_FILE.format(i), 'wb') as fp:
            pickle.dump(data, fp)

    onnx_type = numpy_type_to_onnx_type(dtype_str)
    # genrate single model
    A = helper.make_tensor_value_info('A', onnx_type, rank_shape)
    Y = helper.make_tensor_value_info('Y', onnx_type, data_shape)

    coll_node = coll('NcclAllGatherV2', 'A', 'Y', group_type=0, world_size=world_size)
    origin_model = generate_model([coll_node], [A], [Y], None, 'origin-allgather')
...
def main(args):
    ...
    if args.generate_model:
        origin_model_file=generate_allgather_model(args.type, args.size, data_shape)
    ...
    need_y = np.concatenate(input_datas)
